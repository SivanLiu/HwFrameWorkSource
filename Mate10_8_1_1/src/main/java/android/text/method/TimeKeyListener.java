...
public class TimeKeyListener extends NumberKeyListener {
    ...
    public TimeKeyListener(Locale locale) {
        boolean addFormatCharsFromSkeleton;
        ...
        if (NumberKeyListener.addDigits(chars, locale) && NumberKeyListener.addAmPmChars(chars, locale) && NumberKeyListener.addFormatCharsFromSkeleton(chars, locale, SKELETON_12HOUR, SYMBOLS_TO_IGNORE)) {
            addFormatCharsFromSkeleton = NumberKeyListener.addFormatCharsFromSkeleton(chars, locale, SKELETON_24HOUR, SYMBOLS_TO_IGNORE);
        } else {
            addFormatCharsFromSkeleton = false;
        }
        if (addFormatCharsFromSkeleton) {

...
public class DateKeyListener extends NumberKeyListener {
    ...
    public DateKeyListener(Locale locale) {
        boolean addFormatCharsFromSkeletons;
        LinkedHashSet<Character> chars = new LinkedHashSet();
        if (NumberKeyListener.addDigits(chars, locale)) {
            addFormatCharsFromSkeletons = NumberKeyListener.addFormatCharsFromSkeletons(chars, locale, SKELETONS, SYMBOLS_TO_IGNORE);
        } else {
            addFormatCharsFromSkeletons = false;
        }
        if (addFormatCharsFromSkeletons) {

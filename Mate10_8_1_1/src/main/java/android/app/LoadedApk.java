...
public final class LoadedApk {
    ...
    static final class ReceiverDispatcher {
        ...
        final class Args extends PendingResult {
            ...
            public final Runnable getRunnable() {
                return new -$Lambda$aS31cHIhRx41653CMnd4gZqshIQ((byte) 7, this);
            }

            /* synthetic */ void lambda$-android_app_LoadedApk$ReceiverDispatcher$Args_58970() {
                BroadcastReceiver receiver = ReceiverDispatcher.this.mReceiver;
                boolean ordered = this.mOrdered;
                if (ActivityThread.DEBUG_BROADCAST) {
                    Slog.i(ActivityThread.TAG, "Dispatching broadcast " + this.mCurIntent.getAction() + " seq=" + this.mCurIntent.getIntExtra(BatteryManager.EXTRA_SEQUENCE, -1) + " to " + ReceiverDispatcher.this.mReceiver);
        ...
        static final class InnerReceiver extends Stub {
            ...
            public void performReceive(Intent intent, int resultCode, String data, Bundle extras, boolean ordered, boolean sticky, int sendingUser) {
                ...
                if (ActivityThread.DEBUG_BROADCAST) {
                    Slog.i(ActivityThread.TAG, "Receiving broadcast " + intent.getAction() + " seq=" + intent.getIntExtra(BatteryManager.EXTRA_SEQUENCE, -1) + " to " + (receiverDispatcher != null ? receiverDispatcher.mReceiver : null));
                }
        ...
        public void performReceive(Intent intent, int resultCode, String data, Bundle extras, boolean ordered, boolean sticky, int sendingUser) {
            ...
            if (intent == null) {
                ...
                Slog.i(ActivityThread.TAG, "Enqueueing broadcast " + intent.getAction() + " seq=" + intent.getIntExtra(BatteryManager.EXTRA_SEQUENCE, -1) + " to " + this.mReceiver);
            }
            if (Log.HWINFO) {
    ...
    private void createOrUpdateClassLoaderLocked(List<String> addedPaths) {
        if (!this.mPackageName.equals("android")) {
            ...
            boolean treatVendorApkAsUnbundled = defaultSearchPaths.contains("/vendor/lib") ^ 1;
            if (this.mApplicationInfo.getCodePath() != null && this.mApplicationInfo.getCodePath().startsWith("/vendor/") && treatVendorApkAsUnbundled) {
                isBundledApp = false;
            }
            makePaths(this.mActivityThread, isBundledApp, this.mApplicationInfo, arrayList, libPaths);
            ...
            if (this.mIncludeCode) {
                String zip;
                if (arrayList.size() == 1) {
                    zip = (String) arrayList.get(0);
                } else {
                    zip = TextUtils.join(File.pathSeparator, arrayList);
                }
                boolean needToSetupJitProfiles = false;
                if (this.mClassLoader == null) {
                    oldPolicy = StrictMode.allowThreadDiskReads();
                    this.mClassLoader = ApplicationLoaders.getDefault().getClassLoader(zip, this.mApplicationInfo.targetSdkVersion, isBundledApp, librarySearchPath, libraryPermittedPath, this.mBaseClassLoader, this.mApplicationInfo.classLoaderName);
            ...
            if (this.mClassLoader == null) {
                oldPolicy = StrictMode.allowThreadDiskReads();
                this.mClassLoader = ApplicationLoaders.getDefault().getClassLoader("", this.mApplicationInfo.targetSdkVersion, isBundledApp, librarySearchPath, libraryPermittedPath, this.mBaseClassLoader, null);
                StrictMode.setThreadPolicy(oldPolicy);
            }
    ...
    private void initializeJavaContextClassLoader() {
        try {
            ...
            if (pi.applicationInfo != null) {
                equals = this.mPackageName.equals(pi.applicationInfo.processName) ^ 1;
            } else {
                equals = false;
            }
            if (!sharedUserIdSet ? equals : true) {

...
public class SubscriptionManager {
    ...
    public int setDisplayName(String displayName, int subId, long nameSource) {
        logd("[setDisplayName]+  displayName:" + displayName + " subId:" + subId + " nameSource:" + nameSource + " callingPackage:" + (this.mContext != null ? this.mContext.getOpPackageName() : "") + " callingPid:" + Binder.getCallingPid() + " callingUid:" + Binder.getCallingUid());
        if (isValidSubscriptionId(subId)) {
            int result = 0;
            try {
                ISub iSub = ISub.Stub.asInterface(ServiceManager.getService("isub"));

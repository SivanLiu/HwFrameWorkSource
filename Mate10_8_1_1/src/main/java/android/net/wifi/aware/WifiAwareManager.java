...
public class WifiAwareManager {
    ...
    public NetworkSpecifier createNetworkSpecifier(int clientId, int role, int sessionId, PeerHandle peerHandle, byte[] pmk, String passphrase) {
        if (role != 0 && role != 1) {
            ...
            return new WifiAwareNetworkSpecifier(i, role, clientId, sessionId, peerHandle != null ? peerHandle.peerId : 0, null, pmk, passphrase, Process.myUid());
        }
    }
    ...
    public NetworkSpecifier createNetworkSpecifier(int clientId, int role, byte[] peer, byte[] pmk, String passphrase) {
        if (role != 0 && role != 1) {
            ...
            return new WifiAwareNetworkSpecifier(i, role, clientId, 0, 0, peer, pmk, passphrase, Process.myUid());
        } else {

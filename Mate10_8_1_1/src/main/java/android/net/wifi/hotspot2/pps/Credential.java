...
public final class Credential implements Parcelable {
    ...
    public boolean equals(Object thatObject) {
        ...
        if (TextUtils.equals(this.mRealm, that.mRealm) && this.mCreationTimeInMillis == that.mCreationTimeInMillis && this.mExpirationTimeInMillis == that.mExpirationTimeInMillis && this.mCheckAaaServerCertStatus == that.mCheckAaaServerCertStatus) {
            if (this.mUserCredential != null) {
                if (this.mUserCredential.equals(that.mUserCredential)) {
                }
            }
            if (this.mCertCredential != null) {
                if (this.mCertCredential.equals(that.mCertCredential)) {
                }
            }
            if (this.mSimCredential != null) {
                if (this.mSimCredential.equals(that.mSimCredential)) {
                }
            }
            if (isX509CertificateEquals(this.mCaCertificate, that.mCaCertificate) && isX509CertificatesEquals(this.mClientCertificateChain, that.mClientCertificateChain)) {
    ...
    private static boolean verifySha256Fingerprint(X509Certificate[] certChain, byte[] expectedFingerprint) throws NoSuchAlgorithmException, CertificateEncodingException {
        ...
        MessageDigest digester = MessageDigest.getInstance(EphemeralResolveInfo.SHA_ALGORITHM);

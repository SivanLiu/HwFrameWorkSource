...
*/
    ...
    public LruCache<String, DhcpResultsInfoRecord> getAllDhcpResultsInfo() {
        synchronized (this.mdbLock) {
            ...
            cursor = null;
            try {
                cursor = this.mDatabase.rawQuery("SELECT * FROM DhcpResults", null);
                if (cursor == null) {
                    ...
                    while (cursor.moveToNext()) {
                        DhcpResultsInfoRecord dbr = new DhcpResultsInfoRecord(cursor.getString(cursor.getColumnIndex("apSSID")), cursor.getString(cursor.getColumnIndex("IP")), cursor.getString(cursor.getColumnIndex("DHCPServer")));

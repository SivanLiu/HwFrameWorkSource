...
public class NetworkFactory extends Handler {
    ...
    private void evalRequest(NetworkRequestInfo n) {
        if (!n.requested && n.score < this.mScore && n.request.networkCapabilities.satisfiedByNetworkCapabilities(this.mCapabilityFilter) && acceptRequest(n.request, n.score)) {
            ...
            if (n.score <= this.mScore && n.request.networkCapabilities.satisfiedByNetworkCapabilities(this.mCapabilityFilter)) {
                if (acceptRequest(n.request, n.score)) {
                    return;
                }
            }
            releaseNetworkFor(n.request);
    ...
    protected void reevaluateAllRequests() {
        post(new -$Lambda$1viEGAj7AUZvOpShNdsx1w19_Xc(this));
    }

    /* synthetic */ void lambda$-android_net_NetworkFactory_10219() {
        evalRequests();
    }

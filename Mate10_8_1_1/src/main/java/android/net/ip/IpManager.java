...
public class IpManager extends StateMachine {
    ...
    IpManager(Context context, String ifName, Callback callback, INetworkManagementService nwService, INetd netd) {
        ...
        this.mNetlinkTracker = new NetlinkTracker(this.mInterfaceName, new com.android.server.net.NetlinkTracker.Callback() {
            ...
            /* synthetic */ void lambda$-android_net_ip_IpManager$2_27516(String msg) {
    ...
    /* synthetic */ void lambda$-android_net_ip_IpManager_27797() {
    ...
    private ProvisioningChange compareProvisioning(LinkProperties oldLp, LinkProperties newLp) {
        ...
        boolean wasProvisioned = isProvisioned(oldLp, initialConfiguration);
        boolean isProvisioned = isProvisioned(newLp, initialConfiguration);
        ...
        int isIPv6Provisioned = oldLp.isIPv6Provisioned() ? newLp.isIPv6Provisioned() ^ 1 : 0;
        int hasIPv4Address = oldLp.hasIPv4Address() ? newLp.hasIPv4Address() ^ 1 : 0;
        int hasIPv6DefaultRoute;
        if (oldLp.hasIPv6DefaultRoute()) {
            hasIPv6DefaultRoute = newLp.hasIPv6DefaultRoute() ^ 1;
        } else {
            hasIPv6DefaultRoute = 0;
        }
        boolean ignoreIPv6ProvisioningLoss = this.mMultinetworkPolicyTracker.getAvoidBadWifi() ^ 1;
        if (!(hasIPv4Address == 0 && (isIPv6Provisioned == 0 || (ignoreIPv6ProvisioningLoss ^ 1) == 0))) {
            delta = ProvisioningChange.LOST_PROVISIONING;
            Log.d("IpManager", "compareProvisioning: LOST_PROVISIONING because 2");
        }
        if (!(!oldLp.hasGlobalIPv6Address() || r6 == 0 || (ignoreIPv6ProvisioningLoss ^ 1) == 0)) {
            delta = ProvisioningChange.LOST_PROVISIONING;

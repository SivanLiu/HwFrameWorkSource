...
public final class HwPCUtils {
    ...
    public static boolean isPcDynamicStack(int stackId) {
        if (!enabled() || stackId < 10007 || stackId >= HwVRUtils.VR_DYNAMIC_STACK_ID) {
            return false;
        }
        return true;
    }
    ...
    public static boolean isExtDynamicStack(int stackId) {
        return !isPcDynamicStack(stackId) ? HwVRUtils.isVRDynamicStack(stackId) : true;
    }
    ...
    public static IHwPCManager getHwPCManager() {
        IHwPCManager service = null;
        try {
            HwPCManager hwPcManager = HwFrameworkFactory.getHwPCManager();
            if (hwPcManager != null) {
                service = hwPcManager.getService();
            }
        } catch (Exception e) {

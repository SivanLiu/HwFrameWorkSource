...
public class UserManager {
    ...
    public static boolean supportsMultipleUsers() {
        if (getMaxSupportedUsers() > 1) {
            return SystemProperties.getBoolean("fw.show_multiuserui", Resources.getSystem().getBoolean(R.bool.config_enableMultiUserUI));
        }
        return false;
    }
    ...
    public UserInfo createUser(String name, int flags) {
        try {
            ...
            if (!(user == null || (user.isAdmin() ^ 1) == 0 || (user.isDemo() ^ 1) == 0 || (user.isHwHiddenSpace() ^ 1) == 0)) {

...
public class HwViewRootImpl implements IHwViewRootImpl {
    ...
    public boolean filterDecorPointerEvent(Context context, MotionEvent event, int action, LayoutParams windowattr, Display disp) {
        boolean z = true;
        if (HwPCUtils.isValidExtDisplayId(context) || ((disp != null && HwPCUtils.isValidExtDisplayId(disp.getDisplayId())) || context == null || (isHwNaviBar ^ 1) != 0 || disp == null || windowattr == null || (windowattr.hwFlags & StubController.PERMISSION_WIFI) != 0 || !isNaviEnable(context) || SettingsHelper.isTouchPlusOn(context) || (windowattr.privateFlags & 1024) != 0)) {
            return false;
        }
        Point pt;
        if (action == 0) {
            ...
            this.mHitRegionToMax = (int) (((double) context.getResources().getDimensionPixelSize(17105147)) / 3.5d);
            if (pt.y > pt.x) {

...
final class TextClassifierImpl implements TextClassifier {
    ...
    private static final class LinksInfoFactory {
        ...
        private static List<SpanSpec> avoidOverlaps(List<SpanSpec> spans, String text) {
            Collections.sort(spans, Comparator.comparingInt(-$Lambda$Sy__B53nI_asuVbYEz1JE9PRAk8.$INST$0));
            Map<Integer, SpanSpec> reps = new LinkedHashMap();
            int size = spans.size();
            for (int i = 0; i < size; i++) {
                SpanSpec span = (SpanSpec) spans.get(i);
                SpanSpec rep = (SpanSpec) reps.get(Integer.valueOf(span.mStart));
        ...
        try {
            if (text.length() > 0) {
                ...
                if (start > end || start < 0 || end > string.length() || start > selectionStartIndex || end < selectionEndIndex) {
                    ...
                    Builder tsBuilder = new Builder(start, end);
                    ...
                    return tsBuilder.setLogSource("TextClassifierImpl").setVersionInfo(getVersionInfo()).build();
                }
            }
        } catch (Throwable t) {
    ...
    public TextClassifierConstants getSettings() {
        if (this.mSettings == null) {
            this.mSettings = TextClassifierConstants.loadFromString(Global.getString(this.mContext.getContentResolver(), Global.TEXT_CLASSIFIER_CONSTANTS));
        }
    ...
    private String getVersionInfo() {
        synchronized (this.mSmartSelectionLock) {
            if (this.mLocale != null) {
                String format = String.format("%s_v%d", new Object[]{this.mLocale.toLanguageTag(), Integer.valueOf(this.mVersion)});
                return format;
            }
            format = "";
            return format;
    ...
    private ParcelFileDescriptor getFdLocked(Locale locale) throws FileNotFoundException {
        ...
        try {
            open = ParcelFileDescriptor.open(new File(UPDATED_MODEL_FILE_PATH), 268435456);
        ...
        try {
            ...
            if (factoryModelFilePath != null) {
                open2 = ParcelFileDescriptor.open(new File(factoryModelFilePath), 268435456);
            } else {
                open2 = null;
            }
        ...
        if (open != null) {
            ...
            if (open2 == null) {
                ...
                int factoryVersion = SmartSelection.getVersion(open2.getFd());
                if (updateVersion > factoryVersion) {
                    closeAndLogError(open2);
    ...
    private TextClassification createClassificationResult(ClassificationResult[] classifications, CharSequence text) {
        ...
        if (intent != null) {
            ...
            resolveActivity = pm.resolveActivity(intent, 0);
        } else {
            pm = null;
        ...
        if (!(resolveActivity == null || resolveActivity.activityInfo == null)) {
            ...
            if (ZenModeConfig.SYSTEM_AUTHORITY.equals(packageName)) {
                ...
                intent.setComponent(new ComponentName(packageName, resolveActivity.activityInfo.name));
                Drawable icon = resolveActivity.activityInfo.loadIcon(pm);
                if (icon == null) {
                    icon = resolveActivity.loadIcon(pm);
                }
                builder.setIcon(icon);
                CharSequence label = resolveActivity.activityInfo.loadLabel(pm);
                if (label == null) {
                    label = resolveActivity.loadLabel(pm);
                }
                if (label != null) {
                    str = label.toString();
                }

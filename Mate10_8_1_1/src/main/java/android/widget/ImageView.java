...
public class ImageView extends View implements SRTaskCallback {
    ...
    private void initImageView() {
        ...
        if (HwAISRImageViewTaskManager.isSuperResolutionAvailable() && SRUtils.checkIsInSRWhiteList(getContext()) && (SRUtils.checkIsInSRBlackList(this) ^ 1) != 0) {
    ...
    private void updateDrawable(Drawable d) {
        ...
        if (d != null) {
            ...
            if (!sameDrawable || sCompatDrawableVisibilityDispatch) {
                boolean isShown = sCompatDrawableVisibilityDispatch ? getVisibility() == 0 : (isAttachedToWindow() && getWindowVisibility() == 0) ? isShown() : false;
                d.setVisible(isShown, true);
            }

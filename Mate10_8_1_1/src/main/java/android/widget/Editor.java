...
public class Editor {
    ...
    private class EasyEditPopupWindow extends PinnedPopupWindow implements OnClickListener {
        private static final int POPUP_TEXT_LAYOUT = 17367287;
        ...
        protected void initContentView() {
            ...
            this.mDeleteTextView = (TextView) inflater.inflate(17367287, null);
    ...
    private static class ErrorPopup extends PopupWindow {
        ...
        ErrorPopup(TextView v, int width, int height) {
            ...
            this.mView = v;
            this.mPopupInlineErrorBackgroundId = getResourceId(this.mPopupInlineErrorBackgroundId, 290);
        ...
        void fixDirection(boolean above) {
            this.mAbove = above;
            if (above) {
                this.mPopupInlineErrorAboveBackgroundId = getResourceId(this.mPopupInlineErrorAboveBackgroundId, 289);
            } else {
                this.mPopupInlineErrorBackgroundId = getResourceId(this.mPopupInlineErrorBackgroundId, 290);
            }
    ...
    protected class InsertionHandleView extends HandleView {
        ...
        public void show() {
            ...
            if (Editor.this.mInsertionActionModeRunnable != null) {
                if (!(Editor.this.mTapState == 2 || Editor.this.mTapState == 3)) {
                    if (Editor.this.isCursorInsideEasyCorrectionSpan()) {
                    }
                }
        ...
        protected int getCursorOffset() {
            ...
            return offset + (((drawable.getIntrinsicWidth() - Editor.this.mTempRect.left) - Editor.this.mTempRect.right) / 2);
        }
    ...
    private class PositionListener implements OnPreDrawListener {
        ...
        public boolean onPreDraw() {
            ...
            boolean isActive = (!hasSelection || selectionController == null) ? false : selectionController.isActive() ^ 1;
            int i = 0;
            while (i < 7) {
                if (this.mPositionHasChanged || this.mScrollHasChanged || this.mCanMove[i] || isActive) {
    ...
    protected class SelectionHandleView extends HandleView {
        ...
        protected void updatePosition(float x, float y, boolean fromTouchScreen) {
            ...
            if (!isStartHandle() || initialOffset < anotherHandleOffset) {
                ...
                wordEnd = Editor.this.getWordEnd(offset);
                wordStart = Editor.this.getWordStart(offset);
                if (this.mPrevX == -1.0f) {
                    this.mPrevX = x;
                }
                ...
                if (!isLvlBoundary || ((rtlAtCurrentOffset && (atRtl ^ 1) != 0) || (!rtlAtCurrentOffset && atRtl))) {
                    ...
                    if (isExpanding) {
                        ...
                        if (shrinking) {
                            ...
                            if (!isStartHandle() || adjustedOffset >= this.mPreviousOffset) {
                                if (!isStartHandle() && adjustedOffset > this.mPreviousOffset) {
                                }
                            }
    ...
    class SelectionModifierCursorController implements CursorController {
        ...
        private void updateSelectionInternal(int selectionStart, int selectionEnd, boolean fromTouchScreen) {
            boolean performHapticFeedback = (fromTouchScreen && Editor.this.mHapticTextHandleEnabled) ? Editor.this.mTextView.getSelectionStart() == selectionStart ? Editor.this.mTextView.getSelectionEnd() != selectionEnd : true : false;
    ...
    private class TextActionModeCallback extends Callback2 {
        ...
        private void updateReplaceItem(Menu menu) {
            boolean -wrap13 = Editor.this.mTextView.isSuggestionsEnabled() ? Editor.this.shouldOfferToShowSuggestions() : false;
            boolean replaceItemExists = menu.findItem(R.id.replaceText) != null;
            if (-wrap13 && (replaceItemExists ^ 1) != 0) {
                menu.add(0, (int) R.id.replaceText, 10, (int) R.string.replace).setShowAsAction(1);
            } else if (!-wrap13 && replaceItemExists) {
        ...
        private void updateAssistMenuItem(Menu menu) {
            ...
            if (canAssist()) {
                menu.add((int) R.id.textAssist, (int) R.id.textAssist, 2, textClassification.getLabel()).setIcon(textClassification.getIcon()).setShowAsAction(2);
        ...
        private boolean canAssist() {
            ...
            if (textClassification.getIcon() == null && (TextUtils.isEmpty(textClassification.getLabel()) ^ 1) == 0) {
                return false;
            }
            ...
            if (textClassification.getIntent() != null) {
                return Editor.this.mTextView.getContext().canStartActivityForResult();
            }
            return false;
        }
            ...
            if (layout instanceof DynamicLayout) {
                ...
                if (blockSet != null) {
                    for (i = 0; i < blockSet.size(); i++) {
                        ...
                        if (!(blockIndex == -1 || this.mTextRenderNodes[blockIndex] == null)) {
                            if (!this.mTextRenderNodes[blockIndex].needsToBeShifted) {
                            }
                        }
    ...
    void onTouchUpEvent(MotionEvent event) {
        if (!getSelectionActionModeHelper().resetSelection(getTextView().getOffsetForPosition(event.getX(), event.getY()))) {
            boolean didTouchFocusSelect = this.mSelectAllOnFocus ? this.mTextView.didTouchFocusSelect() : false;
            hideCursorAndSpanControllers();
            stopTextActionMode();
            CharSequence text = this.mTextView.getText();

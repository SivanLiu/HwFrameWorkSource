...
public class ScrollView extends FrameLayout {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (canScroll()) {
            ...
            if (!(nextFocused == null || nextFocused == this)) {
                z = nextFocused.requestFocus(130);
            }
            return z;
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (getScrollY() == 0 && (canScrollVertically(1) ^ 1) != 0) {
            return false;
        }
        int y;
        switch (action & 255) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            ...
            case 1:
                ...
                if (this.mIsHwTheme && ((isTopEdge() || isBottomEdge()) && (this.mScroller.isOverScrolled() ^ 1) != 0)) {
                    if (this.mScroller.springBack(this.mScrollX, this.mScrollY, 0, 0, 0, getScrollRange())) {
                        postInvalidateOnAnimation();
                    }
                    awakenScrollBars();
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        if (overScrollBy(0, deltaY, 0, this.mScrollY, 0, range, 0, this.mOverscrollDistance, true) && (hasNestedScrollingParent() ^ 1) != 0) {
                            this.mVelocityTracker.clear();
                        }
                        ...
                        if (!dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY - scrolledDeltaY, this.mScrollOffset)) {
                            if (canOverscroll) {
                                ...
                                if (!(this.mEdgeGlowTop == null || (this.mEdgeGlowTop.isFinished() && (this.mEdgeGlowBottom.isFinished() ^ 1) == 0))) {
                                    postInvalidateOnAnimation();
                                    break;
                                }
                            }
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {
            ...
            if (SMART_SLIDE_PROPERTIES) {
                this.mScroller.fling(this.mScrollX, this.mScrollY, 0, velocityY, 0, 0, 0, Math.max(0, bottom - height), DEFAULT_MULTIPLE_FLING_LENGTH_THRESHOLD);
            } else {

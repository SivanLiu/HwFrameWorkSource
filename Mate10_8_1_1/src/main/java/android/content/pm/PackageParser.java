    ...
    public static final class Activity extends Component<ActivityIntentInfo> implements Parcelable {
        ...
        private void setMaxAspectRatio(float maxAspectRatio) {
            if (this.info.resizeMode == 2 || this.info.resizeMode == 1) {
                if (PackageParser.mFullScreenDisplay && this.info.applicationInfo.maxAspectRatio < PackageParser.mScreenAspectRatio && this.info.applicationInfo.maxAspectRatio >= PackageParser.mAppAspectRatio) {
                    this.info.applicationInfo.maxAspectRatio = PackageParser.mScreenAspectRatio;
                    if (this.owner.applicationInfo.hasDefaultNoFullScreen == 1) {
                        this.owner.applicationInfo.hasDefaultNoFullScreen = 2;
                    }
                }
                ...
                this.info.maxAspectRatio = 0.0f;
            } else if (maxAspectRatio < 1.0f && maxAspectRatio != 0.0f) {
                this.info.maxAspectRatio = 0.0f;
            } else if (!PackageParser.mFullScreenDisplay || maxAspectRatio >= PackageParser.mScreenAspectRatio || maxAspectRatio <= PackageParser.mAppAspectRatio) {
...
*/
    ...
    public static int getActivityConfigChanges(int r1, int r2) {
        /* JADX: method processing error */
/*
Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.content.pm.PackageParser.getActivityConfigChanges(int, int):int
	at jadx.core.dex.nodes.MethodNode.load(MethodNode.java:116)
	at jadx.core.dex.nodes.ClassNode.load(ClassNode.java:249)
	at jadx.core.ProcessClass.process(ProcessClass.java:31)
	at jadx.core.ProcessClass.processDependencies(ProcessClass.java:56)
...
*/
        ...
        // Can't load method instructions.
        */
        throw new UnsupportedOperationException("Method not decompiled: android.content.pm.PackageParser.getActivityConfigChanges(int, int):int");
    }
    ...
    static {
        ...
        mAppAspectRatio = 0.0f;
        mFullScreenDisplay = false;
        String[] aspects = SystemProperties.get("ro.config.hw_screen_aspect", "").split(":");
        if (aspects.length == 3) {
        ...
        r5[0] = new SplitPermissionInfo(permission.WRITE_EXTERNAL_STORAGE, new String[]{permission.READ_EXTERNAL_STORAGE}, 10001);
    ...
    static PackageLite parseClusterPackageLite(File packageDir, int flags) throws PackageParserException {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ApkLite apk = (ApkLite) apks.get(strArr[i]);
                strArr2[i] = apk.usesSplitName;
                zArr[i] = apk.isFeatureSplit;
                strArr3[i] = apk.configForSplit;
                strArr4[i] = apk.codePath;
                iArr[i] = apk.revisionCode;
            }
        }
        return new PackageLite(packageDir.getAbsolutePath(), baseApk, strArr, zArr, strArr2, strArr3, strArr4, iArr);
    }

    public Package parsePackage(File packageFile, int flags, boolean useCaches, int hwFlags) throws PackageParserException {
        Package cachedResult = useCaches ? getCachedResult(packageFile, flags) : null;
        if (cachedResult != null) {
            return cachedResult;
        }
        long parseTime = LOG_PARSE_TIMINGS ? SystemClock.uptimeMillis() : 0;
        if (packageFile.isDirectory()) {
            cachedResult = parseClusterPackage(packageFile, flags, hwFlags);
        } else {
            cachedResult = parseMonolithicPackage(packageFile, flags, hwFlags);
        }
        long cacheTime = LOG_PARSE_TIMINGS ? SystemClock.uptimeMillis() : 0;
        cacheResult(packageFile, flags, cachedResult);
    ...
    private Package parseClusterPackage(File packageDir, int flags, int hwFlags) throws PackageParserException {
        ...
        if (!this.mOnlyCoreApps || (lite.coreApp ^ 1) == 0) {
            ...
            if (!lite.isolatedSplits || (ArrayUtils.isEmpty(lite.splitNames) ^ 1) == 0) {
                ...
                try {
                    sparseArray = SplitDependencyLoader.createDependenciesFromPackage(lite);
                    assetLoader = new SplitAssetDependencyLoader(lite, sparseArray, flags);
            ...
            try {
                ...
                if (!ArrayUtils.isEmpty(lite.splitNames)) {
                    ...
                    pkg.applicationInfo.splitDependencies = sparseArray;
    ...
    private static void collectCertificates(Package pkg, File apkFile, int parseFlags) throws PackageParserException {
        ...
        try {
            ...
            Exception e32;
            throw new PackageParserException(PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "Failed to collect certificates from " + apkPath + " using APK Signature Scheme v2", e32);
        } catch (Throwable th2) {
            Trace.traceEnd(Trace.TRACE_TAG_PACKAGE_MANAGER);
        }
        if (verified) {
        ...
        try {
            ...
            StrictJarFile strictJarFile2 = new StrictJarFile(apkPath, verified ^ 1, (parseFlags & 64) == 0);
            try {
                Trace.traceEnd(Trace.TRACE_TAG_PACKAGE_MANAGER);
                ...
                e32 = e5;
                strictJarFile = strictJarFile2;
                throw new PackageParserException(PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "Failed to collect certificates from " + apkPath, e32);
            } catch (Throwable th4) {
                th = th4;
                strictJarFile = strictJarFile2;
                closeQuietly(strictJarFile);
    ...
    public static ApkLite parseApkLite(File apkFile, int flags) throws PackageParserException {
        ...
        try {
            autoCloseable = newConfiguredAssetManager();
            int cookie = autoCloseable.addAssetPath(apkPath);
            if (cookie == 0) {
            ...
            parser = autoCloseable.openXmlResourceParser(cookie, ANDROID_MANIFEST_FILENAME);
            if ((flags & 256) != 0) {
            ...
            ApkLite parseApkLite = parseApkLite(apkPath, parser, parser, signatures, certificateArr);
            IoUtils.closeQuietly(parser);
            IoUtils.closeQuietly(autoCloseable);
            return parseApkLite;
        } catch (Exception e) {
    ...
    private static Pair<String, String> parsePackageSplitNames(XmlPullParser parser, AttributeSet attrs) throws IOException, XmlPullParserException, PackageParserException {
        ...
        if (type != 2) {
            ...
            if (!("android".equals(packageName) || ("androidhwext".equals(packageName) ^ 1) == 0 || ("featurelayerwidget".equals(packageName) ^ 1) == 0)) {
    ...
    private Package parseBaseApkCommon(Package pkg, Set<String> acceptedTags, Resources res, XmlResourceParser parser, int flags, String[] outError) throws XmlPullParserException, IOException {
        ...
        if (str != null && str.length() > 0) {
            ...
            if (nameError == null || ("android".equals(pkg.packageName) ^ 1) == 0 || ("androidhwext".equals(pkg.packageName) ^ 1) == 0 || ("featurelayerwidget".equals(pkg.packageName) ^ 1) == 0) {
            ...
            if (type == 1 || (type == 3 && parser.getDepth() <= outerDepth)) {
                ...
                if (acceptedTags == null || (acceptedTags.contains(tagName) ^ 1) == 0) {
                    if (!tagName.equals(TAG_APPLICATION)) {
                                                            ...
                                                            if (tagName.equals(TAG_FEATURE_GROUP)) {
                                                                ...
                                                                if (tagName.equals(TAG_USES_SDK)) {
                                                                    if (SDK_VERSION > 0) {
                                                                        ...
                                                                        sa.recycle();
                                                                        if ("O".equals(targetCode) || "O".equals(str2)) {
                                                                        ...
                                                                        int minSdkVersion = computeMinSdkVersion(minVers, str2, SDK_VERSION, SDK_CODENAMES, outError);
                                                                        if (minSdkVersion < 0) {
                                                                            this.mParseError = -12;
                                                                            return null;
                                                                        }
                        ...
                        if (pkg.applicationInfo.minEmuiSdkVersion > CURRENT_EMUI_SDK_VERSION && CURRENT_EMUI_SDK_VERSION != 0) {
    ...
    private static String buildCompoundName(String pkg, CharSequence procSeq, String type, String[] outError) {
        ...
        if (pkg == null || c != ':') {
            ...
            if (nameError == null || (HwThemeManager.HWT_USER_SYSTEM.equals(proc) ^ 1) == 0) {
                return proc;
            }
    ...
    private static String buildProcessName(String pkg, String defProc, CharSequence procSeq, int flags, String[] separateProcesses, String[] outError) {
        if ((flags & 8) == 0 || (HwThemeManager.HWT_USER_SYSTEM.equals(procSeq) ^ 1) == 0) {
    ...
    private boolean parseBaseApplication(Package owner, Resources res, XmlResourceParser parser, int flags, String[] outError) throws XmlPullParserException, IOException {
        ...
        if (parsePackageItemInfo(owner, ai, outError, "<application>", sa, false, 3, 1, 2, 42, 22, 30)) {
                                ...
                                if (tagName.equals("provider")) {
                                    ...
                                    if (tagName.equals("activity-alias")) {
                                        ...
                                        if (mFullScreenDisplay) {
                                            float maxAspectRatio = owner.mAppMetaData.getFloat(METADATA_MAX_ASPECT_RATIO);
                                            if (maxAspectRatio > 1.0f) {
                                                maxAspectRatioTmp = maxAspectRatio;
                                                ai.maxAspectRatio = maxAspectRatio;
                                            }
                                        }
                                        ai.minEmuiSdkVersion = owner.mAppMetaData.getInt("huawei.emui_minSdk");
                                        ai.targetEmuiSdkVersion = owner.mAppMetaData.getInt("huawei.emui_targetSdk");
                                        ai.gestnav_extra_flags = owner.mAppMetaData.getInt("huawei.gestnav_extra_flags");
                                    } else {
                                        String lname;
            ...
            setMaxAspectRatio(owner);
            hwNotchSupport = false;
            if (owner.mAppMetaData != null && owner.mAppMetaData.containsKey(METADATA_NOTCH_SUPPORT)) {
                hwNotchSupport = owner.mAppMetaData.getBoolean(METADATA_NOTCH_SUPPORT, false);
            }
            int gestureNavOptions = 0;
            if (owner.mAppMetaData != null && owner.mAppMetaData.containsKey(METADATA_GESTURE_NAV_OPTIONS)) {
                gestureNavOptions = owner.mAppMetaData.getInt(METADATA_GESTURE_NAV_OPTIONS, 0);
            }
            int size = owner.activities.size();
            for (i2 = 0; i2 < size; i2++) {
                t = (Activity) owner.activities.get(i2);
                if (hwNotchSupport || (t.metaData != null && t.metaData.getBoolean(METADATA_NOTCH_SUPPORT, false))) {
            ...
            for (Activity activity : owner.activities) {
                if (HwFrameworkFactory.getHwPackageParser().isDefaultFullScreen(activity.info.packageName)) {
                    owner.applicationInfo.hasDefaultNoFullScreen = 0;
                    break;
                }
            }
            if (maxAspectRatioTmp >= 1.0f && maxAspectRatioTmp >= mScreenAspectRatio) {
                owner.applicationInfo.hasDefaultNoFullScreen = 0;
            }
            boolean showAppRatio = mFullScreenDisplay && maxAspectRatioTmp < mScreenAspectRatio && maxAspectRatioTmp > mAppAspectRatio;
            int NA = owner.activities.size();
            for (i2 = 0; i2 < NA; i2++) {
                ActivityInfo activityInfo;
                float f;
                t = (Activity) owner.activities.get(i2);
                if (maxAspectRatioTmp >= 1.0f) {
                    if (owner.applicationInfo.hasDefaultNoFullScreen == 2 || t.info.resizeMode == 2 || t.info.resizeMode == 1) {
                        t.info.maxAspectRatio = 0.0f;
                    } else {
                        activityInfo = t.info;
                        if (showAppRatio) {
                            f = mAppAspectRatio;
                        ...
                        activityInfo.maxAspectRatio = f;
                    }
                } else if (owner.applicationInfo.hasDefaultNoFullScreen == 2) {
                    t.info.maxAspectRatio = 0.0f;
                }
                if (HwFrameworkFactory.getHwPackageParser().isDefaultFullScreen(t.info.packageName)) {
                    t.info.maxAspectRatio = 0.0f;
                }
                if (t.info.resizeMode == 2 || t.info.resizeMode == 1) {
                    t.info.originMaxAspectRatio = mScreenAspectRatio;
                } else {
                    activityInfo = t.info;
                    if (showAppRatio) {
                        f = mAppAspectRatio;
                    ...
                    activityInfo.originMaxAspectRatio = f;
                    if (mFullScreenDisplay && t.info.maxAspectRatio != 0.0f && t.info.maxAspectRatio < mScreenAspectRatio) {
            ...
            while (true) {
                ...
                if (type != 1 && (type != 3 || parser.getDepth() > innerDepth)) {
                    if (!(type == 3 || type == 4)) {
                        ...
                        if (componentInfo != null && componentInfo.splitName == null) {
                            componentInfo.splitName = owner.splitNames[splitIndex];
                        }
                    }
                }
            }
    ...
    private static boolean parsePackageItemInfo(Package owner, PackageItemInfo outInfo, String[] outError, String tag, TypedArray sa, boolean nameRequired, int nameRes, int labelRes, int iconRes, int roundIconRes, int logoRes, int bannerRes) {
        ...
        int roundIconVal = Resources.getSystem().getBoolean(17957053) ? sa.getResourceId(roundIconRes, 0) : 0;
        if (roundIconVal != 0) {
    ...
    private void setMaxAspectRatio(Package owner) {
        float maxAspectRatio = owner.applicationInfo.targetSdkVersion < 26 ? mFullScreenDisplay ? mAppAspectRatio : DEFAULT_PRE_O_MAX_ASPECT_RATIO : 0.0f;
    ...
    private boolean parseIntent(Resources res, XmlResourceParser parser, boolean allowGlobs, boolean allowAutoVerify, IntentInfo outInfo, String[] outError) throws XmlPullParserException, IOException {
        ...
        if (Resources.getSystem().getBoolean(17957053)) {

...
public class Resources extends AbsResources {
    ...
    public CharSequence getText(int id, CharSequence def) {
        CharSequence resourceText = id != 0 ? this.mResourcesImpl.getAssets().getResourceText(id) : null;
        ...
        try {
            ...
            if (value.type == 5) {
                ...
                if (isTypeMM(value.data) && isRogOn()) {
                    int rogRes = (int) (((float) res) * getMMScale());
                    if (id == 17105147 && (rogRes & 1) == 1) {
                        rogRes++;
                    }
                    releaseTempTypedValue(value);
                    return rogRes;
    ...
    private int rebuildSpecialDimens(int id, TypedValue value, int res) {
        if (!(id != 17105265 || (isLockDpi() ^ 1) == 0 || (isTypeMM(value.data) ^ 1) == 0)) {
        ...
        if (id == 17105147 && isTypeMM(value.data) && (res & 1) == 1) {
            return res + 1;
        }
        return res;
    }
    ...
    public static float getMMScale() {
        int dpi = SystemProperties.getInt("persist.sys.dpi", SystemProperties.getInt("ro.sf.lcd_density", SystemProperties.getInt("ro.sf.real_lcd_density", 0)));

...
public final class Configuration implements Parcelable, Comparable<Configuration> {
    ...
    public int diff(Configuration delta, boolean compareUndefined, boolean publicOnly) {
        ...
        if (this.appBounds != null && (publicOnly || (this.appBounds.equals(delta.appBounds) ^ 1) == 0)) {
            if (!publicOnly) {
                return changed;
            }
            if (this.appBounds.width() == delta.appBounds.width() && this.appBounds.height() == delta.appBounds.height()) {
                return changed;
            }
        }
        return changed | 1024;

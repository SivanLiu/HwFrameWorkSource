...
public abstract class ApplicationTestCase<T extends Application> extends AndroidTestCase {
    ...
    private void setupApplication() {
        ...
        try {
            ...
            TestCase.assertNotNull(this.mApplication);
        }
        this.mAttached = true;
    }

    protected final void createApplication() {
        TestCase.assertFalse(this.mCreated);
        if (!this.mAttached) {
            setupApplication();
        }
        TestCase.assertNotNull(this.mApplication);
    ...
    public final void testApplicationTestCaseSetUpProperly() throws Exception {
        setupApplication();
        TestCase.assertNotNull("Application class could not be instantiated successfully", this.mApplication);
    }
}

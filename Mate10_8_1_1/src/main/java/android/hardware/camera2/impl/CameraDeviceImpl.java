        ...
        public void onResultReceived(CameraMetadataNative result, CaptureResultExtras resultExtras) throws RemoteException {
            ...
            synchronized (CameraDeviceImpl.this.mInterfaceLock) {
                ...
                if (holder == null) {
                    ...
                    if (holder.hasBatchedOutputs()) {
                        cameraMetadataNative = new CameraMetadataNative(result);
                    } else {
                        ...
                        resultDispatch = new Runnable() {
                            public void run() {
                                if (!CameraDeviceImpl.this.isClosed()) {
                                    if (holder.hasBatchedOutputs()) {
                                        for (int i = 0; i < holder.getRequestCount(); i++) {
                                            holder.getCallback().onCaptureProgressed(CameraDeviceImpl.this, holder.getRequest(i), new CaptureResult(new CameraMetadataNative(cameraMetadataNative), holder.getRequest(i), captureResultExtras));
                                        }
                                        return;
                                    }
                                ...
                                if (!CameraDeviceImpl.this.isClosed()) {
                                    if (captureCallbackHolder.hasBatchedOutputs()) {
                                        for (int i = 0; i < captureCallbackHolder.getRequestCount(); i++) {
                                            cameraMetadataNative2.set(CaptureResult.SENSOR_TIMESTAMP, Long.valueOf(sensorTimestamp - ((((long) (subsequenceId - i)) * CameraDeviceImpl.NANO_PER_SECOND) / ((long) ((Integer) fpsRange.getUpper()).intValue()))));
                                            captureCallbackHolder.getCallback().onCaptureCompleted(CameraDeviceImpl.this, captureCallbackHolder.getRequest(i), new TotalCaptureResult(new CameraMetadataNative(cameraMetadataNative2), captureCallbackHolder.getRequest(i), captureResultExtras2, list, captureCallbackHolder.getSessionId()));
                                        }
                                        return;
                                    }
        ...
        if (lastFrameNumber == -1) {
            ...
            if (captureCallbackHolder != null) {
                captureCallbackHolder.getHandler().post(new Runnable() {
                    public void run() {
                        if (!CameraDeviceImpl.this.isClosed()) {
                            captureCallbackHolder.getCallback().onCaptureSequenceAborted(CameraDeviceImpl.this, requestId);
                        }
                    }
                });
                return;

...
public final class BeamShareData implements Parcelable {
    public static final Creator<BeamShareData> CREATOR = new Creator<BeamShareData>() {
        public BeamShareData createFromParcel(Parcel source) {
            ...
            if (numUris > 0) {
                uriArr = new Uri[numUris];
                source.readTypedArray(uriArr, Uri.CREATOR);
            }
            return new BeamShareData(msg, uriArr, (UserHandle) source.readParcelable(UserHandle.class.getClassLoader()), source.readInt());
        }

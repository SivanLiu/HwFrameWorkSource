...
public class ZenModeConfig implements Parcelable {
    ...
    public static Condition toTimeCondition(Context context, long time, int minutes, int userHandle, boolean shortVersion) {
        ...
        return new Condition(toCountdownConditionId(time, false), summary, line1, line2, 0, 1, 1);
    }
    ...
    public static Condition toNextAlarmCondition(Context context, long alarm, int userHandle) {
        ...
        return new Condition(toCountdownConditionId(alarm, true), "", context.getResources().getString(R.string.zen_mode_until, new Object[]{formattedTime}), "", 0, 1, 1);
    }

    private static CharSequence getFormattedTime(Context context, long time, boolean isSameDay, int userHandle) {
        return DateFormat.format(DateFormat.getBestDateTimePattern(Locale.getDefault(), (!isSameDay ? "EEE " : "") + (DateFormat.is24HourFormat(context, userHandle) ? "Hm" : "hma")), time);
    }
    ...
    public static Uri toCountdownConditionId(long time, boolean alarm) {
        return new Builder().scheme("condition").authority(SYSTEM_AUTHORITY).appendPath(COUNTDOWN_PATH).appendPath(Long.toString(time)).appendPath(IS_ALARM_PATH).appendPath(Boolean.toString(alarm)).build();
    }
    ...
    public static boolean isValidCountdownToAlarmConditionId(Uri conditionId) {
        if (tryParseCountdownConditionId(conditionId) == 0 || conditionId.getPathSegments().size() < 4 || (IS_ALARM_PATH.equals(conditionId.getPathSegments().get(2)) ^ 1) != 0) {
            return false;
        }
        try {
    ...
    public static ScheduleInfo tryParseScheduleConditionId(Uri conditionId) {
        ...
        if (conditionId != null && conditionId.getScheme().equals("condition") && conditionId.getAuthority().equals(SYSTEM_AUTHORITY) && conditionId.getPathSegments().size() == 1) {
            equals = ((String) conditionId.getPathSegments().get(0)).equals(SCHEDULE_PATH);
        } else {
            equals = false;
        }
        if (!equals) {
    ...
    public static Uri toEventConditionId(EventInfo event) {
        return new Builder().scheme("condition").authority(SYSTEM_AUTHORITY).appendPath("event").appendQueryParameter(HwCmpType.USERID, Long.toString((long) event.userId)).appendQueryParameter("calendar", event.calendar != null ? event.calendar : "").appendQueryParameter("reply", Integer.toString(event.reply)).build();
    }
    ...
    public static EventInfo tryParseEventConditionId(Uri conditionId) {
        boolean equals;
        if (conditionId != null && conditionId.getScheme().equals("condition") && conditionId.getAuthority().equals(SYSTEM_AUTHORITY) && conditionId.getPathSegments().size() == 1) {
            equals = ((String) conditionId.getPathSegments().get(0)).equals("event");
        } else {
            equals = false;
        }
        if (!equals) {

            ...
            if (this.mIsOpen) {
                ...
                try {
                    if (!(waiter.mWantPrimaryConnection || !waiter.mWantExclusiveConnection || (exclusiveConnectionNotAvailable ^ 1) == 0)) {
                        sQLiteConnection = tryAcquireExclusiveConnectionLocked(waiter.mConnectionFlags);
                        if (sQLiteConnection == null) {
                            exclusiveConnectionNotAvailable = true;
                        }
                    }
                    if (!(sQLiteConnection != null || (waiter.mWantPrimaryConnection ^ 1) == 0 || (nonPrimaryConnectionNotAvailable ^ 1) == 0)) {
                        sQLiteConnection = tryAcquireNonPrimaryConnectionLocked(waiter.mSql, waiter.mConnectionFlags);
                        if (sQLiteConnection == null) {
                            nonPrimaryConnectionNotAvailable = true;
                        }
                    }
                    if (sQLiteConnection == null && (((isExclusiveConnectionEnabled() && waiter.mWantPrimaryConnection) || (isExclusiveConnectionEnabled() ^ 1) != 0) && (primaryConnectionNotAvailable ^ 1) != 0)) {
                        sQLiteConnection = tryAcquirePrimaryConnectionLocked(waiter.mConnectionFlags);
                        if (sQLiteConnection == null) {
                            primaryConnectionNotAvailable = true;
                        }
                    }
                    if (sQLiteConnection != null) {
                        waiter.mAssignedConnection = sQLiteConnection;
                        unpark = true;

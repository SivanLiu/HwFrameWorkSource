        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                case 1:
                    ...
                    onSyncEstablished(_arg0, bluetoothDevice, data.readInt(), data.readInt(), data.readInt(), data.readInt());
                    return true;
                case 2:
                    PeriodicAdvertisingReport periodicAdvertisingReport;
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {
                        periodicAdvertisingReport = (PeriodicAdvertisingReport) PeriodicAdvertisingReport.CREATOR.createFromParcel(data);
                    } else {
                        periodicAdvertisingReport = null;
                    }
                    onPeriodicAdvertisingReport(periodicAdvertisingReport);
                    return true;
                case 3:
                    data.enforceInterface(DESCRIPTOR);

...
public class Map {
    ...
    private boolean drawTile(Tile tile, int i, int j, GoogleGraphics g, int xOffset, int yOffset, boolean fetch, boolean drawIncompleteTiles, boolean drawOptionalFeatures, long paintStartTime, long accessTime) {
        ...
        if (!tile.notValid()) {
            ...
            if (g != null) {
                if (mapTile.hasImage() || drawIncompleteTiles) {
    ...
    private boolean isTileOnScreenX(Tile tile) {
        ...
        if (tile.getXIndex() >= this.topLeftDisplayTile.getXIndex()) {
            if (tile.getXIndex() > rightDisplayTileXIndex) {
            }
            return z;
        }
        z = false;
    ...
    public static boolean isMapPointInKoreaBoundingBox(MapPoint point) {
        if (point != null) {
            int lat = point.getLatitude();
            int lng = point.getLongitude();
            if (lat <= 32989084 || lat >= 38693013 || lng <= 124605560 || lng >= 128496094) {
                if (lat <= 34464674 || lat >= 38693013 || lng <= 128496094 || lng >= 128847656) {
                    if (lat <= 35027747 || lat >= 38693013 || lng <= 128847656 || lng >= 131053162) {
                        if (lat > 37027773) {
                            if (lat < 38693013) {
                                if (lng > 131053162) {
                                }

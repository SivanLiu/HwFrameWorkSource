...
public class StylesheetHandler extends DefaultHandler implements TemplatesHandler, PrefixResolver, NodeConsumer {
    ...
    XSLTElementProcessor getProcessorFor(String uri, String localName, String rawName) throws SAXException {
        ...
        if (elemProcessor == null && ((currentProcessor instanceof ProcessorStylesheetDoc) ^ 1) != 0) {
            if (getStylesheet() != null && Double.valueOf(getStylesheet().getVersion()).doubleValue() <= 1.0d && (uri.equals(Constants.S_XSLNAMESPACEURL) || !(currentProcessor instanceof ProcessorStylesheetElement))) {
                if (getElemVersion() > 1.0d) {
                }
            }

    ...
    protected void declareNamespaceInContext(int elementNodeIndex, int namespaceNodeIndex) {
        ...
        if (this.m_namespaceDeclSets == null) {
            ...
            if (last >= 0 && elementNodeIndex == this.m_namespaceDeclSetElements.elementAt(last)) {
                suballocatedIntVector = (SuballocatedIntVector) this.m_namespaceDeclSets.elementAt(last);
            }
        }
        if (suballocatedIntVector == null) {
            ...
            if (inherited != null) {
                ...
                suballocatedIntVector = new SuballocatedIntVector(Math.max(Math.min(isize + 16, DTMFilter.SHOW_NOTATION), 32));
                for (i = 0; i < isize; i++) {
                    suballocatedIntVector.addElement(inherited.elementAt(i));
                }
            } else {
                suballocatedIntVector = new SuballocatedIntVector(32);
            }
        ...
        for (i = suballocatedIntVector.size() - 1; i >= 0; i--) {
            if (newEType == getExpandedTypeID(suballocatedIntVector.elementAt(i))) {
                suballocatedIntVector.setElementAt(makeNodeHandle(namespaceNodeIndex), i);
                return;
            }
        }
        suballocatedIntVector.addElement(makeNodeHandle(namespaceNodeIndex));
    }

...
public class SEService {
    ...
    private final ISmartcardServiceCallback mCallback = new Stub() {
        public void notifyEvent(int eventType, String name) throws RemoteException {
            Log.v(SEService.SERVICE_TAG, "notify eventType=" + eventType + " name=" + name);
            synchronized (SEService.this.mLock) {
                Reader reader = (Reader) SEService.this.mReaders.get(name);
                if (reader == null) {
                    Log.v(SEService.SERVICE_TAG, "Cannot find reader " + name);
                } else {
                    try {
                        reader.notifyEvent(eventType);
                    } catch (Exception e) {
                        Log.e(SEService.SERVICE_TAG, "notify failure", e);
                    }
                }
            }
        }
    ...
    public SEService(Context context, CallBack listener) {
        ...
        this.mConnection = new ServiceConnection() {
            public synchronized void onServiceConnected(ComponentName className, IBinder service) {
                ...
                try {
                    if (SEService.this.mSmartcardService == null || SEService.this.mCallback == null) {
                        Log.v(SEService.SERVICE_TAG, "Cannot registerCallback " + SEService.this.mSmartcardService + " " + SEService.this.mCallback);
                        ...
                        SEService.this.mSmartcardService.registerCallback(SEService.this.mCallback);
                        if (SEService.this.mCallerCallback != null) {
                            SEService.this.mCallerCallback.serviceConnected(SEService.this);
                        }
                        Log.v(SEService.SERVICE_TAG, "Service onServiceConnected");
                    ...
                    Log.v(SEService.SERVICE_TAG, "registerCallback failure", e);
                }
            }
    ...
    public void shutdown() {
        ...
        synchronized (this.mLock) {
            ...
            try {
                if (!(this.mSmartcardService == null || this.mCallback == null)) {
                    try {
                        this.mSmartcardService.unregisterCallback(this.mCallback);

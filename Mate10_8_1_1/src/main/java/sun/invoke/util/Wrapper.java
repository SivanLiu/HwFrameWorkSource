...
public enum Wrapper {
    ...
    CHAR(Character.class, Character.TYPE, 'C', Character.valueOf('\u0000'), new char[0], Format.unsigned(16)),
    ...
    static <T> Class<T> forceType(Class<?> type, Class<T> exampleType) {
        boolean isPrimitive = (type == exampleType || ((type.isPrimitive() && forPrimitiveType(type) == findWrapperType(exampleType)) || (exampleType.isPrimitive() && forPrimitiveType(exampleType) == findWrapperType(type)))) ? true : type == Object.class ? exampleType.isPrimitive() ^ 1 : false;
        if (!isPrimitive) {
            System.out.println(type + " <= " + exampleType);
        }
        if (!-assertionsDisabled) {
            int isPrimitive2 = (type == exampleType || ((type.isPrimitive() && forPrimitiveType(type) == findWrapperType(exampleType)) || (exampleType.isPrimitive() && forPrimitiveType(exampleType) == findWrapperType(type)))) ? 1 : type == Object.class ? exampleType.isPrimitive() ^ 1 : 0;
            if (isPrimitive2 == 0) {
                throw new AssertionError();
            }
        }

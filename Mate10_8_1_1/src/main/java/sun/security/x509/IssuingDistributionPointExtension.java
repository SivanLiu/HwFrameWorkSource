...
public class IssuingDistributionPointExtension extends Extension implements CertAttrSet<String> {
        ...
        if (value instanceof byte[]) {
            ...
            if (val.tag != (byte) 48) {
                ...
                while (in != null && in.available() != 0) {
                    ...
                    } else if (opt.isContextSpecific((byte) 3) && (opt.isConstructed() ^ 1) != 0) {
                        this.revocationReasons = new ReasonFlags(opt);
                    } else if (opt.isContextSpecific((byte) 4) && (opt.isConstructed() ^ 1) != 0) {
                        opt.resetTag((byte) 1);
                        this.isIndirectCRL = opt.getBoolean();
                    } else if (!opt.isContextSpecific((byte) 5) || (opt.isConstructed() ^ 1) == 0) {
    ...
    private void encodeThis() throws IOException {
        if (this.distributionPoint != null || this.revocationReasons != null || (this.hasOnlyUserCerts ^ 1) == 0 || (this.hasOnlyCACerts ^ 1) == 0 || (this.hasOnlyAttributeCerts ^ 1) == 0 || (this.isIndirectCRL ^ 1) == 0) {
            DerOutputStream tmp;
            ...
            if (this.hasOnlyCACerts) {
                ...
                tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, false, (byte) 2), tmp);
            }
            if (this.revocationReasons != null) {
                tmp = new DerOutputStream();
                this.revocationReasons.encode(tmp);
                tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, false, (byte) 3), tmp);
            }
            if (this.isIndirectCRL) {
                tmp = new DerOutputStream();
                tmp.putBoolean(this.isIndirectCRL);
                tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, false, (byte) 4), tmp);
            }
            if (this.hasOnlyAttributeCerts) {
                tmp = new DerOutputStream();
                tmp.putBoolean(this.hasOnlyAttributeCerts);
                tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, false, (byte) 5), tmp);
            }

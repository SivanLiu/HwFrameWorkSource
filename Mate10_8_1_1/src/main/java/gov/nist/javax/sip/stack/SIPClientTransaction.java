...
public class SIPClientTransaction extends SIPTransaction implements ServerResponseInterface, ClientTransaction, ClientTransactionExt {
        ...
        try {
            if (getState() != null && (this.isMapped ^ 1) == 0) {
                ...
                if (!(inv && TransactionState.CALLING == s)) {
                    if (!inv) {
                        if (TransactionState.TRYING != s) {
                            if (TransactionState.PROCEEDING == s) {
                            }
                        }
                    }
                }
    ...
    protected void fireTimeoutTimer() {
        ...
        if (!(TransactionState.CALLING == getState() || TransactionState.TRYING == getState())) {
            ...
            if (TransactionState.COMPLETED != getState()) {
                ...
                if (!getOriginalRequest().getMethod().equalsIgnoreCase(Request.CANCEL)) {
                    inviteTx = (SIPClientTransaction) getOriginalRequest().getInviteTransaction();
                    if (inviteTx != null) {
                        return;
                    }
        ...
        if (dialog == null || !(dialog.getState() == null || dialog.getState() == DialogState.EARLY)) {
            if (dialog != null && getOriginalRequest().getMethod().equalsIgnoreCase("BYE") && dialog.isTerminatedOnBye()) {
                dialog.delete();
            }
            if (TransactionState.COMPLETED != getState()) {
                setState(TransactionState.TERMINATED);
            ...
            if (!getOriginalRequest().getMethod().equalsIgnoreCase(Request.CANCEL)) {
                inviteTx = (SIPClientTransaction) getOriginalRequest().getInviteTransaction();
                if (inviteTx != null) {
                    if (inviteTx.getState() != TransactionState.CALLING) {
                    }
                    inviteTx.setState(TransactionState.TERMINATED);
                    return;
                }
    ...
    public Request createAck() throws SipException {
        ...
        if (originalRequest == null) {
            ...
            if (!((SipURI) ((Route) routeList.getFirst()).getAddress().getURI()).hasLrParam()) {
                ...
                if (contact != null) {
                    sIPHeader = new Route();
                    sIPHeader.setAddress((AddressImpl) ((AddressImpl) contact.getAddress()).clone());
                }

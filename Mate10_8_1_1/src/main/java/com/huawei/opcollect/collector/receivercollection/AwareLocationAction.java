...
public class AwareLocationAction extends Action implements ILocationListener {
    ...
    public void onLocationSuccess(HwLocation hwLocation) {
        if (hwLocation == null) {
            ...
            String cacheCityCode = this.mContext.getSharedPreferences("CachedCity", 32768).getString("CachedCity", EventIdConstant.PURPOSE_STR_BLANK);
            if (!TextUtils.isEmpty(curCountry) && (CHINA_COUNTRY_CODE.equalsIgnoreCase(curCountry) ^ 1) != 0 && (CHINA_COUNTRY_NUM.equalsIgnoreCase(curCountry) ^ 1) != 0) {

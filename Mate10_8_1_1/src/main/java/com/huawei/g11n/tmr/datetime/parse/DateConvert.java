...
public class DateConvert {
        ...
        while (it.hasNext()) {
            ...
            if (match != null) {
                ...
                if (ctype == ntype) {
                    ...
                    if (rel.matcher(content.substring(match.getEnd(), m.getBegin())).matches()) {
                        match.setEnd(m.getEnd());
                        match.setType(3);
                        if (ctype == 2 && ntype == 2) {
                            match.setIsTimePeriod(true);
                        }
                        match.getDp().setEnd(m.getDp().getBegin());
                        it.remove();
                    }
                }
                if (ctype == 0) {
    ...
    private List<Match> filterDateTime(String content, List<Match> ms, String DTBridge) {
        ...
        while (it.hasNext()) {
            ...
            if (match != null) {
                ...
                String ss = content.substring(match.getEnd(), m.getBegin());
                if (ss.trim().equals("")) {
                    flag = true;
                } else {
                ...
                if (flag) {
                    match.setEnd(m.getEnd());
                    if (ctype == 1 && type == 2) {
                        match.getDp().getBegin().setTime(m.getDp().getBegin().getTime());
                    } else if (ctype == 2 && type == 1) {
                        match.getDp().getBegin().setDay(m.getDp().getBegin().getDate());
                    } else if (ctype == 1 && type == 5) {
                        match.getDp().getBegin().setTime(m.getDp().getBegin().getTime());
                        match.getDp().setEnd(m.getDp().getEnd());
                    } else if (ctype == 5 && type == 1) {
                        match.getDp().getBegin().setDay(m.getDp().getBegin().getDate());
                        match.getDp().getEnd().setDay(m.getDp().getBegin().getDate());
                    }
                    it.remove();
                } else {
                    boolean change = true;
                    if (ctype == 2 && type == 1) {
                        boolean flag15;
                        int add = 0;
                        Matcher bcm = Pattern.compile("\\s*\\((.*?)\\s*\\)").matcher(content.substring(match.getEnd()));
                        String d = null;
                        if (bcm.lookingAt()) {
                            d = bcm.group(1);
                    ...
                    if (ctype == 1 && type == 2) {
                        String bStr = content.substring(0, match.getBegin());
                        String eStr = content.substring(match.getEnd(), m.getBegin());
                        if (bStr.trim().endsWith("(") && eStr.trim().startsWith(")")) {
                            match.setBegin(bStr.lastIndexOf(40));
                            match.setEnd(m.getEnd());
                            match.getDp().getBegin().setTime(m.getDp().getBegin().getTime());
                            it.remove();
                            change = false;
    ...
    private Match filterDateTimePunc(String content, List<Match> ms) {
        ...
        while (it.hasNext()) {
            ...
            if (match != null) {
                ...
                boolean flag = false;
                if (!(ctype == 1 && type == 2)) {
                    if (ctype != 1 || type != 3 || !m.isTimePeriod()) {
                        if (ctype != 2 || type != 1) {
                            if (ctype == 3 && match.isTimePeriod()) {
                                if (type != 1) {
                                }
                            }
                            match = m;
    ...
    public List<Date> convert(DatePeriod dp, long rTime) {
        ...
        if (status == 0) {
            ...
            Object obj = null;
            try {
                obj = new SimpleDateFormat(t.toString(), Locale.ENGLISH).parse(s.toString());

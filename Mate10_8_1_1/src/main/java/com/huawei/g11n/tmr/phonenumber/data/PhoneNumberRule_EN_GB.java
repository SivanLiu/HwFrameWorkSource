...
public class PhoneNumberRule_EN_GB extends PhoneNumberRule {
    ...
    public void init() {
        ...
        Object nRegex = new String[]{"\\p{Sc}\\d+[.]?\\d+", "4x\\d0+", "&amp;#\\d{2,5}", "\\d+((th)|%)", "(?<!(call))[iI]n \\d{4}", "([fF]rom|[uU]ntil) \\d{4}", "\\d{4,}[+]"};
        this.negativeRules = new ArrayList();
        Object -l_6_R = nRegex;
        for (String regex : nRegex) {
            this.negativeRules.add(new RegexRule(regex));
        }
        this.negativeRules.add(new RegexRule(utils.getValues(ConstantsUtils.FLOAT_2)));
    ...
    private static int countDigits(String str) {
        int count = 0;
        Object ch = str.toCharArray();
        Object -l_6_R = ch;
        int -l_5_I = ch.length;
        for (int -l_4_I = 0; -l_4_I < -l_5_I; -l_4_I++) {
            if (Character.isDigit(-l_6_R[-l_4_I])) {
                count++;
            }
        }
        return count;

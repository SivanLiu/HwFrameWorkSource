...
public class EventAnalyzed implements IEventAnalyzed {
    ...
    public static final int MAX_EVENT_ANALYZE_WAIT_COUNT = 16;
    ...
    private static final String VERSION = "6.0.1.30";
    ...
    public IApsManagerServiceCallback mApsManagerServiceCallback = null;
    ...
    class ApsManagerServiceCallback extends Stub {
        ...
        public void doCallback(int apsCallbackCode, int data) {
            try {
                ApsCommon.logD(EventAnalyzed.TAG, "ApsManagerServiceCallback, doCallback, apsCallbackCode:" + apsCallbackCode + ", data:" + data + ", mTouchMaxFps:" + EventAnalyzed.this.mTouchMaxFps);
                switch (apsCallbackCode) {
                    case 0:
                        if (EventAnalyzed.this.mTouchMaxFps == 0) {
                            EventAnalyzed.this.setFps(data);
                            return;
                        } else {
                            ApsCommon.logD(EventAnalyzed.TAG, "ApsManagerServiceCallback, doCallback, 0 != mTouchMaxFps, break!");
                            return;
                        }
                    case 1:
                        float fdata = -1.0f;
                        if (data >= 1000) {
                            fdata = (((float) data) * 1.0f) / 100000.0f;
                        }
                        EventAnalyzed.this.setResolutionRatio(fdata);
                        return;
                    case 2:
                    case 3:
                        return;
                    case 4:
                        EventAnalyzed.this.setNewHisiStatus(16, data);
                        return;
                    case 5:
                        EventAnalyzed.this.setNewHisiStatus(32, data);
                        return;
                    case 6:
                        EventAnalyzed.this.setNewHisiStatus(64, data);
                        return;
                    case EventAnalyzed.SCROLL_STATIC_GAME /*7*/:
                        EventAnalyzed.this.setNewHisiStatus(128, data);
                        return;
                    default:
    ...
    public EventAnalyzed() {
        ...
        try {
            ...
            if (pkgName != null && this.mApsManager != null) {
                ...
                HwApsInterface.nativeSetGameTextureQuality(texture);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(TAG, "EventAnalyzed nativeSetGameTextureQuality error, exception " + e);
        }
    }
    ...
    public void initAPS(Context context, int screenWidth, int myPid) {
        boolean z;
        boolean z2 = false;
        Log.i(TAG, "APS: version is 6.0.1.30");
    ...
    private boolean isAPSEnable() {
        ...
        if (apsEnable == 0 && SystemProperties.get("sys.aps.gameProcessName", "").equals(this.mPkgName)) {
            apsEnable = 1;
        }
        if (2 == apsEnable) {
    ...
    private boolean isIdentifyProcess(String strPkgName) {
        if (!this.mHasIdentifyProcess) {
            ...
            if (apsPkgName.equals(strPkgName) || gameProcessName.equals(strPkgName)) {
    ...
    private void analyzeUpAction(long eventTime, long downTime, int x, int y) {
        ...
        if ((this.mXScrollCount < 3 || this.mYScrollCount <= 0) && ((this.mYScrollCount < 3 || this.mXScrollCount <= 0) && this.mXScrollCount < 4)) {
            if (this.mYScrollCount >= 4) {
            }
        ...
        if (this.mDownMoveInterval >= 1000) {
        }
    }
    ...
    public void setHasOnPaused(boolean hasOnPaused) {
        ...
        if (!(this.mHandler == null || this.mStopEmergency == null)) {
            this.mHandler.removeCallbacks(this.mStopEmergency);
        }
        if (!(this.mHandler == null || this.mSendBroadcast == null)) {
        ...
        if (!(this.mHandler == null || this.mSetTouchStateForNonplay == null)) {
            this.mHandler.removeCallbacks(this.mSetTouchStateForNonplay);
        }
        if (!(this.mHandler == null || this.mResumePowerKit == null)) {
    ...
    private void checkAndRegisterCallback() {
        ...
        if (!this.isRegistedApsManagerServiceCallback) {
            ...
            if (this.mPkgName == null || (this.mPkgName.isEmpty() ^ 1) == 0) {
                String str = TAG;
                StringBuilder append = new StringBuilder().append("checkAndRegisterCallback, mPkgName error, do register at next touch event. mPkgName != null : ");
                if (this.mPkgName != null) {
                    z = true;
                }
                Log.e(str, append.append(z).append("mPkgName.isEmpty():").append(this.mPkgName.isEmpty()).toString());
                return;
            }
            String gameProcessName = SystemProperties.get("sys.aps.gameProcessName", "");
            ...
            if (gameProcessName.equals(this.mPkgName) || this.mIsGameProcess || whiteListProcessName.equals(this.mPkgName)) {
                if (this.mSdrController == null && SdrController.isSupportApsSdr()) {
                    this.mSdrController = SdrController.getInstance();
                    ApsCommon.logD(TAG, "SDR: checkAndRegisterCallback: SdrController init,mCanUseDebug : " + this.mCanUseDebug);
                }
    ...
    public void processAnalyze(Context context, int action, long eventTime, int x, int y, int pointCount, long downTime) {
        ...
        if (!(!this.mFirstDynamicAdjustFps || action == 0 || CUST_APP_TYPE != SystemProperties.getInt("debug.aps.enable", 0) || this.mContext.getPackageName() == null || (this.mContext.getPackageName().equals("com.android.systemui") ^ 1) == 0)) {
            if (this.mTimer == null) {
        ...
        if (9000 == SystemProperties.getInt("debug.aps.enable", 0)) {
            ...
            adapterFpsByTouchAndBattery(action, eventTime, x, y, pointCount, downTime);
        }
    }

    private void adapterFpsByTouchAndBattery(int action, long eventTime, int x, int y, int pointCount, long downTime) {
        int identifyByTouchFlag = SystemProperties.getInt("debug.aps.enable", 0);
        if (identifyByTouchFlag == 1 || identifyByTouchFlag == 2) {
    ...
    public void setAdaptSdr(int gameType, int openGLType) {
        ...
        if (CUST_APP_TYPE != this.mGameType || getCurrentBattery() > getCtrlBattery(this.mPkgName)) {
    ...
    public void setFps(int fps) {
        ...
        if (fps < 15 || fps >= 60) {
            if (fps >= 60 && this.mControlByAppSelf) {
                if (this.mResumePowerKit == null) {
                    this.mResumePowerKit = new ResumePowerKit();
                }
                if (this.mHandler == null) {
                    this.mHandler = new Handler();
                }
    ...
    public void setResolutionRatio(float fdata) {
        if (this.mSdrController != null && this.mSdrController.IsSdrCase() && 0.0f < fdata && fdata <= 1.0f) {
            ApsCommon.logD(TAG, "processAnalyze, setSdrRatio:" + fdata);
            this.mSdrController.setSdrRatio(fdata);
            this.mSdrController.startSdr();
        }
    }

    public void setNewHisiStatus(int flag, int data) {
        try {
            HwApsInterface.nativeSetNewHisiStatus(flag, data);
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(TAG, "EventAnalyzed, setNewHisiStatus, exception:" + e);
        }
    }
        ...
        try {
            if (!this.isRegistedApsManagerServiceCallback) {
                ...
                if (!(this.mApsManagerServiceCallback == null || this.mPkgName == null)) {
                    ...
                    if (this.mTouchMaxFps == 0) {
                        Log.d(TAG, "APS: the fps is enable and the fixed fps is " + this.mApsManager.getDynamicFps(this.mContext.getPackageName()));
                        setFps(this.mApsManager.getDynamicFps(this.mContext.getPackageName()));
                    } else {
                        Log.d(TAG, "APS: the fps is enable and the max fps is " + this.mTouchMaxFps);
                        setFps(this.mTouchMaxFps);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public int setGameProcessName(String processName, int pid, int gameType) {
        if (this.mContext == null || processName == null) {
            return 0;
        }
        Intent intent = new Intent("huawei.intent.action.APS_IDENTIFY_IS_GAME");

...
public class AppOpsManagerEx {
    ...
    public static Map<String, Integer> getPackagesForOp(AppOpsManager aom, int op, int userId) {
        ...
        List<PackageOps> packageOpsList = aom.getPackagesForOps(new int[]{op});
        if (packageOpsList != null) {
            for (PackageOps packageOps : packageOpsList) {
                if (userId == UserHandle.getUserId(packageOps.getUid())) {
                    ...
                    List<OpEntry> appOpsList = packageOps.getOps();
                    if (!(appOpsList == null || (appOpsList.isEmpty() ^ 1) == 0 || ((OpEntry) appOpsList.get(0)).getOp() != op)) {
                        opsMode = ((OpEntry) appOpsList.get(0)).getMode();
                    }

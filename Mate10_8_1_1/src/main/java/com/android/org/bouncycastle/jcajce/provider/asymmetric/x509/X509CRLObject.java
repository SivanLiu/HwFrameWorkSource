...
class X509CRLObject extends X509CRL {
    ...
    private Set loadCRLEntries() {
        ...
        while (certs.hasMoreElements()) {
            ...
            entrySet.add(new X509CRLEntryObject(entry, this.isIndirect, x500Name));
            if (this.isIndirect && entry.hasExtensions()) {
                Extension currentCaName = entry.getExtensions().getExtension(Extension.certificateIssuer);
                if (currentCaName != null) {
                    x500Name = X500Name.getInstance(GeneralNames.getInstance(currentCaName.getParsedValue()).getNames()[0].getName());
                }
            }
        }
        return entrySet;
    ...
    public X509CRLEntry getRevokedCertificate(BigInteger serialNumber) {
        ...
        while (certs.hasMoreElements()) {
            ...
            if (serialNumber.equals(entry.getUserCertificate().getValue())) {
                return new X509CRLEntryObject(entry, this.isIndirect, x500Name);
            }
            if (this.isIndirect && entry.hasExtensions()) {
                Extension currentCaName = entry.getExtensions().getExtension(Extension.certificateIssuer);
                if (currentCaName != null) {

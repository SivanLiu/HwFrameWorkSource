...
public abstract class Phone extends AbstractPhoneBase implements PhoneInternalInterface {
    ...
    protected Phone(String name, PhoneNotifier notifier, Context context, CommandsInterface ci, boolean unitTestMode, int phoneId, TelephonyComponentFactory telephonyComponentFactory) {
        ...
        this.mIsVoiceCapable = this.mContext.getResources().getBoolean(17957060);
    ...
    private void onCheckForNetworkSelectionModeAutomatic(Message fromRil) {
        ...
        String operatorNumeric = r != null ? r.getOperatorNumeric() : null;
        if (!("".equals(FORCE_AUTO_PLMN) || operatorNumeric == null || -1 == FORCE_AUTO_PLMN.indexOf(operatorNumeric))) {
            doAutomatic = true;
        }
        if (doAutomatic) {
    ...
    private void handleSetSelectNetwork(AsyncResult ar) {
        if (ar == null || ar.userObj == null || ((ar.userObj instanceof NetworkSelectMessage) ^ 1) != 0) {
    ...
    public void restoreSavedNetworkSelection(Message response) {
        ...
        if (networkSelection != null && (TextUtils.isEmpty(networkSelection.getOperatorNumeric()) ^ 1) != 0) {
    ...
    public void saveClirSetting(int commandInterfaceCLIRMode) {
        ...
        Rlog.i(this.LOG_TAG, "saveClirSetting: clir_key" + getPhoneId() + "=" + commandInterfaceCLIRMode);
        if (!editor.commit()) {
    ...
    private static Locale getLocaleFromCarrierProperties(Context ctx) {
        ...
        CharSequence[] carrierLocales = ctx.getResources().getTextArray(17235978);
    ...
    private void setCallForwardingIndicatorInSharedPref(boolean enable) {
        ...
        int subId = getSubId();
        Rlog.i(this.LOG_TAG, "setCallForwardingIndicatorInSharedPref: Storing status = " + status + " in pref " + CF_STATUS + subId);
    ...
    public boolean getCallForwardingIndicator() {
        ...
        Rlog.v(this.LOG_TAG, "getCallForwardingIndicator: iccForwardingFlag=" + (r != null ? Integer.valueOf(r.getVoiceCallForwardingFlag()) : "null") + ", sharedPrefFlag=" + getCallForwardingIndicatorFromSharedPref());
        if (callForwardingIndicator == 1) {
            z = true;
        } else {
            z = false;
    ...
    public boolean getCallForwardingPreference() {
        ...
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            if (!sp.contains("cf_enabled_key" + getSubId()) && sp.contains("cf_enabled_key" + this.mPhoneId)) {
                setCallForwardingPreference(sp.getBoolean("cf_enabled_key" + this.mPhoneId, false));
                edit = sp.edit();
                edit.remove("cf_enabled_key" + this.mPhoneId);
                edit.commit();
            }
        } else if (!sp.contains("cf_enabled_key" + getSubId()) && sp.contains("cf_enabled_key")) {
    ...
    public String getVmSimImsi() {
        ...
        Editor editor;
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            if (!sp.contains("sim_imsi_key" + getSubId()) && sp.contains("vm_sim_imsi_key" + this.mPhoneId)) {
                imsi = sp.getString("vm_sim_imsi_key" + this.mPhoneId, null);
                if (!(imsi == null || mPasswordUtil == null)) {
                    ...
                    if (!imsi.equals(getSubscriberId()) && oldDecodeVmSimImsi.equals(getSubscriberId())) {
                        ...
                        editor = sp.edit();
                        editor.remove("vm_sim_imsi_key" + this.mPhoneId);
                        editor.commit();
                    }
                }
            }
        } else if (!sp.contains("sim_imsi_key" + getSubId()) && sp.contains("vm_sim_imsi_key")) {
        ...
        String simimsi = sp.getString("sim_imsi_key" + getSubId(), null);
        if (simimsi == null) {
            return simimsi;
        }
        try {
    ...
    public void setVmSimImsi(String imsi) {
        try {
            Editor editor = PreferenceManager.getDefaultSharedPreferences(getContext()).edit();
            if (imsi != null) {
                editor.putString("sim_imsi_key" + getSubId(), new String(Base64.encode(imsi.getBytes("utf-8"), 0), "utf-8"));
            } else {
                editor.putString("sim_imsi_key" + getSubId(), null);
            }
            editor.commit();
        } catch (UnsupportedEncodingException e) {
    ...
    public boolean isImsVideoCallOrConferencePresent() {
        ...
        Rlog.d(this.LOG_TAG, "isImsVideoCallOrConferencePresent: " + isPresent);
        return isPresent;
    }
    ...
    public static void checkWfcWifiOnlyModeBeforeDial(Phone imsPhone, Context context) throws CallStateException {
        if (imsPhone == null || (imsPhone.isWifiCallingEnabled() ^ 1) != 0) {
            boolean wfcWiFiOnly = (ImsManager.isWfcEnabledByPlatform(context) && ImsManager.isWfcEnabledByUser(context)) ? ImsManager.getWfcMode(context) == 0 : false;
            if (wfcWiFiOnly) {

...
public class DcTracker extends AbstractDcTrackerBase {
    ...
    private class ProvisionNotificationBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            DcTracker.this.mProvisioningSpinner.setMessage(context.getText(17040352));
    ...
    private void onSetUserDataEnabled(boolean enabled) {
        ...
        synchronized (this.mDataEnabledSettings) {
            ...
            if (this.mHwCustDcTracker != null && enabled) {
            ...
            if (this.mDataEnabledSettings.isUserDataEnabled() != enabled) {
                ...
                if (enabled) {
                    ...
                    if (!(apnContext == null || (apnContext.isEnabled() ^ 1) == 0 || (isWifiConnect() ^ 1) == 0 || (this.mIsBtConnected ^ 1) == 0)) {
    ...
    private void initApnContexts() {
        ...
        for (String networkConfigString : this.mPhone.getContext().getResources().getStringArray(17236055)) {
    ...
    boolean isDataAllowed(ApnContext apnContext, DataConnectionReasons dataConnectionReasons) {
        ...
        int imsiReady = this.mIccRecords.get() != null ? !((IccRecords) this.mIccRecords.get()).getRecordsLoaded() ? ((IccRecords) this.mIccRecords.get()).getImsiReady() : 1 : 0;
        if (!(this.mIccRecords.get() == null || (imsiReady ^ 1) == 0)) {
            log("isDataAllowed getImsiReady=" + ((IccRecords) this.mIccRecords.get()).getImsiReady());
        }
        ...
        boolean isNeedForceSetup = (attachedState || (this.mAutoAttachOnCreation.get() && this.mPhone.getSubId() == dataSub)) ? true : isNeedForceSetup(apnContext);
        if (!isNeedForceSetup) {
            reasons.add(DataDisallowedReasonType.NOT_ATTACHED);
        }
        if (!(imsiReady != 0 || (isNeedForceSetup(apnContext) ^ 1) == 0 || (isDataAllowedVoWiFi ^ 1) == 0)) {
        ...
        if (!isDataAllowedForRoaming(isMmsApn(apnContext))) {
            if (!(((isXcapApn(apnContext) ? getXcapDataRoamingEnable() : 0) ^ 1) == 0 || !this.mPhone.getServiceState().getDataRoaming() || (getDataRoamingEnabled() ^ 1) == 0)) {
        ...
        if (dataSub == 1) {
            if (isDataConnectivityDisabled(1, "disable-data")) {
                reasons.add(DataDisallowedReasonType.INTERNAL_DATA_DISABLED);
        ...
        if (!(apnContext == null || (reasons.allowed() ^ 1) == 0 || !getAnyDataEnabledByApnContext(apnContext, false))) {
            reasons.add(DataAllowedReasonType.NORMAL);
        }
        if (isMmsApn(apnContext) && this.mPhone.getServiceState().getDataRoaming() && isRoamingPushDisabled()) {
            reasons.add(DataDisallowedReasonType.ROAMING_DISABLED);
        }
        if (reasons.allowed()) {
            reasons.add(DataAllowedReasonType.NORMAL);
        ...
        if (!reasons.allowed()) {
            dataAllowedByApnContext = false;
        }
        return dataAllowedByApnContext;
    }
    ...
    boolean isEmergency() {
        boolean isInEmergencyCall = !this.mPhone.isInEcm() ? this.mPhone.isInEmergencyCall() : true;
        ...
        for (ApnContext apnContext : this.mApnContexts.values()) {
            switch (-getcom-android-internal-telephony-DctConstants$StateSwitchesValues()[apnContext.getState().ordinal()]) {
                ...
                default:
                    ...
                    ApnSetting apnSetting = apnContext.getApnSetting();
                    if (!(apnSetting == null || !apnSetting.isMetered(this.mPhone) || (apnContext.getApnType().equals("xcap") ^ 1) == 0)) {
    ...
    public ApnSetting fetchDunApn() {
        ...
        disableGoogleDunApn(this.mPhone.getContext(), apnData, apnSetting);
        log("fetchDunApn: dunSetting=" + apnSetting);
        return apnSetting;
    }
    ...
    private void setInitialAttachApn() {
        ...
        if (get4gSlot() != this.mPhone.getSubId() && (HwModemCapability.isCapabilitySupport(21) ^ 1) != 0) {
            ...
            if (esmFlag == 0 && this.mPreferredApn != null) {
                if ((isApnPreset(this.mPreferredApn) ^ 1) != 0 && this.mPreferredApn.canHandleType("ia")) {
    ...
    private void onDataRoamingOnOrSettingsChanged(int messageType) {
        ...
        if (this.mHwCustDcTracker != null && settingChanged && getDataRoamingEnabled()) {
        ...
        if (apnContext != null) {
            if (apnContext.isLastApnSetting()) {
                ...
                if (SystemProperties.getBoolean("ro.config.hw_enable_ota_bip_lgu", false) && "bip0".equals(apnContext.getApnType())) {
                    ...
                    if (hasMessages(270385)) {
                        removeMessages(270385);
                    }
    ...
    private void onDisconnectDone(AsyncResult ar) {
        ...
        if (apnContext != null) {
            ...
            if (this.mAttached.get() && apnContext.isReady() && retryAfterDisconnected(apnContext)) {
                ...
                boolean restartRadioAfterProvisioning = this.mPhone.getContext().getResources().getBoolean(17957004);
    ...
    private ArrayList<ApnSetting> buildWaitingApns(String requestedApnType, int radioTech) {
        ...
        String operator = getCTOperator(getOperatorNumeric());
        try {
            usePreferred = this.mPhone.getContext().getResources().getBoolean(17956931) ^ 1;
        ...
        if (usePreferred && this.mCanSetPreferApn && this.mPreferredApn != null && this.mPreferredApn.canHandleType(requestedApnType) && r10 && (isNeedFilterVowifiMmsForPrefApn ^ 1) != 0) {
        ...
        if (this.mAllApnSettings == null || (this.mAllApnSettings.isEmpty() ^ 1) == 0) {
            ...
            for (ApnSetting apn : this.mAllApnSettings) {
                ...
                if (!apn.canHandleType(requestedApnType) || !r10 || (isNeedFilterVowifiMms ^ 1) == 0) {
                    log("buildWaitingApns: couldn't handle requested ApnType=" + requestedApnType);
                } else if (WAP_APN.equals(apn.apn) && isApnPreset(apn) && "default".equals(requestedApnType)) {
            ...
            if (apnType.equals("default") || "hipri".equals(apnType)) {
                ...
                if (enabled != 1) {
                    ...
                    if (cm != null) {
                        ...
                        log("networkInfo = " + networkInfo + " activeNetworkInfo = " + activeNetworkInfo);
                    }
                    if (!apnContext.isDisconnected() && (this.mIsWifiConnected || r3)) {
    ...
    boolean isNeedFilterVowifiMms(ApnSetting apn, String requestedApnType) {
        return "mms".equals(requestedApnType) ? apn != null ? ArrayUtils.contains(apn.types, APN_TYPE_VOWIFIMMS) : false : false;
    }

    private boolean isMmsApn(ApnContext apnContext) {
        if (apnContext != null) {
            return "mms".equals(apnContext.getApnType());
        }
        return false;
    }
    ...
    private boolean isXcapApn(ApnContext apnContext) {
        if (apnContext != null) {
            return "xcap".equals(apnContext.getApnType());
        }
        return false;
    }
    ...
    private boolean isNeedForceSetup(ApnContext apnContext) {
        ...
        if (PhoneInternalInterface.REASON_DATA_ENABLED.equals(apnContext.getReason()) && this.mPhone.getServiceState().getVoiceRegState() == 0) {

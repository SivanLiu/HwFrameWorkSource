...
public class DataConnection extends StateMachine {
        ...
        protected void unwanted() {
            if (DataConnection.this.mNetworkAgent != this) {
                ...
                for (ConnectionParams cp : DataConnection.this.mApnContexts.values()) {
                    ...
                    if (DataConnection.this.mDct.isBtConnected() || DataConnection.this.mDct.isWifiConnect()) {
    ...
    private void onConnect(ConnectionParams cp) {
        ...
        boolean dataRoaming = !this.mPhone.getDataRoamingEnabled() ? isModemRoaming ? this.mPhone.getServiceState().getDataRoaming() ^ 1 : false : true;
        this.mPhone.mCi.setupDataCall(cp.mRilRat, dp, isModemRoaming, dataRoaming, msg);
    }
    ...
    private void updateTcpBufferSizes(int rilRat) {
        ...
        if (!(rilRat == 7 || rilRat == 8)) {
            ...
            configOverride = this.mPhone.getContext().getResources().getStringArray(17236017);
    ...
    NetworkCapabilities getNetworkCapabilities() {
        ...
        if (this.mConnectionParams == null || this.mConnectionParams.mApnContext == null || this.mDct == null) {
            isBipNetwork = 0;
        } else {
            isBipNetwork = this.mDct.isBipApnType(this.mConnectionParams.mApnContext.getApnType());
        }
        if (this.mApnSetting != null) {
            ...
            for (String type : types) {
                boolean shouldDropMeterApn;
                if (this.mRestrictedNetworkOverride || this.mConnectionParams == null || !this.mConnectionParams.mUnmeteredUseOnly) {
                    shouldDropMeterApn = false;
                } else {
                    shouldDropMeterApn = ApnSetting.isMeteredApnType(type, this.mPhone);
                }
            ...
            if (this.mHwCustDataConnection != null ? this.mHwCustDataConnection.isNeedReMakeCapability() : false) {
        ...
        if ((this.mRestrictedNetworkOverride ? isBipNetwork ^ 1 : 0) != 0) {
    ...
    private boolean initConnection(ConnectionParams cp) {
        ...
        if (this.mApnSetting == null || !(this.mApnSetting.canHandleType(apnContext.getApnType()) || (r1 ^ 1) == 0)) {

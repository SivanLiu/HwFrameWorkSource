...
public class HwRILReferenceImpl extends Handler implements HwRILReference {
    ...
    public void riseCdmaCutoffFreq(boolean on, Message response) {
        RILRequestReference rr = RILRequestReference.obtain(524, response, this.mRILDefaultWorkSource);
    ...
    public void supplyDepersonalization(final String netpin, final int type, Message result) {
        invokeIRadio(8, result, new RILCommand() {
            public void excute(IRadio radio, int serial) throws RemoteException, RuntimeException {
                ...
                if (radio1_2 != null) {
                    radio1_2.supplyDepersonalization(serial, HwRILReferenceImpl.this.convertNullToEmptyString(netpin), type);
                } else {
                    HwRILReferenceImpl.this.riljLog("not support below radio 1.2");
                }
            }
        });
    }
    ...
    public void getCdmaChrInfo(Message result) {
        RILRequestReference rr = RILRequestReference.obtain(532, result, this.mRILDefaultWorkSource);
    ...
    public void testVoiceLoopBack(int mode) {
        RILRequestReference rr = RILRequestReference.obtain(531, null, this.mRILDefaultWorkSource);
    ...
    public void setHwRatCombineMode(int combineMode, Message result) {
        RILRequestReference rr = RILRequestReference.obtain(2072, result, this.mRILDefaultWorkSource);
        riljLog(rr.serialString() + "> " + requestToStringEx(rr.getRequest()));
        this.mRil.send(rr);
    }

    public void getHwRatCombineMode(Message result) {
        RILRequestReference rr = RILRequestReference.obtain(2073, result, this.mRILDefaultWorkSource);
        riljLog(rr.serialString() + "> " + requestToStringEx(rr.getRequest()));
        this.mRil.send(rr);
    }

    public void setHwRFChannelSwitch(int rfChannel, Message result) {
        RILRequestReference rr = RILRequestReference.obtain(2107, result, this.mRILDefaultWorkSource);
        riljLog(rr.serialString() + "> " + requestToStringEx(rr.getRequest()));
        this.mRil.send(rr);
    }
    ...
    public void invokeIRadio(int requestId, Message result, RILCommand cmd) {
        ...
        if (radioProxy != null) {
            ...
            try {
                ...
                RIL ril2 = this.mRil;
                ril = this.mRil;
                ril2.handleRadioProxyExceptionForRREx(RIL.requestToString(requestId), e, rr);
                return;
            }
        }
        rr.onError(1, null);

...
public class BipProxy extends Handler {
    ...
    private static final boolean IS_VZ = ("389".equals(SystemProperties.get("ro.config.hw_opta")) ? "840".equals(SystemProperties.get("ro.config.hw_optb")) : false);
    ...
    class TcpClientChannel implements BipChannel {
        ...
        public void receive(CatCmdMessage cmdMsg) {
            ...
            if (requested > 0) {
                ...
                System.arraycopy(this.mRxBuf, this.mRxPos, bArr, 0, requested);
                this.mRxPos += requested;
            }
            BipProxy.this.mStkService.sendTerminalResponse(cmdMsg.mCmdDet, result, false, 0, new ReceiveDataResponseData(bArr, available));
    ...
    private boolean setupSpecificPdpConnection(CatCmdMessage cmdMsg) throws ConnectionSetupFailedException {
        ...
        if (newChannel.networkAccessName == null) {
            ...
            boolean kddiBipOtaEnable = this.mHwCustBipProxy != null ? this.mHwCustBipProxy.kddiBipOtaEnable() : false;
            if (newChannel.userLogin == null) {
                newChannel.userLogin = kddiBipOtaEnable ? "au" : "";
            }
            if (newChannel.userPassword == null) {
                newChannel.userPassword = kddiBipOtaEnable ? "au" : "";
            }
            this.mChannelApnInfo[newChannel.channel - 1] = new ChannelApnInfo(newChannel.channel, cmdMsg);
            this.mIsWifiConnected[newChannel.channel - 1] = cmdMsg.getWifiConnectedFlag();
            if (kddiBipOtaEnable) {

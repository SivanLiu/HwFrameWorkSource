...
public class UiccCard extends AbstractUiccCard {
    ...
    public void update(Context c, CommandsInterface ci, IccCardStatus ics) {
        synchronized (this.mLock) {
            ...
            HwTelephonyFactory.getHwUiccManager().isGoingToshowCountDownTimerDialog(radioState, this.mLastRadioState, oldState, this.mCardState, this.mHandler, this.mPhoneId);
    ...
    private void onIccSwap(boolean isAdded) {
        if (this.mContext.getResources().getBoolean(17956981)) {
    ...
    private void promptForRestart(boolean isAdded) {
        ...
        synchronized (this.mLock) {
            String dialogComponent = this.mContext.getResources().getString(17039799);
    ...
    private void promptInstallCarrierApp(String pkgName) {
        ...
        String message = r.getString(17039718);
        AlertDialog dialog = new Builder(this.mContext).setMessage(message).setNegativeButton(r.getString(17039719), listener).setPositiveButton(r.getString(17039717), listener).create();
    ...
    public void iccOpenLogicalChannel(String AID, byte p2, Message response) {
        if (Log.HWINFO) {
            loglocal("Open Logical Channel with p2: " + AID + " by pid:" + Binder.getCallingPid() + " uid:" + Binder.getCallingUid());
        }
        this.mCi.iccOpenLogicalChannel(AID, p2, this.mHandler.obtainMessage(15, response));
    }

...
public class GsmCdmaPhone extends AbstractGsmCdmaPhone {
    ...
    private void initRatSpecific(int precisePhoneType) {
        ...
        if ((this.mUiccController.getUiccCardApplication(this.mPhoneId, 1) == null || isPhoneTypeCdmaLte()) && (r0 ^ 1) != 0) {
    ...
    public Connection dial(String dialString, UUSInfo uusInfo, int videoState, Bundle intentExtras) throws CallStateException {
        if (isPhoneTypeGsm() || uusInfo == null) {
            ...
            if (imsPhone != null && isEmergency && alwaysTryImsForEmergencyCarrierConfig && ImsManager.isNonTtyOrTtyOnVolteEnabled(this.mContext) && imsPhone.isImsAvailable()) {
            ...
            endsWith = endsWith ? ImsPhoneMmiCode.isVirtualNum(dialPart) ^ 1 : false;
            ...
            if ((isPhoneTypeGsm() || isCarrierSupportVolte) && ((imsUseEnabled && (!endsWith || isUtEnabled)) || useImsForEmergency)) {
                try {
            ...
            boolean shouldRunUtIgnoreCsService = HwTelephonyFactory.getHwPhoneManager().shouldRunUtIgnoreCSService(this, endsWith);
            ...
            if (this.mSST != null && this.mSST.mSS.getState() == 1 && this.mSST.mSS.getDataRegState() != 0 && (isEmergency ^ 1) != 0 && (shouldRunUtIgnoreCsService ^ 1) != 0) {
            ...
            } else if (this.mSST == null || this.mSST.mSS.getState() != 3 || (VideoProfile.isVideo(videoState) ^ 1) == 0 || (isEmergency ^ 1) == 0 || (endsWith ^ 1) == 0) {
                if (this.mSST != null && this.mSST.mSS.getState() == 1) {
                    ...
                    if (!((isLte ^ 1) == 0 || (VideoProfile.isVideo(videoState) ^ 1) == 0 || (isEmergency ^ 1) == 0 || (shouldRunUtIgnoreCsService ^ 1) == 0)) {
    ...
    public boolean isNotificationOfWfcCallRequired(String dialString) {
        ...
        if (config != null) {
            z = config.getBoolean("notify_international_call_on_wfc_bool");
        } else {
            z = false;
        }
        if (!z) {
    ...
    public String getVoiceMailNumber() {
        ...
        if (!isPhoneTypeGsm() && TextUtils.isEmpty(number)) {
            if (getContext().getResources().getBoolean(17957043)) {
    ...
    private void syncClirSetting() {
        ...
        Rlog.i(this.LOG_TAG, "syncClirSetting: clir_key" + getPhoneId() + "=" + clirSetting);
        if (clirSetting >= 0) {
    ...
    private void handleEventModemReset(Message msg) {
        ...
        if (!isPhoneTypeGsm() || (HwCustUtil.isVZW ^ 1) == 0) {
        ...
        if (!beforeHandleMessage(msg)) {
            ...
            switch (msg.what) {
                ...
                case 42:
                    ...
                    break;
                case 43:
                    if (!this.mContext.getResources().getBoolean(17957041)) {
                        this.mCi.getVoiceRadioTechnology(obtainMessage(40));
                    }
    ...
    private boolean isManualSelProhibitedInGlobalMode() {
        ...
        String configString = getContext().getResources().getString(17040854);
    ...
    public void exitEmergencyCallbackMode() {
        Rlog.d(this.LOG_TAG, "exitEmergencyCallbackMode: mImsPhone=" + this.mImsPhone + " isPhoneTypeGsm=" + isPhoneTypeGsm());
        if (!isPhoneTypeGsm()) {
    ...
    private void handleEnterEmergencyCallbackMode(Message msg) {
        Rlog.d(this.LOG_TAG, "handleEnterEmergencyCallbackMode, isInEcm()=" + isInEcm());
        if (!isInEcm()) {
            setIsInEcm(true);
    ...
    private void handleExitEmergencyCallbackMode(Message msg) {
        ...
        Rlog.d(this.LOG_TAG, "handleExitEmergencyCallbackMode,ar.exception , isInEcm=" + ar.exception + isInEcm());
        removeCallbacks(this.mExitEcmRunnable);

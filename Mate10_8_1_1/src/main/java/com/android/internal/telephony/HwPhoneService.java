...
public class HwPhoneService extends Stub implements PhoneServiceInterface {
    ...
    private static final int CMD_SET_DEEP_NO_DISTURB = 202;
    ...
    private static final int EVENT_SET_DEEP_NO_DISTURB_DONE = 203;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case HwPhoneService.EVENT_SET_CALLFORWARDING_DONE /*118*/:
                    ...
                    HwPhoneService.this.handleSetCallforwardDone(msg);
                    return;
                ...
                case HwPhoneService.EVENT_SET_4G_SLOT_DONE /*200*/:
                    HwPhoneService.this.log("EVENT_SET_4G_SLOT_DONE");
                    HwPhoneService.this.handleSet4GSlotDone(msg);
                    return;
                case HwPhoneService.EVENT_SET_SUBSCRIPTION_DONE /*201*/:
                    HwPhoneService.this.log("EVENT_SET_SUBSCRIPTION_DONE");
                    HwPhoneService.this.handleSetSubscriptionDone(msg);
                    return;
                case 500:
    ...
    private String getDeviceIdFromSP(String sharedPref) {
        ...
        try {
            deviceId = HwAESCryptoUtil.decrypt(HwAllInOneController.MASTER_PASSWORD, deviceId);
        } catch (Exception e) {
    ...
    private void handleSetCallforwardDone(Message msg) {
        AsyncResult ar = msg.obj;
        if (ar != null) {
            ...
            if (!(cbMsg == null || cbMsg.replyTo == null)) {
                ...
                try {
                    ...
                    Rlog.e(LOG_TAG, "EVENT_SET_CALLFORWARDING_DONE RemoteException:" + ex);
                }
            }
        }
    }
    ...
    private void handleSet4GSlotDone(Message msg) {
        AsyncResult ar = msg.obj;
        if (ar != null) {
            Message cbMsg = ar.userObj;
            if (!(cbMsg == null || cbMsg.replyTo == null)) {
                ...
                try {
                    ...
                    Rlog.e(LOG_TAG, "EVENT_SET_4G_SLOT_DONE RemoteException:" + ex);
                }
            }
        }
    }

    private void handleSetSubscriptionDone(Message msg) {
        AsyncResult ar = msg.obj;
        if (ar != null) {
            Message cbMsg = ar.userObj;
            if (!(cbMsg == null || cbMsg.replyTo == null)) {
                ...
                try {
                    ...
                    Rlog.e(LOG_TAG, "EVENT_SET_SUBSCRIPTION_DONE RemoteException:" + ex);
                }
            }
        }
    }
    ...
    public String getLine1NumberFromImpu(int subId) {
        ...
        if (TextUtils.isEmpty(number) || (number.equals(getPhone(subId).getSubscriberId()) ^ 1) == 0) {
            return null;
        }
        return number;
    }
    ...
    public int invokeOemRilRequestRaw(int phoneId, byte[] oemReq, byte[] oemResp) {
        ...
        if (phoneId < 0 || phoneId >= this.mPhones.length) {
            ...
            loge("oemReq or oemResp is null!");
            return -1;
        } else {
            int returnValue = 0;
            try {
                ...
                if (result.exception != null) {
                    ...
                    loge("invokeOemRilRequestRaw fail exception + returnValue:" + returnValue);
                    if (returnValue > 0) {
                        returnValue *= -1;
                    }
                    ...
                    log("invokeOemRilRequestRaw success, returnValue" + returnValue);
                } else {
                    loge("invokeOemRilRequestRaw fail result.result is null");
                }
            } catch (RuntimeException e) {
                loge("sendOemRilRequestRaw: Runtime Exception");
    ...
    private void handleCmdOemRilRequestRaw(Message msg) {
        ...
        if (request.subId != null) {
            ...
            phone = this.mPhones[phoneId].getPhone();
            if (phone != null) {
                phone.invokeOemRilRequestRaw((byte[]) request.argument, onCompleted);
                log("invokeOemRilRequestRaw success by phoneId=" + phoneId);
                return;
            }
        }
        phone = this.mPhone.getPhone();
        if (phone != null) {
            phone.invokeOemRilRequestRaw((byte[]) request.argument, onCompleted);
            log("invokeOemRilRequestRaw with default phone");
        }
    }
    ...
    public boolean isSecondaryCardGsmOnly() {
        ...
        enforceReadPermission();
        TelephonyManager mTelephonyManager = (TelephonyManager) this.mContext.getSystemService("phone");
        if (mTelephonyManager != null && (mTelephonyManager.isMultiSimEnabled() ^ 1) != 0) {
            return false;
        }
        if (HwAllInOneController.IS_CMCC_4GSWITCH_DISABLE) {
            z = HwAllInOneController.getInstance().isCMCCHybird();
        }
        return z;
    }

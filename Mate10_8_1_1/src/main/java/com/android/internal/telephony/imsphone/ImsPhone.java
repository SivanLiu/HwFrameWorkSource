...
public class ImsPhone extends ImsPhoneBase {
    ...
    public void setServiceState(int state) {
        synchronized (this) {
            boolean isVoiceRegStateChanged = this.mSS.getVoiceRegState() != state;
    ...
    public void getCallBarring(String facility, Message onComplete) {
        ...
        try {
            this.mCT.getUtInterface().queryCallBarring(getCBTypeFromFacility(facility), obtainMessage(48, onComplete));
        } catch (ImsException e) {
            sendErrorResponse(onComplete, e);
        }
    }
    ...
    private void processWfcDisconnectForNotification(ImsReasonInfo imsReasonInfo) {
        ...
        if (wfcOperatorErrorCodes != null) {
            String[] wfcOperatorErrorAlertMessages = this.mContext.getResources().getStringArray(17236074);
            String[] wfcOperatorErrorNotificationMessages = this.mContext.getResources().getStringArray(17236075);
            for (int i = 0; i < wfcOperatorErrorCodes.length; i++) {
                ...
                if (codes.length != 2) {
                    ...
                    if (!Character.isLetterOrDigit(codes[0].charAt(codeStringLength - 1)) || imsReasonInfo.mExtraMessage.length() <= codeStringLength || !Character.isLetterOrDigit(imsReasonInfo.mExtraMessage.charAt(codeStringLength))) {
                        CharSequence title = this.mContext.getText(17041231);

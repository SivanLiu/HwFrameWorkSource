...
public final class SmsApplication {
    ...
    private static Collection<SmsApplicationData> getApplicationCollectionInternal(Context context, int userId) {
        ...
        for (ResolveInfo resolveInfo22 : packageManager.queryIntentServicesAsUser(new Intent(TelephonyManager.ACTION_RESPOND_VIA_MESSAGE, Uri.fromParts("smsto", "", null)), 0, userId)) {
        ...
        for (ResolveInfo resolveInfo222 : packageManager.queryIntentActivitiesAsUser(new Intent("android.intent.action.SENDTO", Uri.fromParts("smsto", "", null)), 0, userId)) {
        ...
        for (ResolveInfo resolveInfo222222 : packageManager.queryBroadcastReceiversAsUser(new Intent(Intents.SIM_FULL_ACTION), 0, userId)) {
            ...
            if (activityInfo != null) {
                smsApplicationData2 = (SmsApplicationData) receivers.get(activityInfo.packageName);
                if (smsApplicationData2 != null) {
                    smsApplicationData2.mSimFullReceiverClass = activityInfo.name;
                }
            }
        }

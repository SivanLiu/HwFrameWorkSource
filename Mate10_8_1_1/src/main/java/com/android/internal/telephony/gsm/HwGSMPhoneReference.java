...
public class HwGSMPhoneReference extends HwPhoneReferenceBase implements GSMPhoneReference {
    ...
    private String getSelectedApn() {
        ...
        Cursor cursor;
        if (isMultiEnable) {
            cursor = context.getContentResolver().query(ContentUris.withAppendedId(PREFERAPN_NO_UPDATE_URI, (long) this.mPhone.getSubId()), new String[]{"_id", NumMatchs.NAME, "apn"}, null, null, NumMatchs.DEFAULT_SORT_ORDER);
        } else {
            cursor = context.getContentResolver().query(PREFERAPN_NO_UPDATE_URI, new String[]{"_id", NumMatchs.NAME, "apn"}, null, null, NumMatchs.DEFAULT_SORT_ORDER);
        }
    ...
    public String getCustOperatorName(String rplmn) {
        ...
        Context context = this.mPhone != null ? this.mPhone.getContext() : null;
        Object string = context != null ? System.getString(context.getContentResolver(), "hw_cust_operator") : null;
    ...
    public void updateReduceSARState() {
        ...
        if (!reduceOldSar() && SystemProperties.getBoolean("ro.config.hw_ReduceSAR", false)) {
            ...
            boolean z = isWiredHeadsetOn || isSpeakerphoneOn || state != 2;
            if (SystemProperties.getBoolean("persist.gsm.ReceiveTestMode", false)) {
                z = SystemProperties.getBoolean("persist.gsm.ReceiveTestValue", false);
                logd("hw_ReceiveTestMode = true isReceiveOFF = " + z);
            }
            int reduceData = getGradeData(z)[cardType][wifiType];
    ...
    public void globalEccCustom(String operatorNumeric) {
        ...
        if (!(TextUtils.isEmpty(operatorNumeric) || (operatorNumeric.equals(this.preOperatorNumeric) && (forceEccState.equals("invalid") ^ 1) == 0))) {
            ...
            if ((str == null || (str.equals("") ^ 1) == 0) && (str2 == null || (str2.equals("") ^ 1) == 0)) {
                this.mPhone.mCi.requestSetEmergencyNumbers("", "");
            } else {
    ...
    public boolean isUssdOkForRelease() {
        ...
        CarrierConfigManager configLoader = (CarrierConfigManager) context.getSystemService("carrier_config");

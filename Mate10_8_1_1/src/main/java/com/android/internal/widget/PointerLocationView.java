    ...
    public void onPointerEvent(MotionEvent event) {
        ...
        for (int historyPos = 0; historyPos < N; historyPos++) {
            for (i = 0; i < NI; i++) {
                ...
                PointerCoords coords = ps != null ? ps.mCoords : this.mTempCoords;
        ...
        if (action == 1 || action == 3 || (action & 255) == 6) {
            ...
            id = event.getPointerId(index);
            if (id >= NP) {
                Slog.wtf(TAG, "Got pointer ID out of bounds: id=" + id + " arraysize=" + NP + " pointerindex=" + index + " action=0x" + Integer.toHexString(action));
                return;

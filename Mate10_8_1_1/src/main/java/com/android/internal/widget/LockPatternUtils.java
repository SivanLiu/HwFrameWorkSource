...
public class LockPatternUtils {
    ...
    public long getLockoutAttemptDeadline(int userId) {
        ...
        if (deadline >= now || deadline == 0) {
            if (deadline > (mCustLockPatternUtils.needLimit() ? FAILED_ATTEMPT_TIMEOUT_MS : timeoutMs) + now) {
        ...
        if (this.mHandler != null) {
            return new ICheckCredentialProgressCallback.Stub() {
                public void onCredentialVerified() throws RemoteException {
                    ...
                    -get0.post(new -$Lambda$hZenqyGYSNt5KiruOSsv736MIDs((byte) 1, checkCredentialProgressCallback));
                }
            };
        }

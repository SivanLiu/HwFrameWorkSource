...
public class MediaRouteChooserDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isLightTheme(getContext())) {
            i = R.drawable.ic_media_route_off_holo_light;
        } else {
            i = R.drawable.ic_media_route_off_holo_dark;
        }
    ...
    static boolean isLightTheme(Context context) {
        ...
        if (!context.getTheme().resolveAttribute(R.attr.isLightTheme, value, true)) {
            return false;
        }
        if (value.data != 0) {
            return true;

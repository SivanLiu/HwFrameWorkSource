...
public class PhoneWindow extends AbsWindow implements Callback {
    ...
    private static final boolean NAVIGATION_BAR_IS_BLACK_PROP = SystemProperties.getBoolean("ro.config.hw_navbar_is_black", true);
...
*/
    ...
    void onOptionsPanelRotationChanged() {
        PanelFeatureState st = getPanelState(0, false);
        if (st != null) {
            LayoutParams layoutParams = st.decorView != null ? (WindowManager.LayoutParams) st.decorView.getLayoutParams() : null;
            if (layoutParams != null) {
                layoutParams.gravity = getOptionsPanelGravity();
                ViewManager wm = getWindowManager();
                if (wm != null) {
                    wm.updateViewLayout(st.decorView, layoutParams);
                }
            }
        }
    }

...
public class CertificateMonitor {
    ...
    /* synthetic */ void lambda$-com_android_server_devicepolicy_CertificateMonitor_5625(int userId) {
        ...
        if (this.mInjector.getUserManager().isUserUnlocked(userHandle.getIdentifier())) {
            try {
                ...
                if (pendingCertificateCount != 0) {
                    ...
                    if (mHwCustDevicePolicyManagerService == null || (mHwCustDevicePolicyManagerService.isCertNotificationAllowed(this.notAllowedApp) ^ 1) == 0) {
    ...
    private Notification buildNotification(UserHandle userHandle, int pendingCertificateCount) {
        try {
            ...
            if (this.mService.getProfileOwner(userHandle.getIdentifier()) != null) {
                contentText = resources.getString(17041055, new Object[]{this.mService.getProfileOwnerName(userHandle.getIdentifier())});
                smallIconId = 17303488;
                ...
                contentText = resources.getString(17041055, new Object[]{this.mService.getDeviceOwnerName()});
                smallIconId = 17303488;
                this.notAllowedApp = this.mService.getDeviceOwnerName();
            } else {
                contentText = resources.getString(17041054);
            ...
            return new Builder(userContext, SystemNotificationChannels.SECURITY).setSmallIcon(smallIconId).setContentTitle(resources.getQuantityText(18153499, pendingCertificateCount)).setContentText(contentText).setContentIntent(this.mInjector.pendingIntentGetActivityAsUser(userContext, 0, dialogIntent, 134217728, null, UserHandle.of(parentUserId))).setShowWhen(false).setColor(17170772).build();

...
public class DeviceRestrictionPlugin extends DevicePolicyPlugin {
    ...
    public boolean checkCallingPermission(ComponentName who, String policyName) {
        ...
        } else if (policyName.equals("disable-headphone") || policyName.equals("disable-microphone") || policyName.equals("disable-send-notification") || policyName.equals(DISABLE_CHANGE_WALLPAPER) || policyName.equals("disable-charge") || policyName.equals("force_scheduled_power_on") || policyName.equals("force_scheduled_power_off")) {
    ...
    public boolean onSetPolicy(ComponentName who, String policyName, Bundle policyData, boolean effective) {
        ...
        try {
            if (policyName.equals(DISABLE_SYSTEM_UPDATE)) {
                if (effective) {
                    z = disableSystemUpdate(policyData.getBoolean("value"));
                }
                ...
                if (!(list == null || list.size() == 0)) {
                    ...
                    for (int i = 0; i < j; i++) {
                        ...
                        z = disableComponentForPackage(app, true, pm, 0);
                ...
                synchronized (this) {
                    z = updateScreenCaptureDisabledInWindowManager(userHandle, policyData.getBoolean("value"));
                }
            } else if (!policyName.equals("disable-clipboard")) {
                if (policyName.equals("disable-google-account-autosync")) {
                    ...
                    z = changeNavigationBarStatus(UserHandle.getCallingUserId(), policyData.getBoolean("value"));
                } else if (policyName.equals("disable-charge")) {
                    if (policyData.getBoolean("value", false)) {
                        enableCharging(false);
                    } else {
                        enableCharging(true);
                    }
                } else if (policyName.equals("force_scheduled_power_on")) {
                    if (policyData.getBoolean("value", false)) {
                ...
                } else if (policyName.equals("force_scheduled_power_off")) {
                    if (policyData.getBoolean("value", false)) {

...
class BrightnessSeekBarMonitor implements MonitorModule {
    ...
    public boolean isParamOwner(String paramType) {
        if (paramType.equals(TYPE_TEMP_MANUAL_BRIGHTNESS) || paramType.equals(TYPE_TEMP_AUTO_BRIGHTNESS) || paramType.equals(TYPE_ALGO_DEFAULT_BRIGHTNESS)) {
            return true;
        }
        return false;
    }
    ...
    private void recordSeekBarMovement(boolean isAutoMode, int level) {
        if ((isAutoMode || (this.mIsManualModeReceiveAmbientLight ^ 1) == 0) && !this.mCommonData.isWindowManagerBrightnessMode()) {
    ...
    private void recordSeekBarMovementEnd(SeekBarMovement movement, boolean isAutoMode) {
        if (HWFLOW) {
            Slog.i(TAG, "recordSeekBarMovementEnd() " + (isAutoMode ? "AUTO" : "MANUAL") + " " + movement);
        }
        if (movement.startLevel == movement.endLevel) {
            Object movement2 = null;
        }
        if (isAutoMode) {
            if (movement2 != null) {
        ...
        if (movementList.size() > 1) {
            ...
            while (it.hasNext()) {
                ...
                if (isMerge2Movement(pre, now)) {
                    if (HWDEBUG) {
                        Slog.d(TAG, "checkMovementListInterval() merge " + pre + " and " + now);
                    }

...
final class WifiDisplayAdapter extends DisplayAdapter {
        ...
        public void onDisplayConnecting(WifiDisplay display) {
            synchronized (WifiDisplayAdapter.this.getSyncRoot()) {
                ...
                if (WifiDisplayAdapter.this.mActiveDisplayState == 1 && WifiDisplayAdapter.this.mActiveDisplay != null) {
                    if ((WifiDisplayAdapter.this.mActiveDisplay.equals(display) ^ 1) != 0) {
                    }
                }
    ...
    private final class WifiDisplayDevice extends DisplayDevice {
        ...
        public void performTraversalInTransactionLocked() {
            ...
            if (HwFrameworkFactory.getVRSystemServiceManager().isVRDisplayConnected()) {
                int vrLayerStackId = HwVRUtils.getVRDisplayID();
                if (vrLayerStackId > 0) {
                    Slog.i(WifiDisplayAdapter.TAG, "performTraversalInTransactionLocked: setDisplayLayerStack VR layer stack id:" + vrLayerStackId);
                ...
                if (HwPCUtils.isPcCastModeInServer()) {
                    Slog.w(WifiDisplayAdapter.TAG, "performTraversalInTransactionLocked: setDisplayLayerStack BUILT_IN_DISPLAY_ID_HDMI");
                    SurfaceControl.setDisplayLayerStack(getDisplayTokenLocked(), 1);
                    return;
                }
                SurfaceControl.setDisplayLayerStack(getDisplayTokenLocked(), 0);
            }
    ...
    public WifiDisplayAdapter(SyncRoot syncRoot, Context context, Handler handler, DisplayAdapter.Listener listener, PersistentDataStore persistentDataStore) {
        ...
        this.mSupportsProtectedBuffers = context.getResources().getBoolean(17957061);
    }

...
public final class HwNormalizedRampAnimator<T> extends RampAnimator<T> {
    ...
    private void updateHBMData(int target, int rate, float duration) {
        if (this.mHBMData.getInt("target") == target && this.mHBMData.getInt("rate") == rate) {
            if (((double) Math.abs(this.mHBMData.getFloat("duration") - duration)) <= 1.0E-6d) {
                return;
            }
        }
        ...
        Slog.d(this.TAG, "hbm_dimming target=" + this.mHBMData.getInt("target") + " rate=" + this.mHBMData.getInt("rate") + " duration=" + this.mHBMData.getFloat("duration"));

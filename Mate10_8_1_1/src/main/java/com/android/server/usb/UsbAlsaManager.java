...
public final class UsbAlsaManager {
        ...
        if (this.mAudioService == null) {
            ...
            try {
                ...
                if (audioDevice.mHasPlayback) {
                    ...
                    if (DEBUG) {
                        Slog.i(TAG, "pre-call device:0x" + Integer.toHexString(device) + " addr:" + address + " name:" + audioDevice.getDeviceName());
                    }
    ...
    UsbAudioDevice selectAudioCard(int card) {
        return selectAudioCard(card, this.mIsInputHeadset, this.mIsOutputHeadset);
    }

    UsbAudioDevice selectAudioCard(int card, boolean isInputHeadset, boolean isOutputHeadset) {
        if (DEBUG) {
        ...
        audioDevice.setHeadsetStatus(isInputHeadset, isOutputHeadset);
    ...
    void usbDeviceAdded(UsbDevice usbDevice, boolean isInputHeadset, boolean isOutputHeadset) {
        ...
        if (isAudioDevice) {
            if (notifyUsbDeviceAdded(usbDevice, isInputHeadset, isOutputHeadset)) {
    ...
    boolean notifyUsbDeviceAdded(UsbDevice usbDevice, boolean isInputHeadset, boolean isOutputHeadset) {
        ...
        UsbAudioDevice audioDevice = selectAudioCard(addedCard, isInputHeadset, isOutputHeadset);
        if (audioDevice != null) {
    ...
    void setPeripheralMidiState(boolean enabled, int card, int device) {
        if (this.mHasMidiFeature) {
            if (enabled && this.mPeripheralMidiDevice == null) {
                ...
                properties.putString("name", r.getString(17041162));
                properties.putString("manufacturer", r.getString(17041161));
                properties.putString("product", r.getString(17041163));

...
public class UsbHostManager {
    ...
    public UsbHostManager(Context context, UsbAlsaManager alsaManager, UsbSettingsManager settingsManager) {
        ...
        this.mHostBlacklist = context.getResources().getStringArray(17236043);
        this.mUsbAlsaManager = alsaManager;
        this.mSettingsManager = settingsManager;
        String deviceConnectionHandler = context.getResources().getString(17039752);
    ...
    private boolean beginUsbDeviceAdded(String deviceName, int vendorID, int productID, int deviceClass, int deviceSubclass, int deviceProtocol, String manufacturerName, String productName, int version, String serialNumber) {
        ...
        if (this.mContext != null && (isDeviceProvisioned(this.mContext) ^ 1) != 0 && isFrpRestricted(this.mContext)) {
            Slog.d(TAG, "usbDeviceAdded return, Global.DEVICE_PROVISIONED:0");
            return false;
        } else if (isBlackListed(deviceName) || isBlackListed(deviceClass, deviceSubclass)) {
            return false;
        } else {
            synchronized (this.mLock) {
    ...
    private void endUsbDeviceAdded() {
        ...
        synchronized (this.mLock) {
            if (this.mNewDevice != null) {
                ...
                boolean z = false;
                boolean z2 = false;
                boolean[] hasRawDescriptors = new boolean[]{true};
                if (parser.parseDevice(this.mNewDevice.getDeviceName(), hasRawDescriptors)) {
                    z = parser.isInputHeadset();
                    z2 = parser.isOutputHeadset();
    ...
    private void usbDeviceRemoved(String deviceName) {
        if (DEBUG) {
            Slog.d(TAG, "usbDeviceRemoved deviceName=" + deviceName);
        }
        synchronized (this.mLock) {
    ...
    public void systemReady() {
        synchronized (this.mLock) {
            new Thread(null, new -$Lambda$ATEm4_U2eaRn21cN0eCfdiUt5-k((byte) 0, this), "UsbService host thread").start();
        }
    }

    /* synthetic */ void -com_android_server_usb_UsbHostManager-mthref-0() {
        monitorUsbHostBus();
    }
    ...
    public void dump(IndentingPrintWriter pw) {
        synchronized (this.mLock) {
            ...
            Collection<UsbDevice> devices = this.mDevices.values();
            if (devices.size() != 0) {
                ...
                for (UsbDevice device : devices) {
                    ...
                    if (parser.parseDevice(device.getDeviceName())) {
                        ...
                        connection.close();
                        stringBuilder.append("isHeadset[in: ").append(parser.isInputHeadset()).append(" , out: ").append(parser.isOutputHeadset()).append("]");
                    } else {

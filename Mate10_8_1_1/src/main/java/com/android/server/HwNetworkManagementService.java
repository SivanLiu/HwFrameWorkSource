...
public class HwNetworkManagementService extends NetworkManagementService {
    ...
    private Handler mApLinkedStaHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            if (HwNetworkManagementService.this.mLinkedStaCount >= 0 && HwNetworkManagementService.this.mLinkedStaCount <= 8) {
                if (HwNetworkManagementService.this.mLinkedStaCount != HwNetworkManagementService.this.mMacList.size()) {
                }
    ...
    private final BroadcastReceiver mHwSystemServerStartReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            Slog.d(HwNetworkManagementService.TAG, "HwSystemServerStartReceiver action=" + action);
    ...
    private void handleRegisterBoosterCallback() {
        ...
        if (bm != null) {
            ...
            if (ret != 0) {
                Slog.e(TAG, "handleRegisterBoosterCallback:registerCallBack failed, ret=" + ret);
                return;
            }
            return;
        }
...
jadx.core.utils.exceptions.CodegenException: Error generate insn: PHI: (r15_6 'j' int) = (r15_0 'j' int), (r15_9 'j' int) binds: {(r15_0 'j' int)=B:198:0x057f, (r15_9 'j' int)=B:108:0x057c} in method: com.android.server.HwNetworkManagementService.onTransact(int, android.os.Parcel, android.os.Parcel, int):boolean
...
*/
                    ...
                    public void sendDSCPChangeMessage(String[] cooked, String raw) {
                        if (cooked.length < 4 || (cooked[1].equals("DSCPINFO") ^ 1) != 0) {
                            String msg1 = String.format("Invalid event from daemon (%s)", new Object[]{raw});
                    ...
                    private void setNetBoosterVodEnabled(boolean enable) {
                        if (this.mBoosterVideoAccEnabled) {
                            try {
                                Slog.d(TAG, "setNetBoosterVodEnabled enable=" + enable);
                    ...
                    private void setNetBoosterKsiEnabled(boolean enable) {
                        if (this.mBoosterNetAiChangeEnabled) {
                            try {
                                Slog.d(TAG, "setNetBoosterKsiEnabled enable=" + enable);
                                NativeDaemonConnector nativeDaemonConnector = this.mConnector;
                                String str = "hwnb";
                                Object[] objArr = new Object[2];
                                objArr[0] = "ksi";
                    ...
                    public void reportVodParams(int videoSegState, int videoProtocol, int videoRemainingPlayTime, int videoStatus, int aveCodeRate, int segSize, int flowInfoRemote, int flowInfoLocal, int segDuration, int segIndex) {
                        if (this.mBoosterVideoAccEnabled) {
                            Slog.d(TAG, "reportVodParams:videoSegState=" + videoSegState + ",videoProtocol=" + videoProtocol + ",videoRemainingPlayTime=" + videoRemainingPlayTime + ",videoStatus=" + videoStatus + ",aveCodeRate=" + aveCodeRate + ",segSize=" + segSize + ",segDuration=" + segDuration + ",segIndex=" + segIndex);
                            IHwCommBoosterServiceManager bm = HwFrameworkFactory.getHwCommBoosterServiceManager();
                            if (bm != null) {
                                ...
                                if (ret != 0) {
                                    Slog.e(TAG, "reportVodParams:reportBoosterPara failed, ret=" + ret);
                                }
                            } else {
                    ...
                    public void reportKsiParams(int slowType, int avgAmp, int duration, int timeStart) {
                        if (this.mBoosterNetAiChangeEnabled) {
                            Slog.d(TAG, "reportKsiParams:slowType=" + slowType + ",avgAmp=" + avgAmp + ",duration=" + duration + ",timeStart=" + timeStart);
                            IHwCommBoosterServiceManager bm = HwFrameworkFactory.getHwCommBoosterServiceManager();
                            if (bm != null) {
                                Bundle data = new Bundle();
                                ...
                                if (ret != 0) {
                                    Slog.e(TAG, "reportKsiParams:reportBoosterPara failed, ret=" + ret);
                                }
                            } else {

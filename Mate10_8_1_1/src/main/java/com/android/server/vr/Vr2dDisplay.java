...
class Vr2dDisplay {
    ...
    private void startVirtualDisplay() {
        ...
        synchronized (this.mVdLock) {
            ...
            this.mVirtualDisplay = this.mDisplayManager.createVirtualDisplay(null, DISPLAY_NAME, this.mVirtualDisplayWidth, this.mVirtualDisplayHeight, this.mVirtualDisplayDpi, null, ((192 | 1) | 8) | 256, null, null, UNIQUE_DISPLAY_ID);
            if (this.mVirtualDisplay != null) {
    ...
    private boolean shouldRunVirtualDisplay() {
        if (this.mIsVirtualDisplayAllowed) {
            return (this.mBootsToVr || this.mIsPersistentVrModeEnabled) ? true : this.mIsVrModeOverrideEnabled;
        } else {
            return false;
        }
    }

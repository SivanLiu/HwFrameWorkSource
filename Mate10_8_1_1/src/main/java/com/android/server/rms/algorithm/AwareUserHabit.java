...
public class AwareUserHabit {
    ...
    private class UserHabitHandler extends Handler {
        ...
        private void handleActivityChangedEvent(Message msg, String pkgName) {
            if (AwareUserHabit.this.mAwareUserHabitAlgorithm != null && pkgName != null) {
                ...
                AwareProcessBaseInfo processBaseInfo = AwareUserHabit.this.mHwAMS != null ? AwareUserHabit.this.mHwAMS.getProcessBaseInfo(homePkgPid) : null;
                if (isForeground) {
                    ...
                    if (!(homePkgPid == msg.arg1 || processBaseInfo == null || !AwareUserHabit.VISIBLE_ADJ_TYPE.equals(processBaseInfo.mAdjType))) {
                    ...
                    if (AwareUserHabit.this.mLauncherIsVisible && homePkgPid != msg.arg1 && processBaseInfo != null && processBaseInfo.mCurAdj == 0) {
    ...
    public String getHabitDfxData(boolean isClear) {
        ...
        float accuracy = GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
    ...
    public void dumpMostFreqAppByTypeEx(PrintWriter pw, String[] args) {
        if (args.length > 3) {
            try {
                ...
                try {
                    int appNum = Integer.parseInt(args[args.length - 2]);
                    int idx = 2 - 1;
                    try {
                        int dayNum = Integer.parseInt(args[args.length - 1]);
                        List<String> appList = getMostFreqAppByType(appType, 10000);
                        if (appList != null) {
    ...
    private List<String> getMostAppByUsedInfo(int appNum, List<String> mostUsedList, long dayTimes) {
        ...
        synchronized (this.mAppUseInfos) {
            ...
            for (int i = 0; i < length; i++) {
                String pkgName = (String) mostUsedList.get(i);
                Long switchFgTime = (Long) this.mAppUseInfos.get(pkgName);
                if (switchFgTime != null && switchFgTime.longValue() >= diffTime && switchFgTime.longValue() <= now) {
                    result.add(pkgName);

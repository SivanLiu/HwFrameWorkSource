...
public class HwSecurityDiagnosePlugin extends Stub implements IHwSecurityPlugin {
    ...
    private static final long DELAY_TRIGGER_ROOT_SCAN;
    ...
    static {
        ...
        HW_DEBUG = isLoggable;
        if (SystemProperties.get("ro.secure", "1").equals("1")) {
            j = 43200000;
        } else {
            j = MemoryConstant.MIN_INTERVAL_OP_TIMEOUT;
        }
        DELAY_TRIGGER_ROOT_SCAN = j;
    }
    ...
    private void setTriggerTime() {
        ...
        ((AlarmManager) this.mContext.getSystemService("alarm")).setRepeating(3, DELAY_TRIGGER_ROOT_SCAN + SystemClock.elapsedRealtime(), DELAY_TRIGGER_ROOT_SCAN, PendingIntent.getBroadcast(this.mContext, 0, new Intent(ROOT_ALARM_ACTION), 268435456));
    }
    ...
    private HwSecurityDiagnosePlugin(Context contxt) {
        ...
        this.mRootReportListener = new Listener() {
            public void onRootReport(JSONObject json, boolean needReport) {
                ...
                if (HwSecurityDiagnosePlugin.CHINA_RELEASE_VERSION && (needReport ^ 1) == 0) {

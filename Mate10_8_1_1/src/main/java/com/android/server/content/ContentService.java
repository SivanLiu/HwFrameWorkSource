...
public final class ContentService extends Stub {
    ...
    public static final class ObserverNode {
        ...
        public void dumpLocked(FileDescriptor fd, PrintWriter pw, String[] args, String name, String prefix, int[] counts, SparseIntArray pidCounts) {
            ...
            if (this.mObservers.size() > 0) {
                ...
                for (i = 0; i < this.mObservers.size(); i++) {
                    counts[1] = counts[1] + 1;
                    ((ObserverEntry) this.mObservers.get(i)).dumpLocked(fd, pw, args, str, prefix, pidCounts);
                }
            }
            if (this.mChildren.size() > 0) {
                ...
                for (i = 0; i < this.mChildren.size(); i++) {
                    counts[0] = counts[0] + 1;
                    ((ObserverNode) this.mChildren.get(i)).dumpLocked(fd, pw, args, str, prefix, counts, pidCounts);
                }
            }
        }
        ...
        private void collectMyObserversLocked(boolean leaf, IContentObserver observer, boolean observerWantsSelfNotifications, int flags, int targetUserHandle, ArrayList<ObserverCall> calls) {
            ...
            IBinder asBinder = observer == null ? null : observer.asBinder();
            for (int i = 0; i < N; i++) {
                ObserverEntry entry = (ObserverEntry) this.mObservers.get(i);
                boolean selfChange = entry.observer.asBinder() == asBinder;
                if ((!selfChange || (observerWantsSelfNotifications ^ 1) == 0) && !HwServiceFactory.getHwNLPManager().shouldSkipGoogleNlp(entry.pid)) {
                    if (!(targetUserHandle == -1 || entry.userHandle == -1)) {
                        if (targetUserHandle == entry.userHandle) {
                        }
                    }
                    if (leaf) {

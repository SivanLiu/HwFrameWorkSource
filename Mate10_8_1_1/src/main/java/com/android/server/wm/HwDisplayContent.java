...
public class HwDisplayContent extends DisplayContent {
    ...
    GraphicBuffer screenshotApplicationsToBufferForExternalDisplay(IBinder displayToken, IBinder appToken, int width, int height, boolean includeFullDisplay, float frameScale, boolean wallpaperOnly, boolean includeDecor) {
        return (GraphicBuffer) screenshotApplicationsForExternalDisplay(displayToken, appToken, width, height, includeFullDisplay, frameScale, wallpaperOnly, includeDecor, -$Lambda$gdyonVWUyLAUEjsalC_69kEf8YE.$INST$0);
    }
    ...
    private <E> E screenshotApplicationsForExternalDisplay(IBinder displayToken, IBinder appToken, int width, int height, boolean includeFullDisplay, float frameScale, boolean wallpaperOnly, boolean includeDecor, ScreenshoterForExternalDisplay<E> screenshoter) {
        ...
        synchronized (this.mService.mWindowMap) {
            AppWindowToken appWindowToken = this.mService.mInputMethodTarget != null ? this.mService.mInputMethodTarget.mAppToken : null;
            if (appWindowToken == null || appWindowToken.appToken == null || appWindowToken.appToken.asBinder() != appToken) {
            ...
            if (appToken != null && appWin == null) {
                ...
                if (HwPCUtils.isPcCastModeInServer() && HwPCUtils.isValidExtDisplayId(this.mDisplayId)) {
                    ...
                    if (mode != 0) {
                        ...
                        float trans_X = ((float) dw) * scale;
                        float trans_Y = ((float) dh) * scale;
                        rect.scale(pcDisplayScale);
                        rect.set(rect.left + ((int) (trans_X - 1.0f)), rect.top + ((int) (trans_Y - 1.0f)), (rect.width() + rect.left) + ((int) Math.floor((double) trans_X)), (rect.height() + rect.top) + ((int) Math.floor((double) trans_Y)));
                    }
                } else if (this.mService.mLazyModeOn != 0) {
                    this.mService.setCropOnSingleHandMode(this.mService.mLazyModeOn, false, dw, dh, rect);
                } else if (((float) width) / ((float) frame.width()) < ((float) height) / ((float) frame.height())) {
                ...
                E bitmap = screenshoter.screenshotForExternalDisplay(displayToken, rect, width, height, minLayer, maxLayer, isAnimating, rot);
                if (bitmap == null) {

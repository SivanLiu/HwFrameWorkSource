...
public class DockedStackDividerController implements DimLayerUser {
    ...
    private static final boolean IS_DOCOMO = SystemProperties.get("ro.product.custom", "NULL").equals("docomo");
    ...
    private void loadDimens() {
        ...
        this.mDividerWindowWidth = context.getResources().getDimensionPixelSize(17105020);
        this.mDividerInsets = context.getResources().getDimensionPixelSize(17105019);
        this.mDividerWindowWidthInactive = WindowManagerService.dipToPixel(4, this.mDisplayContent.getDisplayMetrics());
        this.mTaskHeightInMinimizedMode = context.getResources().getDimensionPixelSize(17105271);
    ...
    private void resetDragResizingChangeReported() {
        this.mDisplayContent.forAllWindows((Consumer) -$Lambda$SZTLEwEbh2VkY-Dt8NUcOZe2XY0.$INST$1, true);
    }
            ...
            if (homeStack != null) {
                ...
                if (homeTask != null && (isWithinDisplay(homeTask) ^ 1) == 0) {
                    if (!this.mMinimizedDock || !this.mService.mPolicy.isKeyguardShowingAndNotOccluded()) {
                        int hasMultipleTaskWithHomeTaskNotTop;
                        TaskStack fullscreenStack = this.mDisplayContent.getStackById(1);
                        boolean homeVisible = homeTask.getTopVisibleAppToken() != null;
                        if (IS_DOCOMO && homeVisible) {
                            TaskStack recentStack = this.mDisplayContent.getStackById(5);
                            if (recentStack != null && recentStack.isVisible()) {
                                homeVisible = false;
                            }
                        }
                        if (fullscreenStack == null || !fullscreenStack.isVisible()) {
                            hasMultipleTaskWithHomeTaskNotTop = homeStack.hasMultipleTaskWithHomeTaskNotTop();
                        } else {
                            hasMultipleTaskWithHomeTaskNotTop = 1;
                        }
                        setMinimizedDockedStack(homeVisible ? hasMultipleTaskWithHomeTaskNotTop ^ 1 : false, animate);
                    }
                }
            }
        }
    ...
    private void startImeAdjustAnimation(boolean adjustedForIme, boolean adjustedForDivider, WindowState imeWin) {
        ...
        this.mService.mWaitingForDrawnCallback = new -$Lambda$HFbYE8vr4-NA3Y6wUONVd47OH54(adjustedForIme, adjustedForDivider, this);
    }

    /* synthetic */ void lambda$-com_android_server_wm_DockedStackDividerController_34746(boolean adjustedForIme, boolean adjustedForDivider) {

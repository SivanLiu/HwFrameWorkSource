...
class TaskSnapshotLoader {
    ...
    TaskSnapshot loadTask(int taskId, int userId, boolean reducedResolution) {
        ...
        if (bitmapFile == null || (protoFile.exists() ^ 1) != 0 || (bitmapFile.exists() ^ 1) != 0) {
            return null;
        }
        try {
            ...
            return new TaskSnapshot(buffer, proto.orientation, new Rect(proto.insetLeft, proto.insetTop, proto.insetRight, proto.insetBottom), reducedResolution, reducedResolution ? TaskSnapshotPersister.REDUCED_SCALE : 1.0f);
        } catch (IOException e) {

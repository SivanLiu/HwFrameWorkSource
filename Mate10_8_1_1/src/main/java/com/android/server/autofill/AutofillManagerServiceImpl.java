...
final class AutofillManagerServiceImpl {
    ...
    private class PruneTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... ignored) {
            ...
            synchronized (AutofillManagerServiceImpl.this.mLock) {
                int numSessionsToRemove = AutofillManagerServiceImpl.this.mSessions.size();
                SparseArray<IBinder> sessionsToRemove = new SparseArray(numSessionsToRemove);
    ...
    /* synthetic */ void lambda$-com_android_server_autofill_AutofillManagerServiceImpl_4247(Message msg) {
        switch (msg.what) {
            case 1:
    ...
    void updateLocked(boolean disabled) {
        ...
        String componentName2 = getComponentNameFromSettings();
        if (!TextUtils.isEmpty(componentName2)) {
            try {
                componentName = ComponentName.unflattenFromString(componentName2);
                serviceInfo = AppGlobals.getPackageManager().getServiceInfo(componentName, 0, this.mUserId);
            } catch (Exception e) {
                Slog.e(TAG, "Bad autofill service name " + componentName2 + ": " + e);
                return;
            }
        }
        if (serviceInfo != null) {
            try {
                ...
                Slog.e(TAG, "Bad AutofillService '" + componentName2 + "': " + e2);
            }
        } else {
            this.mInfo = null;
            if (Helper.sDebug) {
    ...
    int startSessionLocked(IBinder activityToken, int uid, IBinder appCallbackToken, AutofillId autofillId, Rect virtualBounds, AutofillValue value, boolean hasCallback, int flags, ComponentName componentName) {
        if (!isEnabled()) {
            return 0;
        }
        if (Helper.sVerbose) {
        ...
        Session newSession = createSessionByTokenLocked(activityToken, uid, appCallbackToken, hasCallback, componentName);
        if (newSession == null) {
    ...
    private Session createSessionByTokenLocked(IBinder activityToken, int uid, IBinder appCallbackToken, boolean hasCallback, ComponentName componentName) {
        int tries = 0;
        while (true) {
            tries++;
            if (tries > 2048) {
            ...
            if (sessionId != Integer.MIN_VALUE && this.mSessions.indexOfKey(sessionId) < 0) {
                ...
                Session newSession = new Session(this, this.mUi, this.mContext, this.mHandlerCaller, this.mUserId, this.mLock, sessionId, uid, activityToken, appCallbackToken, hasCallback, this.mUiLatencyHistory, this.mInfo.getServiceInfo().getComponentName(), componentName);
    ...
    private void assertCallerLocked(ComponentName componentName) {
        String packageName = componentName.getPackageName();
        PackageManager pm = this.mContext.getPackageManager();
        int callingUid = Binder.getCallingUid();
        try {
            int packageUid = pm.getPackageUidAsUser(packageName, UserHandle.getCallingUserId());
            if (callingUid != packageUid && (((ActivityManagerInternal) LocalServices.getService(ActivityManagerInternal.class)).hasRunningActivity(callingUid, packageName) ^ 1) != 0) {
                ...
                this.mMetricsLogger.write(new LogMaker(948).setPackageName(callingPackage).addTaggedData(908, getServicePackageName()).addTaggedData(949, componentName == null ? "null" : componentName.flattenToShortString()));
    ...
    void dumpLocked(String prefix, PrintWriter pw) {
        ...
        pw.println(this.mContext.getString(17039767));
        pw.print(prefix);
        pw.print("Disabled: ");

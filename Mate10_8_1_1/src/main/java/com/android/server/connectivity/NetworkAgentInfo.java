...
public class NetworkAgentInfo implements Comparable<NetworkAgentInfo> {
    ...
    private boolean ignoreWifiUnvalidationPenalty() {
        ...
        if (this.networkCapabilities.hasTransport(1)) {
            hasCapability = this.networkCapabilities.hasCapability(12);
        } else {
            hasCapability = false;
        }
        return (!hasCapability || ((!this.mConnService.avoidBadWifi() ? this.avoidUnvalidated : 1) ^ 1) == 0) ? false : this.everValidated;
    }

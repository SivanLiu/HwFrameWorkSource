...
public class PickUpWakeScreenManager {
    ...
    private HWExtDeviceEventListener mHWEDListener = new HWExtDeviceEventListener() {
        public void onDeviceDataChanged(HWExtDeviceEvent hwextDeviceEvent) {
            ...
            if (PickUpWakeScreenManager.this.mIsPickupSupport) {
                ...
                if (MRUtils.getMotionEnableStateAsUser(PickUpWakeScreenManager.this.mContext, PickUpWakeScreenManager.MOTION_PICKUP_WAKEUP_DEVICE, ActivityManager.getCurrentUser()) != 1) {
                    z = false;
                }
                pickUpWakeScreenManager.mIsPickupSwitchOn = z;
    ...
    private ContentObserver mPickupEnabledObserver = new ContentObserver(new Handler()) {
        public void onChange(boolean selfChange) {
            PickUpWakeScreenManager.this.mIsPickupSwitchOn = PickUpWakeScreenManager.this.getPickupSwitchOn();
            Slog.d(PickUpWakeScreenManager.TAG, "PickupEnabledObserver: mIsPickupSwitchOn = " + PickUpWakeScreenManager.this.mIsPickupSwitchOn);
            if (PickUpWakeScreenManager.this.mIsPickupSupport && (PickUpWakeScreenManager.this.mIsPickupSwitchOn ^ 1) != 0 && PickUpWakeScreenManager.this.mHWEDManager != null) {
                PickUpWakeScreenManager.this.mHWEDManager.unregisterDeviceListener(PickUpWakeScreenManager.this.mHWEDListener, PickUpWakeScreenManager.this.mHWExtMotion);
                Slog.d(PickUpWakeScreenManager.TAG, "unregist listener");
            }
        }
    };
    ...
    private final SensorEventListener mProximitySensorListener = new SensorEventListener() {
        public void onSensorChanged(SensorEvent event) {
            if (PickUpWakeScreenManager.this.mProximitySensorEnabled) {
                ...
                boolean positive = distance >= GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO && distance < PickUpWakeScreenManager.this.mProximityThreshold;
    ...
    public void pickUpWakeScreenInit() {
        ...
        MRUtils.observerMotionEnableStateChange(this.mContext, this.mPickupEnabledObserver, ActivityManager.getCurrentUser());
    ...
    public void enablePickupMotionOrNot(boolean isLocked) {
        boolean z = true;
        if (this.mIsPickupSupport) {
            if (MRUtils.getMotionEnableStateAsUser(this.mContext, MOTION_PICKUP_WAKEUP_DEVICE, ActivityManager.getCurrentUser()) != 1) {
                z = false;
            }
            this.mIsPickupSwitchOn = z;
            Slog.d(TAG, "switch = " + MRUtils.getMotionEnableStateAsUser(this.mContext, MOTION_PICKUP_WAKEUP_DEVICE, ActivityManager.getCurrentUser()));
    ...
    private void sendPickupFacedetectBroadcast() {
        this.mContext.sendBroadcastAsUser(new Intent(ACTION_PICKUP_FACEDETECT), UserHandle.CURRENT, PERMISSION_PICKUP_FACEDETECT);
    }
    ...
    private void stopWakeUpReady(Context context, boolean turnOn) {
        if (this.mInFastScreenOn) {
            ...
            if (turnOn) {
                ...
                Slog.d(TAG, "report no proximity ScreenOn waked by pickup motion. EventId:500 ret:" + Flog.bdReport(this.mContext, 500));
            }
            return;
        }

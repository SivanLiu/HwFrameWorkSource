...
public class SearchPanelCircleView extends FrameLayout {
    ...
    private void updateClipping() {
        boolean isEmpty = this.mCircleSize >= ((float) this.mCircleMinSize) ? this.mRipples.isEmpty() ^ 1 : true;
        if (isEmpty != this.mClipToOutline) {
            setClipToOutline(isEmpty);
            this.mClipToOutline = isEmpty;
        }
    }
    ...
    private void updateLogo() {
        ...
        if (exitAnimationRunning) {
            ...
            this.mLogo.setAlpha(alphaTmp > GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO ? alphaTmp : GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO);
        }
    ...
    private void addRipple() {
        if (this.mRipples.size() <= 1) {
            ...
            new Ripple((((float) this.mStaticRect.left) * (1.0f - xInterpolation)) + (((float) this.mStaticRect.right) * xInterpolation), (((float) this.mStaticRect.top) * (1.0f - yInterpolation)) + (((float) this.mStaticRect.bottom) * yInterpolation), (this.mCircleSize > ((float) this.mCircleMinSize) * 1.25f ? this.mCircleSize : ((float) this.mCircleMinSize) * 1.25f) * 0.75f).start();
        }
    }

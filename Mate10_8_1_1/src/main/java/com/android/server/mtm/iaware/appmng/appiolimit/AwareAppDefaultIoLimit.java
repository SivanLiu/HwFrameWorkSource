    ...
    public void doLimitIO(String pkgName, AppIoLimitSource config) {
        ...
        synchronized (this) {
            ...
            for (AwareProcessBlockInfo processInfo : awareProcessBlockInfos) {
                if (processInfo.mCleanType == CleanType.IOLIMIT && !(pkgName.equals(processInfo.mPackageName) && UserHandle.getUserId(processInfo.mUid) == AwareAppAssociate.getInstance().getCurUserId())) {

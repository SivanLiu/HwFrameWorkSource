...
public class DecisionMaker {
    ...
    public List<AwareProcessBlockInfo> decideAll(List<AwareProcessInfo> processInfoList, int level, AppMngFeature feature, EnumWithDesc config) {
        return decideAllInternal(processInfoList, level, feature, config, true);
    }
    ...
    public List<AwareProcessBlockInfo> decideAllWithoutList(List<AwareProcessInfo> processInfoList, int level, AppMngFeature feature, EnumWithDesc config) {
        return decideAllInternal(processInfoList, level, feature, config, false);
    }

    private List<AwareProcessBlockInfo> decideAllInternal(List<AwareProcessInfo> processInfoList, int level, AppMngFeature feature, EnumWithDesc config, boolean shouldConsiderList) {
        ...
        for (AwareProcessInfo processInfo : processInfoList) {
            if (processInfo != null) {
                AwareProcessBlockInfo result = decide(processInfo, level, feature, config, shouldConsiderList);
                if (result != null) {
    ...
    private AwareProcessBlockInfo decide(AwareProcessInfo processInfo, int level, AppMngFeature feature, EnumWithDesc config, boolean shouldConsiderList) {
        ...
        if (shouldConsiderList && featureList != null) {
    ...
    public HashMap<String, ListItem> getProcessList(AppMngFeature feature, EnumWithDesc config) {
        if (feature == null || config == null) {
            return null;
        }
        HashMap<EnumWithDesc, HashMap<String, ListItem>> featureProcessList = (HashMap) this.mAllProcessList.get(feature);
        if (featureProcessList == null) {
            return null;
        }
        return (HashMap) featureProcessList.get(config);
    }
    ...
    public void updateRule(AppMngFeature feature, Context context) {
        ...
        if (feature == null) {
            ...
            this.mAllProcessList.put(feature, (HashMap) reader.getProcessList().get(feature));
    ...
    public void dumpList(PrintWriter pw, AppMngFeature feature) {
        if (pw != null) {
                        ...
                        if (configList != null) {
                            ...
                            for (Entry<String, ListItem> configListEntry : configList.entrySet()) {
                                if (configListEntry != null) {
                                    ...
                                    if (item != null) {
                                        printWriter = pw;
                                        printWriter.println("  " + ((String) configListEntry.getKey()) + ":");
                                        item.dump(pw);
                                    }
                                }
                            }
            ...
            if (featureProcessList == null) {
                ...
                for (Entry<EnumWithDesc, HashMap<String, ListItem>> featureProcessListEntry : featureProcessList.entrySet()) {
                    if (featureProcessListEntry != null) {
                        HashMap<String, ListItem> configProcessList = (HashMap) featureProcessListEntry.getValue();
                        if (configProcessList != null) {
                            pw.println("==========[" + featureProcessListEntry.getKey() + "] ProcessLists==========");
                            for (Entry<String, ListItem> configProcessListEntry : configProcessList.entrySet()) {
                                if (configProcessListEntry != null) {
                                    ListItem processItem = (ListItem) configProcessListEntry.getValue();
                                    if (processItem != null) {
                                        printWriter = pw;
                                        printWriter.println("  " + ((String) configProcessListEntry.getKey()) + ":");
                                        processItem.dump(pw);
                                    }
                                }
                            }

...
public class HwLocationManagerService extends LocationManagerService {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.location.GPS_ENABLED_CHANGE".equals(action)) {
                if (intent.hasExtra("enabled")) {
                    ...
                    HwLocationManagerService.this.sendHwLocationMessage(28, MemoryConstant.REPEAT_RECLAIM_TIME_GAP);
                }
    ...
    protected boolean hwLocationHandleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case 23:
                int always_wifi_scan = Global.getInt(this.mHwContext.getContentResolver(), "wifi_scan_always_enabled", 0);
                int userAction = Global.getInt(this.mHwContext.getContentResolver(), WIFI_SCAN_ALWAYS_AVAILABLE_USER_OPR, 0);
                if ((Secure.getInt(this.mHwContext.getContentResolver(), "device_provisioned", 0) != 0) && always_wifi_scan == 1 && userAction != 1) {
            ...
            case 28:
                ...
                if (this.navigating) {
                    sendHwLocationMessage(28, MemoryConstant.REPEAT_RECLAIM_TIME_GAP);
                    break;
                }
                break;
            default:
        ...
        if (uid <= 1000) {
            ...
            if (isNetworkAvailable(uid) || !mExpectedMode.equals("network")) {
                ...
                synchronized (this.mLocationManagerServiceUtil.getmLock()) {
                    ...
                    if (!(updateProviders == null || (updateProviders.isEmpty() ^ 1) == 0)) {
    ...
    private HashMap<UpdateRecord, String> getRemoveSwitchRecords(String packagename) {
        ...
        if (mReceivers != null) {
            for (Receiver receiver : mReceivers.values()) {
                if (receiver.mIdentity.mPackageName.equals(packagename)) {
                    for (UpdateRecord record2 : receiver.mUpdateRecords.values()) {
                        if (!record2.mProvider.equals(record2.mRealRequest.getProvider())) {
                            mSwitchRecords.put(record2, record2.mRealRequest.getProvider());
                            Slog.d(TAG, "getRemoveSwitchRecords, packagename:" + packagename + " originalMode: " + record2.mRealRequest.getProvider() + ", currentMode: " + record2.mProvider + ", receiver: " + Integer.toHexString(System.identityHashCode(receiver)));
                        }
                    }
                }
            }
    ...
    private boolean isNetworkAvailable(int uid) {
        ...
        try {
            ...
            boolean z = wifiAccess ? isWifiConn : false;
            boolean z2 = mobileAccess ? isMobileConn : false;
            Slog.d(TAG, "wifiAvaiable is " + z + " , mobileAvailable is " + z2);
            if (z || z2) {
                return true;
            }
            return false;
    ...
    protected void readySaveLocationToDataBase() {
        ...
        if (0 == this.mLastLocation.getTime() || nowTime - this.mLastLocation.getTime() > WifiProCommonUtils.RECHECK_DELAYED_MS) {
            ...
            sendHwLocationMessage(27, MemoryConstant.REPEAT_RECLAIM_TIME_GAP);
        }
    }
    ...
    protected void saveLocationToDataBase() {
        ...
        try {
            String encryptedLong = AESLocalDbCrypto.encrypt(MASTER_PASSWORD, this.mLastLocation.getLongitude() + "");
            String encryptedLat = AESLocalDbCrypto.encrypt(MASTER_PASSWORD, this.mLastLocation.getLatitude() + "");
    ...
    protected Location readLastLocationDataBase() {
        try {
            ...
            if (0.0d == longitudeSaved || 0.0d == latitudeSaved) {
            ...
            Log.i(TAG, "readLastLocationDataBase" + location);
            return location;
        } catch (Exception e) {
    ...
    private boolean isDualBandSupported() {
        return this.mContext.getResources().getBoolean(17957064);
    }

...
class SipSessionGroup implements SipListener {
    ...
    synchronized void reset() throws SipException {
        ...
        if (str != null) {
            ...
            try {
                SipProvider provider = this.mSipStack.createSipProvider(this.mSipStack.createListeningPoint(str, port, protocol));
    ...
    private synchronized void process(EventObject event) {
        ...
        try {
            ...
            boolean process = session != null ? session.process(event) : false;
            if (isLoggable && process) {

...
public class FingerPressNavigation {
    ...
    private long[] mAppKeyVibePattern = new long[]{0, 10, 20, 30};
    ...
    protected FingerPressNavigation(Context context) {
        ...
        this.mStatusBarHeight = this.mContext.getResources().getDimensionPixelSize(17105265);
        this.mVibrator = (Vibrator) this.mContext.getSystemService("vibrator");
        if (!(this.mVibrator == null || (this.mVibrator.hasVibrator() ^ 1) == 0)) {
            this.mVibrator = null;
        }
    ...
    public void setCurFocusWindow(WindowState focus) {
        ...
        if (((attrs != null ? attrs.privateFlags : 0) & 1024) != 0) {
            z = true;
        }
        this.mKeyguardOn = z;
        if (this.mCircle != null) {
        ...
        if (startInfo != null) {
            ...
            if (startInfos != null && startInfos.length == 2) {
                try {
                    ...
                    StatisticalUtils.reporte(this.mContext, 82, "{pkg:" + startInfos[0] + "}");
                } catch (Exception ex) {
    ...
    public boolean filterPressueInputEvent(InputEvent event) {
        ...
        if (event instanceof MotionEvent) {
            ...
            if (this.mIsGameScene || (this.mIsFullScreen ^ 1) == 0 || this.RightPkgName == null || ("none_app".equals(this.RightPkgName) ^ 1) == 0 || !isHandledByRightEdge(motionevent, x, y, pressue)) {
                if (this.mIsGameScene || (this.mIsFullScreen ^ 1) == 0 || this.LeftPkgName == null || ("none_app".equals(this.LeftPkgName) ^ 1) == 0 || !isHandledByLeftEdge(motionevent, x, y, pressue)) {
                    if (!this.mImmersiveMode && this.mMode == 1 && isHandledByBottomCenter(motionevent, x, y, pressue)) {
                        if (!isKeyguardLocked()) {
                            handleByAnimation(motionevent, 1);
                            z = interceptByPressureNavi(motionevent, pressue, 3) ^ 1;
                        }
                        if (this.mAllComsume) {
                            z = false;
                        }
                        ...
                        if (!isKeyguardLocked()) {
                            ...
                            z = interceptByPressureNavi(motionevent, pressue, getKeyCode(0)) ^ 1;
                        ...
                        if (!isKeyguardLocked()) {
                            ...
                            z = interceptByPressureNavi(motionevent, pressue, getKeyCode(1)) ^ 1;
                ...
                } else if (!(isKeyguardLocked() || (isPhoneInCall() ^ 1) == 0 || (filterAppStartEvent(motionevent) ^ 1) == 0)) {
                    handleByAnimation(motionevent, 4);
                    z = interceptByAppNavi(motionevent, pressue, 0) ^ 1;
                }
            } else if (!(isKeyguardLocked() || (isPhoneInCall() ^ 1) == 0 || (filterAppStartEvent(motionevent) ^ 1) == 0)) {
                handleByAnimation(motionevent, 4);
                z = interceptByAppNavi(motionevent, pressue, 1) ^ 1;
            }
    ...
    void sendEvent(int action, int flags, int code, long when) {
        InputManager.getInstance().injectInputEvent(new KeyEvent(this.mDownTime, when, action, code, 0, 0, -1, 0, (flags | 8) | 64, CommandsInterface.EMCOM_SD_XENGINE_START_ACC), 0);
    }
}

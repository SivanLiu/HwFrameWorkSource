...
public class TaskRecord extends AbsTaskRecord implements TaskWindowContainerListener {
            ...
            if (this.mWindowContainerController == null) {
                ...
                if (updateOverrideConfiguration(bounds)) {
                    ...
                    if (!(r == null || (deferResume ^ 1) == 0)) {
                        z = r.ensureActivityConfigurationLocked(0, preserveWindow);
                ...
                this.mWindowContainerController.resize(this.mBounds, getOverrideConfiguration(), z, forced);
                Trace.traceEnd(64);
                return z;
            } else {
    ...
    boolean reparent(int preferredStackId, int position, int moveStackMode, boolean animate, boolean deferResume, boolean schedulePictureInPictureModeChange, String reason) {
        ...
        try {
            ...
            boolean z = moveStackMode != 0 ? moveStackMode == 1 ? !wasFocused ? wasFront : true : false : true;
            sourceStack.removeTask(this, reason, z ? 2 : 1);
            ...
            if (r != null) {
                toStack.moveToFrontAndResumeStateIfNeeded(r, z, wasResumed, wasPaused, reason);
            }
            if (!animate) {
    ...
    boolean setLastThumbnailLocked(Bitmap thumbnail) {
        ...
        return setLastThumbnailLocked(thumbnail, taskWidth, taskHeight, getStack().getDisplay().getConfiguration().orientation);
    }
    ...
    static TaskRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws IOException, XmlPullParserException {
        ...
        if (effectiveUid <= 0) {
            Intent checkIntent = intent != null ? intent : intent2;
            effectiveUid = 0;
            if (checkIntent != null) {
                try {
    ...
    boolean updateOverrideConfiguration(Rect bounds, Rect insetBounds) {
        boolean isExtDynamicStack;
        if (this.mStack != null) {
            isExtDynamicStack = HwPCUtils.isExtDynamicStack(this.mStack.getStackId());
        } else {
            isExtDynamicStack = false;
        }
        if (!isExtDynamicStack && Objects.equals(this.mBounds, bounds)) {
            return false;
        }
    ...
    void dump(PrintWriter pw, String prefix) {
        ...
        if (!this.autoRemoveRecents && !this.isPersistable && this.taskType == 0 && this.mTaskToReturnTo == 0) {
            ...
            if (this.mAffiliatedTaskId == this.taskId && this.mPrevAffiliateTaskId == -1 && this.mPrevAffiliate == null && this.mNextAffiliateTaskId == -1) {
                ...
                pw.print(" firstActiveTime=" + this.firstActiveTime);

...
public final class HwActivityStackSupervisor extends ActivityStackSupervisor {
    ...
    private int mNextVrFreeStackId = 20007;
            ...
            if (action != null && action.equals(homeIntent.getAction()) && category != null && category.containsAll(homeIntent.getCategories())) {
                ...
                if (strPkg != null) {
                    if (HwDeviceManager.disallowOp(26, strPkg)) {
                        Slog.i(ActivityManagerService.TAG, strPkg + " is a ignored frequent relaunch app set by MDM!");
    ...
    private final void noteActivityDisplayedEnd(String activityName, int pid, long time) {
        if (AwareConstant.CURRENT_USER_TYPE == 3) {
            ...
            if (resManager != null && resManager.isResourceNeeded(ResourceType.getReousrceId(ResourceType.RES_APP)) && this.mService.mSystemReady) {
                ...
                Slog.d(ActivityManagerService.TAG, "EVENT_APP_ACTIVITY_DISPLAYED_FINISH reportData: " + activityName + " " + pid + " " + time);
            }
        }
    }
    ...
    protected int getNextVrStackId() {
        while (true) {
            if (this.mNextVrFreeStackId >= 20007 && getStack(this.mNextVrFreeStackId) == null) {
    ...
    protected int getNextPcStackId() {
        while (true) {
            if (this.mNextPcFreeStackId >= 20007) {
                this.mNextPcFreeStackId = IDisplayEngineService.DE_ACTION_PG_CAMERA_FRONT;
            }
    ...
    protected ActivityStack getValidLaunchStackOnDisplay(int displayId, ActivityRecord r) {
        ...
        if (HwVRUtils.isValidVRDisplayId(displayId)) {
            newDynamicStackId = getNextVrStackId();
            Slog.e(ActivityManagerService.TAG, "getValidLaunchStackOnDisplay vr stackId = " + newDynamicStackId);
        ...
        if (HwPCUtils.isPcCastModeInServer() && HwPCUtils.isValidExtDisplayId(displayId)) {
            newDynamicStackId = getNextPcStackId();
            if (this.mService.mActivityStarter.isValidLaunchStackId(newDynamicStackId, displayId, r)) {
                return createStackOnDisplay(newDynamicStackId, displayId, true);
            }
        }
        if (HwPCUtils.isPcCastModeInServer() && displayId == 0 && getStack(1) == null) {
            HwPCUtils.log(ActivityManagerService.TAG, " create full screen stack because the stack is null when r is from pc");
            createStackOnDisplay(1, displayId, true);
        }
    ...
    protected boolean restoreRecentTaskLocked(TaskRecord task, int stackId) {
        if ((HwPCUtils.isPcCastModeInServer() || HwVRUtils.isVRDynamicStack(stackId)) && task == null) {
            return false;
        }
    ...
    protected void handleDisplayRemoved(int displayId) {
        ...
        if (HwPCUtils.isPcCastModeInServer() && HwPCUtils.isValidExtDisplayId(displayId)) {
            ...
            Slog.d(ActivityManagerService.TAG, "handleDisplayRemoved vr displayId = " + displayId);
            activityManagerService = this.mService;
            synchronized (activityManagerService) {
                activityDisplay = (ActivityDisplay) this.mActivityDisplays.get(displayId);
                if (activityDisplay != null) {
                    onVRDisplayRemoved(activityDisplay.mStacks, displayId);
                }
            }
        } else {
    ...
    protected boolean isStackInVisible(ActivityStack stack) {
        if (stack != null && HwPCUtils.isExtDynamicStack(stack.mStackId) && stack.shouldBeVisible(null) == 0) {
            return true;
        }
    ...
    public boolean shouldNotKillProcWhenRemoveTask(String pkg) {
        ...
        Slog.d(ActivityManagerService.TAG, " cleanUpRemovedTaskLocked, do not kill process : " + pkg);
        return true;
    }

    public void onVRDisplayRemoved(ArrayList<ActivityStack> stacks, int displayId) {
        Slog.d(ActivityManagerService.TAG, "handleDisplayRemoved in vr mode");
        while (!stacks.isEmpty()) {
            moveTasksToFullscreenStackLocked(((ActivityStack) stacks.get(0)).getStackId(), false);
        }
        this.mActivityDisplays.remove(displayId);
        this.mWindowManager.onDisplayRemoved(displayId);
    ...
    public void onDisplayRemoved(ArrayList<ActivityStack> stacks) {
        ...
        if (pcManager != null) {
            try {
                ...
                HwPCUtils.log(ActivityManagerService.TAG, "fail to saveAppIntent on display removed");
            }
        }
    ...
    boolean hasActivityInStackLocked(ActivityInfo aInfo) {
        ...
        String affinity = aInfo.taskAffinity;
        String pkgName = aInfo.packageName;
        String processName = aInfo.processName;
        if (affinity == null || pkgName == null || processName == null || aInfo.applicationInfo == null) {
            return false;
        }
                ...
                while (taskNdx >= 0) {
                    ...
                    if (task == null || task.userId != userId || task.affinity == null || !task.affinity.equals(affinity)) {
                        ...
                        for (int activityNdx = numActivities - 1; activityNdx >= 0; activityNdx--) {
                            ...
                            if (!r.finishing && r.state != ActivityState.DESTROYED && r.app != null && (pkgName.equals(r.packageName) ^ 1) == 0 && processName.equals(r.app.processName)) {
                                return true;
                            }
                        }
                        return false;

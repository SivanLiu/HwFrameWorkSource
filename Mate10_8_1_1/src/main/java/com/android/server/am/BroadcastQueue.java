                                ...
                                if (r != null) {
                                    ...
                                    if (enqueueProxyBroadcast(false, r, target)) {
                                        ...
                                        if (!this.mPendingBroadcastTimeoutMessage) {
                                            ...
                                            checkTime = r.receiverTime + ((long) CHECK_INTERVAL);
                                            if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                                        ...
                                        if (nextReceiver instanceof BroadcastFilter) {
                                            ...
                                            if (!(skip2 != false || info.activityInfo.applicationInfo.uid == 1000 || r.requiredPermissions == null || r.requiredPermissions.length <= 0)) {
                                            ...
                                            if (!(skip2 != false || r.appOp == -1 || this.mService.mAppOpsService.noteOperation(r.appOp, info.activityInfo.applicationInfo.uid, info.activityInfo.packageName) == 0)) {
                                            ...
                                            if (!skip2 == true) {
                                                skip2 = this.mService.mIntentFirewall.checkBroadcast(r.intent, r.callingUid, r.callingPid, r.resolvedType, info.activityInfo.applicationInfo.uid) ^ 1;
                                            }
                                            if (!skip2 != false && this.mService.shouldPreventSendBroadcast(r.intent, info.activityInfo.packageName, r.callingUid, r.callingPid, r.callerPackage, r.userId)) {
                                                skip2 = true;
                                            }
                                            isSingleton = false;
                                            try {
                                            ...
                                            if (!(skip2 != false || !info.activityInfo.applicationInfo.isInstantApp() || r.callingUid == info.activityInfo.applicationInfo.uid)) {
                                            ...
                                            if (!skip2 != false && r.callerInstantApp && (info.activityInfo.flags & DumpState.DUMP_DEXOPT) == 0 && r.callingUid != info.activityInfo.applicationInfo.uid) {
                                            ...
                                            if (!(!this.mService.mPermissionReviewRequired || (r50 ^ 1) == 0 || requestStartTargetPermissionsReviewIfNeededLocked(r, info.activityInfo.packageName, UserHandle.getUserId(info.activityInfo.applicationInfo.uid)))) {
                                                skip2 = 1;
                                            }
                                            receiverUid = info.activityInfo.applicationInfo.uid;
                                            if (r.callingUid != 1000 && r36 && this.mService.isValidSingletonCall(r.callingUid, receiverUid)) {
                                                info.activityInfo = this.mService.getActivityInfoForUser(info.activityInfo, 0);
                                            }
                                            targetProcess = info.activityInfo.processName;
                                            app = this.mService.getProcessRecordLocked(targetProcess, info.activityInfo.applicationInfo.uid, false);
                                            if (skip2 == 0) {
                                                allowed = this.mService.getAppStartModeLocked(info.activityInfo.applicationInfo.uid, info.activityInfo.packageName, info.activityInfo.applicationInfo.targetSdkVersion, -1, true, false);
                                                if (allowed != 0) {
                                                    if (allowed != 3) {
                                                        Slog.w(TAG, "Background execution disabled: receiving " + r.intent + " to " + componentName.flattenToShortString());
                                                        skip2 = 1;
                                                    } else if ((r.intent.getFlags() & DumpState.DUMP_VOLUMES) != 0 || (r.intent.getComponent() == null && r.intent.getPackage() == null && (r.intent.getFlags() & 16777216) == 0 && (isSignaturePerm(r.requiredPermissions) ^ 1) != 0)) {
                                            ...
                                            if (this.mService.mUserController.getCurrentUserIdLocked() == 0) {
                                                try {
                                                    ...
                                                    applicationInfo = AppGlobals.getPackageManager().getApplicationInfo(packageName, 0, UserHandle.getUserId(info.activityInfo.applicationInfo.uid));
                    ...
                    if (r != null) {
                        ...
                        if (enqueueProxyBroadcast(false, r, target)) {
                            ...
                            if (nextReceiver instanceof BroadcastFilter) {
                                ...
                                for (String requiredPermission2 : r.requiredPermissions) {
                                    perm = AppGlobals.getPackageManager().checkPermission(requiredPermission2, info.activityInfo.applicationInfo.packageName, UserHandle.getUserId(info.activityInfo.applicationInfo.uid));
                                    if (perm != 0) {
                                        Slog.w(TAG, "Permission Denial: receiving " + r.intent + " to " + componentName.flattenToShortString() + " requires " + requiredPermission2 + " due to sender " + r.callerPackage + " (uid " + r.callingUid + ")");
                                        skip2 = true;
                                        break;
                                    }
                                    appOp = AppOpsManager.permissionToOpCode(requiredPermission2);
                                    if (this.mService.mAppOpsService.noteOperation(appOp, info.activityInfo.applicationInfo.uid, info.activityInfo.packageName) != 0) {
                                        Slog.w(TAG, "Appop Denial: receiving " + r.intent + " to " + componentName.flattenToShortString() + " requires appop " + AppOpsManager.permissionToOp(requiredPermission2) + " due to sender " + r.callerPackage + " (uid " + r.callingUid + ")");
                                        skip2 = true;
                                        break;
                                    }
                                }
    ...
    final void broadcastTimeoutLocked(boolean fromMsg) {
        ...
        if (this.mOrderedBroadcasts.size() != 0) {
            ...
            if (r.nextReceiver > 0) {
                ...
                Object curReceiver2 = r.curReceiver;
            }
            ...
            Slog.w(TAG, "Skip anr of PRE_BOOT_COMPLETED for app :" + processRecord);
        }
    }
        ...
        do {
            ...
            if (r != null) {
                ...
                if (dumpAll) {
                    ...
                    if (!(bundle == null || (isFromEmailMDM(r.intent) ^ 1) == 0)) {
                        pw.print("    extras: ");

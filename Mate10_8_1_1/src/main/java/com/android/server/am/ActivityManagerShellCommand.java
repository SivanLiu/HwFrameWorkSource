...
final class ActivityManagerShellCommand extends ShellCommand {
            ...
            do {
                ...
                if (!(this.mProfileFile == null && this.mAgent == null)) {
                    ...
                    if (this.mProfileFile != null) {
                        parcelFileDescriptor = openOutputFileForSystem(this.mProfileFile);
                        if (parcelFileDescriptor == null) {
                            return 1;
                        }
                    }
                    profilerInfo = new ProfilerInfo(this.mProfileFile, parcelFileDescriptor, this.mSamplingInterval, this.mAutoStop, this.mStreaming, this.mAgent);
                }
                ...
                if (this.mWaitOption) {
                    waitResult = this.mInterface.startActivityAndWait(null, null, intent, mimeType, null, null, 0, this.mStartFlags, profilerInfo, activityOptions != null ? activityOptions.toBundle() : null, this.mUserId);
                    res = waitResult.result;
                } else {
                    res = this.mInterface.startActivityAsUser(null, null, intent, mimeType, null, null, 0, this.mStartFlags, profilerInfo, activityOptions != null ? activityOptions.toBundle() : null, this.mUserId);
                }
                ...
                if (this.mWaitOption && launched) {
                    ...
                    pw.println("Status: " + (waitResult.timeout ? "timeout" : "ok"));
                    if (waitResult.who != null) {
                        pw.println("Activity: " + waitResult.who.flattenToShortString());
                    }
                    if (waitResult.thisTime >= 0) {
                        pw.println("ThisTime: " + waitResult.thisTime);
                    }
                    if (waitResult.totalTime >= 0) {
                        pw.println("TotalTime: " + waitResult.totalTime);
                    }
    ...
    int runSendBroadcast(PrintWriter pw) throws RemoteException {
        try {
            ...
            pw.flush();
            this.mInterface.broadcastIntent(null, intent, null, receiver, 0, null, null, strArr, -1, null, true, false, this.mUserId);
    ...
    int runTraceIpcStop(PrintWriter pw) throws RemoteException {
        ...
        new File(str).delete();
        ParcelFileDescriptor fd = openOutputFileForSystem(str);
        if (fd == null) {
            return -1;
        }
        if (this.mInterface.stopBinderTrackingAndDump(fd)) {
            pw.println("Stopped IPC tracing. Dumping logs to: " + str);
            return 0;
        }
    ...
    private int runProfile(PrintWriter pw) throws RemoteException {
        ...
        if (start) {
            ...
            profilerInfo = new ProfilerInfo(profileFile, fd, this.mSamplingInterval, false, this.mStreaming, null);
        }
        if (wall) {
            String props = SystemProperties.get("dalvik.vm.extra-opts");
    ...
    int runStopUser(PrintWriter pw) throws RemoteException {
        ...
        int res = this.mInterface.stopUser(user, force, stopUserCallback);
        if (res != 0) {
            String txt = "";
            switch (res) {
                case -4:
    ...
    int runStackResizeAnimated(PrintWriter pw) throws RemoteException {
        ...
        return resizeStackUnchecked(stackId, rect, 0, true);
    }
    ...
    int runNoHomeScreen(PrintWriter pw) throws RemoteException {
        ...
        pw.println(res.getBoolean(17956992));
        return 0;
    }

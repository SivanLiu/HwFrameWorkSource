...
final class StrictModeViolationDialog extends BaseErrorDialog {
    ...
    public StrictModeViolationDialog(Context context, ActivityManagerService service, AppErrorResult result, ProcessRecord app) {
        ...
        if (app.pkgList.size() != 1 || context.getPackageManager().getApplicationLabel(app.info) == null) {
            setMessage(res.getString(17041026, new Object[]{app.processName.toString()}));
        } else {
            setMessage(res.getString(17041025, new Object[]{context.getPackageManager().getApplicationLabel(app.info).toString(), app.info.processName}));
        }
        setCancelable(false);
        setButton(-1, res.getText(17039912), this.mHandler.obtainMessage(0));
        if (app.errorReportReceiver != null) {
            setButton(-2, res.getText(17040894), this.mHandler.obtainMessage(1));
        }

...
public class ActivityRecord extends AbsActivityRecord implements AppWindowContainerListener {
    ...
    private boolean shouldRelaunchLocked(int r1, android.content.res.Configuration r2) {
        /* JADX: method processing error */
/*
Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.server.am.ActivityRecord.shouldRelaunchLocked(int, android.content.res.Configuration):boolean
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.am.ActivityRecord.shouldRelaunchLocked(int, android.content.res.Configuration):boolean");
    }
    ...
    public ActivityRecord(ActivityManagerService _service, ProcessRecord _caller, int _launchedFromPid, int _launchedFromUid, String _launchedFromPackage, Intent _intent, String _resolvedType, ActivityInfo aInfo, Configuration _configuration, ActivityRecord _resultTo, String _resultWho, int _reqCode, boolean _componentSpecified, boolean _rootVoiceInteraction, ActivityStackSupervisor supervisor, ActivityOptions options, ActivityRecord sourceRecord) {
        ...
        this.translucent = z;
        if (ent == null || (ent.array.getBoolean(4, false) ^ 1) == 0 || (this.translucent ^ 1) == 0) {
            z = isForceRotationMode(this.packageName, _intent);
        } else {
            z = true;
        }
        this.fullscreen = z;
        this.mIsFloating = ent != null ? ent.array.getBoolean(4, false) : false;
        if (isSplitMode()) {
            if (this.fullscreen) {
        ...
        this.mShowWhenLocked = (aInfo.flags & DumpState.DUMP_VOLUMES) != 0;
        this.mTurnScreenOn = (aInfo.flags & 16777216) != 0;
    }
    ...
    boolean isFocusable() {
        return (this.task == null || !StackId.canReceiveKeys(this.task.getStackId())) ? isAlwaysFocusable() : true;
    }
    ...
    boolean canBeLaunchedOnDisplay(int displayId) {
        ...
        return this.service.mStackSupervisor.canPlaceEntityOnDisplay(displayId, task != null ? task.isResizeable() : supportsResizeableMultiWindow(), this.launchedFromPid, this.launchedFromUid, this.info);
    }
    ...
    boolean checkEnterPictureInPictureState(String caller, boolean beforeStopping) {
        ...
        int i = !isKeyguardLocked ? isCurrentAppLocked ^ 1 : 0;
        ...
        switch (-getcom-android-server-am-ActivityStack$ActivityStateSwitchesValues()[this.state.ordinal()]) {
            case 1:
            case 2:
                if (!(i == 0 || (hasPinnedStack ^ 1) == 0)) {
                    z = this.supportsEnterPipOnTaskSwitch;
                }
                return z;
            case 3:
                if (!isCurrentAppLocked) {
                    z = !this.supportsEnterPipOnTaskSwitch ? beforeStopping ^ 1 : true;
                }
                return z;
            case 4:
    ...
    final void deliverNewIntentLocked(int callingUid, Intent intent, String referrer) {
        ...
        boolean unsent = true;
        ActivityStack stack = getStack();
        boolean isTopActivityWhileSleeping = isTopRunningActivity() ? stack != null ? stack.shouldSleepActivities() : this.service.isSleepingLocked() : false;
        if (!(this.state == ActivityState.RESUMED || this.state == ActivityState.PAUSED)) {
            if (isTopActivityWhileSleeping) {
            }
            if (unsent) {
    ...
    final Bitmap screenshotActivityLocked() {
        ...
        if (ActivityManager.ENABLE_TASK_SNAPSHOTS && (this.service.mWindowManager.getGestureState() ^ 1) != 0) {
            if (ActivityManagerDebugConfig.DEBUG_SCREENSHOTS) {
    ...
    void setVisible(boolean newVisible) {
        ...
        this.mDeferHidingClient = !this.visible ? this.mDeferHidingClient : false;
        if (!(this.visible || !this.mUpdateTaskThumbnailWhenHidden || (this.mAlreadyScreenshot ^ 1) == 0)) {
            updateThumbnailLocked(screenshotActivityLocked(), null);
            this.mUpdateTaskThumbnailWhenHidden = false;
        }
        if (this.mAlreadyScreenshot) {
            this.mUpdateTaskThumbnailWhenHidden = false;
            this.mAlreadyScreenshot = false;
        }
    ...
    boolean shouldBeVisibleIgnoringKeyguard(boolean behindFullscreenActivity) {
        ...
        boolean z = behindFullscreenActivity ? this.mLaunchTaskBehind : true;
        if (this.service.mSupportsLeanbackOnly && z && isRecentsActivity()) {
            z = this.mStackSupervisor.getStack(3) == null ? this.mStackSupervisor.isFocusedStack(getStack()) : true;
        }
        return z;
    }
    ...
    public void reportFullyDrawnLocked(boolean restoredFromBundle) {
        ...
        this.mStackSupervisor.mActivityMetricsLogger.logAppTransitionReportedDrawn(this, restoredFromBundle);
    ...
    public boolean keyDispatchingTimedOut(String reason, int windowPid) {
        ...
        synchronized (this.service) {
            try {
                ActivityManagerService.boostPriorityForLockedSection();
                ActivityRecord anrActivity = getWaitingHistoryRecordLocked();
                ProcessRecord anrApp = this.app;
                boolean windowFromSameProcessAsActivity = this.app == null || this.app.pid == windowPid || windowPid == -1;
            } finally {
                ActivityManagerService.resetPriorityAfterLockedSection();
            }
        }
    ...
    public boolean okToShowLocked() {
        boolean z = false;
        if (!StorageManager.isUserKeyUnlocked(this.userId) && (this.info.applicationInfo.isEncryptionAware() ^ 1) != 0) {
            return false;
        }
        if ((this.info.flags & 1024) != 0) {
            z = true;
        } else if (this.mStackSupervisor.isCurrentProfileLocked(this.userId)) {
            z = this.service.mUserController.isUserRunningLocked(this.userId, 0);
        }
        return z;
    }
    ...
    void onOverrideConfigurationChanged(Configuration newConfig) {
        ...
        if (!currentConfig.equals(newConfig) || (r0 ^ 1) == 0) {
    ...
    private void updateOverrideConfiguration() {
        ...
        if (!this.mTmpBounds.equals(this.mBounds) || (r6 ^ 1) == 0) {
            this.mBounds.set(this.mTmpBounds);
            if (!(this.mBounds.isEmpty() && r6 == 0)) {
    ...
    protected void computeBounds(Rect outBounds) {
        ...
        ActivityStack stack = getStack();
        if (this.task != null && stack != null && (this.task.mFullscreen ^ 1) == 0 && maxAspectRatio != 0.0f && !isInVrUiMode(getConfiguration())) {
            ...
            if (containingAppWidth > maxActivityWidth || containingAppHeight > maxActivityHeight) {
                ...
                outBounds.offsetTo(this.service.mWindowManager.getNavBarPosition() == 1 ? configuration.appBounds.right - outBounds.width() : 0, 0);
                return;
            }
    ...
    void relaunchActivityLocked(boolean andResume, boolean preserveWindow) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_SWITCH) {
            Slog.v(TAG_SWITCH, "Relaunching: " + this + " with results=" + list + " newIntents=" + pendingNewIntents + " andResume=" + andResume + " preserveWindow=" + preserveWindow);
        }
        if (andResume) {
        ...
        try {
            ...
            this.app.thread.scheduleRelaunchActivity(this.appToken, list, pendingNewIntents, this.configChangeFlags, andResume ^ 1, new Configuration(this.service.getGlobalConfiguration()), new Configuration(getMergedOverrideConfiguration()), preserveWindow);
        } catch (RemoteException e) {
            if (ActivityManagerDebugConfig.DEBUG_SWITCH || ActivityManagerDebugConfig.DEBUG_STATES) {
    ...
    static ActivityRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws IOException, XmlPullParserException {
        ...
        while (true) {
            ...
            if (event == 1 || (event == 3 && in.getDepth() < outerDepth)) {
                ...
                aInfo = stackSupervisor.resolveActivity(intent, resolvedType, 0, null, userId);
                if (aInfo != null) {
                    throw new XmlPullParserException("restoreActivity resolver error. Intent=" + intent + " resolvedType=" + resolvedType);
                }
                ActivityRecord r = HwServiceFactory.createActivityRecord(service, null, 0, launchedFromUid, launchedFromPackage, intent, resolvedType, aInfo, service.getConfiguration(), null, null, 0, componentSpecified, false, stackSupervisor, null, null);
        ...
        if (intent != null) {
            ...
            aInfo = stackSupervisor.resolveActivity(intent, resolvedType, 0, null, userId);
            if (aInfo != null) {
                ActivityRecord r2 = HwServiceFactory.createActivityRecord(service, null, 0, launchedFromUid, launchedFromPackage, intent, resolvedType, aInfo, service.getConfiguration(), null, null, 0, componentSpecified, false, stackSupervisor, null, null);
    ...
    boolean canShowWhenLocked() {
        return !this.mShowWhenLocked ? this.service.mWindowManager.containsShowWhenLockedWindow(this.appToken) : true;
    }
    ...
    boolean canTurnScreenOn() {
        ActivityStack stack = getStack();
        if (!this.mTurnScreenOn || stack == null) {
            return false;
        }
        return stack.checkKeyguardVisibility(this, true, true);
    }

...
public class ActivityStarter extends AbsActivityStarter {
    ...
    private boolean mPutIntoExistingTask;
    ...
    private void reset() {
        ...
        this.mReusedActivity = null;
        this.mPutIntoExistingTask = false;
    }
    ...
    private int startActivity(IApplicationThread caller, Intent intent, Intent ephemeralIntent, String resolvedType, ActivityInfo aInfo, ResolveInfo rInfo, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int callingPid, int callingUid, String callingPackage, int realCallingPid, int realCallingUid, int startFlags, ActivityOptions options, boolean ignoreTargetSecurity, boolean componentSpecified, ActivityRecord[] outActivity, TaskRecord inTask) {
        int err = 0;
        Bundle popAppVerificationBundle = options != null ? options.popAppVerificationBundle() : null;
        ...
        if (aInfo != null && (this.mHwActivityStarterEx.isAbleToLaunchInVR(this.mService.mContext, aInfo.packageName) ^ 1) != 0) {
            return 0;
        }
        if (caller != null) {
            processRecord = this.mService.getRecordForAppLocked(caller);
        ...
        if (resultTo != null) {
            ...
            if (!(activityRecord == null || requestCode < 0 || (activityRecord.finishing ^ 1) == 0)) {
        ...
        if (!(err != 0 || activityRecord == null || activityRecord.getTask().voiceSession == null || (268435456 & launchFlags) != 0 || activityRecord.info.applicationInfo.uid == aInfo.applicationInfo.uid)) {
            try {
        ...
        ActivityStack stack = resultRecord == null ? null : resultRecord.getStack();
        if (err != 0) {
            if (resultRecord != null) {
                stack.sendActivityResultLocked(-1, resultRecord, resultWho, requestCode, 0, null);
            }
            ActivityOptions.abort(options);
            return err;
        }
        boolean abort = (this.mSupervisor.checkStartAnyActivityPermission(intent, aInfo, resultWho, requestCode, callingPid, callingUid, callingPackage, ignoreTargetSecurity, processRecord, resultRecord, stack, options) ^ 1) | (this.mService.mIntentFirewall.checkStartActivity(intent, callingUid, callingPid, resolvedType, aInfo.applicationInfo) ^ 1);
        ...
        this.mInterceptor.setSourceRecord(activityRecord);
        ...
        if (abort) {
            if (!(resultRecord == null || stack == null)) {
                stack.sendActivityResultLocked(-1, resultRecord, resultWho, requestCode, 0, null);
            }
            ActivityOptions.abort(options);
            return 102;
        ...
        if (!(rInfo == null || rInfo.auxiliaryInfo == null)) {
            intent = createLaunchIntent(rInfo.auxiliaryInfo, ephemeralIntent, callingPackage, popAppVerificationBundle, resolvedType, userId);
            resolvedType = null;
            callingUid = realCallingUid;
            callingPid = realCallingPid;
        ...
        ActivityRecord r = HwServiceFactory.createActivityRecord(this.mService, processRecord, callingPid, callingUid, callingPackage, intent, resolvedType, aInfo, this.mService.getGlobalConfiguration(), resultRecord, resultWho, requestCode, componentSpecified, voiceSession != null, this.mSupervisor, options, activityRecord);
        if (this.mService.shouldPreventActivity(intent, aInfo, r, callingPid, callingUid, processRecord)) {
        ...
        if (voiceSession == null && stack2.mResumedActivity == null && this.mSupervisor.getResumedActivityLocked() != null && r.shortComponentName != null && (r.shortComponentName.equals(this.mSupervisor.getResumedActivityLocked().shortComponentName) ^ 1) != 0 && INCALLUI_ACTIVITY_CLASS_NAME.equals(this.mSupervisor.getResumedActivityLocked().shortComponentName) && this.mService.isStartLauncherActivity(intent, userId) && ((stack2.mLastPausedActivity == null || (INCALLUI_ACTIVITY_CLASS_NAME.equals(stack2.mLastPausedActivity.shortComponentName) ^ 1) != 0) && (this.mService.isSleepingLocked() ^ 1) != 0)) {
            ...
            if (voiceSession == null && (stack2.mResumedActivity == null || stack2.mResumedActivity.info.applicationInfo.uid != callingUid)) {
                if (!this.mService.checkAppSwitchAllowedLocked(callingPid, callingUid, realCallingPid, realCallingUid, "Activity start")) {
                    this.mPendingActivityLaunches.add(new PendingActivityLaunch(r, activityRecord, startFlags, stack2, processRecord));
                    ActivityOptions.abort(options);
                    return 100;
                }
            }
            ...
            err = startActivity(r, activityRecord, voiceSession, voiceInteractor, startFlags, true, options, inTask, outActivity);
            if (Jlog.isUBMEnable() && err >= 0 && intent.getComponent() != null) {
                Jlog.d(272, "AL#" + intent.getComponent().flattenToShortString() + "(" + intent.getAction() + "," + intent.getCategories() + ")");
            }
            return err;
        }
    }
    ...
    int startActivityMayWait(IApplicationThread caller, int callingUid, String callingPackage, Intent intent, String resolvedType, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int startFlags, ProfilerInfo profilerInfo, WaitResult outResult, Configuration globalConfig, Bundle bOptions, boolean ignoreTargetSecurity, int userId, TaskRecord inTask, String reason) {
        ...
        if (intent == null || !intent.hasFileDescriptors()) {
            ...
            if (rInfo2 == null) {
                ...
                synchronized (this.mService) {
                    try {
                        ...
                        z = globalConfig == null ? this.mService.getGlobalConfiguration().diff(globalConfig) == 0 : false;
                        ...
                        if (HwDeviceManager.disallowOp(intent)) {
                            ...
                            res = startActivityLocked(caller, intent, intent2, resolvedType, aInfo2, rInfo2, voiceSession, voiceInteractor, resultTo, resultWho, requestCode, callingPid, callingUid, callingPackage, realCallingPid, realCallingUid, startFlags, options, ignoreTargetSecurity, componentSpecified, outRecord, inTask, reason);
                        ...
                        UiThread.getHandler().post(new Runnable() {
                            public void run() {
                                ...
                                if (context != null) {
                                    Toast toast = Toast.makeText(context, context.getString(33686056), 0);
            ...
            synchronized (this.mService) {
                ...
                Slog.i(TAG, "due to disallow op launching activity aborted");
                UiThread.getHandler().post(/* anonymous class already generated */);
                ActivityManagerService.resetPriorityAfterLockedSection();
                return -96;
            }
        }
    ...
    private int startActivityUnchecked(ActivityRecord r, ActivityRecord sourceRecord, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, int startFlags, boolean doResume, ActivityOptions options, TaskRecord inTask, ActivityRecord[] outActivity) {
        ...
        if (HwPCUtils.isPcCastModeInServer()) {
            if (hasStartedOnOtherDisplay(this.mStartActivity, this.mSourceDisplayId)) {
                ...
                stack = this.mStartActivity.resultTo != null ? this.mStartActivity.resultTo.getStack() : null;
                if (stack != null) {
            ...
            if (killProcessOnOtherDisplay(this.mStartActivity, this.mSourceDisplayId)) {
                reusableIntentActivity = null;
            }
            if (HwPCUtils.enabledInPad() && reusableIntentActivity != null && (("com.android.systemui/.settings.BrightnessDialog".equals(reusableIntentActivity.shortComponentName) || "com.android.incallui".equals(reusableIntentActivity.packageName) || "com.huawei.android.wfdft".equals(reusableIntentActivity.packageName)) && (HwPCUtils.isValidExtDisplayId(reusableIntentActivity.getDisplayId()) ^ 1) != 0)) {
                Slog.i(TAG, "startActivityUnchecked reusedActivity :" + reusableIntentActivity);
                reusableIntentActivity = null;
            }
        } else {
        ...
        if (!(reusableIntentActivity == null || reusableIntentActivity.getTask() == null)) {
            ...
            if (HwPCUtils.enabledInPad() && HwPCUtils.isPcCastModeInServer()) {
                ActivityRecord activityRecord = setTargetStackAndMoveToFrontIfNeeded(reusableIntentActivity);
                if (activityRecord != null) {
                    reusableIntentActivity = activityRecord;
                }
            } else {
                reusableIntentActivity = setTargetStackAndMoveToFrontIfNeeded(reusableIntentActivity);
            }
            ActivityRecord activityRecord2 = (outActivity == null || outActivity.length <= 0) ? null : outActivity[0];
            if (activityRecord2 != null && (activityRecord2.finishing || activityRecord2.noDisplay)) {
                outActivity[0] = reusableIntentActivity;
            }
            if ((this.mStartFlags & 1) != 0) {
                resumeTargetStackIfNeeded();
                return 1;
            }
            setTaskFromIntentActivity(reusableIntentActivity);
        ...
        if (this.mStartActivity.resultTo == null && this.mInTask == null && (this.mAddingToTask ^ 1) != 0 && (this.mLaunchFlags & 268435456) != 0) {
            newTask = true;
            result = setTaskFromReuseOrCreateNewTask(task2, preferredLaunchStackId, topStack);
            ...
            if (HwPCUtils.isPcCastModeInServer() && this.mSourceRecord.getTask() == null && this.mSourceRecord.getStack() == null) {
                Slog.i(TAG, "ActivityStarter startActivityUnchecked task and stack null");
    ...
    protected void setInitialState(ActivityRecord r, ActivityOptions options, TaskRecord inTask, boolean doResume, int startFlags, ActivityRecord sourceRecord, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor) {
        ...
        if (!(this.mOptions == null || this.mOptions.getLaunchTaskId() == -1 || !this.mOptions.getTaskOverlay())) {
            ...
            if (!this.mOptions.canTaskOverlayResume()) {
                ...
                ActivityRecord topActivity = task != null ? task.getTopActivity() : null;
                if (!(topActivity == null || topActivity.state == ActivityState.RESUMED)) {
                    this.mDoResume = false;
    ...
    private ActivityRecord getReusableIntentActivity() {
        ...
        this.mPutIntoExistingTask = putIntoExistingTask;
        if (this.mOptions != null && this.mOptions.getLaunchTaskId() != -1) {
            TaskRecord task = this.mSupervisor.anyTaskForIdLocked(this.mOptions.getLaunchTaskId());
            if (task != null) {
                intentActivity = task.getTopActivity();
            } else {
                intentActivity = null;
            }
        ...
        this.mReusedActivity = intentActivity;
    ...
    private ActivityRecord setTargetStackAndMoveToFrontIfNeeded(ActivityRecord intentActivity) {
        ...
        ActivityRecord topRunningNonDelayedActivityLocked = focusStack == null ? null : focusStack.topRunningNonDelayedActivityLocked(this.mNotTop);
        TaskRecord task = topRunningNonDelayedActivityLocked != null ? topRunningNonDelayedActivityLocked.getTask() : null;
        if (!(task == null || ((task == intentActivity.getTask() && task == focusStack.topTask()) || (this.mAvoidMoveToFront ^ 1) == 0))) {
    ...
    private int setTaskFromSourceRecord() {
        ...
        if (sourceStack.topTask() == sourceTask) {
            canBeLaunchedOnDisplay = this.mStartActivity.canBeLaunchedOnDisplay(targetDisplayId) ^ 1;
        } else {
            canBeLaunchedOnDisplay = 1;
        }
        if (canBeLaunchedOnDisplay != 0) {
    ...
    private ActivityStack computeStackFocus(ActivityRecord r, boolean newTask, Rect bounds, int launchFlags, ActivityOptions aOptions) {
        ...
        ActivityStack stack2 = task != null ? task.getStack() : null;
        if (stack2 != null) {
    ...
    private boolean canLaunchIntoFocusedStack(ActivityRecord r, boolean newTask) {
        ...
        switch (focusedStackId) {
            case 1:
                ...
                break;
            case 2:
                z = r.supportsFreeform();
                break;
            case 3:
                z = r.supportsSplitScreen();
                break;
            case 6:
                z = r.isAssistantActivity();
                break;
            default:
                if (!HwPCUtils.isExtDynamicStack(focusedStackId)) {
                    if (!StackId.isDynamicStack(focusedStackId)) {
                        z = false;
                        break;
                    }
                    z = r.canBeLaunchedOnDisplay(focusedStack.mDisplayId);
                    break;
                }
                return false;
        }
        if (z && (newTask ^ 1) != 0 && this.mSourceDisplayId == focusedStack.mDisplayId) {
            z2 = true;
        }
        return z2;
    }
    ...
    boolean isValidLaunchStackId(int stackId, int displayId, ActivityRecord r) {
        switch (stackId) {
            ...
            default:
                if (HwVRUtils.isVRDynamicStack(stackId)) {
                    Slog.i(TAG, "isVRDynamicStack StackId = " + stackId);
                    return true;
                } else if (HwPCUtils.isPcDynamicStack(stackId) && this.mService.mSupportsMultiDisplay && HwPCUtils.isPcCastModeInServer()) {
                    return true;
                } else {
    ...
    void dump(PrintWriter pw, String prefix, String dumpPackage) {
        ...
        if (dumpPackage == null || ((this.mLastStartActivityRecord[0] != null && (dumpPackage.equals(this.mLastHomeActivityStartRecord[0].packageName) ^ 1) == 0) || ((this.mLastHomeActivityStartRecord[0] != null && (dumpPackage.equals(this.mLastHomeActivityStartRecord[0].packageName) ^ 1) == 0) || (this.mStartActivity != null && (dumpPackage.equals(this.mStartActivity.packageName) ^ 1) == 0)))) {
            ...
            if (ActivityManagerDebugConfig.DEBUG_TASKS) {
                if (this.mSourceRecord != null) {
                    pw.println(prefix + "mSourceRecord=" + this.mSourceRecord);
                }
                if (this.mSourceStack != null) {
                    pw.println(prefix + "mSourceStack=" + this.mSourceStack);
                }
                if (this.mInTask != null) {
                    pw.println(prefix + "mInTask=" + this.mInTask);
                }
                if (this.mReuseTask != null) {
                    pw.println(prefix + "mReuseTask=" + this.mReuseTask);
                }
                if (this.mReusedActivity != null) {
                    pw.println(prefix + "mReusedActivity=" + this.mReusedActivity);
                }
            }
            if (this.mOptions != null) {
                ...
                if (ActivityManagerDebugConfig.DEBUG_TASKS) {
                    pw.println(prefix + "mLaunchStackId=" + this.mOptions.getLaunchStackId() + " mLaunchTaskId=" + this.mOptions.getLaunchTaskId());
                }
            }
            pw.print(prefix);
            ...
            if (ActivityManagerDebugConfig.DEBUG_TASKS) {
                pw.println(prefix + "mAvoidMoveToFront=" + this.mAvoidMoveToFront + " mMovedToFront=" + this.mMovedToFront + " mMovedOtherTask=" + this.mMovedOtherTask + " mPutIntoExistingTask=" + this.mPutIntoExistingTask);
            }
            return;
        }
        pw.print(prefix);

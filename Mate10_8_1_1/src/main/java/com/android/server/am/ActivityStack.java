...
public class ActivityStack<T extends StackWindowController> extends AbsActivityStack implements StackWindowListener {
    ...
    public ActivityStack(ActivityDisplay display, int stackId, ActivityStackSupervisor supervisor, RecentTasks recentTasks, boolean onTop) {
        Rect rect = null;
        ...
        this.mIsAnimationBoostEnabled = this.mService.mContext.getResources().getBoolean(17956964);
        ...
        this.mIsPerfBoostEnabled = this.mService.mContext.getResources().getBoolean(17956948);
    ...
    void findTaskLocked(ActivityRecord target, FindTaskResult result) {
        ...
        if (this.mStackId == 1 && "com.huawei.systemmanager/.applock.password.AuthLaunchLockedAppActivity".equals(target.shortComponentName)) {
            shouldPrintLog = true;
        }
        for (int taskNdx = this.mTaskHistory.size() - 1; taskNdx >= 0; taskNdx--) {
            TaskRecord task = (TaskRecord) this.mTaskHistory.get(taskNdx);
                ...
                if (!(r == null || r.finishing)) {
                    ...
                    if (r.launchMode != 3) {
                        if (r.mActivityType == target.mActivityType) {
                            ...
                            if (taskIntent != null && taskIntent.getComponent() != null && taskIntent.getComponent().compareTo(cls) == 0 && Objects.equals(data, r11)) {
                                if (ActivityManagerDebugConfig.DEBUG_TASKS || shouldPrintLog) {
                                    Slog.d(TAG_TASKS, "Found matching taskIntent for " + intent + " bringing to top: " + r.intent);
                                }
                                result.r = r;
                                result.matchedByRootAffinity = false;
                                return;
                            } else if (affinityIntent != null && affinityIntent.getComponent() != null && affinityIntent.getComponent().compareTo(cls) == 0 && Objects.equals(data, r11)) {
                                if (ActivityManagerDebugConfig.DEBUG_TASKS || shouldPrintLog) {
                                    Slog.d(TAG_TASKS, "Found matching affinityIntent For " + intent + " bringing to top: " + r.intent);
                                }
                                result.r = r;
                                result.matchedByRootAffinity = false;
                                return;
                                ...
                                Slog.d(TAG_TASKS, "Not found matching affinity candidate!");
                            }
                        } else if (ActivityManagerDebugConfig.DEBUG_TASKS) {
    ...
    void setLaunchTime(ActivityRecord r) {
        ...
        if (r.displayStartTime == 0) {
            ...
            if (r.app != null) {
                Jlog.d(43, r.shortComponentName, r.app.pid, "");
            } else {
                Jlog.d(43, r.shortComponentName, 0, "");
            }
            if (r.task != null) {
    ...
    boolean goToSleepIfPossible(boolean shuttingDown) {
        ...
        if (this.mResumedActivity != null) {
            if (ActivityManagerDebugConfig.DEBUG_PAUSE) {
                Slog.v(TAG_PAUSE, "Sleep still need to pause " + this.mResumedActivity);
            }
    ...
    final boolean startPausingLocked(boolean userLeaving, boolean uiSleeping, ActivityRecord resuming, boolean pauseImmediately) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_STATES) {
            Slog.v(TAG_STATES, "Moving to PAUSING: " + prev + " in stack " + this.mStackId + " callers=" + Debug.getCallers(5));
        } else {
...
jadx.core.utils.exceptions.CodegenException: Error generate insn: PHI: (r11_1 'taskNdx' int) = (r11_0 'taskNdx' int), (r11_4 'taskNdx' int) binds: {(r11_0 'taskNdx' int)=B:8:0x0010, (r11_4 'taskNdx' int)=B:26:0x0069} in method: com.android.server.am.ActivityStack.findNextTranslucentActivity(com.android.server.am.ActivityRecord):com.android.server.am.ActivityRecord
...
*/
                    ...
                    if (topStackId == 3 || StackId.isAlwaysOnTop(topStackId)) {
                        if (stackIndex == stackBehindTopIndex) {
                            ...
                            if (stackBehindTopId == 6) {
                                return ((ActivityStack) this.mStacks.get(stackBehindTopIndex)).isStackTranslucent(starting, this.mStackId) ? 1 : 0;
                            }
                        }
                    }
                    if (StackId.isAlwaysOnTop(topStackId) && StackId.isBackdropToTranslucentActivity(stackBehindTopId) && stackBehindTopIndex >= 0 && ((ActivityStack) this.mStacks.get(stackBehindTopIndex)).isStackTranslucent(starting, 0) && stackIndex == stackBehindTopIndex - 1) {
                        return 1;
                    }
                                ...
                                if (!r.finishing) {
                                    ...
                                    if (!aboveTop || (isTop ^ 1) == 0) {
                                        ...
                                        if (reallyVisible) {
                                            ...
                                            if (!behindFullscreenActivity && (("com.android.server.telecom/.components.ErrorDialogActivity".equals(r.shortComponentName) || "com.android.phone/.MMIDialogActivity".equals(r.shortComponentName)) && r == top && r.visible)) {
                                            ...
                                            if ((r.state == ActivityState.RESUMED) || ActivityManagerDebugConfig.DEBUG_VISIBILITY) {
                                                ...
                                                builder.append(this.mStackSupervisor.mKeyguardController.isKeyguardShowing(this.mDisplayId != -1 ? this.mDisplayId : 0));
                                                builder.append(", keyguardLocked = ");
                                                builder.append(this.mStackSupervisor.mKeyguardController.isKeyguardLocked());
                                                builder.append(", r.visibleIgnoringKeyguard = ");
            ...
            boolean checkKeyguardVisibility(ActivityRecord r, boolean shouldBeVisible, boolean isTop) {
                boolean z = false;
                ActivityStack stack = r.getStack();
                if (stack == null || (shouldBeVisible ^ 1) != 0) {
            ...
            private boolean resumeTopActivityInnerLocked(ActivityRecord prev, ActivityOptions options) {
                ...
                if (hasRunningActivity) {
                    ...
                    TaskRecord task = prev != null ? prev.getTask() : null;
                    if (task != null && task.getStack() == this && task.isOverHomeStack() && prev.finishing && prev.frontOfTask) {
                        if (ActivityManagerDebugConfig.DEBUG_STACK) {
                            this.mStackSupervisor.validateTopActivitiesLocked();
                        }
                        if (task == nextTask) {
                            task.setFrontOfTask();
                        } else if (task != topTask()) {
                            ((TaskRecord) this.mTaskHistory.get(this.mTaskHistory.indexOf(task) + 1)).setTaskToReturnTo(1);
                    ...
                    if (shouldSleepOrShutDownActivities() && this.mLastPausedActivity == next && this.mStackSupervisor.allPausedActivitiesComplete()) {
                        ...
                        if (this.mStackSupervisor.allPausedActivitiesComplete()) {
                            ...
                            if ((next.info.flags & 16384) != 0) {
                                i = lastResumedCanPip ^ 1;
                            } else {
                                i = 0;
                            }
                            boolean pausing = this.mStackSupervisor.pauseBackStacks(userLeaving, next, false);
                            if (this.mResumedActivity != null && (!HwPCUtils.isPcCastModeInServer() || (SETTINGS_DASHBROAED_ACTIVITY_NAME.equals(this.mResumedActivity.info.name) && (HwPCUtils.isPcDynamicStack(this.mStackId) ^ 1) != 0))) {
                            ...
                            if (pausing && (r34 ^ 1) != 0) {
                                ...
                                if (prev == null) {
                                    ...
                                    if (this.mNoAnimActivities.contains(prev)) {
                                        ...
                                        r4 = this.mWindowManager;
                                        if (prev.getTask() == next.getTask()) {
                                            r3 = 7;
                                        } else {
                                            r3 = 9;
                                        }
                                        r4.prepareAppTransition(r3, false);
                                ...
                                if (next.app == null || next.app.thread == null) {
                                    ...
                                    synchronized (this.mWindowManager.getWindowManagerLock()) {
                                        if (!(next.visible && !next.stopped && r18 == 0)) {
                                        ...
                                        this.mService.updateCpuStats();
                                        if (ActivityManagerDebugConfig.DEBUG_STATES) {
                                            Slog.v(TAG_STATES, "Moving to RESUMED: " + next + " (in existing)" + " callers=" + Debug.getCallers(5));
                                        }
                                        ...
                                        try {
                                            ...
                                            int canBoost = this.mService.mHwAMSEx.canAppBoost(next.appInfo);
                                            if (bundle == null) {
                                                bundle = new Bundle();
                                            }
                                            bundle.putInt("activity.canboost", canBoost);
                    ...
                    for (int taskNdx = this.mTaskHistory.size() - 1; taskNdx >= 0; taskNdx--) {
                        ...
                        if (taskRecord.getTopActivity() != null) {
                            if (taskRecord == rTask) {
                                ...
                                Flog.i(101, "starting r: " + r + " blocked by task: " + taskRecord);
                                startIt = false;
                            }
                        }
                    }
            ...
            private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
                ...
                if (removeActivity || onlyHasTaskOverlayActivities) {
                    if (ActivityManagerDebugConfig.DEBUG_STACK) {
                        Slog.i(TAG_STACK, "removeActivityFromHistoryLocked: last activity removed from " + this + " onlyHasTaskOverlays=" + onlyHasTaskOverlayActivities);
                    }
            ...
            final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp, String reason) {
                ...
                if (hadApp) {
                    ...
                    r.nowVisible = false;
                    int i = r.finishing ? skipDestroy ^ 1 : 0;
                    Flog.i(101, "Moving to" + (i != 0 ? " DESTROYING: " : " DESTROYED: ") + r + " in stack " + this.mStackId);
                    if (i != 0) {
                        if (ActivityManagerDebugConfig.DEBUG_STATES) {
            ...
            protected boolean moveTaskToBackLocked(int taskId) {
                ...
                if (!(isOverHomeStack || topTask == null || topTask.topRunningActivityLocked() != null || !topTask.isOverHomeStack() || topTask.mActivities == null || topTask.mActivities.size() == 0)) {
                    isOverHomeStack = true;
                ...
                if (!prevIsHome && ((task != tr || !r3) && ((numTasks > 1 && !noValidateTask) || !isOnHomeDisplay()))) {
                ...
                if (updateBoundsAllowed(bounds, tempTaskBounds, tempTaskInsetBounds)) {
                    ...
                    for (int i = this.mTaskHistory.size() - 1; i >= 0; i--) {
                        ...
                        if (task.isResizeable()) {
                            if (this.mStackId == 2 || HwPCUtils.isExtDynamicStack(this.mStackId)) {
                    ...
                    while (activityNdx < numActivities) {
                        try {
                            ...
                            if (r.packageName.equals(packageName)) {
                                ...
                                sameComponent = true;
                                if ((userId == -1 || r.userId == userId) && ((sameComponent || r.getTask() == r12) && (r.app == null || evenPersistent || (r.app.persistent ^ 1) != 0))) {
                                    if (!doit) {
                                        if (r.isHomeActivity()) {

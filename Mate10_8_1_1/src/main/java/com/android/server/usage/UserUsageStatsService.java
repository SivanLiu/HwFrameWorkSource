        ...
        while (intervalType < this.mCurrentStats.length) {
            ...
            if (force || this.mCurrentStats[intervalType] == null || this.mCurrentStats[intervalType].beginTime != tempCal.getTimeInMillis()) {
                ...
                if (lastBeginTime >= tempCal.getTimeInMillis()) {
                    Slog.d(TAG, this.mLogPrefix + "Loading existing stats " + lastBeginTime + " for interval " + intervalType);
                    this.mCurrentStats[intervalType] = this.mDatabase.getLatestUsageStats(intervalType);
                } else {
                    Slog.d(TAG, "latestBeginTime(" + lastBeginTime + ") < expiryTime(" + tempCal.getTimeInMillis() + "), will create new stats for interval " + intervalType);

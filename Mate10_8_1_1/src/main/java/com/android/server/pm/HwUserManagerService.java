...
public class HwUserManagerService extends UserManagerService {
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        switch (code) {
            case 1001:
                Flog.i(HwMtmBroadcastResourceManager.CACHED_APP_MIN_ADJ, "onTransact MKDIR_FOR_USER_TRANSACTION.");
    ...
    void setDeviceProvisioned(int userId) {
        Flog.i(HwMtmBroadcastResourceManager.CACHED_APP_MIN_ADJ, "HwUserManagerService setDeviceProvisioned, userId " + userId);
        ...
        try {
            if ((Global.getInt(cr, "device_provisioned", 0) == 0 || Secure.getIntForUser(cr, "user_setup_complete", 0, userId) == 0) && ((PackageManagerService) ServiceManager.getService("package")).isSetupDisabled()) {
                Flog.i(HwMtmBroadcastResourceManager.CACHED_APP_MIN_ADJ, "Setup is disabled putInt USER_SETUP_COMPLETE for userId " + userId);

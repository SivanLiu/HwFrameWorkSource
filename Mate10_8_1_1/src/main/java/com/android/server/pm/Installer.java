...
public class Installer extends SystemService {
    ...
    private void connect() {
        ...
        BackgroundThread.getHandler().postDelayed(new -$Lambda$iCTRLJcHnavjRcatPDKSIvElD0U((byte) 0, this), 1000);
    }

    /* synthetic */ void lambda$-com_android_server_pm_Installer_4931() {
        connect();
    }
    ...
    public void dexopt(String apkPath, int uid, String pkgName, String instructionSet, int dexoptNeeded, String outputPath, int dexFlags, String compilerFilter, String volumeUuid, String sharedLibraries, String seInfo, boolean downgrade) throws InstallerException {
        assertValidInstructionSet(instructionSet);
        if (checkBeforeRemote()) {
            try {
                this.mInstalld.dexopt(apkPath, uid, pkgName, instructionSet, dexoptNeeded, outputPath, dexFlags, compilerFilter, volumeUuid, sharedLibraries, seInfo, downgrade);
            } catch (Exception e) {
                throw InstallerException.from(e);
            }
        }
    ...
    public boolean copySystemProfile(String systemProfile, int uid, String packageName) throws InstallerException {
        if (!checkBeforeRemote()) {
            return false;
        }
        try {
            return this.mInstalld.copySystemProfile(systemProfile, uid, packageName);
        } catch (Exception e) {
            throw InstallerException.from(e);
        }
    }

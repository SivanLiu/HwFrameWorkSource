...
public class TrustManagerService extends SystemService {
    ...
    private final IBinder mService = new Stub() {
        static /* synthetic */ void lambda$-com_android_server_trust_TrustManagerService$1_35602() {
        }
        ...
        public void reportKeyguardShowingChanged() throws RemoteException {
            ...
            TrustManagerService.this.mHandler.runWithScissors(-$Lambda$RuWUhZ3VT8Cf-F-WFFkHRfck9sI.$INST$0, 0);
        }
    ...
    public void updateTrust(int userId, int flags) {
        ...
        synchronized (this.mUserIsTrusted) {
            boolean changed = this.mUserIsTrusted.get(userId) != trusted;
    ...
    void refreshAgentList(int userIdOrAll) {
        ...
        if (this.mTrustAgentsCanRun) {
                                    ...
                                    for (ResolveInfo resolveInfo : resolveAllowedTrustAgents(pm, userInfo.id)) {
                                        ...
                                        if (enabledAgents.contains(name)) {
                                            ...
                                            if (resolveInfo.serviceInfo.directBootAware) {
                                                i = agentInfo.settings.canUnlockProfile;
                                            } else {
                                                i = 0;
                                            }
                                            if (i != 0 && DEBUG) {
                                            ...
                                            if (this.mUserManager.isUserUnlockingOrUnlocked(userInfo.id) || (i ^ 1) == 0) {
            ...
            for (int i2 = 0; i2 < obsoleteAgents.size(); i2++) {
                AgentInfo info = (AgentInfo) obsoleteAgents.valueAt(i2);
    ...
    private void refreshDeviceLockedForUser(int userId) {
        ...
        for (int i = 0; i < userInfos.size(); i++) {
            ...
            if (!(info == null || info.partial || (info.isEnabled() ^ 1) != 0 || info.guestToRemove || (info.supportsSwitchToByUser() ^ 1) != 0)) {
                ...
                if (secure && r8 && (trusted ^ 1) != 0) {
                    z = fingerprintAuthenticated ^ 1;
                } else {
                    z = false;
                }
                setDeviceLockedForUser(id, z);
            }
        }
    }
    ...
    private void setDeviceLockedForUser(int userId, boolean locked) {
        synchronized (this.mDeviceLockedForUser) {
            boolean changed = isDeviceLockedInner(userId) != locked;
    ...
    private static ComponentName getDefaultFactoryTrustAgent(Context context) {
        String defaultTrustAgent = context.getResources().getString(17039774);

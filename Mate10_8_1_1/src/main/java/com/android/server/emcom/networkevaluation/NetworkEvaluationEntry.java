...
public class NetworkEvaluationEntry {
    ...
    private final LinkedList<INetworkEvaluationCallback> mCallbackList = new LinkedList();
    ...
    private void respondCallbackFunctions(int quality, int bottleneck, int networkType) {
        ...
        synchronized (this.mCallbackList) {
            ...
            for (int i = 0; i < size; i++) {
                INetworkEvaluationCallback callback = (INetworkEvaluationCallback) this.mCallbackList.get(i);
                if (callback != null) {
                    callback.onEvaluationResultChanged(result);
                }
            }
        }
    }

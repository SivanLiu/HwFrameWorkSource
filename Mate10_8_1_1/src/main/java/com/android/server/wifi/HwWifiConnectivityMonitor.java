...
public class HwWifiConnectivityMonitor extends StateMachine {
    ...
    private static final int CMD_WECHAT_WIFI_STATE_BAD = 115;
    ...
    private static final int[] DELAYED_MS_TABLE = new int[]{2000, 4000, HwQoEWeChatBooster.WECHART_COLLECT_HANDOVER_INFO_TIME, 30000, 0};
    ...
    class ConnectedMonitorState extends State {
        ...
        private void handleForegroundAppChanged(Message message) {
            if (this.mLastSignalLevel == 1) {
                HwWifiConnectivityMonitor.this.LOGD("handleForegroundAppChanged, current app : " + message.obj);
                HwWifiConnectivityMonitor.this.sendMessageDelayed(110, PortalAutoFillManager.AUTO_FILL_PW_DELAY_MS);
            }
        }
        ...
        private void switchWifiNetworkQuickly() {
            if (this.mCurrMonitorTopUid != -1 && this.mCurrMonitorTopUid == WifiProCommonUtils.getForegroundAppUid(HwWifiConnectivityMonitor.this.mContext)) {
                String pktName = WifiProCommonUtils.getPackageName(HwWifiConnectivityMonitor.this.mContext, this.mCurrMonitorTopUid);
                int isInMonitorList = HwWifiConnectivityMonitor.this.isFullScreen() ? WifiProCommonUtils.isInMonitorList(pktName, HwWifiConnectivityMonitor.URGENT_MINI_APP_PKT_NAME) ^ 1 : 0;
                if (!HwWifiConnectivityMonitor.this.isMobileDataInactive() && (HwWifiConnectivityMonitor.this.mPowerManager.isScreenOn() ^ 1) == 0 && isInMonitorList == 0 && !WifiProCommonUtils.isLandscapeMode(HwWifiConnectivityMonitor.this.mContext)) {
                    WifiInfo wifiInfo = HwWifiConnectivityMonitor.this.mWifiManager.getConnectionInfo();
                    if (wifiInfo != null) {
                        this.mCurrRssiVal = wifiInfo.getRssi();
                        if (this.mLastSignalLevel <= 1) {
                            ...
                            if (WifiProCommonUtils.isInMonitorList(pktName, HwWifiConnectivityMonitor.URGENT_MINI_APP_PKT_NAME)) {
                                ...
                                if (this.mCurrTopUidBadCnt < 2 && this.mCurrTopUidVeryBadCnt < 2) {
                                    if (this.mCurrTopUidBadCnt == 1 && this.mCurrTopUidVeryBadCnt == 1) {
                                    }
                                }
                            ...
                            if (this.mCurrTopUidBadCnt < 2 || this.mCurrTopUidVeryBadCnt < 1) {
                                if (this.mCurrTopUidVeryBadCnt >= 2) {
                                }
                            }
                            ...
                            notifyWifiLinkPoor(true);
                        } else if (this.mCurrRssiVal > -70 && WifiProCommonUtils.isOpenType(this.mConnectedConfig) && (this.mCurrTopUidVeryBadCnt >= 3 || (this.mCurrTopUidBadCnt == 1 && this.mCurrTopUidVeryBadCnt == 2))) {
        ...
        private void handleNewRssiRcvd(int newRssi) {
            ...
            if (currentSignalLevel >= 0 && currentSignalLevel != this.mLastSignalLevel) {
                ...
                if (is11vNetworkConnected() && (this.m11vRoamingOnGoing ^ 1) != 0) {
                    if (currentSignalLevel <= 2 && this.m11vRoamingFailedCounter <= 1) {
                        if (HwWifiConnectivityMonitor.this.hasMessages(103)) {
                            HwWifiConnectivityMonitor.this.removeMessages(103);
                        }
        ...
        private void handleSignalPoorLevelOne() {
            if (!(this.mConnectedConfig == null || (HwWifiConnectivityMonitor.this.isMobileDataInactive() ^ 1) == 0)) {
                int foregroundUid = WifiProCommonUtils.getForegroundAppUid(HwWifiConnectivityMonitor.this.mContext);
                if (!WifiProCommonUtils.isWpaOrWpa2(this.mConnectedConfig) && HwUidTcpMonitor.getInstance(HwWifiConnectivityMonitor.this.mContext).isAppAccessInternet(foregroundUid)) {
                    notifyWifiLinkPoor(true);
                } else if (WifiProCommonUtils.isInMonitorList(WifiProCommonUtils.getPackageName(HwWifiConnectivityMonitor.this.mContext, this.mCurrMonitorTopUid), HwWifiConnectivityMonitor.URGENT_MINI_APP_PKT_NAME)) {
        ...
        private void handleUserMoveDetected() {
            ...
            int isInMonitorList = HwWifiConnectivityMonitor.this.isFullScreen() ? WifiProCommonUtils.isInMonitorList(WifiProCommonUtils.getPackageName(HwWifiConnectivityMonitor.this.mContext, this.mCurrMonitorTopUid), HwWifiConnectivityMonitor.URGENT_MINI_APP_PKT_NAME) ^ 1 : 0;
            if (HwWifiConnectivityMonitor.this.mPowerManager.isScreenOn() && (HwWifiConnectivityMonitor.this.isMobileDataInactive() ^ 1) != 0 && (isInMonitorList ^ 1) != 0 && (WifiProCommonUtils.isCalling(HwWifiConnectivityMonitor.this.mContext) ^ 1) != 0 && (WifiProCommonUtils.isLandscapeMode(HwWifiConnectivityMonitor.this.mContext) ^ 1) != 0) {
    ...
    public synchronized void weChatPoorWiFi() {
        if (this.mInitialized) {
            sendMessage(115);
        }
    }

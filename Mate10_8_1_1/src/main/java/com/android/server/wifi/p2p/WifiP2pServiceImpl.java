...
public class WifiP2pServiceImpl extends AbsWifiP2pService {
    ...
    protected class P2pStateMachine extends StateMachine {
        ...
        class FrequencyConflictState extends State {
            ...
            private void notifyFrequencyConflict() {
                ...
                AlertDialog dialog = new Builder(WifiP2pServiceImpl.this.mContext).setMessage(r.getString(17041280, new Object[]{P2pStateMachine.this.getDeviceName(P2pStateMachine.this.mSavedPeerConfig.deviceAddress)})).setPositiveButton(r.getString(17039912), new OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        P2pStateMachine.this.sendMessage(WifiP2pServiceImpl.DROP_WIFI_USER_ACCEPT);
                    }
                }).setNegativeButton(r.getString(17039879), new OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
            ...
            public boolean processMessage(Message message) {
                ...
                switch (message.what) {
                    ...
                    case WifiP2pMonitor.P2P_GO_NEGOTIATION_REQUEST_EVENT /*147479*/:
                        P2pStateMachine.this.logd(getName() + " deal with P2P_GO_NEGOTIATION_REQUEST_EVENT");
                        P2pStateMachine.this.deferMessage(message);
                        P2pStateMachine.this.transitionTo(P2pStateMachine.this.mInactiveState);
                        break;
                    case WifiP2pMonitor.P2P_GO_NEGOTIATION_SUCCESS_EVENT /*147481*/:
                            ...
                            if (P2pStateMachine.this.mGroup.isGroupOwner()) {
                                ...
                                if (groupOwner != null) {
                                    ...
                                    if (peer != null) {
                                        ...
                                        if (!(groupOwner == null || (WifiP2pServiceImpl.EMPTY_DEVICE_ADDRESS.equals(groupOwner.deviceAddress) ^ 1) == 0)) {
        ...
        private void notifyP2pEnableFailure() {
            Resources r = Resources.getSystem();
            AlertDialog dialog = new Builder(WifiP2pServiceImpl.this.mContext).setTitle(r.getString(17041275)).setMessage(r.getString(17041279)).setPositiveButton(r.getString(17039370), null).create();
            dialog.setCanceledOnTouchOutside(false);
            dialog.getWindow().setType(2003);
            LayoutParams attrs = dialog.getWindow().getAttributes();
            attrs.privateFlags = 16;
        ...
        private void addRowToDialog(ViewGroup group, int stringId, String value) {
            Resources r = Resources.getSystem();
            View row = LayoutInflater.from(WifiP2pServiceImpl.this.mContext).inflate(17367320, group, false);
            ((TextView) row.findViewById(16909080)).setText(r.getString(stringId));
            ((TextView) row.findViewById(16909460)).setText(value);
            group.addView(row);
        }

        protected void notifyInvitationSent(String pin, String peerAddress) {
            Resources r = Resources.getSystem();
            View textEntryView = LayoutInflater.from(WifiP2pServiceImpl.this.mContext).inflate(17367318, null);
            ViewGroup group = (ViewGroup) textEntryView.findViewById(16908972);
            addRowToDialog(group, 17041285, getDeviceName(peerAddress));
            addRowToDialog(group, 17041284, pin);
            AlertDialog dialog = new Builder(WifiP2pServiceImpl.this.mContext).setTitle(r.getString(17041282)).setView(textEntryView).setPositiveButton(r.getString(17039370), null).create();
            dialog.setCanceledOnTouchOutside(false);
            dialog.getWindow().setType(2003);
            LayoutParams attrs = dialog.getWindow().getAttributes();
            attrs.privateFlags = 16;
        ...
        private void notifyP2pProvDiscShowPinRequest(final String pin, final String peerAddress) {
            ...
            addRowToDialog(group, 17041285, getDeviceName(peerAddress));
            addRowToDialog(group, 17041284, pin);
            AlertDialog dialog = new Builder(WifiP2pServiceImpl.this.mContext).setTitle(r.getString(17041282)).setView(textEntryView).setPositiveButton(r.getString(17039524), new OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    P2pStateMachine.this.mSavedPeerConfig = new WifiP2pConfig();
        ...
        private void notifyInvitationReceived() {
            if (!WifiP2pServiceImpl.this.autoAcceptConnection()) {
                ...
                ((TextView) row.findViewById(16909080)).setText(String.format(r.getString(33685699), new Object[]{getDeviceName(this.mSavedPeerConfig.deviceAddress)}));
                ((TextView) row.findViewById(16909080)).setTextColor(-16777216);
                group.addView(row);
                final EditText pin = (EditText) textEntryView.findViewById(16909480);
                ...
                switch (wps.setup) {
                    case 1:
                        ...
                        addRowToDialog(group, 17041284, wps.pin);
                        break;
                    case 2:
                        logd("Enter pin section visible");
                        textEntryView.findViewById(16908856).setVisibility(0);
                        break;
                }
        ...
        private String getPersistedDeviceName() {
            ...
            if (id == null || id.length() <= 4 || (WifiP2pServiceImpl.IS_ATT ^ 1) == 0 || (WifiP2pServiceImpl.IS_VERIZON ^ 1) == 0) {
                return Build.MODEL;
            }
    ...
    static {
        ...
        HWDBG = z;
        if ("389".equals(SystemProperties.get("ro.config.hw_opta", HwWifiCHRStateManager.TYPE_AP_VENDOR))) {
            z = "840".equals(SystemProperties.get("ro.config.hw_optb", HwWifiCHRStateManager.TYPE_AP_VENDOR));
        } else {
            z = false;
        }
        IS_VERIZON = z;
    ...
    public WifiP2pServiceImpl(Context context) {
        ...
        this.mThisDevice.primaryDeviceType = this.mContext.getResources().getString(17039823);
        if (WifiP2pServiceHisiExt.hisiWifiEnabled()) {
    ...
    private void startIpManager(String ifname) {
        ...
        this.mIpManager.startProvisioning(IpManager.buildProvisioningConfiguration().withoutIPv6().withoutIpReachabilityMonitor().withPreDhcpAction(WifiStateMachine.LAST_SELECTED_NETWORK_EXPIRATION_AGE_MILLIS).withProvisioningTimeoutMs(36000).build());
    }
    ...
    /* synthetic */ void lambda$-com_android_server_wifi_p2p_WifiP2pServiceImpl_28131(IBinder binder) {
        Log.d(TAG, "binderDied: binder=" + binder);
        close(binder);
    }

    /* synthetic */ void lambda$-com_android_server_wifi_p2p_WifiP2pServiceImpl_28963() {

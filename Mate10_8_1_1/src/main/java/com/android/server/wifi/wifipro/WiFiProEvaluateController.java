...
public class WiFiProEvaluateController {
    ...
    public boolean isAllowEvaluate(ScanResult scanResult, int evalate_type) {
        ...
        return calculateSignalLevelHW(scanResult.is5GHz(), scanResult.level) >= level && getWifiConfiguration("\"" + scanResult.SSID + "\"") == null && isOpenAP(scanResult);
    }
    ...
    public int calculateSignalLevelHW(boolean is5G, int rssi) {
        return HwFrameworkFactory.getHwInnerWifiManager().calculateSignalLevelHW(is5G ? 5180 : WifiScanGenieController.CHANNEL_1_FREQ, rssi);
    }

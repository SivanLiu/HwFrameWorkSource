...
public class HidataWechatTraffic {
    ...
    public void updateMobileWechatStateChanged(int state, int type, int uid, HiDataCHRStatisticsInfo statisticsInfo) {
        ...
        if (1 == state) {
            ...
            if (statisticsInfo != null) {
                statisticsInfo.mCallInCellularDur = (int) (((long) statisticsInfo.mCallInCellularDur) + this.mWechatCallDuration);
                statisticsInfo.mTrfficCell = (int) (((long) statisticsInfo.mTrfficCell) + (this.mWechatTraffic / 1024));
                updateWeChatCellLevel(this.mWechatCallDuration, statisticsInfo);
            }
            this.mWechatType = 0;
    ...
    private void updateWeChatCellLevel(long time, HiDataCHRStatisticsInfo statisticsInfo) {
        if (time < 300) {
            statisticsInfo.mCellLv1Cnt++;
        } else if (time < 900) {
            statisticsInfo.mCellLv2Cnt++;
        } else {
            statisticsInfo.mCellLv3Cnt++;
        }
    }

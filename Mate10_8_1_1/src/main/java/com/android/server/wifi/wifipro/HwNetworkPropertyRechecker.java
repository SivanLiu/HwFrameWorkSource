...
public class HwNetworkPropertyRechecker extends HwNetworkPropertyChecker {
    ...
    private int recheckNetworkProperty(boolean portal, boolean authen, boolean needSleep, boolean wifiBackground) {
        ...
        if (WifiProCommonUtils.isRedirectedRespCode(respCode) && portal && authen) {
            ...
            respCode = HwSelfCureUtils.RESET_LEVEL_MIDDLE_REASSOC;
        } else if (WifiProCommonUtils.unreachableRespCode(respCode) && isInOversea() && WifiProCommonUtils.isWifiConnected(this.mWifiManager)) {
            ...
            if (!(wifiInfo == null || config == null || !needSleep)) {
                ...
                if (WifiProCommonUtils.getCurrenSignalLevel(wifiInfo) > 2 && (config.noInternetAccess ^ 1) != 0 && WifiProCommonUtils.matchedRequestByHistory(config.internetHistory, 100) && !WifiProCommonUtils.unreachableRespCode(recheckWithBakcupServer(true))) {
                    respCode = HwSelfCureUtils.RESET_LEVEL_MIDDLE_REASSOC;
                }
            }
        }

...
public class HwMSSHandler implements HwMSSHandlerManager, IHwMSSObserver {
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        Log.d(HwMSSHandler.TAG, "setWifiAntImpl result is " + HwMSSHandler.this.setWifiAntImpl(HwMSSHandler.this.mCellPhoneWIFIIface, HwMSSHandler.this.mCellPhoneWIFIMode, HwMSSHandler.this.mCellPhoneWIFIOperation));
                        HwMSSHandler.this.clearMssCount();
                        break;
                    case 99:
    ...
    private boolean mssPreArbitrate(MSS_TRIG_ARBITRATE_TYPE type, int direction) {
        ...
        switch (-getcom-android-server-wifi-MSS-HwMSSHandler$MSS_TRIG_ARBITRATE_TYPESwitchesValues()[type.ordinal()]) {
            case 1:
                if (1 == direction) {
                    if (tputSwitchMimoTresh == 0 || this.mCurrentTput < tputSwitchMimoTresh) {
                        if (this.mssIsHighTput) {
                        }
                    }
        ...
        if (mssSwitchSupportCheck() && (allowMSSSwitch() ^ 1) == 0) {
            ...
            if (tputSwitchToMimoTresh != 0 && tputSwitchToSisoTresh != 0) {
                ...
                if (mTputMIMOCount >= 3) {
                    ...
                    if (this.mssArbi.getMSSCurrentState() == MSSState.MSSMIMO || (mssPreArbitrate(MSS_TRIG_ARBITRATE_TYPE.TPUT_TRIG, 2) ^ 1) != 0) {
                        mTputSISOCount = 0;
                        mTputMIMOCount = 0;
                        return;
                    }
                    ...
                    if (this.mssArbi.getMSSCurrentState() == MSSState.MSSSISO || (mssPreArbitrate(MSS_TRIG_ARBITRATE_TYPE.TPUT_TRIG, 1) ^ 1) != 0) {
                        mTputSISOCount = 0;
                        mTputMIMOCount = 0;
                        return;
                    }
    ...
    private int setWifiAntImpl(String iface, int mode, int operation) {
        ...
        if (!mssSwitchSupportCheck()) {
            ...
            return -1;
        } else if (!this.mssArbi.isMSSAllowed(operation, this.mWifiInfo.getFrequency())) {
            Log.d(TAG, "mss is not allowed!");
            clearMssCount();
            return -1;

...
public class HwImsManagerInner {
    ...
    public static boolean isVtEnabledByPlatform(Context context, int subId) {
        if (!isValidParameter(context, subId)) {
            ...
            if (context.getResources().getBoolean(17956923) && getBooleanCarrierConfig(context, "carrier_vt_available_bool", subId)) {
                isGbaValid = isGbaValid(context, subId);
            } else {
                isGbaValid = false;
            }
            boolean isVoWiFiRegstered = HwTelephonyManagerInner.getDefault().isWifiCallingAvailable(subId);
            if (isGbaValid && isVoWiFiRegstered) {
                isGbaValid &= getBooleanCarrierConfig(context, "carrier_viwifi_available_bool", subId);
            }
            return isGbaValid;
        } else {
    ...
    public static void setImsSmsConfig(Context context, int imsSmsEnabled, int subId) {
        ImsManager imsManager = ImsManager.getInstance(context, subId);
        if (imsManager != null) {
            try {
                ImsConfig config = imsManager.getConfigInterface();

...
public class SubTabWidget extends LinearLayout {
    ...
    private static int sActivedLeft;
    private static int sActivedMiddle;
    private static int sActivedRight;
    private static int sBorderColor;
    private static int sEntityColor;
    private static int sNormalLeft;
    private static int sNormalMiddle;
    private static int sNormalRight;
    private boolean mBackgroundStyle;
    ...
    private static class SubTabView extends TextView {
        private boolean mBackgroundStyleInner;
        ...
        public void updateTitleAppearance(int type) {
            if (type == 0) {
                if (this.mBackgroundStyleInner) {
                    setTextColor(this.mContext.getResources().getColor(33882257));
                } else {
                    setPressed(false);
                }
            ...
            if (this.mBackgroundStyleInner) {
                setTextColor(this.mContext.getResources().getColor(33882254));
            } else {
                setPressed(true);
            }
        ...
        public void setBackgroundStyle(int style) {
            ...
            this.mBackgroundStyleInner = z;
            if (style == 0) {
                setTextColor(this.mContext.getResources().getColorStateList(33882428));
            }
        }
    }
    ...
    public SubTabWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mSubTabContentView = (LinearLayout) ((LayoutInflater) getContext().getSystemService("layout_inflater")).inflate(34013265, this, true).findViewById(34603094);
        this.mContext = context;
        initDividerDrawable();
        ...
        this.mSubTabAnim.mSecondLayerInsetLeft = a.getDimensionPixelSize(4, 0);
        this.mSubTabAnim.mSecondLayerInsetTop = a.getDimensionPixelSize(6, 0);
        this.mSubTabAnim.mSecondLayerInsetRight = a.getDimensionPixelSize(5, 0);
        this.mSubTabAnim.mSecondLayerInsetBottom = a.getDimensionPixelSize(7, 0);
        a.recycle();
    }

    private void initResource(TypedArray a) {
        this.mNormalWidth = a.getDimensionPixelSize(8, 0);
        this.mSmallWidth = a.getDimensionPixelSize(9, 0);
        this.mTextHeightOffset = a.getDimensionPixelSize(2, 0);
        this.mDividerRes = a.getResourceId(0, 0);
        this.mLeftRightRes = a.getResourceId(3, 0);
        this.mMiddleWidth = a.getDimensionPixelSize(10, 0);
        this.mLargeWidth = a.getDimensionPixelSize(11, 0);
        sNormalLeft = a.getResourceId(12, 0);
        sActivedLeft = a.getResourceId(13, 0);
        sNormalMiddle = a.getResourceId(16, 0);
        sActivedMiddle = a.getResourceId(17, 0);
        sNormalRight = a.getResourceId(14, 0);
        sActivedRight = a.getResourceId(15, 0);
        sBorderColor = a.getColor(18, 0);
        sEntityColor = a.getColor(19, 0);
        this.mMinCount = a.getColor(20, 0);
    }
    ...
    private void updateTabViewContainerWidth(Context context) {
        int subtabContainerViewWidth = getDimensForScaleMode();
    ...
    private int getDimensForScaleMode() {
        DisplayMode displayMode = HwWidgetFactory.getDisplayMode(this.mContext);
        int resDimen = this.mNormalWidth;
        switch (-getandroid-hwcontrol-HwWidgetFactory$DisplayModeSwitchesValues()[displayMode.ordinal()]) {
            case 1:
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 0) {
            ...
            int minChildCountWhenFilled = this.mMinCount;
    ...
    private void initDividerDrawable() {
        ...
        if (!(this.mDividerRes == 0 || this.mContext == null)) {
            divider = this.mBackgroundStyle ? this.mContext.getResources().getDrawable(33751342) : this.mContext.getResources().getDrawable(this.mDividerRes);
        }
        if (divider != null) {
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mSubTabAnim.isAnimEnd()) {
            ...
            if (childCount == 1) {
                int leftRightRes;
                if (this.mBackgroundStyle) {
                    leftRightRes = 33751351;
                } else {
                    leftRightRes = this.mLeftRightRes;
                }
    ...
    private static void updateSubTabColorRes(Context context, Drawable[] array, int index, int first, int last) {
        if (index == first) {
            if (SystemProperties.getRTLFlag()) {
                array[0] = context.getResources().getDrawable(sNormalRight);
                array[1] = context.getResources().getDrawable(sActivedRight);
            } else {
                array[0] = context.getResources().getDrawable(sNormalLeft);
                array[1] = context.getResources().getDrawable(sActivedLeft);
            }
        } else if (index != last) {
            array[0] = context.getResources().getDrawable(sNormalMiddle);
            array[1] = context.getResources().getDrawable(sActivedMiddle);
        ...
        array[0].setTint(sBorderColor);
        array[1].setTint(sEntityColor);
    }
}

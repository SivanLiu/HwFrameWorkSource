    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mTitleView.getVisibility() == 0 && this.mDetailView.getVisibility() == 0) {
            if (titleWidthOrigin > maxTitleWidthWithDetail) {
                this.mTitleView.measure(MeasureSpec.makeMeasureSpec(maxTitleWidthWithDetail, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), titleHeightMeasureSpec);
                this.mDetailView.measure(MeasureSpec.makeMeasureSpec(availableSpace - maxTitleWidthWithDetail, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), detailHeightMeasureSpec);
            }
            if (titleWidthOrigin + detailWidthOrigin > availableSpace) {
                ...
                this.mTitleView.measure(MeasureSpec.makeMeasureSpec(availableSpace - detailWidthOrigin, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), titleHeightMeasureSpec);
                this.mDetailView.measure(MeasureSpec.makeMeasureSpec(detailWidthOrigin, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), detailHeightMeasureSpec);
            }
        }
    }

    private void adjustHeight(int widthMeasureSpec, int titleWidth, int detailWidth) {
        this.mTitleView.measure(MeasureSpec.makeMeasureSpec(titleWidth, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), 0);
        this.mDetailView.measure(MeasureSpec.makeMeasureSpec(detailWidth, SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION), 0);
        int heightSpec = MeasureSpec.makeMeasureSpec(this.mTitleView.getMeasuredHeight() > this.mDetailView.getMeasuredHeight() ? this.mTitleView.getMeasuredHeight() : this.mDetailView.getMeasuredHeight(), SQLiteDatabase.ENABLE_DATABASE_ENCRYPTION);

...
public final class DateUtilsBridge {
    ...
    public static String toSkeleton(Calendar startCalendar, Calendar endCalendar, int flags) {
        ...
        if ((flags & 16384) == 0 || (flags & 128) != 0) {
            ...
            timePart = stringBuilder.toString();
        ...
        if ((flags & 48) != 0) {
            if ((flags & 4) != 0) {
                stringBuilder.append(DateFormat.YEAR);
            }
            stringBuilder.append(monthPart);
            if ((flags & 32) == 0) {
                stringBuilder.append(DateFormat.DAY);
            }
        }
        if ((flags & 2) != 0) {
            stringBuilder.append(weekPart);
        }
        if ((flags & 1) != 0) {
            stringBuilder.append(timePart);
        }

...
public class Parser extends DefaultHandler implements ScanHandler, XMLReader, LexicalHandler {
    ...
    private InputStream getInputStream(String publicid, String systemid) throws IOException, SAXException {
        ...
        return new URL(new URL("file", "", stringBuilder.toString()), systemid).openConnection().getInputStream();
    }
    ...
    private String expandEntities(String src) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (ch == '&' && refStart == -1) {
                ...
                if (!(refStart == -1 || Character.isLetter(ch) || Character.isDigit(ch) || ch == '#')) {
                    if (ch == ';') {
                        dstlen = lookupEntity(dst, refStart, (dstlen2 - refStart) - 1);
                        if (dstlen > 65535) {
                            dstlen -= HTMLModels.M_OPTION;
                            dst[refStart - 1] = (char) ((dstlen >> 10) + 55296);
                            dst[refStart] = (char) ((dstlen & 1023) + 56320);
                            dstlen2 = refStart + 1;
                        } else if (dstlen != 0) {
                            dst[refStart - 1] = (char) dstlen;
                            dstlen2 = refStart;
                        }
                        dstlen = -1;
                    } else {
    ...
    private String makeName(char[] buff, int offset, int length) {
        ...
        length = dst.length();
        if (length == 0 || dst.charAt(length - 1) == ':') {
            dst.append('_');
        }

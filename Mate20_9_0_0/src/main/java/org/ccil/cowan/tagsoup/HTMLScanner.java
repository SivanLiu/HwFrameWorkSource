...
public class HTMLScanner implements Scanner, Locator {
    ...
    static {
        ...
        statetableIndex = (short[][]) Array.newInstance(short.class, new int[]{maxState + 1, maxChar + 3});
        for (i = 0; i <= maxState; i++) {
            for (int ch = -2; ch <= maxChar; ch++) {
                int action = 0;
                int hit = -1;
                for (int i2 = 0; i2 < statetable.length; i2 += 4) {
                    if (i != statetable[i2]) {
                        if (action != 0) {
                            break;
                        }
                    } else if (statetable[i2 + 1] == 0) {
                        hit = i2;
                        action = statetable[i2 + 2];
                    } else if (statetable[i2 + 1] == ch) {
                        hit = i2;
                        action = statetable[i2 + 2];
                        break;
                    }
                }
                statetableIndex[i][ch + 2] = (short) hit;
            }
        }
    }
    ...
    /* JADX WARNING: Missing block: B:55:0x012f, code skipped:
            save(45, r2);
            save(r6, r2);
     */
    /* JADX WARNING: Missing block: B:134:0x0333, code skipped:
            r0.theState = r0.theNextState;
            r3 = r8;
     */
    ...
    public void scan(Reader r0, ScanHandler h) throws IOException, SAXException {
        PushbackReader r;
        Reader reader = r0;
        ScanHandler scanHandler = h;
        this.theState = 28;
        if (reader instanceof BufferedReader) {
            r = new PushbackReader(reader, 5);
        } else {
            r = new PushbackReader(new BufferedReader(reader), 5);
        }
            ...
            if (ch >= 32 || ch == 10 || ch == 9 || ch == -1) {
                ...
                switch (action) {
                    case 0:
                        ...
                        stringBuilder.append("HTMLScanner can't cope with ");
                        stringBuilder.append(Integer.toString(ch));
                        stringBuilder.append(" in state ");
                        stringBuilder.append(Integer.toString(this.theState));
                        throw new Error(stringBuilder.toString());
                    case 1:
                        i = 0;
                        i2 = 28;
                        scanHandler.adup(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        break;
                    case 2:
                        i = 0;
                    ...
                    case 3:
                        ...
                        scanHandler.adup(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        scanHandler.stagc(this.theOutputBuffer, i, this.theSize);
                        break;
                    case 4:
                        i = 0;
                        i2 = 28;
                        scanHandler.aname(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        break;
                    case 5:
                        i = 0;
                    ...
                    case 7:
                        i = 0;
                        i2 = 28;
                        scanHandler.aval(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        break;
                    case 8:
                        i = 0;
                    ...
                    case WarningHeader.ATTRIBUTE_NOT_UNDERSTOOD /*10*/:
                        ...
                        i2 = 28;
                        mark();
                        scanHandler.cmnt(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        break;
                    case 11:
                        i = 0;
                        i2 = 28;
                        scanHandler.decl(this.theOutputBuffer, i, this.theSize);
                        this.theSize = i;
                        break;
                    case 12:
                        i2 = 28;
                        ...
                        this.theSize = i;
                        scanHandler.stage(this.theOutputBuffer, i, this.theSize);
                        break;
                    case 13:
                        mark();
                        char ch1 = (char) ch;
                            ...
                            if (ent != 0) {
                                ...
                                if (ent >= 32) {
                                    if (ent < 55296 || ent > 57343) {
                                        if (ent <= 65535) {
                                            save(ent, scanHandler);
                                        } else {
                                            ent -= HTMLModels.M_OPTION;
                                            save((ent >> 10) + 55296, scanHandler);
                                            save((ent & 1023) + 56320, scanHandler);
                                        }
                                    }
                                }
                                if (ch != 59) {
                    ...
                    default:
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("Can't process state ");
    ...
    private void save(int ch, ScanHandler h) throws IOException, SAXException {
        ...
        if (this.theSize >= this.theOutputBuffer.length - 20) {
            if (this.theState == 28 || this.theState == 10) {
                ...
                newOutputBuffer = new char[(this.theOutputBuffer.length * 2)];
                System.arraycopy(this.theOutputBuffer, 0, newOutputBuffer, 0, this.theSize + 1);
                this.theOutputBuffer = newOutputBuffer;
            }
        }
        newOutputBuffer = this.theOutputBuffer;
        int i = this.theSize;
        this.theSize = i + 1;
        newOutputBuffer[i] = (char) ch;
    }
    ...
    private static String nicechar(int in) {
        ...
        if (in < 32) {
            ...
            stringBuilder.append(Integer.toHexString(in));
            return stringBuilder.toString();
        }
        stringBuilder = new StringBuilder();
        stringBuilder.append(Separators.QUOTE);

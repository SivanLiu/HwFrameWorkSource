...
public class CommandLine {
            ...
            if (argv.length == optind) {
                ...
                for (i = optind; i < argv.length; i++) {
                    ...
                    if (j == -1) {
                        ...
                        stringBuilder = new StringBuilder();
                        stringBuilder.append(src.substring(0, j));
    ...
    private static ContentHandler chooseContentHandler(Writer w) {
        ...
        if (hasOption(options, "--doctype-public=")) {
            method = (String) options.get("--doctype-public=");
            if (method != null) {
                x.setOutputProperty(XMLWriter.DOCTYPE_PUBLIC, method);
            }
        }
        if (hasOption(options, "--doctype-system=")) {
            method = (String) options.get("--doctype-system=");
            if (method != null) {
                x.setOutputProperty(XMLWriter.DOCTYPE_SYSTEM, method);
            }
        }

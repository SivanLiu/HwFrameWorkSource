...
public class Shacal2Engine implements BlockCipher {
    ...
    private void byteBlockToInts(byte[] bArr, int[] iArr, int i, int i2) {
        while (i2 < 8) {
            ...
            i3 = i4 + 1;
            int i5 = i3 + 1;
            iArr[i2] = (i | ((bArr[i4] & 255) << 8)) | (bArr[i3] & 255);
            i2++;
            i = i5;
        }
    }
    ...
    private void decryptBlock(byte[] bArr, int i, byte[] bArr2, int i2) {
        ...
        for (int i3 = 63; i3 > -1; i3--) {
            ...
            iArr[5] = iArr[6];
            iArr[6] = iArr[7];
            iArr[7] = (((i - K[i3]) - this.workingKey[i3]) - ((((iArr[4] >>> 6) | (iArr[4] << -6)) ^ ((iArr[4] >>> 11) | (iArr[4] << -11))) ^ ((iArr[4] >>> 25) | (iArr[4] << -25)))) - (((~iArr[4]) & iArr[6]) ^ (iArr[5] & iArr[4]));
        }

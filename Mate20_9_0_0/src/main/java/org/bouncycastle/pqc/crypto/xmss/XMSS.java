...
public class XMSS {
    ...
    public void importState(byte[] bArr, byte[] bArr2) {
        if (bArr == null) {
            ...
            XMSSPrivateKeyParameters build = new Builder(this.params).withPrivateKey(bArr, getParams()).build();
    ...
    protected void setIndex(int i) {
        this.privateKey = new Builder(this.params).withSecretKeySeed(this.privateKey.getSecretKeySeed()).withSecretKeyPRF(this.privateKey.getSecretKeyPRF()).withPublicSeed(this.privateKey.getPublicSeed()).withRoot(this.privateKey.getRoot()).withBDSState(this.privateKey.getBDSState()).build();
    }

    protected void setPublicSeed(byte[] bArr) {
        this.privateKey = new Builder(this.params).withSecretKeySeed(this.privateKey.getSecretKeySeed()).withSecretKeyPRF(this.privateKey.getSecretKeyPRF()).withPublicSeed(bArr).withRoot(getRoot()).withBDSState(this.privateKey.getBDSState()).build();
    ...
    protected void setRoot(byte[] bArr) {
        this.privateKey = new Builder(this.params).withSecretKeySeed(this.privateKey.getSecretKeySeed()).withSecretKeyPRF(this.privateKey.getSecretKeyPRF()).withPublicSeed(getPublicSeed()).withRoot(bArr).withBDSState(this.privateKey.getBDSState()).build();
    ...
    public byte[] sign(byte[] bArr) {
        if (bArr != null) {
            ...
            bArr = xMSSSigner.generateSignature(bArr);

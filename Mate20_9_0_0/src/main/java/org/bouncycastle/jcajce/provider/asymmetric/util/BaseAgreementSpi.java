...
public abstract class BaseAgreementSpi extends KeyAgreementSpi {
    ...
    protected int engineGenerateSecret(byte[] bArr, int i) throws IllegalStateException, ShortBufferException {
        ...
        stringBuilder.append(" key agreement: need ");
        stringBuilder.append(engineGenerateSecret.length);
    ...
    protected SecretKey engineGenerateSecret(String str) throws NoSuchAlgorithmException {
        ...
        toUpperCase = oids.containsKey(toUpperCase) ? ((ASN1ObjectIdentifier) oids.get(toUpperCase)).getId() : str;
        int keySize = getKeySize(toUpperCase);
        if (this.kdf != null) {
            if (keySize >= 0) {
                ...
                if (this.kdf instanceof DHKEKGenerator) {
                    try {
                        dHKDFParameters = new DHKDFParameters(new ASN1ObjectIdentifier(toUpperCase), keySize, calcSecret, this.ukmParameters);
                    } catch (IllegalArgumentException e) {
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("no OID for algorithm: ");
        ...
        return new SecretKeySpec(calcSecret, str);
    }

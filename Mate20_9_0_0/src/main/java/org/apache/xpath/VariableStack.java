...
public class VariableStack implements Cloneable {
    ...
    public int link(int size) {
        ...
        if (this._linksTop + 1 >= this._links.length) {
            newlinks = new int[(this._links.length + DTMFilter.SHOW_NOTATION)];
            System.arraycopy(this._links, 0, newlinks, 0, this._links.length);
            this._links = newlinks;
        }
        newlinks = this._links;
        int i = this._linksTop;
        this._linksTop = i + 1;
        newlinks[i] = this._currentFrameBottom;
        return this._currentFrameBottom;
    }
        ...
        if (prefixResolver instanceof ElemTemplateElement) {
            ...
            if (!(prev instanceof Stylesheet)) {
                while (!(prev.getParentNode() instanceof Stylesheet)) {
                    ElemTemplateElement prev2 = prev;
                    while (true) {
                        ElemTemplateElement previousSiblingElem = prev2.getPreviousSiblingElem();
                        prev2 = previousSiblingElem;
                        if (previousSiblingElem == null) {
                            break;
                        } else if (prev2 instanceof ElemVariable) {
                            ElemVariable vvar = (ElemVariable) prev2;
            ...
            ElemVariable vvar2 = prev.getStylesheetRoot().getVariableOrParamComposed(qname);
            if (vvar2 != null) {

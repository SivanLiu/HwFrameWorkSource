...
public class Compiler extends OpMap {
    ...
    public Expression locationPath(int opPos) throws TransformerException {
        ...
        try {
            expression = (Expression) WalkerFactory.newDTMIterator(this, opPos, this.locPathDepth == 0 ? 1 : null);
            return expression;
        } finally {
            expression = this.locPathDepth - 1;
            this.locPathDepth = expression;
        }
    }
        ...
        if (stepType != 25) {
            switch (stepType) {
                case 50:
                    ...
                    int opPos3 = OpMap.getFirstChildPosOfStep(opPos);
                    pattern = new StepPattern(1280, 10, 3);
                    opPos2 = opPos3;
                    break;
                case 51:
                    argLen = getArgLengthOfStep(opPos);
                    opPos2 = OpMap.getFirstChildPosOfStep(opPos);
                    pattern = new StepPattern(2, getStepNS(startOpPos), getStepLocalName(startOpPos), 10, 2);
                    break;
                case 52:
                    argLen = getArgLengthOfStep(opPos);
                    int opPos4 = OpMap.getFirstChildPosOfStep(opPos);
    ...
    public void warn(String msg, Object[] args) throws TransformerException {
        ...
        stringBuilder.append(this.m_locator.getSystemId());
        stringBuilder.append("; line ");
        stringBuilder.append(this.m_locator.getLineNumber());
        stringBuilder.append("; column ");
        stringBuilder.append(this.m_locator.getColumnNumber());

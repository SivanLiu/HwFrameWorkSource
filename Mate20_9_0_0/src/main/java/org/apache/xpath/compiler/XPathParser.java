...
public class XPathParser {
        ...
        try {
            ...
            if (this.m_token != null) {
                ...
                while (this.m_token != null) {
                    ...
                    stringBuilder.append(extraTokens);
                    stringBuilder.append("'");
                    stringBuilder.append(this.m_token);
                    stringBuilder.append("'");
                    extraTokens = stringBuilder.toString();
                    nextToken();
                    if (this.m_token != null) {
    ...
    protected void LocationPath() throws TransformerException {
        int opPos = this.m_ops.getOp(1);
        appendOp(2, 28);
        boolean seenSlash = tokenIs((char) true);
        if (seenSlash) {
            appendOp(4, 50);
    ...
    protected boolean Step() throws TransformerException {
        int opPos = this.m_ops.getOp(1);
        boolean doubleSlash = tokenIs((char) true);
        if (doubleSlash) {
            nextToken();
            appendOp(2, 42);
            this.m_ops.setOp(1, this.m_ops.getOp(1) + 1);

...
public class BasicHeaderValueParser implements HeaderValueParser {
    ...
    public NameValuePair parseNameValuePair(CharArrayBuffer buffer, ParserCursor cursor, char[] delimiters) {
        ...
        if (charArrayBuffer == null) {
            ...
            int i12 = i1;
            i1 = pos3;
            while (i12 < i1 && HTTP.isWhitespace(charArrayBuffer.charAt(i12))) {
                i12++;
            }
            while (i1 > i12 && HTTP.isWhitespace(charArrayBuffer.charAt(i1 - 1))) {
                i1--;
            }
            if (i1 - i12 >= 2 && charArrayBuffer.charAt(i12) == '\"' && charArrayBuffer.charAt(i1 - 1) == '\"') {
                i12++;
                i1--;
            }
            String value = charArrayBuffer.substring(i12, i1);
            if (terminated) {
                pos3++;
            }

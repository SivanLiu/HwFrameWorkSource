...
public class RFC2965Spec extends RFC2109Spec {
    ...
    public List<Cookie> parse(Header header, CookieOrigin origin) throws MalformedCookieException {
        ...
        if (header == null) {
            ...
            while (i < length) {
                ...
                Iterator it = attribmap.entrySet().iterator();
                while (it.hasNext()) {
                    param = (NameValuePair) ((Entry) it.next()).getValue();
                    Iterator it2 = it;
                    String s = param.getName().toLowerCase(Locale.ENGLISH);
                    cookie.setAttribute(s, param.getValue());
                    CookieAttributeHandler handler = rFC2965Spec.findAttribHandler(s);
                    if (handler != null) {
                        handler.parse(cookie, param.getValue());
                    }
                    it = it2;
                    rFC2965Spec = this;
                }
    ...
    private static CookieOrigin adjustEffectiveHost(CookieOrigin origin) {
        ...
        return new CookieOrigin(stringBuilder.toString(), origin.getPort(), origin.getPath(), origin.isSecure());
    }

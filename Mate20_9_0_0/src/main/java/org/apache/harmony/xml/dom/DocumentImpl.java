...
public final class DocumentImpl extends InnerNodeImpl implements Document {
    ...
    public Node adoptNode(Node node) {
        ...
        switch (nodeImpl.getNodeType()) {
            ...
            case (short) 12:
                stringBuilder = new StringBuilder();
                stringBuilder.append("Cannot adopt nodes of type ");
                stringBuilder.append(nodeImpl.getNodeType());
                throw new DOMException((short) 9, stringBuilder.toString());
            default:
                stringBuilder = new StringBuilder();
                stringBuilder.append("Unsupported node type ");
    ...
    private void changeDocumentToThis(NodeImpl node) {
        ...
        for (int i2 = 0; i2 < list.getLength(); i2++) {
            changeDocumentToThis((NodeImpl) list.item(i2));
        }
    ...
    Map<String, UserData> getUserDataMap(NodeImpl node) {
        ...
        HashMap userDataMap2 = new HashMap();
    ...
    Map<String, UserData> getUserDataMapForRead(NodeImpl node) {
        ...
        Map<String, UserData> emptyMap;
        Map<String, UserData> userDataMap = (Map) this.nodeToUserData.get(node);
        if (userDataMap == null) {
            emptyMap = Collections.emptyMap();
        } else {
            emptyMap = userDataMap;
        }
        return emptyMap;
    }

...
public class ElementImpl extends InnerNodeImpl implements Element {
    ...
    private int indexOfAttribute(String name) {
        for (int i = 0; i < this.attributes.size(); i++) {
            if (Objects.equals(name, ((AttrImpl) this.attributes.get(i)).getNodeName())) {
                return i;
            }
        }
        return -1;
    ...
    private int indexOfAttributeNS(String namespaceURI, String localName) {
        for (int i = 0; i < this.attributes.size(); i++) {
            ...
            if (Objects.equals(namespaceURI, attr.getNamespaceURI()) && Objects.equals(localName, attr.getLocalName())) {
                return i;
            }
        }
        return -1;

...
public class FastStringBuffer {
    ...
    public final void reset() {
        ...
        this.m_innerFSB = null;
        this.m_array = (char[][]) Array.newInstance(char.class, new int[]{16, 0});
        this.m_array[0] = new char[this.m_chunkSize];
    }
            ...
            if (strlen != 0) {
                ...
                while (strlen > 0) {
                    ...
                    if (strlen > 0) {
                        ...
                        int i = this.m_array.length;
                        if (this.m_lastChunk + 1 == i) {
                            newarray = new char[(i + 16)][];
                            System.arraycopy(this.m_array, 0, newarray, 0, i);
                            this.m_array = newarray;
                        }
                        newarray = this.m_array;
                        int i2 = this.m_lastChunk + 1;
                        this.m_lastChunk = i2;
                        if (newarray[i2] == null) {
                            if (this.m_lastChunk == (1 << this.m_rebundleBits) && this.m_chunkBits < this.m_maxChunkBits) {
        ...
        if (value != null) {
            ...
            if (strlen != 0) {
                ...
                while (strlen > 0) {
                    ...
                    int sourcecolumn = value.m_chunkMask & copyfrom;
    ...
    public boolean isWhitespace(int start, int length) {
        ...
        int sourcecolumn = this.m_chunkMask & start;
    ...
    public String getString(int start, int length) {
        int startColumn = this.m_chunkMask & start;
    ...
    StringBuffer getString(StringBuffer sb, int startChunk, int startColumn, int length) {
        ...
        int stopColumn = this.m_chunkMask & stop;
        int startColumn2 = startColumn;
        for (startColumn = startChunk; startColumn < stopChunk; startColumn++) {
            if (startColumn != 0 || this.m_innerFSB == null) {
                sb.append(this.m_array[startColumn], startColumn2, this.m_chunkSize - startColumn2);
            } else {
                this.m_innerFSB.getString(sb, startColumn2, this.m_chunkSize - startColumn2);
            }
            startColumn2 = 0;
        }
        if (stopChunk == 0 && this.m_innerFSB != null) {
            this.m_innerFSB.getString(sb, startColumn2, stopColumn - startColumn2);
        } else if (stopColumn > startColumn2) {
            sb.append(this.m_array[stopChunk], startColumn2, stopColumn - startColumn2);
        }
        return sb;
    }
    ...
    public void sendSAXcharacters(ContentHandler ch, int start, int length) throws SAXException {
        ...
        if (startColumn + length >= this.m_chunkMask || this.m_innerFSB != null) {
            ...
            for (startColumn = startChunk; startColumn < stopChunk; startColumn++) {
                if (startColumn != 0 || this.m_innerFSB == null) {
                    ch.characters(this.m_array[startColumn], startColumn2, this.m_chunkSize - startColumn2);
                } else {
                    this.m_innerFSB.sendSAXcharacters(ch, startColumn2, this.m_chunkSize - startColumn2);
                }
                startColumn2 = 0;
            }
            if (stopChunk == 0 && this.m_innerFSB != null) {
                this.m_innerFSB.sendSAXcharacters(ch, startColumn2, stopColumn - startColumn2);
            } else if (stopColumn > startColumn2) {
                ch.characters(this.m_array[stopChunk], startColumn2, stopColumn - startColumn2);
            }
            return;
        }
    ...
    static int sendNormalizedSAXcharacters(char[] ch, int start, int length, ContentHandler handler, int edgeTreatmentFlags) throws SAXException {
        ...
        int startNonWhitespace = start;
        int limit = start + length;
        if (processingLeadingWhitespace) {
            while (startNonWhitespace < limit && XMLCharacterRecognizer.isWhiteSpace(ch[startNonWhitespace])) {
                startNonWhitespace++;
            }
            if (startNonWhitespace == limit) {
                return edgeTreatmentFlags;
            }
        }
        while (startNonWhitespace < limit) {
            int startWhitespace = startNonWhitespace;
            while (startWhitespace < limit && !XMLCharacterRecognizer.isWhiteSpace(ch[startWhitespace])) {
                startWhitespace++;
            }
            if (startNonWhitespace != startWhitespace) {
                if (seenWhitespace) {
                    handler.characters(SINGLE_SPACE, 0, 1);
                    seenWhitespace = false;
                }
                handler.characters(ch, startNonWhitespace, startWhitespace - startNonWhitespace);
            }
            int currPos = startWhitespace;

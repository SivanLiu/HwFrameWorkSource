            ...
            case (short) 1:
                ...
                while (i < nAttrs) {
                    ...
                    if (attrName.equals("xmlns") || attrName.startsWith(Constants.ATTRNAME_XMLNS)) {
                        int indexOf = attrName.indexOf(":");
                        this.m_contentHandler.startPrefixMapping(indexOf < 0 ? "" : attrName.substring(indexOf + 1), attr.getNodeValue());
                    }
                    i++;
                }
    ...
    protected void endNode(Node node) throws SAXException {
        ...
        if (nodeType == (short) 1) {
            ...
            for (int i = 0; i < nAttrs; i++) {
                ...
                if (attrName.equals("xmlns") || attrName.startsWith(Constants.ATTRNAME_XMLNS)) {
                    int indexOf = attrName.indexOf(":");
                    this.m_contentHandler.endPrefixMapping(indexOf < 0 ? "" : attrName.substring(indexOf + 1));
                }
            }
        } else if (nodeType != (short) 9) {
            switch (nodeType) {
                case (short) 5:
                    EntityReference eref = (EntityReference) node;
                    if (this.m_contentHandler instanceof LexicalHandler) {

...
final class URI {
    ...
    private void initialize(URI p_base, String p_uriSpec) throws MalformedURIException {
        if (p_base == null && (p_uriSpec == null || p_uriSpec.trim().length() == 0)) {
            ...
            if (uriSpec.startsWith("//")) {
                ...
                int index2 = index;
                char testChar = 0;
                while (index2 < uriSpecLen) {
                    testChar = uriSpec.charAt(index2);
                ...
                if (index2 > startPos) {
                    initializeAuthority(uriSpec.substring(startPos, index2));
                } else {
                    this.m_host = "";
                }
                index = index2;
            ...
            if (p_base != null) {
                ...
                if (this.m_host == null) {
                    ...
                    if (this.m_path.length() <= 0 || !this.m_path.startsWith(PsuedoNames.PSEUDONAME_ROOT)) {
                        ...
                        if (basePath != null) {
                            lastSlash = basePath.lastIndexOf(47);
                            if (lastSlash != -1) {
                        ...
                        while (true) {
                            ...
                            if (segIndex != -1) {
                                lastSlash = segIndex + 1;
                                if (!tempString.substring(segIndex).equals(Constants.ATTRVAL_PARENT)) {
                                    path = path.substring(0, lastSlash).concat(path.substring(index + 4));
                                }
                            }
                        }
                        if (path.endsWith("/..")) {
            ...
            if (portStr.length() > 0) {
                ...
                while (i < portStr.length()) {
                    if (isDigit(portStr.charAt(i))) {
                        ...
                        stringBuilder.append(" is invalid. Port should only contain digits!");
                        throw new MalformedURIException(stringBuilder.toString());
                    }
                }
                try {
            ...
            if (testChar == '?') {
                ...
                while (index < end) {
                    ...
                    if (testChar == '%') {
                        ...
                        stringBuilder.append("Query string contains invalid character:");
            ...
            if (testChar == '#') {
                ...
                while (index < end) {
                    ...
                    if (testChar == '%') {
                        ...
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("Fragment contains invalid character:");
                        stringBuilder.append(testChar);
                        throw new MalformedURIException(stringBuilder.toString());
                    }
                    index++;
        ...
        if (p_userinfo == null) {
            ...
            while (index < end) {
                ...
                if (testChar == '%') {
                    ...
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("Userinfo contains invalid character:");
                    stringBuilder.append(testChar);
                    throw new MalformedURIException(stringBuilder.toString());
                }
                index++;
            ...
            if (isURIString(p_addToPath)) {
                if (this.m_path == null || this.m_path.trim().length() == 0) {
                    if (p_addToPath.startsWith(PsuedoNames.PSEUDONAME_ROOT)) {
                        ...
                        stringBuilder.append(p_addToPath);
                        this.m_path = stringBuilder.toString();
                    }
                    ...
                    stringBuilder2.append(PsuedoNames.PSEUDONAME_ROOT);
                    stringBuilder2.append(p_addToPath);
                    this.m_path = str.concat(stringBuilder2.toString());
                }
                return;
            }
    ...
    public static boolean isWellFormedAddress(String p_address) {
        ...
        if (index + 1 >= addrLength || !isDigit(p_address.charAt(index + 1))) {
            ...
            i = 0;
            while (i < addrLength) {
                char testChar2 = address.charAt(i);
                if (testChar2 == '.') {

...
public class ToTextStream extends ToStream {
    ...
    void writeNormalizedChars(char[] ch, int start, int length, boolean useLineSep) throws IOException, SAXException {
        ...
        char[] cArr = ch;
        String encoding = toTextStream.getEncoding();
        Writer writer = toTextStream.m_writer;
        int end = start + length;
        int i = start;
        while (i < end) {
            char c = cArr[i];
            if (10 == c && useLineSep) {
                writer.write(toTextStream.m_lineSep, 0, toTextStream.m_lineSepLen);
            } else if (toTextStream.m_encodingInfo.isInEncoding(c)) {
                writer.write(c);
            } else if (Encodings.isHighUTF16Surrogate(c)) {
                int codePoint = toTextStream.writeUTF16Surrogate(c, cArr, i, end);
                if (codePoint != 0) {
                ...
                System.err.println(Utils.messages.createMessage(MsgKey.ER_ILLEGAL_CHARACTER, new Object[]{integralValue2, encoding}));
            } else {
                writer.write(c);
            }
            i++;

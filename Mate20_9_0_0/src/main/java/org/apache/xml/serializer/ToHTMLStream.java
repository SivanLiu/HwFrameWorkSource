...
public class ToHTMLStream extends ToStream {
    ...
    public void writeAttrURI(Writer writer, String string, boolean doURLEscaping) throws IOException {
            ...
            if (ch < ' ' || ch > '~') {
                ...
                if (doURLEscaping) {
                    ...
                    if (ch <= 127) {
                        ...
                        low = 128 | (ch & 63);
                        writer2.write(37);
                        writer2.write(makeHHString(high));
                        writer2.write(37);
                        writer2.write(makeHHString(low));
                    } else if (Encodings.isHighUTF16Surrogate(ch)) {
                        high = ch & 1023;
                        int uuuuu = ((high & 960) >> 6) + 1;
                        int zzzz = (high & 60) >> 2;
                        int yyyyyy = ((high & 3) << 4) & 48;
                        i2++;
                        ch = chars[i2];
                        i = ch & 1023;
                        low = 240 | (uuuuu >> 2);
                        byte2 = (128 | (((uuuuu & 3) << 4) & 48)) | zzzz;
                        end2 = end;
                        end = 128 | (yyyyyy | ((i & 960) >> 6));
                        i = 128 | (i & 63);
                        int i3 = i2;
                        writer2.write(37);
                        writer2.write(makeHHString(low));
                        writer2.write(37);
                        writer2.write(makeHHString(byte2));
                        writer2.write(37);
                        writer2.write(makeHHString(end));
                        writer2.write(37);
                        writer2.write(makeHHString(i));
                        i2 = i3;
                    } else {
                        end2 = end;
                        byte2 = (ch >> 12) | 224;
        ...
        byte2 = i;
        if (cleanLength > byte2) {
            if (cleanStart == 0) {
                writer.write(string);
            } else {
                writer2.write(chars, cleanStart, cleanLength);
            }
        } else if (cleanLength == byte2) {
            writer2.write(ch);
        }
    }
    ...
    public void writeAttrString(Writer writer, String string, String encoding) throws IOException {
        ...
        while (true) {
            ...
            char ch3 = chars[i2];
            if (!escapingNotNeeded(ch3) || this.m_charInfo.shouldMapAttrChar(ch3)) {
                if ('<' == ch3) {
                    ch = ch3;
                } else if ('>' == ch3) {
                    ch = ch3;
                } else if ('&' == ch3 && i2 + 1 < end && '{' == chars[i2 + 1]) {
                    cleanLength++;
                } else {
                    if (cleanLength > 0) {
                        writer2.write(chars, cleanStart, cleanLength);
                    ...
                    char cleanLength2 = cleanLength;
                    ch = ch3;
                    int pos = accumDefaultEntity(writer2, ch3, i2, chars, end, false, 1);
                    if (i2 != pos) {
                        i2 = pos - 1;
                    } else {

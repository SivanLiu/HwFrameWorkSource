...
public abstract class ToStream extends SerializerBase {
    ...
    void setProp(String name, String val, boolean defaultVal) {
        ...
        String val2;
        if (str2 != null) {
            char first = SerializerBase.getFirstCharLocName(name);
            if (first != 'i') {
                if (first != 'o') {
                    if (first != 's') {
                        if (first != 'v') {
                                    ...
                                    if ("encoding".equals(str)) {
                                        ...
                                        if ((z && (oldDefaultEncoding == null || !oldDefaultEncoding.equalsIgnoreCase(newEncoding))) || (!z && (oldExplicitEncoding == null || !oldExplicitEncoding.equalsIgnoreCase(newEncoding)))) {
                                            ...
                                            if (newEncoding == null || encodingInfo.name != null) {
                                                ...
                                                stringBuilder.append(newEncoding);
    ...
    protected int writeUTF16Surrogate(char c, char[] ch, int i, int end) throws IOException {
        if (i + 1 < end) {
            ...
            throw new IOException(messages.createMessage("ER_INVALID_UTF16_SURROGATE", objArr));
        }
    ...
    public void characters(char[] chars, int start, int length) throws SAXException {
        if (length != 0 && (!this.m_inEntityRef || this.m_expandDTDEntities)) {
            ...
            try {
                ...
                lastDirtyCharProcessed = start;
                while (lastDirtyCharProcessed < end && isAllWhitespace) {
                    char ch1 = chars[lastDirtyCharProcessed];
                    if (this.m_charInfo.shouldMapTextChar(ch1)) {
                        writeOutCleanChars(chars, lastDirtyCharProcessed, lastDirtyCharProcessed2);
                        writer.write(this.m_charInfo.getOutputStringForChar(ch1));
                        isAllWhitespace = false;
                        lastDirtyCharProcessed2 = lastDirtyCharProcessed;
                        ...
                        switch (ch1) {
                            case 9:
                                ...
                                break;
                            case 10:
                                lastDirtyCharProcessed2 = processLineFeed(chars, lastDirtyCharProcessed, lastDirtyCharProcessed2, writer);
                                lastDirtyCharProcessed++;
                                break;
                            default:
                                isAllWhitespace = false;
                ...
                if (lastDirtyCharProcessed < end || !isAllWhitespace) {
                    this.m_ispreserve = true;
                }
                while (lastDirtyCharProcessed < end) {
                    char ch = chars[lastDirtyCharProcessed];
                ...
                if (lastDirtyCharProcessed > startClean) {
                    this.m_writer.write(chars, startClean, lastDirtyCharProcessed - startClean);
                }
    ...
    private int accumDefaultEscape(Writer writer, char ch, int i, char[] chars, int len, boolean fromTextNode, boolean escLF) throws IOException {
        ...
        if (i2 != pos) {
            ...
            stringBuilder.append(" ");
            stringBuilder.append(Integer.toHexString(next));
            objArr[0] = stringBuilder.toString();
            throw new IOException(messages.createMessage("ER_INVALID_UTF16_SURROGATE", objArr));
        } else {
    ...
    public boolean startPrefixMapping(String prefix, String uri, boolean shouldFlush) throws SAXException {
        ...
        if (pushed) {
            if ("".equals(prefix)) {
                ...
                addAttributeAlways(SerializerConstants.XMLNS_URI, prefix, stringBuilder.toString(), "CDATA", uri, false);
            }
        }
        return pushed;
    }
    ...
    protected String ensureAttributesNamespaceIsDeclared(String ns, String localName, String rawName) throws SAXException {
        ...
        if (index > 0) {
            ...
            String str = SerializerConstants.XMLNS_URI;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(Constants.ATTRNAME_XMLNS);
            stringBuilder.append(prefixFromRawName);
            addAttribute(str, prefixFromRawName, stringBuilder.toString(), "CDATA", ns, false);
        ...
        if (prefix == null) {
            ...
            stringBuilder2.append(Constants.ATTRNAME_XMLNS);
            stringBuilder2.append(prefix);
            addAttribute(str2, prefix, stringBuilder2.toString(), "CDATA", ns, false);
        }
        return prefix;
    }
        ...
        if (ns != null && ns.length() > 0) {
            ...
            if (prefix != null) {
                ...
                if (foundURI == null || !foundURI.equals(ns)) {
                    ...
                    String str3 = SerializerConstants.XMLNS_URI;
                    if (no_prefix) {
                        str = "xmlns";
                    } else {
                        str = prefix;
                    ...
                    addAttributeAlways(str3, str, str2, "CDATA", ns, false);
                }
            }
        }
    }
    ...
    public boolean addAttributeAlways(String uri, String localName, String rawName, String type, String value, boolean xslAttribute) {
        ...
        if (index >= 0) {
            ...
            if (xslAttribute) {
                ...
                if (colonIndex > 0) {
                    MappingRecord existing_mapping = this.m_prefixMap.getMappingFromPrefix(rawName.substring(null, colonIndex));
                    if (!(existing_mapping == null || existing_mapping.m_declarationDepth != this.m_elemContext.m_currentElemDepth || existing_mapping.m_uri.equals(uri))) {
                        ...
                        rawName = stringBuilder.toString();
                    }
                }
                try {
    ...
    public void addCdataSectionElements(String URI_and_localNames) {
        ...
        stringBuilder.append(" ");
        stringBuilder.append(URI_and_localNames);

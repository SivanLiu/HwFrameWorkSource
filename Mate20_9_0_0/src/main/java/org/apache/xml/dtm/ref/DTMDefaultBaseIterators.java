...
public abstract class DTMDefaultBaseIterators extends DTMDefaultBaseTraversers {
    ...
    public class NthDescendantIterator extends DescendantIterator {
        ...
        public int next() {
            while (true) {
                ...
                next = DTMDefaultBaseIterators.this.makeNodeIdentity(node);
                int child = DTMDefaultBaseIterators.this._firstch(DTMDefaultBaseIterators.this._parent(next));
                int pos = 0;
                int _nextsib;
                do {
    ...
    public final class TypedPrecedingIterator extends PrecedingIterator {
        ...
        public int next() {
            ...
            this._currentNode = node;
            return node == -1 ? -1 : returnNode(DTMDefaultBaseIterators.this.makeNodeHandle(node));
        }
    }

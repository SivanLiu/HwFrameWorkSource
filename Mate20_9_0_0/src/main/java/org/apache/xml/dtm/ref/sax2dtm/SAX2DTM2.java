...
public class SAX2DTM2 extends SAX2DTM {
    ...
    public class AncestorIterator extends InternalAxisIteratorBase {
        ...
        public DTMAxisIterator setStartNode(int node) {
            ...
            while (nodeID != -1) {
                ...
                newAncestors = this.m_ancestors;
                int i2 = this.m_size;
                this.m_size = i2 + 1;
                newAncestors[i2] = node;
                nodeID = SAX2DTM2.this._parent2(nodeID);
                node = SAX2DTM2.this.makeNodeHandle(nodeID);
            }
     ...
     */
        ...
        public int next() {
            ...
            if (startNode != 0) {
                while (true) {
                    i++;
                    type = SAX2DTM2.this._type2(i);
                    if (-1 != type && isDescendant(i)) {
                ...
                while (true) {
                    i++;
                    int eType = SAX2DTM2.this._exptype2(i);
                    if (-1 == eType) {
                        this._currentNode = -1;
                        return -1;
                    } else if (eType != 3) {
        ...
        public int next() {
            ...
            while (true) {
                ...
                if (this._currentNode < this._stack[this._sp]) {
                    i = SAX2DTM2.this._type2(this._currentNode);
                    if (!(i == 2 || i == 13)) {
                        return returnNode(SAX2DTM2.this.makeNodeHandle(this._currentNode));
                    }
                }
            ...
            if (nodeType >= 14) {
                while (nodeID != -1) {
                    if (SAX2DTM2.this._exptype2(nodeID) == nodeType) {
                        ...
                        int i2 = this.m_size;
                        this.m_size = i2 + 1;
                        newAncestors[i2] = SAX2DTM2.this.makeNodeHandle(nodeID);
                    }
                    nodeID = SAX2DTM2.this._parent2(nodeID);
                }
            } else {
                while (nodeID != -1) {
                    ...
                    if ((eType < 14 && eType == nodeType) || (eType >= 14 && SAX2DTM2.this.m_extendedTypes[eType].getNodeType() == nodeType)) {
                        int[] newAncestors2;
                        if (this.m_size >= this.m_ancestors.length) {
                            newAncestors2 = new int[(this.m_size * 2)];
                            System.arraycopy(this.m_ancestors, 0, newAncestors2, 0, this.m_ancestors.length);
                            this.m_ancestors = newAncestors2;
                        }
                        newAncestors2 = this.m_ancestors;
                        int i3 = this.m_size;
                        this.m_size = i3 + 1;
                        newAncestors2[i3] = SAX2DTM2.this.makeNodeHandle(nodeID);
                    }
                    nodeID = SAX2DTM2.this._parent2(nodeID);
                }
            }
    ...
    public final class TypedDescendantIterator extends DescendantIterator {
        ...
        public int next() {
            ...
            if (nodeType != 1) {
                ...
                while (true) {
                    node++;
                    i = SAX2DTM2.this._exptype2(node);
                    if (-1 != i) {
                        if (i >= 14 && SAX2DTM2.this.m_extendedTypes[i].getNodeType() == 1) {
                            break;
                        }
                    } else {
                        this._currentNode = -1;
                ...
                while (true) {
                    node++;
                    i = SAX2DTM2.this._exptype2(node);
                    if (-1 != i && (SAX2DTM2.this._parent2(node) >= startNode || startNode == node)) {
                        if (i >= 14 && SAX2DTM2.this.m_extendedTypes[i].getNodeType() == 1) {
                            break;
                        }
                    } else {
    ...
    public final class TypedFollowingSiblingIterator extends FollowingSiblingIterator {
        ...
        public int next() {
            ...
            if (nodeType == 1) {
                while (true) {
                    _nextsib2 = SAX2DTM2.this._nextsib2(node);
                    node = _nextsib2;
                    if (_nextsib2 == -1 || SAX2DTM2.this._exptype2(node) >= 14) {
                        break;
                    }
                }
            } else {
                while (true) {
                    _nextsib2 = SAX2DTM2.this._nextsib2(node);
                    node = _nextsib2;
                    if (_nextsib2 == -1 || SAX2DTM2.this._exptype2(node) == nodeType) {
                        break;
                    }
                }
            }
    ...
    public final class TypedPrecedingIterator extends PrecedingIterator {
        ...
        public int next() {
            ...
            if (nodeType >= 14) {
                ...
                while (true) {
                    ...
                    if (this._sp < 0) {
                        ...
                        i2 = SAX2DTM2.this._exptype2(node);
                        if (i2 < 14) {
                            if (i2 == nodeType) {
                                break;
                            }
                        } else if (SAX2DTM2.this.m_extendedTypes[i2].getNodeType() == nodeType) {
                            break;
                        }
                    }
                }
            ...
            this._currentNode = node;
            return node == -1 ? -1 : returnNode(SAX2DTM2.this.makeNodeHandle(node));
        }
    }
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        charactersFlush();
        int exName2 = this.m_expandedNameTable.getExpandedTypeID(uri, localName, 1);
        boolean z = false;
        ...
        int elemNode2 = addNode(1, exName2, this.m_parents.peek(), this.m_previous, prefixIndex, true);
        if (this.m_indexing) {
            indexNode(exName2, elemNode2);
        }
        ...
        if (this.m_pastFirstElement) {
            ...
            String declURL2 = "http://www.w3.org/XML/1998/namespace";
            ...
            addNode(13, exName, elemNode2, -1, val, false);
        ...
        for (i = startDecls; i < nDecls; i += 2) {
            ...
            if (prefix == null) {
                ...
                declURL = (String) this.m_prefixMappings.elementAt(i + 1);
                exName = this.m_expandedNameTable.getExpandedTypeID(null, prefix, 13);
                this.m_values.addElement(declURL);
                val = this.m_valueIndex;
                this.m_valueIndex = val + 1;
                String str3 = declURL;
                addNode(13, exName, elemNode2, -1, val, null);
            }
        }
        exName2 = attributes.getLength();
        i = 0;
        while (true) {
            int i2 = i;
            if (i2 >= exName2) {
            ...
            int val2;
            int prefixIndex2;
            str = attributes2.getURI(i2);
            declURL = attributes2.getQName(i2);
            String valString = attributes2.getValue(i2);
            ...
            if (declURL == null || !(declURL.equals("xmlns") || declURL.startsWith(Constants.ATTRNAME_XMLNS))) {
                if (this.m_buildIdIndex && attributes2.getType(i2).equalsIgnoreCase("ID")) {
                    setIDAttribute(valString, elemNode2);
                }
                nodeType = 2;
            } else if (declAlreadyDeclared(getPrefix(declURL, str))) {
                nDecls2 = nDecls;
                elemNode = elemNode2;
                i = i2 + 1;
                elemNode2 = elemNode;
            ...
            i = this.m_valueIndex;
            this.m_valueIndex = i + 1;
            if (attrLocalName.length() != declURL.length()) {
                int prefixIndex3 = this.m_valuesOrPrefixes.stringToIndex(declURL);
            ...
            nDecls2 = nDecls;
            elemNode = elemNode2;
            addNode(nodeType, this.m_expandedNameTable.getExpandedTypeID(str, attrLocalName, nodeType), elemNode2, -1, val2, null);
    ...
    public String getNodeName(int nodeHandle) {
        ...
        type = this.m_dataOrQName.elementAt(nodeID);
        if (type == 0) {
            return extType.getLocalName();
        }
        if (type < 0) {
        ...
        if (type == 1 || type == 9) {
            ...
            do {
                ...
                if (type == 3 || type == 4) {
                    ...
                    if (dataIndex2 >= 0) {
                        if (-1 == offset) {
                            offset = dataIndex2 >>> 10;
                        }
                        length += dataIndex2 & TEXT_LENGTH_MAX;
                    } else {
                        if (-1 == offset) {
    ...
    protected final String copyElement(int nodeID, int exptype, SerializationHandler handler) throws SAXException {
        ...
        if (prefixIndex > 0) {
            prefix = qName.substring(null, prefixIndex);
        } else {
            prefix = null;
        }

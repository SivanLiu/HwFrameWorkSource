    ...
    private Hashtable getRefsTable() {
        if (this.m_refsTable == null) {
            ...
            while (true) {
                ...
                nextNode = 0;
                while (nextNode < nKeyDecls) {
                    try {
                        XObject xuse = ((KeyDeclaration) keyDecls.elementAt(nextNode)).getUse().execute(xctxt, currentNode, ki.getPrefixResolver());
                        if (xuse.getType() == 4) {
                            DTMIterator i = ((XNodeSet) xuse).iterRaw();
                            while (true) {
                                int nextNode2 = i.nextNode();
                                int currentNodeInUseClause = nextNode2;
                                if (-1 == nextNode2) {
                                    break;
                                }

    ...
    public void setTemplate(ElemTemplate template) {
        ...
        if (template.getName() != null) {
            ...
            if (existingTemplate == null) {
                ...
                newPrecedence = template.getStylesheetComposed().getImportCountComposed();
        ...
        if (matchXPath != null) {
            ...
            if (matchExpr instanceof StepPattern) {
                ...
                StepPattern[] pats = ((UnionPattern) matchExpr).getPatterns();
                newPrecedence = pats.length;
                while (i < newPrecedence) {
    ...
    void dumpAssociationTables() {
        ...
        while (associations.hasMoreElements()) {
            for (head = (TemplateSubPatternAssociation) associations.nextElement(); head != null; head = head.getNext()) {
                ...
                stringBuilder.append(head.getTargetString());
                stringBuilder.append(", ");
                stringBuilder.append(head.getPattern());
                stringBuilder.append(")");
        ...
        if (head != null) {
            ...
            while (true) {
                ...
                if (head.m_stepPattern.execute(xctxt, targetNode) == NodeTest.SCORE_NONE || !head.matchMode(mode)) {
                    try {
                        TemplateSubPatternAssociation next = head.getNext();
                        head = next;
                        if (next == null) {
                            break;
                        }

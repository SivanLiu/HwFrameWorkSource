...
public class StylesheetRoot extends StylesheetComposed implements Serializable, Templates {
    ...
    public void recompose() throws TransformerException {
        ...
        i = 0;
        for (j = 0; j < n; j++) {
            getGlobalImport(j).recompose(recomposableElements);
        }
        ...
        while (true) {
            ...
            StylesheetComposed imported = getGlobalImport(i2);
            j = imported.getIncludeCountComposed();
            for (int j3 = -1; j3 < j; j3++) {
                composeTemplates(imported.getIncludeComposed(j3));
            }
            i = i2 + 1;
        }
        if (this.m_extNsMgr != null) {

...
public class StylesheetHandler extends DefaultHandler implements TemplatesHandler, PrefixResolver, NodeConsumer {
    ...
    XSLTElementProcessor getProcessorFor(String uri, String localName, String rawName) throws SAXException {
        ...
        if (elemProcessor == null && !(currentProcessor instanceof ProcessorStylesheetDoc) && (getStylesheet() == null || Double.valueOf(getStylesheet().getVersion()).doubleValue() > 1.0d || ((!uri.equals(Constants.S_XSLNAMESPACEURL) && (currentProcessor instanceof ProcessorStylesheetElement)) || getElemVersion() > 1.0d))) {
    ...
    public void processingInstruction(String target, String data) throws SAXException {
        if (this.m_shouldProcess) {
            ...
            if (colon >= 0) {
                String substring = target.substring(0, colon);
                prefix = substring;
                ns = getNamespaceForPrefix(substring);

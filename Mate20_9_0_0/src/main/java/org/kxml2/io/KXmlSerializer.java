...
public class KXmlSerializer implements XmlSerializer {
        ...
        if (namespace.isEmpty()) {
            ...
            do {
                ...
                for (i2 = (this.nspCounts[this.depth + 1] * 2) - 2; i2 >= 0; i2 -= 2) {
                    if (prefix.equals(this.nspStack[i2])) {
                        prefix = null;
                        continue;
                        break;
                    }
    ...
    public XmlSerializer startTag(String namespace, String name) throws IOException {
        ...
        if (this.elementStack.length < i + 3) {
            ...
            System.arraycopy(this.elementStack, 0, hlp, 0, i);
        ...
        int esp = i + 1;
        this.elementStack[i] = namespace;
        i2 = esp + 1;
        this.elementStack[esp] = prefix;
        this.elementStack[i2] = name;
        append('<');
        if (!prefix.isEmpty()) {
            append(prefix);
            append(':');
    ...
    public XmlSerializer attribute(String namespace, String name, String value) throws IOException {
        if (this.pending) {
            ...
            if (namespace.isEmpty()) {
                ...
                prefix = getPrefix(namespace, null, true);
            }
            append(' ');
            if (!prefix.isEmpty()) {
                append(prefix);
    ...
    public void cdsect(String data) throws IOException {
        ...
        while (i < data.length()) {
            ...
            boolean allowedInCdata = (ch >= ' ' && ch <= 55295) || ch == 9 || ch == 10 || ch == 13 || (ch >= 57344 && ch <= 65533);
            if (allowedInCdata) {
                append(ch);

        ...
        while (i != args.length) {
            ...
            if (arg.startsWith("--filter=") || arg.equals("--filter")) {
                ...
                if (arg.equals("--filter")) {
                    i++;
                    if (i >= args.length) {
                        list = this.parserErrors;
                        stringBuilder = new StringBuilder();
                        stringBuilder.append(arg);
                        stringBuilder.append(" value not specified");
                        list.add(new CommandLineParserError(stringBuilder.toString()));
                        break;
                    }
                    filterSpec = args[i];
                } else {
                ...
                list = this.parserErrors;
                stringBuilder = new StringBuilder();
                stringBuilder.append("JUnit knows nothing about the ");
                stringBuilder.append(arg);
    ...
    void parseParameters(String[] args) {
        for (String arg : args) {
            try {
                ...
                List list = this.parserErrors;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("Could not find class [");
                stringBuilder.append(arg);
                stringBuilder.append("]");
                list.add(new IllegalArgumentException(stringBuilder.toString(), e));
            }
        }
    }

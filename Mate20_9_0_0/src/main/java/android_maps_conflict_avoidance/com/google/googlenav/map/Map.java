...
public class Map {
    ...
    Tile getTopLeftTile(int numXTiles, int numYTiles) {
        ...
        if (yTilesIsEven && centerYOffsetFromCenterTile > 128) {
            yIndex++;
        }
    ...
    private void precalculateTiles() {
        if (this.tiles != null) {
            ...
            while (i < this.xTiles) {
                int index2 = index;
                index = 0;
                while (index < this.yTiles) {
                    int index3 = index2 + 1;
                    this.tiles[index2] = Tile.getTile(tileFlags, this.topLeftTile.getXIndex() + i, this.topLeftTile.getYIndex() + index, this.mapState.getZoom());
                    index++;
                    index2 = index3;
                }
                i++;
    ...
    private synchronized Zoom findZoom(int latSpanE6, int lonSpanE6, MapState newMapState) {
        ...
        while (newZoom.getNextLowerZoom() != null && (getLatitudeSpan(newMapState.newMapState(newZoom)) < latSpanE6 || getLongitudeSpan(newMapState.newMapState(newZoom)) < lonSpanE6)) {
    ...
    private boolean drawMapBackground(GoogleGraphics g, boolean fetch, boolean drawIncompleteTiles) {
        ...
        int tilesDrawn = 0;
        Clock clock2 = Config.getInstance().getClock();
        long paintStartTimeRelative2 = clock2.relativeTimeMillis();
        long paintStartTimeCurrent = clock2.currentTimeMillis();
        boolean drawOptionalFeatures = true;
        if (g == null) {
            drawOptionalFeatures = false;
        }
        this.earliestTileNeededTime = Long.MAX_VALUE;
        boolean drawOptionalFeatures2 = drawOptionalFeatures;
        int tileIndex = 0;
        int i = 0;
        while (true) {
            int i2 = i;
            ...
            int tilesDrawn2 = tilesDrawn;
            boolean drawOptionalFeatures3 = drawOptionalFeatures2;
            i = 0;
            while (true) {
                int j = i;
                if (j >= this.yTiles) {
                ...
                int j2 = j;
                long paintStartTimeRelative3 = paintStartTimeRelative2;
                clock = clock2;
                int i3 = i2;
                tilesDrawn2 += drawTile(this.tiles[tileIndex], i2, j, g, xOffset, yOffset, fetch, drawIncompleteTiles, drawOptionalFeatures3, paintStartTimeRelative2, tileAccessTime);
                if (drawOptionalFeatures3) {
                    paintStartTimeRelative = paintStartTimeRelative3;
                    if (checkPaintTimeExceeded(paintStartTimeRelative)) {
                        drawOptionalFeatures3 = false;
                    }
                } else {
                    paintStartTimeRelative = paintStartTimeRelative3;
                }
                i = j2 + 1;
                paintStartTimeRelative2 = paintStartTimeRelative;
            ...
            clock = clock2;
            i = i2 + 1;
            tilesDrawn = tilesDrawn2;
            drawOptionalFeatures2 = drawOptionalFeatures3;
        }
        paintStartTimeRelative = paintStartTimeRelative2;
        clock = clock2;
        ...
        return checkPaintTimeExceeded(paintStartTimeRelative) ^ 1;
    }
    ...
    private void logPerceivedTileLatency(int tilesDrawn) {
        ...
        if (this.completeTilesInViewport == 0) {
            ...
            stringBuilder3.append("tf");
            stringBuilder3.append(tileType);
            stringBuilder = stringBuilder3.toString();
            stringBuilder2 = new StringBuilder();
            stringBuilder2.append("");
            stringBuilder2.append(latency);
            Log.addEvent((short) 22, stringBuilder, stringBuilder2.toString());
            this.isViewportAllNew = true;
        }
        if (this.completeTilesInViewport < tilesDrawn && tilesDrawn == this.xTiles * this.yTiles) {
            stringBuilder = this.isViewportAllNew ? "tc" : "tp";
            stringBuilder2 = new StringBuilder();
            stringBuilder2.append(stringBuilder);
            stringBuilder2.append(tileType);
            String stringBuilder4 = stringBuilder2.toString();
            StringBuilder stringBuilder5 = new StringBuilder();
            stringBuilder5.append("");
            stringBuilder5.append(latency);
            Log.addEvent((short) 22, stringBuilder4, stringBuilder5.toString());
        }
        this.completeTilesInViewport = tilesDrawn;
    }
    ...
    private void logIfPreCached(MapTile mapTile) {
        if (mapTile.getIsPreCached() && mapTile.getCompletePaintCount() == 1) {
            ...
            stringBuilder.append(now - mapTile.getFirstPaintTime());
            Log.addEvent((short) 22, "pc", stringBuilder.toString());
        }
    }
    ...
    private boolean drawTile(Tile tile, int i, int j, GoogleGraphics g, int xOffset, int yOffset, boolean fetch, boolean drawIncompleteTiles, boolean drawOptionalFeatures, long paintStartTime, long accessTime) {
        GoogleGraphics googleGraphics = g;
        int x = xOffset + (i * 256);
        int y = yOffset + (j * 256);
        if (!tile.notValid()) {
            boolean fetch2;
            ...
            boolean fetch3 = (!fetch || tileOnScreen) ? fetch : false;
            boolean fetch4 = fetch3;
            MapTile mapTile = this.mapService.getTile(tile, priority, fetch4, drawOptionalFeatures ? 2 : 1, accessTime);
            if (tileOnScreen && this.tileOverlayRenderer != null && drawOptionalFeatures) {
                fetch2 = fetch4;
                if (this.tileOverlayRenderer.renderTile(mapTile, fetch2) && mapTile.hasRenderedImage()) {
            ...
            fetch3 = false;
            if (googleGraphics == null || !(mapTile.hasImage() || drawIncompleteTiles)) {
                long j2 = paintStartTime;
            } else {
                googleGraphics.drawImage(mapTile.getImage(accessTime), x, y);
            ...
            Vector layerTileImages = this.mapService.getLayerTiles(tile, fetch2);
            if (googleGraphics != null) {
                int n = layerTileImages.size() - 1;
                while (true) {
                    int n2 = n;
                    if (n2 < 0) {
                        break;
                    }
                    Vector layerTileImages2 = layerTileImages;
                    googleGraphics.drawImage((GoogleImage) layerTileImages.elementAt(n2), x, y);
                    n = n2 - 1;
                    layerTileImages = layerTileImages2;
                }
            }
            return fetch3;
        } else if (googleGraphics == null) {
            return false;
        } else {
            googleGraphics.setColor(16777215);
            googleGraphics.fillRect(x, y, 256, 256);
            return true;
        }
    }
    ...
    private static boolean checkPaintTimeExceeded(long paintStartTime) {
        return Config.getInstance().getClock().relativeTimeMillis() - paintStartTime > 200;
    }
    ...
    public int getMaxMapZoomForPoint(MapPoint point) {
        ...
        int extra = MapTile.getTextSize() == 3 ? 1 : 0;

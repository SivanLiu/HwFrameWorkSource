    ...
    void doBilling(boolean locationDisplayed, boolean routeDisplayed, Map map) {
        ...
        if (!mapState.isSatellite() || isBillingRequiredForSatellite()) {
            if (this.lastBilled == null || this.lastBilledZoom != mapState.getZoom()) {
                ...
                if (newBillPoint != null && (this.previousBilled == null || this.previousBilledZoom != this.lastBilledZoom || mapState.getCenterPoint().distanceSquared(newBillPoint) < mapState.getCenterPoint().distanceSquared(this.lastBilled))) {

    ...
    public static int generate(byte[] dest, int off, int variant, int width, int height, int quality, int qualityAlgorithm) {
        if (variant != 0) {
            ...
            if (off + len <= dest.length) {
                ...
                int yxOffset = JPEG_STANDARD_HEADER_Y_X_OFFSET + off;
                dest[yxOffset] = (byte) (width >> 8);
                dest[yxOffset + 1] = (byte) (width & 255);
                dest[yxOffset + 2] = (byte) (height >> 8);
                dest[yxOffset + 3] = (byte) (height & 255);
                if (quality != 75) {
                    int cOff = JPEG_STANDARD_HEADER_CHROMINANCE_QUANT_OFFSET + off;
                    copyQuantTable(dest, JPEG_STANDARD_HEADER_LUMINANCE_QUANT_OFFSET + off, 0, quality, qualityAlgorithm);
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("qualityAlgorithm: ");
            stringBuilder.append(qualityAlgorithm);
            throw new IllegalArgumentException(stringBuilder.toString());
        }
    }

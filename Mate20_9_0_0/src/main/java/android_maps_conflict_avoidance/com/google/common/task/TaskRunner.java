...
public class TaskRunner implements Runnable {
    ...
    void scheduleTimerTaskInternal(TimerTask task) {
        synchronized (this.mutex) {
            ...
            while (lower < upper) {
                int midway = (lower + upper) / 2;
                if (scheduled >= ((TimerTask) this.timerTaskQueue.elementAt(midway)).getScheduledTime()) {
                    lower = midway + 1;
                } else {
                    upper = midway;
                }
    ...
    /* JADX WARNING: Missing block: B:24:0x003b, code:
            ...
            r2.append(r0);
            r2.append("]");
            android_maps_conflict_avoidance.com.google.debug.Log.logThrowable(r2.toString(), r1);
            r1.printStackTrace();
     */

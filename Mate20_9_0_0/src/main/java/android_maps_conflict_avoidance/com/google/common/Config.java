...
public abstract class Config {
    ...
    private static int[] parseAltKeys(int numberOfKeys, String altKeys) {
        ...
        try {
            ...
            int index = 0;
            while (true) {
                indexOf = altKeys.indexOf(",", lastComma);
                int nextComma = indexOf;
                if (indexOf == -1) {
                    break;
                }
                int index2 = index + 1;
                keyCodes[index] = Integer.parseInt(altKeys.substring(lastComma, nextComma));
                lastComma = nextComma + 1;
                index = index2;
            }
            indexOf = index + 1;
            keyCodes[index] = Integer.parseInt(altKeys.substring(lastComma));
            if (indexOf != numberOfKeys) {
                return null;
            }
            return keyCodes;

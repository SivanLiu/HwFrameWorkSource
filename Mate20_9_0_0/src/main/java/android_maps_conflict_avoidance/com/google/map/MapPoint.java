...
public class MapPoint implements Geometry {
    ...
    public static int getYPixelFromLat(int latitude, Zoom zoom) {
        ...
        int cubicA = ((((-1 * values[indexFloor - 1]) + (values[indexFloor] * 3)) - (values[indexFloor + 1] * 3)) + values[indexFloor + 2]) / 6;
        cubicA = (int) (((((((((((long) cubicA) * ((long) latDiff)) * ((long) latDiff)) / 1000000) * ((long) latDiff)) / 1000000) / 1000000) + ((((((long) ((((values[indexFloor - 1] * 3) - (values[indexFloor] * 6)) + (values[indexFloor + 1] * 3)) / 6)) * ((long) latDiff)) * ((long) latDiff)) / 1000000) / 1000000)) + ((((long) (((((-2 * values[indexFloor - 1]) - (3 * values[indexFloor])) + (values[indexFloor + 1] * 6)) - values[indexFloor + 2]) / 6)) * ((long) latDiff)) / 1000000)) + ((long) values[indexFloor]));
        if (latitude < 0) {
            cubicA = -cubicA;
        }
        return (int) ((((long) zoom.getEquatorPixels()) / 2) - ((long) (mercatorToPixelsTimesTen(cubicA, zoom) / 10)));
    }
    ...
    private static int getLatitudeFromY(int y, Zoom zoom) {
        ...
        int latitude = (int) (((((((((((long) (((((-1 * data[indexFloor - 1]) + (data[indexFloor] * 3)) - (data[indexFloor + 1] * 3)) + data[indexFloor + 2]) / 6)) * ((long) yDiff)) * ((long) yDiff)) / 5000000) * ((long) yDiff)) / 5000000) / 5000000) + ((((((long) ((((data[indexFloor - 1] * 3) - (data[indexFloor] * 6)) + (data[indexFloor + 1] * 3)) / 6)) * ((long) yDiff)) * ((long) yDiff)) / 5000000) / 5000000)) + ((((long) (((((-2 * data[indexFloor - 1]) - (3 * data[indexFloor])) + (data[indexFloor + 1] * 6)) - data[indexFloor + 2]) / 6)) * ((long) yDiff)) / 5000000)) + ((long) data[indexFloor]));
        if (latitude > 80000000) {
            latitude = 80000000;
        }
        if (mercator < 0) {
    ...
    public static String latLonToString(int latitude, int longitude) {
        ...
        stringBuilder.append(TextUtil.e6ToString(latitude));
        stringBuilder.append(',');
        stringBuilder.append(TextUtil.e6ToString(longitude));
    ...
    public int hashCode() {
        return (29 * this.latitudeE6) + this.longitudeE6;
    }
}

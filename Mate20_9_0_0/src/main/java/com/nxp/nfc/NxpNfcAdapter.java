...
import com.nxp.nfc.INfcEventCallback.Stub;
...
public final class NxpNfcAdapter {
    ...
    private class NfcEventCallbackWrapper extends Stub implements NfcEventCallback {
        public final NfcEventCallback callback;

        private NfcEventCallbackWrapper(NfcEventCallback callback) {
    ...
    private static void updateNxpSupportedSElist() {
        try {
            ...
            if (fwVer != null) {
                if (fwVer[0] == (byte) 1 && fwVer[2] == (byte) 17) {
                    ALL_SE_ID_TYPE |= 4;
                }
                return;
            }
        ...
        try {
            ...
            if (seList == null || seList.length == 0) {
                ...
                while (i < seList.length) {
                    ...
                    stringBuilder.append("getAvailableSecure seList[i]");
                    stringBuilder.append(seList[i]);
    ...
    public String[] getActiveSecureElementList(String pkg) throws IOException {
        try {
            ...
            if (activeSEList == null || activeSEList.length == 0) {
                ...
                while (i < activeSEList.length) {
                    String str = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("getActiveSecureElementList activeSE[i]");
                    stringBuilder.append(activeSEList[i]);
                    Log.e(str, stringBuilder.toString());
                    if (activeSEList[i] == 1) {
                        arr[i] = NxpConstants.SMART_MX_ID;
    ...
    public void setNfcEventCallback(NfcEventCallback callback) throws IOException {
        try {
            ...
            INfcEventCallback iNfcEventCallback = null;
            if (callback != null) {
                iNfcEventCallback = new NfcEventCallbackWrapper(callback);
            }
            iNxpNfcAdapter.setNfcEventCallback(iNfcEventCallback);
        } catch (RemoteException e) {
            attemptDeadServiceRecovery(e);

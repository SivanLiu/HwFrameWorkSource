...
public class AsyncHttpClientX {
    ...
    private Map<Context, List<RequestHandleX>> requestMap = new WeakHashMap();
    private ExecutorService threadPool = getDefaultThreadPool();
    private int timeout = 30000;
    private boolean useSynchronousMode = false;

    public AsyncHttpClientX(boolean useSynchronousMode) {
    ...
    public RequestHandleX post(Context context, String url, String request, ResponseHandlerInterfaceX responseHandler) {
        ...
        try {
            ...
            conn = NetworkCheckerThread.TYPE_HTTPS.equals(mUrl.getProtocol()) ? (HttpsURLConnection) mUrl.openConnection() : (HttpURLConnection) mUrl.openConnection();
    ...
    protected RequestHandleX sendRequest(HttpURLConnection conn, String request, ResponseHandlerInterfaceX responseHandler, Context context) {
        if (conn == null) {
            ...
            try {
                ...
                if (context != null) {
                    List<RequestHandleX> requestList = (List) this.requestMap.get(context);
                    if (requestList == null) {
                    ...
                    while (iterator.hasNext()) {
                        if (((RequestHandleX) iterator.next()).shouldBeGarbageCollected()) {
                            iterator.remove();
                        }
                    }
                }

...
class AssistantCallDndHelper {
    ...
    public static void notifyGameBackground(Context ctx, int fwkEvent) {
        if ((fwkEvent == 2 || fwkEvent == 3) && isSystemConfigCallDnd()) {
            turnOffCallDnd(ctx);
        }
    }

    private static void turnOffCallDnd(Context ctx) {
        if (Secure.getInt(ctx.getContentResolver(), GAME_DEEP_DND_MODE, 0) != 0) {
            Secure.putInt(ctx.getContentResolver(), GAME_DEEP_DND_MODE, 2);
        }
    }

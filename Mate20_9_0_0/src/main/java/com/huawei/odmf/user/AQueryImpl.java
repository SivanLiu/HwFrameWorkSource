...
public class AQueryImpl<T extends ManagedObject> implements Query<T> {
    ...
    private FetchRequest fetchRequest = null;
    ...
    public AQueryImpl(String entityName, FetchRequest<T> fetchRequest, ObjectContext objectContext) {
        this.entityName = entityName;
        this.fetchRequest = fetchRequest;
        this.objectContext = (AObjectContext) objectContext;
        this.needOpBeforeThis = false;
        this.tableName = entityName.substring(entityName.lastIndexOf(".") + 1);
    }
    ...
    public Query<T> endGroup() {
        if (!this.debug || this.needOpBeforeThis) {
            this.fetchRequest.getSqlRequest().append(" ) ");
            return this;
        }
    ...
    public Query<T> max(String field) {
        ...
        this.columnsWithAggregateFunction.add("MAX(" + (field.equals("*") ? field : appendTableName(surroundWithQuote(field))) + ")");
        this.columns.add(field);
        this.aggregateOp.add(Integer.valueOf(0));
        return this;
    }
    ...
    public Query<T> min(String field) {
        ...
        field = removeQuotes(field);
        this.columnsWithAggregateFunction.add("MIN(" + (field.equals("*") ? field : appendTableName(surroundWithQuote(field))) + ")");
        this.columns.add(field);
        this.aggregateOp.add(Integer.valueOf(1));
        return this;
    }
    ...
    public Query<T> count(String field) {
        ...
        field = removeQuotes(field);
        this.columnsWithAggregateFunction.add("COUNT(" + (field.equals("*") ? field : appendTableName(surroundWithQuote(field))) + ")");
        this.columns.add(field);
        this.aggregateOp.add(Integer.valueOf(2));
        return this;
    }
    ...
    public Query<T> avg(String field) {
        ...
        field = removeQuotes(field);
        this.columnsWithAggregateFunction.add("AVG(" + (field.equals("*") ? field : appendTableName(surroundWithQuote(field))) + ")");
        this.columns.add(field);
        this.aggregateOp.add(Integer.valueOf(3));
        return this;
    }
    ...
    public Query<T> sum(String field) {
        ...
        field = removeQuotes(field);
        this.columnsWithAggregateFunction.add("SUM(" + (field.equals("*") ? field : appendTableName(surroundWithQuote(field))) + ")");
        this.columns.add(field);
        this.aggregateOp.add(Integer.valueOf(4));
        return this;
    }
    ...
    private String processFieldAndJoinClause(String fieldName) {
        ...
        try {
            ...
            if (classField.getType().toString().equals("interface java.util.List")) {
                ...
                joinedEntityName = genericType.substring(genericType.indexOf("<") + 1, genericType.indexOf(">"));
                joinedTableName = joinedEntityName.substring(joinedEntityName.lastIndexOf(".") + 1);
                ...
                fieldName = joinedTableName + "." + afterDot;
                this.fetchRequest.addToJoinedEntities(joinedEntityName);
                return fieldName;
            }
            joinedEntityName = classField.getType().getName();
            joinedTableName = joinedEntityName.substring(joinedEntityName.lastIndexOf(".") + 1);
            if (this.fetchRequest.getJoinedEntities().contains(joinedEntityName)) {
                return joinedTableName + "." + afterDot;
            }

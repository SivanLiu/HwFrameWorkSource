...
public class AndroidDatabaseHelper extends SQLiteOpenHelper implements DatabaseHelper {
    ...
    public AndroidDatabaseHelper(Context context, String databaseName, ObjectModel model) {
        int databaseVersionCode;
        if (model != null) {
            databaseVersionCode = model.getDatabaseVersionCode();
        } else {
            databaseVersionCode = 1;
        }
        super(context, databaseName, null, databaseVersionCode);
        this.model = model;
    ...
    public void onDowngrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        LOG.logW("The database " + db.getPath() + " may need to be downgraded. Please pay attention.");
    }
    ...
    public void alterTableName(DataBase androidDb, String oldEntityName, String newEntityName) {
        ...
        entity = this.model.getEntity(newEntityName);
        if (entity != null) {
            DatabaseHelperTools.rebuildMidTable(androidDb, oldEntityName, entity);
        }
    }

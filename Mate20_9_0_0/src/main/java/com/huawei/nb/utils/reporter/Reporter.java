...
public class Reporter {
    ...
    public static void f(Fault fault) {
        if (!isDuplicatedLog(fault)) {
            EventStreamEx eventStreamEx = fault.createEventStream();
    ...
    public static void a(Audit audit) {
        EventStreamEx eventStreamEx = audit.createEventStream();
        IMonitorEx.sendEvent(eventStreamEx);
        IMonitorEx.closeEventStream(eventStreamEx);
    }
    ...
    private static boolean isDuplicatedAudit(Audit audit) {
        ...
        if (!sAuditMap.containsKey(Integer.valueOf(auditId)) || currentTime - ((Long) sAuditMap.get(Integer.valueOf(auditId))).longValue() >= 86400000) {

...
import com.huawei.nb.searchmanager.client.ISearchServiceCall.Stub;
import com.huawei.nb.searchmanager.client.connect.RemoteServiceConnection;
import com.huawei.nb.searchmanager.client.connect.RemoteServiceConnection.OnConnectListener;
...
public class SearchServiceProxy implements ISearchClient {
    ...
    public int executeInsertIndex(String pkgName, List<SearchIndexData> dataList, List<Attributes> attrsList) {
        int i = 0;
        if (this.searchService == null) {
            DSLog.e("Failed to executeInsertIndex, error: searchService is null.", new Object[i]);
            return i;
        }
        try {
            return this.searchService.executeInsertIndex(pkgName, dataList, attrsList, this.callingPkgName);
        } catch (RemoteException e) {
            DSLog.e("SearchServiceProxy Failed to executeInsertIndex " + e.getMessage(), new Object[i]);
            return i;
        }
    }
    ...
    public int executeUpdateIndex(String pkgName, List<SearchIndexData> dataList, List<Attributes> attrsList) {
        int i = 0;
        if (this.searchService == null) {
            DSLog.e("Failed to executeUpdateIndex, error: searchService is null.", new Object[i]);
            return i;
        }
        try {
            return this.searchService.executeUpdateIndex(pkgName, dataList, attrsList, this.callingPkgName);
        } catch (RemoteException e) {
            DSLog.e("SearchServiceProxy Failed to executeUpdateIndex " + e.getMessage(), new Object[i]);
            return i;
        }
    }
    ...
    public int executeDeleteIndex(String pkgName, List<String> idList, List<Attributes> attrsList) {
        int i = 0;
        if (this.searchService == null) {
            DSLog.e("Failed to executeDeleteIndex, error: searchService is null.", new Object[i]);
            return i;
        }
        try {
            return this.searchService.executeDeleteIndex(pkgName, idList, attrsList, this.callingPkgName);
        } catch (RemoteException e) {
            DSLog.e("SearchServiceProxy Failed to executeDeleteIndex " + e.getMessage(), new Object[i]);
            return i;
        }
    }

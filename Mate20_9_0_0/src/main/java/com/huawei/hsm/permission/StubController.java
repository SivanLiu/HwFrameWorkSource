...
public class StubController {
    ...
    private static IHoldService mHoldService;
    ...
    private static final class HoldServiceDeathRecipient implements DeathRecipient {
        private HoldServiceDeathRecipient() {
        }

        /* synthetic */ HoldServiceDeathRecipient(AnonymousClass1 x0) {
            this();
        }
        ...
        public void binderDied() {
            ...
            StubController.mHoldService = null;
        }
    }
    ...
    private static void notifyBinderDie() {
        ...
        for (int i = 0; i < size; i++) {
            ((HoldServiceDieListener) listeners.get(i)).notifyServiceDie();
        }
    }
            ...
            public void run() {
                try {
                    ...
                    if (StubController.mRequestCount > 16) {
                        String str = StubController.TAG;
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append("holdForInsertBroadcastRecord mRequestCount = ");
                        stringBuilder.append(StubController.mRequestCount);
    ...
    private static void getHoldServiceByUidInner(int callingUserId) {
        try {
            ...
            if (b != null) {
                ...
                mHoldService = Stub.asInterface(b);
                mUseridOfMoniterBinder = UserHandle.getUserId(callingUserId);
                if (!hasLinkToDeath) {
                    b.linkToDeath(mServiceDeathRecipient, 0);
                }
            }
        ...
        try {
            ...
            if (Log.HWLog) {
                synchronized (mRequestCountSync) {
                    str2 = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("preCheckCode:");
                    stringBuilder.append(preCheckCode);
                    stringBuilder.append(", validUid ");
                    stringBuilder.append(validUid);
                    stringBuilder.append(", validPid ");
                    stringBuilder.append(validPid);
                    stringBuilder.append(", permissionType ");
                    ...
                    if (letCurrentRequestGoOrNot(permissionType)) {
                        ...
                        synchronized (syncObj) {
                            ...
                            if (Log.HWLog) {
                                ...
                                stringBuilder2.append("holdResult:");
                                stringBuilder2.append(holdResult);
    ...
    public static int authenticateSmsSend(IBinder notifyResult, int uidOf3RdApk, int smsId, String smsBody, String smsAddress) {
        IHoldService hService = getHoldServiceByUid(handleIncomingUser(uidOf3RdApk));
        int retVal = 0;
        if (hService == null) {
            Log.e(TAG, "service = null");
            notifyBinderDie();
        ...
        try {
            hService.authenticateSmsSend(notifyResult, uidOf3RdApk, smsId, smsBody, smsAddress);
        } catch (RemoteException e) {
            retVal = 1;
    ...
    public static void notifyBackgroundMgr(String pkgName, int pid, int uidOf3RdApk, int permType, int permCfg) {
        ...
        if (validUid >= 10000) {
            ...
            try {
                ...
                bundle.putInt(FreezeScreenScene.PID_PARAM, validPid);
                bundle.putInt(TABLE_COLUM_UID, uidOf3RdApk);

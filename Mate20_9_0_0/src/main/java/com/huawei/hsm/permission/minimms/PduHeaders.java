...
class PduHeaders {
    ...
    protected void setEncodedStringValue(EncodedStringValue value, int field) {
        if (value != null) {
            if (!(field == 137 || field == 147 || field == 150 || field == RETRIEVE_TEXT || field == PREVIOUSLY_SENT_BY || field == MM_FLAGS || field == STORE_STATUS_TEXT)) {
                switch (field) {
    ...
    protected void setLongInteger(long value, int field) {
        if (!(field == 133 || field == 142 || field == REPLY_CHARGING_DEADLINE || field == REPLY_CHARGING_SIZE || field == PREVIOUSLY_SENT_DATE || field == MESSAGE_COUNT || field == START || field == LIMIT)) {
            switch (field) {
                case 135:
                case 136:
                    break;

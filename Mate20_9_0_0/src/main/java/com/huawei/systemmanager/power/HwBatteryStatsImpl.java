...
class HwBatteryStatsImpl implements IBatteryStats {
    ...
    private long getRadioTime(long rawRealTime) {
        ...
        long signalTimeMs = 0;
        for (long signalTimeMs2 = 0; signalTimeMs2 < 5; signalTimeMs2++) {
            signalTimeMs += this.mBatteryStatsHelper.getStats().getPhoneSignalStrengthTime(signalTimeMs2, rawRealTime, 0);
        }
        return signalTimeMs;
    }
    ...
    public long computeTimePerLevel() {
        ...
        if (numSteps == 1) {
            ...
            for (i = former; i < middle; i++) {
                total_middle += steps[i] & STEP_LEVEL_TIME_MASK;
            }
            for (i = middle; i < numSteps; i++) {
                total_latter += steps[i] & STEP_LEVEL_TIME_MASK;
            }
            long[] jArr = steps;

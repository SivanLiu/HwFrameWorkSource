...
public class DeviceEmailManager {
    ...
    public boolean isAppDataClearanceDisabled() {
        ...
        boolean z = false;
        if (bundle.getBoolean("value", false) || isAccountDeletionDisabled(null)) {
            z = true;
        }
        return z;
    }
    ...
    public boolean isAppDataClearanceDisabled(Context context) {
        ...
        boolean z = false;
        if (bundle.getBoolean("value", false) || isAccountDataDeletionDisabled(context)) {
            z = true;
        }
        return z;
    }
    ...
    private boolean isAccountDataDeletionDisabled(Context context) {
        ...
        int i;
        if (disable) {
            whiteList = getAccountsWhiteListDisablingDeletion(null);
            i = 0;
            while (i < emailLen) {
        ...
        whiteList = getAccountsBlackListAllowingDeletion(null);
        for (Account account : emailAccounts) {
            if (isInRuleList(account.name, whiteList)) {
                Log.d(TAG, "MDM6->isClearAppDtaDisabled->email in black");
                return true;
            }
        }
        for (Account account2 : exchangeAccounts) {
            if (isInRuleList(account2.name, whiteList)) {
    ...
    public boolean isAccountAdditionDisabled(ComponentName admin, String emailAddress) {
        ...
        if (isAccountAdditionDisabled(admin)) {
            ...
            stringBuilder.append("MDM6->isAccountAdditionDisabled->matched in white: ");
        ...
        str = TAG;
        stringBuilder = new StringBuilder();
        stringBuilder.append("MDM6->isAccountAdditionDisabled->matched in black: ");
        stringBuilder.append(matched);
        Log.d(str, stringBuilder.toString());
        return matched;
    }
    ...
    public boolean isAccountDeletionDisabled(ComponentName admin, String emailAddress) {
        ...
        if (isAccountDeletionDisabled(admin)) {
            ...
            str = TAG;
            stringBuilder = new StringBuilder();
            stringBuilder.append("MDM6->isAccountDeletionAllowed->matched in white: ");
            stringBuilder.append(matched);
            Log.d(str, stringBuilder.toString());
            return matched ^ 1;
        }
        ...
        str = TAG;
        stringBuilder = new StringBuilder();
        stringBuilder.append("MDM6->isAccountDeletionAllowed->matched in black: ");
        stringBuilder.append(matched);
        Log.d(str, stringBuilder.toString());
        return matched;
    }
    ...
    private boolean wildCardMatch(String text, String pattern) {
        ...
        if (!pattern.endsWith(PhoneConstantsEx.APN_TYPE_ALL) && cards.length > 1 && !text.endsWith(cards[cards.length - 1])) {
            return false;
        }
        boolean isFirstPart = true;
        String text2 = text;
        for (String card : cards) {
            int idx = text2.indexOf(card);
            if (isFirstPart) {
                if (!text2.startsWith(card)) {
                    return false;
                }
                isFirstPart = false;
            }
            ...
            text2 = text2.substring(card.length() + idx);
        }
        return true;
    }

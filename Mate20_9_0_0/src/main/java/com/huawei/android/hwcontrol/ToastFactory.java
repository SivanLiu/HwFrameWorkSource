...
public class ToastFactory implements HwToast {
    ...
    private void updateBlurStatus(Context context, Toast toast, AttributeSet set) {
        if (toast != null) {
            ...
            stringBuilder.append("HwToast-");
            stringBuilder.append(lp != null ? lp.getTitle().toString() : "");
            HwRTBlurUtils.updateBlurStatus(lp, HwRTBlurUtils.obtainBlurStyle(context, set, 33619992, 33951767, stringBuilder.toString()));
        }
    }
}

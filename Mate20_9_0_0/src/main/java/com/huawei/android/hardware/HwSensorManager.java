...
public class HwSensorManager implements SensorEventListener {
    ...
    public boolean registerSensorListener(int sensorType, int rateUs) {
        ...
        if (isValid(sensorType) && sensorType < 16) {
            resetSensorData();
            this.mTimeStamp = 0;
            boolean registered = registerSystemSensor(rateUs, true);
            if (!swingEventRegistered) {
                z = registered;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (type == 1) {
            ...
            if (this.mTimeStamp != 0) {
                dT = (float) (event.timestamp - this.mTimeStamp);
            ...
            if (Math.abs(gyro_wy) >= SENSOR_CORRENT_THRESHOLD || Math.abs(gyro_wx) >= SENSOR_CORRENT_THRESHOLD || Math.abs(gyro_wz) >= SENSOR_CORRENT_THRESHOLD || this.mSensorEventDetector == null) {
                ...
                dT = reviseAngleY(this.mAngleY, this.mAngleX);
                ...
                if ((4 & this.mType) != 0) {
                    gyro_wx = 0.0f;
                } else if ((2 & this.mType) != 0) {
                    gyro_wy = 0.0f;
                }
                if (!(this.mHandler == null || this.mHandler.hasMessages(1) || !getDirectionChanged(gyro_wy, gyro_wx, dT, this.mAngleX))) {
                    z = true;
                }
                if (z && this.mSensorEventDetector != null) {
    ...
    public void unRegisterListener(int sensorType) {
        ...
        stringBuilder.append("unRegisterListener type invalid : ");
    ...
    private boolean registerSystemSensor(int delayUs, int type) {
        ...
        if (sensor == null) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("unspported system sensor type : ");
        ...
        if (!rt) {
            ...
            stringBuilder2.append("registerListener fail : ");
    ...
    private float getAngleLagerThanPI(float angle) {
        ...
        if (((double) temp) <= -3.141592653589793d) {
            return 0.0f;
        }
        return temp;
    }

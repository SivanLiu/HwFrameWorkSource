...
public class FmTransceiver {
    ...
    protected boolean acquire(String device) {
        ...
        if (sFd <= 0) {
                    ...
                    synchronized (this) {
                        ...
                        while (!isRadioEnabled() && mRadioMethodImplemented && retry < 100) {
                            try {
                                ...
                                stringBuilder2.append("wait 100ms for radio on , retry =");
            ...
            if (sFd > 0) {
                ...
                stringBuilder.append(sFd);
                Log.d(str, stringBuilder.toString());
                bStatus = true;
            } else {
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("Fail to Open ");
                stringBuilder.append(sFd);
                Log.d(str, stringBuilder.toString());
                bStatus = false;
            }
            ...
            str = TAG;
            stringBuilder = new StringBuilder();
            stringBuilder.append("Already Opened:");
            stringBuilder.append(sFd);
            Log.d(str, stringBuilder.toString());
            bStatus = true;
        }
    ...
    boolean release(String device) {
        if (sFd != 0) {
            ...
            stringBuilder.append("Turned off: ");
            stringBuilder.append(sFd);
            Log.d(str, stringBuilder.toString());
            if (!("hi110x".equals(FM_CHIPTYPE) || "hisi".equals(FM_CHIPTYPE) || "Qualcomm".equals(FM_CHIPTYPE))) {
                int retry = 0;
                    ...
                    synchronized (this) {
                        ...
                        while (isRadioEnabled() && mRadioMethodImplemented && retry < 100) {
                            try {
                                String str2 = TAG;
                                StringBuilder stringBuilder2 = new StringBuilder();
                                stringBuilder2.append("wait 100ms for radio off , retry =");
                                stringBuilder2.append(retry);
                                Log.d(str2, stringBuilder2.toString());
                                retry++;
                                wait(100);
    ...
    public boolean enable(FmConfig configSettings, int device) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("turning on %d");

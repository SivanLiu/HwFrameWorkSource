...
final class DistinguishedNameParser {
    ...
    private char getEscaped() {
        ...
        if (this.pos != this.length) {
            ...
            if (!(c == ' ' || c == '%' || c == '\\' || c == '_')) {
                switch (c) {
                    case AcquireInfo.FACEID_OUT_OF_BOUNDS /*34*/:
                    case AcquireInfo.FACEID_NOT_THE_SAME_FACE /*35*/:
                        break;
                    default:
                        switch (c) {
                            case '*':
    ...
    private char getUTF8() {
        ...
        this.pos++;
        if (res < AppOpsManagerEx.TYPE_MICROPHONE) {
            return (char) res;
        }
        if (res < HwCarrierConfigManager.HD_ICON_MASK_DIALER || res > 247) {
            return '?';
        }
        int count;
        if (res <= 223) {

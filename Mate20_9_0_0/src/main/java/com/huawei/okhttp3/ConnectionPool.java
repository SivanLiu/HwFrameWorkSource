...
public final class ConnectionPool {
    ...
    long cleanup(long now) {
        ...
        synchronized (this) {
            for (RealConnection connection : this.connections) {
                ...
                } else if (connection.http2Connection == null || now - connection.keepaliveTimestampNs >= 1000000000) {
                    idleConnectionCount++;
    ...
    public synchronized int http2ConnectionCount(Address address) {
        ...
        count = 0;
        for (RealConnection connection : this.connections) {
            if (address.equals(connection.route().address) && !connection.noNewStreams && connection.http2Connection != null && (connection.successCount == 0 || connection.isHealthy(true))) {
                count++;
            }
        }
        return count;
    ...
    public int http2ConnectionCount(String hostName, int port, String scheme) {
        int count = 0;
        for (RealConnection connection : this.connections) {
            if (connection.http2Connection != null && hostName.equals(connection.route().address.url().host()) && port == connection.route().address.url().port() && scheme.equals(connection.route().address.url().scheme()) && !connection.noNewStreams && (connection.successCount == 0 || connection.isHealthy(true))) {
                count++;
            }
        }
        return count;
    ...
    public boolean keepHttp2ConnectionAlive(String hostName, int port, String scheme) {
        for (RealConnection connection : this.connections) {
            if (connection.http2Connection != null && hostName.equals(connection.route().address.url().host()) && port == connection.route().address.url().port() && scheme.equals(connection.route().address.url().scheme()) && !connection.noNewStreams && connection.isHealthy(true)) {

    ...
    public static CacheControl parse(Headers headers) {
        ...
        while (i < size) {
            ...
            while (true) {
                ...
                noCache = pos;
                noStore2 = noStore;
                int pos3 = HttpHeaders.skipUntil(value, pos, "=,;");
                ...
                if (pos3 != value.length()) {
                    ...
                    if (!(value.charAt(pos3) == ',' || value.charAt(pos3) == ';')) {
                        pos3 = HttpHeaders.skipWhitespace(value, pos3 + 1);
                        int parameterStart;
                        if (pos3 >= value.length() || value.charAt(pos3) != '\"') {
                            ...
                            pos3++;
                            parameterStart = pos3;
                            pos3 = HttpHeaders.skipUntil(value, pos3, "\"");
                        ...
                        if (!"no-cache".equalsIgnoreCase(noStore)) {
                            ...
                            if ("max-age".equalsIgnoreCase(noStore)) {
                                maxAgeSeconds = HttpHeaders.parseSeconds(noCache, -1);
                                noCache = noCache2;
                                noStore = noStore2;
                                pos = pos2;
                            } else {
                                if ("s-maxage".equalsIgnoreCase(noStore) != 0) {
                                    sMaxAgeSeconds = HttpHeaders.parseSeconds(noCache, -1);
                                    ...
                                    maxStaleSeconds = HttpHeaders.parseSeconds(noCache, Integer.MAX_VALUE);
                                } else if ("min-fresh".equalsIgnoreCase(noStore)) {
                                    minFreshSeconds = HttpHeaders.parseSeconds(noCache, -1);

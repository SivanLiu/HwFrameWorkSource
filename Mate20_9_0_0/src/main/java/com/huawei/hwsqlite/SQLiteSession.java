...
public final class SQLiteSession {
    ...
    private boolean mStepCursor = false;
    ...
    public int executeForCursorWindow(String sql, Object[] bindArgs, CursorWindow window, int startPos, int requiredPos, boolean countAllRows, int connectionFlags, CancellationSignal cancellationSignal) {
        ...
        if (str == null) {
            ...
            objArr = bindArgs;
            if (executeSpecial(str, objArr, i, cancellationSignal2)) {
                window.clear();
                return 0;
            }
            acquireConnection(str, i, cancellationSignal2);
            try {
                int executeForCursorWindow = this.mConnection.executeForCursorWindow(str, objArr, window, startPos, requiredPos, countAllRows, cancellationSignal2);
    ...
    PreparedStatement beginStepQuery(String sql, int connectionFlags, CancellationSignal cancellationSignal) {
        acquireConnection(sql, connectionFlags, cancellationSignal);
        this.mStepCursor = true;
        return this.mConnection.beginStepQuery(sql);
    }

    void endStepQuery(PreparedStatement statement) {
        if (this.mStepCursor) {
            this.mConnection.endStepQuery(statement);
            this.mStepCursor = false;
            releaseConnection();
            return;
        }
        throw new IllegalStateException("end a unopened step query");
    }

    int executeForStepQuery(String sql, Object[] bindArgs, CancellationSignal cancellationSignal) {
        if (sql != null) {
            return this.mConnection.executeForStepQuery(sql, bindArgs, cancellationSignal);
        }
        throw new IllegalArgumentException("sql must not be null.");
    }
    ...
    private boolean executeSpecial(String sql, Object[] bindArgs, int connectionFlags, CancellationSignal cancellationSignal) {
        ...
        switch (SQLiteDatabaseUtils.getSqlStatementType(sql)) {
            case 4:
    ...
    private void acquireConnection(String sql, int connectionFlags, CancellationSignal cancellationSignal) {
        if (this.mStepCursor) {
            throw new IllegalStateException("Cloud not acquire new connection because the session is busy with step query!");
        }

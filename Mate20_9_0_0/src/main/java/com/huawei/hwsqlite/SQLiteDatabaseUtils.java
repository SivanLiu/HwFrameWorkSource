...
public class SQLiteDatabaseUtils {
        ...
        private long insertInternal(ContentValues values, boolean allowReplace) {
            ...
            try {
                ...
                for (Entry<String, Object> e : values.valueSet()) {
                    SQLiteDatabaseUtils.bindObjectToProgram(stmt, getColumnIndex((String) e.getKey()), e.getValue());
                }
                ...
                String str = SQLiteDatabaseUtils.TAG;
                StringBuilder stringBuilder = new StringBuilder();
    ...
    private static char[] encodeHex(byte[] input) {
        ...
        for (int i = 0; i < l; i++) {
            int j2 = j + 1;
            out[j] = DIGITS[(240 & input[i]) >>> 4];
            j = j2 + 1;
            out[j2] = DIGITS[15 & input[i]];
        }
        return out;
    }

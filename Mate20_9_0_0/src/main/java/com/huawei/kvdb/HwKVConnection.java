...
public class HwKVConnection {
    ...
    HwKVConnection(String path, String dbName, String tableName, String packageName, Context context, Boolean readOnly) {
        ...
        if (!readOnly.booleanValue() || UserHandle.myUserId() == 0) {
            ...
            stringBuilder.append(externalPath.substring(0, externalPath.lastIndexOf("/")));
            stringBuilder.append("/0/");
            stringBuilder.append("Android/data/com.android.providers.media/thumbnail_cache");
    ...
    private void deleteDbFiles() {
        String db = new StringBuilder();
        db.append(this.mPath);
        db.append("/");
        db.append(this.mDbName);
        db = db.toString();
        String shm = new StringBuilder();
        shm.append(db);
        shm.append("-shm");
        shm = shm.toString();
        String wal = new StringBuilder();
        wal.append(db);
        wal.append("-wal");
        wal = wal.toString();

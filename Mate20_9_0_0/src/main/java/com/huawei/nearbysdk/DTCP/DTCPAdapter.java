...
public final class DTCPAdapter {
    ...
    /* synthetic */ DTCPAdapter(DTCPServiceConnection x0, IDTCPService x1, AnonymousClass1 x2) {
        this(x0, x1);
    }
    ...
    public static synchronized boolean createInstance(Context context, AdapterCreateCallback cb) {
        synchronized (DTCPAdapter.class) {
            if (context == null) {
                ...
                DTCPServiceConnection svcConnection = new DTCPServiceConnection(context, cb);
                boolean isBindServiceAsUser = false;
                try {
                    UserHandle current = (UserHandle) UserHandle.class.getField("CURRENT").get(UserHandle.class);
                    isBindServiceAsUser = ((Boolean) Context.class.getDeclaredMethod("bindServiceAsUser", new Class[]{Intent.class, ServiceConnection.class, Integer.TYPE, UserHandle.class}).invoke(context, new Object[]{intent, svcConnection, Integer.valueOf(1), current})).booleanValue();
                } catch (Throwable e) {
                    String str = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("isBindServiceAsUser ERROR:");

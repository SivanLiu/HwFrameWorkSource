    ...
    private HashMap<Integer, String> rules = new HashMap<Integer, String>() {
        {
            ...
            put(Integer.valueOf(21014), "(([regex_y][param_digitYear][param_textyear])\\s*年\\s*)?(([regex_m][param_digitMonth][param_textmonth])\\s*月)\\s*([regex_d][param_digitDay])\\s*(日|号)(\\s*[\\(（]?([param_EEEE]|[param_E]|[param_days])[\\)）]?){0,1}");

...
public class ReguExp extends Regexs {
    ...
    private static String regED1;
    private static String regED2;
    private static String regST;
    ...
    static {
        StringBuilder stringBuilder = new StringBuilder(White);
        stringBuilder.append(buildkeyED_New);
        stringBuilder.append("(");
        stringBuilder.append(prepandwords);
        stringBuilder.append(")");
        regED1 = stringBuilder.toString();
        stringBuilder = new StringBuilder(White);
        stringBuilder.append(buildkeyED_Rest);
        stringBuilder.append("(");
        stringBuilder.append(words);
        stringBuilder.append(")");
        regED2 = stringBuilder.toString();
        stringBuilder = new StringBuilder("(\\s{0,6},\\s{0,6})?(");
        stringBuilder.append(stresskey1_New);
        stringBuilder.append("(");
        stringBuilder.append(prepandwords_ST);
        stringBuilder.append(")|(");
        stringBuilder.append(stresskey1_Rest);
        stringBuilder.append(words);
        stringBuilder.append("))([,-]{0,1}\\s{0,6}(?<!\\d)\\d{1,4}(?!\\d)\\s{0,6}(-\\d{1,4})?)?([,-]*\\s*(IT-(?<!\\d)\\d{5}(?!\\d)|I-(?<!\\d)\\d{5}(?!\\d)|(?<!\\d)\\d{5}(?!\\d))\\s*[,-]*)?");
        regST = stringBuilder.toString();
    }

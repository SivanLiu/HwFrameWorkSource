...
public class SerEngine {
    ...
    private static int[] findAddress(String sentence) {
        ...
        int t;
        String str = sentence;
        Matcher mBui1 = matchers(str, "pBui1");
        Matcher mStr1 = matchers(str, "pStr1");
        Matcher mPos1 = matchers(str, "mPos1");
        Matcher mNum = matchers(str, "pNumber");
        ...
        switch (type) {
            case 1:
                mList.add(matchers(str, "pBuiCity"));
                beginList = beginListB;
                break;
            case 2:
                mList.add(matchers(str, "pStrCity"));
                beginList = beginListS;
                break;
            case 3:
                mList.add(matchers(str, "pBuiStrCity"));
                mList.add(matchers(str, "pBuiCity"));
                mList.add(matchers(str, "pStrCity"));
            ...
            case 5:
                ...
                break;
            case 6:
                mList.add(matchers(str, "pStrPos"));
                mList.add(matchers(str, "pStrCity"));
                beginList = beginListS;
                break;
            case 7:
                mList.add(matchers(str, "pBuiCity"));
                mList.add(matchers(str, "pBuiStrPos"));
                mList.add(matchers(str, "pBuiStrCity"));
                mList.add(matchers(str, "pStrPos"));
                mList.add(matchers(str, "pStrCity"));
                beginList = beginListC;
        ...
        int size = indAddr.size();
        t = size;
        int[] re = new int[(size + 1)];
        re[0] = t / 2;
        size = 0;
        while (size < t) {
            re[size + 1] = ((Integer) indAddr.get(size)).intValue();
            size++;
            str = sentence;
        }
        return re;

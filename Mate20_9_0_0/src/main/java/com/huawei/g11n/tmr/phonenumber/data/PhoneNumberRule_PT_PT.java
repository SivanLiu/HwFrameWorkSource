...
public class PhoneNumberRule_PT_PT extends PhoneNumberRule {
    ...
    public void init() {
        ...
        String nRegex_ShortMonthDate2 = new StringBuilder("([012]?\\d|3[01])(.|-|\\p{Blank}){0,2}(");
        nRegex_ShortMonthDate2.append("jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez|janeiro|fevereiro|março|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro");
        nRegex_ShortMonthDate2.append(")(.|-|\\p{Blank}){0,2}(1[4-9]\\d{2}|20[01]\\d)(\\p{Blank}{0,2})(([01]?\\d|2[0-4])\\p{Blank}{0,2}[.:]\\p{Blank}{0,2}[0-5]\\d(\\p{Blank}{0,2}[.:]\\p{Blank}{0,2}[0-5]\\d))?");
        nRegex_ShortMonthDate2 = nRegex_ShortMonthDate2.toString();
        StringBuilder stringBuilder = new StringBuilder("[0-9.,]+(?<![.,])\\p{Blank}*(‰|%|\\p{Sc}|");
        stringBuilder.append(money);
        stringBuilder.append("(?!\\p{L}))|(\\p{Sc}|(?<!\\p{L})(");
        stringBuilder.append(money);
        stringBuilder.append("))\\p{Blank}*[0-9.,]+(?<![.,])");
        nRules.add(new RegexRule(stringBuilder.toString(), 66));
        String nRegex_sCurrency = new StringBuilder("\\d{2,16}\\p{Blank}{0,2},\\p{Blank}{0,2}-\\p{Blank}{0,2}(");
        nRegex_sCurrency.append(money);
        nRegex_sCurrency.append(")");
        nRules.add(new RegexRule(nRegex_sCurrency.toString(), 66));
        String nRegex_Measure = new StringBuilder("\\d[0-9\\p{Blank}.,-]*\\d\\p{Blank}{0,4}(");
        nRegex_Measure.append("minutos|segundos|kilobytes|megabytes|toneladas|dias|kilohertz|byte|metros\\p{Blank}+quadrados|quilómetros|MB|decilitros|m²|MW|mililitros|gigahertz|pints|oz|ha|bit|hectopascais|Tb|pm|gal|Mb|milissegundos|bytes|quilocalorias|A|hPa|GB|jardas|J|K|picómetros|W|V|quilojoules|kelvins|mg|dm|dl|megawatts|ml|kHz|joules|min|quilogramas|g|sem\\.|mm|graus|miliamperes|miligramas|Gb|centímetros|ms|l|milhas\\p{Blank}+náuticas|m|nmi|h|kWh|s|quilates|semanas|volts|kW|horas|cm²|calorias|libras|meses|anos|kg|Ω|quilowatts|amperes|kb|quilowatts-hora|gigabytes|hertz|megahertz|watts|kilobits|onças|ct|km|milímetros|Hz|cm|GHz|gigabits|mA|graus\\p{Blank}+Celsius|°|centímetros\\p{Blank}+quadrados|lb|metros|ohms|litros|hectares|decímetros|bits|ton|cal|MHz|TB|megabits|°C|°F|yd|terabits|kcal|galões|kJ|pt|gramas|terabytes|graus\\p{Blank}+Fahrenheit|kB");
        nRegex_Measure.append(")(?![\\p{L}0-9])");
        nRegex_Measure = nRegex_Measure.toString();
        ...
        nRules.add(new RegexRule("\\d[0-9\\p{Blank}.,-]*\\d\\p{Blank}{0,4}(vezes|(de\\p{Blank}+)?tweets?|twittar|rts|temporada|contas|capitulos|páginas|no\\p{Blank}+total\\p{Blank}+em\\p{Blank}+dinheiro|milhoes|mil)(?!\\p{L})", 2));
        ...
        this.codesRules.add(new RegexRule(this, "") {
            public PhoneNumberMatch isValid(PhoneNumberMatch possibleNumber, String msg) {
                ...
                if (possibleNumber.start() - 1 >= 0) {
                    beChars = msg.substring(null, possibleNumber.start()).toCharArray();
                    isTwo = true;
                    i = 0;

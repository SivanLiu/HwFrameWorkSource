            ...
            case 1001:
                if (this.searchServiceProxy.hasConnected()) {
                    if (this.indexTaskQueue.getQueueSize() > 0) {
                        task = this.indexTaskQueue.take();
                        if (task.getIds() != null) {
                            this.searchServiceProxy.executeDBCrawl(task.getPkgName(), task.getIds(), task.getOp());
                        } else {
                    ...
                    return;
                } else if (!this.searchServiceProxy.isBinded()) {

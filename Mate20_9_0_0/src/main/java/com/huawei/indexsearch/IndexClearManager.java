...
public class IndexClearManager implements IIndexClearManager {
    ...
    static final String[] MONITOR_ALL_PACKAGE_NAME = new String[]{"com.android.email", "com.example.android.notepad", "com.android.providers.calendar", "com.android.providers.telephony", "com.android.mms"};
    ...
    private SearchServiceProxy searchServiceProxy = new SearchServiceProxy(ActivityThread.currentApplication().getApplicationContext());

    class CachedItem {
        public String pkgName;
        public int userId;

        public CachedItem(String pkgName, int userId) {
                ...
                case 1001:
                    if (IndexClearManager.this.searchServiceProxy.hasConnected()) {
                        if (IndexClearManager.this.mCachedItemList.size() > 0) {
                            item = (CachedItem) IndexClearManager.this.mCachedItemList.remove(0);
                            IndexClearManager.this.searchServiceProxy.clearUserIndexSearchData(item.pkgName, item.userId);
                            if (IndexClearManager.this.mCachedItemList.size() != 0) {
                        ...
                        return;
                    } else if (!IndexClearManager.this.searchServiceProxy.isBinded()) {

...
final class BoundingBox {
    ...
    BoundingBox addMissingRows(int missingStartRows, int missingEndRows, boolean isLeft) throws NotFoundException {
        ...
        if (missingStartRows > 0) {
            top = isLeft ? this.topLeft : this.topRight;
            newMinY = ((int) top.getY()) - missingStartRows;
            if (newMinY < 0) {
                newMinY = 0;
            }
            newTop = new ResultPoint(top.getX(), (float) newMinY);
            if (isLeft) {
                newTopLeft = newTop;
            } else {
                newTopRight = newTop;
        ...
        if (missingEndRows > 0) {
            top = isLeft ? this.bottomLeft : this.bottomRight;
            newMinY = ((int) top.getY()) + missingEndRows;
            if (newMinY >= this.image.getHeight()) {
                newMinY = this.image.getHeight() - 1;
            }
            newTop = new ResultPoint(top.getX(), (float) newMinY);
            if (isLeft) {
                newBottomLeft = newTop;
            } else {
                newBottomRight = newTop;
            }
        }
        calculateMinMaxValues();

    ...
    private int adjustRowNumbersFromLRI() {
        ...
        while (codewordsRow < codewords.length) {
            if (codewords[codewordsRow] != null) {
                int rowIndicatorRowNumber = codewords[codewordsRow].getRowNumber();
                int invalidRowCounts = 0;
                int unadjustedCount2 = unadjustedCount;
                for (unadjustedCount = 1; unadjustedCount < this.barcodeColumnCount + 1 && invalidRowCounts < 2; unadjustedCount++) {
                    Codeword codeword = this.detectionResultColumns[unadjustedCount].getCodewords()[codewordsRow];
                    if (codeword != null) {
                        invalidRowCounts = adjustRowNumberIfValid(rowIndicatorRowNumber, invalidRowCounts, codeword);
                        if (!codeword.hasValidRowNumber()) {
                            unadjustedCount2++;
                ...
                unadjustedCount = unadjustedCount2;
            }
            codewordsRow++;
        }
        return unadjustedCount;

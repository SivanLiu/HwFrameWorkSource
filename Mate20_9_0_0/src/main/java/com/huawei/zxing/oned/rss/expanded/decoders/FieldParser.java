    ...
    static String parseFieldsInGeneralPurpose(String rawInformation) throws NotFoundException {
        ...
        if (rawInformation.length() >= 2) {
            ...
            if (rawInformation.length() >= 3) {
                ...
                length2 = 0;
                while (length2 < i) {
                    dataLength2 = objArr3[length2];
                    if (!dataLength2[0].equals(firstThreeDigits)) {
                        length2++;
                    } else if (dataLength2[1] == VARIABLE_LENGTH) {
                        return processVariableAI(4, ((Integer) dataLength2[2]).intValue(), rawInformation);
                    } else {
                        return processFixedAI(4, ((Integer) dataLength2[1]).intValue(), rawInformation);
                    }
                }
                if (rawInformation.length() >= 4) {
                    String firstFourDigits = rawInformation.substring(0, 4);
                    objArr2 = FOUR_DIGIT_DATA_LENGTH;
                    length2 = objArr2.length;
                    int i3 = 0;
                    while (i3 < length2) {
                        Object[] dataLength3 = objArr2[i3];

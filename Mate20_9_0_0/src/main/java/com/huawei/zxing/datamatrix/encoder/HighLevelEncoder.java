...
public final class HighLevelEncoder {
    ...
    public static String encodeHighLevel(String msg, SymbolShapeHint shape, Dimension minSize, Dimension maxSize) {
        encoders = new Encoder[6];
        int encodingMode = 0;
        encoders[0] = new ASCIIEncoder();
        encoders[1] = new C40Encoder();
        encoders[2] = new TextEncoder();
        encoders[3] = new X12Encoder();
        encoders[4] = new EdifactEncoder();
        encoders[5] = new Base256Encoder();
    ...
    static int lookAheadTest(CharSequence msg, int startpos, int currentMode) {
        ...
        while (i + charsProcessed != msg.length()) {
            ...
            if (charsProcessed >= 4) {
                ...
                if (intCharCounts[1] + 1 < intCharCounts[0] && intCharCounts[1] + 1 < intCharCounts[5] && intCharCounts[1] + 1 < intCharCounts[4] && intCharCounts[1] + 1 < intCharCounts[2]) {
                    ...
                    if (intCharCounts[1] == intCharCounts[3]) {
                        for (i2 = (i + charsProcessed) + 1; i2 < msg.length(); i2++) {
                            char tc = charSequence.charAt(i2);
                            if (isX12TermSep(tc)) {
                                return 3;
                            }
                            if (!isNativeX12(tc)) {
        ...
        if (minCount2 == 1 && mins2[5] > (byte) 0) {
            return 5;
        }
        if (minCount2 == 1 && mins2[4] > (byte) 0) {
            return 4;
        }
        if (minCount2 != 1 || mins2[2] <= (byte) 0) {
            return (minCount2 != 1 || mins2[3] <= (byte) 0) ? 1 : 3;
        } else {
            return 2;
        }
    }
    ...
    private static int findMinimums(float[] charCounts, int[] intCharCounts, int min, byte[] mins) {
        ...
        int min2 = min;
        for (min = 0; min < 6; min++) {
            intCharCounts[min] = (int) Math.ceil((double) charCounts[min]);
            int current = intCharCounts[min];
            if (min2 > current) {
                min2 = current;
                Arrays.fill(mins, (byte) 0);
            }
            if (min2 == current) {
                mins[min] = (byte) (mins[min] + 1);
            }
        }
        return min2;
    }
    ...
    static void illegalCharacter(char c) {
        ...
        stringBuilder.append("0000".substring(0, 4 - hex.length()));

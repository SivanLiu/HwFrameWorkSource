...
public class Encoder {
    private static final byte HMAC_DRBG_TYPE_COHORT = (byte) 0;
    private static final byte HMAC_DRBG_TYPE_PRR = (byte) 1;
    ...
    public Encoder(Random random, MessageDigest md5, MessageDigest sha256, byte[] userSecret, String encoderId, int numBits, double probabilityF, double probabilityP, double probabilityQ, int numCohorts, int numBloomHashes) {
        ...
        MessageDigest messageDigest2 = sha256;
        byte[] bArr = userSecret;
        int i = numBits;
        double d = probabilityP;
        double d2 = probabilityQ;
        int i2 = numCohorts;
        ...
        boolean z = probabilityF >= 0.0d && probabilityF <= 1.0d;
        checkArgument(z, "probabilityF must be on range [0.0, 1.0]");
        this.probabilityF = ((double) Math.round(probabilityF * 128.0d)) / 128.0d;
        z = d >= 0.0d && d <= 1.0d;
        checkArgument(z, "probabilityP must be on range [0.0, 1.0]");
        this.probabilityP = d;
        z = d2 >= 0.0d && d2 <= 1.0d;
        checkArgument(z, "probabilityQ must be on range [0.0, 1.0]");
        this.probabilityQ = d2;
        z = i >= 1 && i <= 4096;
        checkArgument(z, "numBits must be on range [1, 4096].");
        this.numBits = i;
        this.inputMask = new BitSet(i);
        this.inputMask.set(0, i, true);
        z = i3 >= 1 && i3 <= i;
        checkArgument(z, "numBloomHashes must be on range [1, numBits).");
        this.numBloomHashes = i3;
        boolean z2 = i2 >= 1 && i2 <= 128;
        checkArgument(z2, "numCohorts must be on range [1, 128].");
        z2 = ((i2 + -1) & i2) == 0;
        checkArgument(z2, "numCohorts must be a power of 2.");
        this.numCohorts = i2;
    ...
    public byte[] encodeOrdinal(int ordinal) {
        boolean z = ordinal >= 0 && ordinal < this.numBits;
        checkArgument(z, "Ordinal value must be in range [0, numBits).");
        BitSet input = new BitSet(this.numBits);
    ...
    public byte[] encodeString(String string) {
        ...
        while (true) {
            int i2 = i;
            if (i2 >= this.numBloomHashes) {
                return encodeBits(input);
            }
            input.set((((digest[i2 * 2] & 255) * 256) + (digest[(i2 * 2) + 1] & 255)) % this.numBits, true);
            i = i2 + 1;
        }
    }

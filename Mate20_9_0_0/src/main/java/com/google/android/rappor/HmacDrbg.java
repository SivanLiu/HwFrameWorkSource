...
public class HmacDrbg {
    private static final byte[] BYTE_ARRAY_0 = new byte[]{(byte) 0};
    private static final byte[] BYTE_ARRAY_1 = new byte[]{(byte) 1};
    ...
    public void nextBytes(byte[] out, int start, int count) {
        if (count != 0) {
            if (this.bytesGenerated + count <= MAX_BYTES_TOTAL) {
                int bytesWritten = 0;
                while (bytesWritten < count) {
                    try {
            ...
            stringBuilder.append("Cannot generate more than a total of ");

...
public abstract class MapActivity extends Activity {
    ...
    private final Handler mNetworkHandler = new Handler() {
        public void handleMessage(Message message) {
            if (MapActivity.this.mNetworkWatcher != null) {
                ...
                stringBuilder.append("Handling network change notification:");
                stringBuilder.append(state.toString());
                Log.i("MapActivity", stringBuilder.toString());
    ...
    protected void onPause() {
        ...
        stringBuilder.append(sActivityReference.get());
    ...
    private void createMap() {
        ...
        if (this.mDataRequestDispatcher != null) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Recycling dispatcher ");
            stringBuilder.append(this.mDataRequestDispatcher);
            Log.w("MapActivity", stringBuilder.toString());
            this.mDataRequestDispatcher.resetConnectionFactory();
        } else {
            String platformId = new StringBuilder("android:");
            platformId.append(SystemProperties.get("ro.product.brand", "unknown").replace('-', '_'));
            platformId.append("-");
            platformId.append(SystemProperties.get("ro.product.device", "unknown").replace('-', '_'));
            platformId.append("-");
            platformId.append(SystemProperties.get("ro.product.model", "unknown").replace('-', '_'));
            platformId = platformId.toString();
            String distChannel = Partner.getString(getContentResolver(), "client_id", "unknown");
            StringBuilder stringBuilder2 = new StringBuilder();
            stringBuilder2.append("gmm-");
            stringBuilder2.append(distChannel);
            this.mDataRequestDispatcher = DataRequestDispatcher.createInstance(getServerUrl(), platformId, "1.6", stringBuilder2.toString(), true);
        }

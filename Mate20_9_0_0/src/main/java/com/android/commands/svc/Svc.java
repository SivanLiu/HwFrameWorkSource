...
public class Svc {
    public static final Command[] COMMANDS = new Command[]{COMMAND_HELP, new PowerCommand(), new DataCommand(), new WifiCommand(), new UsbCommand(), new NfcCommand(), new BluetoothCommand()};
    public static final Command COMMAND_HELP = new Command("help") {
        ...
        public void run(String[] args) {
            if (args.length == 2) {
                Command c = Svc.lookupCommand(args[1]);
                if (c != null) {
            ...
            format.append("    %-");
            format.append(maxlen);
            format.append("s    %s");
            format = format.toString();
            for (Command c3 : Svc.COMMANDS) {
                System.err.println(String.format(format, new Object[]{c3.name(), c3.shortHelp()}));
            }
        }
    };

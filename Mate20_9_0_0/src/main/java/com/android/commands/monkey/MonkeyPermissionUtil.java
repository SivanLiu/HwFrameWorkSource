...
public class MonkeyPermissionUtil {
    ...
    private boolean shouldTargetPackage(PackageInfo info) {
        ...
        if (!this.mTargetSystemPackages || MonkeyUtils.getPackageFilter().isPackageInvalid(info.packageName) || (info.applicationInfo.flags & 1) == 0) {
            return false;
        }
        return true;
    }
    ...
    private boolean shouldTargetPermission(String pkg, PermissionInfo pi) throws RemoteException {
        ...
        if (pi.group != null && pi.protectionLevel == 1 && (flags & 20) == 0 && isModernPermissionGroup(pi.group)) {
            return true;
        }
        return false;
    }
    ...
    public boolean populatePermissionsMapping() {
        ...
        try {
            Iterator it = this.mPm.getInstalledPackages(4096, UserHandle.myUserId()).getList().iterator();
            while (it.hasNext()) {
                PackageInfo info = (PackageInfo) it.next();
                if (shouldTargetPackage(info)) {
                    List<PermissionInfo> permissions = new ArrayList();
                    if (info.applicationInfo.targetSdkVersion > 22) {

...
public class MonkeySourceRandomScript implements MonkeyEventSource {
    ...
    public MonkeySourceRandomScript(String setupFileName, ArrayList<String> scriptFileNames, long throttle, boolean randomizeThrottle, Random random, long profileWaitTime, long deviceSleepTime, boolean randomizeScript) {
        ...
        Iterator it = scriptFileNames.iterator();
        while (it.hasNext()) {
            this.mScriptSources.add(new MonkeySourceScript(random, (String) it.next(), throttle, randomizeThrottle, profileWaitTime, deviceSleepTime));
        }
        this.mRandom = random;
    ...
    public void setVerbose(int verbose) {
        ...
        Iterator it = this.mScriptSources.iterator();
        while (it.hasNext()) {
            ((MonkeySourceScript) it.next()).setVerbose(verbose);
        }
    }
    ...
    public boolean validate() {
        ...
        Iterator it = this.mScriptSources.iterator();
        while (it.hasNext()) {
            if (!((MonkeySourceScript) it.next()).validate()) {
                return false;
            }
        }
        return true;

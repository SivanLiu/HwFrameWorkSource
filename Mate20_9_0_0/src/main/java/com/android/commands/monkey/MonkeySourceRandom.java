...
public class MonkeySourceRandom implements MonkeyEventSource {
    ...
    static {
        int i = 0;
        for (int i2 = 0; i2 < PHYSICAL_KEY_EXISTS.length; i2++) {
            PHYSICAL_KEY_EXISTS[i2] = true;
        }
        while (i < SYS_KEYS.length) {
        ...
        if (userSum > 100.0f) {
            ...
            for (int i3 = 0; i3 < 12; i3++) {
                if (this.mFactors[i3] <= 0.0f) {
                    ...
                    fArr[i3] = fArr[i3] * defaultsAdjustment;
                }
            }
            if (this.mVerbose > 0) {
                ...
                for (int i4 = 0; i4 < 12; i4++) {
                    ...
                    stringBuilder.append(": ");
                    stringBuilder.append(this.mFactors[i4]);
                    stringBuilder.append("%");
    ...
    private static boolean validateKeyCategory(String catName, int[] keys, float factor) {
        ...
        stringBuilder.append(" has no physical keys but with factor ");
    ...
    private void generatePointerEvent(Random random, int gesture) {
        ...
        int count;
        if (i2 == 1) {
            count = random2.nextInt(10);
            int i5 = 0;
            while (i5 < count) {
                randomWalk(random2, display, p1, v1);
                this.mQ.addLast(new MonkeyTouchEvent(i4).setDownTime(downAt).addPointer(0, p1.x, p1.y).setIntermediateNote(true));
                i5++;
                i4 = 2;
            }
        } else if (i2 == 2) {
            ...
            while (i6 < count) {
                ...
                this.mQ.addLast(new MonkeyTouchEvent(2).setDownTime(downAt).addPointer(i3, p1.x, p1.y).addPointer(1, p2.x, p2.y).setIntermediateNote(true));
                i6++;
                i2 = gesture;
                i3 = 0;
            }
            ...
            randomWalk(random2, display, p1, v1);
            this.mQ.addLast(new MonkeyTouchEvent(i).setDownTime(downAt).addPointer(0, p1.x, p1.y).setIntermediateNote(false));
        }
        i = 1;
        randomWalk(random2, display, p1, v1);
        this.mQ.addLast(new MonkeyTouchEvent(i).setDownTime(downAt).addPointer(0, p1.x, p1.y).setIntermediateNote(false));

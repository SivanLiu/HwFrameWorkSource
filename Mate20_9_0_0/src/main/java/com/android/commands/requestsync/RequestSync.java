...
public class RequestSync {
    ...
    enum Operation {
        REQUEST_SYNC {
            void invoke(RequestSync caller) {
                ...
                ContentResolver.requestSync(new Builder().setSyncAdapter(caller.mAccount, caller.mAuthority).setExtras(caller.mExtras).syncOnce().build());
            }
        },
        ADD_PERIODIC_SYNC {
            void invoke(RequestSync caller) {
        ...
        if (parseArgs()) {
            ...
            if (this.mExtras.size() > 0) {
                ...
                for (String str2 : this.mExtras.keySet()) {
                    System.out.printf("    %s: %s\n", new Object[]{str2, this.mExtras.get(str2)});
                }
            }
    ...
    private String nextOption() {
        ...
        if (this.mCurArgData != null) {
            prev = this.mArgs[this.mNextArg - 1];
            StringBuilder stringBuilder = new StringBuilder();

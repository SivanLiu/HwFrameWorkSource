...
public class RunTestCommand extends Command {
        ...
        while (i < args.length) {
            if (args[i].equals("-e")) {
                ...
                if (CLASS_PARAM.equals(key)) {
                    ...
                    boolean z = "true".equals(value) || "1".equals(value);
                    this.mDebug = z;
    ...
    protected UiAutomatorTestRunner getRunner() {
        ...
        UiAutomatorTestRunner o = null;
        try {
            o = Class.forName(this.mRunnerClassName).newInstance();
        } catch (ClassNotFoundException e) {
            printStream = System.err;
            stringBuilder = new StringBuilder();
            stringBuilder.append("Cannot find runner: ");
            stringBuilder.append(this.mRunnerClassName);
            printStream.println(stringBuilder.toString());
            System.exit(ARG_FAIL_RUNNER);
        } catch (InstantiationException e2) {
            printStream = System.err;
            stringBuilder = new StringBuilder();
            stringBuilder.append("Cannot instantiate runner: ");
            stringBuilder.append(this.mRunnerClassName);
            printStream.println(stringBuilder.toString());
            System.exit(ARG_FAIL_RUNNER);
            ...
            printStream = System.err;
            stringBuilder = new StringBuilder();
            stringBuilder.append("Constructor of runner ");
            stringBuilder.append(this.mRunnerClassName);
            stringBuilder.append(" is not accessibile");
            printStream.println(stringBuilder.toString());
            System.exit(ARG_FAIL_RUNNER);
        }
        try {
            UiAutomatorTestRunner runner = o;
            this.mRunner = runner;
            return runner;
        } catch (ClassCastException e4) {
            printStream = System.err;
            stringBuilder = new StringBuilder();
            stringBuilder.append("Specified runner is not subclass of ");
            stringBuilder.append(UiAutomatorTestRunner.class.getSimpleName());
            printStream.println(stringBuilder.toString());
            System.exit(ARG_FAIL_RUNNER);
            return null;
        }

...
public class RC4Engine implements StreamCipher {
    ...
    private void setKey(byte[] keyBytes) {
        ...
        while (i2 < STATE_LENGTH) {
            i22 = (((keyBytes[i] & 255) + this.engineState[i2]) + i22) & 255;
            byte tmp = this.engineState[i2];
            this.engineState[i2] = this.engineState[i22];
            this.engineState[i22] = tmp;
            i = (i + 1) % keyBytes.length;
            i2++;
        }
    }
}

    ...
    public int calculateHashCode(X500Name name) {
        ...
        for (int i = 0; i != rdns.length; i++) {
            if (rdns[i].isMultiValued()) {
                ...
                for (hashCodeValue = 0; hashCodeValue != atv.length; hashCodeValue++) {
                    hashCodeValue2 = (hashCodeValue2 ^ atv[hashCodeValue].getType().hashCode()) ^ calcHashCode(atv[hashCodeValue].getValue());
                }

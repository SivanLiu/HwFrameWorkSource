...
public abstract class WNafUtil {
        ...
        if ((k.bitLength() >>> 16) != 0) {
            ...
            while (i < highBit) {
                if (diff.testBit(i)) {
                    ...
                    naf[length] = ((k.testBit(i) ? -1 : 1) << 16) | zeroes;
                    i++;
                    zeroes = 1;
                    length = length2;
                } else {
    ...
    public static ECPoint mapPointWithPrecomp(ECPoint p, int width, boolean includeNegated, ECPointMap pointMap) {
        ...
        int i = 0;
        for (int i2 = 0; i2 < preCompP.length; i2++) {
            preCompQ[i2] = pointMap.map(preCompP[i2]);
        }
        wnafPreCompQ.setPreComp(preCompQ);
        if (includeNegated) {
            ECPoint[] preCompNegQ = new ECPoint[preCompQ.length];
            while (i < preCompNegQ.length) {
            ...
            if (reqPreCompLen == 1) {
                ...
                if (reqPreCompLen == 2) {
                    ...
                    while (curPreCompLen < reqPreCompLen) {
                        int curPreCompLen2 = curPreCompLen + 1;

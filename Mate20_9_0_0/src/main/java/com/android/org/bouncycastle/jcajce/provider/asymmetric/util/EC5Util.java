...
public class EC5Util {
    ...
    public static ECCurve getCurve(ProviderConfiguration configuration, X962Parameters params) {
        ...
        if (params.isNamedCurve()) {
            ...
            if (acceptableCurves.isEmpty() || acceptableCurves.contains(oid)) {
                curve = ECUtil.getNamedCurveByOid(oid);
                if (curve == null) {
                    curve = (X9ECParameters) configuration.getAdditionalECParameters().get(oid);
                }
    ...
    public static ECParameterSpec convertToSpec(X962Parameters params, ECCurve curve) {
        ...
        } else if (params.isImplicitlyCA() != null) {
            return null;
        } else {
            ECParameterSpec eCParameterSpec;
            ECParameterSpec ecSpec = X9ECParameters.getInstance(params.getParameters());
            EllipticCurve ellipticCurve = convertCurve(curve, ecSpec.getSeed());
            if (ecSpec.getH() != null) {
                eCParameterSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecSpec.getG().getAffineXCoord().toBigInteger(), ecSpec.getG().getAffineYCoord().toBigInteger()), ecSpec.getN(), ecSpec.getH().intValue());
            } else {
                eCParameterSpec = new ECParameterSpec(ellipticCurve, new ECPoint(ecSpec.getG().getAffineXCoord().toBigInteger(), ecSpec.getG().getAffineYCoord().toBigInteger()), ecSpec.getN(), 1);
            }

...
public class PKCS10CertificationRequest extends CertificationRequest {
        ...
        if (subject == null) {
            ...
            try {
                ...
                try {
                    ...
                    stringBuilder.append("exception encoding TBS cert request - ");
    ...
    public boolean verify(PublicKey pubKey, String provider) throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeyException, SignatureException {
        ...
        if (provider == null) {
            try {
                e = Signature.getInstance(getSignatureName(this.sigAlgId));
        ...
        e2 = Signature.getInstance(getSignatureName(this.sigAlgId), provider);
        setSignatureParameters(e2, this.sigAlgId.getParameters());
        e2.initVerify(pubKey);
        try {
            e2.update(this.reqInfo.getEncoded(ASN1Encoding.DER));
            return e2.verify(this.sigBits.getOctets());
        } catch (Exception e3) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("exception encoding TBS cert request - ");
    ...
    static String getSignatureName(AlgorithmIdentifier sigAlgId) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(getDigestAlgName(rsaParams.getHashAlgorithm().getAlgorithm()));

    ...
    public static class Fp extends ECFieldElement {
        ...
        public ECFieldElement sqrt() {
            ...
            if (!this.q.testBit(0)) {
                ...
                t1 = this.q.shiftRight(1);
                if (!this.x.modPow(t1, this.q).equals(ECConstants.ONE)) {
                    return null;
                }
                BigInteger X = this.x;
                BigInteger fourX = modDouble(modDouble(X));
                BigInteger k = t1.add(ECConstants.ONE);
                BigInteger qMinusOne = this.q.subtract(ECConstants.ONE);
                Random rand = new Random();
                while (true) {
                    BigInteger P = new BigInteger(this.q.bitLength(), rand);
                    if (P.compareTo(this.q) < 0 && modReduce(P.multiply(P).subtract(fourX)).modPow(t1, this.q).equals(qMinusOne)) {
        ...
        private BigInteger[] lucasSequence(BigInteger P, BigInteger Q, BigInteger k) {
            ...
            for (int j2 = 1; j2 <= s; j2++) {
                Vl = modMult(Vl, Ql);
                Ql = modReduce(Ql.multiply(Ql).subtract(Ql2.shiftLeft(1)));
                Ql2 = modMult(Ql2, Ql2);
            }
            return new BigInteger[]{Vl, Ql};
        }

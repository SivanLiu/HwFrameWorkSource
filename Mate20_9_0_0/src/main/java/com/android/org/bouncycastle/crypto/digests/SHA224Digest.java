...
public class SHA224Digest extends GeneralDigest implements EncodableDigest {
    ...
    protected void processBlock() {
        ...
        b = t;
        for (t = 0; t < 8; t++) {
            h += ((Sum1(e) + Ch(e, f, g)) + K[t2]) + this.X[t2];
            d += h;
            h += Sum0(b) + Maj(b, c, c2);
            t2++;
            g += ((Sum1(d) + Ch(d, e, f)) + K[t2]) + this.X[t2];
            c2 += g;
            g += Sum0(h) + Maj(h, b, c);
            t2++;
            f += ((Sum1(c2) + Ch(c2, d, e)) + K[t2]) + this.X[t2];
            c += f;
            f += Sum0(g) + Maj(g, h, b);
            t2++;
            e += ((Sum1(c) + Ch(c, c2, d)) + K[t2]) + this.X[t2];
            b += e;
            e += Sum0(f) + Maj(f, g, h);
            t2++;
            d += ((Sum1(b) + Ch(b, c, c2)) + K[t2]) + this.X[t2];
            h += d;
            d += Sum0(e) + Maj(e, f, g);
            t2++;
            c2 += ((Sum1(h) + Ch(h, b, c)) + K[t2]) + this.X[t2];
            g += c2;
            c2 += Sum0(d) + Maj(d, e, f);
            t2++;
            c += ((Sum1(g) + Ch(g, h, b)) + K[t2]) + this.X[t2];
            f += c;
            c += Sum0(c2) + Maj(c2, d, e);
            t2++;
            b += ((Sum1(f) + Ch(f, g, h)) + K[t2]) + this.X[t2];
            e += b;
            b += Sum0(c) + Maj(c, c2, d);
            t2++;
        }

...
public class DESEngine implements BlockCipher {
    ...
    protected int[] generateWorkingKey(boolean encrypting, byte[] key) {
        ...
        for (j = 0; j < 16; j++) {
            ...
            for (j3 = 0; j3 < 24; j3++) {
                if (pcr[pc2[j3]]) {
                    newKey[l] = newKey[l] | bigbyte[j3];
                }
                if (pcr[pc2[j3 + 24]]) {
                    newKey[n] = newKey[n] | bigbyte[j3];
                }
            }
        }
        while (true) {
            ...
            i = newKey[i2];
            j = newKey[i2 + 1];
            newKey[i2] = (((16515072 & j) >>> 10) | (((i & 16515072) << 6) | ((i & 4032) << 10))) | ((j & 4032) >>> 6);
            newKey[i2 + 1] = (((258048 & j) >>> 4) | (((i & 258048) << 12) | ((i & 63) << 16))) | (j & 63);
            i = i2 + 2;
        }
    }
    ...
    protected void desFunc(int[] wKey, byte[] in, int inOff, byte[] out, int outOff) {
        ...
        left ^= work;
        right ^= work;
        int left2 = ((left << 1) | ((left >>> 31) & 1)) & -1;
        for (left = 0; left < 8; left++) {
            work = ((right << 28) | (right >>> 4)) ^ wKey[(left * 4) + 0];
            int fval = ((SP7[work & 63] | SP5[(work >>> 8) & 63]) | SP3[(work >>> 16) & 63]) | SP1[(work >>> 24) & 63];
            work = right ^ wKey[(left * 4) + 1];
            left2 ^= (((fval | SP8[work & 63]) | SP6[(work >>> 8) & 63]) | SP4[(work >>> 16) & 63]) | SP2[(work >>> 24) & 63];
            work = ((left2 << 28) | (left2 >>> 4)) ^ wKey[(left * 4) + 2];
            fval = ((SP7[work & 63] | SP5[(work >>> 8) & 63]) | SP3[(work >>> 16) & 63]) | SP1[(work >>> 24) & 63];
            work = left2 ^ wKey[(left * 4) + 3];
            right ^= (((fval | SP8[work & 63]) | SP6[(work >>> 8) & 63]) | SP4[(work >>> 16) & 63]) | SP2[(work >>> 24) & 63];
        }
        left = (right << 31) | (right >>> 1);
        right = (left2 ^ left) & -1431655766;
        int left3 = left2 ^ right;
        left ^= right;
        left3 = (left3 << 31) | (left3 >>> 1);
        right = ((left3 >>> 8) ^ left) & 16711935;
        left ^= right;
        left3 ^= right << 8;
        right = ((left3 >>> 2) ^ left) & 858993459;
        left ^= right;
        left3 ^= right << 2;
        right = ((left >>> 16) ^ left3) & 65535;
        left3 ^= right;
        left ^= right << 16;
        right = ((left >>> 4) ^ left3) & 252645135;
        left3 ^= right;
        left ^= right << 4;
        out[outOff + 0] = (byte) ((left >>> 24) & 255);
        out[outOff + 1] = (byte) ((left >>> 16) & 255);
        out[outOff + 2] = (byte) ((left >>> 8) & 255);
        out[outOff + 3] = (byte) (left & 255);
        out[outOff + 4] = (byte) ((left3 >>> 24) & 255);
        out[outOff + 5] = (byte) ((left3 >>> 16) & 255);
        out[outOff + 6] = (byte) ((left3 >>> 8) & 255);
        out[outOff + 7] = (byte) (left3 & 255);
    }
}

...
public class TBSCertList extends ASN1Object {
    ...
    public TBSCertList(ASN1Sequence seq) {
        ...
        if (seqPos2 < seq.size() && !(seq.getObjectAt(seqPos2) instanceof ASN1TaggedObject)) {
            seqPos = seqPos2 + 1;
            this.revokedCertificates = ASN1Sequence.getInstance(seq.getObjectAt(seqPos2));
            seqPos2 = seqPos;
        }
        if (seqPos2 < seq.size() && (seq.getObjectAt(seqPos2) instanceof ASN1TaggedObject)) {
            this.crlExtensions = Extensions.getInstance(ASN1Sequence.getInstance((ASN1TaggedObject) seq.getObjectAt(seqPos2), true));
        }
    }

...
public class IETFUtils {
    ...
    public static RDN[] rDNsFromString(String name, X500NameStyle x500Style) {
        ...
        while (nTok.hasMoreTokens()) {
            ...
            pTok = new X500NameTokenizer(token, '=');
            String attr2 = pTok.nextToken();
            if (pTok.hasMoreTokens()) {
                builder.addRDN(x500Style.attrNameToOID(attr2.trim()), unescape(pTok.nextToken()));
            } else {
                throw new IllegalArgumentException("badly formatted directory string");
            }
        }
    ...
    public static String valueToString(ASN1Encodable value) {
        ...
        if (!(value instanceof ASN1String) || (value instanceof DERUniversalString)) {
            try {
                ...
                vBuf.append(stringBuilder.toString());
            } catch (IOException e) {
        ...
        if (v.length() <= 0 || v.charAt(0) != '#') {
            ...
            vBuf.append(stringBuilder2.toString());
        }
        int end = vBuf.length();
        int index = 0;
    ...
    public static String canonicalize(String s) {
        ...
        if (value.length() > 1) {
            ...
            while (true) {
                start2 = start;
                if (start2 + 1 < value.length() && value.charAt(start2) == '\\' && value.charAt(start2 + 1) == ' ') {
                    start = start2 + 2;
                } else {
            ...
            start = value.length() - 1;
            while (start - 1 > 0 && value.charAt(start - 1) == '\\' && value.charAt(start) == ' ') {
                start -= 2;
            }
            if (start2 > 0 || start < value.length() - 1) {
                value = value.substring(start2, start + 1);
            }
        }

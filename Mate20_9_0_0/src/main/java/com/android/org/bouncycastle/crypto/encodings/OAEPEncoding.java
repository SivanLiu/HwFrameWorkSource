...
public class OAEPEncoding implements AsymmetricBlockCipher {
    ...
    public byte[] decodeBlock(byte[] in, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        for (int index = 2 * this.defHash.length; index != block.length; index++) {
            if (((block[index] != (byte) 0 ? 1 : 0) & (i == block.length ? 1 : 0)) != 0) {
                i = index;
            }
        }
        boolean dataStartWrong = i > block.length - 1;
        int i2 = block[i] != (byte) 1 ? 1 : 0;
        i++;
        if (((defHashWrong | shortData) | (dataStartWrong | i2)) == 0) {
            byte[] output = new byte[(block.length - i)];
            System.arraycopy(block, i, output, 0, output.length);
            return output;
        }

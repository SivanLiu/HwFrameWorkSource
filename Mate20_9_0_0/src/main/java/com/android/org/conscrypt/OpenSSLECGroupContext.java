...
final class OpenSSLECGroupContext {
    ...
    OpenSSLECGroupContext(EC_GROUP groupCtx) {
    ...
    EC_GROUP getNativeRef() {
        return this.groupCtx;
    }

    static OpenSSLECGroupContext getInstance(ECParameterSpec params) throws InvalidAlgorithmParameterException {
        ...
        if (field instanceof ECFieldFp) {
            ...
            if (bitLength != 224) {
                if (bitLength != PSKKeyManager.MAX_KEY_LENGTH_BYTES) {
                    if (bitLength != 384) {
            ...
            String curveName2 = curveName;
            if (curveName2 != null) {
                return getCurveByName(curveName2);
            }
            ...
            } catch (Throwable curveName3) {
                InvalidAlgorithmParameterException invalidAlgorithmParameterException = new InvalidAlgorithmParameterException("EC_GROUP_new_arbitrary failed", curveName3);
            }
        } else {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("unhandled field class ");
            stringBuilder.append(field.getClass().getName());

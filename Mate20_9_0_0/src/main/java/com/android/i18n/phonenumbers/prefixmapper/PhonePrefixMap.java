...
public class PhonePrefixMap implements Externalizable {
    ...
    PhonePrefixMapStorageStrategy getSmallerMapStorage(SortedMap<Integer, String> phonePrefixMap) {
        try {
            ...
            return sizeOfFlyweightMapStorage < getSizeOfPhonePrefixMapStorage(defaultMapStorage, phonePrefixMap) ? flyweightMapStorage : defaultMapStorage;
        } catch (IOException e) {
    ...
    public String lookup(PhoneNumber number) {
        ...
        stringBuilder.append(number.getCountryCode());
        stringBuilder.append(this.phoneUtil.getNationalSignificantNumber(number));
        return lookup(Long.parseLong(stringBuilder.toString()));
    }

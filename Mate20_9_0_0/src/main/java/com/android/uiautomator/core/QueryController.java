                    ...
                    if (eventType != 32) {
                        if (eventType == 131072) {
                            ...
                            if (QueryController.DEBUG) {
                                ...
                                stringBuilder.append("Last text selection reported: ");
                                stringBuilder.append(QueryController.this.mLastTraversedText);
    ...
    private AccessibilityNodeInfo translateCompoundSelector(UiSelector selector, AccessibilityNodeInfo fromNode, boolean isCounting) {
        ...
        if (fromNode == null) {
            if (DEBUG) {
                ...
                stringBuilder.append("Container selector not found: ");
                stringBuilder.append(selector.dumpToString(false));
                Log.d(str, stringBuilder.toString());
            }
            return null;
        }
        if (selector.hasPatternSelector()) {
            ...
            if (isCounting) {
                ...
                if (DEBUG) {
                    str = LOG_TAG;
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Pattern selector not found: ");
                    stringBuilder.append(selector.dumpToString(false));
                    Log.d(str, stringBuilder.toString());
                }
                return null;
        ...
        if (fromNode == null) {
            if (DEBUG) {
                ...
                stringBuilder2.append("Object Not Found for selector ");
                stringBuilder2.append(selector);

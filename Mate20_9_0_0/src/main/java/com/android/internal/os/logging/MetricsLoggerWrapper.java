...
public class MetricsLoggerWrapper {
    ...
    public static void logPictureInPictureDismissByTap(Context context, Pair<ComponentName, Integer> topActivityInfo) {
        MetricsLogger.action(context, (int) MetricsEvent.ACTION_PICTURE_IN_PICTURE_DISMISSED, 0);
        StatsLog.write(52, getUid(context, (ComponentName) topActivityInfo.first, ((Integer) topActivityInfo.second).intValue()), ((ComponentName) topActivityInfo.first).flattenToString(), 4);
    }

    public static void logPictureInPictureDismissByDrag(Context context, Pair<ComponentName, Integer> topActivityInfo) {
        MetricsLogger.action(context, (int) MetricsEvent.ACTION_PICTURE_IN_PICTURE_DISMISSED, 1);
        StatsLog.write(52, getUid(context, (ComponentName) topActivityInfo.first, ((Integer) topActivityInfo.second).intValue()), ((ComponentName) topActivityInfo.first).flattenToString(), 4);
    }
    ...
    public static void logPictureInPictureMinimize(Context context, boolean isMinimized, Pair<ComponentName, Integer> topActivityInfo) {
        MetricsLogger.action(context, (int) MetricsEvent.ACTION_PICTURE_IN_PICTURE_MINIMIZED, isMinimized);
        StatsLog.write(52, getUid(context, (ComponentName) topActivityInfo.first, ((Integer) topActivityInfo.second).intValue()), ((ComponentName) topActivityInfo.first).flattenToString(), 3);
    }
    ...
    public static void logPictureInPictureMenuVisible(Context context, boolean menuStateFull) {
        MetricsLogger.visibility(context, (int) MetricsEvent.ACTION_PICTURE_IN_PICTURE_MENU, menuStateFull);
    }
    ...
    public static void logAppOverlayEnter(int uid, String packageName, boolean changed, int type, boolean usingAlertWindow) {
        ...
        if (type != AbstractRILConstants.RIL_REQUEST_HW_VSIM_GET_SIM_STATE) {

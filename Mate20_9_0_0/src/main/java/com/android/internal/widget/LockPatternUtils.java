...
public class LockPatternUtils {
    ...
    private byte[] verifyCredential(String credential, int type, long challenge, int userId) throws RequestThrottledException {
        try {
            ...
            stringBuilder.append("verifyCredential userId:");
    ...
    public void clearLock(String savedCredential, int userHandle) {
        ...
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("clearLock success by uid = ");
    ...
    public void saveLockPattern(List<Cell> pattern, String savedPattern, int userId) {
        ...
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("saveLockPattern success by uid = ");
            stringBuilder.append(Binder.getCallingUid());
            Log.i(str, stringBuilder.toString());
        } catch (Exception e) {
    ...
    private void updateEncryptionPasswordIfNeeded(String password, int quality, int userHandle) {
        if (userHandle == 0 && isDeviceEncryptionEnabled()) {
            ...
            if (shouldEncryptWithCredentials(true)) {
                int type = 0;
                boolean numeric = quality == Protocol.BASE_WIFI;
                if (quality != Protocol.BASE_DHCP) {
    ...
    public String legacyPasswordToHash(String password, int userId) {
        ...
        try {
            ...
            stringBuilder.append(getSalt(userId));
            byte[] saltedPassword = stringBuilder.toString().getBytes();
    ...
    private String passwordToHistoryHash(String passwordToHash, byte[] hashFactor, int userId) {
        ...
        try {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(passwordToHash);
            stringBuilder.append(getSalt(userId));
            sha256.update(stringBuilder.toString().getBytes());
    ...
    private boolean isLockPasswordEnabled(int mode, int userId) {
        boolean passwordEnabled = mode == 262144 || mode == Protocol.BASE_WIFI || mode == Protocol.BASE_DHCP || mode == Protocol.BASE_TETHERING || mode == Protocol.BASE_NSD_MANAGER || mode == 524288;
    ...
    private boolean isLockPatternEnabled(int mode, int userId) {
        return mode == Protocol.BASE_SYSTEM_RESERVED && savedPatternExists(userId);
    }
    ...
    public void setCredentialRequiredToDecrypt(boolean required) {
        if (!getUserManager().isSystemUser() && !getUserManager().isPrimaryUser()) {
            ...
            Global.putInt(this.mContext.getContentResolver(), "require_password_to_decrypt", required);
        }
    }
    ...
    public boolean setLockCredentialWithToken(String credential, int type, int requestedQuality, long tokenHandle, byte[] token, int userId) {
        ...
        if (i == -1) {
            ...
            } else if (!localService.setLockCredentialWithToken(null, -1, tokenHandle, token, 0, i2)) {
                return false;
            } else {
            ...
            int quality = computePasswordQuality(i, str, requestedQuality);
            int quality2 = quality;
            if (!localService.setLockCredentialWithToken(str, i, tokenHandle, token, quality, i2)) {
                return false;
            }
            setKeyguardStoredPasswordQuality(quality2, i2);
            updateEncryptionPasswordIfNeeded(str, quality2, i2);
    ...
    public void monitorCheckPassword(int errorType, Exception e) {
        ...
        try {
            ...
            stringBuilder2.append(KEY_PREFIX);
            stringBuilder2.append(errorType);
            if (currentTime > ONE_DAY + sp.getLong(stringBuilder2.toString(), 0)) {
                Bundle bundle = new Bundle();
                bundle.putInt("errorType", errorType);
                bundle.putSerializable("reason", e);
                mMonitor.monitor(907034001, bundle);
                Editor editor = this.mContext.getSharedPreferences(SPF_XML_NAME, 0).edit();
                StringBuilder stringBuilder3 = new StringBuilder();
                stringBuilder3.append(KEY_PREFIX);
                stringBuilder3.append(errorType);
                editor.putLong(stringBuilder3.toString(), currentTime).apply();
            } else {
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("monitorCheckPassword: ");
                stringBuilder.append(errorType);
                stringBuilder.append(" already report, ignore it!");
            ...
            stringBuilder.append("monitorCheckPassword: ");
            stringBuilder.append(errorType);
            stringBuilder.append(" monitorCheckPassword RequestThrottledException!");
            Log.w(str, stringBuilder.toString());
        }
    }
}

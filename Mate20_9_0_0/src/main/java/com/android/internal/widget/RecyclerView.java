...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
     ...
     */
        ...
        public void onInitializeAccessibilityNodeInfo(Recycler recycler, State state, AccessibilityNodeInfo info) {
            if (this.mRecyclerView.canScrollVertically(-1) || this.mRecyclerView.canScrollHorizontally(-1)) {
                info.addAction(SmsCbConstants.SERIAL_NUMBER_ETWS_EMERGENCY_USER_ALERT);
        ...
        public boolean performAccessibilityAction(Recycler recycler, State state, int action, Bundle args) {
            ...
            } else if (action == SmsCbConstants.SERIAL_NUMBER_ETWS_EMERGENCY_USER_ALERT) {
                if (this.mRecyclerView.canScrollVertically(-1)) {
                    vScroll = -((getHeight() - getPaddingTop()) - getPaddingBottom());
                }
                if (this.mRecyclerView.canScrollHorizontally(-1)) {
    ...
    public final class Recycler {
        ...
        ViewHolder tryGetViewHolderForPositionByDeadline(int position, boolean dryRun, long deadlineNs) {
            ...
            if (fromScrapOrHiddenOrCache2 && !RecyclerView.this.mState.isPreLayout() && holder2.hasAnyOfTheFlags(SmsCbConstants.SERIAL_NUMBER_ETWS_EMERGENCY_USER_ALERT)) {
                holder2.setFlags(0, SmsCbConstants.SERIAL_NUMBER_ETWS_EMERGENCY_USER_ALERT);
    ...
    class ViewFlinger implements Runnable {
        ...
        public void smoothScrollBy(int dx, int dy, Interpolator interpolator) {
            ...
            smoothScrollBy(dx, dy, computeScrollDuration, interpolator2);
        }
                ...
                case 2:
                    ...
                    if (index >= 0) {
                        ...
                        if (this.mScrollState == i) {
                            ...
                            this.mLastTouchY = y - this.mScrollOffset[i];
                            if (scrollByInternal(canScrollHorizontally ? x : 0, canScrollVertically ? dy : 0, vtev)) {
                                getParent().requestDisallowInterceptTouchEvent(true);
                            }
                            if (!(this.mGapWorker == null || (x == 0 && dy == 0))) {
                                this.mGapWorker.postFromTraversal(this, x, dy);
                                break;
                            }
                        }
                    }
    ...
    private void recoverFocusFromState() {
        if (this.mPreserveFocusAfterLayout && this.mAdapter != null && hasFocus() && getDescendantFocusability() != Protocol.BASE_NSD_MANAGER && (getDescendantFocusability() != Protocol.BASE_WIFI || !isFocused())) {
            if (!isFocused()) {
    ...
    void recordAnimationInfoIfBouncedHiddenView(ViewHolder viewHolder, ItemHolderInfo animationInfo) {
        viewHolder.setFlags(0, SmsCbConstants.SERIAL_NUMBER_ETWS_EMERGENCY_USER_ALERT);

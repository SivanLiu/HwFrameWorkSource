...
public class ToolbarWidgetWrapper implements DecorToolbar {
    ...
    public void animateToVisibility(int visibility) {
        Animator anim = setupAnimatorToVisibility(visibility, 200);
        if (anim != null) {
            anim.start();
        }
    }

    /* JADX WARNING: Incorrect type for fill-array insn 0x000b, element type: float, insn element type: null */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public Animator setupAnimatorToVisibility(int visibility, long duration) {
        ObjectAnimator anim;
        if (visibility == 8) {
            anim = ObjectAnimator.ofFloat(this.mToolbar, View.ALPHA, new float[]{1.0f, 0.0f});
            ...
            anim = ObjectAnimator.ofFloat(this.mToolbar, View.ALPHA, new float[]{0.0f, 1.0f});
            anim.setDuration(duration);
            anim.addListener(new AnimatorListenerAdapter() {

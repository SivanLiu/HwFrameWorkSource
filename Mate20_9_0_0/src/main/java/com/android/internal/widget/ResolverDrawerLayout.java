...
public class ResolverDrawerLayout extends ViewGroup {
    ...
    private boolean updateCollapseOffset(int oldCollapsibleHeight, boolean remainClosed) {
        ...
        if (isLaidOut()) {
            ...
            if (this.mCollapseOffset != 0) {
                isCollapsedNew2 = true;
            }
            if (isCollapsedOld != isCollapsedNew2) {
                onCollapsedChanged(isCollapsedNew2);
            }
        } else {
        ...
        if (action != 6) {
            ...
            switch (action) {
                case 0:
                    ...
                    boolean z = isListChildUnderClipped(x, y) && this.mCollapseOffset > 0;
                    this.mOpenOnClick = z;
                    break;
                case 1:
                case 3:
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case 2:
                ...
                y = ev.getY(index);
                if (!this.mIsDragging) {
                    yvel = y - this.mInitialTouchY;
                    if (Math.abs(yvel) > ((float) this.mTouchSlop) && findChildUnder(y2, y) != null) {
                        this.mIsDragging = true;
                        handled = true;
                        this.mLastTouchY = Math.max(this.mLastTouchY - ((float) this.mTouchSlop), Math.min(this.mLastTouchY + yvel, this.mLastTouchY + ((float) this.mTouchSlop)));
                    }
                }
                if (this.mIsDragging) {
            ...
            case 5:
                ...
                this.mInitialTouchX = ev.getX(index);
                y2 = ev.getY(index);
                this.mLastTouchY = y2;
                this.mInitialTouchY = y2;
                break;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        while (true) {
            ...
            child = getChildAt(i);
            lp = (LayoutParams) child.getLayoutParams();
            if (lp.alwaysShow || child.getVisibility() == i2) {
                i3 = i2;
                i4 = i;
                ...
                i4 = i;
                measureChildWithMargins(child, widthSpec, widthPadding, heightSpec, heightUsed);
                heightUsed += widthSize2.getMeasuredHeight();
            }
            widthSize = i4 + 1;
            i2 = i3;
            widthSize2 = widthSize3;
        }
        widthSize = this.mCollapsibleHeight;
        ...
        updateCollapseOffset(widthSize, isDragging() ^ 1);
        if (getShowAtTop()) {
            this.mTopOffset = 0;
        } else {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        while (i < childCount) {
            ...
            if (child.getVisibility() == 8) {
                ...
                int ypos2 = child.getMeasuredHeight() + top;
                int childWidth = child.getMeasuredWidth();
                int left = (((rightEdge - leftEdge) - childWidth) / 2) + leftEdge;
                child.layout(left, top, left + childWidth, ypos2);
                width2 = width;
                ypos = ypos2 + lp.bottomMargin;
            }
            i++;
            width = width2;
        }
        ...
        if (indicatorHost != null) {
            ...
            this.mScrollIndicatorDrawable.setBounds(i, bottom - this.mScrollIndicatorDrawable.getIntrinsicHeight(), right, bottom);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        boolean z = this.mCollapsibleHeight > 0 && this.mCollapseOffset == 0;
        ss.open = z;
        return ss;
    }

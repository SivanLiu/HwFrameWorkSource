...
public final class RotationPolicy {
    ...
    public static boolean isRotationLockToggleVisible(Context context) {
        if (isRotationSupported(context) && System.getIntForUser(context.getContentResolver(), "hide_rotation_lock_toggle_for_accessibility", 0, -2) == 0) {
            return true;
        }
        return false;
    }
    ...
    public static void setRotationLockForAccessibility(Context context, boolean enabled) {
        System.putIntForUser(context.getContentResolver(), "hide_rotation_lock_toggle_for_accessibility", enabled, -2);
    ...
    public static void registerRotationPolicyListener(Context context, RotationPolicyListener listener, int userHandle) {
        context.getContentResolver().registerContentObserver(System.getUriFor("accelerometer_rotation"), false, listener.mObserver, userHandle);
        context.getContentResolver().registerContentObserver(System.getUriFor("hide_rotation_lock_toggle_for_accessibility"), false, listener.mObserver, userHandle);
    }

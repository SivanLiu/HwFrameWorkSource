...
public class HwReportManagerImpl implements HwReportManager {
    ...
    public void reportSetTimeZoneByNitz(Phone phone, String zoneId, int tzOffset, String source) {
        ...
        stringBuilder.append("reportSetTimeZoneByNitz  tzOffset:");
        stringBuilder.append(tzOffset);
        ...
        if (tzOffset != -1) {
            if (((long) tzOffset) == this.mPreOffset || mReportTimes > 5) {
                ...
                stringBuilder2.append("tzOffset is not chaneged or report time is ");
                stringBuilder2.append(mReportTimes);
        ...
        EventStream regInfoStream = IMonitor.openEventStream(EVENT_NETWORK_INFO);
        if (regInfoStream != null) {
            setRegisteredInfoParam(regInfoStream);
        }
        EventStream eventStream = IMonitor.openEventStream(EVENT_SET_TIME_ZONE_BY_NITZ);
        if (eventStream != null) {
            eventStream.setParam((short) 0, regInfoStream);
            ...
            stringBuilder3.append("Send infomation this report time is :");
            stringBuilder3.append(mReportTimes);
    ...
    public void reportSetTimeZoneByIso(Phone phone, String zoneId, boolean mNitzUpdatedTime, String source) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("reportSetTimeZoneByIso  nitzUpdatedTime:");
        stringBuilder.append(mNitzUpdatedTime);
        ...
        if (!isAllowedReport(zoneId) || mReportTimes > 5) {
            ...
            stringBuilder.append("zoneId is not chaneged or report time is ");
            stringBuilder.append(mReportTimes);
            log(stringBuilder.toString());
            return;
        }
        EventStream regInfoStream = IMonitor.openEventStream(EVENT_NETWORK_INFO);
        ...
        EventStream eventStream = IMonitor.openEventStream(EVENT_SET_TIME_ZONE_BY_ISO);
        if (eventStream != null) {
            eventStream.setParam((short) 0, regInfoStream);
            ...
            mReportTimes++;
            StringBuilder stringBuilder2 = new StringBuilder();
            stringBuilder2.append("Send infomation this report time is :");
            stringBuilder2.append(mReportTimes);
            log(stringBuilder2.toString());
        }
        IMonitor.closeEventStream(regInfoStream);
    ...
    public void reportNitzIgnore(int phoneId, String forbidden) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("reportNitzIgnore  phoneId : ");
        ...
        if (this.mPreForidden.equals(forbidden) || mReportTimes > 5) {
            stringBuilder = new StringBuilder();
            stringBuilder.append("forbidden condition is not chaneged or report time is ");
            stringBuilder.append(mReportTimes);
            log(stringBuilder.toString());
            return;
        }
        ...
        EventStream eventStream = IMonitor.openEventStream(EVENT_NITZ_IGNORE);
        if (eventStream != null) {
            eventStream.setParam((short) 0, regInfoStream);
    ...
    public void reportMultiTZRegistered() {
        ...
        EventStream eventStream = IMonitor.openEventStream(EVENT_MULTI_TZ_REG);
        if (eventStream != null) {
            boolean z = false;
            eventStream.setParam((short) 0, regInfoStream);
            ...
            eventStream.setParam((short) 1, Boolean.valueOf(z));
            eventStream.setParam((short) 2, getLocationMode());
    ...
    public void reportMultiTZNoNitz() {
        ...
        EventStream eventStream = IMonitor.openEventStream(EVENT_NULTI_TZ_NO_NITZ);
        if (eventStream != null) {
            boolean z = false;
            eventStream.setParam((short) 0, regInfoStream);
    ...
    public void reportSetTimeZoneByLocation(String zoneId) {
        ...
        EventStream eventStream = IMonitor.openEventStream(EVENT_SET_TIMZ_ZONE_BY_LOCATION);
        if (eventStream != null) {
            boolean z = false;
            eventStream.setParam((short) 0, regInfoStream);
            eventStream.setParam((short) 1, zoneId);

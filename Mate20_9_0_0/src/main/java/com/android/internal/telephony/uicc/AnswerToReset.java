...
public class AnswerToReset {
    ...
    public static final byte DIRECT_CONVENTION = (byte) 59;
    public static final byte EUICC_SUPPORTED = (byte) -126;
    private static final int EXTENDED_APDU_INDEX = 2;
    public static final int INTERFACE_BYTES_MASK = 240;
    public static final byte INVERSE_CONVENTION = (byte) 63;
    ...
    public static class HistoricalBytes {
        ...
        public byte[] getValue(int tag) {
            return (byte[]) this.mNodes.get(Integer.valueOf(tag));
        }

        private static HistoricalBytes parseHistoricalBytes(byte[] originalData, int startIndex, int length) {
        ...
        private static int parseLtvNode(int index, ArrayMap<Integer, byte[]> nodes, byte[] data, int lastByteIndex) {
            ...
            int index2 = index + 1;
            index = data[index] & 15;
            if (index2 + index > lastByteIndex + 1 || index == 0) {
                return -1;
            }
            byte[] value = new byte[index];
            System.arraycopy(data, index2, value, 0, index);
    ...
    private static String byteToStringHex(Byte b) {
        return b == null ? null : IccUtils.byteToHex(b.byteValue());
    }

    private void checkIsEuiccSupported() {
        int i = 0;
        while (i < this.mInterfaceBytes.size() - 1) {
            if (((InterfaceByte) this.mInterfaceBytes.get(i)).getTD() == null || (((InterfaceByte) this.mInterfaceBytes.get(i)).getTD().byteValue() & 15) != 15 || ((InterfaceByte) this.mInterfaceBytes.get(i + 1)).getTB() == null || ((InterfaceByte) this.mInterfaceBytes.get(i + 1)).getTB().byteValue() != EUICC_SUPPORTED) {
                i++;
            } else {
    ...
    private int parseConventionByte(byte[] atrBytes, int index) {
        ...
        if (value == DIRECT_CONVENTION) {
            ...
            stringBuilder.append("Unrecognized convention byte ");
            stringBuilder.append(IccUtils.byteToHex(value));
    ...
    private boolean parseAtrString(String atr) {
        if (atr == null) {
            ...
            stringBuilder.append(atr.length());
            ...
            stringBuilder2.append("Successfully parsed the ATR string ");
            stringBuilder2.append(atr);
    ...
    public String toString() {
        ...
        Iterator it = this.mInterfaceBytes.iterator();
        while (it.hasNext()) {
            sb.append(((InterfaceByte) it.next()).toString());
        }
        sb.append("},");

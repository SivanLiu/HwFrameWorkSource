...
public class HwUiccManagerImpl implements HwUiccManager {
        ...
        if (radioState == RadioState.RADIO_OFF && mLastRadioState == RadioState.RADIO_ON) {
            if (!((Global.getInt(c.getContentResolver(), "airplane_mode_on", 0) != 0) || mCardState == CardState.CARDSTATE_ABSENT)) {
                if (HWFLOW) {
                    ...
                    stringBuilder.append("powerUpRadioIfhasCard: power radio on for slot ");
    ...
    public boolean getSwitchingSlot() {
        return HwFullNetworkManager.getInstance().getWaitingSwitchBalongSlot() || !(HwModemCapability.isCapabilitySupport(9) || HwFullNetworkManager.getInstance().isSet4GDoneAfterSimInsert());
    }
    ...
    public boolean isFullNetworkSupported() {
        return SystemProperties.getBoolean(HwFullNetworkConfig.PROPERTY_FULL_NETWORK_SUPPORT, false) && !HwModemCapability.isCapabilitySupport(9) && SystemProperties.getBoolean("persist.hisi.fullnetwork", true) && "normal".equals(SystemProperties.get("ro.runmode", "normal"));
    }

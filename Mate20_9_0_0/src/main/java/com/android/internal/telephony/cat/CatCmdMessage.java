...
public class CatCmdMessage implements Parcelable {
        ...
        if (i2 != 22) {
            switch (i2) {
                ...
                case 19:
                    this.mSetupEventListSettings = new SetupEventListSettings();
                    len = in.readInt();
                    this.mSetupEventListSettings.eventList = new int[len];
                    while (i < len) {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeByte((byte) this.mLoadIconFailed);
        if (!HuaweiTelephonyConfigs.isModemBipEnable()) {
            switch (getCmdType()) {
                case OPEN_CHANNEL:
        ...
        len = AnonymousClass2.$SwitchMap$com$android$internal$telephony$cat$AppInterface$CommandType[getCmdType().ordinal()];
        if (len != 22) {
            switch (len) {
                case 16:

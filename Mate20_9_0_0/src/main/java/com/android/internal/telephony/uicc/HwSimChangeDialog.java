...
public class HwSimChangeDialog {
    ...
    public AlertDialog getSimAddDialog(Context c, boolean isAdded, int mSlotId) {
        ...
        if (isAdded) {
            title = r.getString(17041111);
        } else {
            title = r.getString(17041114);
        }
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            if (isAdded) {
                string = r.getString(33685757);
            } else {
                string = r.getString(33685758);
            }
            message = String.format(string, new Object[]{Integer.valueOf(mSlotId + 1)});
        } else {
            if (isAdded) {
                string = r.getString(33685759);
            } else {
                string = r.getString(33685761);
            }
            message = string;
        }
        if (isAdded) {
            string = r.getString(33685762);
        } else {
            string = r.getString(33685763);
        }
        this.dialog = new Builder(this.mContext, 33947691).setTitle(title).setMessage(message).create();
        AnonymousClass1 listenerCancel = new OnClickListener() {
        ...
        View layout = new LinearLayout(this.mContext);
        layout.setOrientation(0);
        Button buttonRestart;
        LayoutParams lpButtonRestart;
        if (isAdded) {
            buttonRestart = new Button(themeContext);
            buttonRestart.setText(string);
            ...
            lpButtonRestart = new LayoutParams(-2, -2, 1.0f);
            ...
            buttonRestart = new Button(themeContext);
            buttonRestart.setText(string);
            buttonRestart.setOnClickListener(listenerCancel);
            lpButtonRestart = new LayoutParams(-2, -2, 1.0f);
            buttonRestart.setLayoutParams(lpButtonRestart);
            layout.addView(buttonRestart, lpButtonRestart);
        }
        ...
        this.dialog.getWindow().setType(HwFullNetworkConstants.EVENT_GET_PREF_NETWORK_MODE_DONE);
        return this.dialog;
    }
    ...
    private AlertDialog getDialogWithCountDownTimer(Context context, boolean isAdded, int mSlotId) {
        ...
        ContextThemeWrapper themeContext = new ContextThemeWrapper(context2, themeID);
        if (this.dialog != null) {
            this.dialog.dismiss();
        }
        if (isAdded) {
            buttonTxt = context2.getString(33685762);
        } else {
            buttonTxt = context2.getString(33685945, new Object[]{Integer.valueOf(this.mCurrentTime)});
        }
        String title = context2.getString(33685946);
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            String string;
            if (isAdded) {
                string = context2.getString(33685757);
            } else {
                string = context2.getString(33685758);
            }
            message = String.format(string, new Object[]{Integer.valueOf(mSlotId + 1)});
        } else {
            message = context2.getString(33685952);
        }
        this.dialog = new Builder(context2, 33947691).setTitle(title).setMessage(message).create();
        View layout = new LinearLayout(context2);
        layout.setOrientation(0);
        ...
        this.dialog.setCanceledOnTouchOutside(false);
        Button restartButton2 = restartButton;
        this.dialog.setView(layout, Dp2Px(context2, 15.0f), Dp2Px(context2, 12.0f), Dp2Px(context2, 15.0f), Dp2Px(context2, 12.0f));
        this.dialog.getWindow().setType(2009);
        if (!isAdded) {
            Rlog.d(TAG, "handleCountdown invoked.");
            handleCountdown(context2, restartButton2);
        }
        return this.dialog;
    }
    ...
    public void isGoingToshowCountDownTimerDialog(RadioState radioState, RadioState lastRadioState, CardState oldState, CardState cardState, Handler handler, int phoneId) {
        if (IS_SHOW_TIME) {
            ...
            stringBuilder.append("isGoingToshowCountDownTimerDialog radioState=");
            stringBuilder.append(radioState);
            stringBuilder.append(" lastRadioState=");
            stringBuilder.append(lastRadioState);
            ...
            if (radioState == RadioState.RADIO_OFF && lastRadioState == RadioState.RADIO_OFF) {

...
public class UiccSlot extends Handler {
    ...
    public void update(CommandsInterface ci, IccCardStatus ics, int phoneId) {
        ...
        stringBuilder.append(ics.toString());
        ...
        synchronized (this.mLock) {
            ...
            stringBuilder2.append("update: radioState=");
            stringBuilder2.append(radioState);
            stringBuilder2.append(" mLastRadioState=");
            stringBuilder2.append(this.mLastRadioState);
            stringBuilder2.append(", phoneId = ");
            stringBuilder2.append(phoneId);
            log(stringBuilder2.toString());
            if (absentStateUpdateNeeded(oldState)) {
                updateCardStateAbsent();
            } else if ((oldState != null && oldState != CardState.CARDSTATE_ABSENT && this.mUiccCard != null) || this.mCardState == CardState.CARDSTATE_ABSENT) {
                UiccCard uiccCard = this.mUiccCard;
            } else if (radioState == RadioState.RADIO_ON && this.mLastRadioState == RadioState.RADIO_ON) {
    ...
    public void update(CommandsInterface ci, IccSlotStatus iss) {
        ...
        stringBuilder.append(iss.toString());
        log(stringBuilder.toString());
        synchronized (this.mLock) {
            CardState oldState = this.mCardState;
            this.mCi = ci;
            ...
            if (iss.slotState != SlotState.SLOTSTATE_INACTIVE) {
                this.mActive = true;
    ...
    private boolean absentStateUpdateNeeded(CardState oldState) {
        return !(oldState == CardState.CARDSTATE_ABSENT && this.mUiccCard == null) && this.mCardState == CardState.CARDSTATE_ABSENT;
    }

    private void updateCardStateAbsent() {
        RadioState radioState = this.mCi == null ? RadioState.RADIO_UNAVAILABLE : this.mCi.getRadioState();
    ...
    public boolean isStateUnknown() {
        return (this.mCardState == null || this.mCardState == CardState.CARDSTATE_ABSENT) && this.mStateIsUnknown;
    }
    ...
    private void promptForRestart(boolean isAdded) {
        if (VSimUtilsInner.isPlatformTwoModems()) {
            ...
            if (VSimUtilsInner.isVSimSub(this.mPhoneId) || VSimUtilsInner.isVSimInProcess() || VSimUtilsInner.isVSimReconnecting()) {
                ...
                stringBuilder.append("[2Cards] mPhoneId=");
                stringBuilder.append(this.mPhoneId);
                stringBuilder.append(" is VSIM sub or VSIM is on(");
                stringBuilder.append(VSimUtilsInner.isVSimInProcess());
                ...
                stringBuilder = new StringBuilder();
                stringBuilder.append("[2Cards] Slot ");
                stringBuilder.append(this.mPhoneId);
                stringBuilder.append(" VsimCauseCard Reload , not prompt!");
                log(stringBuilder.toString());
                return;
            }
        }
        synchronized (this.mLock) {
            ...
            if (dialogComponent != null) {
                try {
                    ...
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("Unable to find ICC hotswap prompt for restart activity: ");

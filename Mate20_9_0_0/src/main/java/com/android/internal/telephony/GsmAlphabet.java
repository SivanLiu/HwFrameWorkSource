...
public class GsmAlphabet {
        ...
        while (i3 < offset + i) {
            ...
            if (c == 27) {
                ...
                if (prevWasEscape) {
                    ...
                    i4 = 2;
                } else if (!isMbcs || c < 128 || i3 + 1 >= offset + i) {
                    i4 = 2;
    ...
    public static TextEncodingDetails countGsmSeptets(CharSequence s, boolean use7bitOnly) {
        ...
        if (sEnabledSingleShiftTables.length + sEnabledLockingShiftTables.length == 0) {
            ...
            i3 = countGsmSeptetsUsingTables(charSequence, z, 0, 0);
            if (i3 == -1) {
                return null;
            }
            ted.codeUnitSize = 1;
            ted.codeUnitCount = i3;
            if (i3 > 160) {
                ted.msgCount = (i3 + 152) / 153;
                ted.codeUnitsRemaining = (ted.msgCount * 153) - i3;
            } else {
                ted.msgCount = 1;
                ted.codeUnitsRemaining = 160 - i3;
            }
            ted.codeUnitSize = 1;
            return ted;
        }
            ...
            if (c == 27) {
                ...
                for (LanguagePairCount lpc : lpcList) {
                    if (sCharsToGsmTables[lpc.languageCode].get(c, -1) == -1) {
                        for (table = i3; table <= maxSingleShiftCode; table++) {
        ...
        for (LanguagePairCount lpc2 : lpcList) {
            int minUnencodableCount = i5;
            i5 = 0;
            while (i5 <= maxSingleShiftCode) {
                int septets = lpc2.septetCounts[i5];
                if (septets != i2) {
                    int msgCount;
                    int septetsRemaining;
                    if (lpc2.languageCode != 0 && i5 != 0) {
                        table = 8;
                    } else if (lpc2.languageCode == 0 && i5 == 0) {
                        table = 0;
                    } else {
                        table = 5;
                    }
                    if (septets + table > i) {
                        ...
                        i2 = 160 - (table + 6);
                        msgCount = ((septets + i2) - 1) / i2;
                        septetsRemaining = (msgCount * i2) - septets;
                    } else {
                        msgCount = 1;
                        septetsRemaining = (160 - table) - septets;
                    }
                    i2 = msgCount;
                    int septetsRemaining2 = septetsRemaining;
                    i = lpc2.unencodableCounts[i5];
                    if (!z || i <= minUnencodableCount) {
                        if (z && i < minUnencodableCount) {
                            i3 = septetsRemaining2;
                        } else if (i2 < ted2.msgCount) {
                            i3 = septetsRemaining2;
                        } else if (i2 == ted2.msgCount) {
                            i3 = septetsRemaining2;
                            if (i3 <= ted2.codeUnitsRemaining) {
                            }
                        }
                        minUnencodableCount = i;
                        ted2.msgCount = i2;
                        ted2.codeUnitCount = septets;
                        ted2.codeUnitsRemaining = i3;
                        ted2.languageTable = lpc2.languageCode;
                        ted2.languageShiftTable = i5;
                    }
                }
                i5++;
                i = 160;

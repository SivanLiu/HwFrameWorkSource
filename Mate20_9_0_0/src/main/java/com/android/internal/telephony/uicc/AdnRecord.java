...
public class AdnRecord implements Parcelable {
    ...
    public String toString() {
        ...
        stringBuilder.append(Rlog.pii(LOG_TAG, this.mNumber));
        stringBuilder.append(" ");
        stringBuilder.append(Rlog.pii(LOG_TAG, this.mEmails));
        stringBuilder.append("'");
        stringBuilder.append(Rlog.pii(LOG_TAG, this.mAdditionalNumbers));
        stringBuilder.append("'");
    ...
    public void appendExtRecord(byte[] extRecord) {
        try {
            if (extRecord.length == 13 && (extRecord[0] & 3) == 2 && (extRecord[1] & 255) <= 10) {
                ...
                stringBuilder.append(PhoneNumberUtils.calledPartyBCDFragmentToString(extRecord, 2, 255 & extRecord[1], 1));
    ...
    private void parseRecord(byte[] record) {
        try {
            ...
            this.mExtRecord = 255 & record[record.length - 1];
            this.mEmails = null;
            this.mAdditionalNumbers = null;
        } catch (RuntimeException ex) {

...
public class HwVSimService extends Stub {
    ...
    public boolean hasVSimIccCard() {
        ...
        if (getVSimPhone().getIccCard() != null) {
    ...
    public int enableVSimV3(int operation, Bundle bundle) {
        if (-1 == this.mContext.checkCallingPermission(HwVSimConstants.VSIM_BUSSINESS_PERMISSION)) {
            Rlog.e(LOG_TAG, "enableVSimV3 , checkCallingPermission is fasle");
            return 3;
        }
        this.mContext.enforceCallingOrSelfPermission("android.permission.MODIFY_PHONE_STATE", null);
        int result = 3;
        if (sIsPlatformSupportVSim && this.mVSimController != null) {
            result = this.mVSimController.enableVSim(operation, bundle);
        }
        return result;
    }
    ...
    public boolean isSupportVSimByOperation(int operation) {
        if (-1 == this.mContext.checkCallingPermission(HwVSimConstants.VSIM_BUSSINESS_PERMISSION)) {
            Rlog.e(LOG_TAG, "isSupportVSimByOperation , checkCallingPermission is fasle");
            return false;
        }
        this.mContext.enforceCallingOrSelfPermission("android.permission.MODIFY_PHONE_STATE", null);
        boolean result = false;
        if (sIsPlatformSupportVSim && this.mVSimController != null) {
            result = this.mVSimController.isSupportVSimByOperation(operation);
        }
        return result;
    }

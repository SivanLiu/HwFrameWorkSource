...
public class DataConnectionReasons {
    ...
    public String toString() {
        ...
        if (this.mDataDisallowedReasonSet.size() > 0) {
            ...
            Iterator it = this.mDataDisallowedReasonSet.iterator();
            while (it.hasNext()) {
                DataDisallowedReasonType reason = (DataDisallowedReasonType) it.next();
                reasonStr.append(" ");
    ...
    boolean containsHardDisallowedReasons() {
        Iterator it = this.mDataDisallowedReasonSet.iterator();
        while (it.hasNext()) {
            if (((DataDisallowedReasonType) it.next()).isHardReason()) {
                return true;
            }
        }
        return false;

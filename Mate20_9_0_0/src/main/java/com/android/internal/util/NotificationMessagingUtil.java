...
public class NotificationMessagingUtil {
    ...
    private final ContentObserver mSmsContentObserver = new ContentObserver(new Handler(Looper.getMainLooper())) {
        public void onChange(boolean selfChange, Uri uri, int userId) {
            if (Secure.getUriFor(NotificationMessagingUtil.DEFAULT_SMS_APP_SETTING).equals(uri)) {
    ...
    public NotificationMessagingUtil(Context context) {
        ...
        this.mContext.getContentResolver().registerContentObserver(Secure.getUriFor(DEFAULT_SMS_APP_SETTING), false, this.mSmsContentObserver);
    }
    ...
    private void cacheDefaultSmsApp(int userId) {
        this.mDefaultSmsApp.put(Integer.valueOf(userId), Secure.getStringForUser(this.mContext.getContentResolver(), DEFAULT_SMS_APP_SETTING, userId));
    }

...
public class DumpUtils {
    public static void writeAccessory(DualDumpOutputStream dump, String idName, long id, UsbAccessory accessory) {
        ...
        dump.write("manufacturer", 1138166333441L, accessory.getManufacturer());
    ...
    public static void writePortStatus(DualDumpOutputStream dump, String idName, long id, UsbPortStatus status) {
        ...
        long token = dump.start(idName, id);
        dualDumpOutputStream.write("connected", 1133871366145L, status.isConnected());
        if (dualDumpOutputStream.isProto()) {
            dualDumpOutputStream.write("current_mode", 1159641169922L, status.getCurrentMode());
        } else {
            dualDumpOutputStream.write("current_mode", 1159641169922L, UsbPort.modeToString(status.getCurrentMode()));
        }
        writePowerRole(dualDumpOutputStream, "power_role", 1159641169923L, status.getCurrentPowerRole());
        writeDataRole(dualDumpOutputStream, "data_role", 1159641169924L, status.getCurrentDataRole());
        ...
        while (undumpedCombinations != 0) {
            ...
            long roleCombinationToken = dualDumpOutputStream.start("role_combinations", 2246267895813L);
            writePowerRole(dualDumpOutputStream, "power_role", 1159641169921L, powerRole);
            writeDataRole(dualDumpOutputStream, "data_role", 1159641169922L, dataRole);
            dualDumpOutputStream.end(roleCombinationToken);
        }

...
public class PipSnapAlgorithm {
    ...
    public Point getEdgeIntersect(Rect stackBounds, Rect movementBounds, float velX, float velY, Point dragStartPosition) {
        ...
        vertPoint.x = f2 > 0.0f ? rect2.right : rect2.left;
        vertPoint.y = findY(slope, yIntercept, (float) vertPoint.x);
        horizPoint.y = f3 > 0.0f ? rect2.bottom : rect2.top;
        ...
        if (maxDistance > 0) {
            ...
            if ((i < center && endPoint < center) || (i > center && endPoint > center)) {
                ...
                int slope2 = Math.min(((int) (0.0f - Math.pow(slope, 0))) / (this.mFlingDeceleration * 2), maxDistance);
                if (isLandscape) {
                    horizPoint.x = rect.left + (f2 > 0.0f ? slope2 : -slope2);
                } else {
                    horizPoint.y = rect.top + (f3 > 0.0f ? slope2 : -slope2);
                }
                return horizPoint;
            }
        }
    ...
    public Rect findClosestSnapBounds(Rect movementBounds, Rect stackBounds) {
        ...
        if (this.mSnapMode == 4 || this.mSnapMode == 3) {
            ...
            while (true) {
                ...
                Gravity.apply(((Integer) this.mSnapGravities.get(i2)).intValue(), stackBounds.width(), stackBounds.height(), pipBounds, 0, 0, tmpBounds2);

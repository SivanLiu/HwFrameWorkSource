...
public class DumpUtils {
    ...
    public static void writePrinterCapabilities(Context context, DualDumpOutputStream proto, String idName, long id, PrinterCapabilitiesInfo cap) {
        ...
        while (true) {
            ...
            writeMediaSize(context, dualDumpOutputStream, "media_sizes", 2246267895810L, (MediaSize) cap.getMediaSizes().get(i3));
            i2 = i3 + 1;
        }
        i2 = cap.getResolutions().size();
        while (true) {
            int i4 = i;
            if (i4 >= i2) {
                break;
            }
            writeResolution(dualDumpOutputStream, "resolutions", 2246267895811L, (Resolution) cap.getResolutions().get(i4));
            i = i4 + 1;
        }
    ...
    public static void writeMediaSize(Context context, DualDumpOutputStream proto, String idName, long id, MediaSize mediaSize) {
        long token = proto.start(idName, id);
    ...
    public static void writeResolution(DualDumpOutputStream proto, String idName, long id, Resolution res) {
        long token = proto.start(idName, id);
    ...
    public static void writeMargins(DualDumpOutputStream proto, String idName, long id, Margins margins) {
        long token = proto.start(idName, id);
    ...
    public static void writePrintAttributes(Context context, DualDumpOutputStream proto, String idName, long id, PrintAttributes attributes) {
        ...
        Resolution res = attributes.getResolution();
        if (res != null) {
    ...
    public static void writePrintJobInfo(Context context, DualDumpOutputStream proto, String idName, long id, PrintJobInfo printJobInfo) {
        ...
        String str = "has_advanced_options";
        if (printJobInfo.getAdvancedOptions() == null) {
            z = false;
        }
        dualDumpOutputStream.write(str, 1133871366155L, z);

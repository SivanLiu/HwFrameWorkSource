            ...
            if (scale == 0.0f) {
                ...
                if (RampAnimator.this.mIsAutoBrightnessMode && (RampAnimator.this.mIsFirstValidAutoBrightness || RampAnimator.this.mTargetValue < RampAnimator.this.mCurrentValue || RampAnimator.this.mAutoBrightnessIntervened)) {
                    if (RampAnimator.this.mFirstTimeCalculateAmount) {
                        f = (RampAnimator.this.mIsFirstValidAutoBrightness || RampAnimator.this.mAutoBrightnessIntervened) ? 0.5f : 3.0f;
                        duration = f;
                        f = (((float) Math.abs(RampAnimator.this.mCurrentValue - RampAnimator.this.mTargetValue)) / duration) * timeDelta;
                        if (RampAnimator.this.mTargetValue < 1254 && RampAnimator.this.mCurrentValue < 1300 && duration == 3.0f) {
                            f = (((float) Math.abs(RampAnimator.this.mCurrentValue - RampAnimator.this.mTargetValue)) / 0.5f) * timeDelta;
                        }
                        RampAnimator.this.mDecreaseFixAmount = f;
                        if (((double) timeDelta) > 0.016d) {
                            RampAnimator.this.mFirstTimeCalculateAmount = false;
                        }
                    } else {
                        f = RampAnimator.this.mDecreaseFixAmount;
                    }
                } else if (RampAnimator.this.mTargetValue > 1254) {
                    f = Math.abs(RampAnimator.this.mTargetValue - RampAnimator.this.mCurrentValue) < 40 ? ((((float) RampAnimator.this.mRate) * timeDelta) * sigma) / scale : ((((((float) Math.abs(RampAnimator.this.mTargetValue - RampAnimator.this.mCurrentValue)) * timeDelta) * ((float) RampAnimator.this.mRate)) * sigma) / 40.0f) / scale;
                ...
                if (RampAnimator.this.DEBUG) {
                    String access$400 = RampAnimator.this.TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("mIsAutoBrightnessMode=");
                    stringBuilder.append(RampAnimator.this.mIsAutoBrightnessMode);
                    stringBuilder.append(",mTargetValue=");
                    stringBuilder.append(RampAnimator.this.mTargetValue);
                    stringBuilder.append(",mCurrentValue=");
                    stringBuilder.append(RampAnimator.this.mCurrentValue);
                    stringBuilder.append(",amount=");
                    stringBuilder.append(f);
                    stringBuilder.append(",mRate=");
                    stringBuilder.append(RampAnimator.this.mRate);
                    ...
                    stringBuilder.append(scale);
                    stringBuilder.append(",mIsFirstValidAutoBrightness=");
                    stringBuilder.append(RampAnimator.this.mIsFirstValidAutoBrightness);
                    stringBuilder.append(",mAutoBrightnessIntervened=");
                    stringBuilder.append(RampAnimator.this.mAutoBrightnessIntervened);
                    Slog.d(access$400, stringBuilder.toString());
                }
                if (RampAnimator.this.mTargetValue > RampAnimator.this.mCurrentValue) {
                    RampAnimator.this.mAnimatedValue = Math.min(RampAnimator.this.mAnimatedValue + f, (float) RampAnimator.this.mTargetValue);
                } else {
                    RampAnimator.this.mAnimatedValue = Math.max(RampAnimator.this.mAnimatedValue - f, (float) RampAnimator.this.mTargetValue);
                }
            }
            int oldCurrentValue = RampAnimator.this.mCurrentValue;
            RampAnimator.this.mCurrentValue = Math.round(RampAnimator.this.mAnimatedValue);

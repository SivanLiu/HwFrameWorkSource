...
public class HwSmartBackLightNormalizedController implements LightSensorCallbacks {
    ...
    private final class SceneRecognition implements IPGPlugCallbacks {
        ...
        public boolean onEvent(int actionID, String value) {
            boolean z = false;
            if (actionID == IDisplayEngineService.DE_ACTION_PG_GALLERY_FRONT) {
                this.mInGalleryScene = true;
            } else if (actionID != IDisplayEngineService.DE_ACTION_PG_CAMERA_FRONT) {
                switch (actionID) {
    ...
    public HwSmartBackLightNormalizedController(Context context, LightsManager lightsManager, SensorManager sensorManager) {
        ...
        this.mSmartBackLight = this.mLightsManager.getLight(257);
        try {
        ...
        this.mLightSensorController = new HwLightSensorController(context, this, sensorManager, this.mData.lightSensorRateMills);
        ...
        if (this.mSmartBackLightEnable || useSmartBacklight) {
            if (this.mSmartBackLightEnable != useSmartBacklight) {
                if (DEBUG) {
                    ...
                    stringBuilder.append("mSmartBackLightEnable change ");
                    stringBuilder.append(this.mSmartBackLightEnable);
            ...
            boolean z = wantScreenOn(state) && useSmartBacklight;
    ...
    private void turnOnSBL() {
        ...
        if (DEBUG) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("turnOnSBL, mSmoothAmbientLux = ");
            stringBuilder.append(this.mSmoothAmbientLux);
            Slog.i(str, stringBuilder.toString());
        }
    ...
    private void updateSBLLux() {
        if (this.mSmartBackLightOn) {
            if (DEBUG) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("updateSBLLux, mSmoothAmbientLux = ");
                stringBuilder.append(this.mSmoothAmbientLux);
                Slog.i(str, stringBuilder.toString());
            }
            this.mSmartBackLight.sendSmartBackLightWithRefreshFrames(1, getApicalADLevel(), getSmoothAmbientLux(), 100, false, 0, 0);
    ...
    private void processPolicy() {
        if (needMakePolicy()) {
            ...
            if (DEBUG && !this.mIsBrightnessUpdating) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("OutdoorScence = ");
                stringBuilder.append(isOutdoor);
                stringBuilder.append(", AppSBLScene = ");
                stringBuilder.append(this.mAppSBLScene);
                stringBuilder.append(", UpdateSBL = ");
                stringBuilder.append(this.mNeedUpdateSBL);
                stringBuilder.append(", CurrentLux = ");
                stringBuilder.append(this.mCurrentLux);
                stringBuilder.append(", SmoothLux = ");
                stringBuilder.append(this.mSmoothAmbientLux);
                stringBuilder.append(", UpdataBright = ");
                stringBuilder.append(this.mIsBrightnessUpdating);
                Slog.i(str, stringBuilder.toString());
            }

...
public class SnoozeHelper {
    ...
    static {
        ...
        stringBuilder.append(SnoozeHelper.class.getSimpleName());
        stringBuilder.append(".EVALUATE");
        REPOST_ACTION = stringBuilder.toString();
    }
    ...
    protected boolean isSnoozed(int userId, String pkg, String key) {
        return this.mSnoozedNotifications.containsKey(Integer.valueOf(userId)) && ((ArrayMap) this.mSnoozedNotifications.get(Integer.valueOf(userId))).containsKey(pkg) && ((ArrayMap) ((ArrayMap) this.mSnoozedNotifications.get(Integer.valueOf(userId))).get(pkg)).containsKey(key);
    }
    ...
    protected List<NotificationRecord> getSnoozed() {
        ...
        if (userIds != null) {
            for (int valueOf : userIds) {
                int valueOf2;
                ArrayMap<String, ArrayMap<String, NotificationRecord>> snoozedPkgs = (ArrayMap) this.mSnoozedNotifications.get(Integer.valueOf(valueOf2));
                if (snoozedPkgs != null) {
                    valueOf2 = snoozedPkgs.size();
                    for (int j = 0; j < valueOf2; j++) {
    ...
    private void scheduleRepost(String pkg, String key, int userId, long duration) {
        ...
        try {
            ...
            if (DEBUG) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("Scheduling evaluate for ");
    ...
    public void dump(PrintWriter pw, DumpFilter filter) {
        ...
        for (Integer userId : this.mSnoozedNotifications.keySet()) {
            ...
            ArrayMap<String, ArrayMap<String, NotificationRecord>> snoozedPkgs = (ArrayMap) this.mSnoozedNotifications.get(Integer.valueOf(userId2));

...
public class ZenModeFiltering {
            ...
            if (!REPEAT_CALLERS.mCalls.isEmpty()) {
                ...
                for (int i = 0; i < REPEAT_CALLERS.mCalls.size(); i++) {
                    ...
                    if (callNumLength >= 4) {
                        ...
                        stringBuilder.append(callNum.substring(callNumLength - 4));
                        callNum = stringBuilder.toString();
                    }
                    pw.print(callNum);
                    pw.print(" at ");
    ...
    private static Bundle extras(NotificationRecord record) {
        return (record == null || record.sbn == null || record.sbn.getNotification() == null) ? null : record.sbn.getNotification().extras;
    }
    ...
    public boolean shouldIntercept(int zen, ZenModeConfig config, NotificationRecord record) {
        ...
        if (Policy.areAllVisualEffectsSuppressed(config.suppressedVisualEffects) && PackageManagerService.PLATFORM_PACKAGE_NAME.equals(record.sbn.getPackageName()) && 48 == record.sbn.getId()) {
    ...
    private boolean isDefaultPhoneApp(String pkg) {
        if (this.mDefaultPhoneApp == null) {
            ...
            if (DEBUG) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("Default phone app: ");
                stringBuilder.append(this.mDefaultPhoneApp);

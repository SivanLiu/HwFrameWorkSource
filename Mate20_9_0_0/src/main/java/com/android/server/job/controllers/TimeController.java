...
public class TimeController extends StateController {
    ...
    public void maybeStartTrackingJobLocked(JobStatus job, JobStatus lastJob) {
        ...
        if (jobStatus.hasTimingDelayConstraint() || jobStatus.hasDeadlineConstraint()) {
            ...
            if (maybeProxyServiceLocked(jobStatus)) {
                ...
                stringBuilder.append("startTrack, service is proxy: ");
                stringBuilder.append(jobStatus.getServiceComponent());
                stringBuilder.append(", tag:");
                stringBuilder.append(jobStatus.getTag());
            ...
            if (!jobStatus.hasDeadlineConstraint() || !evaluateDeadlineConstraint(jobStatus, nowElapsedMillis)) {
                if (!jobStatus.hasTimingDelayConstraint() || !evaluateTimingDelayConstraint(jobStatus, nowElapsedMillis) || jobStatus.hasDeadlineConstraint()) {
                    ...
                    while (true) {
                        ...
                        if (!it.hasPrevious()) {
                            break;
                        } else if (((JobStatus) it.previous()).getLatestRunTimeElapsed() < jobStatus.getLatestRunTimeElapsed()) {
                            isInsert = true;
                            break;
                        } else {
                            it2 = it;
                    ...
                    long earliestRunTime = jobStatus.hasTimingDelayConstraint() ? jobStatus.getEarliestRunTime() : JobStatus.NO_LATEST_RUNTIME;
                    if (jobStatus.hasDeadlineConstraint()) {
                        latestRunTimeElapsed = jobStatus.getLatestRunTimeElapsed();
                    } else {
                        latestRunTimeElapsed = Long.MAX_VALUE;
                    }
    ...
    private boolean isDisplayOn() {
        ...
        if (this.mCurDisplay != null) {
            ...
            if (DEBUG) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("cur display is screen on: ");
    ...
    private boolean isCharging() {
        ...
        if (DEBUG) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("pluggedStatus : ");
            stringBuilder.append(pluggedStatus);
            Slog.i(str, stringBuilder.toString());
        }
        if (pluggedStatus != 0) {
    ...
    private void setDelayExpiredAlarmLocked(long alarmTimeElapsedMillis, WorkSource ws) {
        ...
        if (isDisplayOn() || isCharging() || alarmTimeElapsedMillis <= earliestWakeupTimeElapsed) {
            this.mNextDelayExpiredElapsedMillis = alarmTimeElapsedMillis;
        } else {
            this.mNextDelayExpiredElapsedMillis = alarmTimeElapsedMillis % 300000 != 0 ? 300000 * ((alarmTimeElapsedMillis / 300000) + 1) : alarmTimeElapsedMillis;
        }
        if (this.mOldDelayExpiredElapsedMillis != this.mNextDelayExpiredElapsedMillis) {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("mNextDelayExpiredElapsedMillis: ");
            stringBuilder.append(this.mNextDelayExpiredElapsedMillis);
            Slog.i(str, stringBuilder.toString());
        }
    }
    ...
    private void setDeadlineExpiredAlarmLocked(long alarmTimeElapsedMillis, WorkSource ws) {
        ...
        if (isDisplayOn() || isCharging() || alarmTimeElapsedMillis <= earliestWakeupTimeElapsed) {
            ...
            this.mNextJobExpiredElapsedMillis = alarmTimeElapsedMillis % 300000 != 0 ? 300000 * ((alarmTimeElapsedMillis / 300000) + 1) : alarmTimeElapsedMillis;
        }
        if (this.mOldJobExpiredElapsedMillis != this.mNextJobExpiredElapsedMillis) {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("mNextJobExpiredElapsedMillis: ");
            stringBuilder.append(this.mNextJobExpiredElapsedMillis);
            Slog.i(str, stringBuilder.toString());
        }
    }
    ...
    private void updateAlarmWithListenerLocked(String tag, OnAlarmListener listener, long alarmTimeElapsed, WorkSource ws) {
    ...
    public void dumpControllerStateLocked(IndentingPrintWriter pw, Predicate<JobStatus> predicate) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Elapsed clock: ");

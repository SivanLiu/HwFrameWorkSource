...
public final class GrantedUriPermissions {
    ...
    private GrantedUriPermissions(IActivityManager am, int grantFlags, int uid, String tag) throws RemoteException {
        ...
        this.mPermissionOwner = am.newUriPermissionOwner(stringBuilder.toString());
    }
    ...
    private static GrantedUriPermissions grantUri(IActivityManager am, Uri uri, int sourceUid, String targetPackage, int targetUserId, int grantFlags, String tag, GrantedUriPermissions curPerms) {
        ...
        try {
            try {
                ...
                uri2 = ContentProvider.getUriWithoutUserId(uri);
                if (curPerms == null) {
                    iActivityManager = am;
                    i = sourceUid;
                    i2 = grantFlags;
                    try {
                        curPerms2 = new GrantedUriPermissions(iActivityManager, i2, i, tag);
                    } catch (RemoteException e) {
                        curPerms2 = curPerms;
                ...
                curPerms2 = curPerms;
                try {
                    iActivityManager.grantUriPermissionFromOwner(curPerms2.mPermissionOwner, i, targetPackage, uri2, i2, sourceUserId, targetUserId);

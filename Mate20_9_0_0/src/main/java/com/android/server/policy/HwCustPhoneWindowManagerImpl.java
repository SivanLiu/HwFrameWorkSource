...
public class HwCustPhoneWindowManagerImpl extends HwCustPhoneWindowManager {
    ...
    public void processCustInterceptKey(int keyCode, boolean down, Context context) {
        ...
        if (this.mContext != null) {
            ...
            stringBuilder.append("answer call by keyCode = ");
            stringBuilder.append(keyCode);
            stringBuilder.append(", down = ");
            stringBuilder.append(down);
            stringBuilder.append(", isSupVolumnkeyAnswerCall = ");
            stringBuilder.append(isSupVolumnkeyAnswerCall);
    ...
    public void volumnkeyWakeup(Context mContext, boolean isScreenOn, PowerManager mPowerManager) {
        if (mContext != null && mPowerManager != null && !isScreenOn && Secure.getInt(mContext.getContentResolver(), "incall_power_button_behavior", 1) == 2) {
    ...
    public boolean isVolumnkeyWakeup(Context mContext) {
        if (mContext != null) {
            return "true".equalsIgnoreCase(Global.getString(mContext.getContentResolver(), "is_volumnkey_wakeup"));
        }
        return false;
    }
    ...
    private void notifyRapidRecordService(Context context) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("start Rapid Record Service, command:");
        stringBuilder.append(intent.getExtras().get("command"));
        Log.d(str, stringBuilder.toString());
    }
    ...
    public boolean interceptVolumeUpKey(KeyEvent event, Context context, boolean isScreenOn, boolean keyguardActive, boolean isMusicOrFMOrVoiceCallActive, boolean isInjected, boolean down) {
        ...
        if (HWRIDEMODE_FEATURE_SUPPORTED && SystemProperties.getBoolean("sys.ride_mode", false)) {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("interceptVolumeUpKey now=");
        ...
        if (context != null) {
            ...
            if (mTelephonyManager != null) {
                if (mTelephonyManager.isMultiSimEnabled()) {
                    boolean z = false;
                    if (isCustPlmn(mCustSimOperator, mTelephonyManager.getSimOperator(0)) || isCustPlmn(mCustSimOperator, mTelephonyManager.getSimOperator(1))) {
                        z = true;
                    }
                    flag = z;
                } else {
                    flag = isCustPlmn(mCustSimOperator, mTelephonyManager.getSimOperator());
                }
            }
        }
        return flag;
    ...
    private void answerRingingCall(int keyCode, Context context) {
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("acceptRingingCall by KeyEvent.KEYCODE_VOLUME keyCode = ");
        stringBuilder.append(keyCode);
        Log.d(str, stringBuilder.toString());
        switch (keyCode) {
            case 24:
    ...
    private boolean isGoogleCountSignInSetupWizard(Context context, WindowState focusedWindow) {
        ...
        if (windowName != null && windowName.contains("com.google.android.gms") && Secure.getInt(context.getContentResolver(), "device_provisioned", 1) == 0) {
            return true;
        }
        return false;
    }

...
public class HwSplitScreenArrowView extends LinearLayout {
    ...
    private void moveToPositionX(float aPosX) {
        if (aPosX > ((float) (4 * this.mNavBarHeight))) {
            this.mParams.x = ((int) aPosX) - (3 * this.mNavBarHeight);
            this.mWindowMgr.updateViewLayout(this, this.mParams);
        }
    }
    ...
    public void handleSplitScreenGesture(MotionEvent event) {
        ...
        stringBuilder.append("event.x=:");
        stringBuilder.append(event.getX());
        stringBuilder.append(" event.y=:");
        stringBuilder.append(event.getY());
        Log.d("fingerTest", stringBuilder.toString());
        boolean z = false;
    ...
    private void processFingerMoveEvent(MotionEvent event) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("lDistance = :");
        stringBuilder.append(lDistance);
        Log.d("fingerTest", stringBuilder.toString());
    ...
    private void animateView(float aPos, boolean islaunchingMultiWindow) {
        ...
        PropertyValuesHolder animValue = PropertyValuesHolder.ofFloat("value", new float[]{aPos > aMin ? aPos : aMin, aMax});
        ValueAnimator animation = ValueAnimator.ofPropertyValuesHolder(new PropertyValuesHolder[]{animValue});
        int duration = (int) (1137180672 * (Math.abs(aMax - aPos) / (aMax / 2.0f)));
        int i = 200;
        if (duration > 200) {
            i = duration;
        }
    ...
    private boolean isTopTaskHome() {
        ...
        return isInHomeStack((RunningTaskInfo) tasks.get(0));
    }
    ...
    private boolean isInHomeStack(RunningTaskInfo runningTask) {
        boolean z = false;
        if (runningTask == null) {
            return false;
        }
    ...
    private void showToastForAllUser(Context mContext, int message) {
        if (mContext != null) {
            Toast toast = Toast.makeText(mContext, message, null);

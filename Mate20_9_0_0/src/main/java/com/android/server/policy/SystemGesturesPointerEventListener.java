...
public class SystemGesturesPointerEventListener implements PointerEventListener {
    ...
    private static final boolean IS_USER_DOMESTIC_BETA;
    ...
    private final class FlingGestureDetector extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent down, MotionEvent up, float velocityX, float velocityY) {
            ...
            if (SystemGesturesPointerEventListener.IS_USER_DOMESTIC_BETA) {
                LogPower.push(NetdResponseCode.IpFwdStatusResult, String.valueOf(velocityX), String.valueOf(velocityY));
            }
            return true;
        }
    ...
    private final class ScaleDetector extends SimpleOnScaleGestureListener {
        private ScaleDetector() {
        }

        public void onScaleEnd(ScaleGestureDetector detector) {
            if (SystemGesturesPointerEventListener.IS_USER_DOMESTIC_BETA) {
                LogPower.push(NetdResponseCode.TetherStatusResult);
            }
        }
    }
    ...
    private boolean filterTouchPadEventForPCMode(MotionEvent event) {
        if (HwPCUtils.isPcCastModeInServer() && event.getSource() != UsbACInterface.FORMAT_III_IEC1937_MPEG1_Layer1) {
            ...
            switch (actionMasked) {
                ...
                case 2:
                    if (this.mSwipeFireable) {
                        actionMasked = detectSwipe(event);
                        if (actionMasked == 0) {
                            z = true;
                        }
        ...
        for (int p = 0; p < pointerCount; p++) {
            ...
            if (i != -1) {
                ...
                while (true) {
                    int h2 = h;
                    if (h2 < historySize) {
                        h = detectSwipe(i, motionEvent.getHistoricalEventTime(h2), motionEvent.getHistoricalX(p, h2), motionEvent.getHistoricalY(p, h2));
                        if (h != 0) {
                            return h;
                        }
                        h = h2 + 1;
                    } else {
                        h = detectSwipe(i, move.getEventTime(), motionEvent.getX(p), motionEvent.getY(p));
                        if (h != 0) {
                            return h;
                        }
                    }

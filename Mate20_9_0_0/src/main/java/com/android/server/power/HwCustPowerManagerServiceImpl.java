...
public class HwCustPowerManagerServiceImpl extends HwCustPowerManagerService {
    ...
    private UEventObserver mCharingObserver = new UEventObserver() {
        public void onUEvent(UEvent event) {
            ...
            if (HwCustPowerManagerServiceImpl.HWDBG) {
                String str = HwCustPowerManagerServiceImpl.TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("mCharingObserver:state=");
    ...
    public void updateSettingsLocked() {
        if (isChargingAlbumSupported()) {
            ContentResolver resolver = this.mContext.getContentResolver();
            String str = CHARGING_ALBUM_ENABLED;
            boolean z = HWLOGW_E;
            if (System.getInt(resolver, str, 1) == 0) {
                z = false;
            }
    ...
    public boolean isStartDreamFromUser() {
        return (isChargingAlbumSupported() && this.startDreamFromUser) ? HWLOGW_E : false;
    }
    ...
    public boolean isChargingAlbumEnabled() {
        ...
        if (HWDBG) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("isChargingAlbumenable ");
            stringBuilder.append(this.mChargingAlbumEnabled);
            Log.d(str, stringBuilder.toString());
        }
    ...
    public boolean canDreamLocked() {
        ...
        if (this.mChargingMode != 0 || this.mCharingBase) {
            return false;
        }
        return HWLOGW_E;
    }
    ...
    public void handleDreamLocked() {
        ...
        if (isChargingAlbumSupported() && this.mDreamManager != null && !this.mBatteryManagerInternal.isPowered(7)) {
    ...
    public void init(Context context) {
        if (context != null) {
            ...
            if (HWDBG) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("c2dm delay time ");
                stringBuilder.append(this.C2DM_DELAY_TIMEOUT);

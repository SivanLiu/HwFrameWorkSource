...
public class BatterySavingStats {
    ...
    class MetricsLoggerHelper {
        ...
        public void transitionStateLocked(int newState, long now, int batteryLevel, int batteryPercent) {
            ...
            long j = now;
            boolean z = false;
            if (!(((this.mLastState >= 0 ? 1 : 0) ^ (i >= 0 ? 1 : 0)) == 0 && ((this.mLastState ^ i) & 3) == 0)) {
                z = true;
            }
            if (z) {
    ...
    static class Stat {
        ...
        String toStringForTest() {
            ...
            stringBuilder.append(this.totalBatteryDrain);
            stringBuilder.append(",");
            stringBuilder.append(String.format("%.2f", new Object[]{Double.valueOf(drainPerHour())}));
            stringBuilder.append("uA/H,");
            stringBuilder.append(String.format("%.2f", new Object[]{Double.valueOf(drainPercentPerHour())}));
            stringBuilder.append("%}");
    ...
    static String stateToString(int state) {
        switch (state) {
            ...
            default:
                ...
                stringBuilder.append(BatterySaverState.fromIndex(state));
                stringBuilder.append(",I=");
                stringBuilder.append(InteractiveState.fromIndex(state));
                stringBuilder.append(",D=");
                stringBuilder.append(DozeState.fromIndex(state));
                return stringBuilder.toString();
        }
    }
    ...
    Stat getStat(int stateIndex) {
        Stat stat;
        synchronized (this.mLock) {
            stat = (Stat) this.mStats.get(Integer.valueOf(stateIndex));
            if (stat == null) {
                stat = new Stat();
    ...
    private void endLastStateLocked(long now, int batteryLevel, int batteryPercent) {
        if (this.mCurrentState >= 0) {
            ...
            int fromIndex2 = InteractiveState.fromIndex(this.mCurrentState);
            int fromIndex3 = DozeState.fromIndex(this.mCurrentState);
            long j = stat.totalTimeMillis;
            EventLogTags.writeBatterySavingStats(fromIndex, fromIndex2, fromIndex3, deltaTime, deltaDrain, deltaPercent, j, stat.totalBatteryDrain, stat.totalBatteryDrainPercent);
        }
    }
        ...
        synchronized (this.mLock) {
            ...
            try {
                ...
                try {
                    ...
                    stringBuilder.append("  ");
                    indent2 = stringBuilder.toString();

...
public class DataConnectionStats extends BroadcastReceiver {
    ...
    private void notePhoneDataConnectionState() {
        if (this.mServiceState != null) {
            boolean z = true;
            boolean simReadyOrUnknown = this.mSimState == State.READY || this.mSimState == State.UNKNOWN;
            if (!((simReadyOrUnknown || isCdma()) && hasService() && this.mDataState == 2)) {
                z = false;
            }
            try {
                this.mBatteryStats.notePhoneDataConnectionState(this.mServiceState.getDataNetworkType(), z);

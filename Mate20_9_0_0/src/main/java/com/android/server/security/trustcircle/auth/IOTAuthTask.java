...
public class IOTAuthTask extends HwSecurityTaskBase {
    ...
    RetCallback mReqAckCallback = new RetCallback() {
        public void onTaskCallback(HwSecurityTaskBase child, int ret) {
            ...
            stringBuilder.append(child.getClass().getSimpleName());
            stringBuilder.append("-->onTaskCallback, ret: ");
            stringBuilder.append(ret);
            LogHelper.d(access$400, stringBuilder.toString());
            if (ret != 0) {
    ...
    public int doAction() {
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("doAction, authID: ");
        stringBuilder.append(this.mInitAuthInfo.mAuthID);

...
public class HwSecurityService extends SystemService {
        ...
        public void bind(IBinder client) {
            if (client != null) {
                ...
                if (HwSecurityService.HW_DEBUG) {
                    ...
                    stringBuilder.append("HwSecurityDynamicPluginRef mClient size:");
                    stringBuilder.append(this.mClient.size());
    ...
    private void startResidentPlugin(int startupTiming) {
        if (!this.mMapPlugins.isEmpty()) {
            ...
            for (Integer key : this.mMapPlugins.keySet()) {
                HwSecurityPluginObj obj = (HwSecurityPluginObj) this.mMapPlugins.get(Integer.valueOf(key.intValue()));

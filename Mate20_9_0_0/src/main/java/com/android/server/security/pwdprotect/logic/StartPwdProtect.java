...
public class StartPwdProtect {
    ...
    private boolean saveMainSpaceEncryFile(String mainSpacePin, String pwQuestionAnswer) {
        if (DeviceEncryptUtils.deviceEncode(encryMainSpacePin(mainSpacePin, pwQuestionAnswer)).length == 0) {
            Log.e(TAG, "Device encrypt E_SK2 failed");
            return false;
        }
        byte[] fileHashs = DeviceEncryptUtils.hmacSign(StringUtils.byteMerger(this.mSecretKeyIV, this.mPin2Iv, this.mSalt, DeviceEncryptUtils.deviceEncode(encryMainSpacePin(mainSpacePin, pwQuestionAnswer))));
    ...
    private boolean savePrivSpaceEncryFile(byte[] ePrivSpaceValues, byte[] publicKey) {
        if (DeviceEncryptUtils.deviceEncode(ePrivSpaceValues).length == 0) {
            Log.e(TAG, "Device encrypt E_PIN2 failed");
            return false;
        }
        byte[] fileHashs = DeviceEncryptUtils.hmacSign(StringUtils.byteMerger(DeviceEncryptUtils.deviceEncode(ePrivSpaceValues), publicKey));
    ...
    private boolean savePwdQAnswer(String pwQuestionAnswer) {
        ...
        byte[] fileHashs = DeviceEncryptUtils.hmacSign(pwdQAnswerHashEncry);
        FileUtils.writeFile(StringUtils.byteMerger(pwdQAnswerHashEncry, fileHashs), PasswordIvsCache.FILE_E_PWDQANSWER);
        return true;
    }
    ...
    private boolean savePwQ(String pwQuestion) {
        ...
        byte[] fileHashs = DeviceEncryptUtils.hmacSign(pwdQHashEncry);
        FileUtils.writeFile(StringUtils.byteMerger(pwdQHashEncry, fileHashs), PasswordIvsCache.FILE_E_PWDQ);
        return true;
    }
}

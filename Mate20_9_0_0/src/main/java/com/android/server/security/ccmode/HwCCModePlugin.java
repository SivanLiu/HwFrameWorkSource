...
public class HwCCModePlugin extends Stub implements IHwSecurityPlugin {
        ...
        if (isCCModeOn()) {
            new Thread() {
                public void run() {
                    ...
                    try {
                        boolean z = CCModeFuncInternal.native_dxCryptoSelftest() && CCModeFuncInternal.native_kernelCryptoSelftest() && CCModeFuncInternal.native_boringSslSelftest();
                        testOk = z;
                    ...
                    if (HwCCModePlugin.HWDBG) {
                        String str = HwCCModePlugin.TAG;
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append("CC mode self test ");
                        stringBuilder.append(testOk ? "OK" : "failed");
                        stringBuilder.append("!!!");
                        Slog.w(str, stringBuilder.toString());
                        str = HwCCModePlugin.TAG;
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("selfTest cost:");
    ...
    private void setCCMode(boolean isOn) {
        String str = PROPERTIES_CC_MODE;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("");
        stringBuilder.append(isOn);

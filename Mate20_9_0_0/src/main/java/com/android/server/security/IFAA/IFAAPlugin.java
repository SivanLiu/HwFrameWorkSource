...
public class IFAAPlugin extends Stub implements IHwSecurityPlugin {
    ...
    private int getCmdId(byte[] data) {
        ...
        position += 4;
        if (signLen < 0 || MAX_INT - position < signLen || dataLen < position + signLen) {
            return -1;
        }
        position += signLen;
    ...
    public void onStart() {
        ...
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("start error");
            stringBuilder.append(e.toString());
            Slog.e(str, stringBuilder.toString());
        }
    ...
    private void showHardwareSupportPrompt(byte[] reqData, byte[] resData) {
        ...
        if (2 == cmdID && retCode == 0) {
            Flog.bdReport(this.mContext, NetdResponseCode.ApLinkedStaListChangeQCOM);
        }

...
public class HwServiceFactoryImpl implements Factory {
    ...
    public static class DisplayEngineInterfaceImpl implements IDisplayEngineInterface {
        ...
        public boolean getSupported(String feature) {
            ...
            HwServiceFactoryImpl.mDisplayEngineInterface;
            if (ret != 0) {
                z = true;
            }
            return z;
    ...
    public static class HwAutomaticBrightnessControllerImpl implements IHwAutomaticBrightnessController {
        public AutomaticBrightnessController getInstance(Callbacks callbacks, Looper looper, SensorManager sensorManager, BrightnessMappingStrategy mapper, int lightSensorWarmUpTime, int brightnessMin, int brightnessMax, float dozeScaleFactor, Context context) {
            return new HwNormalizedAutomaticBrightnessController(callbacks, looper, sensorManager, mapper, lightSensorWarmUpTime, brightnessMin, brightnessMax, dozeScaleFactor, context);
        }

        public AutomaticBrightnessController getInstance(Callbacks callbacks, Looper looper, SensorManager sensorManager, BrightnessMappingStrategy mapper, int lightSensorWarmUpTime, int brightnessMin, int brightnessMax, float dozeScaleFactor, int lightSensorRate, int initialLightSensorRate, long brighteningLightDebounceConfig, long darkeningLightDebounceConfig, boolean resetAmbientLuxAfterWarmUpConfig, HysteresisLevels hysteresisLevels, Context context) {
    ...
    public static class HwNetworkManagementServiceImpl implements IHwNetworkManagermentService {
        ...
        public NetworkManagementService getInstance(Context context, String socket, SystemServices services) {
    ...
    public static class HwNetworkStatsServiceImpl implements IHwNetworkStatsService {
        public NetworkStatsService getInstance(Context context, INetworkManagementService networkManager, AlarmManager alarmManager, WakeLock wakeLock, Clock clock, TelephonyManager teleManager, NetworkStatsSettings settings, NetworkStatsObservers statsObservers, File systemDir, File baseDir) {
            return new HwNetworkStatsService(context, networkManager, alarmManager, wakeLock, clock, teleManager, settings, statsObservers, systemDir, baseDir);
        }
    }
    ...
    public DefaultPermissionGrantPolicy getHwDefaultPermissionGrantPolicy(Context context, Looper looper, DefaultPermissionGrantedCallback callback, PermissionManagerService permissionManager) {
    ...
    public void setupHwServices(Context context) {
        ...
        if (HwGeneralManager.getInstance().isSupportForce()) {
            try {
                HwGeneralService generalService2 = new HwGeneralService(context, UiThread.getHandler());
                Slog.i(TAG, "General Service");
                ServiceManager.addService("hwGeneralService", generalService2);
            } catch (Throwable e3) {
                Slog.e(TAG, "Failure starting generalService", e3);
            }
        }
        ...
        if (apsSupportValue != 0) {
            try {
                ...
                Slog.e(TAG, "Failure starting apsService", e32);
            }
        } else {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Not support APS! sys.aps.support = ");
            stringBuilder.append(apsSupportValue);
        ...
        try {
            ...
            stringBuilder2.append("[effect] Failed to start DisplayEngineServiceEx, ");
    ...
    public ActivityRecord createActivityRecord(ActivityManagerService _service, ProcessRecord _caller, int _launchedFromPid, int _launchedFromUid, String _launchedFromPackage, Intent _intent, String _resolvedType, ActivityInfo aInfo, Configuration _configuration, ActivityRecord _resultTo, String _resultWho, int _reqCode, boolean _componentSpecified, boolean _rootVoiceInteraction, ActivityStackSupervisor supervisor, ActivityOptions options, ActivityRecord sourceRecord) {
        return new HwActivityRecord(_service, _caller, _launchedFromPid, _launchedFromUid, _launchedFromPackage, _intent, _resolvedType, aInfo, _configuration, _resultTo, _resultWho, _reqCode, _componentSpecified, _rootVoiceInteraction, supervisor, options, sourceRecord);
    }
    ...
    public TaskRecord createTaskRecord(ActivityManagerService service, int taskId, ActivityInfo info, Intent intent, TaskDescription taskDescription) {
    ...
    public ActivityStack createActivityStack(ActivityDisplay display, int stackId, ActivityStackSupervisor supervisor, int windowingMode, int activityType, boolean onTop) {
        return new HwActivityStack(display, stackId, supervisor, windowingMode, activityType, onTop);
    }
    ...
    public FocusRequester getHwFocusRequester(AudioAttributes aa, int focusRequest, int grantFlags, IAudioFocusDispatcher afl, IBinder source, String id, AudioFocusDeathHandler hdlr, String pn, int uid, MediaFocusControl ctlr, int sdk, boolean isInExternal) {
        return new HwFocusRequester(aa, focusRequest, grantFlags, afl, source, id, hdlr, pn, uid, ctlr, sdk, isInExternal);
    }

    public FocusRequester getHwFocusRequester(AudioFocusInfo afi, IAudioFocusDispatcher afl, IBinder source, AudioFocusDeathHandler hdlr, MediaFocusControl ctlr, boolean isInExternal) {
    ...
    public HWExtMotionRotationProcessorEx getHWExtMotionRotationProcessor(WindowOrientationListenerProxy proxy) {

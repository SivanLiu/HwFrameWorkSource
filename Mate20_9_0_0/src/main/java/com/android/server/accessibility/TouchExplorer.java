...
class TouchExplorer extends BaseEventStreamTransformation implements Listener {
    ...
    private final class ExitGestureDetectionModeDelayed implements Runnable {
        ...
        /* synthetic */ ExitGestureDetectionModeDelayed(TouchExplorer x0, AnonymousClass1 x1) {
            this();
        }
    ...
    public boolean onDoubleTap(MotionEvent event, int policyFlags) {
        ...
        PointerCoords[] coords2 = coords;
        MotionEvent click_event = MotionEvent.obtain(event.getDownTime(), event.getEventTime(), 0, 1, properties, coords2, 0, 0, 1.0f, 1.0f, event.getDeviceId(), 0, event.getSource(), event.getFlags());
        if (result == 1) {
        ...
        if (actionMasked != 5) {
            ...
            switch (actionMasked) {
                ...
                case 1:
                    ...
                    return;
                case 2:
                    actionMasked = receivedTracker.getPrimaryPointerId();
                    int pointerIndex = event.findPointerIndex(actionMasked);
                    pointerIdBits = 1 << actionMasked;
                    switch (event.getPointerCount()) {
                        ...
                        case 2:
                            if (this.mSendHoverEnterAndMoveDelayed.isPending()) {
                                ...
                                if (Math.hypot((double) (receivedTracker.getReceivedPointerDownX(actionMasked) - rawEvent.getX(pointerIndex)), (double) (receivedTracker.getReceivedPointerDownY(actionMasked) - rawEvent.getY(pointerIndex))) >= ((double) this.mDoubleTapSlop)) {
                                    sendHoverExitAndTouchExplorationGestureEndIfNeeded(policyFlags);
                                } else {
                                    return;
                                }
                            ...
                            if (isDraggingGesture(event)) {
                                ...
                                this.mDraggingPointerId = actionMasked;
    ...
    private void handleMotionEventStateDragging(MotionEvent event, int policyFlags) {
        ...
        if (event.findPointerIndex(this.mDraggingPointerId) == -1) {
            String str = LOG_TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("mDraggingPointerId doesn't match any pointers on current event. mDraggingPointerId: ");
            stringBuilder.append(Integer.toString(this.mDraggingPointerId));
    ...
    private void sendMotionEvent(MotionEvent prototype, int action, int pointerIdBits, int policyFlags) {
        ...
        if (pointerIdBits == -1) {
            ...
            try {
                ...
                String str = LOG_TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("sendMotionEvent: Failed to split motion event: ");
    ...
    private MotionEvent offsetEvent(MotionEvent event, int offsetX, int offsetY) {
        ...
        for (int i3 = 0; i3 < pointerCount; i3++) {
            ...
            if (i3 == remappedIndex) {
                PointerCoords pointerCoords = coords[i3];
                pointerCoords.x += (float) i;
                pointerCoords = coords[i3];
                pointerCoords.y += (float) i2;
            }
        }

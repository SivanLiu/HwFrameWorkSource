...
public class MagnificationController implements Callback {
    ...
    private static class SpecAnimationBridge implements AnimatorUpdateListener {
        ...
        /* synthetic */ SpecAnimationBridge(Context x0, Object x1, WindowManagerInternal x2, ValueAnimator x3, AnonymousClass1 x4) {
            this(x0, x1, x2, x3);
        }
        ...
        synchronized (this.mLock) {
            ...
            try {
                if (this.mRegistered) {
                    f = scale;
                    try {
                        scale2 = MathUtils.constrain(f, 1.0f, 5.0f);
                        ...
                        float centerX = normPivotX + ((((((((float) viewport.width()) / DEFAULT_MAGNIFICATION_SCALE) - spec.offsetX) + ((float) viewport.left)) / oldScale) - normPivotX) * (oldScale / scale2));
                        float centerY = normPivotY + ((((((((float) viewport.height()) / DEFAULT_MAGNIFICATION_SCALE) - spec.offsetY) + ((float) viewport.top)) / oldScale) - normPivotY) * (oldScale / scale2));
                        this.mIdOfLastServiceToMagnify = id;
                        boolean scaleAndCenterLocked = setScaleAndCenterLocked(scale2, centerX, centerY, animate, id);
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case 1:
                ...
                boolean animate = z;
                this.mSpecAnimationBridge.updateSentSpecMainThread(msg.obj, animate);
                break;
            case 2:
                resetIfNeeded(false);
                break;
    ...
    public String toString() {
        ...
        stringBuilder.append("MagnificationController{mCurrentMagnificationSpec=");
        stringBuilder.append(this.mCurrentMagnificationSpec);
        stringBuilder.append(", mMagnificationRegion=");
        stringBuilder.append(this.mMagnificationRegion);
        stringBuilder.append(", mMagnificationBounds=");
        stringBuilder.append(this.mMagnificationBounds);
        stringBuilder.append(", mUserId=");
        stringBuilder.append(this.mUserId);
        stringBuilder.append(", mIdOfLastServiceToMagnify=");
        stringBuilder.append(this.mIdOfLastServiceToMagnify);
        stringBuilder.append(", mRegistered=");
        stringBuilder.append(this.mRegistered);
        stringBuilder.append(", mUnregisterPending=");
        stringBuilder.append(this.mUnregisterPending);

...
public class KeyboardInterceptor extends BaseEventStreamTransformation implements Callback {
    ...
    private void addEventToQueue(KeyEvent event, int policyFlags, long delay) {
        ...
        KeyEventHolder obtain;
        if (this.mEventQueueStart == null) {
            obtain = KeyEventHolder.obtain(event, policyFlags, dispatchTime);
        ...
        obtain = KeyEventHolder.obtain(event, policyFlags, dispatchTime);
        obtain.next = this.mEventQueueStart;
        this.mEventQueueStart.previous = obtain;
        this.mEventQueueStart = obtain;
    }

...
final class MemoryStatUtil {
    ...
    static MemoryStat parseMemoryStatFromMemcg(String memoryStatContents) {
        ...
        memoryStat.pgmajfault = m.find() ? Long.valueOf(m.group(1)).longValue() : 0;
        m = RSS_IN_BYTES.matcher(memoryStatContents);
        memoryStat.rssInBytes = m.find() ? Long.valueOf(m.group(1)).longValue() : 0;
        m = CACHE_IN_BYTES.matcher(memoryStatContents);
        memoryStat.cacheInBytes = m.find() ? Long.valueOf(m.group(1)).longValue() : 0;
        m = SWAP_IN_BYTES.matcher(memoryStatContents);
        if (m.find()) {
            j = Long.valueOf(m.group(1)).longValue();
        }

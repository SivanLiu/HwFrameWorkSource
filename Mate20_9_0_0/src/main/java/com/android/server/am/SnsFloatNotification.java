...
public class SnsFloatNotification extends FrameLayout {
    ...
    private static final boolean sHasNotch = (TextUtils.isEmpty(SystemProperties.get("ro.config.hw_notch_size", "")) ^ 1);
    ...
    public void initLayout() {
        if (sHasNotch && getResources().getConfiguration().orientation == 1) {
            this.mMarginTop = getContext().getResources().getDimensionPixelSize(34472309);
        } else {
            this.mMarginTop = getContext().getResources().getDimensionPixelSize(34472308);
        }
    ...
    public void addNotificationView(View view) {
        ...
        addView(view);
        this.mLp = new LayoutParams(-2, -2, 2003, 40, -3);
    ...
    private void flying(final boolean scroll, float current, float target, final boolean dismiss) {
            ...
            this.mAnimator.addListener(new AnimatorListenerAdapter() {
                public void onAnimationEnd(Animator animation) {
                    if (dismiss) {
                        ...
                        stringBuilder.append(manager.getPkgName());
                        stringBuilder.append("}");
                        Reporter.e(access$200, 802, stringBuilder.toString());
                        manager.cancelRemoveRunnable();
                        manager.setReadyToShowActivity(false);
                        SnsFloatNotification.this.removeNotificationView();
                    }
                    SnsFloatNotification.this.mAnimating = false;
                }
        ...
        if (dismiss) {
            ...
            stringBuilder.append(manager.getPkgName());
            stringBuilder.append("}");
            Reporter.e(context, 802, stringBuilder.toString());
            manager.cancelRemoveRunnable();
            manager.setReadyToShowActivity(false);

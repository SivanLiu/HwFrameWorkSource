...
class AppWarnings {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2:
                    ...
                    return;
                case 3:
                    AppWarnings.this.showUnsupportedCompileSdkDialogUiThread(msg.obj);
                    return;
                case 4:
                    AppWarnings.this.hideDialogsForPackageUiThread(msg.obj);
                    return;
                case 5:
                    AppWarnings.this.showDeprecatedTargetSdkDialogUiThread(msg.obj);
                    return;
                default:
                    return;
            }
    ...
    public void showUnsupportedCompileSdkDialogIfNeeded(ActivityRecord r) {
        if (r.appInfo.compileSdkVersion != 0 && r.appInfo.compileSdkVersionCodename != null && this.mAlwaysShowUnsupportedCompileSdkWarningActivities.contains(r.realActivity)) {
            ...
            int platformSdk = VERSION.SDK_INT;
            boolean isCompileSdkPreview = "REL".equals(r.appInfo.compileSdkVersionCodename) ^ 1;
            boolean isPlatformSdkPreview = "REL".equals(VERSION.CODENAME) ^ 1;
            if ((isCompileSdkPreview && compileSdk < platformSdk) || ((isPlatformSdkPreview && platformSdk < compileSdk) || (isCompileSdkPreview && isPlatformSdkPreview && platformSdk == compileSdk && !VERSION.CODENAME.equals(r.appInfo.compileSdkVersionCodename)))) {
    ...
    public void showDeprecatedTargetDialogIfNeeded(ActivityRecord r) {
        if (r.appInfo.targetSdkVersion < VERSION.MIN_SUPPORTED_TARGET_SDK_INT) {
    ...
    private int getPackageFlags(String name) {
        int intValue;
        synchronized (this.mPackageFlags) {
            intValue = ((Integer) this.mPackageFlags.getOrDefault(name, Integer.valueOf(0))).intValue();
        }
        return intValue;
    }
    ...
    private void writeConfigToFileAmsThread() {
        synchronized (this.mPackageFlags) {
            HashMap<String, Integer> packageFlags = new HashMap(this.mPackageFlags);
        }
        ...
        try {
            ...
            for (Entry<String, Integer> entry : packageFlags.entrySet()) {

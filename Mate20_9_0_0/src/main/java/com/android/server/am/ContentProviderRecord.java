...
public final class ContentProviderRecord implements WithComponentName {
    ...
    private class ExternalProcessHandle implements DeathRecipient {
        ...
        public ExternalProcessHandle(IBinder token) {
            ...
            try {
                ...
                stringBuilder.append("Couldn't register for death for token: ");
    ...
    public ContentProviderRecord(ActivityManagerService _service, ProviderInfo _info, ApplicationInfo ai, ComponentName _name, boolean _singleton) {
        ...
        if (_info.processName != null) {
            Iterator it = NEED_RELEASE_PROVIDERS.iterator();
            while (it.hasNext()) {
                if (_info.processName.contains((String) it.next())) {
                    String str = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("Install provider in app ui process, name:");
        ...
        boolean z = (this.uid == 0 || this.uid == 1000) && !releaseProvider;

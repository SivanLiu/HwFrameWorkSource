...
public final class HwActivityStartInterceptor extends ActivityStartInterceptor {
    ...
    private boolean interceptStartActivityForAppLock(Intent intent) {
        if ((this.mUserId != 0 && !UserManagerService.getInstance().getUserInfo(this.mUserId).isClonedProfile()) || this.mSupervisor.getKeyguardController().isKeyguardLocked() || !this.mSupervisor.isAppInLockList(intent.getComponent().getPackageName())) {
            return false;
        }
    ...
    IntentSender makeIntentSender(Intent intent) {

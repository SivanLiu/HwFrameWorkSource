    ...
    private void uploadExceptionLog(int bugType, int sceneDef, String pkg, String apkVersion, String body, Bundle extras) {
        try {
            if (!isPackageUploaded(sceneDef, pkg, extras.getString(KEY_ACTION, null))) {
                ...
                if (sceneDef == SCENE_DEF_BROADCAST_OVERLENGTH) {
                    Flog.i(104, "Trigger order broadcast queue overlength radar upload.");
                } else if (sceneDef != 2803) {
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Trigger broadcast radar upload for scene ");
                    stringBuilder.append(sceneDef);
                    stringBuilder.append(".");
                    Flog.i(104, stringBuilder.toString());
                } else {
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Trigger receiver timeout radar upload. Receiver[");
                    stringBuilder.append(extras.getString(KEY_RECEIVER, Shell.NIGHT_MODE_STR_UNKNOWN));
                    stringBuilder.append("] from ");
                    stringBuilder.append(pkg);
                    stringBuilder.append(" receiving ");
                    stringBuilder.append(extras.getParcelable(KEY_BROADCAST_INTENT));
                    stringBuilder.append(" tooks ");
                    stringBuilder.append(extras.getFloat(KEY_RECEIVE_TIME, -1.0f));
                    stringBuilder.append("s.");
                    Flog.i(104, stringBuilder.toString());
                }
                mLogException.msg(CATEGORY, 65, header.toString(), body);
                stringBuilder = new StringBuilder();
                stringBuilder.append("Radar upload for Package: ");
                stringBuilder.append(pkg);

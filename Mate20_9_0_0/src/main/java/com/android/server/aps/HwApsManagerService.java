...
public class HwApsManagerService extends Stub {
                ...
                try {
                    if (intent.getAction() != null) {
                        if (intent.getAction().equals(UiModeManager.ACTION_ENTER_CAR_MODE) || intent.getAction().equals(UiModeManager.ACTION_EXIT_CAR_MODE)) {
                            ...
                            String str = HwApsManagerService.TAG;
                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.append("APS: HwApsManagerService.BroadcastReceiver.onRecive, now is in car mode? = ");
                            stringBuilder.append(HwApsManagerService.this.mInCarMode);
                            Slog.i(str, stringBuilder.toString());
                        }
                    }
                } catch (RemoteException e) {
                    String str2 = HwApsManagerService.TAG;
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("APS: HwApsManagerService.BroadcastReceiver.onRecive, remote exception is thrown. e = ");
    ...
    private final class LocalService implements IApsManager {
        ...
        /* synthetic */ LocalService(HwApsManagerService x0, AnonymousClass1 x1) {
            this();
        }
        ...
        public int setColorPlusPkgList(List<String> list) {
            return -1;
        }
    ...
    public HwApsManagerService(Context context) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("APS ManagerService.screen longSize");
        stringBuilder.append(this.longSize);
        stringBuilder.append(",shortSize=");
        stringBuilder.append(this.shortSize);
    ...
    private boolean validBitmask(int bitmask) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("invalide param bitmask =");
        stringBuilder.append(bitmask);
        Slog.e(str, stringBuilder.toString());
        return false;
    }
    ...
    public boolean isSupportApsColorPlus(String pkgName) {
        boolean isSupportApsColorPlusLocked;
        enforceNotIsolatedCaller("isSupportApsColorPlus");
        synchronized (this) {
            isSupportApsColorPlusLocked = this.mHwApsManagerServiceConfig.isSupportApsColorPlusLocked(pkgName);
        }
        return isSupportApsColorPlusLocked;
    }

    public int setColorPlusPkgList(List<String> pkgList) {
        if (!checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "setColorPlusPkgList")) {
            return -3;
        }
        int colorPlusPkgListLocked;
        synchronized (this) {
            colorPlusPkgListLocked = this.mHwApsManagerServiceConfig.setColorPlusPkgListLocked(pkgList);
        }
        return colorPlusPkgListLocked;
    }
    ...
    public int setDescentGradeResolution(String pkgName, int reduceLevel, boolean switchable) {
        ...
        if (this.shortSize == 1080 && this.longSize == 1920) {
            if (reduceLevel == 1) {
                ...
                stringBuilder.append("setDescentGradeResolution invalid param =");
                stringBuilder.append(reduceLevel);
                Slog.e(str, stringBuilder.toString());
                return -1;
            }
    ...
    public boolean stopPackages(List<String> pkgs) {
        ...
        for (String pkgName : pkgs) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("forceStopPackage=");

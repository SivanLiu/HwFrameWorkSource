...
public class NetManager {
    ...
    private void onProcessAppForeground(int pid, int uid, boolean isFg) {
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("ProcessAppForeground uid:");
        stringBuilder.append(uid);
        stringBuilder.append(", isFg:");
        stringBuilder.append(isFg);
    ...
    private void appendDownloadAppUid(int uid, int eventType) {
        if (isAppUid(uid)) {
            ...
            if (eventType == 1) {
                ...
                stringBuilder.append("uid:");
                stringBuilder.append(uid);
                stringBuilder.append(" enters download");
                AwareLog.d(str, stringBuilder.toString());
                ...
                stringBuilder.append("uid:");
                stringBuilder.append(uid);
                stringBuilder.append(" exits download");
                AwareLog.d(str, stringBuilder.toString());
                this.mDownloadUidSet.remove(Integer.valueOf(uid));
                removeBgUid(uid);
            }
    ...
    private void appendAudioAppUid(int uid, int eventType) {
        if (isAppUid(uid)) {
            ...
            if (eventType == 1) {
                ...
                stringBuilder.append("uid:");
                stringBuilder.append(uid);
                stringBuilder.append(" enters audio out");
                AwareLog.d(str, stringBuilder.toString());
                ...
                stringBuilder.append("uid:");
                stringBuilder.append(uid);
                stringBuilder.append(" exits audio out");
                AwareLog.d(str, stringBuilder.toString());
                this.mAudioUidSet.remove(Integer.valueOf(uid));
                addBgUid(uid);
            }
    ...
    private void appendVisiblePid(int msg, int pid, int mode, int winType) {
        ...
        if (msg == 0 || msg == 1 || msg == 3 || msg == 4) {
            if (deleteVisiblePid(pid, winType)) {
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("appendVisiblePid delete pid:");
                stringBuilder.append(pid);
                stringBuilder.append(" from visible window");
                AwareLog.d(str, stringBuilder.toString());
                onVisiblePidsDeleted();
            }
            ...
            str = TAG;
            stringBuilder = new StringBuilder();
            stringBuilder.append("appendVisiblePid add pid:");
            stringBuilder.append(pid);
            stringBuilder.append(" to visible window");
            AwareLog.d(str, stringBuilder.toString());
            onVisiblePidsAdded();
        }
    }
    ...
    private boolean addVisiblePid(int pid, int mode, int winType) {
        ...
        if (value != -1) {
            if (value != 1 || isVisibleAllowed(mode)) {

...
public class HwIpcChecker implements IHwIpcChecker, Runnable {
    ...
    private boolean isOverdueLocked() {
        ...
        if (Utils.DEBUG || Utils.HWFLOW) {
            ...
            stringBuilder.append("isOverdueLocked ");
            stringBuilder.append(flag);
            stringBuilder.append(" waittime");
            stringBuilder.append(this.mWaitMax);
    ...
    public IHwIpcMonitor getCurrentIpcMonitor() {
        if (isOverdueLocked()) {
            ...
            if (this.mCurrentMonitor != null) {
                ...
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("recovery for ipc monitor [");
    ...
    public void run() {
        ...
        if (Utils.DEBUG || Utils.HWFLOW) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Ipc Monitor size ");
            stringBuilder.append(size);
            Log.i(str, stringBuilder.toString());
        }
        for (int i = 0; i < size; i++) {
            synchronized (this.mLock) {
                this.mCurrentMonitor = (IHwIpcMonitor) this.mHwIpcMonitors.get(i);
            }
            if (Utils.DEBUG || Utils.HWFLOW) {

...
class AppHibernateMgr {
    ...
    private ByteBuffer creatMsg(HiberBean hiberBean) {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(8 + msgBodyLen);
        try {
    ...
    private int[] parseData(byte[] buf) {
        ...
        if (4 == func_Id) {
            ...
            stringBuilder.append(func_Id);
            stringBuilder.append(" is not HIBER_MANAGER_CMD_DUMP/_RECLAIM, Neglect!");
    ...
    private int[] parseRecliamRes(Parcel recvMsgParcel) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("reclaim part success, be interrupted process number = ");
        stringBuilder.append(len);
        stringBuilder.append(" (, max effectNum = ");
        stringBuilder.append(effectNum);

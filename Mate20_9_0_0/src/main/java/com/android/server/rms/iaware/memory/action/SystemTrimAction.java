...
public class SystemTrimAction extends Action {
    ...
    public int execute(Bundle extras) {
        if (extras == null) {
            ...
            stringBuilder.append("system trim function can be excuted on middle or low level, level=");
            stringBuilder.append(DeviceInfo.getDeviceLevel());
            ...
            stringBuilder2.append("generic system memory cost time:");
    ...
    private void addProcToListForTrim(Set<ProcessInfo> procInfos) {
        for (ProcessInfo proc : procInfos) {
            if (!(proc == null || proc.mPackageName == null)) {
                if (!proc.mPackageName.isEmpty()) {
                    String packageName = (String) proc.mPackageName.get(0);
                    if (mPackageNames.containsKey(packageName)) {
                        if (SystemClock.elapsedRealtime() - ((Long) mPackageNames.get(packageName)).longValue() > 1800000) {
    ...
    private void doTrim() {
        synchronized (mProcessInfos) {
            ...
            ProcessInfo processInfo = (ProcessInfo) mProcessInfos.remove(0);

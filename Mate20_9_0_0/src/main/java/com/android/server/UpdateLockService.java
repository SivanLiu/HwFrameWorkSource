...
public class UpdateLockService extends Stub {
    ...
    void sendLockChangedBroadcast(boolean state) {
        ...
        try {
            this.mContext.sendStickyBroadcastAsUser(new Intent("android.os.UpdateLock.UPDATE_LOCK_CHANGED").putExtra("nowisconvenient", state).putExtra(WatchlistEventKeys.TIMESTAMP, System.currentTimeMillis()).addFlags(67108864), UserHandle.ALL);
        } finally {

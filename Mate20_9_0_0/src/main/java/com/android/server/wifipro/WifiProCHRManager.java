...
public class WifiProCHRManager {
    ...
    public static final String WIFIPRO_STATISTICS_MANAGER_CLASS = "com.android.server.wifi.wifipro.WifiProStatisticsManager";
    ...
    private static Object wifiproStatisticObj = null;
    ...
    private static void init() {
        ...
        try {
            ...
            stringBuilder.append("WifiProCHRManager, init(), Exception msg = ");
    ...
    private void invokeMethod(boolean betaUserOnly, String methodName, Object... args) {
        if ((!betaUserOnly || !isCommercialUser) && chrManagerObj != null && methodName != null) {
            try {
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("WifiProCHRManager, invokeMethod(), Exception msg = ");
                stringBuilder.append(e.getMessage());
                loge(stringBuilder.toString());
            }
        }
    ...
    public void updateWifiException(int ucErrorCode, String ucSubErrorCode) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("updateWifiException call for err code:");
        stringBuilder.append(ucErrorCode);
        stringBuilder.append(", sub err code:");
        stringBuilder.append(ucSubErrorCode);
    ...
    private static void initCommercialInterface() {
        try {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("initCommercialInterface, Exception msg = ");
            stringBuilder.append(e.getMessage());
            loge(stringBuilder.toString());
        }
    }
        ...
        synchronized (wsm) {
            if (commercialUserMethod == null || logManagerObj == null) {
                ...
                try {
                    ...
                    stringBuilder.append("get isCommercialUser = ");
                    stringBuilder.append(isCommercialUser);
                    logd(stringBuilder.toString());
                } catch (Exception e) {
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("isCommercialUser, Exception msg = ");
                    stringBuilder.append(e.getMessage());
                    loge(stringBuilder.toString());
                }
            }
    ...
    public void updateWifiproTimeLen(short timeLen) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("updateWifiproTimeLen:");
        stringBuilder.append(timeLen);
        logd(stringBuilder.toString());
    ...
    private void getWifiProStaticsticsManager() {
        try {
            wifiproStatisticObj = Class.forName(WIFIPRO_STATISTICS_MANAGER_CLASS).getDeclaredMethod(LOG_GET_INSTANCE_API_NAME, new Class[0]).invoke(null, new Object[0]);
        } catch (Exception e) {
            loge("WifiProStatisticsManager, invokeMethod() error, can't get WifiProStaticsticsManager!");
        }
    }

    public void increasePortalUnauthCount() {
        if (wifiproStatisticObj == null) {
            getWifiProStaticsticsManager();
        }
        if (wifiproStatisticObj != null) {
            try {
                Method method = wifiproStatisticObj.getClass().getMethod("increasePortalUnauthCount", null);
                method.setAccessible(true);
                method.invoke(wifiproStatisticObj, null);
            } catch (Exception e) {
                loge("WifiProStatisticsManager, invokeMethod() error, can't increase PortalUnauthCount");
            }
        }
    }
    ...
    public void increasePortalNoAutoConnCnt() {
        ...
        if (wifiproStatisticObj != null) {
            try {
                Method method = wifiproStatisticObj.getClass().getMethod("increasePortalNoAutoConnCnt", null);
                method.setAccessible(true);
                method.invoke(wifiproStatisticObj, null);
            } catch (Exception e) {
                loge("WifiProStatisticsManager, invokeMethod() error, can't increase PortalNoAutoConnCnt ");
            }
        }
    }

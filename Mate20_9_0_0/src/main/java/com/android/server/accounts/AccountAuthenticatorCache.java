...
class AccountAuthenticatorCache extends RegisteredServicesCache<AuthenticatorDescription> implements IAccountAuthenticatorCache {
    ...
    public AuthenticatorDescription parseServiceAttributes(Resources res, String packageName, AttributeSet attrs) {
        ...
        try {
            ...
            AuthenticatorDescription isEmpty = TextUtils.isEmpty(accountType);
            if (isEmpty == true) {
                isEmpty = null;
                return isEmpty;
            }
            isEmpty = new AuthenticatorDescription(accountType, packageName, labelId, iconId, smallIconId, prefId, customTokens);
            sa.recycle();
            return isEmpty;
        } finally {
            sa.recycle();

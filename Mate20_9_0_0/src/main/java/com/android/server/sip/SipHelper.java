...
class SipHelper {
    ...
    private CSeqHeader createCSeqHeader(String method) throws ParseException, InvalidArgumentException {
        return this.mHeaderFactory.createCSeqHeader((long) (Math.random() * 4666723172467343360L), method);
    }
    ...
    public ClientTransaction sendOptions(SipProfile caller, SipProfile callee, String tag) throws SipException {
        ...
        if (caller == callee) {
            try {
                e = createRequest("OPTIONS", caller, tag);
            } catch (Exception e2) {
                throw new SipException("sendOptions()", e2);
            }
        }
        e2 = createRequest("OPTIONS", caller, callee, tag);
        ClientTransaction clientTransaction = this.mSipProvider.getNewClientTransaction(e2);
    ...
    private Request createRequest(String requestType, SipProfile userProfile, String tag) throws ParseException, SipException {
        SipProfile sipProfile = userProfile;
        FromHeader fromHeader = createFromHeader(sipProfile, tag);
        ToHeader toHeader = createToHeader(sipProfile);
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(userProfile.getUserName());
        stringBuilder.append("@");
        SipURI requestURI = this.mAddressFactory.createSipURI(userProfile.getUriString().replaceFirst(Pattern.quote(stringBuilder.toString()), ""));

...
class SipWakeupTimer extends BroadcastReceiver {
    ...
    private static class MyEvent {
        ...
        public String toString() {
            ...
            stringBuilder.append(":");
            stringBuilder.append(toString(this.mCallback));
    ...
    private void recalculatePeriods() {
        if (!this.mEventQueue.isEmpty()) {
            ...
            Iterator it = this.mEventQueue.iterator();
            while (it.hasNext()) {
    ...
    public synchronized void onReceive(Context context, Intent intent) {
        if (getAction().equals(intent.getAction()) && intent.getExtras().containsKey(TRIGGER_TIME)) {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("onReceive: unrecognized intent: ");
    ...
    private void printQueue() {
        ...
        Iterator it = this.mEventQueue.iterator();
        while (it.hasNext()) {
            MyEvent event = (MyEvent) it.next();
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("     ");
            stringBuilder.append(event);
            stringBuilder.append(": scheduled at ");
            stringBuilder.append(showTime(event.mTriggerTime));
            stringBuilder.append(": last at ");
            stringBuilder.append(showTime(event.mLastTriggerTime));
            log(stringBuilder.toString());
            count++;
            if (count >= 5) {
                break;

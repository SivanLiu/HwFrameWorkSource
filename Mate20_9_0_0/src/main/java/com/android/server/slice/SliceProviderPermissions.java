...
public class SliceProviderPermissions implements DirtyTracker, Persistable {
    ...
    public static class SliceAuthority implements Persistable {
        ...
        public synchronized void writeTo(XmlSerializer out) throws IOException {
            ...
            for (int i = 0; i < N; i++) {
                ...
                out.text(((PkgUser) this.mPkgs.valueAt(i)).toString());
        ...
        public synchronized void readFrom(XmlPullParser parser) throws IOException, XmlPullParserException {
            ...
            while (parser.getDepth() >= depth) {
                if (parser.getEventType() == 2 && AbsLocationManagerService.DEL_PKG.equals(parser.getName())) {
                    this.mPkgs.add(new PkgUser(parser.nextText()));
                }
                parser.next();
            }
        }
    ...
    public SliceProviderPermissions(PkgUser pkg, DirtyTracker tracker) {
        this.mPkg = pkg;
        this.mTracker = tracker;
    }
    ...
    public static SliceProviderPermissions createFrom(XmlPullParser parser, DirtyTracker tracker) throws XmlPullParserException, IOException {
        ...
        int depth = parser.getDepth();
        SliceProviderPermissions provider = new SliceProviderPermissions(new PkgUser(parser.getAttributeValue(NAMESPACE, AbsLocationManagerService.DEL_PKG)), tracker);
        parser.next();
        while (parser.getDepth() > depth) {

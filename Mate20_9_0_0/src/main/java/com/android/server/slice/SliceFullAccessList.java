...
public class SliceFullAccessList {
    ...
    public boolean hasFullAccess(String pkg, int userId) {
        ArraySet<String> pkgs = (ArraySet) this.mFullAccessPkgs.get(userId, null);
    ...
    public void writeXml(XmlSerializer out, int user) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            ArraySet<String> pkgs = (ArraySet) this.mFullAccessPkgs.valueAt(i);
            if (user == -1 || user == userId) {
                ...
                if (pkgs != null) {
                    ...
                    for (int j = 0; j < M; j++) {
                        ...
                        out.text((String) pkgs.valueAt(j));
                        out.endTag(null, "pkg");
                    }
                }
                ...
                if (type != 3 || !TAG_LIST.equals(tag)) {
                    if (type == 2 && TAG_USER.equals(tag)) {
                        ...
                        while (true) {
                            ...
                            if (type == 3 && TAG_USER.equals(userTag)) {
                                break;
                            } else if (type == 2 && "pkg".equals(userTag)) {

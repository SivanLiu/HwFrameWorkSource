...
public class HwLbsLogger implements IHwLbsLogger {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case HwArbitrationDEFS.MSG_INSTANT_PAY_APP_END /*112*/:
                    break;
                case 101:
                    sessionBundle = msg.obj;
                    if (sessionBundle != null) {
                        SessionRecord sessionRecord = null;
                        String action = sessionBundle.getString("action");
                        if (action.equals(HwLbsLogger.SESSION_START)) {
                            sessionRecord = new SessionRecord(sessionBundle.getLong(HwLbsLogger.BUNDLE_PARAMETER_TIMESTAMP), action, sessionBundle.getString("pkgName"), sessionBundle.getString(HwLbsLogger.BUNDLE_PARAMETER_SESSION_PROVIDER), sessionBundle.getInt(HwLbsLogger.BUNDLE_PARAMETER_SESSION_INTERVAL), sessionBundle.getInt("receiver"));
                        } else if (action.equals(HwLbsLogger.SESSION_STOP)) {
                            sessionRecord = new SessionRecord(sessionBundle.getLong(HwLbsLogger.BUNDLE_PARAMETER_TIMESTAMP), action, sessionBundle.getString("pkgName"), sessionBundle.getInt("receiver"));
                        }
                ...
                case HwArbitrationDEFS.MSG_INSTANT_PAY_APP_START /*111*/:
                    sessionBundle = msg.obj;
                    if (sessionBundle != null) {
                        HwLbsLogger.this.saveLocationResultRecord(new LocationResultRecord((Location) sessionBundle.getParcelable(HwLbsLogger.BUNDLE_PARAMETER_LOCATION), sessionBundle.getLong(HwLbsLogger.BUNDLE_PARAMETER_TIMESTAMP), sessionBundle.getIntegerArrayList(HwLbsLogger.BUNDLE_PARAMETER_LOCATION_RECEIVERS)));
                        break;
                    }
        ...
        public LocationResultRecord(Location location, long timestamp, List<Integer> receivers) {
            ...
            if (receivers != null) {
                ...
                for (int i = 0; i < size; i++) {
                    this.mReceivers.add((Integer) receivers.get(i));
                }
            }
        }
    ...
    private void saveSessionRecord(SessionRecord sr) {

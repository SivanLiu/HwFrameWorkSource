...
class ContextHubServiceUtil {
    ...
    static byte[] createPrimitiveByteArray(ArrayList<Byte> array) {
        ...
        for (int i = 0; i < array.size(); i++) {
            primitiveArray[i] = ((Byte) array.get(i)).byteValue();
        }
    ...
    static int[] createPrimitiveIntArray(Collection<Integer> collection) {
        ...
        int i = 0;
        for (Integer contextHubId : collection) {
            int i2 = i + 1;
            primitiveArray[i] = contextHubId.intValue();
            i = i2;
        }
        return primitiveArray;
    }
    ...
    static NanoAppMessage createNanoAppMessage(ContextHubMsg message) {
        return NanoAppMessage.createMessageFromNanoApp(message.appName, message.msgType, createPrimitiveByteArray(message.msg), message.hostEndPoint == (short) -1);
    }

    ...
    private ServiceInfo findService(String packageName, int userId) {
        ...
        try {
            ...
            if (list.size() > 1) {
                ...
                stringBuilder.append("More than one DeviceAdminService's found in package ");
                stringBuilder.append(packageName);
                stringBuilder.append(".  They'll all be ignored.");
            ...
            stringBuilder2.append("DeviceAdminService ");
            stringBuilder2.append(si.getComponentName().flattenToShortString());
            stringBuilder2.append(" must be protected with ");
            stringBuilder2.append("android.permission.BIND_DEVICE_ADMIN");
    ...
    public void dump(String prefix, PrintWriter pw) {
        synchronized (this.mLock) {
            ...
            for (int i = 0; i < this.mConnections.size(); i++) {
                ...
                pw.print("User: ");
                pw.println(userId);
                DevicePolicyServiceConnection con = (DevicePolicyServiceConnection) this.mConnections.valueAt(i);
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append(prefix);
                stringBuilder.append("    ");
                con.dump(stringBuilder.toString(), pw);
            }
            pw.println();
        }
    }

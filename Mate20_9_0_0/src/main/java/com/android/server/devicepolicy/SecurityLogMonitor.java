...
class SecurityLogMonitor implements Runnable {
    ...
    private final ArrayList<SecurityEvent> mLastEvents;
    ...
    private void saveLastEvents(ArrayList<SecurityEvent> newLogs) {
        ...
        if (!newLogs.isEmpty()) {
            ...
            while (pos >= 0 && this.mLastEventNanos - ((SecurityEvent) newLogs.get(pos)).getTimeNanos() < OVERLAP_NS) {
                pos--;
            }
    ...
    public long forceLogs() {
        ...
        synchronized (this.mForceSemaphore) {
            ...
            if (toWaitNanos > 0) {
                long toMillis = TimeUnit.NANOSECONDS.toMillis(toWaitNanos) + 1;
                return toMillis;
            }

...
public class FingerPressNavigation {
        ...
        if (startInfo != null) {
            ...
            if (startInfos != null && startInfos.length == 2) {
                try {
                    ...
                    stringBuilder.append("pressure:FingerPressNavi startFingerPressActivity and this app is : ");
                    stringBuilder.append(startInfos[0]);
                    stringBuilder.append(" ; ");
                    stringBuilder.append(startInfos[1]);
                    ...
                    stringBuilder2.append(startInfos[0]);
                    stringBuilder2.append("}");
                    StatisticalUtils.reporte(context, 82, stringBuilder2.toString());
                } catch (Exception ex) {
                    String str2 = TAG;
                    StringBuilder stringBuilder3 = new StringBuilder();
                    stringBuilder3.append("get exception: ");
                ...
                if (this.mIsGameScene || this.mIsFullScreen || this.LeftPkgName == null || "none_app".equals(this.LeftPkgName) || !isHandledByLeftEdge(motionevent, x, y, pressue)) {
                    if (!this.mImmersiveMode && this.mMode == 1 && isHandledByBottomCenter(motionevent, x, y, pressue)) {
                        if (!isKeyguardLocked()) {
                            ...
                            isComsume = interceptByPressureNavi(motionevent, pressue, 3) ^ 1;
                        }
                        if (this.mAllComsume) {
                            isComsume = false;
                        }
                        ...
                        if (!isKeyguardLocked()) {
                            ...
                            isComsume = interceptByPressureNavi(motionevent, pressue, getKeyCode(0)) ^ 1;
                        ...
                        if (!isKeyguardLocked()) {
                            ...
                            isComsume = interceptByPressureNavi(motionevent, pressue, getKeyCode(1)) ^ 1;
                    ...
                    isComsume = interceptByAppNavi(motionevent, pressue, 0) ^ 1;
                }
            } else if (!(isKeyguardLocked() || isPhoneInCall() || filterAppStartEvent(motionevent))) {
                handleByAnimation(motionevent, 4);
                isComsume = interceptByAppNavi(motionevent, pressue, 1) ^ 1;
            }
    ...
    void sendEvent(int action, int flags, int code, long when) {
        InputManager.getInstance().injectInputEvent(new KeyEvent(this.mDownTime, when, action, code, 0, 0, -1, 0, (flags | 8) | 64, 257), 0);
    }
}

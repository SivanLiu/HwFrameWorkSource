...
public class HwCircleWindow {
    ...
    class hwCircle extends View implements AnimationUpdateListener {
        ...
        public boolean onTouchEvent(MotionEvent event, int type) {
            ...
            if (HwCircleWindow.this.DEBUG) {
                access$200 = HwCircleWindow.this.TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("action xxx");
                ...
                stringBuilder.append(event.getPressure());
                Log.d(access$200, stringBuilder.toString());
            }
            if (HwCircleWindow.this.DEBUG) {
                access$200 = HwCircleWindow.this.TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("pressLaunchLimit  = ");
                stringBuilder.append(HwCircleWindow.this.pressLaunchLimit);
                stringBuilder.append("  pressStartLimit  = ");
                stringBuilder.append(HwCircleWindow.this.pressStartLimit);
            ...
            if (action == 0) {
                ...
                if (!(action == this.mOldX && currentY == this.mOldY)) {
                    this.mOldX = action;
                    this.mOldY = currentY;
                    onAnimationUpdate();
                }
                if (!this.isLunched && pressure > HwCircleWindow.this.pressStartLimit) {
                    ...
                    this.mCircleAnim.startRingOutAnim(action, currentY, type);
    ...
    private void createCircleWindowLock() {
        synchronized (this.mLock) {
            if (!this.viewAdd) {
                ...
                stringBuilder.append("createCircleWindowLock viewAdd = ");
                stringBuilder.append(this.viewAdd);
    ...
    public void destoryCircleWindowAndBitmapLock() {
        synchronized (this.mLock) {
            if (this.viewAdd && this.mhwCircle != null) {
                ...
                String str = this.TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append(" destoryCircleWindowAndBitmapLock viewAdd = ");
                stringBuilder.append(this.viewAdd);
                Log.v(str, stringBuilder.toString());
                this.viewAdd = false;
            }
    ...
    void setPressureLimit(float limit) {
        if (this.DEBUG) {
            String str = this.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("hwCircleAnimation limit =  ");
    ...
    public void setImmersiveMode(boolean mode) {
        if (this.mImmersiveMode != mode) {
            ...
            if (this.DEBUG) {
                String str = this.TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("hwCircleWindow setImmersiveMode  has Changedi mode = ");
                stringBuilder.append(mode);
                Log.d(str, stringBuilder.toString());
            }
            if (this.mHandler != null) {

...
public class AwareJobSchedulerService implements AwareStateChangedListener {
    ...
    public boolean schedule(HwBroadcastRecord hwBr) {
        ...
        if (this.DEBUG) {
            ...
            stringBuilder.append("iaware_brjob schedule begin, receiver: ");
            stringBuilder.append(jobStatus.getComponentName());
            stringBuilder.append(", action: ");
            stringBuilder.append(hwBr.getAction());
    ...
    private void dumpAllJobs(PrintWriter pw) {
        synchronized (this.mLock) {
            ...
            while (it.hasNext()) {
                ((AwareJobStatus) it.next()).dump(pw);
            }
        }
    }
    ...
    private void dumpJobsForAction(PrintWriter pw, String action) {
        synchronized (this.mLock) {
            ...
            if (!hasAction) {
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("    There is no job for action: ");
                stringBuilder.append(action);
                pw.println(stringBuilder.toString());
            }
        }
    ...
    private void dumpJobsForReceiver(PrintWriter pw, String receiver) {
        synchronized (this.mLock) {
            ...
            if (!hasReceiver) {
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("    There is no job for receiver: ");

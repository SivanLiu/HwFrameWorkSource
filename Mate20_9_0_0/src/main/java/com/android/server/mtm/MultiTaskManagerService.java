...
public class MultiTaskManagerService extends Stub {
    ...
    private IProcessObserver mProcessObserver = new IProcessObserver.Stub() {
        public void onForegroundActivitiesChanged(int pid, int uid, boolean foregroundActivities) {
            if (Log.HWINFO) {
                String str = MultiTaskManagerService.TAG;
    ...
    private void enforceCallingPermission() {
        ...
        if (pid != Process.myPid() && uid != 0 && uid != 1000) {
            ...
            msg.append("Permission Denial: can not access MultiTaskManagerService! pid = ");
            msg.append(pid);
            msg.append(",uid = ");
            msg.append(uid);
            msg = msg.toString();
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        if (checkCallingPermission("android.permission.DUMP") != 0) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Permission Denial: can't dump Multi task service from from pid=");
    ...
    private void enforceCallerRootOrSys() {
        ...
        if (uid != 0 && 1000 != UserHandle.getAppId(uid)) {
            String msg = new StringBuilder();
            msg.append("Permission Denial: can not access MultiTaskManagerService! uid = ");
            msg.append(uid);
    ...
    public void requestAppClean(AppCleanSource source) {
        ...
        if (AppCleanSource.SMART_CLEAN.equals(source)) {
            ...
            if (((long) AppMngConfig.getSmartCleanInterval()) <= currTime - this.mLastCheckSmartClean) {
                ...
                this.mHandler.sendMessageDelayed(msg, HwArbitrationDEFS.NotificationMonitorPeriodMillis);
            }
        } else {
            this.mHandler.sendMessage(msg);
        }
    ...
    private void addAllProcessToMTM() {
        ...
        for (Entry<Integer, AwareProcessBaseInfo> info : allProcInfo.entrySet()) {
            this.mProcInfo.recordProcessInfo(((Integer) info.getKey()).intValue(), ((AwareProcessBaseInfo) info.getValue()).mUid);
        }
    }
}

...
public class SmartClean extends CleanSource {
    ...
    public void clean() {
        ...
        for (AwareProcessBlockInfo block : rawInfo) {
            if (!(block == null || CleanType.NONE.equals(block.mCleanType))) {
                String str = TAG;
    ...
    public List<AwareProcessBlockInfo> getSmartCleanList(List<AwareProcessBlockInfo> rawInfo) {
        ...
        List<AwareProcessBlockInfo> needClean = CleanSource.mergeBlockForMemory(rawInfo, DecisionMaker.getInstance().getProcessList(AppMngFeature.APP_CLEAN, AppCleanSource.SMART_CLEAN));
        if (needClean == null || needClean.isEmpty()) {
    ...
    private void removeNoUiBlock(List<AwareProcessBlockInfo> rawInfo, List<AwareProcessBlockInfo> needClean) {
        ...
        if (lru != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (block != null) {
                    if (((Long) lru.get(block.mPackageName)) == null) {
                        iter.remove();

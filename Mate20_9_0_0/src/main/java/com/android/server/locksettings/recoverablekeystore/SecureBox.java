...
public class SecureBox {
    ...
    private static final byte[] CONSTANT_01 = new byte[]{(byte) 1};
    ...
    private static final byte EC_PUBLIC_KEY_PREFIX = (byte) 4;
    ...
    private static final byte[] VERSION = new byte[]{(byte) 2, (byte) 0};
    ...
    public static byte[] encrypt(PublicKey theirPublicKey, byte[] sharedSecret, byte[] header, byte[] payload) throws NoSuchAlgorithmException, InvalidKeyException {
        PublicKey publicKey = theirPublicKey;
        byte[] sharedSecret2 = emptyByteArrayIfNull(sharedSecret);
        if (publicKey == null && sharedSecret2.length == 0) {
        ...
        if (publicKey == null) {
            ...
            dhSecret = dhComputeSecret(senderKeyPair.getPrivate(), publicKey);
            hkdfInfo = HKDF_INFO_WITH_PUBLIC_KEY;
        }
        byte[] ciphertext = aesGcmEncrypt(hkdfDeriveKey(concat(dhSecret, sharedSecret2), HKDF_SALT, hkdfInfo), genRandomNonce(), payload2, header2);
        ...
        if (encryptedPayload != null) {
            ...
            if (Arrays.equals(readEncryptedPayload(ciphertextBuffer, VERSION.length), VERSION)) {
                ...
                if (ourPrivateKey == null) {
                    ...
                    dhSecret2 = dhComputeSecret(ourPrivateKey, decodePublicKey(readEncryptedPayload(ciphertextBuffer, 65)));
                    dhSecret = HKDF_INFO_WITH_PUBLIC_KEY;
                }
                return aesGcmDecrypt(hkdfDeriveKey(concat(dhSecret2, sharedSecret), HKDF_SALT, dhSecret), readEncryptedPayload(ciphertextBuffer, 12), readEncryptedPayload(ciphertextBuffer, ciphertextBuffer.remaining()), header);
            }

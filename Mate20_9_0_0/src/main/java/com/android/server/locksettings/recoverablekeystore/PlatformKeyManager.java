...
public class PlatformKeyManager {
    ...
    private PlatformEncryptionKey getEncryptKeyInternal(int userId) throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, InsecureUserException {
        ...
        if (isKeyLoaded(userId, generationId)) {
            return new PlatformEncryptionKey(generationId, (AndroidKeyStoreSecretKey) this.mKeyStore.getKey(alias, null));
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("KeyStore doesn't contain key ");
    ...
    private PlatformDecryptionKey getDecryptKeyInternal(int userId) throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, InsecureUserException {
        ...
        if (isKeyLoaded(userId, generationId)) {
            return new PlatformDecryptionKey(generationId, (AndroidKeyStoreSecretKey) this.mKeyStore.getKey(alias, null));
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("KeyStore doesn't contain key ");
        stringBuilder.append(alias);
    ...
    void init(int userId) throws KeyStoreException, NoSuchAlgorithmException, InsecureUserException, IOException {
        if (isAvailable(userId)) {
            int generationId = getGenerationId(userId);
            if (isKeyLoaded(userId, generationId)) {
                Log.i(TAG, String.format(Locale.US, "Platform key generation %d exists already.", new Object[]{Integer.valueOf(generationId)}));
                return;
            }
            if (generationId == -1) {
                ...
                Log.w(TAG, String.format(Locale.US, "Platform generation ID was %d but no entry was present in AndroidKeyStore. Generating fresh key.", new Object[]{Integer.valueOf(generationId)}));
                generationId++;
            }
    ...
    private String getEncryptAlias(int userId, int generationId) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(KEY_ALIAS_PREFIX);
        stringBuilder.append(userId);
        stringBuilder.append(SliceAuthority.DELIMITER);
        stringBuilder.append(generationId);
        stringBuilder.append(SliceAuthority.DELIMITER);
        stringBuilder.append(ENCRYPT_KEY_ALIAS_SUFFIX);
    ...
    private String getDecryptAlias(int userId, int generationId) {
        ...
        stringBuilder.append(generationId);
        stringBuilder.append(SliceAuthority.DELIMITER);
        stringBuilder.append(DECRYPT_KEY_ALIAS_SUFFIX);
        return stringBuilder.toString();
    }
    ...
    private void generateAndLoadKey(int userId, int generationId) throws NoSuchAlgorithmException, KeyStoreException, IOException {
        ...
        this.mKeyStore.setEntry(decryptAlias, new SecretKeyEntry(secretKey), new Builder(2).setUserAuthenticationRequired(true).setUserAuthenticationValidityDurationSeconds(15).setBlockModes(new String[]{"GCM"}).setEncryptionPaddings(new String[]{"NoPadding"}).setBoundToSpecificSecureUserId((long) userId).build());
        this.mKeyStore.setEntry(encryptAlias, new SecretKeyEntry(secretKey), new Builder(1).setBlockModes(new String[]{"GCM"}).setEncryptionPaddings(new String[]{"NoPadding"}).build());

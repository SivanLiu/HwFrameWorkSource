...
public class KeySyncUtils {
    ...
    public static byte[] thmEncryptRecoveryKey(PublicKey publicKey, byte[] lockScreenHash, byte[] vaultParams, SecretKey recoveryKey) throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        return SecureBox.encrypt(publicKey, calculateThmKfHash(lockScreenHash), concat(THM_ENCRYPTED_RECOVERY_KEY_HEADER, vaultParams), encryptedRecoveryKey);
    }
    ...
    public static Map<String, byte[]> encryptKeysWithRecoveryKey(SecretKey recoveryKey, Map<String, SecretKey> keys) throws NoSuchAlgorithmException, InvalidKeyException {
        HashMap<String, byte[]> encryptedKeys = new HashMap();
        for (String alias : keys.keySet()) {
            encryptedKeys.put(alias, SecureBox.encrypt(null, recoveryKey.getEncoded(), ENCRYPTED_APPLICATION_KEY_HEADER, ((SecretKey) keys.get(alias)).getEncoded()));
        }
    ...
    public static byte[] decryptApplicationKey(byte[] recoveryKey, byte[] encryptedApplicationKey) throws NoSuchAlgorithmException, InvalidKeyException, AEADBadTagException {
        return SecureBox.decrypt(null, recoveryKey, ENCRYPTED_APPLICATION_KEY_HEADER, encryptedApplicationKey);
    }
    ...
    public static byte[] packVaultParams(PublicKey thmPublicKey, long counterId, int maxAttempts, byte[] vaultHandle) {
        return ByteBuffer.allocate(77 + vaultHandle.length).order(ByteOrder.LITTLE_ENDIAN).put(SecureBox.encodePublicKey(thmPublicKey)).putLong(counterId).putInt(maxAttempts).put(vaultHandle).array();
    }

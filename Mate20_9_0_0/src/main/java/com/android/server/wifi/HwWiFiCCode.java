...
public class HwWiFiCCode {
    ...
    public String getActiveCountryCode() {
        ...
        if (countryCode == null || countryCode.isEmpty()) {
            ...
            if (countryCode == null || countryCode.isEmpty()) {
                ...
                if (countryCode == null || countryCode.isEmpty()) {
                    ...
                    stringBuilder.append("countryCode got by DEFAULT == ");
                ...
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("countryCode got by LOCALE_LANGUAGE == ");
                stringBuilder.append(countryCode);
                Log.d(str, stringBuilder.toString());
                return countryCode;
            }
            ...
            str = TAG;
            stringBuilder = new StringBuilder();
            stringBuilder.append("countryCode got by RECORD == ");
            stringBuilder.append(countryCode);
            Log.d(str, stringBuilder.toString());
            return countryCode;
        }
        ...
        str = TAG;
        stringBuilder = new StringBuilder();
        stringBuilder.append("countryCode got from by MCC == ");
        stringBuilder.append(countryCode);
        Log.d(str, stringBuilder.toString());
        return countryCode;
    }
    ...
    private int getNetworkMCC() {
        ...
        if (this.mContext != null) {
            regPlmn1 = TelephonyManager.from(this.mContext).getNetworkOperator(0);
        ...
        regPlmn1 = residentPlmn.substring(0, 3);
        if (!("".equals(regPlmnMcc) || regPlmnMcc.length() != 3 || regPlmnMcc.equals(regPlmn1))) {
            regPlmn1 = regPlmnMcc;
        }
        try {
            return Integer.parseInt(regPlmn1);
            ...
            while (true) {
                ...
                if (MCC_TABLE_V2_PROP_NODE_ENTRY.equalsIgnoreCase(parser.getName())) {
                    try {
                        ...
                        tmpTable.add(new MccEntry(mcc, language, smallestDigit, language2, parser.getAttributeValue(null, MCC_TABLE_V2_PROP_TIME_ZONE)));
                    } catch (Exception e3) {
                        Exception exception2 = e3;
                        str = TAG;
                        StringBuilder stringBuilder4 = new StringBuilder();
                        stringBuilder4.append("Exception in mcctable parser ");

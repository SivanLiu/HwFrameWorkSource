...
public class HwDualBandQualityEngine {
    ...
    private static final RttScore[] RTT_SCORE_TABLE = new RttScore[]{new RttScore(400, 15), new RttScore(800, 10), new RttScore(WifiproBqeUtils.BQE_GOOD_RTT, 5), new RttScore(2000, 0), new RttScore(4000, -5), new RttScore(16000, -10)};
    ...
    public int getScoreByRssi(int rssi) {
        ...
        for (int index = 0; index <= maxIndex; index++) {
            if (rssi > RSSI_SCORE_TABLE[index].RSSI) {
                ...
                stringBuilder.append("getScoreByRssi: rssi=");
    ...
    public int getScoreByConnectTime(int connectTime) {
        ...
        for (int index = 0; index <= maxIndex; index++) {
            if (connectTime > AP_CONNECT_TIME_SCORE_TABLE[index].TOTAL_CONNECT_TIME) {
                ...
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("getScoreByConnectTime: connectTime=");
    ...
    public int getScoreByRtt(int rtt) {
        ...
        for (int index = 0; index <= maxIndex; index++) {
            if (rtt < RTT_SCORE_TABLE[index].RTT) {
                ...
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("getScoreByRtt: rtt=");
    ...
    public int getScoreByLossRate(double lossRate) {
        ...
        for (int index = 0; index <= maxIndex; index++) {
            if (lossRate < LOSE_RATE_SCORE_TABLE[index].LOSE_RATE) {
                ...
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("getScoreByLossRate: lossRate=");
                stringBuilder.append(String.valueOf(lossRate));
                stringBuilder.append(" score=");
                stringBuilder.append(score);
                Log.d(str, stringBuilder.toString());
                return score;

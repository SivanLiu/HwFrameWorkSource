...
public class RttService extends SystemService {
    ...
    public void onBootPhase(int phase) {
        if (phase == 500) {
            ...
            IWifiAwareManager awareBinder = (IWifiAwareManager) ServiceManager.getService("wifiaware");
            IWifiAwareManager iWifiAwareManager = awareBinder;
            this.mImpl.start(handlerThread.getLooper(), wifiInjector.getClock(), iWifiAwareManager, new RttNative(this.mImpl, halDeviceManager), rttMetrics, wifiPermissionsUtil, wifiInjector.getFrameworkFacade());
        }
    }
}

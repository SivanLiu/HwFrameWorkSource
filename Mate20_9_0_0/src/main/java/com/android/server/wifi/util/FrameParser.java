...
public class FrameParser {
    ...
    private void parseEthernetFrame(ByteBuffer data) {
        ...
        if (etherType == ETHERTYPE_IP_V6) {
            parseIpv6Packet(data);
        } else if (etherType == ETHERTYPE_EAPOL) {
            parseEapolPacket(data);
        } else if (etherType == ETHERTYPE_IP_V4) {
    ...
    private void parseIpv4Packet(ByteBuffer data) {
        ...
        if (version != 4) {
            ...
            stringBuilder.append("IPv4 header: Unrecognized protocol version ");
        ...
        if (protocolNumber == (short) 1) {
            parseIcmpPacket(data);
        } else if (protocolNumber == (short) 6) {
            parseTcpPacket(data);
        } else if (protocolNumber == (short) 17) {
            parseUdpPacket(data);
        }
    }
    ...
    private void parseDhcpPacket(ByteBuffer data) {
        ...
        while (data.remaining() > 0) {
            ...
            if (dhcpOptionTag != (short) 0) {
                ...
                if (dhcpOptionTag != DHCP_OPTION_TAG_MESSAGE_TYPE) {
                    data.position(data.position() + dhcpOptionLen);
                } else if (dhcpOptionLen != (short) 1) {
                    String str = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("DHCP option len: ");
                    stringBuilder.append(dhcpOptionLen);
                    stringBuilder.append(" (expected |1|)");
                    Log.e(str, stringBuilder.toString());
                    return;
                } else {
    ...
    private void parseIcmpPacket(ByteBuffer data) {
        ...
        if (messageType == (short) 0) {
            this.mTypeString = "Echo Reply";
        } else if (messageType == (short) 3) {
            this.mTypeString = "Destination Unreachable";
        } else if (messageType == (short) 5) {
            this.mTypeString = "Redirect";
        } else if (messageType != (short) 8) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Type ");
            stringBuilder.append(messageType);
            this.mTypeString = stringBuilder.toString();
        } else {
    ...
    private void parseIpv6Packet(ByteBuffer data) {
        ...
        if (version != 6) {
            ...
            stringBuilder.append("IPv6 header: invalid IP version ");
            stringBuilder.append(version);
            Log.e(str, stringBuilder.toString());
            return;
        }
        ...
        if (nextHeaderType != (short) 58) {
            stringBuilder = new StringBuilder();
            stringBuilder.append("Option/Protocol ");
            stringBuilder.append(nextHeaderType);
            this.mTypeString = stringBuilder.toString();
            return;
        }
    ...
    private void parseIcmpV6Packet(ByteBuffer data) {
        ...
        if (icmpV6Type != ICMP_V6_TYPE_MULTICAST_LISTENER_DISCOVERY) {
            switch (icmpV6Type) {
                case (short) 128:
                    this.mTypeString = "Echo Request";
                    return;
                ...
                default:
                    switch (icmpV6Type) {
                        ...
                        default:
                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.append("Type ");
                            stringBuilder.append(icmpV6Type);
                            this.mTypeString = stringBuilder.toString();
                            return;
                    }
            }
    ...
    private void parseEapolPacket(ByteBuffer data) {
        ...
        if (eapolVersion < (short) 1 || eapolVersion > (short) 2) {
            str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Unrecognized EAPOL version ");
            stringBuilder.append(eapolVersion);
            Log.e(str, stringBuilder.toString());
            return;
        }
        ...
        if (eapolType != (short) 3) {
            ...
            stringBuilder2.append("Unrecognized EAPOL type ");
            stringBuilder2.append(eapolType);
        ...
        if (eapolKeyDescriptorType != (short) 2) {
            ...
            stringBuilder3.append("Unrecognized key descriptor ");
            stringBuilder3.append(eapolKeyDescriptorType);
        ...
        if ((wpaKeyInfo & 256) == 0) {
            ...
            stringBuilder4.append(this.mTypeString);
            stringBuilder4.append(" message 1/4");
            this.mTypeString = stringBuilder4.toString();
        } else if ((wpaKeyInfo & 64) != 0) {
            stringBuilder4 = new StringBuilder();
            stringBuilder4.append(this.mTypeString);
            stringBuilder4.append(" message 3/4");
            this.mTypeString = stringBuilder4.toString();
        } else {
            ...
            if (getUnsignedShort(data) > 0) {
                ...
                stringBuilder5.append(this.mTypeString);
                stringBuilder5.append(" message 2/4");
                this.mTypeString = stringBuilder5.toString();
            } else {
                stringBuilder5 = new StringBuilder();
                stringBuilder5.append(this.mTypeString);
                stringBuilder5.append(" message 4/4");
                this.mTypeString = stringBuilder5.toString();
            }
        }
    }
    ...
    private void parseManagementFrame(ByteBuffer data) {
        ...
        if (ieee80211Version != (byte) 0) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Unrecognized 802.11 version ");
            stringBuilder.append(ieee80211Version);
            Log.e(str, stringBuilder.toString());
            return;
        }
        ...
        if (ieee80211FrameType != (byte) 0) {
            String str2 = TAG;
            StringBuilder stringBuilder2 = new StringBuilder();
            stringBuilder2.append("Unexpected frame type ");
            stringBuilder2.append(ieee80211FrameType);
        ...
        switch (ieee80211FrameSubtype) {
            ...
            default:
                StringBuilder stringBuilder3 = new StringBuilder();
                stringBuilder3.append("Unexpected subtype ");
                stringBuilder3.append(ieee80211FrameSubtype);
                this.mTypeString = stringBuilder3.toString();
                return;
        }
    }
    ...
    private String decodeIeee80211StatusCode(short statusCode) {
        switch (statusCode) {
            ...
            case StatusCode.REQ_TCLAS_NOT_SUPPORTED /*80*/:
            ...
            case StatusCode.REJECT_WITH_SCHEDULE /*83*/:
                return "Reserved";
            case StatusCode.REJECT_NO_WAKEUP_SPECIFIED /*84*/:
            case StatusCode.SUCCESS_POWER_SAVE_MODE /*85*/:
            case StatusCode.PENDING_ADMITTING_FST_SESSION /*86*/:
            case StatusCode.PERFORMING_FST_NOW /*87*/:
            ...
            case StatusCode.REFUSED_AP_OUT_OF_MEMORY /*93*/:
                return "Refused; AP out of memory";
            case StatusCode.REJECTED_EMERGENCY_SERVICE_NOT_SUPPORTED /*94*/:
                return "Refused; emergency services not supported";
            case StatusCode.QUERY_RESP_OUTSTANDING /*95*/:
                return "GAS query response outstanding";
            case StatusCode.REJECT_DSE_BAND /*96*/:
            case StatusCode.TCLAS_PROCESSING_TERMINATED /*97*/:
            case StatusCode.TS_SCHEDULE_CONFLICT /*98*/:

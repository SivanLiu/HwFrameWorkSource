...
public class PasspointNetworkEvaluator implements NetworkEvaluator {
    ...
    public WifiConfiguration evaluateNetworks(List<ScanDetail> scanDetails, WifiConfiguration currentNetwork, String currentBssid, boolean connected, boolean untrustedNetworkAllowed, List<Pair<ScanDetail, WifiConfiguration>> connectableNetworks) {
        ...
        for (ScanDetail scanDetail : scanDetails) {
            if (scanDetail.getNetworkDetail() == null || scanDetail.getNetworkDetail().isInterworking()) {
                Pair<PasspointProvider, PasspointMatch> bestProvider = this.mPasspointManager.matchProvider(scanDetail.getScanResult());
                if (bestProvider != null) {
                    if (!((PasspointProvider) bestProvider.first).isSimCredential() || this.mWifiConfigManager.isSimPresent()) {
        ...
        if (currentNetwork == null || !TextUtils.equals(currentNetwork.SSID, ScanResultUtil.createQuotedSSID(bestNetwork.mScanDetail.getSSID())) || HwWifiServiceFactory.getHwWifiDevicePolicy().isWifiRestricted(currentNetwork, false)) {
            ...
            if (config != null) {
                ...
                stringBuilder.append("Passpoint network to connect to: ");
        ...
        stringBuilder = new StringBuilder();
        stringBuilder.append("Staying with current Passpoint network ");
        stringBuilder.append(currentNetwork.SSID);
        localLog(stringBuilder.toString());
    ...
    private PasspointNetworkCandidate findBestNetwork(List<PasspointNetworkCandidate> networkList, String currentNetworkSsid) {
        ...
        for (PasspointNetworkCandidate candidate : networkList) {
            ...
            int score = PasspointNetworkScore.calculateScore(match == PasspointMatch.HomeProvider ? 1 : 0, scanDetail, this.mPasspointManager.getANQPElements(scanDetail.getScanResult()), TextUtils.equals(currentNetworkSsid, ScanResultUtil.createQuotedSSID(scanDetail.getSSID())));
            if (score > bestScore) {
        ...
        if (bestCandidate != null) {
            ...
            stringBuilder.append("Best Passpoint network ");
            stringBuilder.append(bestCandidate.mScanDetail.getSSID());
            stringBuilder.append(" provided by ");
            stringBuilder.append(bestCandidate.mProvider.getConfig().getHomeSp().getFqdn());
            localLog(stringBuilder.toString());
        }

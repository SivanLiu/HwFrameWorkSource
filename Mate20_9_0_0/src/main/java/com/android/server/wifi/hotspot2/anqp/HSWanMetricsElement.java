...
public class HSWanMetricsElement extends ANQPElement {
    ...
    public static HSWanMetricsElement parse(ByteBuffer payload) throws ProtocolException {
        ...
        if (payload.remaining() == 13) {
            ...
            int downlinkLoad = payload.get() & Constants.BYTE_MASK;
            int i = payload.get() & Constants.BYTE_MASK;
            int uplinkLoad = i;
            return new HSWanMetricsElement(status, symmetric, capped, ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 4) & Constants.INT_MASK, ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 4) & Constants.INT_MASK, downlinkLoad, i, ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & Constants.SHORT_MASK);
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Unexpected buffer size: ");

...
public class WifiProApQualityRcd {
    ...
    public void putAvgRttToRecord(long rtt, int index) {
        putValueToRecord(this.mHistoryAvgRtt, getByteArray(rtt), 8 * index);
    }
    ...
    public void putRttProductToRecord(long product, int index) {
        putValueToRecord(this.mRTT_Product, getByteArray(product), 8 * index);
    }
    ...
    public void putRttVolumeToRecord(long volume, int index) {
        putValueToRecord(this.mRTT_PacketVolume, getByteArray(volume), 8 * index);
    }
    ...
    public void putLostRateToRecord(double lostRate, int index) {
        putValueToRecord(this.mOTA_LostRateValue, getByteArray(lostRate), 8 * index);
    }
    ...
    public void putLostVolumeToRecord(double volume, int index) {
        putValueToRecord(this.mOTA_PktVolume, getByteArray(volume), 8 * index);
    }
    ...
    public void putLostProductToRecord(double product, int index) {
        putValueToRecord(this.mOTA_BadPktProduct, getByteArray(product), 8 * index);
    }
}

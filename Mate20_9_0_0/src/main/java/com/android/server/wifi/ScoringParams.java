...
public class ScoringParams {
    ...
    private class Values {
        ...
        Values(Values source) {
            ...
            int i2 = 0;
            this.expid = 0;
            for (i = 0; i < this.rssi2.length; i++) {
                this.rssi2[i] = source.rssi2[i];
            }
            for (i = 0; i < this.rssi5.length; i++) {
                this.rssi5[i] = source.rssi5[i];
            }
            while (i2 < this.pps.length) {
                this.pps[i2] = source.pps[i2];
                i2++;
            }
        ...
        public void parseString(String kvList) throws IllegalArgumentException {
            ...
            stringBuilder.append(kvList);
            if (size == stringBuilder.toString().split(",").length) {
    ...
    private void loadResources(Context context) {
        ...
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Inconsistent config_wifi_framework_ resources: ");
        ...
        ContentObserver observer = new ContentObserver(handler) {
            public void onChange(boolean selfChange) {
                ...
                if (!this.update(params)) {
                    String str = ScoringParams.TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("Error in wifi_score_params: ");
                    stringBuilder.append(ScoringParams.this.sanitize(params));
                    Log.e(str, stringBuilder.toString());
                }
                Log.i(ScoringParams.TAG, this.toString());
            }
        };
        facade.registerContentObserver(context, Global.getUriFor("wifi_score_params"), true, observer);
    ...
    public boolean update(String kvList) {
        ...
        stringBuilder.append(",");
        stringBuilder.append(kvList);
        if (!stringBuilder.toString().matches(COMMA_KEY_VAL_STAR)) {
            return false;
        }
    ...
    public String sanitize(String params) {
        ...
        if (printable.length() > 100) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(printable.substring(0, 98));
            stringBuilder.append("...");
            printable = stringBuilder.toString();
        }
        return printable;
    }

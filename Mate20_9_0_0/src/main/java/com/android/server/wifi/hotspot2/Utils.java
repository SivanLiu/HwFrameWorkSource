...
public abstract class Utils {
    ...
    public static long parseMac(String s) {
        if (s != null) {
            ...
            stringBuilder.append("Bad MAC address: '");
    ...
    public static String macToString(long mac) {
        ...
        while (true) {
            ...
            sb.append(String.format("%02x", new Object[]{Long.valueOf((mac >>> n2) & 255)}));
            n = n2 - 8;
        }
    }
    ...
    public static String getMccMnc(List<String> domain) {
        ...
        int n = 0;
        for (int n2 = 0; n2 < PLMNText.length; n2++) {
            String expect = PLMNText[n2];
            if (!((String) domain.get(n2)).regionMatches(0, expect, 0, expect.endsWith("*") ? expect.length() - 1 : expect.length())) {
                return null;
            }
        }
        String prefix = new StringBuilder();
        prefix.append(((String) domain.get(2)).substring(3));
        prefix.append(((String) domain.get(3)).substring(3));
    ...
    public static String roamingConsortiumsToString(Collection<Long> ois) {
        ...
        boolean first = true;
        for (Long oi : ois) {
            long oi2 = oi.longValue();
            if (first) {
                first = false;
            } else {
                sb.append(", ");
            }
            if (Long.numberOfLeadingZeros(oi2) > 40) {
                sb.append(String.format("%06x", new Object[]{Long.valueOf(oi2)}));
            } else {
                sb.append(String.format("%010x", new Object[]{Long.valueOf(oi2)}));
            }
        }
        return sb.toString();
    }
    ...
    public static String toUnicodeEscapedString(String s) {
        ...
        for (int n = 0; n < s.length(); n++) {
            ...
            if (ch < ' ' || ch >= 127) {
                sb.append("\\u");
                sb.append(String.format("%04x", new Object[]{Integer.valueOf(ch)}));
            } else {
                sb.append(ch);
            }
        }
    ...
    public static int fromHex(char ch, boolean lenient) throws NumberFormatException {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Bad hex-character: ");

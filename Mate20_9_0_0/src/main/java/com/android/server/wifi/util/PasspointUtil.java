...
public class PasspointUtil {
    ...
    public static boolean ishs2Enabled(Context context) {
        if (context.getPackageManager().hasSystemFeature("android.hardware.wifi.passpoint") && Global.getInt(context.getContentResolver(), DBKEY_HOTSPOT20_VALUE, 1) == 1) {
            return true;
        }
        return false;
    }
    ...
    public static boolean ishs20EanbledBySim(Context context) {
        if (context != null) {
            return "true".equalsIgnoreCase(Global.getString(context.getContentResolver(), DBKEY_IS_SIM_SUPPORT_HS20));
        }
        return false;
    }
}

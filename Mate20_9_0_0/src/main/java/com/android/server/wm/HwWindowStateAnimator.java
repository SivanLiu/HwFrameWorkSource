...
public class HwWindowStateAnimator extends WindowStateAnimator {
    ...
    void computeShownFrameLocked(int type) {
        ...
        if (screenAnimation) {
            ...
            this.mShownPosition.set((int) x, (int) ((tmpFloats[5] * ratio) + pendingY));
    ...
    private boolean isMultiWindowInSingleHandMode() {
        return (this.mWin.mAttrs.type == HwArbitrationDEFS.MSG_Recovery_Flag_By_Notification && this.mLazyIsEntering && floatEqualCompare(0.8f)) || (this.mLazyIsExiting && floatEqualCompare(0.95f));
    }
    ...
    public float[] getPCDisplayModeSurfacePos(Rect tmpSize) {
        ...
        if (HwPCUtils.isPcCastModeInServer() && HwPCUtils.isValidExtDisplayId(w.getDisplayId())) {
            ...
            if (mode != 0) {
                ...
                stringBuilder.append("getPCDisplayModeSurfacePos name:");
                stringBuilder.append(w.mLastTitle);
                stringBuilder.append(" posLeft:");
                stringBuilder.append(posLeft);
                stringBuilder.append(" posTop:");
                stringBuilder.append(posTop);
    ...
    private int checkWindowType(WindowState win) {
        ...
        if (HwArbitrationDEFS.MSG_MPLINK_BIND_CHECK_OK_NOTIFY == win.getAttrs().type) {
            return 2;
        }
        return -1;
    }

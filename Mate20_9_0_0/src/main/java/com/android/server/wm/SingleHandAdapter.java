...
final class SingleHandAdapter {
    ...
    public SingleHandAdapter(Context context, Handler handler, Handler uiHandler, WindowManagerService service) {
        ...
        userFilter.addAction(HwWallpaperManagerService.WALLPAPER_MANAGER_SERVICE_READY_ACTION);
        context.registerReceiver(new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                if (HwWallpaperManagerService.WALLPAPER_MANAGER_SERVICE_READY_ACTION.equals(intent.getAction())) {
                    SingleHandAdapter.this.updateBlur();
                }
            }
        }, userFilter);
            ...
            public void run() {
                ...
                SingleHandAdapter.this.mContext.getContentResolver().registerContentObserver(System.getUriFor("single_hand_screen_zoom"), true, new ContentObserver(SingleHandAdapter.this.mHandler) {
                    public void onChange(boolean selfChange) {
                        ...
                        String str = SingleHandAdapter.TAG;
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append(" KEY_SINGLE_HAND_SCREEN_ZOOM onChange isSingleHandEnabled=");
                        stringBuilder.append(SingleHandAdapter.isSingleHandEnabled);
    ...
    private void updateSingleHandMode() {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("updateSingleHandMode value: ");
        ...
        if (IS_NOTCH_PROP) {
            ...
            mRoundedStateFlag = "".equals(value);
        ...
        if (!value.equals(this.mCurrentOverlaySetting)) {
            ...
            synchronized (mLock) {
                if (!this.mOverlays.isEmpty()) {
                    ...
                    while (it.hasNext()) {
                        ((SingleHandHandle) it.next()).dismissLocked();
                    }
    ...
    private void updateScaleWallpaperForBlur() {
        synchronized (mLock) {
            ...
            int wwidth = (int) (((float) this.mBlurBitmap.getWidth()) * 1065353216);
            ...
            if (!this.mOverlays.isEmpty()) {
                Iterator it = this.mOverlays.iterator();
                while (it.hasNext()) {
                    ((SingleHandHandle) it.next()).onBlurWallpaperChanged();
                }
            }
        }
    }

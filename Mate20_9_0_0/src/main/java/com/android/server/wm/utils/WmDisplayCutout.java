...
public class WmDisplayCutout {
    ...
    private static Rect computeSafeInsets(Size displaySize, DisplayCutout cutout) {
        ...
        if (displaySize.getWidth() < displaySize.getHeight()) {
            boundingRects = cutout.replaceSafeInsets(new Rect(0, displaySize.getHeight() / 2, 0, displaySize.getHeight() / 2)).getBoundingRects();
            ...
            boundingRects = cutout.replaceSafeInsets(new Rect(displaySize.getWidth() / 2, 0, displaySize.getWidth() / 2, 0)).getBoundingRects();
            return new Rect(findInsetForSide(displaySize, boundingRects, 3), 0, findInsetForSide(displaySize, boundingRects, 5), 0);
        } else {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("not implemented: display=");
    ...
    private static int findInsetForSide(Size display, List<Rect> boundingRects, int gravity) {
        ...
        for (int i = 0; i < size; i++) {
            Rect boundingRect = (Rect) boundingRects.get(i);
            if (gravity != 3) {
                if (gravity != 5) {
                    if (gravity != 48) {
                        if (gravity != 80) {

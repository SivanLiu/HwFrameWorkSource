...
final class HotplugDetectionAction extends HdmiCecFeatureAction {
    ...
    private void checkHotplug(List<Integer> ackedAddress, boolean audioOnly) {
        ...
        int index = -1;
        while (true) {
            nextSetBit = removed.nextSetBit(index + 1);
            index = nextSetBit;
            if (nextSetBit == -1) {
                break;
            }
            if (index == 5) {
                ...
                if (avr != null && tv().isConnected(avr.getPortId())) {
                    ...
                    stringBuilder2.append("Ack not returned from AVR. count: ");
                    stringBuilder2.append(this.mAvrStatusCount);
            ...
            stringBuilder.append("Remove device by hot-plug detection:");
        ...
        index = -1;
        while (true) {
            nextSetBit = added.nextSetBit(index + 1);
            index = nextSetBit;
            if (nextSetBit != -1) {
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("Add device by hot-plug detection:");
                stringBuilder.append(index);
                Slog.v(str, stringBuilder.toString());
                addDevice(index);
            } else {

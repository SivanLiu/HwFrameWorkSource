...
public class BuildBenefitStatisticsChrInfo {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:34:0x01e8  */
    ...
    /* JADX WARNING: Removed duplicated region for block: B:43:0x02d3  */
    ...
    public boolean commitCHR(String freqlocation) {
        ...
        EventStream buildBenefitEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_BENEFIT_STATISTICS_EVENTID);
        EventStream freqLocRecEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_FREQ_LOC_REC_CLASSID);
        EventStream appQoEBenefitEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_APPQOE_BENEFIT_TABLE_CLASSID);
        EventStream userPrefEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_USR_PREF_STATISTIC_CLASSID);
        EventStream lowPowerEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_LOW_POWER_STATISTIC_CLASSID);
        EventStream fastBack4GEstream = IMonitor.openEventStream(MSG_WAVEMAPPING_FAST_BACK_TO_LTE_STATISTIC_CLASSID);
        ...
        EventStream buildBenefitEstream2;
        EventStream signalFingerprintEstream;
        boolean ret;
        ...
        if (str.equals(this.mLocationDAO.getFrequentLocation())) {
            ...
            if (maxtime > 0 && now > maxtime) {
                ...
                if (results.containsKey("FREQLOCATION")) {
                    buildBenefitEstream = buildBenefitEstream2;
                } else {
                ...
                ret = IMonitor.sendEvent(buildBenefitEstream);

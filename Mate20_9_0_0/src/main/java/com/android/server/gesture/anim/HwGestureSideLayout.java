...
public class HwGestureSideLayout extends RelativeLayout {
    ...
    public HwGestureSideLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (CHINA_REGION) {
            this.mLeftIcon.setImageResource(33751950);
        } else {
            this.mLeftIcon.setImageResource(33751951);
        }
        this.mLeftIcon.setVisibility(4);
        LayoutParams leftIconLayoutParams = new LayoutParams((int) (density * 60.0f), (int) (density * 60.0f));
        ...
        if (CHINA_REGION) {
            this.mRightIcon.setImageResource(33751948);
        } else {
            this.mRightIcon.setImageResource(33751951);
        }
        this.mRightIcon.setVisibility(4);
        LayoutParams rightIconLayoutParams = new LayoutParams((int) (density * 60.0f), (int) (density * 60.0f));
        ...
        if (CHINA_REGION) {
            this.mRightInnerCircleMask.setImageResource(33751972);
        } else {
            this.mRightInnerCircleMask.setImageResource(33751973);
        }
        ...
        LayoutParams innerMaskLayoutParams = new LayoutParams((int) (density * 60.0f), (int) (density * 60.0f));
        ...
        if (CHINA_REGION) {
            this.mLeftInnerCircleMask.setImageResource(33751971);
        } else {
            this.mLeftInnerCircleMask.setImageResource(33751973);
        }
    ...
    public void applySlowStartAnimation(float progress, int animationType, int animationTarget) {
        ...
        stringBuilder.append("applySlowStartAnimation invoked,  progress: ");
        stringBuilder.append(progress);
        stringBuilder.append(" animationType: ");
        stringBuilder.append(animationType);
        stringBuilder.append(" animationTarget: ");
        stringBuilder.append(animationTarget);
    ...
    private void quickStartAnimationHivisonStyle(ImageView icon, int animationTarget, final Runnable runnable) {
        ...
        AnimatorSet iconAnim = getIconAnimatorSet(icon);
        getInnerCircleMaskAnimatorSet(innerCircleMask).setStartDelay(250);
        AnimatorSet quickStartAnim = new AnimatorSet();
        quickStartAnim.addListener(new AnimatorListenerAdapter() {
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
    ...
    private void applySlowSwapAnimationHivoiceStyle(float progress, int animationTarget) {
        ...
        if (icon != null) {
            ...
            if (Float.isNaN(scale)) {
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("scale is Float.NaN. progress = ");
                stringBuilder.append(progress);
    ...
    private AnimatorSet getInnerCircleMaskAnimatorSet(ImageView innerMask) {
        ...
        ObjectAnimator innerMaskTransitionX = ObjectAnimator.ofFloat(imageView, "translationX", new float[]{GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, transX});
    ...
    public void setSlidingProcess(float process) {
        applySlowStartAnimation(process, this.mSlidingOnLeft ^ 1, this.mSlidingOnLeft ^ 1);
    }

    public void startEnterAnimation() {
        applySlowStartAnimation(GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, this.mSlidingOnLeft ^ 1, this.mSlidingOnLeft ^ 1);
    }

    public void startExitAnimation(Runnable runnable, boolean isFastSlide, boolean startMaskAnimation) {
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("startExitAnimation isFastSlide: ");

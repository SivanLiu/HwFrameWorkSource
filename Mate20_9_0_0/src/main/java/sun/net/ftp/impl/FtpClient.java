...
public class FtpClient extends sun.net.ftp.FtpClient {
    ...
    private class DefaultParser implements FtpDirParser {
        /* synthetic */ DefaultParser(FtpClient x0, AnonymousClass1 x1) {
            this();
        }
        ...
        public FtpDirEntry parseLine(String line) {
            ...
            while (j < FtpClient.patterns.length) {
                ...
                if (m.find()) {
                    filename = m.group(FtpClient.patternGroups[j][i]);
                    ...
                    if (FtpClient.patternGroups[j][4] > 0) {
                        ...
                        stringBuilder.append(fdate);
                        stringBuilder.append(", ");
                        stringBuilder.append(m.group(FtpClient.patternGroups[j][4]));
                        fdate = stringBuilder.toString();
                    } else if (FtpClient.patternGroups[j][3] > 0) {
                        StringBuilder stringBuilder2 = new StringBuilder();
                        stringBuilder2.append(fdate);
                        stringBuilder2.append(", ");
                        stringBuilder2.append(String.valueOf(year));
                        fdate = stringBuilder2.toString();
                    }
            ...
            if (filename != null) {
                ...
                Date d2 = d;
                if (d2 == null || time == null) {
                    d2 = d2;
                } else {
                    i = time.indexOf(":");
                    now.setTime(d2);
                    now.set(10, Integer.parseInt(time.substring(0, i)));
                    now.set(12, Integer.parseInt(time.substring(i + 1)));
                    d2 = now.getTime();
                }
                ...
                boolean[][] fdate2 = (boolean[][]) Array.newInstance(boolean.class, 3, 3);
                i = 0;
                while (true) {
                    time2 = time;
                    int i2 = 3;
                    ...
                    while (true) {
                        ...
                        permstring2 = permstring;
                        fdate2[i][j3] = permstring.charAt((i * 3) + j3) != '-';
                        j2 = j3 + 1;
                        now = now2;
                ...
                now2 = now;
                time = new FtpDirEntry(filename);
                time.setUser(username).setGroup(groupname);
                time.setSize(Long.parseLong(fsize)).setLastModified(d2);
                time.setPermissions(fdate2);
                Type type = dir ? Type.DIR : str.charAt(0) == 'l' ? Type.LINK : Type.FILE;
                time.setType(type);
                return time;
            }
            String str2 = fdate;
    ...
    private class MLSxParser implements FtpDirParser {
        ...
        /* synthetic */ MLSxParser(FtpClient x0, AnonymousClass1 x1) {
            this();
        }
        ...
        public FtpDirEntry parseLine(String line) {
            ...
            s2 = file.getFact("Type");
            if (s2 != null) {
                if (s2.equalsIgnoreCase("file")) {
                    file.setType(Type.FILE);
                }
                if (s2.equalsIgnoreCase("dir")) {
                    file.setType(Type.DIR);
                }
                if (s2.equalsIgnoreCase("cdir")) {
                    file.setType(Type.CDIR);
                }
                if (s2.equalsIgnoreCase("pdir")) {
    ...
    static {
        ...
        r0 = new int[4][];
        int i = 0;
        r0[0] = new int[]{7, 4, 5, 6, 0, 1, 2, 3};
        r0[1] = new int[]{7, 4, 5, 0, 6, 1, 2, 3};
        r0[2] = new int[]{4, 3, 1, 2, 0, 0, 0, 0};
        r0[3] = new int[]{4, 3, 1, 2, 0, 0, 0, 0};
        patternGroups = r0;
    ...
    private boolean issueCommand(String cmd) throws IOException, FtpProtocolException {
        if (isConnected()) {
            ...
            if (cmd.indexOf(10) == -1) {
                ...
                stringBuilder.append(cmd);
                stringBuilder.append("\r\n");
                sendServer(stringBuilder.toString());
                return readReply();
            }
    ...
    private void issueCommandCheck(String cmd) throws FtpProtocolException, IOException {
        if (!issueCommand(cmd)) {
            ...
            stringBuilder.append(cmd);
            stringBuilder.append(":");
            stringBuilder.append(getResponseString());
            throw new FtpProtocolException(stringBuilder.toString(), getLastReplyCode());
        }
    }
    ...
    private Socket openPassiveDataConnection(String cmd) throws FtpProtocolException, IOException {
        ...
        if (issueCommand("EPSV ALL")) {
            ...
            if (m.find()) {
                ...
                stringBuilder.append("EPSV failed : ");
                stringBuilder.append(serverAnswer);
                throw new FtpProtocolException(stringBuilder.toString());
            }
        }
        ...
        if (m.find()) {
            dest = new InetSocketAddress(m.group(1).replace(',', '.'), (Integer.parseInt(m.group(2)) << 8) + Integer.parseInt(m.group(3)));
        } else {
            stringBuilder = new StringBuilder();
            stringBuilder.append("PASV failed : ");
            stringBuilder.append(serverAnswer);
            throw new FtpProtocolException(stringBuilder.toString());
        }
        if (this.proxy == null) {
            ...
            m = (Socket) AccessController.doPrivileged(new PrivilegedAction<Socket>() {
                public Socket run() {
                    return new Socket(FtpClient.this.proxy);
                }
            });
        } else {
            m = new Socket(Proxy.NO_PROXY);
        }
        m.bind(new InetSocketAddress((InetAddress) AccessController.doPrivileged(new PrivilegedAction<InetAddress>() {
            public InetAddress run() {
                return FtpClient.this.server.getLocalAddress();
            }
        }), 0));
        if (this.connectTimeout >= 0) {
            m.connect(dest, this.connectTimeout);
        } else if (defaultConnectTimeout > 0) {
            m.connect(dest, defaultConnectTimeout);
        } else {
            m.connect(dest);
        }
        if (this.readTimeout >= 0) {
            m.setSoTimeout(this.readTimeout);
        } else if (defaultSoTimeout > 0) {
            m.setSoTimeout(defaultSoTimeout);
        }
        if (this.useCrypto) {
            try {
                m = this.sslFact.createSocket(m, dest.getHostName(), dest.getPort(), true);
            } catch (Exception e) {
                StringBuilder stringBuilder2 = new StringBuilder();
                stringBuilder2.append("Can't open secure data channel: ");
                stringBuilder2.append(e);
                throw new FtpProtocolException(stringBuilder2.toString());
            }
        }
        if (issueCommand(cmd)) {
            return m;
        ...
        stringBuilder3.append(getResponseString());
        throw new FtpProtocolException(stringBuilder3.toString(), getLastReplyCode());
    }
    ...
    private Socket openDataConnection(String cmd) throws FtpProtocolException, IOException {
        ...
        try {
            ...
            if (!errmsg.startsWith("PASV") && !errmsg.startsWith("EPSV")) {
                ...
                String portCmd = new StringBuilder();
                portCmd.append("EPRT |");
                portCmd.append(myAddress instanceof Inet6Address ? "2" : "1");
                portCmd.append("|");
                portCmd.append(myAddress.getHostAddress());
                portCmd.append("|");
                portCmd.append(portSocket.getLocalPort());
                portCmd.append("|");
                if (!(issueCommand(portCmd.toString()) && issueCommand(cmd))) {
                    portCmd = "PORT ";
                    byte[] addr = myAddress.getAddress();
                    while (i < addr.length) {
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append(portCmd);
                        stringBuilder.append(addr[i] & 255);
                        stringBuilder.append(",");
                        portCmd = stringBuilder.toString();
                        i++;
                    }
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append(portCmd);
                    stringBuilder2.append((portSocket.getLocalPort() >>> 8) & 255);
                    stringBuilder2.append(",");
                    stringBuilder2.append(portSocket.getLocalPort() & 255);
                    issueCommandCheck(stringBuilder2.toString());
    ...
    private void tryConnect(InetSocketAddress dest, int timeout) throws IOException {
        ...
        try {
            ...
            throw new InternalError(stringBuilder.toString(), e);
        }
    }
    ...
    public sun.net.ftp.FtpClient connect(SocketAddress dest, int timeout) throws FtpProtocolException, IOException {
        if (dest instanceof InetSocketAddress) {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Welcome message: ");
            stringBuilder.append(getResponseString());
            throw new FtpProtocolException(stringBuilder.toString(), this.lastReplyCode);
        }
    ...
    public sun.net.ftp.FtpClient login(String user, char[] password, String account) throws FtpProtocolException, IOException {
        if (!isConnected()) {
            ...
            if (this.lastReplyCode == FtpReplyCode.NEED_ACCOUNT) {
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("ACCT ");
                stringBuilder.append(account);
                issueCommandCheck(stringBuilder.toString());
            }
            StringBuffer sb = new StringBuffer();
            if (this.serverResponse != null) {
                Iterator it = this.serverResponse.iterator();
                while (it.hasNext()) {
    ...
    public sun.net.ftp.FtpClient changeDirectory(String remoteDirectory) throws FtpProtocolException, IOException {
        ...
        stringBuilder.append(remoteDirectory);
        issueCommandCheck(stringBuilder.toString());
        return this;
    }
    ...
    public sun.net.ftp.FtpClient getFile(String name, OutputStream local) throws FtpProtocolException, IOException {
        ...
        if (this.restartOffset > 0) {
            try {
                ...
                s = openDataConnection(stringBuilder.toString());
                StringBuilder stringBuilder2 = new StringBuilder();
                stringBuilder2.append("RETR ");
                stringBuilder2.append(name);
                issueCommandCheck(stringBuilder2.toString());
                getTransferSize();
                remote = createInputStream(s.getInputStream());
                buf = new byte[(1500 * 10)];
                while (true) {
                    read = remote.read(buf);
                    l = read;
                    if (read < 0) {
                        break;
                    } else if (l > 0) {
    ...
    public OutputStream putFileStream(String name, boolean unique) throws FtpProtocolException, IOException {
        ...
        stringBuilder.append(cmd);
        stringBuilder.append(name);
        Socket s = openDataConnection(stringBuilder.toString());
        if (s == null) {
            return null;
        }
    ...
    public sun.net.ftp.FtpClient putFile(String name, InputStream local, boolean unique) throws FtpProtocolException, IOException {
        ...
        if (this.type == TransferType.BINARY) {
            ...
            stringBuilder.append(cmd);
            stringBuilder.append(name);
            OutputStream remote = createOutputStream(openDataConnection(stringBuilder.toString()).getOutputStream());
    ...
    private boolean sendSecurityData(byte[] buf) throws IOException, FtpProtocolException {
        ...
        return issueCommand(stringBuilder.toString());
    }
        ...
        if (isConnected()) {
            ...
            try {
                ...
                try {
                    ...
                    stringBuilder.append(encoding);
                    stringBuilder.append("encoding not found");
                    throw new InternalError(stringBuilder.toString(), e2);
                }

...
public abstract class UnixFileSystemProvider extends AbstractFileSystemProvider {
    ...
    public AsynchronousFileChannel newAsynchronousFileChannel(Path obj, Set<? extends OpenOption> options, ExecutorService executor, FileAttribute<?>... attrs) throws IOException {
        UnixPath file = checkPath(obj);
        try {
            return UnixChannelFactory.newAsynchronousFileChannel(file, options, UnixFileModeAttribute.toUnixMode(UnixFileModeAttribute.ALL_READWRITE, attrs), executor == null ? null : ThreadPool.wrap(executor, null));
        } catch (UnixException x) {
            x.rethrowAsIOException(file);
            return null;
        }
    ...
    public void checkAccess(Path obj, AccessMode... modes) throws IOException {
        ...
        if (e || r) {
            file.checkRead();
            i = 0 | (r ? UnixConstants.R_OK : UnixConstants.F_OK);
        }
        if (w) {
            file.checkWrite();
        ...
        try {
            UnixNativeDispatcher.access(file, i);

...
class UnixCopyFile {
    ...
    static void move(UnixPath source, UnixPath target, CopyOption... options) throws IOException {
        ...
        try {
            UnixNativeDispatcher.rename(source, target);
        } catch (UnixException x32) {
            if (!(x32.errno() == UnixConstants.EXDEV || x32.errno() == UnixConstants.EISDIR)) {
                x32.rethrowAsIOException(source, target);
            }
            ...
            try {
                ...
                if (sourceAttrs2.isDirectory() && (x322.errno() == UnixConstants.EEXIST || x322.errno() == UnixConstants.ENOTEMPTY)) {
                    throw new DirectoryNotEmptyException(source.getPathForExceptionMessage());
                }
                x322.rethrowAsIOException(source);
            }
        }
    }
            ...
            try {
                Cancellable.runInterruptibly(new Cancellable() {
                    public void implRun() throws IOException {
                        UnixCopyFile.copyFile(source, sourceAttrs, target, flags, addressToPollForCancel());
                    }
                });

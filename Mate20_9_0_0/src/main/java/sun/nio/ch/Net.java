    ...
    static void setSocketOption(FileDescriptor fd, ProtocolFamily family, SocketOption<?> name, Object value) throws IOException {
        if (value != null) {
            ...
            if (type == SocketFlow.class) {
                ...
                i = SocketOptionRegistry.findOption(name, family);
                if (i != 0) {
                    int b;
                    if (type == Integer.class) {
                        b = ((Integer) value).intValue();
                    } else {
                        b = ((Boolean) value).booleanValue();
                    }
                    setIntOption0(fd, family == UNSPEC, i.level(), i.name(), b, family == StandardProtocolFamily.INET6);
                    return;
                }

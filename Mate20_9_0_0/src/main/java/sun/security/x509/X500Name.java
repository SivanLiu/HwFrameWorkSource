...
public class X500Name implements GeneralNameInterface, Principal {
    ...
    private String getString(DerValue attribute) throws IOException {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("not a DER string encoding, ");
        stringBuilder.append(attribute.tag);
        throw new IOException(stringBuilder.toString());
    }
    ...
    private DerValue findAttribute(ObjectIdentifier attribute) {
        if (this.names != null) {
            for (DerValue value : this.names) {
                DerValue value2 = value2.findAttribute(attribute);
                if (value2 != null) {
                    return value2;
                }
            }
        ...
        while (true) {
            if (nextComma >= 0 || nextSemiColon >= 0) {
                ...
                if (!(rdnEnd < 0 || quoteCount == 1 || escaped(rdnEnd, searchOffset, dnString))) {
                    dnVector.add(new RDN(dnString.substring(dnOffset, rdnEnd), (Map) keywordMap));
                    dnOffset = rdnEnd + 1;
                    quoteCount = 0;
                }
                ...
                dnVector.add(new RDN(dnString.substring(dnOffset), (Map) keywordMap));
    ...
    static ObjectIdentifier intern(ObjectIdentifier oid) {
        ObjectIdentifier interned = (ObjectIdentifier) internedOIDs.putIfAbsent(oid, oid);
    ...
    public int constrains(GeneralNameInterface inputName) throws UnsupportedOperationException {
        ...
        if (inputName == null) {
            inputX5002 = -1;
        } else if (inputName.getType() != 4) {
            inputX5002 = -1;
        } else {
            inputX5002 = (X500Name) inputName;
            if (inputX5002.equals(this)) {
                constraintType = 0;
            } else if (inputX5002.names.length == 0) {
                constraintType = 2;
            } else if (this.names.length == 0) {
                constraintType = 1;
            } else if (inputX5002.isWithinSubtree(this)) {
                constraintType = 1;
            } else if (isWithinSubtree(inputX5002)) {
                constraintType = 2;
            } else {
                inputX5002 = 3;
            }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:18:0x0031 A:{LOOP_END, LOOP:1: B:17:0x002f->B:18:0x0031} */

...
public class DistributionPoint {
    ...
    public void encode(DerOutputStream out) throws IOException {
        DerOutputStream distributionPoint;
        DerOutputStream tagged = new DerOutputStream();
        if (!(this.fullName == null && this.relativeName == null)) {
            distributionPoint = new DerOutputStream();
        ...
        if (this.reasonFlags != null) {
            distributionPoint = new DerOutputStream();
            distributionPoint.putTruncatedUnalignedBitString(new BitArray(this.reasonFlags));
            tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, false, (byte) 1), distributionPoint);
        }
        if (this.crlIssuer != null) {
            distributionPoint = new DerOutputStream();
            this.crlIssuer.encode(distributionPoint);
            tagged.writeImplicit(DerValue.createTag(Byte.MIN_VALUE, true, (byte) 2), distributionPoint);
        }
    ...
    public String toString() {
        ...
        if (this.reasonFlags != null) {
            ...
            for (int i = 0; i < this.reasonFlags.length; i++) {
                if (this.reasonFlags[i]) {
                    ...
                    stringBuilder2.append(reasonToString(i));
                    stringBuilder2.append("\n");
        ...
        if (this.crlIssuer != null) {
            stringBuilder = new StringBuilder();
            stringBuilder.append("   CRLIssuer:");
            stringBuilder.append(this.crlIssuer);
            stringBuilder.append("\n");
            sb.append(stringBuilder.toString());
        }
        return sb.toString();

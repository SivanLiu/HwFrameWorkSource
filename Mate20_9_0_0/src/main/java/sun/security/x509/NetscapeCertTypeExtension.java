...
public class NetscapeCertTypeExtension extends Extension implements CertAttrSet<String> {
    ...
    static {
        ...
        r1 = new MapEntry[7];
        int i = 0;
        r1[0] = new MapEntry(SSL_CLIENT, 0);
        r1[1] = new MapEntry(SSL_SERVER, 1);
        r1[2] = new MapEntry(S_MIME, 2);
        r1[3] = new MapEntry(OBJECT_SIGNING, 3);
        r1[4] = new MapEntry(SSL_CA, 5);
        r1[5] = new MapEntry(S_MIME_CA, 6);
        r1[6] = new MapEntry(OBJECT_SIGNING_CA, 7);
        mMapData = r1;
    ...
    private static int getPosition(String name) throws IOException {
        ...
        stringBuilder.append("] not recognized by CertAttrSet:NetscapeCertType.");
    ...
    private void set(int position, boolean val) {
        if (position >= this.bitString.length) {
            Object tmp = new boolean[(position + 1)];

...
public final class ObjectIdentifier implements Serializable {
    ...
    public ObjectIdentifier(String oid) throws IOException {
        ...
        do {
            try {
                ...
                stringBuilder.append("ObjectIdentifier() -- Invalid format: ");
    ...
    public ObjectIdentifier(DerInputStream in) throws IOException {
        ...
        if (type_id == (byte) 6) {
            ...
            stringBuilder.append(in.available());
            throw new IOException(stringBuilder.toString());
        }
        StringBuilder stringBuilder2 = new StringBuilder();
        stringBuilder2.append("ObjectIdentifier() -- data isn't an object ID (tag = ");
        stringBuilder2.append(type_id);
    ...
    private void init(int[] components, int length) {
        ...
        if (components[1] < Integer.MAX_VALUE - (components[0] * 40)) {
            ...
            pos = 0 + pack7Oid(BigInteger.valueOf((long) components[1]).add(BigInteger.valueOf((long) (components[0] * 40))), tmp, 0);
        }
            ...
            if ((this.encoding[i] & 128) == 0) {
                ...
                if ((i - fromPos) + 1 > 4) {
                    ...
                    if (fromPos == 0) {
                        int which3 = which + 1;
                        result[which] = 2;
                        which = big.subtract(BigInteger.valueOf(80));
                        if (which.compareTo(BigInteger.valueOf(2147483647L)) == 1) {
                            return null;
                        }
                        which2 = which3 + 1;
                        result[which3] = which.intValue();
                    ...
                    which2 = 0;
                    for (j = fromPos; j <= i; j++) {
                        which2 = (which2 << 7) | (this.encoding[j] & 127);
                    }
                    if (fromPos != 0) {
                        j = which + 1;
                        result[which] = which2;

    ...
    public void setEntry(String name, JarEntry entry) throws IOException {
        ...
        if (this.man != null && name != null) {
            ...
            if (attr == null) {
                ...
                attr = manifest.getAttributes(stringBuilder.toString());
                if (attr == null) {
                    manifest = this.man;
    ...
    public CodeSigner[] verify(Hashtable<String, CodeSigner[]> verifiedSigners, Hashtable<String, CodeSigner[]> sigFileSigners) throws JarException {
        ...
        while (i < this.digests.size()) {
            ...
            if (debug != null) {
                Debug debug = debug;
                stringBuilder = new StringBuilder();
                stringBuilder.append("Manifest Entry: ");
                ...
                stringBuilder.append(toHex(manHash));
                debug.println(stringBuilder.toString());
                debug = debug;
                stringBuilder = new StringBuilder();
                stringBuilder.append("  computed ");
                stringBuilder.append(toHex(theHash));
                debug.println(stringBuilder.toString());
                debug.println();
            }
            if (MessageDigest.isEqual(theHash, manHash)) {
                ...
                stringBuilder = new StringBuilder();
                stringBuilder.append(digest.getAlgorithm());
                stringBuilder.append(" digest error for ");
                stringBuilder.append(this.name);

...
public class FormattedFloatingDecimal {
    ...
    public static FormattedFloatingDecimal valueOf(double d, int precision, Form form) {
        return new FormattedFloatingDecimal(precision, form, FloatingDecimal.getBinaryToASCIIConverter(d, form == Form.COMPATIBLE ? true : null));
    }
        ...
        if (exp <= 0 || exp >= 8) {
            if (exp > 0 || exp <= -3) {
                ...
                if (nDigits > 1) {
                    ...
                    System.arraycopy((Object) digits, 1, this.mantissa, startIndex + 2, nDigits - 1);
                } else {
            ...
            zeros = Math.max(0, Math.min(-exp, precision));
            ...
            if (zeros > 0) {
                ...
                if (t > 0) {
                    System.arraycopy((Object) digits, 0, this.mantissa, (startIndex + 2) + zeros, t);
                }
            } else if (t > 0) {
                this.mantissa = create(isNegative, (zeros + 2) + t);
                this.mantissa[startIndex] = '0';
                this.mantissa[startIndex + 1] = '.';
                System.arraycopy((Object) digits, 0, this.mantissa, startIndex + 2, t);
            } else {
            ...
            t = exp - nDigits;
            this.mantissa = create(isNegative, (nDigits + t) + 2);
            System.arraycopy((Object) digits, 0, this.mantissa, startIndex, nDigits);
            Arrays.fill(this.mantissa, startIndex + nDigits, (startIndex + nDigits) + t, '0');
            this.mantissa[(startIndex + nDigits) + t] = '.';
            this.mantissa[((startIndex + nDigits) + t) + 1] = '0';
        } else if (exp < nDigits) {
            zeros = Math.min(nDigits - exp, precision);
            this.mantissa = create(isNegative, (exp + 1) + zeros);
            System.arraycopy((Object) digits, 0, this.mantissa, startIndex, exp);
            this.mantissa[startIndex + exp] = '.';
            System.arraycopy((Object) digits, exp, this.mantissa, (startIndex + exp) + 1, zeros);
        } else {
    ...
    private void fillDecimal(int precision, char[] digits, int nDigits, int exp, boolean isNegative) {
        ...
        if (exp > 0) {
            ...
            if (t > 0) {
                this.mantissa[startIndex + exp] = '.';
                System.arraycopy((Object) digits, exp, this.mantissa, (startIndex + exp) + 1, t);
            }
        } else if (exp <= 0) {
            ...
            if (zeros > 0) {
                ...
                Arrays.fill(this.mantissa, startIndex + 2, (startIndex + 2) + zeros, '0');
                if (t2 > 0) {
                    System.arraycopy((Object) digits, 0, this.mantissa, (startIndex + 2) + zeros, t2);
                }
            } else if (t2 > 0) {
                this.mantissa = create(isNegative, (zeros + 2) + t2);
                this.mantissa[startIndex] = '0';
                this.mantissa[startIndex + 1] = '.';
                System.arraycopy((Object) digits, 0, this.mantissa, startIndex + 2, t2);
            } else {
                this.mantissa = create(isNegative, 1);
                this.mantissa[startIndex] = '0';
            }
    ...
    private void fillScientific(int precision, char[] digits, int nDigits, int exp, boolean isNegative) {
        ...
        boolean startIndex = z;
        int t = Math.max(0, Math.min(nDigits - 1, precision));
        if (t > 0) {
            this.mantissa = create(z, t + 2);
            this.mantissa[startIndex] = obj[0];
            this.mantissa[startIndex + 1] = '.';
            System.arraycopy(obj, 1, this.mantissa, startIndex + 2, t);
        } else {

...
public final class ClientOperation implements Operation, BaseStream {
    ...
    private boolean sendRequest(int opCode) throws IOException {
        ...
        if ((headerArray.length + 3) + 3 > this.mMaxPacketSize) {
            ...
            while (end != headerArray.length) {
                ...
                if (!this.mParent.sendRequest(opCode, sendHeader, this.mReplyHeader, this.mPrivateInput, false) || this.mReplyHeader.responseCode != ResponseCodes.OBEX_HTTP_CONTINUE) {
                    return false;
                }
                start = end;
            }
        ...
        if (bodyLength > 0) {
            ...
            if (!this.mPrivateOutput.isClosed() || returnValue || this.mEndOfBodySent || (start & 128) == 0) {
                out.write(72);
            } else {
                out.write(73);
        ...
        if (out.size() == 0) {
            if (!this.mParent.sendRequest(start, null, this.mReplyHeader, this.mPrivateInput, this.mSrmActive)) {
                return false;
            }
            checkForSrm();
            return returnValue;
        ...
        if (out.size() > 0) {
            if (!this.mParent.sendRequest(start, out.toByteArray(), this.mReplyHeader, this.mPrivateInput, this.mSrmActive)) {
                return false;
            }
        }
        checkForSrm();
    ...
    public void streamClosed(boolean inStream) throws IOException {
        if (this.mGetOperation) {
            ...
            while (more2 && this.mReplyHeader.responseCode == ResponseCodes.OBEX_HTTP_CONTINUE) {
                more2 = sendRequest(2);
            }

...
public final class ServerOperation implements Operation, BaseStream {
    ...
    public ServerOperation(ServerSession p, InputStream in, int request, int maxSize, ServerRequestHandler listen) throws IOException {
        ...
        stringBuilder.append("Packet received was too large. Length: ");
        stringBuilder.append(packet.mLength);
        stringBuilder.append(" maxLength: ");
        stringBuilder.append(ObexHelper.getMaxRxPacketSize(this.mTransport));
        ...
        synchronized (this) {
            ...
            if (headerArray.length + 3 > this.mMaxPacketLength) {
                ...
                while (end != headerArray.length) {
                    end = ObexHelper.findHeaderEnd(headerArray, start, this.mMaxPacketLength - i2);
                    if (end == -1) {
                        this.mClosed = z;
                    ...
                    sendHeader = new byte[(end - start)];
                    System.arraycopy(headerArray, start, sendHeader, 0, sendHeader.length);
                    this.mParent.sendResponse(i, sendHeader);
                    start = end;
                    z = true;
                    i2 = 3;
                }
            ...
            if (this.mGetOperation && i == ResponseCodes.OBEX_HTTP_OK) {
                this.finalBitSet = true;
            }
            if (this.mSrmActive) {
                if (!this.mGetOperation && i == ResponseCodes.OBEX_HTTP_CONTINUE && this.mSrmResponseSent) {
                    skipSend = true;
            ...
            if (this.finalBitSet && i == ResponseCodes.OBEX_HTTP_OK && orginalBodyLength <= 0 && this.mSendBodyHeader) {
                out.write(73);
                out.write((byte) (3 >> 8));
                out.write((byte) 3);
            }
            if (!skipSend) {
                this.mResponseSize = 3;
                this.mParent.sendResponse(i, out.toByteArray());
            }
            if (i != ResponseCodes.OBEX_HTTP_CONTINUE) {
                return false;
            } else if (this.mGetOperation && skipReceive) {
                checkSrmRemoteAbort();
            } else {
                ObexPacket packet = ObexPacket.read(this.mInput);
                int headerId = packet.mHeaderId;
                if (headerId == 2 || headerId == ObexHelper.OBEX_OPCODE_PUT_FINAL || headerId == 3 || headerId == ObexHelper.OBEX_OPCODE_GET_FINAL) {
    ...
    private void checkSrmRemoteAbort() throws IOException {
        if (this.mInput.available() > 0) {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Received unexpected request from client - discarding...\n   headerId: ");
            stringBuilder.append(packet.mHeaderId);

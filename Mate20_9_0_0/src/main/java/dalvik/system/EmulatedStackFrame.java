...
public class EmulatedStackFrame {
    ...
    public static final class Range {
        ...
        public static Range of(MethodType frameType, int startArg, int endArg) {
            ...
            for (int i2 = startArg; i2 < endArg; i2++) {
                cl = ptypes[i2];
                if (cl.isPrimitive()) {
                    i += EmulatedStackFrame.getSize(cl);
                } else {
                    numReferences++;
                }
            }
            return new Range(referencesStart, numReferences, stackFrameStart, i);
        }
    }
    ...
    public static class StackFrameAccessor {
        ...
        protected void checkType(Class<?> type) {
            if (this.argumentIdx >= this.numArgs || this.argumentIdx == -1) {
                ...
                stringBuilder.append("Invalid argument index: ");
                stringBuilder.append(this.argumentIdx);
            ...
            if (expectedType != type) {
                ...
                stringBuilder2.append("Incorrect type: ");
    ...
    public void setReference(int idx, Object reference) {
        ...
        if (idx < 0 || idx >= ptypes.length) {
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append("reference is not of type: ");
            stringBuilder.append(this.type.ptypes()[idx]);
    ...
    public void setReturnValueTo(Object reference) {
        ...
        if (returnType.isPrimitive()) {
            stringBuilder = new StringBuilder();
            stringBuilder.append("return type is not a reference type: ");

...
public final class RILDsFlowInfoResponse {
    ...
    public final void readEmbeddedFromParcel(HwParcel parcel, HwBlob _hidl_blob, long _hidl_offset) {
        ...
        this.lastDsTime = hwBlob.getString(_hidl_offset + 0);
        parcel.readEmbeddedBuffer((long) (this.lastDsTime.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 0) + 0, false);
        this.lastTxFlow = hwBlob.getString(_hidl_offset + 16);
        parcel.readEmbeddedBuffer((long) (this.lastTxFlow.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 16) + 0, false);
        this.lastRxFlow = hwBlob.getString(_hidl_offset + 32);
        parcel.readEmbeddedBuffer((long) (this.lastRxFlow.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 32) + 0, false);
        this.totalDsTime = hwBlob.getString(_hidl_offset + 48);
        parcel.readEmbeddedBuffer((long) (this.totalDsTime.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 48) + 0, false);
        this.totalTxFlow = hwBlob.getString(_hidl_offset + 64);
        parcel.readEmbeddedBuffer((long) (this.totalTxFlow.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 64) + 0, false);
        this.totalRxFlow = hwBlob.getString(_hidl_offset + 80);
        parcel.readEmbeddedBuffer((long) (this.totalRxFlow.getBytes().length + 1), _hidl_blob.handle(), (_hidl_offset + 80) + 0, false);
    }

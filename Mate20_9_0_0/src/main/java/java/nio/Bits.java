...
class Bits {
    ...
    static boolean unaligned() {
        ...
        boolean z = (arch.equals("i386") || arch.equals("x86") || arch.equals("amd64") || arch.equals("x86_64")) ? true : $assertionsDisabled;
        unaligned = z;
        unalignedKnown = true;
        return unaligned;
    }
    ...
    static void copyFromArray(Object src, long srcBaseOffset, long srcPos, long dstAddr, long length) {
        ...
        long dstAddr2 = dstAddr;
        long length2 = length;
        while (length2 > 0) {
            long j = UNSAFE_COPY_THRESHOLD;
            if (length2 <= UNSAFE_COPY_THRESHOLD) {
                j = length2;
            }
            long size = j;
            unsafe.copyMemoryFromPrimitiveArray(src, offset, dstAddr2, size);
            length2 -= size;
            offset += size;
            dstAddr2 += size;
        }
    ...
    static void copyToArray(long srcAddr, Object dst, long dstBaseOffset, long dstPos, long length) {
        ...
        while (length2 > 0) {
            ...
            long size = j;
            unsafe.copyMemoryToPrimitiveArray(srcAddr2, dst, offset, size);
            length2 -= size;
            srcAddr2 += size;
            offset += size;
        }

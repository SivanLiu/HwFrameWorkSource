...
final class CharsetDecoderICU extends CharsetDecoder {
    ...
    protected final CoderResult implFlush(CharBuffer out) {
        try {
            ...
            CoderResult U_FAILURE = ICU.U_FAILURE(error);
            if (U_FAILURE == true) {
                if (error == 15) {
                    U_FAILURE = CoderResult.OVERFLOW;
                    return U_FAILURE;
                }
                U_FAILURE = 11;
                if (error == 11) {
                    U_FAILURE = this.data[2];
                    if (U_FAILURE > false) {
                        U_FAILURE = CoderResult.malformedForLength(this.data[2]);
                        setPosition(out);
                        implReset();
                        return U_FAILURE;
                    }
            ...
            U_FAILURE = CoderResult.UNDERFLOW;
            setPosition(out);
            implReset();
            return U_FAILURE;
        } finally {

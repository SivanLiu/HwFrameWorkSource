...
public final class FileSystems {
    ...
    private static class DefaultFileSystemHolder {
        ...
        private static FileSystemProvider getDefaultProvider() {
            ...
            int provider3 = 0;
            while (provider3 < length) {
                try {
                    provider2 = (FileSystemProvider) Class.forName(split[provider3], true, ClassLoader.getSystemClassLoader()).getDeclaredConstructor(new Class[]{FileSystemProvider.class}).newInstance(provider2);
    ...
    public static FileSystem getFileSystem(URI uri) {
        ...
        throw new ProviderNotFoundException(stringBuilder.toString());
    }
    ...
    public static FileSystem newFileSystem(URI uri, Map<String, ?> env, ClassLoader loader) throws IOException {
        ...
        if (loader != null) {
            Iterator it = ServiceLoader.load(FileSystemProvider.class, loader).iterator();
            while (it.hasNext()) {
                FileSystemProvider provider2 = (FileSystemProvider) it.next();
    ...
    public static FileSystem newFileSystem(Path path, ClassLoader loader) throws IOException {
        if (path != null) {
            ...
            if (loader != null) {
                ...
                while (it.hasNext()) {
                    try {
                        return ((FileSystemProvider) it.next()).newFileSystem(path, env);

...
class TempFileHelper {
    ...
    private static Path generatePath(String prefix, String suffix, Path dir) {
        ...
        Path name = dir.getFileSystem();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(prefix);
        stringBuilder.append(Long.toString(n));
        stringBuilder.append(suffix);
        name = name.getPath(stringBuilder.toString(), new String[0]);
    ...
    private static Path create(Path dir, String prefix, String suffix, boolean createDirectory, FileAttribute<?>[] attrs) throws IOException {
        ...
        if (isPosix && dir.getFileSystem() == FileSystems.getDefault()) {
            if (attrs.length == 0) {
                ...
                for (FileAttribute name : attrs) {
                    if (name.name().equals("posix:permissions")) {
                        hasPermissions = true;
                        break;
                    }
                }

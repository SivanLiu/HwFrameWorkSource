...
public final class Character implements Serializable, Comparable<Character> {
    ...
    public static int offsetByCodePoints(CharSequence seq, int index, int codePointOffset) {
        ...
        if (codePointOffset >= 0) {
            i2 = 0;
            while (i < length && i2 < codePointOffset) {
                int x = i + 1;
                if (isHighSurrogate(seq.charAt(i)) && x < length && isLowSurrogate(seq.charAt(x))) {
                    x++;
                }
                i = x;
                i2++;
        ...
        while (i2 > 0 && i < 0) {
            i2--;
            if (isLowSurrogate(seq.charAt(i2)) && i2 > 0 && isHighSurrogate(seq.charAt(i2 - 1))) {
                i2--;
            }
            i++;
        }
    ...
    static int offsetByCodePointsImpl(char[] a, int start, int count, int index, int codePointOffset) {
        ...
        if (codePointOffset >= 0) {
            limit = start + count;
            int i2 = 0;
            while (i < limit && i2 < codePointOffset) {
                int x = i + 1;
                if (isHighSurrogate(a[i]) && x < limit && isLowSurrogate(a[x])) {
                    x++;
                }
                i = x;
                i2++;
        ...
        limit = i;
        i = codePointOffset;
        while (limit > start && i < 0) {
            limit--;
            if (isLowSurrogate(a[limit]) && limit > start && isHighSurrogate(a[limit - 1])) {
                limit--;
            }
            i++;
        }
    ...
    public static boolean isJavaIdentifierStart(int codePoint) {
        ...
        if (codePoint < 64) {
            ...
            if ((576460745995190270L & (1 << (codePoint - 64))) != 0) {
                z = true;
            }
            return z;
        } else {
    ...
    public static int digit(int codePoint, int radix) {
        ...
        if (48 > codePoint || codePoint > 57) {
            ...
            if (97 <= codePoint && codePoint <= 122) {
                result2 = 10 + (codePoint - 97);
            } else if (65 <= codePoint && codePoint <= 90) {
                result2 = 10 + (codePoint - 65);
            }
            result = result2;
        } else {
            result = codePoint - 48;
    ...
    public static String getName(int codePoint) {
        if (isValidCodePoint(codePoint)) {
            ...
            stringBuilder.append(block.toString().replace('_', ' '));
            stringBuilder.append(" ");
            stringBuilder.append(Integer.toHexString(codePoint).toUpperCase(Locale.ENGLISH));

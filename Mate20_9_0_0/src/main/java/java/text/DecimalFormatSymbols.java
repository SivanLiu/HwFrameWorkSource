...
public class DecimalFormatSymbols implements Cloneable, Serializable {
    ...
    private void initialize(Locale locale) {
        ...
        this.locale = locale2;
        if (locale2 != null) {
            Locale locale3 = LocaleData.mapInvalidAndNullLocales(locale);
            ...
            if (locale3.getCountry().length() > 0) {
                try {
                    this.currency = Currency.getInstance(locale3);
                } catch (IllegalArgumentException e) {
                }
            }
            if (this.currency != null) {
                ...
                if (data[1] == null || data[1] != this.intlCurrencySymbol) {
                    this.currencySymbol = this.currency.getSymbol(locale3);

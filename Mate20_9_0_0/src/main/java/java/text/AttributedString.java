...
public class AttributedString {
    ...
    private final class AttributedStringIterator implements AttributedCharacterIterator {
                    ...
                    if (AttributedString.this.runStarts[i] < this.endIndex && (i == AttributedString.this.runCount - 1 || AttributedString.this.runStarts[i + 1] > this.beginIndex)) {
                        ...
                        if (currentRunAttributes != null) {
                            ...
                            while (true) {
                                ...
                                keys.add((Attribute) currentRunAttributes.get(j2));
                                j = j2;
                            }
                        }
                    }
        ...
        private void updateRunInfo() {
            ...
            synchronized (AttributedString.this) {
                ...
                while (true) {
                    runIndex2 = runIndex;
                    if (runIndex2 >= AttributedString.this.runCount - 1 || AttributedString.this.runStarts[runIndex2 + 1] > this.currentIndex) {
                        this.currentRunIndex = runIndex2;
                    } else {
                ...
                this.currentRunIndex = runIndex2;
                if (runIndex2 >= 0) {
                    this.currentRunStart = AttributedString.this.runStarts[runIndex2];
                ...
                if (runIndex2 < AttributedString.this.runCount - 1) {
                    this.currentRunLimit = AttributedString.this.runStarts[runIndex2 + 1];
    ...
    AttributedString(AttributedCharacterIterator[] iterators) {
        if (iterators == null) {
            ...
            if (this.text.length() > 0) {
                ...
                while (counter2 < iterators.length) {
                    AttributedCharacterIterator iterator = iterators[counter2];
                    int start = iterator.getBeginIndex();
                    int end = iterator.getEndIndex();
                    Map<Attribute, Object> last2 = last;
                    for (int index = start; index < end; index = iterator.getRunLimit()) {
                        ...
                        if (mapsDiffer(last2, attrs)) {
                            setAttributes(attrs, (index - start) + counter);
                        }
                        last2 = attrs;
                    }
                    counter += end - start;
    ...
    private final int ensureRunBreak(int offset, boolean copyAttrs) {
        ...
        Vector<Attribute> newRunAttributes2 = null;
    ...
    private Object getAttributeCheckRange(Attribute attribute, int runIndex, int beginIndex, int endIndex) {
        ...
        if (value instanceof Annotation) {
            ...
            if (endIndex < currIndex) {
                runStart = runIndex;
                int runLimit = runStart < this.runCount + -1 ? this.runStarts[runStart + 1] : currIndex;
                while (runLimit <= endIndex && valuesMatch(value, getAttribute(attribute, runStart + 1))) {
                    runStart++;
                    runLimit = runStart < this.runCount + -1 ? this.runStarts[runStart + 1] : currIndex;
                }
    ...
    private void setAttributes(Map<Attribute, Object> attrs, int offset) {
        ...
        int index = ensureRunBreak(offset, 0);
        if (attrs != null) {
            int size = attrs.size();
            int size2 = size;
            if (size > 0) {
                Vector<Attribute> runAttrs = new Vector(size2);

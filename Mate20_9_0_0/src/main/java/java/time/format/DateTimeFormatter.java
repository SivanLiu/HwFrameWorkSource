...
public final class DateTimeFormatter {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:18:0x0032 A:{Splitter:B:3:0x000f, ExcHandler: DateTimeParseException (r0_6 'ex' java.time.format.DateTimeParseException)} */
    ...
    private DateTimeParseException createError(CharSequence text, RuntimeException ex) {
        ...
        if (text.length() > 64) {
            ...
            abbr.append(text.subSequence(0, 64).toString());
            abbr.append("...");
            abbr = abbr.toString();
        } else {
            abbr = text.toString();
        ...
        return new DateTimeParseException(stringBuilder.toString(), text, 0, ex);
    }
    ...
    private TemporalAccessor parseResolved0(CharSequence text, ParsePosition position) {
        ...
        if (text.length() > 64) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(text.subSequence(0, 64).toString());
        ...
        if (pos.getErrorIndex() >= 0) {
            ...
            stringBuilder2.append(abbr);
            stringBuilder2.append("' could not be parsed at index ");
            stringBuilder2.append(pos.getErrorIndex());
            throw new DateTimeParseException(stringBuilder2.toString(), text, pos.getErrorIndex());
        }
        stringBuilder2 = new StringBuilder();
        stringBuilder2.append("Text '");
        stringBuilder2.append(abbr);
        stringBuilder2.append("' could not be parsed, unparsed text found at index ");
        stringBuilder2.append(pos.getIndex());
        throw new DateTimeParseException(stringBuilder2.toString(), text, pos.getIndex());
    }

        ...
        boolean removedAt(int removedIndex) {
            ...
            if (cursor >= 0) {
                ...
                if (x == removedDistance) {
                    ...
                    dec = ArrayBlockingQueue.this.dec(cursor);
                    cursor = dec;
                    this.cursor = dec;
                }
            }
            x = this.lastRet;
            if (x >= 0) {
                x2 = distance(x, prevTakeIndex, len);
                if (x2 == removedDistance) {
                    x = -2;
                    this.lastRet = -2;
                } else if (x2 > removedDistance) {
                    dec2 = ArrayBlockingQueue.this.dec(x);
                    x = dec2;
                    this.lastRet = dec2;
                }
            }
            x2 = this.nextIndex;
            if (x2 >= 0) {
                dec2 = distance(x2, prevTakeIndex, len);
                if (dec2 == removedDistance) {
                    x2 = -2;
                    this.nextIndex = -2;
                } else if (dec2 > removedDistance) {
                    dec = ArrayBlockingQueue.this.dec(x2);
                    x2 = dec;
                    this.nextIndex = dec;
                }
            }

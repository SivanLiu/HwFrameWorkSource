...
public abstract class Calendar implements Serializable, Cloneable, Comparable<Calendar> {
    ...
    public static class Builder {
        ...
        public Builder setCalendarType(String type) {
            ...
            stringBuilder.append("unknown calendar type: ");
        ...
        public Calendar build() {
            ...
            boolean z = Calendar.$assertionsDisabled;
            ...
            if (this.firstDayOfWeek != 0) {
                gcal.setFirstDayOfWeek(this.firstDayOfWeek);
            ...
            if (this.fields != null) {
                boolean weekDate = (!isSet(17) || this.fields[17] <= this.fields[1]) ? Calendar.$assertionsDisabled : true;
                if (!weekDate || gcal.isWeekDateSupported()) {
                    while (stamp < this.nextStamp) {
                        int index = 0;
                        while (index <= this.maxFieldIndex) {
                            if (this.fields[index] == stamp) {
                                gcal.set(index, this.fields[18 + index]);
                                stamp++;
                            } else {
                                index++;
                            }
                    ...
                    if (weekDate) {
                        ...
                        gcal.setWeekDate(this.fields[35], z2, isSet(7) ? this.fields[25] : gcal.getFirstDayOfWeek());
                    }
                    gcal.complete();
                } else {
                    StringBuilder stringBuilder2 = new StringBuilder();
                    stringBuilder2.append("week date is unsupported by ");
    ...
    final void setFieldsComputed(int fieldMask) {
        ...
        fieldMask = 0;
        while (fieldMask < this.fields.length) {
            if ((fieldMask2 & 1) == 1) {
                this.stamp[fieldMask] = 1;
                this.isSet[fieldMask] = true;
            } else if (this.areAllFieldsSet && !this.isSet[fieldMask]) {
                this.areAllFieldsSet = $assertionsDisabled;
            }
            fieldMask2 >>>= 1;
            fieldMask++;
    ...
    final void setFieldsNormalized(int fieldMask) {
        if (fieldMask != ALL_FIELDS) {
            int fieldMask2 = fieldMask;
            for (fieldMask = 0; fieldMask < this.fields.length; fieldMask++) {
                if ((fieldMask2 & 1) == 0) {
                    int[] iArr = this.stamp;
                    this.fields[fieldMask] = 0;
                    iArr[fieldMask] = 0;
                    this.isSet[fieldMask] = $assertionsDisabled;
                }
                fieldMask2 >>= 1;
            }
        }
    ...
    final boolean isFullyNormalized() {
        return (this.areFieldsSet && this.areAllFieldsSet) ? true : $assertionsDisabled;
    }
    ...
    public boolean before(Object when) {
        return (!(when instanceof Calendar) || compareTo((Calendar) when) >= 0) ? $assertionsDisabled : true;
    }
    ...
    private void adjustStamp() {
        ...
        while (true) {
            ...
            while (true) {
                max = i;
                if (max >= this.stamp.length) {
                    break;
                }
                if (this.stamp[max] == min) {
                    this.stamp[max] = newStamp;
                }
                i = max + 1;
            }
            newStamp++;

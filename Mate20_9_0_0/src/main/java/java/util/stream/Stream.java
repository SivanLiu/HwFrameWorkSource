...
public interface Stream<T> extends BaseStream<T, Stream<T>> {
    ...
    static <T> Stream<T> iterate(final T seed, final UnaryOperator<T> f) {
        ...
        return StreamSupport.stream(Spliterators.spliteratorUnknownSize(new Iterator<T>() {
            ...
            public T next() {
                Object apply = this.t == Streams.NONE ? seed : f.apply(this.t);
                this.t = apply;
                return apply;
            }
        }, 1040), false);
    ...
    static <T> Stream<T> concat(Stream<? extends T> a, Stream<? extends T> b) {
        ...
        Spliterator<T> split = new OfRef(a.spliterator(), b.spliterator());
        Stream<T> stream = (a.isParallel() || b.isParallel()) ? true : null;
        return (Stream) StreamSupport.stream(split, stream).onClose(Streams.composedClose(a, b));
    }
}

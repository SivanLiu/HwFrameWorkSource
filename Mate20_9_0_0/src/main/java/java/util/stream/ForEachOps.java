...
final class ForEachOps {
    ...
    static final class ForEachOrderedTask<S, T> extends CountedCompleter<Void> {
        ...
        private static <S, T> void doCompute(ForEachOrderedTask<S, T> task) {
            ...
            if (task2.getPendingCount() > 0) {
                task2.node = ((Builder) task2.helper.wrapAndCopyInto(task2.helper.makeNodeBuilder(task2.helper.exactOutputSizeIfKnown(rightSplit), -$$Lambda$ForEachOps$ForEachOrderedTask$XLqga2XPr4V7tlS8H12fiz-In-o.INSTANCE), rightSplit)).build();
    ...
    static final class ForEachTask<S, T> extends CountedCompleter<Void> {
        ...
        public void compute() {
            ...
            if (j == 0) {
                j = AbstractTask.suggestTargetSize(sizeEstimate);
                sizeThreshold = j;
                this.targetSize = j;
            }

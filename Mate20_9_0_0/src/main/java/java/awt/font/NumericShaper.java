...
public final class NumericShaper implements Serializable {
    ...
    public enum Range {
        ...
        private Range(int base, int start, int end) {
            this.base = base - (48 + getNumericBase());
            this.start = start;
            this.end = end;
        }
    ...
    private Range rangeForCodePoint(int codepoint) {
        ...
        for (lo = 0; lo < ranges.length; lo++) {
            if (ranges[lo].inRange(codepoint)) {
                return ranges[lo];
            }
        }
    ...
    private static int getKeyFromMask(int mask) {
        ...
        stringBuilder.append(Integer.toHexString(mask));
                    ...
                    try {
                        ...
                        if (isStrongDirectional(c)) {
                            ...
                            if (newkey != base) {
                                base = newkey;
                                lastkey = newkey;
                                if ((this.mask & 4) != 0 && (lastkey == 1 || lastkey == 2)) {
                                    lastkey = 2;
                                } else if ((this.mask & 2) != 0 && (lastkey == 1 || lastkey == 2)) {
                                    lastkey = 1;
                                } else if ((this.mask & (1 << lastkey)) == 0) {
                                    lastkey = 0;
                                }
                                minDigit = lastkey == 16 ? '1' : '0';
                                base2 = bases[lastkey];
                            }
                        }
                        i++;

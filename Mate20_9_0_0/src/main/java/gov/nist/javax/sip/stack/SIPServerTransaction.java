...
public class SIPServerTransaction extends SIPTransaction implements ServerRequestInterface, ServerTransaction, ServerTransactionExt {
    ...
    class SendTrying extends SIPStackTimerTask {
        ...
        protected void runTask() {
            ...
            if (realState == null || TransactionState.TRYING == realState) {
                ...
                if (SIPServerTransaction.this.sipStack.isLoggingEnabled()) {
                    stackLogger = SIPServerTransaction.this.sipStack.getStackLogger();
                    stringBuilder = new StringBuilder();
                    stringBuilder.append(" sending Trying current state = ");
                    stringBuilder.append(serverTransaction.getRealState());
                    stackLogger.logDebug(stringBuilder.toString());
                }
                try {
                    ...
                    if (SIPServerTransaction.this.sipStack.isLoggingEnabled()) {
                        stackLogger = SIPServerTransaction.this.sipStack.getStackLogger();
                        stringBuilder = new StringBuilder();
                        stringBuilder.append(" trying sent ");
                        stringBuilder.append(serverTransaction.getRealState());
                        stackLogger.logDebug(stringBuilder.toString());
                    }
                } catch (IOException e) {
            ...
            if (isReliable()) {
                ...
                if (transport != null) {
                    ...
                    if (messageChannel != null) {
                        ...
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append("Could not create a message channel for ");
    ...
    protected SIPServerTransaction(SIPTransactionStack sipStack, MessageChannel newChannelToUse) {
        ...
        if (sipStack.isLoggingEnabled()) {
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Creating Server Transaction");
            stringBuilder.append(getBranchId());
            stackLogger.logDebug(stringBuilder.toString());
    ...
    public void processRequest(SIPRequest transactionRequest, MessageChannel sourceChannel) {
        ...
        if (this.sipStack.isLoggingEnabled()) {
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append("processRequest: ");
            stringBuilder.append(transactionRequest.getFirstLine());
            stackLogger.logDebug(stringBuilder.toString());
            stackLogger = this.sipStack.getStackLogger();
            stringBuilder = new StringBuilder();
            stringBuilder.append("tx state = ");
            stringBuilder.append(getRealState());
            stackLogger.logDebug(stringBuilder.toString());
        }
        try {
            if (getRealState() == null) {
                ...
                if (this.sipStack.isNon2XXAckPassedToListener()) {
                    ...
                    if (this.sipStack.isLoggingEnabled()) {
                        stackLogger = this.sipStack.getStackLogger();
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("ACK received for server Tx ");
                        stringBuilder.append(getTransactionId());
                        stringBuilder.append(" not delivering to application!");
                        stackLogger.logDebug(stringBuilder.toString());
                    }
                    semRelease();
                }
                ...
                if (TransactionState.PROCEEDING != getRealState()) {
                    if (TransactionState.COMPLETED != getRealState()) {
                        ...
                        if (this.sipStack.isLoggingEnabled()) {
                            stackLogger = this.sipStack.getStackLogger();
                            stringBuilder = new StringBuilder();
                            stringBuilder.append("completed processing retransmitted request : ");
                            stringBuilder.append(transactionRequest.getFirstLine());
                            stringBuilder.append(this);
                            stringBuilder.append(" txState = ");
                            stringBuilder.append(getState());
                            stringBuilder.append(" lastResponse = ");
                            stringBuilder.append(getLastResponse());
                            stackLogger.logDebug(stringBuilder.toString());
                        }
                        return;
                    }
            ...
            if (TransactionState.COMPLETED == getRealState() || TransactionState.TERMINATED == getRealState() || this.requestOf == null) {
                ...
                if (this.sipStack.isLoggingEnabled()) {
                    stackLogger = this.sipStack.getStackLogger();
                    stringBuilder = new StringBuilder();
                    stringBuilder.append("Dropping request ");
                    stringBuilder.append(getRealState());
                    stackLogger.logDebug(stringBuilder.toString());
                }
            }
    ...
    public void sendMessage(SIPMessage messageToSend) throws IOException {
        ...
        if (transactionResponse.getCSeq().getMethod().equals(getOriginalRequest().getMethod())) {
            ...
            if (this.sipStack.isLoggingEnabled()) {
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("sendMessage : tx = ");
                stringBuilder.append(this);
                stringBuilder.append(" getState = ");
                stringBuilder.append(getState());
                stackLogger.logDebug(stringBuilder.toString());
    ...
    protected void fireTimeoutTimer() {
        if (this.sipStack.isLoggingEnabled()) {
            StackLogger stackLogger = this.sipStack.getStackLogger();
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("SIPServerTransaction.fireTimeoutTimer this = ");
            stringBuilder.append(this);
            stringBuilder.append(" current state = ");
            stringBuilder.append(getRealState());
            stringBuilder.append(" method = ");
            stringBuilder.append(getOriginalRequest().getMethod());
            stackLogger.logDebug(stringBuilder.toString());
        }
        ...
        if (response != null) {
            try {
                ...
                if (sipResponse.getCSeq().getMethod().equals(getMethod())) {
                    ...
                    if (sipResponse.getStatusCode() == Response.OK && sipResponse.getCSeq().getMethod().equals("INVITE") && sipResponse.getHeader("Contact") == null) {
                                ...
                                if (sipResponse.getStatusCode() / 100 == 2) {
                                    ...
                                    if (SIPTransactionStack.isDialogCreated(sipResponse.getCSeq().getMethod())) {
                                        if (dialog.getLocalTag() == null && sipResponse.getTo().getTag() == null) {
                                            ...
                                            if (!dialog.getLocalTag().equals(sipResponse.getToTag())) {
                                                StringBuilder stringBuilder = new StringBuilder();
                                                stringBuilder.append("Tag mismatch dialogTag is ");
                                                stringBuilder.append(dialog.getLocalTag());
                                                stringBuilder.append(" responseTag is ");
                                                stringBuilder.append(sipResponse.getToTag());
                ...
                throw new SipException(ex12.getMessage());
            }
        }
    ...
    public void scheduleAckRemoval() throws IllegalStateException {
        if (getMethod() == null || !getMethod().equals("ACK")) {
            ...
            stringBuilder.append(getMethod() == null);

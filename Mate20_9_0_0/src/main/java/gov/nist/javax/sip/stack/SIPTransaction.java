...
public abstract class SIPTransaction extends MessageChannel implements Transaction, TransactionExt {
    ...
    class LingerTimer extends SIPStackTimerTask {
        ...
        protected void runTask() {
            ...
            if (sipStack.isLoggingEnabled()) {
                ...
                stringBuilder.append("LingerTimer: run() : ");
            ...
            if (transaction instanceof SIPClientTransaction) {
                ...
                if (!sipStack.cacheServerConnections) {
                    MessageChannel access$000 = transaction.encapsulatedChannel;
                    int i = access$000.useCount - 1;
                    access$000.useCount = i;
                    if (i <= 0) {
                        transaction.close();
                        return;
                    }
                ...
                if (sipStack.isLoggingEnabled() && !sipStack.cacheServerConnections && transaction.isReliable()) {
                    ...
                    stringBuilder2.append("Use Count = ");
    ...
    public void setOriginalRequest(SIPRequest newOriginalRequest) {
        ...
        if (newBranch != null) {
            if (this.sipStack.isLoggingEnabled()) {
                ...
                stringBuilder.append(newBranch);
                stackLogger.logDebug(stringBuilder.toString());
            }
            setBranch(newBranch);
            return;
        ...
        if (this.sipStack.isLoggingEnabled()) {
            ...
            stringBuilder.append(newOriginalRequest.encode());
            stackLogger.logDebug(stringBuilder.toString());
        }
    ...
    public void setState(TransactionState newState) {
        ...
        if (this.sipStack.isLoggingEnabled()) {
            StackLogger stackLogger = this.sipStack.getStackLogger();
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Transaction:setState ");
            stringBuilder.append(newState2);
            stringBuilder.append(Separators.SP);
            stringBuilder.append(this);
            stringBuilder.append(" branchID = ");
            stringBuilder.append(getBranch());
            stringBuilder.append(" isClient = ");
            stringBuilder.append(this instanceof SIPClientTransaction);
            stackLogger.logDebug(stringBuilder.toString());
        ...
        if (viaHeaders != null) {
            ...
            if (messageBranch == null || getBranch() == null) {
                if (this.sipStack.isLoggingEnabled()) {
                    StackLogger stackLogger = this.sipStack.getStackLogger();
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("testing against ");
                    stringBuilder.append(getOriginalRequest());
                    stackLogger.logDebug(stringBuilder.toString());
                }
    ...
    public boolean acquireSem() {
        ...
        try {
            ...
            if (this.sipStack.getStackLogger().isLoggingEnabled()) {
                stackLogger = this.sipStack.getStackLogger();
                stringBuilder = new StringBuilder();
                stringBuilder.append("acquireSem [[[[");
                stringBuilder.append(this);
                stackLogger.logDebug(stringBuilder.toString());
                this.sipStack.getStackLogger().logStackTrace();
            }
            ...
            if (this.sipStack.isLoggingEnabled()) {
                stackLogger = this.sipStack.getStackLogger();
                stringBuilder = new StringBuilder();
                stringBuilder.append("acquireSem() returning : ");
    ...
    protected synchronized boolean testAndSetTransactionTerminatedEvent() {
        boolean retval;
        retval = this.terminatedEventDelivered ^ true;

        ...
        if (getHeaderNames().hasNext() && b.getHeaderNames().hasNext()) {
            try {
                ...
                while (i2.hasNext()) {
                    ...
                    if (p2 != null || h2 != null) {
                        try {
                            if (!headerFactory.createHeader(p1, RFC2396UrlDecoder.decode(p2)).equals(headerFactory.createHeader(p1, RFC2396UrlDecoder.decode(h2)))) {
                                return false;
                            }
                        } catch (ParseException e) {
                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.append("Cannot parse one of the header of the sip uris to compare ");
                            stringBuilder.append(this);
                            stringBuilder.append(Separators.SP);
                            stringBuilder.append(b);
                            Debug.logError(stringBuilder.toString(), e);
                            return false;
                        }
                    }
                }

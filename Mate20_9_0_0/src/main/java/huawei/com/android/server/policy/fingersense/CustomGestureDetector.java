...
public class CustomGestureDetector {
    ...
    public CustomGestureDetector(Context context, GestureLibrary multipleOrientationLib, GestureLibrary singleOrientationLib, OnGesturePerformedListener listener) {
        ...
        this.mLineGestureStrokePortraitAngle = GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
        this.mLineGestureStrokeLandscapeAngle = GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
        ...
        this.knuckGestureSetting = KnuckGestureSetting.getInstance();
    ...
    public void setLineGestureStrokeStraightness(float straightness) {
        if (straightness < GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO || straightness > 9.0f) {
    ...
    public void setMaxLineGestureStrokeAngleDeviation(float angleDeviation) {
        if (angleDeviation < GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO || angleDeviation > DEFAULT_MAX_LINE_GESTURE_STROKE_ANGLE_DEVIATION) {
    ...
    public void setLineGestureStrokePortraitAngle(float angle) {
        if (angle < GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO || angle > DEFAULT_MAX_LINE_GESTURE_STROKE_ANGLE_DEVIATION) {
    ...
    private void touchDown(MotionEvent event) {
        ...
        this.mCurrentX = x;
        this.mCurrentY = y;
        this.mTotalLength = GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
        this.mIsGesturing = false;
        this.mLetterGestureStarted = false;
    ...
    private void fireOnGesturePerformed(GesturePerformedRunnable gesturePerformed) {
        if (this.mCurrentGesture == null || this.mCurrentGesture.getStrokesCount() <= 0) {
            ...
            Log.d(TAG, "Fingersense: Not a Region gesture or letter gesture");
            EventStream eStream = IMonitor.openEventStream(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD);
            if (!(eStream == null || this.mIsStylus)) {
                eStream.setParam((short) 15, 1);
            ...
            if (this.mIsStylus) {
                Flog.bdReport(this.mContext, 952, "false");
            }
        }
    }
}

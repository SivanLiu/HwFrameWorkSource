...
public class HwActionBarContainer extends ActionBarContainer {
    ...
    private boolean hasTabs() {
        ...
        return (tabContainer == null || tabContainer.getVisibility() == 8) ? false : true;
    }

    private void setBackgroundColors(Context context, boolean forceRefresh) {
        if (getId() == 16908688 && !HwWidgetUtils.isActionbarBackgroundThemed(context) && !HwWidgetFactory.isHwDarkTheme(context)) {
            ...
            if (abv != null && Color.alpha(color) != 0) {
                if (this.mColor != color || force) {
                    ...
                    if (needSplitLine(color) && !this.mDisplayNoSplitLine) {
                        Drawable drawable = context.getDrawable(33751637);
                        Drawable splitDrawable = drawable;
                        if (drawable != null) {
    ...
    private boolean needSplitLine(int color) {
        return color == -197380 || color == CoverManager.DEFAULT_COLOR;
    }
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        } else if (getId() == 16908688) {
            setBackgroundDrawable(getContext().getResources().getDrawable(33751393));
        }
    }

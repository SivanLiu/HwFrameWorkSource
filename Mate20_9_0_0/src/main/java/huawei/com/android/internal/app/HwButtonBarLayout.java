...
public class HwButtonBarLayout extends LinearLayout {
    ...
    private void unifyBtnTextSize(float uniTextSize) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (!(child == null || child.getVisibility() == 8 || !(child instanceof Button))) {
                ...
                float btnTextSize = btn.getTextSize();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isStacked() || MeasureSpec.getMode(widthMeasureSpec) != StubController.PERMISSION_ACCESS_BROWSER_RECORDS) {
            initialWidthMeasureSpec = widthMeasureSpec;
        } else {
            initialWidthMeasureSpec = MeasureSpec.makeMeasureSpec(layoutWidth, AppTypeInfo.APP_ATTRIBUTE_OVERSEA);
            needsRemeasure = true;
        }
        super.onMeasure(initialWidthMeasureSpec, heightMeasureSpec);
        if (!isStacked() && (CoverManager.DEFAULT_COLOR & getMeasuredWidthAndState()) == StubController.PERMISSION_EDIT_SHORTCUT) {
            setStacked(true);
            needsRemeasure = true;
        }
        if (needsRemeasure) {

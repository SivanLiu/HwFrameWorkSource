...
public class HwActionBarView extends ActionBarView {
    ...
    protected int measureChildView(View child, int availableWidth, int childSpecHeight, int spacing) {
        ...
        if (getUpGoerFive() == null || getUpGoerFive().getChildCount() != 0) {
            ...
            child.measure(MeasureSpec.makeMeasureSpec(maxSize, StubController.PERMISSION_ACCESS_BROWSER_RECORDS), childSpecHeight);
            if (title != null) {
            ...
            child.measure(MeasureSpec.makeMeasureSpec(maxSize, AppTypeInfo.APP_ATTRIBUTE_OVERSEA), childSpecHeight);
        }
    ...
    private void invalidateTitleLayout() {
        boolean marginNotZero = true;
        int margin = 0;
        if (((getDisplayOptions() & 4) != 0) || this.mIcon1Visible) {
            marginNotZero = false;
        }
        int margin2 = getResources().getDimensionPixelSize(34472184);
        if (marginNotZero) {
            margin = margin2;
        }
    ...
    public void saveState(ButtonState state) {
        ...
        state.mUsed1 = used1;
        float f = 0.0f;
        state.mAlpha1 = used1 ? this.mIcon1View.getAlpha() : 0.0f;
        state.mEnabled1 = used1 ? this.mIcon1View.isEnabled() : false;
        if (this.mIcon2View == null) {
            used2 = false;
        }
        state.mUsed2 = used2;
    ...
    protected void onFinishInflate() {
        ...
        homeView.setContentDescription(getContext().getResources().getText(17039538));
    ...
    public static void invalidateTitleLayout(int gravity, int margin, TextView title, TextView subTitle) {
        ...
        if (title != null) {
            lpTitle = (LayoutParams) title.getLayoutParams();
        ...
        if (subTitle != null && subTitle.getVisibility() == 0) {
            lpTitle = (LayoutParams) subTitle.getLayoutParams();
            lpTitle.gravity = gravity;
    ...
    public void setSmartColor(ColorStateList iconColor, ColorStateList titleColor) {
        ...
        if (menuView instanceof HwSmartColorListener) {
            ((HwSmartColorListener) menuView).onSetSmartColor(iconColor, titleColor);
        }
    }
}

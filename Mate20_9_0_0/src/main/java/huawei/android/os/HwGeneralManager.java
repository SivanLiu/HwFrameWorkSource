...
public class HwGeneralManager {
    ...
    public boolean isSupportTrikey() {
        if (!this.mInitTrikey) {
            ...
            this.mIsSupportTrikey = z;
            this.mInitTrikey = true;
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("isSupportTrikey is:");
            stringBuilder.append(String.valueOf(this.mIsSupportTrikey));
    ...
    public String getTouchWeightValue() {
        try {
            if (this.mService != null) {
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("HwGeneralManager getTouchWeightValue ");
                stringBuilder.append(val);
                Slog.i("touch weight", stringBuilder.toString());
                return val;
            }
        } catch (RemoteException e) {
            Slog.e(TAG, "HwGeneralManager binder error!");
    ...
    public boolean isCurveScreen() {
        if (this.mIsCurveScreen == -1) {
            try {
                this.mIsCurveScreen = this.mService.isCurveScreen();
            } catch (RemoteException e) {
                Flog.e(ConstantValue.transaction_turnOnGPS, "HwGeneralManager binder error!");
            }
        }
    ...
    private void initEffectNames() {
        ...
        effectNames.put(Integer.valueOf(HwWifiChrMsgID.EVENT_UPDATE_ACCESS_WEB_FAIL), "BULK_MOVE_ICONPICKUP");
        ...
        effectNames.put(Integer.valueOf(2000), "TEXTVIEW_SETCURSOR");

...
public class IconBitmapUtils {
    ...
    public static void mask(int[] pixels, int w, int h, Bitmap mask) {
        if (mask != null) {
            ...
            if (wMask >= w && hMask >= h) {
                ...
                stringBuilder.append("Mask size error, wMask=");
                stringBuilder.append(wMask);
                stringBuilder.append(",hMask=");
                stringBuilder.append(hMask);
    ...
    public static Bitmap mask(Bitmap bmpSrc, Bitmap bmpMask) {
        ...
        Bitmap newBitMap2 = bitmap.copy(Config.ARGB_8888, true);
        try {
            int h;
            int srcStartY;
            ...
            for (h2 = (w * h) - 1; h2 >= 0; h2--) {
                maskPixels[h2] = ((((maskPixels[h2] >>> 24) * (srcPixels[h2] >>> 24)) / 255) << 24) | (srcPixels[h2] & 16777215);
            }
    ...
    public static Bitmap overlap2Bitmap(Bitmap bmpSrc, Bitmap bmpBg, int deltaX, int deltaY) {
        ...
        if (w > wBg || h > hBg) {
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("The size of background(");
            stringBuilder.append(wBg);
            stringBuilder.append(",");
            stringBuilder.append(hBg);
            stringBuilder.append(") little than the source bitmap(");
            stringBuilder.append(w);
            stringBuilder.append(",");
            stringBuilder.append(h);
            stringBuilder.append(")");
    ...
    private static void drawBackground(Canvas canvas, Bitmap background, int w, int h, int[] pixels) {
        ...
        while (true) {
            ...
            if (i2 >= 0) {
                i = backPixels[i2];
                backPixels[i2] = (((CoverManager.DEFAULT_COLOR & i) | (16711680 & (((16711680 & i) * r) >>> 8))) | (65280 & (((65280 & i) * g) >>> 8))) | ((((i & 255) * b) >>> 8) & 255);
                i = i2 - 1;
            } else {

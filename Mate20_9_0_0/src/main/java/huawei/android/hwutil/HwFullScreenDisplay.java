...
public class HwFullScreenDisplay {
    ...
    public static void getAppDisplayRect(float appMaxRatio, Rect rect, int left, int rotation) {
        if (mDeviceWidth != 0 && appMaxRatio > 1.0f && appMaxRatio < mScreenAspectRatio) {
            int realBarHeight = mNavBarHeight;
            if (rotation == 0 || rotation == 2) {
                int realBarHeight2 = (mNavBarHeight * rect.width()) / mDeviceWidth;
                realBarHeight = (int) (((float) rect.width()) * mScreenAspectRatio);
                if (rect.bottom > realBarHeight - realBarHeight2) {
                    rect.set(rect.left, rect.top, rect.width(), realBarHeight - realBarHeight2);
                }
            } else {
                ...
                realBarHeight = (int) (((float) rect.height()) * mScreenAspectRatio);
                int moveRight = (realBarHeight - rectWidth) / 2;
                int mustShowWidth = mNotchHeight + ((mNavBarHeight * rect.height()) / mDeviceWidth);
                if (showInMiddleWhenLand(appMaxRatio)) {
                    if (rectWidth + moveRight > realBarHeight - mustShowWidth) {
                        int newLeft = moveRight - (mustShowWidth - moveRight);
                        rect.set(newLeft > 0 ? newLeft : 0, rect.top, realBarHeight - mustShowWidth, rect.bottom);
                    } else {
                        rect.set(moveRight, rect.top, rectWidth + moveRight, rect.bottom);
                    }
                } else if (rect.right > realBarHeight - mustShowWidth) {
                    rect.set(rect.left, rect.top, realBarHeight - mustShowWidth, rect.bottom);
                }
            }
        }
    }
    ...
    public static Rect getTopAppDisplayBounds(float appMaxRatio, int rotation, int screenWidth) {
        ...
        if (appMaxRatio > 1.0f && appMaxRatio < mScreenAspectRatio) {
            int temp = (int) ((((float) screenWidth) * appMaxRatio) + 1056964608);
            if (rotation == 0 || rotation == 2) {

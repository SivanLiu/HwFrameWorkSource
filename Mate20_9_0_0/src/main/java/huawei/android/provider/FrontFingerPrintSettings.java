...
public class FrontFingerPrintSettings {
    ...
    public static final int SINGLE_VIRTUAL_AI_MODE = SystemProperties.getInt("ro.config.show_navbar_slide", SystemProperties.get("ro.config.hw_optb", System.FINGERSENSE_KNUCKLE_GESTURE_OFF).equals("156"));
    ...
    public static boolean isNaviBarEnabled(ContentResolver resolver) {
        ...
        if (FRONT_FINGERPRINT_NAVIGATION_TRIKEY == 0) {
            if (isChinaArea()) {
                NAVI_BAR_DEFAULT_STATUS = SINGLE_VIRTUAL_NAVIGATION_MODE == 1;
            } else {
    ...
    public static boolean isSingleVirtualNavbarEnable(ContentResolver resolver) {
        ...
        if (Settings.System.getIntForUser(resolver, System.SINGLE_VIRTUAL_NAVBAR, SINGLE_VIRTUAL_NAVIGATION_MODE == 1 ? 1 : 0, ActivityManager.getCurrentUser()) == 1) {
            z = true;
        }
        return z;
    }
    ...
    public static boolean isGestureNavigationMode(ContentResolver resolver) {
        return Secure.getIntForUser(resolver, HwSettings.Secure.KEY_SECURE_GESTURE_NAVIGATION, -1, ActivityManager.getCurrentUser()) == 1;
    }
}

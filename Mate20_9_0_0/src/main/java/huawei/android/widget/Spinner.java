...
public class Spinner extends android.widget.Spinner {
    ...
    private void setClickEffice(Context context, int defStyleAttr) {
        if (getBackground() != null) {
            setBackground(new LayerDrawable(new Drawable[]{HwWidgetUtils.getHwAnimatedGradientDrawable(context, 16842881), getBackground()}));
            return;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isLayoutRtl() && spinnerPadding.left < spinnerPadding.right) {
            ...
            temp = this.mPaddingRight;
            this.mPaddingRight = this.mPaddingLeft;
            this.mPaddingLeft = temp;
        }
    }
}

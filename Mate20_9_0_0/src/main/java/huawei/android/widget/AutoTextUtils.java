...
public class AutoTextUtils {
    ...
    private static void measureHeight(int parentHeight, int parentWidth, TextView textView) {
        ...
        if (maxLines > 1) {
            ...
            if (availedHeight >= 0) {
                StaticLayout staticLayout = new StaticLayout(textView.getText(), textView.getPaint(), (parentWidth - textView.getTotalPaddingLeft()) - textView.getTotalPaddingRight(), Alignment.ALIGN_NORMAL, textView.getLineSpacingMultiplier(), textView.getLineSpacingExtra(), false);

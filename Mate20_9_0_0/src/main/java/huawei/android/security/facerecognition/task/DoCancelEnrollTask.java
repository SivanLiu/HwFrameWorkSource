    ...
    public boolean onEvent(FaceRecognizeEvent ev) {
        switch (ev.getType()) {
            case 7:
                ...
                CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 1, 1, this.mRetErrorCode);
                break;
            case 8:
                CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 1, 3, (int) ev.getArgs()[0]);
                break;
            case 9:
                endWithResult(0);
                break;

...
public class AuthenticateTask extends FaceRecognizeTask {
    ...
    private static final int[] UNIPERF_TAG = new int[]{57};
    ...
    private RetCallback mDoCancelCallback = new RetCallback() {
        public void onTaskCallback(HwSecurityTaskBase child, int ret) {
            if (AuthenticateTask.this.mTaskRequest.isActiveCanceled()) {
                CallbackHolder.getInstance().onCallbackEvent((int) AuthenticateTask.this.mTaskRequest.getReqId(), 2, 2, 0);
            }
    ...
    private RetCallback mPrepareCameraCallback = new RetCallback() {
        public void onTaskCallback(HwSecurityTaskBase child, int ret) {
            if (ret != 0 && ret != 2) {
                if (ret == 7) {
                    CallbackHolder.getInstance().onCallbackEvent((int) AuthenticateTask.this.mTaskRequest.getReqId(), 2, 1, 12);
                } else {
                    CallbackHolder.getInstance().onCallbackEvent((int) AuthenticateTask.this.mTaskRequest.getReqId(), 2, 1, 1);
                }
                AuthenticateTask.this.endWithResult(ret);
            } else if (AuthenticateTask.this.mTaskRequest.isCanceled()) {
                CallbackHolder.getInstance().onCallbackEvent((int) AuthenticateTask.this.mTaskRequest.getReqId(), 2, 1, 2);
                CallbackHolder.getInstance().onCallbackEvent((int) AuthenticateTask.this.mTaskRequest.getReqId(), 2, 2, 0);
                AuthenticateTask.this.endWithResult(2);
            } else {
                HwSecurityTaskThread.staticPushTask(new DoAuthTask(AuthenticateTask.this, AuthenticateTask.this.mDoAuthCallback, AuthenticateTask.this.mTaskRequest, AuthenticateTask.this.mFlags, AuthenticateTask.this.mOpId), 1);
            }
        }
    };
    ...
    public AuthenticateTask(FaceRecognizeTask parent, RetCallback callback, AuthenticateRequest request, Context context, long opId) {
    ...
    public int doAction() {
        ...
        stringBuilder.append("\", \"nano_time\":\"");
        stringBuilder.append(System.nanoTime());
        stringBuilder.append("\"}");
        this.mReportTempCap = stringBuilder.toString();
        ...
        if (DeviceUtil.reachDisabledTempCap(temp, cap)) {
            ...
            CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 2, 1, 11);
            return 5;
        }
    ...
    private void endAuth(int ret, int userId, int errorCode) {
        ...
        CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 2, 1, errorCode);
        if (this.mTaskRequest.isActiveCanceled()) {
            CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 2, 2, errorCode);
        }
        endWithResult(ret);
    }
}

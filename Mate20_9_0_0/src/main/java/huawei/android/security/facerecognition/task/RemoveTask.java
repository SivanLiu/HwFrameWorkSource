...
public class RemoveTask extends FaceRecognizeTask implements EventListener {
    ...
    private void sendRemoveResult(int errorCode) {
        CallbackHolder.getInstance().onCallbackEvent((int) this.mTaskRequest.getReqId(), 3, 1, errorCode);
    }

    public boolean onEvent(FaceRecognizeEvent ev) {
        int removeResult = (int) ev.getArgs()[0];
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("remove result : ");
        stringBuilder.append(removeResult);

...
public abstract class SensorManager {
    ...
    protected abstract boolean supportSensorFeatureImpl(int i);
    ...
    public static boolean getRotationMatrix(float[] R, float[] I, float[] gravity, float[] geomagnetic) {
        ...
        if (fArr2 != null) {
            ...
            float c = (((Ex * invH) + (Ey * My)) + (Ez * Mz)) * invE;
    ...
    private static boolean remapCoordinateSystemImpl(float[] inR, int X, int Y, float[] outR) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (x == i4) {
                    fArr2[offset + i4] = sx ? -fArr[offset + 0] : fArr[offset + 0];
                }
                if (y == i4) {
                    fArr2[offset + i4] = sy ? -fArr[offset + 1] : fArr[offset + 1];
                }
                if (z == i4) {
                    fArr2[offset + i4] = sz ? -fArr[offset + 2] : fArr[offset + 2];
                }
                j = i4 + 1;
                i3 = 3;
            }

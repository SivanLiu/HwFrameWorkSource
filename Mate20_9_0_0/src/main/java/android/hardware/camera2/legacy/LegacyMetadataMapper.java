...
public class LegacyMetadataMapper {
    ...
    private static void mapScalerStreamConfigs(CameraMetadataNative m, Parameters p) {
        ...
        while (!previewSizes.isEmpty()) {
            ...
            size = (Size) previewSizes.get(index);
        ...
        if (previewSizes.isEmpty()) {
            ...
            stringBuilder.append(jpegAspectRatio);
        ...
        for (Integer format2 : p.getSupportedPreviewFormats()) {
            format = format2.intValue();
        ...
        for (Size size2 : jpegSizes) {
            long stallDuration = calculateJpegStallDuration(size2);
            int i = format + 1;
            jpegStalls[format] = new StreamConfigurationDuration(33, size2.width, size2.height, stallDuration);
    ...
    private static void mapControlAe(CameraMetadataNative m, Parameters p) {
        ...
        if (fpsRanges != null) {
            ...
            List<int[]> fpsRanges2;
            if (j > 0) {
                Object ranges = new Range[j];
                int i2 = 0;
                for (int[] r : fpsRanges) {
                    int i3 = i2 + 1;
                    fpsRanges2 = fpsRanges;
                    ranges[i2] = Range.create(Integer.valueOf((int) Math.floor(((double) r[i]) / 1000.0d)), Integer.valueOf((int) Math.ceil(((double) r[1]) / 1000.0d)));
                    i2 = i3;
                    fpsRanges = fpsRanges2;
                    i = 0;
                }
                cameraMetadataNative.set(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES, ranges);
                ranges = ArrayUtils.convertStringListToIntArray(p.getSupportedFlashModes(), new String[]{"off", "auto", Parameters.FLASH_MODE_ON, Parameters.FLASH_MODE_RED_EYE, Parameters.FLASH_MODE_TORCH}, new int[]{1, 2, 3, 4});
                if (ranges == null || ranges.length == 0) {
                    ranges = new int[]{1};
                }
                cameraMetadataNative.set(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES, ranges);
    ...
    private static void mapRequest(CameraMetadataNative m, Parameters p) {
        ...
        cameraMetadataNative.set(CameraCharacteristics.REQUEST_AVAILABLE_CHARACTERISTICS_KEYS, getTagsForKeys((Key[]) characteristicsKeys.toArray(new Key[0])));
    ...
    public static CameraMetadataNative createRequestTemplate(CameraCharacteristics c, int templateId) {
        if (ArrayUtils.contains(sAllowedTemplates, templateId)) {
            ...
            Object bestRange2 = bestRange;
            for (Range<Integer> r : availableFpsRange) {
                if (((Integer) bestRange2.getUpper()).intValue() < ((Integer) r.getUpper()).intValue()) {
                    bestRange2 = r;
                } else if (bestRange2.getUpper() == r.getUpper() && ((Integer) bestRange2.getLower()).intValue() < ((Integer) r.getLower()).intValue()) {
                    bestRange2 = r;
                }
            }

...
public class ParameterUtils {
    ...
    public static class WeightedRectangle {
        ...
        public MeteringRectangle toMetering() {
            int weight = clip(this.weight, 0, 1000, this.rect, PreviewPrograms.COLUMN_WEIGHT);
            return new MeteringRectangle(clipLower(this.rect.left, 0, this.rect, "left"), clipLower(this.rect.top, 0, this.rect, "top"), clipLower(this.rect.width(), 0, this.rect, MediaFormat.KEY_WIDTH), clipLower(this.rect.height(), 0, this.rect, MediaFormat.KEY_HEIGHT), weight);
        }
    ...
    public static int getClosestAvailableZoomCrop(Parameters params, Rect activeArray, Size streamSize, Rect cropRegion, Rect reportedCropRegion, Rect previewCropRegion) {
        ...
        if (availableReportedCropRegions.size() == availablePreviewCropRegions.size()) {
            ...
            Rect bestPreviewCropRegion2 = null;
            Rect bestReportedCropRegion = null;
            while (bestReportedCropRegion < availableReportedCropRegions.size()) {
                Rect actualCrop2;
                boolean isBest;
                Rect currentPreviewCropRegion = (Rect) availablePreviewCropRegions.get(bestReportedCropRegion);
                Rect currentReportedCropRegion = (Rect) availableReportedCropRegions.get(bestReportedCropRegion);
                if (bestZoomIndex == -1) {
                ...
                bestPreviewCropRegion2 = currentReportedCropRegion;
                bestZoomIndex = bestReportedCropRegion;
                bestReportedCropRegion++;
                actualCrop = actualCrop2;
                rect = activeArray;
            }
            if (bestZoomIndex != -1) {
                rect2.set(bestPreviewCropRegion2);
    ...
    private static List<Rect> getAvailableCropRectangles(Parameters params, Rect activeArray, Size streamSize) {
        ...
        if (params.isZoomSupported()) {
            ...
            for (Integer zoom : params.getZoomRatios()) {
                float shrinkRatio = 100.0f / ((float) zoom.intValue());

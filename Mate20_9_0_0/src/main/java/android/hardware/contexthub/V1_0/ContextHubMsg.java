package android.hardware.contexthub.V1_0;

import android.os.HidlSupport;
import android.os.HwBlob;
import android.os.HwParcel;
import java.util.ArrayList;
import java.util.Objects;

public final class ContextHubMsg {
    public long appName;
    public short hostEndPoint;
    public final ArrayList<Byte> msg = new ArrayList();
    public int msgType;

    public final boolean equals(Object otherObject) {
        if (this == otherObject) {
            return true;
        }
        if (otherObject == null || otherObject.getClass() != ContextHubMsg.class) {
            return false;
        }
        ContextHubMsg other = (ContextHubMsg) otherObject;
        if (this.appName == other.appName && this.hostEndPoint == other.hostEndPoint && this.msgType == other.msgType && HidlSupport.deepEquals(this.msg, other.msg)) {
            return true;
        }
        return false;
    }

    public final int hashCode() {
        return Objects.hash(new Object[]{Integer.valueOf(HidlSupport.deepHashCode(Long.valueOf(this.appName))), Integer.valueOf(HidlSupport.deepHashCode(Short.valueOf(this.hostEndPoint))), Integer.valueOf(HidlSupport.deepHashCode(Integer.valueOf(this.msgType))), Integer.valueOf(HidlSupport.deepHashCode(this.msg))});
    }

    public final String toString() {
        StringBuilder builder = new StringBuilder();
        builder.append("{");
        builder.append(".appName = ");
        builder.append(this.appName);
        builder.append(", .hostEndPoint = ");
        builder.append(this.hostEndPoint);
        builder.append(", .msgType = ");
        builder.append(this.msgType);
        builder.append(", .msg = ");
        builder.append(this.msg);
        builder.append("}");
        return builder.toString();
    }

    public final void readFromParcel(HwParcel parcel) {
        readEmbeddedFromParcel(parcel, parcel.readBuffer(32), 0);
    }

    public static final ArrayList<ContextHubMsg> readVectorFromParcel(HwParcel parcel) {
        ArrayList<ContextHubMsg> _hidl_vec = new ArrayList();
        HwBlob _hidl_blob = parcel.readBuffer(16);
        int _hidl_vec_size = _hidl_blob.getInt32(8);
        HwBlob childBlob = parcel.readEmbeddedBuffer((long) (_hidl_vec_size * 32), _hidl_blob.handle(), 0, true);
        _hidl_vec.clear();
        for (int _hidl_index_0 = 0; _hidl_index_0 < _hidl_vec_size; _hidl_index_0++) {
            ContextHubMsg _hidl_vec_element = new ContextHubMsg();
            _hidl_vec_element.readEmbeddedFromParcel(parcel, childBlob, (long) (_hidl_index_0 * 32));
            _hidl_vec.add(_hidl_vec_element);
        }
        return _hidl_vec;
    }

    public final void readEmbeddedFromParcel(HwParcel parcel, HwBlob _hidl_blob, long _hidl_offset) {
        HwBlob hwBlob = _hidl_blob;
        this.appName = hwBlob.getInt64(_hidl_offset + 0);
        this.hostEndPoint = hwBlob.getInt16(_hidl_offset + 8);
        this.msgType = hwBlob.getInt32(_hidl_offset + 12);
        int _hidl_vec_size = hwBlob.getInt32((_hidl_offset + 16) + 8);
        HwBlob childBlob = parcel.readEmbeddedBuffer((long) (_hidl_vec_size * 1), _hidl_blob.handle(), (_hidl_offset + 16) + 0, true);
        this.msg.clear();
        for (int _hidl_index_0 = 0; _hidl_index_0 < _hidl_vec_size; _hidl_index_0++) {
            this.msg.add(Byte.valueOf(childBlob.getInt8((long) (_hidl_index_0 * 1))));
        }
    }

    public final void writeToParcel(HwParcel parcel) {
        HwBlob _hidl_blob = new HwBlob(32);
        writeEmbeddedToBlob(_hidl_blob, 0);
        parcel.writeBuffer(_hidl_blob);
    }

    public static final void writeVectorToParcel(HwParcel parcel, ArrayList<ContextHubMsg> _hidl_vec) {
        HwBlob _hidl_blob = new HwBlob(16);
        int _hidl_vec_size = _hidl_vec.size();
        _hidl_blob.putInt32(8, _hidl_vec_size);
        int _hidl_index_0 = 0;
        _hidl_blob.putBool(12, false);
        HwBlob childBlob = new HwBlob(_hidl_vec_size * 32);
        while (_hidl_index_0 < _hidl_vec_size) {
            ((ContextHubMsg) _hidl_vec.get(_hidl_index_0)).writeEmbeddedToBlob(childBlob, (long) (_hidl_index_0 * 32));
            _hidl_index_0++;
        }
        _hidl_blob.putBlob(0, childBlob);
        parcel.writeBuffer(_hidl_blob);
    }

    public final void writeEmbeddedToBlob(HwBlob _hidl_blob, long _hidl_offset) {
        _hidl_blob.putInt64(_hidl_offset + 0, this.appName);
        _hidl_blob.putInt16(_hidl_offset + 8, this.hostEndPoint);
        _hidl_blob.putInt32(_hidl_offset + 12, this.msgType);
        int _hidl_vec_size = this.msg.size();
        _hidl_blob.putInt32((_hidl_offset + 16) + 8, _hidl_vec_size);
        int _hidl_index_0 = 0;
        _hidl_blob.putBool((_hidl_offset + 16) + 12, false);
        HwBlob childBlob = new HwBlob(_hidl_vec_size * 1);
        while (true) {
            int _hidl_index_02 = _hidl_index_0;
            if (_hidl_index_02 < _hidl_vec_size) {
                childBlob.putInt8((long) (_hidl_index_02 * 1), ((Byte) this.msg.get(_hidl_index_02)).byteValue());
                _hidl_index_0 = _hidl_index_02 + 1;
            } else {
                _hidl_blob.putBlob((16 + _hidl_offset) + 0, childBlob);
                return;
            }
        }
    }
}

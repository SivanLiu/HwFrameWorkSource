...
public class Gesture implements Parcelable {
    ...
    public float getLength() {
        ...
        for (int i = 0; i < strokes.size(); i++) {
            len = (int) (((float) len) + ((GestureStroke) strokes.get(i)).length);
        }
        return (float) len;
    }
    ...
    public Path toPath(Path path, int width, int height, int edge, int numSample) {
        ...
        int count = strokes.size();
        for (int i = 0; i < count; i++) {
            path.addPath(((GestureStroke) strokes.get(i)).toPath((float) (width - (2 * edge)), (float) (height - (2 * edge)), numSample));
        }
        return path;
    }
    ...
    public Bitmap toBitmap(int width, int height, int edge, int numSample, int color) {
        ...
        int count = strokes.size();
        for (int i = 0; i < count; i++) {
            canvas.drawPath(((GestureStroke) strokes.get(i)).toPath((float) (width - (2 * edge)), (float) (height - (2 * edge)), numSample), paint);
        }
        return bitmap;
    }

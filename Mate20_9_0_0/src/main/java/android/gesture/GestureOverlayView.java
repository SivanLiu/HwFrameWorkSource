...
public class GestureOverlayView extends FrameLayout {
        ...
        public void run() {
            if (GestureOverlayView.this.mIsFadingOut) {
                ...
                if (duration > GestureOverlayView.this.mFadeDuration) {
                    ...
                    GestureOverlayView.this.mFadingAlpha = 1.0f - GestureOverlayView.this.mInterpolator.getInterpolation(Math.max(0.0f, Math.min(1.0f, ((float) duration) / ((float) GestureOverlayView.this.mFadeDuration))));
    ...
    public void cancelGesture() {
        ...
        for (int i2 = 0; i2 < count; i2++) {
            ((OnGestureListener) listeners.get(i2)).onGestureCancelled(this, event);
        }
        event.recycle();
        clear(false);
        this.mIsGesturing = false;
        ...
        if (dx >= 3.0f || dy >= 3.0f) {
            ...
            if (!this.mHandleGestureActions || this.mIsGesturing) {
                ...
                if (this.mTotalLength > this.mGestureStrokeLengthThreshold) {
                    areaToRefresh2 = GestureUtils.computeOrientedBoundingBox(this.mStrokeBuffer);
                    x = Math.abs(areaToRefresh2.orientation);
                    if (x > 90.0f) {
                        x = 180.0f - x;
                    }
                    if (areaToRefresh2.squareness > this.mGestureStrokeSquarenessTreshold || (this.mOrientation != 1 ? x <= this.mGestureStrokeAngleThreshold : x >= this.mGestureStrokeAngleThreshold)) {
    ...
    private void touchUp(MotionEvent event, boolean cancel) {
        ...
        if (this.mCurrentGesture != null) {
            ...
            if (cancel) {
                ...
                for (int i2 = 0; i2 < count; i2++) {
                    ((OnGestureListener) listeners.get(i2)).onGestureEnded(this, event);
                }
                boolean z = true;

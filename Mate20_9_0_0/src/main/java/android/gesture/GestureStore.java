...
public class GestureStore {
    ...
    public void save(OutputStream stream, boolean closeStream) throws IOException {
        ...
        try {
            ...
            while (true) {
                ...
                Entry<String, ArrayList<Gesture>> entry = (Entry) it.next();
                ...
                out.writeInt(count);
                while (i < count) {
                    ((Gesture) examples.get(i)).serialize(out);
                    i++;
                }
            }
            out.flush();
    ...
    public void load(InputStream stream, boolean closeStream) throws IOException {
        try {
            DataInputStream in = new DataInputStream(stream instanceof BufferedInputStream ? stream : new BufferedInputStream(stream, 32768));
            if (in.readShort() == (short) 1) {
                readFormatV1(in);
            }
            if (closeStream) {

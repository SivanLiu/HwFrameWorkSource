...
public class SoterKeyStoreKeyPairGeneratorSpi extends HwUniversalKeyStoreKeyPairGeneratorSpi {
    ...
    protected void addExtraParameters(HwKeymasterArguments keymasterArgs) {
        ...
        if (this.isAutoSignedWithCommonkWhenGetPublicKey) {
            ...
            if (!SoterUtil.isNullOrNil(this.mAutoSignedKeyNameWhenGetPublicKey)) {
                ...
                keymasterArgs.addBytes(HwKeymasterDefs.KM_TAG_SOTER_AUTO_SIGNED_COMMON_KEY_WHEN_GET_PUBLIC_KEY, stringBuilder.toString().getBytes(StandardCharsets.UTF_8));
            }
        }
    ...
    protected byte[] getChallenge(HwKeyGenParameterSpec spec) {
        byte[] challenge = spec.getAttestationChallenge();
        if (challenge != null || (spec.getPurposes() & 16) == 0) {
            return challenge;
        }
        challenge = SOTER_DEFAULT_CHALLENGE.getBytes(StandardCharsets.UTF_8);

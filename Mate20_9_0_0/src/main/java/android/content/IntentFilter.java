...
public class IntentFilter implements Parcelable {
    ...
    public void writeToXml(XmlSerializer serializer) throws IOException {
        ...
        while (i2 < N) {
            ...
            PatternMatcher pe2 = (PatternMatcher) this.mDataPaths.get(i2);
            switch (pe2.getType()) {
                case 0:
            ...
            if (type != 3) {
                if (type != 4) {
                    ...
                    if (tagName.equals("action")) {
                        ...
                        ssp = parser.getAttributeValue(null, LITERAL_STR);
                        if (ssp != null) {
                            addDataSchemeSpecificPart(ssp, 0);
                        } else {
                            attributeValue = parser.getAttributeValue(null, PREFIX_STR);
                            ssp = attributeValue;
                            if (attributeValue != null) {
                                addDataSchemeSpecificPart(ssp, 1);
                            } else {
                                attributeValue2 = parser.getAttributeValue(null, SGLOB_STR);
                                ssp = attributeValue2;
                                if (attributeValue2 != null) {
                                    addDataSchemeSpecificPart(ssp, 2);
                                } else {
                                    attributeValue2 = parser.getAttributeValue(null, AGLOB_STR);
                                    ssp = attributeValue2;
                                    if (attributeValue2 != null) {
                        ...
                        attributeValue2 = parser.getAttributeValue(null, "port");
                        if (name != null) {
                            addDataAuthority(name, attributeValue2);
                        }
    ...
    public void writeToProto(ProtoOutputStream proto, long fieldId) {
        ...
        if (this.mDataSchemes != null) {
            ...
            while (it.hasNext()) {
                proto.write(IntentFilterProto.DATA_SCHEMES, (String) it.next());
            }
        }
        if (this.mDataSchemeSpecificParts != null) {
        ...
        if (this.mDataPaths != null) {
            ...
            while (it2.hasNext()) {
                ((PatternMatcher) it2.next()).writeToProto(proto, IntentFilterProto.DATA_PATHS);
            }
        }
        if (this.mDataTypes != null) {
            it = this.mDataTypes.iterator();
            while (it.hasNext()) {
                proto.write(IntentFilterProto.DATA_TYPES, (String) it.next());
            }
        }

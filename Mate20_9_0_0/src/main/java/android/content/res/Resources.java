...
public class Resources extends AbsResources {
    ...
    public Resources(AssetManager assets, DisplayMetrics metrics, Configuration config) {
        ...
        CompatibilityInfo makeCompatibilityInfo = (metrics == null || metrics.noncompatWidthPixels == 0 || Float.compare((float) metrics.widthPixels, (float) metrics.noncompatWidthPixels) == 0) ? CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO : CompatibilityInfo.makeCompatibilityInfo((((float) metrics.widthPixels) * 1.0f) / ((float) metrics.noncompatWidthPixels));
        ...
        try {
            ...
            if (value.type == 5) {
                ...
                if (isTypeMM(value.data) && isRogOn()) {
                    int rogRes = (int) ((((float) res) * getMMScale()) + 1056964608);
                    if (id == R.dimen.navigation_bar_height && (rogRes & 1) == 1) {
                        rogRes++;
                    }
                    releaseTempTypedValue(value);
                    return rogRes;
    ...
    public int getDimensionPixelSize(int id) throws NotFoundException {
        ...
        try {
            ...
            if (value.type == 5) {
                ...
                int rebuildSpecialDimens = rebuildSpecialDimens(id, value, res);
                releaseTempTypedValue(value);
                return rebuildSpecialDimens;
            }
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Resource ID #0x");
            stringBuilder.append(Integer.toHexString(id));
    ...
    private int rebuildSpecialDimens(int id, TypedValue value, int res) {
        if (!((id != R.dimen.status_bar_height && id != R.dimen.status_bar_height_portrait && id != R.dimen.status_bar_height_landscape) || isLockDpi() || isTypeMM(value.data))) {
            ...
            if (dpi > 0 && dpi != srcDpi && isSupprotLockDpi) {
                res = (((res * srcDpi) + dpi) - 1) / dpi;
            }
        }
        if (id == R.dimen.navigation_bar_height && isTypeMM(value.data) && (res & 1) == 1) {
            return res + 1;
        }
        return res;
    }
    ...
    private boolean isLockDpi() {
        ...
        return multidpiInfo == null ? false : multidpiInfo.getBoolean("LockDpi", false);
    }
    ...
    public static float getMMScale() {
        int dpi = "persist.sys.dpi";
        dpi = SystemProperties.getInt(dpi, SystemProperties.getInt("ro.sf.real_lcd_density", SystemProperties.getInt("ro.sf.lcd_density", 0)));
    ...
    public Drawable getDrawable(int id) throws NotFoundException {
        ...
        if (d != null && d.canApplyTheme()) {
            ...
            Log.w(str, stringBuilder.toString(), new RuntimeException());
        }
        return d;
    }
    ...
    protected TypedValue hwObtainTempTypedValue() {
        return obtainTempTypedValue();
    }

    protected void hwReleaseTempTypedValue(TypedValue value) {
        releaseTempTypedValue(value);
    }
    ...
    public static Bundle getPreMultidpiInfo(String packageName) {
        ...
        synchronized (sMultidpiInfosLock) {
            dpiInfo = (Bundle) sMultidpiInfos.get(packageName);
        }
        if (dpiInfo != null) {
            return dpiInfo;
        }
        ...
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("getApplicationInfo for ");

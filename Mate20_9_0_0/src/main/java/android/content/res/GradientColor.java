...
public class GradientColor extends ComplexColor {
    ...
    /* JADX WARNING: Missing block: B:21:0x00c2, code skipped:
    ...
    private void inflateChildElements(Resources r, XmlPullParser parser, AttributeSet attrs, Theme theme) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (next < innerDepth && type == 3) {
                ...
                i4 = type;
                break;
            } else if (type == 2 && depth <= innerDepth && parser.getName().equals(HwThemeManager.TAG_ITEM)) {
                theme2 = theme;
                TypedArray a = Resources.obtainAttributes(r, theme2, attrs, R.styleable.GradientColorItem);
                boolean hasColor = a.hasValue(i2);
                boolean hasOffset = a.hasValue(i);
                if (hasColor && hasOffset) {
                    int[] themeAttrs = a.extractThemeAttrs();
                    i3 = innerDepth;
                    innerDepth = a.getColor(i2, i2);
                    ...
                    colorList = GrowingArrayUtils.append(colorList, listSize, innerDepth);

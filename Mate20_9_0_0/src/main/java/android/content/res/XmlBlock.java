...
final class XmlBlock implements AutoCloseable {
        ...
        public String nextText() throws XmlPullParserException, IOException {
            if (getEventType() == 2) {
                ...
                if (eventType == 4) {
                    ...
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append(getPositionDescription());
                    stringBuilder.append(": event TEXT it must be immediately followed by END_TAG");
                    ...
                    stringBuilder2.append(": parser must be on START_TAG or TEXT to read text");
                    throw new XmlPullParserException(stringBuilder2.toString(), this, null);
                }
            }
            StringBuilder stringBuilder3 = new StringBuilder();
            stringBuilder3.append(getPositionDescription());
            stringBuilder3.append(": parser must be on START_TAG to read next text");
            throw new XmlPullParserException(stringBuilder3.toString(), this, null);
        }
    ...
    public XmlResourceParser newParser() {
        synchronized (this) {
            if (this.mNative != 0) {
                Parser parser = new Parser(nativeCreateParseState(this.mNative), this);
                return parser;
            }
            return null;
        }

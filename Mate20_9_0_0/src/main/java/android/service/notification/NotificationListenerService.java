...
public abstract class NotificationListenerService extends Service {
    ...
    public static class RankingMap implements Parcelable {
        ...
        public boolean getRanking(String key, Ranking outRanking) {
            ...
            outRanking.populate(key, rank, isIntercepted(key) ^ 1, getVisibilityOverride(key), getSuppressedVisualEffects(key), getImportance(key), getImportanceExplanation(key), getOverrideGroupKey(key), getChannel(key), getOverridePeople(key), getSnoozeCriteria(key), getShowBadge(key), getUserSentiment(key), getHidden(key));
            if (rank >= 0) {
                return true;
            }
            return false;
        ...
        private int getUserSentiment(String key) {
            ...
            Integer userSentiment = (Integer) this.mUserSentiment.get(key);
            return userSentiment == null ? 0 : userSentiment.intValue();
        }
        ...
        private boolean getHidden(String key) {
            ...
            Boolean hidden = (Boolean) this.mHidden.get(key);
        ...
        public void onNotificationPosted(IStatusBarNotificationHolder sbnHolder, NotificationRankingUpdate update) {
            try {
                ...
                if (sbn != null) {
                    ...
                    Log.w(NotificationListenerService.this.TAG, "sbn is null");
                }
    ...
    private void maybePopulatePeople(Notification notification) {
        if (getContext().getApplicationInfo().targetSdkVersion < 28) {
            ArrayList<Person> people = notification.extras.getParcelableArrayList("android.people.list");

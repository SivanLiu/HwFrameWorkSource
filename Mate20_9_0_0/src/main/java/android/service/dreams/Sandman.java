...
public final class Sandman {
    ...
    private static boolean isScreenSaverEnabled(Context context) {
        if (Secure.getIntForUser(context.getContentResolver(), Secure.SCREENSAVER_ENABLED, context.getResources().getBoolean(17956941) ? 1 : 0, -2) != 0) {
            return true;
        }
        return false;
    }
    ...
    private static boolean isScreenSaverActivatedOnDock(Context context) {
        if (Secure.getIntForUser(context.getContentResolver(), Secure.SCREENSAVER_ACTIVATE_ON_DOCK, context.getResources().getBoolean(17956939) ? 1 : 0, -2) != 0) {
            return true;
        }
        return false;
    }

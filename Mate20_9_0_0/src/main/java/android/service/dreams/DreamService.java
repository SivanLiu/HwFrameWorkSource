...
public class DreamService extends Service implements Callback {
    ...
    private HwCustDreamService mCust = ((HwCustDreamService) HwCustUtils.createObj(HwCustDreamService.class, new Object[]{this}));
    ...
    private final void attach(IBinder windowToken, boolean canDoze, final IRemoteCallback started) {
        if (this.mWindowToken != null) {
            String str = this.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("attach() called when already attached with token=");
            stringBuilder.append(this.mWindowToken);
            ...
            if (!this.mWindowless || this.mCanDoze) {
                if (!this.mWindowless) {
                    ...
                    if (!(this.mCust == null || this.mCust.isChargingAlbumEnabled())) {
                        applySystemUiVisibilityFlags(this.mLowProfile, 1);
                    }
                    try {
    ...
    protected void dumpOnHandler(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (this.mWindowToken == null) {
            ...
            stringBuilder.append("running (token=");
            stringBuilder.append(this.mWindowToken);
            stringBuilder.append(")");

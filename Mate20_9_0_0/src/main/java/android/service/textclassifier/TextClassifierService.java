...
public abstract class TextClassifierService extends Service {
    ...
    private final Stub mBinder = new Stub() {
        ...
        public void onSuggestSelection(TextClassificationSessionId sessionId, Request request, final ITextSelectionCallback callback) throws RemoteException {
            Preconditions.checkNotNull(request);
            Preconditions.checkNotNull(callback);
            TextClassifierService.this.onSuggestSelection(request.getText(), request.getStartIndex(), request.getEndIndex(), Options.from(sessionId, request), this.mCancellationSignal, new Callback<TextSelection>() {
    ...
    public void onSuggestSelection(CharSequence text, int selectionStartIndex, int selectionEndIndex, Options options, CancellationSignal cancellationSignal, Callback<TextSelection> callback) {
        Request request;
        TextClassificationSessionId sessionId = options.getSessionId();
        if (options.getRequest() != null) {
            request = options.getRequest();
        } else {
            request = new Builder(text, selectionStartIndex, selectionEndIndex).setDefaultLocales(options.getDefaultLocales()).build();
        }
    ...
    public void onGenerateLinks(CharSequence text, TextLinks.Options options, CancellationSignal cancellationSignal, Callback<TextLinks> callback) {
        ...
        onGenerateLinks(sessionId, request, cancellationSignal, (Callback) callback);
    }

...
public class DiskInfo implements Parcelable {
    ...
    private boolean isInteresting(String label) {
        if (TextUtils.isEmpty(label) || label.equalsIgnoreCase("ata") || label.toLowerCase().contains("generic") || label.toLowerCase().startsWith("usb") || label.toLowerCase().startsWith("multiple")) {
            return false;
        }
        return true;
    }
    ...
    public String getDescription() {
        ...
        if ((this.flags & 4) != 0) {
            if (!isInteresting(this.label)) {
                return res.getString(17041214);
            }
            return res.getString(17041215, new Object[]{this.label});
            ...
            if (!isInteresting(this.label)) {
                return res.getString(17041217);
            }
            return res.getString(17041218, new Object[]{this.label});
        }
    }

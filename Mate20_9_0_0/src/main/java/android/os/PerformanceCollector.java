...
public class PerformanceCollector {
    ...
    private void endPerformanceSnapshot() {
        ...
        long dalvikFree2 = runtime.freeMemory() / Trace.TRACE_TAG_CAMERA;
        long dalvikAllocated2 = dalvikMax - dalvikFree2;
        Bundle binderCounts = getBinderCounts();
        Iterator it = binderCounts.keySet().iterator();
        while (it.hasNext()) {
            Iterator it2 = it;
            key = (String) it.next();
        ...
        nativeMax = dalvikFree;
        this.mPerfSnapshot.putLong(METRIC_KEY_JAVA_FREE, nativeMax);

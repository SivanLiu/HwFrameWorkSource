...
public class BackDropperFilter extends Filter {
    ...
    private static final String[] mInputNames = new String[]{MetricsConstants.MODE_VIDEO, "background"};
    ...
    private static final String[] mOutputNames = new String[]{MetricsConstants.MODE_VIDEO};
    ...
    public void setupPorts() {
        ...
        for (String inputName2 : mOutputNames) {
            addOutputBasedOnInput(inputName2, MetricsConstants.MODE_VIDEO);
        }
        if (this.mProvideDebugOutputs) {
            String[] strArr = mDebugOutputNames;
            length = strArr.length;
            while (i < length) {
                addOutputBasedOnInput(strArr[i], MetricsConstants.MODE_VIDEO);
                i++;
            }
        }
    }
    ...
    private void allocateFrames(FrameFormat inputFormat, FilterContext context) {
        if (createMemoryFormat(inputFormat)) {
            ...
            for (i = 0; i < numBytes; i++) {
                initialBgMean[i] = BluetoothHidDevice.SUBCLASS1_MOUSE;
            ...
            stringBuilder2.append(mBgDistanceShader);
            this.mBgDistProgram = new ShaderProgram(context, stringBuilder2.toString());
            this.mBgDistProgram.setHostValue("subsample_level", Float.valueOf((float) this.mSubsampleLevel));
            stringBuilder2 = new StringBuilder();
            stringBuilder2.append(mSharedUtilShader);
            stringBuilder2.append(mBgMaskShader);
            this.mBgMaskProgram = new ShaderProgram(context, stringBuilder2.toString());
            ...
            if (this.mUseTheForce) {
                ...
                stringBuilder.append(mBgSubtractShader);
                stringBuilder.append(mBgSubtractForceShader);
                this.mBgSubtractProgram = new ShaderProgram(context, stringBuilder.toString());
            } else {
                stringBuilder = new StringBuilder();
                stringBuilder.append(mSharedUtilShader);
                stringBuilder.append(mBgSubtractShader);
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append(mSharedUtilShader);
            stringBuilder.append(mUpdateBgModelMeanShader);
            this.mBgUpdateMeanProgram = new ShaderProgram(context, stringBuilder.toString());
            this.mBgUpdateMeanProgram.setHostValue("subsample_level", Float.valueOf((float) this.mSubsampleLevel));
            stringBuilder = new StringBuilder();
            stringBuilder.append(mSharedUtilShader);
            stringBuilder.append(mUpdateBgModelVarianceShader);
            this.mBgUpdateVarianceProgram = new ShaderProgram(context, stringBuilder.toString());
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append(mSharedUtilShader);
            stringBuilder.append(mAutomaticWhiteBalance);
            this.mAutomaticWhiteBalanceProgram = new ShaderProgram(context, stringBuilder.toString());
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append(mSharedUtilShader);
            stringBuilder.append(mMaskVerifyShader);
            this.mMaskVerifyProgram = new ShaderProgram(context, stringBuilder.toString());
    ...
    public void process(FilterContext context) {
        Frame output;
        Frame video = pullInput(MetricsConstants.MODE_VIDEO);
        ...
        if (this.mFrameCount <= this.mLearningDuration) {
            pushOutput(MetricsConstants.MODE_VIDEO, video);
            ...
            if (this.mFrameCount == this.mLearningDuration) {
                ...
                if (this.mLogVerbose) {
                    ...
                    objArr[0] = Integer.valueOf(bi);
                    i = 20;
                    z = true;
                    objArr[1] = Integer.valueOf(20);
        ...
        if (this.mProvideDebugOutputs) {
            output = context.getFrameManager().newFrame(video.getFormat());
            this.mCopyOutProgram.process(video, output);
            pushOutput("debug1", output);
            output.release();
        ...
        if (!this.mLogVerbose || this.mFrameCount % 30 != 0) {
            ...
            objArr2[0] = Double.valueOf(((double) (endTime - this.startTime)) / 30.0d);
            stringBuilder.append(String.format("%.2f", objArr2));
            stringBuilder.append(" ms. Avg. fps: ");
            stringBuilder.append(String.format("%.2f", new Object[]{Double.valueOf(1000.0d / (((double) (endTime - this.startTime)) / 30.0d))}));

...
public final class SmsManager {
    ...
    private void sendMultipartTextMessageInternal(String destinationAddress, String scAddress, List<String> parts, List<PendingIntent> sentIntents, List<PendingIntent> deliveryIntents, boolean persistMessage) {
        ...
        if (TextUtils.isEmpty(destinationAddress)) {
            ...
            sendTextMessage(destinationAddress, scAddress, (String) list.get(0), sentIntent, deliveryIntent);
        }
    }
    ...
    private void sendMultipartTextMessageInternal(String destinationAddress, String scAddress, List<String> parts, List<PendingIntent> sentIntents, List<PendingIntent> deliveryIntents, boolean persistMessage, int priority, boolean expectMore, int validityPeriod) {
        ...
        if (TextUtils.isEmpty(destinationAddress)) {
            ...
            sendTextMessageInternal(destinationAddress, scAddress, (String) list.get(0), sentIntent, deliveryIntent2, persistMessage, priority, expectMore, validityPeriod);
        }
    }
    ...
    public static Bundle getMmsConfig(BaseBundle config) {
        ...
        filtered.putBoolean("smsDeliveryReportSettingOnByDefault", config.getBoolean("smsDeliveryReportSettingOnByDefault"));
        filtered.putBoolean("enableWapPushSI", config.getBoolean("enableWapPushSI"));
        filtered.putBoolean("allowEnablingWapPushSI", config.getBoolean("allowEnablingWapPushSI"));
        return filtered;
    }
    ...
    public boolean setCellBroadcastRangeList(int[] messageIds, int ranType) {
        return HwFrameworkFactory.getHwBaseInnerSmsManager().setCellBroadcastRangeList(getSubscriptionId(), messageIds, ranType);
    }
}

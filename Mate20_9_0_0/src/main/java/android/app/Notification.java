...
public class Notification implements Parcelable {
    ...
    public static class Builder {
        ...
        private void bindProfileBadge(RemoteViews contentView) {
            ...
            if (profileBadge != null) {
                ...
                if (isColorized()) {
                    contentView.setDrawableTint(R.id.profile_badge, false, getPrimaryTextColor(), Mode.SRC_ATOP);
                }
            }
        }
        ...
        private RemoteViews applyStandardTemplate(int resId, boolean hasProgress, TemplateBindResult result) {
            ...
            if (this.mPrimaryTextColor == 1 || this.mSecondaryTextColor == 1 || this.mTextColorsAreForBackground != backgroundColor) {
                ...
                if (hasForegroundColor() && isColorized()) {
                    ...
                    if (contrast >= 4.5d) {
                        ...
                        this.mSecondaryTextColor = NotificationColorUtil.changeColorLightness(i3, i2);
                        if (NotificationColorUtil.calculateContrast(this.mSecondaryTextColor, backgroundColor) < 4.5d) {
                            ...
                            this.mPrimaryTextColor = NotificationColorUtil.changeColorLightness(i4, i);
                            return;
                        }
                        return;
        ...
        private void bindLargeIconAndReply(RemoteViews contentView, StandardTemplateParams p, TemplateBindResult result) {
            boolean replyIconShown = true;
            int i = 0;
            boolean largeIconShown = p.hideLargeIcon || p.ambient;
            largeIconShown = bindLargeIcon(contentView, largeIconShown);
            if (!(p.hideReplyIcon || p.ambient)) {
                replyIconShown = false;
            }
            replyIconShown = bindReplyIcon(contentView, replyIconShown);
        ...
        private boolean bindReplyIcon(RemoteViews contentView, boolean hideReplyIcon) {
            boolean actionVisible = hideReplyIcon ^ 1;
            Action action = null;
            int i = 0;
            if (actionVisible) {
            ...
            if (actionVisible) {
                ...
                contentView.setDrawableTint(R.id.reply_icon_action, false, getNeutralColor(), Mode.SRC_ATOP);
        ...
        public String loadHeaderAppName() {
            ...
            if (this.mN.extras.containsKey(Notification.EXTRA_SUBSTITUTE_APP_NAME)) {
                ...
                if (pm.checkPermission(permission.SUBSTITUTE_NOTIFICATION_APP_NAME, pkg) == 0) {
                    ...
                    stringBuilder.append(permission.SUBSTITUTE_NOTIFICATION_APP_NAME);
        ...
        private RemoteViews applyStandardTemplateWithActions(int layoutId, StandardTemplateParams p, TemplateBindResult result) {
            ...
            boolean validRemoteInput2 = false;
            ...
            if (N > 0) {
                ...
                for (int i2 = 0; i2 < N; i2++) {
                    Action action = (Action) this.mActions.get(i2);
                    if (action != null) {
                        validRemoteInput |= hasValidRemoteInput(action);
                        big.addView(R.id.actions, generateActionButton(action, false, p.ambient));
                    }
                }
                validRemoteInput2 = validRemoteInput;
            } else {
            ...
            if (!p.ambient && validRemoteInput2 && replyText != null && replyText.length > 0 && !TextUtils.isEmpty(replyText[0]) && p.maxRemoteInputHistory > 0) {
                validRemoteInput = this.mN.extras.getBoolean(Notification.EXTRA_SHOW_REMOTE_INPUT_SPINNER);
        ...
        public RemoteViews createLineView() {
            RemoteViews contentView = new BuilderRemoteViews(this.mContext.getApplicationInfo(), 34013310);
            contentView.setOnClickPendingIntent(34603224, this.mN.contentIntent != null ? this.mN.contentIntent : this.mN.fullScreenIntent);
            contentView.setTextViewText(16908310, null);
            contentView.setTextViewText(R.id.text, null);
            contentView.setTextViewText(R.id.app_name_text, null);
            ...
            bindExpandButton(contentView);
            makeHeaderUnExpanded(contentView);
            return contentView;
        }
        ...
        private RemoteViews generateActionButton(Action action, boolean emphazisedMode, boolean ambient) {
            ...
            if (this.mN.extras.getBoolean(Notification.EXTRA_SHOW_ACTION_ICON)) {
                ...
                button.setTextViewCompoundDrawablesWithBounds(R.id.action0, action.getIcon(), null, null, null, actionIconLength, actionIconLength, this.mContext.getResources().getDimensionPixelSize(34472165));
            }
            if (IS_HONOR_PRODUCT) {
                button.setTextColor(R.id.action0, this.mContext.getResources().getColorStateList(33882454, null));
            }
            if (!tombstone) {
                button.setOnClickPendingIntent(R.id.action0, action2.actionIntent);
            }
            button.setContentDescription(R.id.action0, action2.title);
            if (action.mRemoteInputs != null) {
                button.setRemoteInputs(R.id.action0, action.mRemoteInputs);
            }
            if (emphazisedMode) {
                int rippleColor;
                CharSequence title = action2.title;
                ...
                if (hasColorOverride) {
                    ...
                    rippleColor = NotificationColorUtil.resolvePrimaryColor(this.mContext, background);
                ...
                button.setColorStateList(R.id.action0, "setRippleColor", ColorStateList.valueOf((AdvertisingSetParameters.INTERVAL_MAX & rippleColor) | 855638016));
                button.setColorStateList(R.id.action0, "setButtonBackground", ColorStateList.valueOf(background));
                String str = "setHasStroke";
                if (hasColorOverride) {
                    z = false;
                }
                button.setBoolean(R.id.action0, str, z);
            } else {
                button.setTextViewText(R.id.action0, processTextSpans(processLegacyText(action2.title)));
                if (isColorized() && !ambient) {
                    setTextViewColorPrimary(button, R.id.action0);
            ...
            while (i2 < length) {
                ...
                if (resultSpan2 instanceof TextAppearanceSpan) {
                    ...
                    if (textColor != null) {
                        int[] colors;
                        spans2 = spans;
                        spans = textColor.getColors();
                        i3 = length;
                        int[] newColors = new int[spans.length];
                        int i5 = 0;
                        while (true) {
                            i4 = i2;
                            i2 = i5;
                            if (i2 >= newColors.length) {
                                break;
                            }
                            colors = spans;
                            newColors[i2] = NotificationColorUtil.ensureLargeTextContrast(spans[i2], i, this.mInNightMode);
                            i5 = i2 + 1;
                            i2 = i4;
                            spans = colors;
                        ...
                        if (fullLength) {
                            outResultColor[null] = textColor2;
                            textColor2 = null;
                        }
                    ...
                    if (resultSpan2 instanceof ForegroundColorSpan) {
                        spans = NotificationColorUtil.ensureLargeTextContrast(resultSpan2.getForegroundColor(), i, this.mInNightMode);
                        if (fullLength) {
                            z2 = false;
                            outResultColor[0] = ColorStateList.valueOf(spans);
                            resultSpan = null;
                        } else {
                            z2 = false;
                            resultSpan = new ForegroundColorSpan(spans);
                        }
                    } else {
                        z2 = false;
                        resultSpan = span;
        ...
        private void processSmallIconColor(Icon smallIcon, RemoteViews contentView, boolean ambient) {
            ...
            contentView.setInt(R.id.notification_header, str, i);
        }

        private boolean isPureColorIcon(Icon icon) {
            int type = getHWColorUtil().getSmallIconColorType(this.mContext, icon);
            if (type != 4) {
                switch (type) {
                    case 0:
                    case 1:
        ...
        private void processLargeLegacyIcon(Icon largeIcon, RemoteViews contentView) {
            if (largeIcon != null && isLegacy() && isPureColorIcon(largeIcon)) {
                contentView.setDrawableTint(R.id.right_icon, false, resolveContrastColor(), Mode.SRC_ATOP);
            }
        }
    ...
    private static class StandardTemplateParams {
        ...
        final StandardTemplateParams ambient(boolean ambient) {
            boolean z = this.title == null && this.text == null;
    ...
    public static class Action implements Parcelable {
        ...
        public static final class Builder {
            ...
            public Action build() {
                ...
                return new Action(this.mIcon, this.mTitle, this.mIntent, this.mExtras, textInputs.isEmpty() ? null : (RemoteInput[]) textInputs.toArray(new RemoteInput[textInputs.size()]), this.mAllowGeneratedReplies, this.mSemanticAction, null);
            }
        }
        ...
        /* synthetic */ Action(Icon x0, CharSequence x1, PendingIntent x2, Bundle x3, RemoteInput[] x4, boolean x5, int x6, AnonymousClass1 x7) {
        ...
        private Action(Icon icon, CharSequence title, PendingIntent intent, Bundle extras, RemoteInput[] remoteInputs, boolean allowGeneratedReplies, int semanticAction) {
            this.mAllowGeneratedReplies = true;
            this.mIcon = icon;
            if (icon != null && icon.getType() == 2) {
    ...
    public static class DecoratedCustomViewStyle extends Style {
        ...
        private void buildIntoRemoteViewContent(RemoteViews remoteViews, RemoteViews customContent, TemplateBindResult result) {
            ...
            remoteViews.setViewLayoutMarginBottomDimen(R.id.notification_main_column, 34472294);
            remoteViews.setViewLayoutMarginTopDimen(R.id.notification_main_column, 34472295);
            remoteViews.setUseAppContext(true);
        }
    ...
    public static class InboxStyle extends Style {
        ...
        public RemoteViews makeBigContentView() {
            ...
            if (remoteInputHistory != null && remoteInputHistory.length > 1) {
                totalNumRows = (this.mTexts.size() + Math.min(remoteInputHistory.length, 3)) - 1;
                if (totalNumRows > maxRows) {
                    first = totalNumRows - maxRows;
            ...
            while (totalNumRows < this.mTexts.size() && totalNumRows < maxRows2) {
                CharSequence str = (CharSequence) this.mTexts.get(totalNumRows);
                if (TextUtils.isEmpty(str)) {
                    ...
                    this.mBuilder.setTextViewColorSecondary(contentView, rowIds[totalNumRows]);
                    z = first;
                    contentView.setViewPadding(rowIds[totalNumRows], 0, topPadding, 0, false);
                    handleInboxImageMargin(contentView, rowIds[totalNumRows], z, result.getIconMarginEnd());
                    if (z) {
                        i2 = rowIds[totalNumRows];
                    } else {
                        i2 = 0;
                    }
                    onlyViewId = i2;
    ...
    public static class MediaStyle extends Style {
        ...
        private RemoteViews generateMediaActionButton(Action action, int color) {
            ...
            int tintColor = (this.mBuilder.shouldTintActionButtons() || this.mBuilder.isColorized()) ? color : NotificationColorUtil.resolveColor(this.mBuilder.mContext, 0);
            button.setDrawableTint(R.id.action0, false, tintColor, Mode.SRC_ATOP);
            if (!tombstone) {
        ...
        private RemoteViews makeMediaContentView() {
            RemoteViews view = this.mBuilder.applyStandardTemplate((int) R.layout.notification_template_material_media, false, null);
        ...
        private RemoteViews makeMediaBigContentView() {
            ...
            RemoteViews big = this.mBuilder.applyStandardTemplate((int) R.layout.notification_template_material_big_media, false, null);
            if (actionCount > 0) {
    ...
    public static class MessagingStyle extends Style {
                    ...
                    if (bundle.containsKey("text")) {
                        if (bundle.containsKey(KEY_TIMESTAMP)) {
                            ...
                            if (senderPerson == null) {
                                ...
                                if (senderName != null) {
                                    senderPerson = new android.app.Person.Builder().setName(senderName).build();
                                }
                            }
        ...
        private RemoteViews makeMessagingView(boolean displayImagesAtEnd, boolean hideRightIcons) {
            ...
            String str = "setLayoutColor";
            if (this.mBuilder.isColorized()) {
                primaryTextColor = this.mBuilder.getPrimaryTextColor();
            } else {
                primaryTextColor = this.mBuilder.resolveContrastColor();
            }
            contentView.setInt(R.id.status_bar_latest_event_content, str, primaryTextColor);
        ...
        private CharSequence createConversationTitleFromMessages() {
            ...
            for (int i2 = 0; i2 < this.mMessages.size(); i2++) {
                Person sender = ((Message) this.mMessages.get(i2)).getSenderPerson();
                if (sender != null) {
            ...
            int size = names.size();
            while (i < size) {
                CharSequence name = (CharSequence) names.valueAt(i);
    ...
    public void visitUris(Consumer<Uri> visitor) {
        ...
        if (MessagingStyle.class.equals(getNotificationStyle()) && this.extras != null) {
            ...
            if (!ArrayUtils.isEmpty(messages)) {
                for (Message message : Message.getMessagesFromBundleArray(messages)) {
    ...
    public static CharSequence safeCharSequence(CharSequence cs) {
        ...
        Object cs2;
        if (cs2.length() > MAX_CHARSEQUENCE_LENGTH) {
            cs2 = cs2.subSequence(0, MAX_CHARSEQUENCE_LENGTH);
        }
    ...
    public boolean isColorizedMedia() {
        ...
        if (MediaStyle.class.equals(style)) {
            Boolean tmpColorized = this.extras.get(EXTRA_COLORIZED);
            if (tmpColorized instanceof Boolean) {
                Boolean colorized = tmpColorized;

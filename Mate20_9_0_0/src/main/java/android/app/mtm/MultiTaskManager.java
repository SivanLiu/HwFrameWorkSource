...
public class MultiTaskManager {
    ...
    public static MultiTaskManager getInstance() {
        synchronized (mLock) {
            if (SystemProperties.getBoolean("ro.config.enable_iaware", false)) {
                if (instance == null) {
                    if (Log.HWINFO) {
    ...
    public void requestAppCleanFromPG(List<String> pkgName, List<Integer> userId, int level, final String reason) {
        requestAppCleanWithCallback(new Builder(AppCleanSource.POWER_GENIE.ordinal()).level(level).stringList(pkgName).intList(userId).build(), new IAppCleanCallback.Stub() {
            public void onCleanFinish(AppCleanParam result) {
                String str = MultiTaskManager.TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("onCleanFinish:");
    ...
    public void requestAppClean(List<String> pkgName, List<Integer> userId, int level, String reason, int source) {
        requestAppCleanWithCallback(new Builder(source).level(level).stringList(pkgName).intList(userId).build(), null);
    }
}

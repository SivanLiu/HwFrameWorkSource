...
public class SearchManager implements android.content.DialogInterface.OnDismissListener, android.content.DialogInterface.OnCancelListener {
    ...
    private static final String CALENDAR_PROVIDERS = "com.android.providers.calendar";
    private static final String CONTACTS_PKG = "com.android.contacts";
    private static final String CONTACTS_PROVIDERS = "com.android.providers.contacts";
    ...
    private MatrixCursor cursor = new MatrixCursor((String[]) this.columns.toArray(this.strArray));
    ...
    private IIndexSearchManager mIndexSearchManager;
    ...
    void startGlobalSearch(String initialQuery, boolean selectInitialQuery, Bundle appSearchData, Rect sourceBounds) {
        ...
        if (!appSearchData.containsKey(Slice.SUBTYPE_SOURCE)) {
            appSearchData.putString(Slice.SUBTYPE_SOURCE, this.mContext.getPackageName());
        }
        ...
        try {
            ...
            stringBuilder.append(globalSearchActivity);
    ...
    public void trigerGlobalSearchService(boolean enabled) {
        if (enabled) {
            ...
            this.mIndexSearchManager = HwFrameworkFactory.getIndexSearchManager();
            if (this.mIndexSearchManager != null) {
                this.mIndexSearchManager.connect();
                return;
            }
            return;
        }
        ...
        if (this.mIndexSearchManager != null) {
            this.mIndexSearchManager.destroy();
            this.mIndexSearchManager = null;
        }
    }
    ...
    public Cursor getSuggestions(SearchableInfo searchable, String query, int limit) {
        ...
        String pkgName = searchable.getSuggestPackage();
        if (CALENDAR_PROVIDERS.equals(pkgName)) {
            pkgName = CALENDAR_PKG;
        }
        if (CONTACTS_PROVIDERS.equals(pkgName)) {
            pkgName = CONTACTS_PKG;
        }
        String pkgName2 = pkgName;
        if (Secure.getInt(this.mContext.getContentResolver(), "app_lock_func_status", 0) == 1) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(Secure.getString(this.mContext.getContentResolver(), "app_lock_list"));
            stringBuilder.append(";");
            pkgName = stringBuilder.toString();
            StringBuilder stringBuilder2 = new StringBuilder();
            stringBuilder2.append(pkgName2);
            stringBuilder2.append(";");
            if (pkgName.contains(stringBuilder2.toString())) {
                return this.cursor;
            }
        }
            ...
            try {
                if (this.mIndexSearchManager != null) {
                    for (int times = 10; times > 0; times--) {
                        if (this.mIndexSearchManager.hasConnected()) {
                            Log.d(TAG, " getSuggestions call IndexSearchManager.search()");
                            return this.mIndexSearchManager.search(searchable.getSuggestPackage(), str, searchable.getFullTextSearchFields());
                        }
                        Log.d(TAG, " getSuggestions IndexSearchManager not connected");
                        Thread.sleep(10);
                    }
                }
        ...
        pkgName = searchable.getSuggestAuthority();
        if (pkgName == null) {
            return null;
        }
        String[] selArgs;
        Builder uriBuilder = new Builder().scheme("content").authority(pkgName).query("").fragment("");
    ...
    public List<SearchableInfo> getOnlineSearchablesInGlobalSearch() {
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("getOnlineSearchablesInGlobalSearch() failed: ");
    ...
    public Intent getAssistIntent(boolean inclContext) {
        try {
            ...
            if (inclContext) {
                Bundle extras = ActivityManager.getService().getAssistContextExtras(null);
                if (extras != null) {

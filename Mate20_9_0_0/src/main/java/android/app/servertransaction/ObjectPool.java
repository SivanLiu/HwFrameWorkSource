...
class ObjectPool {
    ...
    public static <T extends ObjectPoolItem> T obtain(Class<T> itemClass) {
        synchronized (sPoolSync) {
            ArrayList<T> itemPool = (ArrayList) sPoolMap.get(itemClass);
            if (itemPool == null || itemPool.isEmpty()) {
                return null;
            }
            ObjectPoolItem objectPoolItem = (ObjectPoolItem) itemPool.remove(itemPool.size() - 1);
    ...
    public static <T extends ObjectPoolItem> void recycle(T item) {
        synchronized (sPoolSync) {
            ArrayList<T> itemPool = (ArrayList) sPoolMap.get(item.getClass());
            if (itemPool == null) {

...
public class NativeActivity extends Activity implements Callback2, Callback, OnGlobalLayoutListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (ai.metaData != null) {
                ln = ai.metaData.getString(META_DATA_LIB_NAME);
                if (ln != null) {
                    libname = ln;
                }
            ...
            libname = classLoader.findLibrary(libname2);
            if (libname == null) {
                ln = System.getenv("LD_LIBRARY_PATH");
                if (ln != null) {
                    String[] paths = ln.split(":");
                    String libName = System.mapLibraryName(libname2);
                    for (String file : paths) {
                        File libraryFile = new File(file, libName);
                        if (libraryFile.exists()) {
                            libname = libraryFile.getPath();
                            break;
                        }
                    }
                }
            ...
            funcname.append("Unable to find native library ");
            funcname.append(libname2);
            funcname.append(" using classloader: ");
            funcname.append(classLoader2.toString());
            throw new IllegalArgumentException(funcname.toString());

...
public class DevicePolicyManager {
        ...
        if (proxySpec == null) {
            ...
            try {
                ...
                if (proxySpec.equals(Proxy.NO_PROXY)) {
                    ...
                    hostBuilder.append(hostName);
                    hostBuilder.append(":");
                    hostBuilder.append(Integer.toString(port));
                    String hostSpec = hostBuilder.toString();
                ...
                return this.mService.setGlobalProxy(admin, sa, hostName);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }
        }
            ...
            try {
                ...
                for (String alias : certStore.userAliases()) {
                    try {
                        ...
                        Log.w(str, stringBuilder.toString(), ce);
                    }
                }
    ...
    public boolean setKeyPairCertificate(ComponentName admin, String alias, List<Certificate> certs, boolean isUserSelectable) {
        ...
        try {
            byte[] pemChain;
            byte[] pemCert = Credentials.convertToPem(new Certificate[]{(Certificate) certs.get(0)});
            if (certs.size() > 1) {
    ...
    public void clearApplicationUserData(ComponentName admin, String packageName, final Executor executor, final OnClearApplicationUserDataListener listener) {
        ...
        try {
            this.mService.clearApplicationUserData(admin, packageName, new Stub() {
                public void onRemoveCompleted(String pkg, boolean succeeded) {
                    executor.execute(new -$$Lambda$DevicePolicyManager$1$k6Rmp3Fg9FFATYRU5Z7rHDXGemA(listener, pkg, succeeded));
                }
            });
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();

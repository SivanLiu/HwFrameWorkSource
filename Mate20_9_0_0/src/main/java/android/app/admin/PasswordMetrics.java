...
public class PasswordMetrics implements Parcelable {
    ...
    public static PasswordMetrics computeForPassword(String password) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            switch (categoryChar(password.charAt(i2))) {
                case 0:
                    letters++;
                    lowerCase++;
                    break;
        ...
        if (hasNonNumeric && hasNumeric) {
            i = DevicePolicyManager.PASSWORD_QUALITY_ALPHANUMERIC;
        ...
        return new PasswordMetrics(i, length, letters, upperCase, lowerCase, numeric, symbols, nonLetter);
    }

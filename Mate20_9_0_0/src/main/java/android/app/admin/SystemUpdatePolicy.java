...
public final class SystemUpdatePolicy implements Parcelable {
    ...
    public void validateType() {
        if (this.mPolicyType != 1 && this.mPolicyType != 3) {
            ...
            } else if (this.mMaintenanceWindowStart < 0 || this.mMaintenanceWindowStart >= WINDOW_BOUNDARY || this.mMaintenanceWindowEnd < 0 || this.mMaintenanceWindowEnd >= WINDOW_BOUNDARY) {
                    ...
                    if (type != 3) {
                        if (type != 4) {
                            if (parser.getName().equals(KEY_FREEZE_TAG)) {
                                policy.mFreezePeriods.add(new FreezePeriod(MonthDay.parse(parser.getAttributeValue(null, KEY_FREEZE_START)), MonthDay.parse(parser.getAttributeValue(null, "end"))));
                            }
                        }
                    }
                }

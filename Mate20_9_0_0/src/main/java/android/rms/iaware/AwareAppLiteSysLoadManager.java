...
import android.rms.iaware.ISceneCallback.Stub;
...
public class AwareAppLiteSysLoadManager {
    ...
    private AtomicBoolean mIAwareLiteEnabled = new AtomicBoolean(false);
    ...
    private final class IAwareSceneCallback extends Stub {
        private IAwareSceneCallback() {
        }

        public void onSceneChanged(int scene, boolean start, int uid, int pid, String pkg) {
            String str = AwareAppLiteSysLoadManager.TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("onSceneChanged: secne =");
            stringBuilder.append(scene);
            stringBuilder.append(" start=");
            stringBuilder.append(start);
            AwareLog.d(str, stringBuilder.toString());
            if ((scene & 2) != 0) {
                AtomicBoolean access$400 = AwareAppLiteSysLoadManager.this.mIsGameScene;
                boolean z = start && AwareAppLiteSysLoadManager.this.mIAwareLiteEnabled.get();
                access$400.set(z);
    ...
    private void registerIAwareSceneCallback() {
    ...
    private int getScenes() {
        ...
        if (type == 9 || type == AppTypeInfo.PG_APP_TYPE_GAME) {
            return 2;
        }
        return -1;
    }

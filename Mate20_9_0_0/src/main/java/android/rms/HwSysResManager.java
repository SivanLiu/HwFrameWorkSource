...
public final class HwSysResManager {
    ...
    public boolean registerSceneCallback(IBinder callback, int scenes) {
        ...
        try {
            if (service.registerSceneCallback(callback, scenes)) {
                return true;
            }
        } catch (RemoteException e) {
            if (Log.HWINFO) {
    ...
    public boolean isScene(int scene) {
        ...
        if (service == null) {
            if (Log.HWINFO) {
                Log.i(TAG, "HwSysResManager_isGameScene service is null ");
            }
            return false;
        }
        try {
            if (service.isScene(scene)) {
                return true;
            }
        } catch (RemoteException e) {
            if (Log.HWINFO) {
                Log.i(TAG, "isGameScene catch RemoteException");
            }
        }
        return false;
    }
    ...
    public String fetchDFTDataByVersion(int iVer, int featureId, boolean beta, boolean clear, boolean betaEncode) {
        IHwSysResManager service = getService();
        if (service == null) {
            if (Log.HWINFO) {
        ...
        try {
            return service.fetchDFTDataByVersion(iVer, featureId, beta, clear, betaEncode);
        } catch (RemoteException e) {
            if (Log.HWINFO) {
    ...
    public void reportProximitySensorEventToIAware(boolean positive) {
        if (isResourceNeeded(ResourceType.getReousrceId(ResourceType.RESOURCE_SCENE_REC))) {
            Bundle bundleArgs = new Bundle();
            bundleArgs.putInt("positive", positive);
    ...
    public void reportGoogleConn(boolean conn) {
        if (isResourceNeeded(ResourceType.getReousrceId(ResourceType.RESOURCE_APPASSOC))) {
            Bundle bundleArgs = new Bundle();
            bundleArgs.putBoolean("gms_conn", conn);

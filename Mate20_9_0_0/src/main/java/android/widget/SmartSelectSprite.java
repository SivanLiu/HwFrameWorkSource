...
final class SmartSelectSprite {
    ...
    public void startAnimation(PointF start, List<RectangleWithTextSelectionLayout> destinationRectangles, Runnable onAnimationEnd) {
        ...
        for (RectangleWithTextSelectionLayout rectangleWithTextSelectionLayout2 : destinationRectangles) {
            RectF rectangle = rectangleWithTextSelectionLayout2.getRectangle();
            if (contains(rectangle, pointF)) {
                centerRectangle = rectangleWithTextSelectionLayout2;
                break;
            }
        ...
        if (centerRectangle2 != null) {
            ...
            for (startingOffset = 0; startingOffset < rectangleCount; startingOffset++) {
                rectangleWithTextSelectionLayout2 = (RectangleWithTextSelectionLayout) list.get(startingOffset);
                RoundedRectangleShape shape = new RoundedRectangleShape(rectangleWithTextSelectionLayout2.getRectangle(), expansionDirections[startingOffset], rectangleWithTextSelectionLayout2.getTextSelectionLayout() == 0, null);
    ...
    private Animator createAnimator(RectangleList rectangleList, float startingOffsetLeft, float startingOffsetRight, List<Animator> cornerAnimators, AnimatorUpdateListener updateListener, Runnable onAnimationEnd) {
        ObjectAnimator rightBoundaryAnimator = ObjectAnimator.ofFloat(rectangleList, "rightBoundary", new float[]{startingOffsetRight, (float) rectangleList.getTotalWidth()});
        ObjectAnimator leftBoundaryAnimator = ObjectAnimator.ofFloat(rectangleList, "leftBoundary", new float[]{startingOffsetLeft, 0.0f});
        ...
        new AnimatorSet().playTogether(cornerAnimators);
        new AnimatorSet().playTogether(new Animator[]{leftBoundaryAnimator, rightBoundaryAnimator});
        AnimatorSet animatorSet = new AnimatorSet();
        animatorSet.playSequentially(new Animator[]{boundaryAnimator, cornerAnimator});
    ...
    private ObjectAnimator createCornerAnimator(RoundedRectangleShape shape, AnimatorUpdateListener listener) {
        ObjectAnimator animator = ObjectAnimator.ofFloat(shape, "roundRatio", new float[]{shape.getRoundRatio(), 0.0f});

...
class FastScroller {
    ...
    private void layoutTrack() {
        ...
        if (this.mThumbPosition == 1) {
            ...
            top2 = container.top + top;
            top = container.bottom - top;
        }
        int trackWidth = track.getMeasuredWidth();
        int left = thumb.getLeft() + ((thumb.getWidth() - trackWidth) / 2);
        track.layout(left, top2, left + trackWidth, top);
    }
    ...
    private void updateOffsetAndRange() {
        ...
        if (this.mThumbPosition == 1) {
            halfThumbHeight = ((float) thumbImage.getHeight()) / 2.0f;
        ...
        this.mThumbRange = halfThumbHeight - min;
    }
    ...
    private void scrollTo(float position) {
        ...
        if (sections == null || sectionCount <= 1) {
            ...
            if (count == 0) {
                ...
                sections = 1040187392 / ((float) count);
            }
            if (prevSection != exactSection || position - prevPosition >= sections) {
                sectionCount = ((int) ((((float) (nextIndex - prevIndex)) * (position - prevPosition)) / (nextPosition - prevPosition))) + prevIndex;
            } else {
                sectionCount = prevIndex;
            }
            sectionCount = MathUtils.constrain(sectionCount, (int) 0.0f, count - 1);
            if (this.mList instanceof ExpandableListView) {
                ExpandableListView expList2 = this.mList;
                expList2.setSelectionFromTop(expList2.getFlatListPosition(ExpandableListView.getPackedPositionForGroup(this.mHeaderCount + sectionCount)), 0);
            } else {
                int i2 = targetSection;
                if (this.mList instanceof ListView) {
                    ((ListView) this.mList).setSelectionFromTop(this.mHeaderCount + sectionCount, 0);
                } else {
                    this.mList.setSelection(this.mHeaderCount + sectionCount);
                }
            }
        }
        index = sectionIndex;
        if (this.mCurrentSection != index) {
            this.mCurrentSection = index;
            boolean hasPreview = transitionPreviewLayout(index);
    ...
    private float getPosFromItemCount(int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        int firstVisibleItem2 = firstVisibleItem;
        ...
        if (hasSections && this.mMatchDragPosition) {
            firstVisibleItem2 -= this.mHeaderCount;
            ...
            totalItemCount2 -= this.mHeaderCount;
            ...
            if (section < sectionCount - 1) {
                if (section + 1 < sectionCount) {
                    positionsInSection = sectionIndexer.getPositionForSection(section + 1);
                } else {
                    positionsInSection = totalItemCount2 - 1;
                }
                positionsInSection -= sectionPos;
            } else {
            ...
            if (firstVisibleItem2 <= 0 || firstVisibleItem2 + i != totalItemCount2) {
                int i2 = totalItemCount2;
            } else {
                int maxSize;
                View lastChild = this.mList.getChildAt(i - 1);
                int bottomPadding = this.mList.getPaddingBottom();
                if (this.mList.getClipToPadding() != 0) {
                    firstVisibleItem2 = (this.mList.getHeight() - bottomPadding) - lastChild.getTop();
                    maxSize = lastChild.getHeight();
                } else {
                    firstVisibleItem2 = this.mList.getHeight() - lastChild.getTop();
                    maxSize = lastChild.getHeight() + bottomPadding;
                }
                if (firstVisibleItem2 <= 0 || maxSize <= 0) {
                } else {
                    int currentVisibleSize = firstVisibleItem2;
                    result += (1.0f - result) * (((float) firstVisibleItem2) / ((float) maxSize));
                }
            }
            return result;
        } else if (i == totalItemCount2) {
            return 0.0f;
        } else {
            return ((float) firstVisibleItem2) / ((float) (totalItemCount2 - i));
        }
    }

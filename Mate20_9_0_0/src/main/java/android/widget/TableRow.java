    ...
    private void mapIndexAndColumns() {
        if (this.mColumnToChildIndex == null) {
            ...
            while (i < count) {
                ...
                virtualCount = 0;
                while (virtualCount < layoutParams.span) {
                    int virtualCount3 = virtualCount2 + 1;
                    columnToChild.put(virtualCount2, i);
                    virtualCount++;
                    virtualCount2 = virtualCount3;
                }
                i++;
    ...
    void measureChildBeforeLayout(View child, int childIndex, int widthMeasureSpec, int totalWidth, int heightMeasureSpec, int totalHeight) {
        if (this.mConstrainedColumnWidths != null) {
            ...
            if (isHorizontalGravity) {
                ...
                measureMode = Gravity.getAbsoluteGravity(i, getLayoutDirection()) & 7;
                if (measureMode == 1) {
                    lp.mOffset[0] = lp.mOffset[1] / 2;
                    return;
                } else if (measureMode != 3 && measureMode == 5) {

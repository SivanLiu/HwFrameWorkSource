...
class CalendarViewLegacyDelegate extends AbstractCalendarViewDelegate {
    ...
    private class WeekView extends View {
        ...
        private void drawWeekNumbersAndDates(Canvas canvas) {
            ...
            if (isLayoutRtl()) {
                while (i < nDays - 1) {
                    ...
                    canvas.drawText(this.mDayNumbers[(nDays - 1) - i], (float) ((((2 * i) + 1) * this.mWidth) / divisor), (float) y, this.mMonthNumDrawPaint);
                    i++;
                }
                if (CalendarViewLegacyDelegate.this.mShowWeekNumber) {
            ...
            while (i < nDays) {
                ...
                canvas.drawText(this.mDayNumbers[i], (float) ((((2 * i) + 1) * this.mWidth) / divisor), (float) y, this.mMonthNumDrawPaint);
                i++;
            }
        }
        ...
        private void drawWeekSeparators(Canvas canvas) {
            ...
            if (firstFullyVisiblePosition != this.mWeek) {
                ...
                if (isLayoutRtl()) {
                    startX = 0.0f;
                    f = (float) (CalendarViewLegacyDelegate.this.mShowWeekNumber ? this.mWidth - (this.mWidth / this.mNumCells) : this.mWidth);
                } else {
                    startX = CalendarViewLegacyDelegate.this.mShowWeekNumber ? (float) (this.mWidth / this.mNumCells) : 0.0f;
                    f = (float) this.mWidth;
                }
                canvas.drawLine(startX, 0.0f, f, 0.0f, this.mDrawPaint);
            }
        }
    ...
    CalendarViewLegacyDelegate(CalendarView delegator, Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        View content = ((LayoutInflater) this.mContext.getSystemService("layout_inflater")).inflate(17367106, null, false);
        this.mDelegator.addView(content);
        this.mListView = (ListView) this.mDelegator.findViewById(16908298);
        this.mDayNamesHeader = (ViewGroup) content.findViewById(16908846);
        this.mMonthName = (TextView) content.findViewById(16909100);
        setUpHeader();
        setUpListView();
        setUpAdapter();
    ...
    private void setUpHeader() {
        ...
        for (calendarDay = 1; calendarDay < count2; calendarDay++) {
            label = (TextView) this.mDayNamesHeader.getChildAt(calendarDay);
            if (this.mWeekDayTextAppearanceResId > -1) {
                label.setTextAppearance(this.mWeekDayTextAppearanceResId);
            }
            if (calendarDay < this.mDaysPerWeek + 1) {
                label.setText(this.mDayNamesShort[calendarDay - 1]);
                label.setContentDescription(this.mDayNamesLong[calendarDay - 1]);
                label.setVisibility(0);
            } else {
                label.setVisibility(8);
            }
    ...
    private void goTo(Calendar date, boolean animate, boolean setSelected, boolean forceScroll) {
        ...
        if (position < firstFullyVisiblePosition || position > lastFullyVisiblePosition || forceScroll) {
            ...
            if (this.mFirstDayOfMonth.before(this.mMinDate)) {
                ...
                position2 = getWeeksSinceMinDate(this.mFirstDayOfMonth);
            }
            position = position2;

...
public class ScrollView extends FrameLayout {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            case 0:
                if (getChildCount() != 0) {
                    isFinished = this.mScroller.isFinished() ^ 1;
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    if (Math.abs(initialVelocity) > this.mMinimumVelocity) {
                        ...
                        i = this.mScrollX;
                        if (overScroller.springBack(i, this.mScrollY, 0, 0, 0, getScrollRange())) {
                            postInvalidateOnAnimation();
                        }
                    }
                    this.mActivePointerId = -1;
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    isFinished = this.mLastMotionY - y;
                    if (dispatchNestedPreScroll(0, isFinished, this.mScrollConsumed, this.mScrollOffset)) {
                        isFinished -= this.mScrollConsumed[1];
                        vtev.offsetLocation(0.0f, (float) this.mScrollOffset[1]);
                        this.mNestedYOffset += this.mScrollOffset[1];
                    }
                    if (!this.mIsBeingDragged && Math.abs(isFinished) > this.mTouchSlop) {
                        ...
                        this.mIsBeingDragged = true;
                        if (isFinished > 0) {
                            isFinished -= this.mTouchSlop;
                        } else {
                            isFinished += this.mTouchSlop;
                        }
                    }
                    int deltaY = isFinished;
                    if (this.mIsBeingDragged) {
                        ...
                        initialVelocity = this.mScrollY;
                        int range2 = range;
                        int oldY2 = oldY;
                        int deltaY2 = deltaY;
                        int activePointerIndex2 = activePointerIndex;
                        if (overScrollBy(0, deltaY, 0, initialVelocity, 0, range2, 0, this.mOverscrollDistance, 1) && !hasNestedScrollingParent()) {
                            this.mVelocityTracker.clear();
                        }
                        oldY = this.mScrollY - oldY2;
                        int activePointerIndex3;
                        if (!dispatchNestedScroll(0, oldY, 0, deltaY2 - oldY, this.mScrollOffset)) {
                            if (!canOverscroll) {
                                break;
                            }
                            pulledToY = oldY2 + deltaY2;
                            if (pulledToY < 0) {
                                this.mEdgeGlowTop.onPull(((float) deltaY2) / ((float) getHeight()), motionEvent.getX(activePointerIndex2) / ((float) getWidth()));
                                ...
                                if (pulledToY > range2) {
                                    this.mEdgeGlowBottom.onPull(((float) deltaY2) / ((float) getHeight()), 1.0f - (motionEvent.getX(activePointerIndex3) / ((float) getWidth())));
                ...
                stringBuilder2.append("Invalid pointerId=");
                stringBuilder2.append(this.mActivePointerId);
                stringBuilder2.append(" in onTouchEvent");
            ...
            case 5:
                pulledToY = ev.getActionIndex();
                if (pulledToY != -1) {
                    this.mLastMotionY = (int) motionEvent.getY(pulledToY);
                    this.mActivePointerId = motionEvent.getPointerId(pulledToY);
                    break;
                }
                str = TAG;
                stringBuilder = new StringBuilder();
                stringBuilder.append("Invalid index=");
                stringBuilder.append(pulledToY);
                stringBuilder.append(" in onTouchEvent");
                Log.e(str, stringBuilder.toString());
                break;
            case 6:
                onSecondaryPointerUp(ev);
                pulledToY = motionEvent.findPointerIndex(this.mActivePointerId);
                if (pulledToY != -1) {
                    this.mLastMotionY = (int) motionEvent.getY(pulledToY);
                    break;
                }
    ...
    public void computeScroll() {
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (!(oldX == x && oldY == y)) {
                ...
                boolean canOverscroll = z;
                overScrollBy(x - oldX, y - oldY, oldX, oldY, 0, range, 0, this.mOverflingDistance, 0);
                onScrollChanged(this.mScrollX, this.mScrollY, oldX, oldY);
                if (canOverscroll) {
                    if (y < 0 && oldY >= 0) {

    ...
    private PowerMode(Context context, boolean deviceOwner) {
        ...
        if (context == null) {
            ...
            if (this.mIsDeviceOwner) {
                try {
                    Log.i(TAG, String.format("retvalu:%d", new Object[]{Integer.valueOf(System.getInt(this.mContext.getContentResolver(), "SmartModeStatus", 1))}));
                    this.mIsNormalPowerSaveMode = System.getInt(this.mContext.getContentResolver(), "SmartModeStatus", 1) == 4;
                } catch (Exception e) {

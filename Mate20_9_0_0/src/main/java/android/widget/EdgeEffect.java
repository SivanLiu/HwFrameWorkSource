...
public class EdgeEffect {
    ...
    public void setSize(int width, int height) {
        float r = (((float) width) * RADIUS_FACTOR) / SIN;
        float h = r - (COS * r);
        float or = (((float) height) * RADIUS_FACTOR) / SIN;
        float oh = or - (COS * or);
        this.mRadius = r;
        float f = 1.0f;
        if (h > 0.0f) {
    ...
    public void onPull(float deltaDistance, float displacement) {
        ...
        if (this.mState != 4 || ((float) (now - this.mStartTime)) >= this.mDuration) {
            ...
            if (this.mPullDistance == 0.0f) {
                ...
                min = (float) (Math.max(0.0d, (1.0d - (1.0d / Math.sqrt((double) (Math.abs(this.mPullDistance) * ((float) this.mBounds.height()))))) - 0.3d) / 0.7d);
    ...
    public boolean draw(Canvas canvas) {
        ...
        float translateX = (((float) this.mBounds.width()) * (Math.max(0.0f, Math.min(this.mDisplacement, 1.0f)) - 0.5f)) / MAX_GLOW_SCALE;

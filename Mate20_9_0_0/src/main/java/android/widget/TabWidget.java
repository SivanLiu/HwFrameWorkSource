    ...
    void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (extraWidth > 0) {
            ...
            if (childCount > 0) {
                ...
                i = extraWidth;
                for (extraWidth = 0; extraWidth < count; extraWidth++) {
                    View child = getChildAt(extraWidth);
                    if (child.getVisibility() != 8) {
                        int childWidth = child.getMeasuredWidth();
                        int newWidth = Math.max(0, childWidth - (i / childCount));
                        this.mImposedTabWidths[extraWidth] = newWidth;
                        i -= childWidth - newWidth;
                        childCount--;

...
public class StackView extends AdapterViewAnimator {
    ...
    private class StackSlider {
        ...
        private float getDuration(boolean invert, float velocity) {
            ...
            float maxd = (float) Math.hypot((double) StackView.this.mSlideAmount, (double) (0.4f * ((float) StackView.this.mSlideAmount)));
            if (d > maxd) {
                d = maxd;
            }
            if (velocity == 0.0f) {
    ...
    void transformViewForTransition(int fromIndex, int toIndex, final View view, boolean animate) {
        ...
        if (fromIndex == -1 && toIndex == getNumActiveViews() - 1) {
            ...
            if (animate) {
                ...
                fArr = new float[]{0.0f};
                slideIn = ObjectAnimator.ofPropertyValuesHolder(animationSlider, new PropertyValuesHolder[]{PropertyValuesHolder.ofFloat("XProgress", fArr), slideInY});
            ...
            if (animate) {
                ...
                fArr = new float[]{0.0f};
                slideIn = ObjectAnimator.ofPropertyValuesHolder(animationSlider2, new PropertyValuesHolder[]{PropertyValuesHolder.ofFloat("XProgress", fArr), slideOutY});
    ...
    private void transformViewAtIndex(int index, View view, boolean animate) {
        ...
        if (animate) {
            ...
            r = PropertyValuesHolder.ofFloat("translationY", new float[]{transY});
            maxPerspectiveShiftY = PropertyValuesHolder.ofFloat("scaleX", new float[]{scale});
            maxPerspectiveShiftX = PropertyValuesHolder.ofFloat("scaleY", new float[]{scale});
            ObjectAnimator oa = ObjectAnimator.ofPropertyValuesHolder(view2, new PropertyValuesHolder[]{maxPerspectiveShiftY, maxPerspectiveShiftX, r, translationX});
            PropertyValuesHolder translationY = r;
            oa.setDuration(100);
            if (view2 instanceof StackFrame) {
        ...
        float f = maxPerspectiveShiftY;
        float f2 = maxPerspectiveShiftX;
    ...
    private void onLayout() {
        ...
        int newSlideAmount = Math.round(1060320051 * ((float) getMeasuredHeight()));
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        ...
        if (ev.getPointerId(activePointerIndex) == this.mActivePointerId) {
            ...
            if (v != null) {
                while (index < ev.getPointerCount()) {
    ...
    private void handlePointerUp(MotionEvent ev) {
        ...
        if (deltaY > this.mSwipeThreshold && this.mSwipeGestureType == 2 && this.mStackSlider.mMode == 0) {
            ...
            if (this.mSwipeGestureType == 1) {
                ...
                fArr = new float[]{0.0f};
                pa = ObjectAnimator.ofPropertyValuesHolder(animationSlider, new PropertyValuesHolder[]{PropertyValuesHolder.ofFloat("XProgress", fArr), snapBackY});
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightSpecMode == 0) {
            if (haveChildRefSize) {
                round = (Math.round(((float) this.mReferenceChildHeight) * (1.0f + 1.1111112f)) + this.mPaddingTop) + this.mPaddingBottom;
            } else {
                round = 0;
            }

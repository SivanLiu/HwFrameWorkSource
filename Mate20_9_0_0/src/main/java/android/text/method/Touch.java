...
public class Touch {
    ...
    public static void scrollTo(TextView widget, Layout layout, int x, int y) {
        ...
        if (actualWidth >= availableWidth) {
            x = Math.max(Math.min(x, right2 - availableWidth), left);
    ...
    public static boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case 2:
                ...
                if (ds.length > 0) {
                    if (!ds[0].mFarEnough) {
                        i = ViewConfiguration.get(widget.getContext()).getScaledTouchSlop();
                        if (Math.abs(event.getX() - ds[0].mX) >= ((float) i) || Math.abs(event.getY() - ds[0].mY) >= ((float) i)) {
                    ...
                    if (ds[0].mFarEnough) {
                        ...
                        float dy;
                        ds[0].mUsed = true;
                        boolean cap = ((event.getMetaState() & 1) == 0 && MetaKeyKeyListener.getMetaState((CharSequence) buffer, 1) != 1 && MetaKeyKeyListener.getMetaState((CharSequence) buffer, 2048) == 0) ? false : true;
                        if (cap) {
                        ...
                        ny = widget.getScrollX();
                        int oldY = widget.getScrollY();
                        scrollTo(widget, layout, nx, ny2);
                        if (!(ny == widget.getScrollX() && oldY == widget.getScrollY())) {

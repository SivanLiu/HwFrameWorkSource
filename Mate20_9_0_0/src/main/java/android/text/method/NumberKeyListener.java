...
public abstract class NumberKeyListener extends BaseKeyListener implements InputFilter {
    ...
    public boolean onKeyDown(View view, Editable content, int keyCode, KeyEvent event) {
        int selEnd = Selection.getSelectionStart(content);
        int b = Selection.getSelectionEnd(content);
        int selStart = Math.min(selEnd, b);
        selEnd = Math.max(selEnd, b);
        b = 0;
        ...
        } else if (i == 48 && b == 1 && selStart == selEnd && selEnd > 0 && content.charAt(selStart - 1) == '0') {
    ...
    static boolean addFormatCharsFromSkeletons(Collection<Character> collection, Locale locale, String[] skeletons, String symbolsToIgnore) {
        for (boolean success : skeletons) {
            if (!addFormatCharsFromSkeleton(collection, locale, success, symbolsToIgnore)) {
                return false;
            }
        }
        return true;

...
public abstract class MetaKeyKeyListener {
    ...
    public static final int getMetaState(long state, int meta) {
        ...
        } else if ((Trace.TRACE_TAG_CAMERA & state) != 0) {
            return 2;
        } else {
    ...
    public static long handleKeyDown(long state, int keyCode, KeyEvent event) {
        ...
        if (i == 63) {
            return press(state, 4, META_SYM_MASK, Trace.TRACE_TAG_CAMERA, META_SYM_PRESSED, META_SYM_RELEASED, 17179869184L);
        }
        return state;
    }
    ...
    public long clearMetaKeyState(long state, int which) {
        ...
        if ((which & 4) == 0 || (Trace.TRACE_TAG_CAMERA & state) == 0) {
            return state;
        }
        return state & -1130315133223941L;
    }

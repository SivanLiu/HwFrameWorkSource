...
public class DateUtils {
    ...
    private static void initFormatStringsLocked() {
        ...
        if (sLastConfig == null || !sLastConfig.equals(cfg)) {
            sLastConfig = cfg;
            sElapsedFormatMMSS = r.getString(17039978);
            sElapsedFormatHMMSS = r.getString(17039977);
        }
    }
    ...
    public static CharSequence formatDuration(long millis, int abbrev) {
        ...
        if (millis >= MINUTE_IN_MILLIS) {
            return formatter.format(new Measure(Integer.valueOf((int) ((30000 + millis) / MINUTE_IN_MILLIS)), MeasureUnit.MINUTE));
        }
    ...
    public static CharSequence getRelativeTimeSpanString(Context c, long millis, boolean withPreposition) {
        ...
        synchronized (DateUtils.class) {
            ...
            if (span < DAY_IN_MILLIS && sNowTime.weekDay == sThenTime.weekDay) {
                result = formatDateRange(c, j, j, 1);
                prepositionId = 17040950;
            } else if (sNowTime.year != sThenTime.year) {
                result = formatDateRange(c, j, j, 131092);
                prepositionId = 17040949;
            } else {
                result = formatDateRange(c, j, j, 65552);
                prepositionId = 17040949;
            }
            if (withPreposition) {

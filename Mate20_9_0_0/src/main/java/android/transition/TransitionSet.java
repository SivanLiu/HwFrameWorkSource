...
public class TransitionSet extends Transition {
    ...
    protected void runAnimators() {
        ...
        if (this.mPlayTogether) {
            while (true) {
                i2 = i;
                if (i2 >= numTransitions) {
                    break;
                }
                ((Transition) this.mTransitions.get(i2)).runAnimators();
                i = i2 + 1;
            }
        } else {
            for (i2 = 1; i2 < numTransitions; i2++) {
                final Transition nextTransition = (Transition) this.mTransitions.get(i2);

...
public class Allocation extends BaseObj {
    ...
    public void setFromFieldPacker(int xoff, int yoff, int zoff, int component_number, FieldPacker fp) {
        ...
        if (i >= this.mType.mElement.mElements.length) {
            ...
            stringBuilder2.append(data_length2);
            stringBuilder2.append(" does not match component size ");
            stringBuilder2.append(eSize2);
        ...
        try {
            ...
            if (b.getConfig() != null) {
                ...
                if (mipmapControl == MipmapControl.MIPMAP_NONE && t.getElement().isCompatible(Element.RGBA_8888(rs)) && i == 131) {
                    ...
                    id2 = renderScript.nAllocationCreateBitmapBackedAllocation(id, mipmapControl.mID, bitmap, i);
                    if (id2 != 0) {
                        alloc = new Allocation(id2, renderScript, t, true, i, mipmapControl);
                ...
                id = t.getID(renderScript);
                id2 = renderScript.nAllocationCreateFromBitmap(id, mipmapControl.mID, bitmap, i);
                if (id2 != 0) {
                    alloc = new Allocation(id2, renderScript, t, true, i, mipmapControl);
                    Trace.traceEnd(Trace.TRACE_TAG_RS);
                    return alloc;
    ...
    static Allocation createFromAllocation(RenderScript rs, Allocation alloc) {
        ...
        try {
            ...
            if (allocation.getID(renderScript) != 0) {
                ...
                if (id != 0) {
                    Allocation outAlloc = new Allocation(id, renderScript, type, false, usage, mips);
                    if ((usage & 32) != 0) {
    ...
    public static Allocation createCubemapFromBitmap(RenderScript rs, Bitmap b, MipmapControl mips, int usage) {
        ...
        if (width % 6 != 0) {
            ...
            if (((height + -1) & height) == 0) {
                ...
                if (id != 0) {
                    return new Allocation(id, renderScript, t, true, usage, mipmapControl);
                }
                StringBuilder stringBuilder = new StringBuilder();

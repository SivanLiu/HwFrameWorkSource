...
public final class CalendarContract {
    ...
    public static final class CalendarAlerts implements BaseColumns, CalendarAlertsColumns, EventsColumns, CalendarColumns {
        ...
        public static void scheduleAlarm(Context context, AlarmManager manager, long alarmTime) {
            if (manager == null) {
                manager = (AlarmManager) context.getSystemService(ZenModeConfig.IS_ALARM_PATH);
            }
    ...
    public static final class EventsEntity implements BaseColumns, SyncColumns, EventsColumns {
        ...
        private static class EntityIteratorImpl extends CursorEntityIterator {
            ...
            public Entity getEntityAndIncrementCursor(Cursor cursor) throws RemoteException {
                ...
                DatabaseUtils.cursorIntToContentValuesIfPresent(cursor2, cv, EventsColumns.AVAILABILITY);
                ...
                if (this.mResolver != null) {
                    i = this.mResolver;
                    subCursor = i.query(Attendees.CONTENT_URI, ATTENDEES_PROJECTION, WHERE_EVENT_ID, new String[]{Long.toString(eventId)}, null);
                } else {
                    i = this.mProvider;
                    subCursor = i.query(Attendees.CONTENT_URI, ATTENDEES_PROJECTION, WHERE_EVENT_ID, new String[]{Long.toString(eventId)}, null);
                }
                ...
                while (true) {
                    try {
                        ...
                        reminderValues = new ContentValues();
                        reminderValues.put(AttendeesColumns.ATTENDEE_NAME, subCursor2.getString(0));
                        reminderValues.put(AttendeesColumns.ATTENDEE_EMAIL, subCursor2.getString(1));
                        reminderValues.put(AttendeesColumns.ATTENDEE_RELATIONSHIP, Integer.valueOf(subCursor2.getInt(2)));
                        reminderValues.put(AttendeesColumns.ATTENDEE_TYPE, Integer.valueOf(subCursor2.getInt(3)));
                        reminderValues.put(AttendeesColumns.ATTENDEE_STATUS, Integer.valueOf(subCursor2.getInt(4)));
                        reminderValues.put(AttendeesColumns.ATTENDEE_IDENTITY, subCursor2.getString(5));
                        reminderValues.put(AttendeesColumns.ATTENDEE_ID_NAMESPACE, subCursor2.getString(6));
                        i = Attendees.CONTENT_URI;
                        entity.addSubValue(i, reminderValues);
                    } finally {
                        subCursor2.close();
                    }
                }
                ...
                while (subCursor2.moveToNext()) {
                    try {
                        reminderValues = new ContentValues();
                        reminderValues.put("_id", subCursor2.getString(0));
                        reminderValues.put("name", subCursor2.getString(1));
                        reminderValues.put("value", subCursor2.getString(i));
                        entity.addSubValue(ExtendedProperties.CONTENT_URI, reminderValues);
                    } finally {
                        subCursor2.close();
                    }
                }

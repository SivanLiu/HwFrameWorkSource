...
public final class ContactsContract {
        ...
        public static Intent rebuildManagedQuickContactsIntent(String lookupKey, long contactId, boolean isContactIdIgnored, long directoryId, Intent originalIntent) {
            ...
            if (!TextUtils.isEmpty(lookupKey)) {
                ...
                if (isContactIdIgnored) {
                    withAppendedPath = Uri.withAppendedPath(Contacts.CONTENT_LOOKUP_URI, lookupKey);
                } else {
                    withAppendedPath = Contacts.getLookupUri(contactId, lookupKey);
                }
                uri = withAppendedPath;
            }
    ...
    public static class Contacts implements BaseColumns, ContactsColumns, ContactOptionsColumns, ContactNameColumns, ContactStatusColumns, ContactCounts {
        ...
        public static final Uri CONTENT_GROUP_URI = Uri.withAppendedPath(CONTENT_URI, GroupCipher.varName);
            ...
            public static final class Builder {
                ...
                public Uri build() {
                    ...
                    for (int i = 0; i < count; i++) {
                        String str = SuggestionColumns.QUERY;
                        StringBuilder stringBuilder = new StringBuilder();
                        stringBuilder.append("name:");
                        stringBuilder.append((String) this.mValues.get(i));
                        builder.appendQueryParameter(str, stringBuilder.toString());
                    }
        ...
        public static Uri getLookupUri(ContentResolver resolver, Uri contactUri) {
            Uri uri = new String[]{ContactsColumns.LOOKUP_KEY, "_id"};
            Cursor c = resolver.query(contactUri, uri, null, null, null);
            if (c == null) {
                return null;
            }
            try {
                if (c.moveToFirst()) {
                    uri = getLookupUri(c.getLong(1), c.getString(null));
                    return uri;
                }
                c.close();
                return null;
        ...
        public static InputStream openContactPhotoInputStream(ContentResolver cr, Uri contactUri, boolean preferHighres) {
            ...
            InputStream inputStream = new String[]{"data15"};
            Cursor cursor = cr.query(photoUri, inputStream, null, null, null);
            if (cursor != null) {
                try {
                    if (cursor.moveToNext()) {
                        byte[] data = cursor.getBlob(null);
                        if (data == null) {
                            if (cursor != null) {
                                cursor.close();
                            }

...
public class SyncStateContract {
    ...
    public static final class Helpers {
        ...
        public static Pair<Uri, byte[]> getWithUri(ContentProviderClient provider, Uri uri, Account account) throws RemoteException {
            ...
            Cursor c = provider.query(uri, DATA_PROJECTION, SELECT_BY_ACCOUNT, pair, null);
            if (c != null) {
                try {
                    if (c.moveToNext()) {
                        long rowId = c.getLong(1);
                        pair = Pair.create(ContentUris.withAppendedId(uri, rowId), c.getBlob(c.getColumnIndexOrThrow("data")));
                        return pair;
                    }
                    c.close();
                    return null;

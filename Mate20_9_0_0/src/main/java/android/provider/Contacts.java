...
public class Contacts {
    ...
    public static final class ContactMethods implements BaseColumns, ContactMethodsColumns, PeopleColumns {
        ...
        public static String encodeCustomImProtocol(String protocolString) {
            ...
            stringBuilder.append(protocolString);
            return stringBuilder.toString();
        }
        ...
        public static final CharSequence getDisplayLabel(Context context, int kind, int type, CharSequence label) {
            ...
            switch (kind) {
                case 1:
                    if (type != 0) {
                        labels = context.getResources().getTextArray(17235968);
                        try {
                ...
                case 2:
                    if (type != 0) {
                        labels = context.getResources().getTextArray(17235972);
                        try {
                            return labels[type - 1];
    ...
    public static final class People implements BaseColumns, PeopleColumns, PhonesColumns, PresenceColumns {
        ...
        private static final String[] GROUPS_PROJECTION = new String[]{"_id"};
        @Deprecated
        ...
        public static Uri addToGroup(ContentResolver resolver, long personId, String groupName) {
            long groupId = 0;
            ContentResolver groupsCursor = resolver;
            Cursor groupsCursor2 = groupsCursor.query(Groups.CONTENT_URI, GROUPS_PROJECTION, "name=?", new String[]{groupName}, null);
            if (groupsCursor2 != null) {
                try {
                    if (groupsCursor2.moveToFirst()) {
                        groupId = groupsCursor2.getLong(0);
                    }
                    groupsCursor2.close();
                } catch (Throwable th) {
                    groupsCursor2.close();
    ...
    public static final class Settings implements BaseColumns, SettingsColumns {
        ...
        public static String getSetting(ContentResolver cr, String account, String key) {
            ...
            ContentResolver cursor = cr;
            Cursor cursor2 = cursor.query(CONTENT_URI, strArr, "key=?", new String[]{key}, null);
            try {

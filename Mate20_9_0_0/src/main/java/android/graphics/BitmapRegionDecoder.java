...
public final class BitmapRegionDecoder {
    ...
    public static BitmapRegionDecoder newInstance(String pathName, boolean isShareable) throws IOException {
        ...
        try {
            ...
            BitmapRegionDecoder decoder = HwFrameworkFactory.getHwDrmManager().newDrmBitmapRegionDecoderInstance((FileInputStream) stream, pathName, isShareable);
            if (decoder != null) {
                try {
                    stream.close();
                } catch (IOException e) {
    ...
    public static BitmapRegionDecoder hwNewInstance(FileDescriptor fd, boolean isShareable) {
        return nativeNewInstance(fd, isShareable);
    }
}

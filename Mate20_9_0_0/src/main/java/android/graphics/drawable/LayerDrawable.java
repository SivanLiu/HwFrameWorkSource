        ...
        while (true) {
            ...
            if (i < count2) {
                ...
                if (d == null) {
                    ...
                    isLayoutRtl2 = isLayoutRtl;
                    int insetL = insetRtlL ? r.mInsetL : insetRtlL;
                    int insetR = insetRtlR == Integer.MIN_VALUE ? r.mInsetR : insetRtlR;
                    Rect container = this.mTmpContainer;
                    i2 = paddingT;
                    container.set((rect.left + insetL) + paddingL, (rect.top + insetT) + paddingT, (rect.right - insetR) - paddingR, (rect.bottom - count) - paddingB);
                    paddingT = d.getIntrinsicWidth();
                    isLayoutRtl = d.getIntrinsicHeight();
                    insetRtlR = r.mWidth;
                    boolean layerH = r.mHeight;
                    int gravity = resolveGravity(r.mGravity, insetRtlR, layerH, paddingT, isLayoutRtl);
                    count = insetRtlR < 0 ? paddingT : insetRtlR;
                    Drawable d2 = d;
                    Gravity.apply(gravity, count, layerH >= false ? isLayoutRtl : layerH, container, outRect, layoutDirection);
                    d2.setBounds(outRect);
                    if (isPaddingNested) {
                        paddingL += this.mPaddingL[i];
                        paddingR += this.mPaddingR[i];
                        paddingB += this.mPaddingB[i];
                        paddingT = i2 + this.mPaddingT[i];
                        i++;
                        count = count3;
                        array = array2;

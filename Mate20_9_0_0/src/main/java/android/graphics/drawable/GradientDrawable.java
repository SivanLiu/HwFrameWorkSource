...
public class GradientDrawable extends Drawable {
    ...
    static final class GradientState extends ConstantState {
        ...
        Mode mTintMode = Drawable.DEFAULT_TINT_MODE;
    ...
    public void draw(Canvas canvas) {
        ...
        if (ensureValidRect()) {
            ...
            boolean z = true;
            boolean z2 = currStrokeAlpha > 0 && this.mStrokePaint != null && this.mStrokePaint.getStrokeWidth() > 0.0f;
            boolean haveStroke = z2;
            ...
            if (!haveStroke || !haveFill || st.mShape == 2 || currStrokeAlpha >= 255 || (this.mAlpha >= 255 && colorFilter == null)) {
                z = false;
            }
            boolean useLayer = z;
            ...
            if (useLayer) {
                ...
                if (colorFilter2 != null && st.mSolidColors == null) {
            ...
            switch (st.mShape) {
                ...
                case 2:
                    ...
                    rad = r.centerY();
                    if (haveStroke) {
                        canvas2.drawLine(r.left, rad, r.right, rad, this.mStrokePaint);
                        break;
                    }
                    break;
                case 3:
        ...
        if (this.mGradientIsDirty) {
            ...
            if (gradientColors != null) {
                ...
                if (st.mGradient == 0) {
                    ...
                    Shader linearGradient = new LinearGradient(x0, f, x1, y12, gradientColors, st.mPositions, TileMode.CLAMP);
                    ...
                    if (st.mUseLevel) {
                        ...
                        while (true) {
                            ...
                            tempPositions[i2] = (((float) i2) * fraction) * level;
                            i = i2 + 1;
                        }
    ...
    private void updateGradientDrawableGradient(Resources r, TypedArray a) throws XmlPullParserException {
        ...
        if (st.mGradient == 0) {
            radiusType = ((int) typedArray.getFloat(3, (float) st.mAngle)) % 360;
            if (radiusType % 45 == 0) {
                st.mAngle = radiusType;
                if (radiusType == 0) {
                    st.mOrientation = Orientation.LEFT_RIGHT;
                    return;
                } else if (radiusType == 45) {
                    st.mOrientation = Orientation.BL_TR;
                    return;
                } else if (radiusType == 90) {
                    st.mOrientation = Orientation.BOTTOM_TOP;
                    return;
                } else if (radiusType == 135) {
                    st.mOrientation = Orientation.BR_TL;
                    return;
                } else if (radiusType == 180) {
                    st.mOrientation = Orientation.RIGHT_LEFT;
                    return;
                } else if (radiusType == 225) {
                    st.mOrientation = Orientation.TR_BL;
                    return;
                } else if (radiusType == 270) {
                    st.mOrientation = Orientation.TOP_BOTTOM;
                    return;
                } else if (radiusType == R.styleable.Theme_preferenceFrameLayoutStyle) {
            ...
            stringBuilder.append("<gradient> tag requires 'angle' attribute to be a multiple of 45");
        ...
        if (tv != null) {
            ...
            stringBuilder2.append("<gradient> tag requires 'gradientRadius' attribute with radial type");
    ...
    public void getOutline(Outline outline) {
        ...
        switch (st.mShape) {
            case 0:
                ...
                modulateAlpha = 0.0f;
                if (st.mRadius > 0.0f) {
                    modulateAlpha = Math.min(st.mRadius, ((float) Math.min(bounds.width(), bounds.height())) * 0.5f);
                }
                outline.setRoundRect(bounds, modulateAlpha);
                return;
            case 1:
            ...
            case 2:
                ...
                modulateAlpha = (float) bounds.centerY();
                outline.setRect(bounds.left, (int) Math.floor((double) (modulateAlpha - halfStrokeWidth)), bounds.right, (int) Math.ceil((double) (modulateAlpha + halfStrokeWidth)));
                return;
            default:
                return;
        }

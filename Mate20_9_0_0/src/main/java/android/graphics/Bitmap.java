...
public final class Bitmap implements Parcelable {
    ...
    Bitmap(long nativeBitmap, int width, int height, int density, boolean isMutable, boolean requestPremultiplied, byte[] ninePatchChunk, InsetStruct ninePatchInsets) {
        ...
        if (j != 0) {
            ...
            new NativeAllocationRegistry(Bitmap.class.getClassLoader(), nativeGetNativeFinalizer(), nativeSize).registerNativeAllocation(this, j);
        ...
        InsetStruct insetStruct = ninePatchInsets;
    ...
    public static Bitmap createBitmap(Bitmap source, int x, int y, int width, int height, Matrix m, boolean filter) {
        ...
        if (i + i3 > source.getWidth()) {
            ...
            if (matrix == null || m.isIdentity()) {
                bitmap = null;
                bitmap2 = createBitmap(neww, newh, newConfig, source2.hasAlpha());
                paint = null;
            } else {
                ...
                if (neww <= 0 || newh <= 0) {
                    ...
                    stringBuilder.append("width and height must be > 0, width=");
                ...
                boolean bitmap3 = (transformed || source2.hasAlpha()) ? true : null;
                bitmap = createBitmap(neww, newh, transformedConfig, bitmap3);
                if (bitmap == null) {
            ...
            nativeCopyColorSpace(source2.mNativePtr, bitmap2.mNativePtr);
            bitmap2.mDensity = source2.mDensity;
            bitmap2.setHasAlpha(source2.hasAlpha());
            bitmap2.setPremultiplied(source2.mRequestPremultiplied);
            ...
            if (HwGalleryCacheManager.isGalleryCacheEffect() && source2.mGalleryCached && source2.mCacheInfo != null) {
                if (bitmap2 != source2) {
                    bitmap2.mGalleryCached = source2.mGalleryCached;
                    bitmap2.mCacheInfo = source2.mCacheInfo;
                    bitmap2.mCacheInfo.addRef();
                }
                if (!(matrix == null || m.isIdentity())) {
                    if (bitmap2.mCacheInfo.mMatrix == null) {
                        bitmap2.mCacheInfo.mMatrix = new Matrix(matrix);
                    } else {
                        bitmap2.mCacheInfo.mMatrix.postConcat(matrix);
                    }
                    bitmap2.mCacheInfo.mFilter = z;
                }
            }
            if (isHardware) {
                return bitmap2.copy(Config.HARDWARE, false);
            }
            return bitmap2;
        }
    }
    ...
    public final ColorSpace getColorSpace() {
        ...
        if (this.mColorSpace == null) {
            if (nativeIsSRGB(this.mNativePtr)) {
                ...
                this.mColorSpace = ColorSpace.get(Named.LINEAR_EXTENDED_SRGB);
            } else {
                ...
                if (hasColorSpace) {
                    ...
                    if (cs != null) {
                        ...
                        bitmap = this;
                        bitmap.mColorSpace = new Rgb("Unknown", xyz, parameters);
                    }

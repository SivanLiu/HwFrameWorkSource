...
public class Canvas extends BaseCanvas {
    ...
    private static void checkValidClipOp(Op op) {
        if (sCompatiblityVersion >= 28 && op != Op.INTERSECT && op != Op.DIFFERENCE) {
    ...
    public boolean clipRectUnion(Rect rect) {
        return nClipRect(this.mNativeCanvasWrapper, (float) rect.left, (float) rect.top, (float) rect.right, (float) rect.bottom, Op.UNION.nativeInt);
    }

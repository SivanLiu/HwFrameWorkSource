...
public class Paint {
    ...
    private void syncTextLocalesWithMinikin() {
        ...
        synchronized (sCacheLock) {
            Integer minikinLocaleListId = (Integer) sMinikinLocaleListIdCache.get(languageTags);
    ...
    public float measureText(char[] text, int index, int count) {
        if (text == null) {
            ...
            if (this.mHasCompatScaling) {
                ...
                float w = nGetTextAdvances(this.mNativePaint, text, index, count, index, count, this.mBidiFlags, null, 0);
                setTextSize(oldSize);
                return (float) Math.ceil((double) (this.mInvCompatScaling * w));
            }
            return (float) Math.ceil((double) nGetTextAdvances(this.mNativePaint, text, index, count, index, count, this.mBidiFlags, null, 0));
        }
    }
    ...
    public float measureText(String text, int start, int end) {
        if (text == null) {
            ...
            if (this.mHasCompatScaling) {
                float oldSize = getTextSize();
                setTextSize(this.mCompatScaling * oldSize);
                float w = nGetTextAdvances(this.mNativePaint, text, start, end, start, end, this.mBidiFlags, null, 0);
                setTextSize(oldSize);
                return (float) Math.ceil((double) (this.mInvCompatScaling * w));
            }
            return (float) Math.ceil((double) nGetTextAdvances(this.mNativePaint, text, start, end, start, end, this.mBidiFlags, null, 0));
        }
    }
    ...
    public float getTextRunAdvances(char[] chars, int index, int count, int contextIndex, int contextCount, boolean isRtl, float[] advances, int advancesIndex) {
        ...
        if (cArr != null) {
            ...
            if ((length | i) < 0) {
                ...
                setTextSize(this.mCompatScaling * oldSize);
                float oldSize2 = oldSize;
                float res = nGetTextAdvances(this.mNativePaint, cArr, index, count, contextIndex, contextCount, isRtl ? 5 : 4, fArr2, advancesIndex);
                setTextSize(oldSize2);
                if (fArr2 != null) {
                    i = advancesIndex;
                    int e = i + count;
                    while (i < e) {
                        fArr2[i] = fArr2[i] * this.mInvCompatScaling;
                        i++;
                    }
                }
    ...
    public float getTextRunAdvances(CharSequence text, int start, int end, int contextStart, int contextEnd, boolean isRtl, float[] advances, int advancesIndex) {
        ...
        if (charSequence != null) {
            if ((((((((((i | i2) | i3) | i4) | advancesIndex) | (i2 - i)) | (i - i3)) | (i4 - i2)) | (text.length() - i4)) | (fArr == null ? 0 : (fArr.length - advancesIndex) - (i2 - i))) < 0) {
                ...
                char[] buf2 = buf;
                float result = getTextRunAdvances(buf, i - i3, len, 0, contextLen, isRtl, fArr, advancesIndex);
                TemporaryBuffer.recycle(buf2);
                return result;
            }
        }
        throw new IllegalArgumentException("text cannot be null");
    ...
    public float getTextRunAdvances(String text, int start, int end, int contextStart, int contextEnd, boolean isRtl, float[] advances, int advancesIndex) {
        ...
        if (text != null) {
            int i3;
            int length = (((((((i | i2) | contextStart) | contextEnd) | advancesIndex) | (i2 - i)) | (i - contextStart)) | (contextEnd - i2)) | (text.length() - contextEnd);
            if (fArr == null) {
                i3 = 0;
            } else {
                i3 = (fArr.length - advancesIndex) - (i2 - i);
            }
            if ((length | i3) < 0) {
                fArr2 = fArr;
                throw new IndexOutOfBoundsException();
                ...
                setTextSize(this.mCompatScaling * oldSize);
                float oldSize2 = oldSize;
                float totalAdvance = nGetTextAdvances(this.mNativePaint, text, i, i2, contextStart, contextEnd, isRtl ? 5 : 4, fArr2, advancesIndex);
                setTextSize(oldSize2);
                if (fArr2 != null) {
                    i3 = advancesIndex;
                    int e = (i2 - i) + i3;
                    while (i3 < e) {
                        fArr2[i3] = fArr2[i3] * this.mInvCompatScaling;
                        i3++;
                    }
                }
                ...
                fArr2 = fArr;
                return nGetTextAdvances(this.mNativePaint, text, i, i2, contextStart, contextEnd, isRtl ? 5 : 4, fArr, advancesIndex);
            }
        }
        fArr2 = fArr;
        throw new IllegalArgumentException("text cannot be null");
    ...
    public float getRunAdvance(CharSequence text, int start, int end, int contextStart, int contextEnd, boolean isRtl, int offset) {
        ...
        if (charSequence == null) {
            ...
            info.append(", end : ");
            info.append(i2);
            info.append(", contextEnd : ");
            info.append(i4);
            info.append(", text.length() : ");
            info.append(text.length());
            info.append(", isRtl : ");
            info.append(isRtl);
            info.append(", text : ");

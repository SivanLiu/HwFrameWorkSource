...
public class FontListParser {
    ...
    private static Family readFamily(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        return new Family(name, (Font[]) fonts.toArray(new Font[fonts.size()]), langs, intVariant);
    }
    ...
    private static Font readFont(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        while (true) {
            ...
            if (parser.next() != 3) {
                ...
                return new Font(FILENAME_WHITESPACE_PATTERN.matcher(filename2).replaceAll(""), index, (FontVariationAxis[]) axes.toArray(new FontVariationAxis[axes.size()]), weight, isItalic, fallbackFor);
            }
        }
    }

...
public class BitmapFactory {
    ...
    public static Bitmap decodeResource(Resources res, int id, Options opts, Rect padding) {
        ...
        } catch (Exception | RuntimeException e5) {
        }
    }
    ...
    public static Bitmap decodeStream(InputStream is, Rect outPadding, Options opts) {
        ...
        try {
            ...
            if (false && threadName.contains(FB_THREAD_NAME) && HwWidgetFactory.isSuperResolutionSupport()) {
    ...
    public static Bitmap decodeFileDescriptor(FileDescriptor fd, Rect outPadding, Options opts) {
        ...
        try {
            if (nativeIsSeekable(fd)) {
                long start = System.currentTimeMillis();
                fis = nativeDecodeFileDescriptor(fd, outPadding, opts);
                Jlog.checkBitmapDecodeTime(start);
            } else {
    ...
    public static boolean convertHeifToJpeg(String pathName, OutputStream stream) {
        ...
        opts.inPreferredConfig = Config.YCRCB_420_SP;
        opts.inSampleSize = 1;
        opts.inScaled = false;

...
public class HwEidServiceManager {
    ...
    public final class EncryptionFactor {
        ...
        public void setCertificate(byte[] certificate) throws Exception {
            if (certificate == null || certificate.length > this.maxCertificateLen) {
                throw new Exception("certificate[] is invalid!");
            }
            System.arraycopy(certificate, 0, this.certificate, 0, certificate.length);
        }

        public void setCertificateLen(int certificateLen) throws Exception {
            if (certificateLen <= 0 || certificateLen > this.maxCertificateLen) {
                throw new Exception("certificateLen is invalid!");
            }
            this.certificateLen = certificateLen;
        }

        public void setEncryptionMethod(int encryptionMethod) throws Exception {
    ...
    public final class ImageZipContainer {
        ...
        public void setHashLen(int hashLen) throws Exception {
            if (hashLen <= 0 || hashLen > this.maxHashLen) {
        ...
        public void setImageZipLen(int imageZipLen) throws Exception {
            if (imageZipLen <= 0 || imageZipLen > this.maxImageZipLen) {
                throw new Exception("imageZipLen is invalid!");
            }
            this.imageZipLen = imageZipLen;
        }
        ...
        public void setHash(byte[] hash) throws Exception {
            if (hash == null || hash.length > this.maxHashLen) {
        ...
        public void setimageZip(byte[] imageZip) throws Exception {
            if (imageZip == null || imageZip.length > this.maxImageZipLen) {
                throw new Exception("imageZip[] is invalid!");
            }
            System.arraycopy(imageZip, 0, this.imageZip, 0, imageZip.length);
        }
    }
    ...
    public int eid_get_image(int transpotCounter, int encryption_method, byte[] certificate, int certificate_len, byte[] image, int[] image_len, byte[] de_skey, int[] de_skey_len) {
        int ret = RET_DEFAULT_ERROR_VALUE;
        if (transpotCounter < 0 || encryption_method < 0 || certificate == null || certificate_len < 0 || image == null || image_len[0] < 0 || image_len == null || de_skey == null || de_skey_len[0] < 0 || de_skey_len == null) {
            return RET_DEFAULT_ERROR_VALUE;
        }
        if (getHwEidPlugin() != null) {
            try {
    ...
    public int eid_get_unsec_image(byte[] src_image, int src_image_len, int transpotCounter, int encryption_method, byte[] certificate, int certificate_len, byte[] image, int[] image_len, byte[] de_skey, int[] de_skey_len) {
        ...
        if (getHwEidPlugin() != null) {
            try {
                ...
                stringBuilder.append("RemoteException when eid_get unsec image is invoked");

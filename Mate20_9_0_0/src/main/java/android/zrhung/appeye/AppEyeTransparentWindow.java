...
public final class AppEyeTransparentWindow extends ZrHungImpl {
    ...
    private HungConfig mHungConfig;
    ...
    private class AppEyeTransparentWindowHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 1) {
                ...
                AppEyeTransparentWindow.this.sendEvent(msg.obj);
            }
        }
    }
    ...
    public int init(ZrHungData args) {
        try {
            if (this.mHungConfig == null || this.mHungConfigStatus == 1) {
                this.mHungConfig = ZRHung.getHungConfig((short) 273);
    ...
    public boolean check(ZrHungData args) {
        try {
            ...
            String str = TAG;
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("TransparentWindow mHungConfigStatus = ");
            stringBuilder.append(this.mHungConfigStatus);
            stringBuilder.append(",mHungConfigEnable = ");
            stringBuilder.append(this.mHungConfigEnable);
    ...
    public boolean sendEvent(ZrHungData args) {
        try {
            LayoutParams layoutParams = (LayoutParams) args.get("layoutParams");
            if (isWinMayCauseFreezeScreen(layoutParams)) {
                ...
                Object obj = args.get(FreezeScreenScene.PID_PARAM);
                ...
                if (this.mHungConfig != null && this.mHungConfigStatus == 0 && "1".equals(this.mHungConfigEnable)) {
                    ...
                    data.putInt(FreezeScreenScene.PID_PARAM, pid);
                    sendAppEyeEvent((short) 273, data, null, sb.toString());
                }
            }
            return true;
        } catch (Exception ex) {
    ...
    private final boolean isZeroAlphaWindow(LayoutParams l) {
        return l.alpha < 0.01f;
    }

    private final boolean isZeroPixelWindow(LayoutParams l) {
        return l.width <= 1 && l.width >= 0 && l.height <= 1 && l.height >= 0;
    }

    private final boolean isTranslucentThemeWithoutActionBar(LayoutParams l) {

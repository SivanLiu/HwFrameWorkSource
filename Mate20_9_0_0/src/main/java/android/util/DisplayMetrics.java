...
public class DisplayMetrics {
    ...
    public String toString() {
        ...
        stringBuilder.append(", noncompatWidthPixels=");
        stringBuilder.append(this.noncompatWidthPixels);
        stringBuilder.append(", noncompatHeightPixels=");
        stringBuilder.append(this.noncompatHeightPixels);
        stringBuilder.append(", noncompatDensity=");
        stringBuilder.append(this.noncompatDensity);
        stringBuilder.append(", noncompatDensityDpi=");
        stringBuilder.append(this.noncompatDensityDpi);
        stringBuilder.append(", noncompatXdpi=");
        stringBuilder.append(this.noncompatXdpi);
        stringBuilder.append(", noncompatYdpi=");
        stringBuilder.append(this.noncompatYdpi);
    ...
    public static int getROGDensity(int width) {
        ...
        if (realdpi == 640 || realdpi == 480) {
            if (width == SCREEN_SIZE_720P_WIDTH) {
                dpi = dpi > 0 ? (320 * dpi) / realdpi : 320;
            } else if (width == SCREEN_SIZE_1080P_WIDTH) {
                dpi = dpi > 0 ? (480 * dpi) / realdpi : 480;
            } else if (width == SCREEN_SIZE_2K_WIDTH) {
                dpi = dpi > 0 ? (640 * dpi) / realdpi : 640;
            }
        }
        ...
        if (width != SCREEN_SIZE_720P_WIDTH) {
            if (width != SCREEN_SIZE_1080P_WIDTH) {
                if (width != SCREEN_SIZE_2K_WIDTH) {
                    return dpi;
                }
                if (dpi > 0) {
                    return (560 * dpi) / realdpi;
                }
                return 560;
            } else if (dpi > 0) {
                return (420 * dpi) / realdpi;
            } else {
                return 420;
            }
        } else if (dpi > 0) {
            return (280 * dpi) / realdpi;
        } else {
            return 280;
        }
    }

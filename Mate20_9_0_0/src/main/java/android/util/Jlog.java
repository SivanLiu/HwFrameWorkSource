...
public final class Jlog {
    ...
    public static final long DECODE_BMP_LIMIT = 500;
    ...
    private static final String MICROTEST_FLAG_FILE = "etc/microtest.xml";
    ...
    private static boolean mbMicroTestEnabled;
    ...
    static {
        ...
        if (new File(Environment.getRootDirectory(), MICROTEST_FLAG_FILE).exists()) {
            mbMicroTestEnabled = true;
        } else {
            mbMicroTestEnabled = false;
        }
    }
    ...
    public static boolean isMicroTest() {
        return mbMicroTestEnabled;
    }
    ...
    public static String extractAppName(String msg) {
        ...
        nStartPos = msg.indexOf(WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER, nStartPos + 2);
        if (nStartPos < 0) {
            return msg;
        }
    ...
    public static void animationSkipFrames(long num) {
        if (mbEmuiAnimation) {
            ...
            if (num >= 3 || mnEmuiAnimationSkipFrames >= 5) {
                ...
                stringBuilder.append(msEmuiAnimationSkippedFrames.toString());
                stringBuilder.append(" frames");
                d(173, str, i, stringBuilder.toString());
                animationEnd();
            }
        }
    }
    ...
    public static void coldLaunchingAppBegin(String pkgname) {
        if (pkgname != null) {
            ...
            print_janklog_native(3, 341, stringBuilder.toString());
        }
    }
    ...
    public static void warmLaunchingAppBegin(String pkgname) {
        if (pkgname != null) {
            ...
            stringBuilder.append(pkgname);
            stringBuilder.append(">");
            print_janklog_native(3, 339, stringBuilder.toString());
        }
    }
    ...
    public static void coldLaunchingAppEnd(String pkgname) {
        if (msLaunchingPkgName != null && pkgname != null && msLaunchingPkgName.length() >= 1) {
            if (pkgname.startsWith(msLaunchingPkgName)) {
                ...
                stringBuilder.append(pkgname);
                stringBuilder.append(">");
                print_janklog_native(3, 342, stringBuilder.toString());
                msLaunchingPkgName = "";
                return;
            }
            msLaunchingPkgName = "";
    ...
    public static void warmLaunchingAppEnd(String pkgname) {
        if (msLaunchingPkgName != null && pkgname != null && msLaunchingPkgName.length() >= 1) {
            if (pkgname.startsWith(msLaunchingPkgName)) {
                ...
                stringBuilder.append(pkgname);
                stringBuilder.append(">");
                print_janklog_native(3, 340, stringBuilder.toString());
                msLaunchingPkgName = "";
                return;
            }
            msLaunchingPkgName = "";
    ...
    public static int d(int tag, String arg1, String msg) {
        ...
        return print_janklog_native(3, tag, stringBuilder.toString());
    }
    ...
    public static void checkBitmapDecodeTime(long start) {
        long diff = System.currentTimeMillis() - start;
        if (diff > 500 && Looper.myLooper() == Looper.getMainLooper()) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("#ARG2:<");
            stringBuilder.append(diff);
            stringBuilder.append(">");
            d(JlogConstants.JLID_DECODE_BMP_TIMEOUT, stringBuilder.toString());
        }
    }

    public static String getMessage(String className, String method, Object key) {
        StringBuffer buffer = new StringBuffer();
        buffer.append("#<");
        buffer.append(className);
        buffer.append(Session.SESSION_SEPARATION_CHAR_CHILD);
        buffer.append(method);
        buffer.append(">#<");
        buffer.append(key);
        buffer.append(">#<");
        buffer.append(String.valueOf(System.nanoTime()));
        buffer.append(">");
        return buffer.toString();
    }

    public static void changeJlogFlag(boolean flag) {
        mbMicroTestEnabled = flag;
    }
}

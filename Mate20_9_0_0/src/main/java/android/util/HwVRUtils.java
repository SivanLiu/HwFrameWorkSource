...
public final class HwVRUtils {
    ...
    private static final String VIRTUAL_SCREEN_PKG = "com.huawei.vrvirtualscreen";
    ...
    public static int getVRDisplayID(Context context) {
        ...
        if ((!isVRMode() && !VR_SERVICE.equals(packageName)) || "android".equals(packageName) || "com.huawei.vrvirtualscreen".equals(packageName)) {
            return -1;
        }
        if (isVRApp(context)) {
    ...
    private static boolean isVRApp(Context context) {
        ...
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("no vr app metadata ");
        stringBuilder.append(vrOnly);

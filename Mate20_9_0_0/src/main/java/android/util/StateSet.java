    ...
    static {
        if (VIEW_STATE_IDS.length / 2 == R.styleable.ViewDrawableStates.length) {
            ...
            for (int i = 0; i < R.styleable.ViewDrawableStates.length; i++) {
                viewState = R.styleable.ViewDrawableStates[i];
            ...
            for (viewState = 0; viewState < VIEW_STATE_SETS.length; viewState++) {
                int[] set = new int[Integer.bitCount(viewState)];
                int pos = 0;
                for (int j2 = 0; j2 < orderedIds.length; j2 += 2) {
                    if ((orderedIds[j2 + 1] & viewState) != 0) {
                        int pos2 = pos + 1;
                        set[pos] = orderedIds[j2];
                        pos = pos2;
                    }
                }
                VIEW_STATE_SETS[viewState] = set;
            }
            return;
        }

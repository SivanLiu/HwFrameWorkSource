...
public final class LauncherIcons {
    ...
    private Bitmap getShadowBitmap(AdaptiveIconDrawable d) {
        ...
        synchronized (this.mShadowCache) {
            ...
            Bitmap shadow2 = Bitmap.createBitmap(bitmapSize, bitmapSize, Config.ARGB_8888);
    ...
    public Drawable getBadgedDrawable(Drawable base, int foregroundRes, int backgroundColor) {
        ...
        sysRes.getDrawable(foregroundRes).setTint(backgroundColor);
        if (base == null) {
            drawableArr = new Drawable[]{badgeShadow, badgeColor, badgeForeground};
        } else {
            drawableArr = new Drawable[]{base, badgeShadow, badgeColor, badgeForeground};
        }

...
public class HwLogExceptionInner implements LogException {
    ...
    public int msg(String category, int level, String header, String body) {
        ...
        msgAll.append(10);
        msgAll.append(body);
        return println_exception_native(category, level, "message", msgAll.toString());
    }
    ...
    public int setliblogparam(int paramid, String val) {
        ...
        stringBuilder.append("Log blacklist ");
    ...
    public static boolean updateBlackList_static(int file) {
        ...
        switch (file) {
            case 1:
                ...
                stringBuilder.append(Environment.getRootDirectory().getPath());
                stringBuilder.append("/etc/hiview/log_blacklist.cfg");
                blackListFile = new File(stringBuilder.toString());
                break;
            case 2:
                stringBuilder = new StringBuilder();
                stringBuilder.append(Environment.getDataDirectory().getPath());
                stringBuilder.append("/system/hiview/log_blacklist.cfg");
                blackListFile = new File(stringBuilder.toString());
                break;
            default:
                return false;
        ...
        mLogBlackList.clear();
        try {
            fi = new FileInputStream(blackListFile);
            is = new InputStreamReader(fi, "UTF-8");
            in = new BufferedReader(is);
            while (true) {
                String readLine = in.readLine();
                String blackPackageName = readLine;
                if (readLine != null) {
    ...
    private static int getConfigFileUpdated() {
        ...
        stringBuilder.append(Environment.getDataDirectory().getPath());
        stringBuilder.append("/system/hiview/log_blacklist.cfg");
        File blackListFile = new File(stringBuilder.toString());
        if (blackListFile.isFile() && blackListFile.canRead()) {

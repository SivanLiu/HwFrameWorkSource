...
public class SparseSetArray<T> {
    ...
    public boolean add(int n, T value) {
        ArraySet<T> set = (ArraySet) this.mData.get(n);
        if (set == null) {
            set = new ArraySet();
    ...
    public int sizeAt(int index) {
        ArraySet<T> set = (ArraySet) this.mData.valueAt(index);
        if (set == null) {
            return 0;
        }
        return set.size();
    ...
    public T valueAt(int intIndex, int valueIndex) {
        return ((ArraySet) this.mData.valueAt(intIndex)).valueAt(valueIndex);
    }
}

...
public class IndianCalendar extends Calendar {
    ...
    protected void handleComputeFields(int julianDay) {
        ...
        if (yday < leapMonth) {
            ...
            IndianDayOfMonth = yday - leapMonth;
            if (IndianDayOfMonth < 155) {
                IndianMonth = (IndianDayOfMonth / 31) + 1;
                IndianDayOfMonth = (IndianDayOfMonth % 31) + 1;
            } else {
                IndianDayOfMonth -= 155;
                IndianMonth = (IndianDayOfMonth / 30) + 6;
                IndianDayOfMonth = (IndianDayOfMonth % 30) + 1;
            }
        }
        internalSet(0, 0);
    ...
    protected int handleComputeMonthStart(int year, int month, boolean useMonth) {
        ...
        return (int) IndianToJD(year, month + 1, Double.MIN_VALUE);
    }
    ...
    private static double gregorianToJD(int year, int month, int date) {
        int y = year - 1;
        int i = ((((365 * y) + (y / 4)) - (y / 100)) + (y / 400)) + (((367 * month) - 362) / 12);
        int i2 = month <= 2 ? 0 : isGregorianLeap(year) ? -1 : -2;
        return ((double) (((i + i2) + date) - 1)) + 1721425.5d;
    }
    ...
    private static int[] jdToGregorian(double jd) {
        ...
        depoch = Math.floor((dcent % 1461.0d) / 365.0d);
        int year = (int) ((((400.0d * quadricent) + (100.0d * cent)) + (quad * 4.0d)) + depoch);
        if (!(cent == 4.0d || depoch == 4.0d)) {
            year++;
        }
        double yearday = wjd - gregorianToJD(year, 1, 1);
        int i = wjd < gregorianToJD(year, 1.5E-323d, 1) ? 0 : isGregorianLeap(year) ? 1 : 2;
        depoch = (double) i;
        i = ((int) (wjd - gregorianToJD(year, (int) Math.floor((((yearday + depoch) * 12.0d) + 373.0d) / 367.0d), 1))) + 1;
        return new int[]{year, (int) Math.floor((((yearday + depoch) * 12.0d) + 373.0d) / 367.0d), i};
    }

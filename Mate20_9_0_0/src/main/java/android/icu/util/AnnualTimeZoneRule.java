...
public class AnnualTimeZoneRule extends TimeZoneRule {
    ...
    public Date getStartInYear(int year, int prevRawOffset, int prevDSTSavings) {
        ...
        if (type == 0) {
            ...
            if (type == 1) {
                ...
                weeks = this.dateTimeRule.getRuleMonth();
                int dom = this.dateTimeRule.getRuleDayOfMonth();
                if (type == 3) {
                    after = false;
                    if (weeks == 1 && dom == 29 && !Grego.isLeapYear(year)) {
                        dom--;
                    }
                }
                ruleDay2 = Grego.fieldsToDay(year, weeks, dom);
            }
            long ruleDay3 = ruleDay2;
            int delta = this.dateTimeRule.getRuleDayOfWeek() - Grego.dayOfWeek(ruleDay3);
            if (after) {
    ...
    public String toString() {
        ...
        stringBuilder.append(this.dateTimeRule);

...
public class GregorianCalendar extends Calendar {
    ...
    protected void handleComputeFields(int julianDay) {
        ...
        if (i >= this.cutoverJulianDay) {
            ...
            month2 = month;
            month = getGregorianYear();
        } else {
            long julianEpochDay = (long) (i - 1721424);
            month = (int) Calendar.floorDivide((4 * julianEpochDay) + 1464, 1461);
            int dayOfYear2 = (int) (julianEpochDay - ((365 * (((long) month) - 1)) + Calendar.floorDivide(((long) month) - 1, 4)));
            boolean isLeap = (month & 3) == 0;
            int correction = 0;
            if (dayOfYear2 >= (isLeap ? 60 : 59)) {
                correction = isLeap ? 1 : 2;
            }
            month2 = ((12 * (dayOfYear2 + correction)) + 6) / 367;
            dayOfYear = dayOfYear2 + 1;
            dayOfMonth = (dayOfYear2 - MONTH_COUNT[month2][isLeap ? 3 : 2]) + 1;
        }
        internalSet(2, month2);
        internalSet(5, dayOfMonth);
        internalSet(6, dayOfYear);
        internalSet(19, month);
        int era = 1;
        if (month < 1) {
            era = 0;
            month = 1 - month;

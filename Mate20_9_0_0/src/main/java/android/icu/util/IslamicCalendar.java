...
public class IslamicCalendar extends Calendar {
    ...
    private long yearStart(int year) {
        if (this.cType == CalculationType.ISLAMIC_CIVIL || this.cType == CalculationType.ISLAMIC_TBLA || (this.cType == CalculationType.ISLAMIC_UMALQURA && (year < UMALQURA_YEAR_START || year > UMALQURA_YEAR_END))) {
            return ((long) ((year - 1) * 354)) + ((long) Math.floor(((double) (3 + (11 * year))) / 30.0d));
        }
        if (this.cType == CalculationType.ISLAMIC) {
            return trueMonthStart((long) (12 * (year - 1)));
        }
        if (this.cType != CalculationType.ISLAMIC_UMALQURA) {
            return 0;
        }
        year -= 1300;
        return (long) (UMALQURA_YEAR_START_ESTIMATE_FIX[year] + ((int) (((354.3672d * ((double) year)) + 460322.05d) + 0.5d)));
    }

    private long monthStart(int year, int month) {
        int realYear = (month / 12) + year;
        ...
        if (this.cType == CalculationType.ISLAMIC_CIVIL || this.cType == CalculationType.ISLAMIC_TBLA || (this.cType == CalculationType.ISLAMIC_UMALQURA && year < UMALQURA_YEAR_START)) {
            return (((long) Math.ceil(29.5d * ((double) realMonth))) + ((long) ((realYear - 1) * 354))) + ((long) Math.floor(((double) (3 + (11 * realYear))) / 30.0d));
        }
        if (this.cType == CalculationType.ISLAMIC) {
            return trueMonthStart((long) ((12 * (realYear - 1)) + realMonth));
        }
        if (this.cType != CalculationType.ISLAMIC_UMALQURA) {
            return 0;
        }
    ...
    private static final long trueMonthStart(long month) {
        ...
        long origin = (((long) Math.floor(((double) month) * 29.530588853d)) * 86400000) + HIJRA_MILLIS;
    ...
    protected int handleGetMonthLength(int extendedYear, int month) {
        if (this.cType == CalculationType.ISLAMIC_CIVIL || this.cType == CalculationType.ISLAMIC_TBLA || (this.cType == CalculationType.ISLAMIC_UMALQURA && (extendedYear < UMALQURA_YEAR_START || extendedYear > UMALQURA_YEAR_END))) {
            ...
            return (UMALQURA_MONTHLENGTH[extendedYear + -1300] & (1 << (11 - month))) == 0 ? 29 : 30;
        }
    }
    ...
    protected int handleGetYearLength(int extendedYear) {
        int length = 0;
        if (this.cType == CalculationType.ISLAMIC_CIVIL || this.cType == CalculationType.ISLAMIC_TBLA || (this.cType == CalculationType.ISLAMIC_UMALQURA && (extendedYear < UMALQURA_YEAR_START || extendedYear > UMALQURA_YEAR_END))) {
            return 354 + civilLeapYear(extendedYear);
        }
        if (this.cType == CalculationType.ISLAMIC) {
    ...
    protected int handleComputeMonthStart(int eyear, int month, boolean useMonth) {
        return (int) ((monthStart(eyear, month) + (this.cType == CalculationType.ISLAMIC_TBLA ? ASTRONOMICAL_EPOC : CIVIL_EPOC)) - 1);
    }
    ...
    protected void handleComputeFields(int julianDay) {
        ...
        int year = 0;
        int month = 0;
        long days = ((long) i) - CIVIL_EPOC;
        int year2;
        int i2;
        if (this.cType == CalculationType.ISLAMIC_CIVIL) {
            year2 = 0;
            i2 = 0;
        } else if (this.cType == CalculationType.ISLAMIC_TBLA) {
            year2 = 0;
            i2 = 0;
        } else {
            if (this.cType == CalculationType.ISLAMIC) {
                months = (int) Math.floor(((double) days) / 29.530588853d);
                if (days - ((long) Math.floor((((double) months) * 29.530588853d) - 1.0d)) >= 25 && moonAge(internalGetTimeInMillis()) > 0.0d) {
                    months++;
                }
                while (true) {
                    long trueMonthStart = trueMonthStart((long) months);
                    long monthStart = trueMonthStart;
                    if (trueMonthStart <= days) {
                        break;
                    }
                    months--;
                }

...
abstract class CECalendar extends Calendar {
    ...
    public static int ceToJD(long year, int month, int day, int jdEpochOffset) {
        ...
        return (int) (((((((long) jdEpochOffset) + (365 * year)) + Calendar.floorDivide(year, 4)) + ((long) (30 * month))) + ((long) day)) - 1);
    }

    public static void jdToCE(int julianDay, int jdEpochOffset, int[] fields) {
        int[] r4 = new int[1];
        int i = 365;
        fields[0] = (4 * Calendar.floorDivide(julianDay - jdEpochOffset, 1461, r4)) + ((r4[0] / 365) - (r4[0] / 1460));
        if (r4[0] != 1460) {
            i = r4[0] % 365;
        }
        int doy = i;

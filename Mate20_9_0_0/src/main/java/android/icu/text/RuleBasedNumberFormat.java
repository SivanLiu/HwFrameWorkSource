...
public class RuleBasedNumberFormat extends NumberFormat {
    ...
    public RuleBasedNumberFormat(ULocale locale, int format) {
        ...
        try {
            ...
            stringBuilder.append("RBNFRules/");
            stringBuilder.append(rulenames[format - 1]);
            UResourceBundleIterator it = bundle.getWithFallback(stringBuilder.toString()).getIterator();
            while (it.hasNext()) {
    ...
    private String[] getNameListForLocale(ULocale loc) {
        if (!(loc == null || this.ruleSetDisplayNames == null)) {
            localeNames = new String[2];
            int i = 0;
            localeNames[0] = loc.getBaseName();
            localeNames[1] = ULocale.getDefault(Category.DISPLAY).getBaseName();
    ...
    public String getRuleSetDisplayName(String ruleSetName, ULocale loc) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("unrecognized rule set name: ");
        stringBuilder.append(ruleSetName);
    ...
    DecimalFormat getDecimalFormat() {
        if (this.decimalFormat == null) {
            this.decimalFormat = new DecimalFormat(NumberFormat.getPattern((ULocale) this.locale, 0), getDecimalFormatSymbols());
        }
    ...
    NFRule getDefaultInfinityRule() {
        if (this.defaultInfinityRule == null) {
            ...
            stringBuilder.append(getDecimalFormatSymbols().getInfinity());
    ...
    NFRule getDefaultNaNRule() {
        if (this.defaultNaNRule == null) {
            ...
            stringBuilder.append(getDecimalFormatSymbols().getNaN());
    ...
    private void init(String description, String[][] localizations) {
        ...
        while (curRuleSet < this.ruleSets.length) {
            indexOf = descBuf.indexOf(";%", p);
            if (indexOf < 0) {
                indexOf = descBuf.length() - 1;
            }
            ruleSetDescriptions[curRuleSet] = descBuf.substring(p, indexOf + 1);
        ...
        for (i2 = this.ruleSets.length - 1; i2 >= 0; i2--) {
            if (!this.ruleSets[i2].getName().startsWith("%%")) {
                int publicRuleSetCount2 = publicRuleSetCount + 1;
        ...
        if (this.publicRuleSetNames != null) {
            i2 = 0;
            while (i2 < this.publicRuleSetNames.length) {
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("did not find public rule set: ");
        ...
        if (localizations != null) {
            ...
            while (i < localizations.length) {
                ...
                if (names.length == this.publicRuleSetNames.length) {
                    ...
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("public name length: ");
                    stringBuilder.append(this.publicRuleSetNames.length);
                    stringBuilder.append(" != localized names[");
            ...
            if (this.postProcessor == null) {
                ...
                try {
                    ...
                    if (DEBUG) {
                        ...
                        stringBuilder.append(ppClassName);
    ...
    NFRuleSet findRuleSet(String name) throws IllegalArgumentException {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("No rule set named ");
        stringBuilder.append(name);
        throw new IllegalArgumentException(stringBuilder.toString());
    }
}

...
public final class RuleBasedCollator extends Collator {
    ...
    private void internalSetVariableTop(long varTop) {
        if (varTop != ((CollationSettings) this.settings.readOnly()).variableTop) {
            ...
            varTop = this.data.getLastPrimaryForGroup(group);
            if (varTop != ((CollationSettings) this.settings.readOnly()).variableTop) {
                CollationSettings ownedSettings = getOwnedSettings();
    ...
    public void setReorderCodes(int... order) {
        ...
        if (length != 0 ? !Arrays.equals(order, ((CollationSettings) this.settings.readOnly()).reorderCodes) : ((CollationSettings) this.settings.readOnly()).reorderCodes.length != 0) {
            SharedObject defaultSettings = getDefaultSettings();
    ...
    public CollationKey getCollationKey(String source) {
        ...
        try {
            ...
            CollationKey collationKey = getCollationKey(source, buffer);
            return collationKey;
        } finally {
    ...
    public RawCollationKey getRawCollationKey(String source, RawCollationKey key) {
        ...
        try {
            buffer = getCollationBuffer();
            RawCollationKey rawCollationKey = getRawCollationKey(source, key, buffer);
    ...
    protected int doCompare(CharSequence left, CharSequence right) {
        ...
        if (equalPrefixLength > 0 && ((equalPrefixLength != left.length() && this.data.isUnsafeBackward(left.charAt(equalPrefixLength), numeric)) || (equalPrefixLength != right.length() && this.data.isUnsafeBackward(right.charAt(equalPrefixLength), numeric)))) {
            while (true) {
                equalPrefixLength--;
                if (equalPrefixLength <= 0 || !this.data.isUnsafeBackward(left.charAt(equalPrefixLength), numeric)) {
                    break;
                }
            }
        }
        ...
        if (result == -2) {
            try {
                CollationBuffer buffer2 = getCollationBuffer();
                if (roSettings.dontCheckFCD()) {
                    ...
                    result = CollationCompare.compareUpToQuaternary(buffer2.leftUTF16CollIter, buffer2.rightUTF16CollIter, roSettings);
                } else {
                    ...
                    result = CollationCompare.compareUpToQuaternary(buffer2.leftFCDUTF16Iter, buffer2.rightFCDUTF16Iter, roSettings);
                }
        ...
        if (result == 0) {
            ...
            if (roSettings.getStrength() >= 15) {
                try {
                    ...
                    i = compareNFDIter(nfcImpl, buffer.leftFCDUTF16NFDIter, buffer.rightFCDUTF16NFDIter);
                    releaseCollationBuffer(buffer);
                    return i;
                } finally {
                    releaseCollationBuffer(buffer);

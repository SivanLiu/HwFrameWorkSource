...
abstract class CharsetRecog_mbcs extends CharsetRecognizer {
    ...
    static abstract class CharsetRecog_euc extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar it, CharsetDetector det) {
            it.error = false;
            int nextByte = it.nextByte(det);
            it.charValue = nextByte;
    ...
    int match(CharsetDetector det, int[] commonChars) {
        ...
        while (nextChar(iter, det)) {
            ...
            if (iter.error) {
                ...
                if (cv <= 255) {
                    ...
                    doubleByteCharCount++;
                    if (iArr != null && Arrays.binarySearch(iArr, (int) cv) >= 0) {
                        commonCharCount++;
                    }
                }
            }
        ...
        if (doubleByteCharCount > 10 || badCharCount != 0) {
            ...
            if (iArr == null) {
                int confidence = (30 + doubleByteCharCount) - (20 * badCharCount);
                if (confidence > 100) {
                    return 100;
                }
                return confidence;
            ...
            return Math.min((int) ((Math.log((double) (commonCharCount + 1)) * (90.0d / maxVal)) + 10.0d), 100);

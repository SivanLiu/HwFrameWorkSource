...
public final class UTF16 {
    ...
    public static final class StringComparator implements Comparator<String> {
        ...
        private int compareCaseSensitive(String s1, String s2) {
            ...
            if (c1 >= 55296 && c2 >= 55296 && codepointcompare) {
                if ((c1 > UCharacter.MAX_HIGH_SURROGATE || index + 1 == length1 || !UTF16.isTrailSurrogate(s1.charAt(index + 1))) && !(UTF16.isTrailSurrogate(c1) && index != 0 && UTF16.isLeadSurrogate(s1.charAt(index - 1)))) {
                    c1 = (char) (c1 - 10240);
                }
                if ((c2 > UCharacter.MAX_HIGH_SURROGATE || index + 1 == length2 || !UTF16.isTrailSurrogate(s2.charAt(index + 1))) && !(UTF16.isTrailSurrogate(c2) && index != 0 && UTF16.isLeadSurrogate(s2.charAt(index - 1)))) {
                    c2 = (char) (c2 - 10240);
                }
            }
            return c1 - c2;
    ...
    public static int charAt(StringBuffer source, int offset16) {
        ...
        if (offset16 >= 0) {
            trail = source.charAt(offset16);
            if (isLeadSurrogate(trail)) {
                return Character.toCodePoint(trail, single);
            }
        }
        return single;
    }
    ...
    public static char getLeadSurrogate(int char32) {
        if (char32 >= 65536) {
            return (char) (LEAD_SURROGATE_OFFSET_ + (char32 >> 10));
        }
        return 0;
    }
    ...
    public static StringBuffer append(StringBuffer target, int char32) {
        if (char32 < 0 || char32 > 1114111) {
            ...
            stringBuilder.append("Illegal codepoint: ");
    ...
    public static StringBuffer delete(StringBuffer target, int offset16) {
        int count = 1;
        int bounds = bounds(target, offset16);
        if (bounds == 2) {
            count = 1 + 1;
        } else if (bounds == 5) {
            count = 1 + 1;
    ...
    public static int delete(char[] target, int limit, int offset16) {
        int count = 1;
        int bounds = bounds(target, 0, limit, offset16);
        if (bounds == 2) {
            count = 1 + 1;
        } else if (bounds == 5) {
            count = 1 + 1;
    ...
    public static boolean hasMoreCodePointsThan(String source, int number) {
        ...
        while (length != 0) {
            ...
            int start = number + 1;
            if (isLeadSurrogate(source.charAt(number)) && start != length && isTrailSurrogate(source.charAt(start))) {
                start++;
                maxsupplementary2--;
    ...
    public static String newString(int[] codePoints, int offset, int count) {
        if (count >= 0) {
            ...
            for (int r = offset; r < e; r++) {
                ...
                chars[w] = (char) (LEAD_SURROGATE_OFFSET_ + (cp >> 10));
                chars[w + 1] = (char) (56320 + (cp & 1023));
                w += 2;
            }

...
class AnyTransliterator extends Transliterator {
    ...
    private AnyTransliterator(String id, String theTarget, String theVariant, int theTargetScript) {
        ...
        if (theVariant.length() > 0) {
            ...
            stringBuilder.append(theTarget);
            stringBuilder.append(VARIANT_SEP);
            stringBuilder.append(theVariant);
            this.target = stringBuilder.toString();
        }
    }
    ...
    private Transliterator getTransliterator(int source) {
        if (source != this.targetScript && source != -1) {
            ...
            if (t == null) {
                ...
                id.append(sourceName);
                id.append(TARGET_SEP);
                id.append(this.target);
                try {
                    t = Transliterator.getInstance(id.toString(), 0);
                } catch (RuntimeException e) {
                }
                if (t == null) {
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append(sourceName);
                    stringBuilder.append(LATIN_PIVOT);
                    stringBuilder.append(this.target);
                    try {
                        t = Transliterator.getInstance(stringBuilder.toString(), 0);

...
public final class StringPrep {
    ...
    public static StringPrep getInstance(int profile) {
        ...
        synchronized (CACHE) {
            ...
            if (instance == null) {
                ...
                stringBuilder.append(PROFILE_NAMES[profile]);
                stringBuilder.append(".spp");
                ByteBuffer bytes = ICUBinary.getRequiredData(stringBuilder.toString());
                if (bytes != null) {
                    try {
        ...
        while (true) {
            ...
            if (val.type != 0 || allowUnassigned) {
                if (val.type == 1) {
                    if (val.isIndex) {
                        int length;
                        nextCodePoint = val.value;
                        if (nextCodePoint >= this.indexes[3] && nextCodePoint < this.indexes[4]) {
                            length = 1;
                        } else if (nextCodePoint >= this.indexes[4] && nextCodePoint < this.indexes[5]) {
                            length = 2;
                        } else if (nextCodePoint < this.indexes[5] || nextCodePoint >= this.indexes[6]) {
                            int index = nextCodePoint + 1;
                            length = this.mappingData[nextCodePoint];
                            nextCodePoint = index;
                        } else {
                            length = 3;
                        }
                        dest.append(this.mappingData, nextCodePoint, length);
                    } else {
                        ch -= val.value;
                    }
                } else if (val.type == 3) {
    ...
    /* JADX WARNING: Missing block: B:35:0x0092, code skipped:
     ...
     */
    /* JADX WARNING: Missing block: B:41:0x00a3, code skipped:
            r13 = r7;
     */

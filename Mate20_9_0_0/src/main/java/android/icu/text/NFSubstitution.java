        ...
        switch (str.charAt(0)) {
            case '<':
                if (rule.getBaseValue() == -1) {
                    ...
                    return new NumeratorSubstitution(i, (double) rule.getBaseValue(), formatter.getDefaultRuleSet(), str);
                }
            case '=':
            ...
            case '>':
                ...
                if (!ruleSet.isFractionSet()) {
                    return new ModulusSubstitution(i, rule, rulePredecessor, nFRuleSet, str);
                }

...
public class SpoofChecker {
    ...
    public static class Builder {
        ...
        private static class ConfusabledataBuilder {
            ...
            void build(Reader confusables, SpoofData dest) throws ParseException, IOException {
                            ...
                            if (keyChar <= 1114111) {
                                ...
                                while (m.find()) {
                                    ...
                                    if (c <= 1114111) {
                                        ...
                                        stringBuilder.append("Confusables, line ");
                                        stringBuilder.append(this.fLineNum);
                                        stringBuilder.append(": Bad code point: ");
                                        stringBuilder.append(Integer.toString(c, 16));
                                        throw new ParseException(stringBuilder.toString(), matcher.start(2));
                                    }
                                }
                                ...
                                stringBuilder2.append("Confusables, line ");
                                stringBuilder2.append(this.fLineNum);
                                stringBuilder2.append(": Bad code point: ");
                                stringBuilder2.append(matcher.group(1));
                                throw new ParseException(stringBuilder2.toString(), matcher.start(1));
                            }
                        }
                        stringBuilder = new StringBuilder();
                        stringBuilder.append("Confusables, line ");
                        stringBuilder.append(this.fLineNum);
                        stringBuilder.append(": Unrecognized Line: ");
                        stringBuilder.append(matcher.group(8));
                        throw new ParseException(stringBuilder.toString(), matcher.start(8));
                    }
                }
                ...
                Iterator it = this.fKeySet.iterator();
                while (it.hasNext()) {
                    strLen = ((String) it.next()).codePointAt(line);
                    SPUString targetMapping = (SPUString) this.fTable.get(Integer.valueOf(strLen));
                    if (targetMapping.fStr.length() <= 256) {
                        int key = ConfusableDataUtils.codePointAndLengthToKey(strLen, targetMapping.fStr.length());
                        c = targetMapping.fCharOrStrTableIndex;
                        this.fKeyVec.add(Integer.valueOf(key));
                        this.fValueVec.add(Integer.valueOf(c));
                        line = null;
                    } else {
                        throw new IllegalArgumentException("Confusable prototypes cannot be longer than 256 entries.");
                    }
                }
                line = this.fKeyVec.size();
                spoofData.fCFUKeys = new int[line];
                for (i = 0; i < line; i++) {
                    int key2 = ((Integer) this.fKeyVec.get(i)).intValue();
                    strLen = ConfusableDataUtils.keyToCodePoint(key2);
                    spoofData.fCFUKeys[i] = key2;
                    int previousCodePoint = strLen;
                }
                spoofData.fCFUValues = new short[this.fValueVec.size()];
                i = 0;
                Iterator it2 = this.fValueVec.iterator();
                while (it2.hasNext()) {
                    c = i + 1;
                    spoofData.fCFUValues[i] = (short) ((Integer) it2.next()).intValue();
                    i = c;
                }
    ...
    private static class SpoofData {
        ...
        private static final class IsAcceptable implements Authenticate {
            ...
            public boolean isDataVersionAcceptable(byte[] version) {
                return (version[0] != (byte) 2 && version[1] == (byte) 0 && version[2] == (byte) 0 && version[3] == (byte) 0) ? false : true;
            }
        }
    ...
    public boolean failsChecks(String text, CheckResult checkResult) {
        ...
        if ((this.fChecks & 32) != 0) {
            ...
            c = 0;
            while (c < length) {
                int c2 = Character.codePointAt(nfdText, c);
                c = Character.offsetByCodePoints(nfdText, c, 1);

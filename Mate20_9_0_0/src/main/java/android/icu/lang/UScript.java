...
public final class UScript {
    ...
    public static final boolean hasScript(int c, int sc) {
        ...
        if (sc == (32767 & scriptExtensions[scx])) {
            z = true;
        }
        return z;
    }
    ...
    public static final int getScriptExtensions(int c, BitSet set) {
        ...
        int sx = scriptX & 255;
        if (scriptX >= UCharacterProperty.SCRIPT_X_WITH_OTHER) {
            sx = scriptExtensions[sx + 1];
        }
        while (true) {
            int scx = sx + 1;
            sx = scriptExtensions[sx];
            set.set(sx & 32767);
            length++;
            if (sx >= 32768) {
                return -length;

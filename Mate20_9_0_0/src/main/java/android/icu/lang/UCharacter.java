...
public final class UCharacter implements ECharacterCategory, ECharacterDirection {
    ...
    public static final class UnicodeBlock extends Subset {
        ...
        public static final UnicodeBlock CJK_UNIFIED_IDEOGRAPHS_EXTENSION_F = new UnicodeBlock("CJK_UNIFIED_IDEOGRAPHS_EXTENSION_F", CJK_UNIFIED_IDEOGRAPHS_EXTENSION_F_ID);
        ...
        public static final UnicodeBlock KANA_EXTENDED_A = new UnicodeBlock("KANA_EXTENDED_A", KANA_EXTENDED_A_ID);
        ...
        public static final UnicodeBlock MASARAM_GONDI = new UnicodeBlock("MASARAM_GONDI", MASARAM_GONDI_ID);
        ...
        public static final UnicodeBlock NUSHU = new UnicodeBlock("NUSHU", NUSHU_ID);
        ...
        public static final UnicodeBlock SOYOMBO = new UnicodeBlock("SOYOMBO", SOYOMBO_ID);
        ...
        public static final UnicodeBlock SYRIAC_SUPPLEMENT = new UnicodeBlock("SYRIAC_SUPPLEMENT", SYRIAC_SUPPLEMENT_ID);
        ...
        public static final UnicodeBlock ZANABAZAR_SQUARE = new UnicodeBlock("ZANABAZAR_SQUARE", ZANABAZAR_SQUARE_ID);
        ...
        static {
            int blockId = 0;
            while (true) {
                int blockId2 = blockId;
                if (blockId2 >= COUNT) {
                    return;
                }
                if (BLOCKS_[blockId2] != null) {
                    blockId = blockId2 + 1;
                } else {
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("UnicodeBlock.BLOCKS_[");
                    stringBuilder.append(blockId2);
    ...
    public static boolean isUnicodeIdentifierPart(int ch) {
        if (((1 << getType(ch)) & 4196222) != 0 || isIdentifierIgnorable(ch)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isIdentifierIgnorable(int ch) {
        boolean z = false;
        if (ch <= 159) {
            if (isISOControl(ch) && ((ch < 9 || ch > 13) && (ch < 28 || ch > 31))) {
                z = true;
            }
            return z;
        }
    ...
    public static boolean isPrintable(int ch) {
        int cat = getType(ch);
        return (cat == 0 || cat == 15 || cat == 16 || cat == 17 || cat == 18 || cat == 0) ? false : true;
    }
    ...
    public static int foldCase(int ch, boolean defaultmapping) {
        return foldCase(ch, defaultmapping ^ 1);
    }

    public static String foldCase(String str, boolean defaultmapping) {
        return foldCase(str, defaultmapping ^ 1);
    }
    ...
    public static String getStringPropertyValue(int propertyEnum, int codepoint, int nameChoice) {
        if ((propertyEnum >= 0 && propertyEnum < 64) || (propertyEnum >= 4096 && propertyEnum < UProperty.INT_LIMIT)) {
    ...
    public static final int codePointAt(CharSequence seq, int c1) {
        int index = c1 + 1;
        char c12 = seq.charAt(c1);
        if (isHighSurrogate(c12) && index < seq.length()) {
            char c2 = seq.charAt(index);
            if (isLowSurrogate(c2)) {
                return toCodePoint(c12, c2);
            }
        }
        return c12;
    }

    public static final int codePointAt(char[] text, int c1) {
        int index = c1 + 1;
        char c12 = text[c1];
        if (isHighSurrogate(c12) && index < text.length) {
            char c2 = text[index];
            if (isLowSurrogate(c2)) {
                return toCodePoint(c12, c2);
            }
    ...
    public static final int codePointAt(char[] text, int index, int limit) {
        ...
        int index2 = index + 1;
        index = text[index];
        if (isHighSurrogate(index) && index2 < limit) {
            char c2 = text[index2];
            if (isLowSurrogate(c2)) {
                return toCodePoint(index, c2);
            }
        }
        return index;
    }
    ...
    public static int codePointCount(CharSequence text, int start, int limit) {
        if (start < 0 || limit < start || limit > text.length()) {
            ...
            stringBuilder.append(") invalid or out of range 0, ");
    ...
    public static int offsetByCodePoints(CharSequence text, int index, int codePointOffset) {
        ...
        if (codePointOffset >= 0) {
            ...
            while (true) {
                ...
                while (ch2 >= MIN_LOW_SURROGATE && ch2 <= 57343 && index > 0) {
                    index--;
                    ch2 = text.charAt(index);
                    if (ch2 < 55296 || ch2 > MAX_HIGH_SURROGATE) {
                        codePointOffset++;
                        if (codePointOffset > 0) {
                            return index + 1;
                        }
    ...
    public static int offsetByCodePoints(char[] text, int start, int count, int index, int codePointOffset) {
        ...
        if (start < 0 || limit < start || limit > text.length || index < start || index > limit) {
            ...
            stringBuilder.append(limit);
            stringBuilder.append(" in array 0, ");
            stringBuilder.append(text.length);
            throw new IndexOutOfBoundsException(stringBuilder.toString());
        }
        if (codePointOffset >= 0) {
        ...
        while (true) {
            ...
            char ch2 = text[index];
            if (index >= start) {
                while (ch2 >= MIN_LOW_SURROGATE && ch2 <= 57343 && index > start) {
                    index--;
                    ch2 = text[index];
                    if (ch2 < 55296 || ch2 > MAX_HIGH_SURROGATE) {
                        codePointOffset++;

...
public final class UCharacterProperty {
    ...
    BinaryProperty[] binProps = new BinaryProperty[]{new BinaryProperty(1, 256), new BinaryProperty(1, 128), new BinaryProperty(5) {
        ...
        boolean contains(int c) {
            ...
            if (c < 0) {
                return 1 ^ UCharacter.foldCase(nfd, true).equals(nfd);
            }
        ...
        boolean contains(int c) {
            ...
            String str = src;
            normalizer2Impl.compose(str, 0, src.length(), false, true, new ReorderingBuffer(kcf, dest, 5));
    ...
    public double getUnicodeNumericValue(int c) {
        ...
        if (ntv < 768) {
            int exp = (ntv & 31) + 2;
            double numValue2 = (double) ((ntv >> 5) - 14);
            while (exp >= 4) {
                numValue2 *= 10000.0d;
                exp -= 4;
            }
            ...
            numValue = ntv - 804;
            return ((double) ((2 * (numValue & 3)) + 1)) / ((double) (20 << (numValue >> 2)));
        }
    }

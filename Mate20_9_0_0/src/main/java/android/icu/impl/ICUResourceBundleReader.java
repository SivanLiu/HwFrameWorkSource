...
public final class ICUResourceBundleReader {
    ...
    private static final class ResourceCache {
        ...
        private static final class Level {
            ...
            static {
                Class cls = ICUResourceBundleReader.class;
            }

            Level(int levelBitsList, int shift) {
                this.levelBitsList = levelBitsList;
                this.shift = shift;
                int length = 1 << (levelBitsList & 15);
                this.mask = length - 1;
    ...
    private static final class IsAcceptable implements Authenticate {
        ...
        public boolean isDataVersionAcceptable(byte[] formatVersion) {
            return (formatVersion[0] == (byte) 1 && (formatVersion[1] & 255) >= 1) || ((byte) 2 <= formatVersion[0] && formatVersion[0] <= (byte) 3);
        }
    }
    ...
    static class Table extends Container implements android.icu.impl.UResource.Table {
        ...
        String getKey(ICUResourceBundleReader reader, int index) {
            ...
            if (this.keyOffsets != null) {
                access$1100 = reader.getKey16String(this.keyOffsets[index]);
            } else {
                access$1100 = reader.getKey32String(this.key32Offsets[index]);
            }
            return access$1100;
        }
        ...
        protected ICUResourceBundleReader createInstance(ReaderCacheKey key, ClassLoader loader) {
            ...
            try {
                ...
                if (key.baseName == null || !key.baseName.startsWith(ICUData.ICU_BASE_NAME)) {
                    ...
                    byteBufferFromInputStreamAndCloseStream = ICUBinary.getData(loader, fullName, fullName.substring(ICUData.ICU_BASE_NAME.length() + 1));
                    if (byteBufferFromInputStreamAndCloseStream == null) {
                        return ICUResourceBundleReader.NULL_READER;
                    }
                }
                return new ICUResourceBundleReader(byteBufferFromInputStreamAndCloseStream, key.baseName, key.localeID, loader);
                ...
                throw new ICUUncheckedIOException(stringBuilder.toString(), ex);
            }
        }
    }
    ...
    private static final class Table1632 extends Table {
        ...
        Table1632(ICUResourceBundleReader reader, int offset) {
            ...
            this.itemsOffset = (2 * ((this.size + 2) & -2)) + offset;
        }
    }
    ...
    private static final class Table32 extends Table {
        ...
        Table32(ICUResourceBundleReader reader, int offset) {
            ...
            this.itemsOffset = (4 * (1 + this.size)) + offset;
        }
    }
                ...
                if (dataLength >= (indexesInt << 2)) {
                    ...
                    if (keysTop > att) {
                        if (this.isPoolBundle) {
                            this.keyBytes = new byte[((keysTop - att) << 2)];
                            this.bytes.position(att << 2);
                        } else {
                    ...
                    if (!this.isPoolBundle || this.b16BitUnits.length() > 1) {
                        this.resourceCache = new ResourceCache(indexesInt);
                    }
    ...
    private char[] getTable16KeyOffsets(int length) {
        int offset = length + 1;
        length = this.b16BitUnits.charAt(length);
        if (length <= 0) {
            return emptyChars;
        }
    ...
    String getStringV2(int res) {
        ...
        if ((first & -1024) == UTF16.TRAIL_SURROGATE_MIN_VALUE) {
            ...
            while (true) {
                offset++;
                char charAt = this.b16BitUnits.charAt(offset);
                char c = charAt;
                if (charAt == 0) {
                    break;
                }

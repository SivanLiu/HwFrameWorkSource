...
public abstract class Trie2 implements Iterable<Range> {
    ...
    public static class Range {
        ...
        public int hashCode() {
            return Trie2.hashByte(Trie2.hashInt(Trie2.hashUChar32(Trie2.hashUChar32(Trie2.initHash(), this.startCodePoint), this.endCodePoint), this.value), this.leadSurrogate);
        }
    }
    ...
    public static int getVersion(InputStream is, boolean littleEndianOk) throws IOException {
        if (is.markSupported()) {
            ...
            if (littleEndianOk) {
                ...
                if (sig[0] == (byte) 50 && sig[1] == (byte) 105 && sig[2] == (byte) 114 && sig[3] == (byte) 84) {
                    return 2;
                }
                return 0;
            }
    ...
    public final boolean equals(Object other) {
        ...
        while (it.hasNext()) {
            Range rangeFromThis = (Range) it.next();
            if (!otherIter.hasNext() || !rangeFromThis.equals((Range) otherIter.next())) {
                return false;
            }
        }
    ...
    public int hashCode() {
        if (this.fHash == 0) {
            ...
            Iterator it = iterator();
            while (it.hasNext()) {
                hash = hashInt(hash, ((Range) it.next()).hashCode());
            }
            if (hash == 0) {
                hash = 1;
            }

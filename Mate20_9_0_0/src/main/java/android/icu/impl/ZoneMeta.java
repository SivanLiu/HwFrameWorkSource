...
public final class ZoneMeta {
    ...
    public static String getCanonicalCountry(String tzid, Output<Boolean> isPrimary) {
        ...
        if (singleZone == null) {
            boolean z = true;
            if (TimeZone.getAvailableIDs(SystemTimeZoneType.CANONICAL_LOCATION, country, null).size() > 1) {
                z = false;
            }
    ...
    public static UResourceBundle openOlsonResource(UResourceBundle top, String id) {
        ...
        MissingResourceException e2 = top.get(kZONES);
        UResourceBundle zone = e2.get(zoneIdx);
        if (zone.getType() == 7) {
            zone = e2.get(zone.getInt());
        }
        return zone;
    }
        ...
        if (str != null && id.length() > "GMT".length() && str.toUpperCase(Locale.ENGLISH).startsWith("GMT")) {
            ...
            if (pos.getIndex() >= id.length()) {
                ...
                if (pos.getIndex() < id.length()) {
                    ...
                    length = pos.getIndex();
                    n = numberFormat.parse(str, pos);
                    if (pos.getIndex() != id.length() || pos.getIndex() - length != 2) {
                        return false;
                    }
                    sec = n.intValue();
                }

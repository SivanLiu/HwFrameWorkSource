...
public final class UCharacterName {
     ...
     */
        ...
        int getChar(String name) {
            ...
            switch (this.m_type_) {
                ...
                case (byte) 1:
                    result = this.m_rangestart_;
                    while (result <= this.m_rangeend_) {
                        int offset = result - this.m_rangestart_;
                        int[] indexes = this.m_utilIntBuffer_;
                        synchronized (this.m_utilIntBuffer_) {
                            for (int i = this.m_variant_ - 1; i > 0; i--) {
                                int factor = this.m_factor_[i] & 255;
        ...
        int add(int[] set, int maxlength) {
            int length = UCharacterName.add(set, (String) this.m_prefix_);
            switch (this.m_type_) {
                case (byte) 0:
    ...
    public int getGroupLengths(int index, char[] offsets, char[] lengths) {
        ...
        while (i < 32) {
            ...
            for (int shift = 4; shift >= 0; shift -= 4) {
                ...
                if (length2 != 65535 || n <= (byte) 11) {
                    if (length2 != 65535) {
                        lengths[i2] = (char) ((length2 | n) + 12);
                    } else {
                        lengths[i2] = (char) n;
                    }
                    if (i2 < 32) {
    ...
    public String getGroupName(int index, int length, int choice) {
        ...
        synchronized (this.m_utilStringBuffer_) {
            ...
            while (true) {
                ...
                byte b = this.m_groupstring_[index + oldindex];
                oldindex++;
                if (b < this.m_tokentable_.length) {
                    char token = this.m_tokentable_[b & 255];
                    if (token == 65534) {
                        token = this.m_tokentable_[(b << 8) | (this.m_groupstring_[index + oldindex] & 255)];
                        oldindex++;
                    }
                    if (token != 65535) {
    ...
    private int getGroupChar(int index, char[] length, String name, int choice) {
        ...
        while (result <= 32) {
            ...
            while (count < len && nindex != -1 && nindex < namelen) {
                b2 = this.m_groupstring_[index2 + count];
                count++;
                if (b2 >= this.m_tokentable_.length) {
                    nindex = str.charAt(nindex) != (b2 & 255) ? -1 : nindex + 1;
                } else {
                    char token = this.m_tokentable_[b2 & 255];
                    if (token == 65534) {
                        token = this.m_tokentable_[(this.m_groupstring_[index2 + count] & 255) | (b2 << 8)];
                        count++;
                    }
                    if (token == 65535) {
                        nindex = str.charAt(nindex) != (b2 & 255) ? -1 : nindex + 1;
            ...
            if (namelen == nindex && (count == len || this.m_groupstring_[index2 + count] == (byte) 59)) {
                return result;
            }
            index2 += len;
            result++;
    ...
    private void addGroupName(int maxlength) {
        ...
        maxlength = 0;
        while (maxlength < this.m_groupcount_) {
            int offset = getGroupLengths(maxlength, offsets, lengths);
            int maxisolength2 = maxisolength;
            for (maxisolength = 0; maxisolength < 32; maxisolength++) {
                int lineoffset = offsets[maxisolength] + offset;
                int length = lengths[maxisolength];
                if (length != 0) {
                    ...
                    if (parsed[1] < length) {
                        ...
                        if (parsed[1] < length) {
                            length -= parsed[1];
                            if (addGroupName(lineoffset, length, tokenlengths, this.m_ISOCommentSet_)[1] > maxisolength2) {
                                maxisolength2 = length;
                            }
                        }
                    }
                }

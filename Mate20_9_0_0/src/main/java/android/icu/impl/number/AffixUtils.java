...
public class AffixUtils {
    ...
    public static final Field getFieldForType(int type) {
        if (type == -15) {
    ...
    public static int unescape(CharSequence affixPattern, NumberStringBuilder output, int position, SymbolProvider provider) {
        ...
        while (hasNext(tag, affixPattern)) {
            ...
            if (typeOrCp == -15) {
                insertCodePoint = output.insertCodePoint(position + length, UCharacter.REPLACEMENT_CHAR, Field.CURRENCY);
            } else if (typeOrCp < 0) {
                insertCodePoint = output.insert(position + length, provider.getSymbol(typeOrCp), getFieldForType(typeOrCp));
            } else {
                insertCodePoint = output.insertCodePoint(position + length, typeOrCp, null);
            }
    ...
    public static boolean hasCurrencySymbols(CharSequence affixPattern) {
        ...
        while (hasNext(tag, affixPattern)) {
            tag = nextToken(tag, affixPattern);
            int typeOrCp = getTypeOrCp(tag);
            if (typeOrCp < 0 && getFieldForType(typeOrCp) == Field.CURRENCY) {
                return true;
            }
        }
        return false;
    ...
    public static boolean hasNext(long tag, CharSequence string) {
        ...
        int offset = getOffset(tag);
        boolean z = false;
        if (state == 2 && offset == string.length() - 1 && string.charAt(offset) == PatternTokenizer.SINGLE_QUOTE) {
            return false;
        }
        if (state != 0) {
            return true;

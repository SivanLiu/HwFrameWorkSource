...
public final class MotionEvent extends InputEvent implements Parcelable {
    ...
    public final boolean addBatch(MotionEvent event) {
        ...
        synchronized (gSharedTempLock) {
            ...
            while (true) {
                int h = i;
                if (h <= historySize) {
                    i = h == historySize ? Integer.MIN_VALUE : h;
                    for (int i2 = z; i2 < pointerCount; i2++) {
                        nativeGetPointerCoords(motionEvent.mNativePtr, i2, i, pc[i2]);
                    }
                    int historySize2 = historySize;
                    int h2 = h;
                    nativeAddBatch(this.mNativePtr, nativeGetEventTimeNanos(motionEvent.mNativePtr, i), pc, metaState);
                    i = h2 + 1;
    ...
    public final MotionEvent split(int idBits) {
        Throwable th;
        MotionEvent ev = obtain();
        Object historySize = gSharedTempLock;
        synchronized (historySize) {
                ...
                if (newPointerCount != 0) {
                    ...
                    if (oldActionMasked == 5 || oldActionMasked == 6) {
                        if (newActionPointerIndex < 0) {
                            ...
                            i2 = (newActionPointerIndex << 8) | oldActionMasked;
                        }
                        newAction = i2;
                    } else {
                        newAction = oldAction;
                    ...
                    while (true) {
                        ...
                        if (h2 <= historySize3) {
                            ...
                            int h3;
                            int historyPos2 = h2 == historySize3 ? Integer.MIN_VALUE : h2;
                            i2 = 0;
                            while (i2 < newPointerCount) {
                                oldPointerCount2 = oldPointerCount;
                                historySize2 = historySize3;
                                nativeGetPointerCoords(this.mNativePtr, map[i2], historyPos2, pc[i2]);
                                i2++;
                            ...
                            oldPointerCount2 = oldPointerCount;
                            historySize2 = historySize3;
                            long eventTimeNanos = nativeGetEventTimeNanos(this.mNativePtr, historyPos2);
                            if (h2 == 0) {
                                long j = ev.mNativePtr;
                                historyPos = historyPos2;
                                h = h2;
                                newPointerCount2 = newPointerCount;
                                int oldActionPointerIndex2 = oldActionPointerIndex;
                                int oldActionMasked2 = oldActionMasked;
                                int oldAction2 = oldAction;
                                MotionEvent ev2 = ev;
                                obj = historySize;
                                try {
                                    oldPointerCount = historyPos;
                                    h3 = h;
                                    historySize4 = historySize2;
                                    historySize2 = newPointerCount2;
                                    historyPos = oldActionPointerIndex2;
                                    h = oldActionMasked2;
                                    newPointerCount2 = oldAction2;
                                    motionEvent = ev2;
                                    motionEvent.mNativePtr = nativeInitialize(j, nativeGetDeviceId(this.mNativePtr), nativeGetSource(this.mNativePtr), newAction, nativeGetFlags(this.mNativePtr), nativeGetEdgeFlags(this.mNativePtr), nativeGetMetaState(this.mNativePtr), nativeGetButtonState(this.mNativePtr), nativeGetXOffset(this.mNativePtr), nativeGetYOffset(this.mNativePtr), nativeGetXPrecision(this.mNativePtr), nativeGetYPrecision(this.mNativePtr), nativeGetDownTimeNanos(this.mNativePtr), eventTimeNanos, historySize2, pp, pc);
                                } catch (Throwable th3) {
                                    th = th3;
                                    throw th;
                                }
                            ...
                            motionEvent = ev;
                            obj = historySize;
                            oldPointerCount = historyPos2;
                            h3 = h2;
                            historyPos = oldActionPointerIndex;
                            h = oldActionMasked;
                            newPointerCount2 = oldAction;
                            historySize4 = historySize2;
                            ...
                            oldActionPointerIndex = historyPos;
                            oldActionMasked = h;
                            oldAction = newPointerCount2;
                            historySize = obj;
                        } else {
                            motionEvent = ev;
                            obj = historySize;
                            historySize4 = historySize3;
                            historySize2 = newPointerCount;
                            historyPos = oldActionPointerIndex;
                            h = oldActionMasked;
                            newPointerCount2 = oldAction;
    ...
    public String toString() {
        ...
        for (int i = 0; i < pointerCount; i++) {
            ...
            appendUnless(str, msg, stringBuilder2.toString(), toolTypeToString(getToolType(i)));
        }
    ...
    public static String buttonStateToString(int buttonState) {
        ...
        while (buttonState2 != 0) {
            ...
            buttonState2 >>>= 1;
            if (isSet) {
                String name = BUTTON_SYMBOLIC_NAMES[buttonState];
                if (result != null) {
                    result.append('|');
                    result.append(name);

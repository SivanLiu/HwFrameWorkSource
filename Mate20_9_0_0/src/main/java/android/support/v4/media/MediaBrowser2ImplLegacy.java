...
class MediaBrowser2ImplLegacy extends MediaController2ImplLegacy implements SupportLibraryImpl {
    ...
    private class GetChildrenCallback extends SubscriptionCallback {
        ...
        public void onChildrenLoaded(final String parentId, List<MediaItem> children, Bundle options) {
            List<MediaItem2> items;
            if (children == null) {
                items = null;
            } else {
                items = new ArrayList();
                for (int i = 0; i < children.size(); i++) {
                    items.add(MediaUtils2.convertToMediaItem2((MediaItem) children.get(i)));
                }
            }
    ...
    private class SubscribeCallback extends SubscriptionCallback {
        ...
        /* synthetic */ SubscribeCallback(MediaBrowser2ImplLegacy x0, AnonymousClass1 x1) {
            this();
        }
    ...
    MediaBrowser2ImplLegacy(@NonNull Context context, MediaBrowser2 instance, @NonNull SessionToken2 token, @NonNull Executor executor, @NonNull BrowserCallback callback) {
    ...
    public void subscribe(@NonNull String parentId, @Nullable Bundle extras) {
        if (parentId != null) {
            ...
            if (browser != null) {
                SubscribeCallback callback = new SubscribeCallback(this, null);
                synchronized (this.mLock) {
                    List<SubscribeCallback> list = (List) this.mSubscribeCallbacks.get(parentId);
                    if (list == null) {
                        list = new ArrayList();
            ...
            if (browser != null) {
                synchronized (this.mLock) {
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        browser.unsubscribe(parentId, (SubscriptionCallback) list.get(i));
                    }
                    return;
                }
            }
    ...
    public void getSearchResult(@NonNull String query, final int page, final int pageSize, @Nullable final Bundle extras) {
        ...
        if (browser != null) {
            ...
            options.putInt(MediaBrowserCompat.EXTRA_PAGE, page);
            options.putInt(MediaBrowserCompat.EXTRA_PAGE_SIZE, pageSize);
            browser.search(query, options, new SearchCallback() {
                public void onSearchResult(final String query, Bundle extrasSent, final List<MediaItem> items) {
                    MediaBrowser2ImplLegacy.this.getCallbackExecutor().execute(new Runnable() {
                        public void run() {
    ...
    private MediaBrowserCompat getBrowserCompat(Bundle extras) {
        ...
        synchronized (this.mLock) {
            mediaBrowserCompat = (MediaBrowserCompat) this.mBrowserCompats.get(extras);
        }

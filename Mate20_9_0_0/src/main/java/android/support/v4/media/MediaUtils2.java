...
class MediaUtils2 {
    ...
    static MediaItem convertToMediaItem(MediaItem2 item2) {
        ...
        if (metadata == null) {
            descCompat = new Builder().setMediaId(item2.getMediaId()).build();
        } else {
            descCompat = new Builder().setMediaId(item2.getMediaId()).setSubtitle(metadata.getText("android.media.metadata.DISPLAY_SUBTITLE")).setDescription(metadata.getText("android.media.metadata.DISPLAY_DESCRIPTION")).setIconBitmap(metadata.getBitmap("android.media.metadata.DISPLAY_ICON")).setExtras(metadata.getExtras());
            String title = metadata.getString("android.media.metadata.TITLE");
            if (title != null) {
                descCompat.setTitle(title);
            } else {
                descCompat.setTitle(metadata.getString("android.media.metadata.DISPLAY_TITLE"));
            }
            String displayIconUri = metadata.getString("android.media.metadata.DISPLAY_ICON_URI");
            if (displayIconUri != null) {
                descCompat.setIconUri(Uri.parse(displayIconUri));
            }
            String mediaUri = metadata.getString("android.media.metadata.MEDIA_URI");
            if (mediaUri != null) {
                descCompat.setMediaUri(Uri.parse(mediaUri));
            }
            descCompat = descCompat.build();
        }
    ...
    static List<MediaItem> convertToMediaItemList(List<MediaItem2> items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            result.add(convertToMediaItem((MediaItem2) items.get(i)));
        }
        return result;
    }
    ...
    static List<MediaItem2> convertMediaItemListToMediaItem2List(List<MediaItem> items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            result.add(convertToMediaItem2((MediaItem) items.get(i)));
        }
        return result;
    }
    ...
    static List<MediaItem2> convertBundleListToMediaItem2List(List<Bundle> itemBundleList) {
        ...
        for (int i = 0; i < itemBundleList.size(); i++) {
            Bundle itemBundle = (Bundle) itemBundleList.get(i);
            if (itemBundle != null) {
    ...
    static List<Bundle> convertMediaItem2ListToBundleList(List<MediaItem2> playlist) {
        ...
        for (int i = 0; i < playlist.size(); i++) {
            MediaItem2 item = (MediaItem2) playlist.get(i);
            if (item != null) {
    ...
    static List<Bundle> convertCommandButtonListToBundleList(List<CommandButton> commandButtonList) {
        List<Bundle> commandButtonBundleList = new ArrayList();
        for (int i = 0; i < commandButtonList.size(); i++) {
            Bundle bundle = ((CommandButton) commandButtonList.get(i)).toBundle();
            if (bundle != null) {
    ...
    static Parcelable[] convertMediaItem2ListToParcelableArray(List<MediaItem2> playlist) {
        ...
        for (int i = 0; i < playlist.size(); i++) {
            MediaItem2 item = (MediaItem2) playlist.get(i);
            if (item != null) {
                Parcelable itemBundle = item.toBundle();
                if (itemBundle != null) {
    ...
    static Parcelable[] convertCommandButtonListToParcelableArray(List<CommandButton> layout) {
        ...
        for (int i = 0; i < layout.size(); i++) {
            Bundle bundle = ((CommandButton) layout.get(i)).toBundle();
            if (bundle != null) {
    ...
    static List<CommandButton> convertToCommandButtonList(List<Bundle> commandButtonBundleList) {
        ...
        for (int i = 0; i < commandButtonBundleList.size(); i++) {
            Bundle bundle = (Bundle) commandButtonBundleList.get(i);
            if (bundle != null) {

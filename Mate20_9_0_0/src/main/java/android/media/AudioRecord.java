...
public class AudioRecord implements AudioRouting {
    ...
    public AudioRecord(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int sessionId) throws IllegalArgumentException {
        ...
        if (audioAttributes == null) {
            ...
            int concurrentType = HwAudioServiceManager.getRecordConcurrentType();
            rate = this.mAudioAttributes.mFlags & -1610612737;
            if (concurrentType > 0) {
                rate |= concurrentType;
            }
            int flags = rate;
            this.mAudioAttributes.mFlags = flags;
            ...
            flags = this.mNativeBufferSizeInBytes;
            int[] session = iArr;
            rate = native_setup(weakReference, audioAttributes2, sampleRate, i, i2, i3, flags, iArr, ActivityThread.currentOpPackageName(), 0);
            if (rate != 0) {
                StringBuilder stringBuilder = new StringBuilder();
    ...
    private static int getChannelMaskFromLegacyConfig(int inChannelConfig, boolean allowLegacyConfig) {
        ...
        if (inChannelConfig != 12) {
            if (inChannelConfig != 16) {
                ...
                } else if (Parameters.FLASH_MODE_ON.equalsIgnoreCase(AudioSystem.getParameters("four_channel_input"))) {
                    mask = inChannelConfig;
                } else {
                    throw new IllegalArgumentException("Unsupported channel configuration.");
                }
    ...
    private void audioParamCheck(int audioSource, int sampleRateInHz, int audioFormat) throws IllegalArgumentException {
        ...
        if (audioSource < 0 || !(audioSource <= MediaRecorder.getAudioSourceMax() || audioSource == AudioSource.RADIO_TUNER || audioSource == 1999)) {

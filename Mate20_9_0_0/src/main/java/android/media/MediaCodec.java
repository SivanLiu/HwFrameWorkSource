    ...
    public static final class CryptoInfo {
        ...
        public String toString() {
            ...
            for (int i2 = 0; i2 < this.key.length; i2++) {
                builder.append(hexdigits.charAt((this.key[i2] & 240) >> 4));
                builder.append(hexdigits.charAt(this.key[i2] & 15));
            }
            builder.append("], iv [");
            ...
            if (format != null) {
                ...
                for (Entry<String, Object> entry : formatMap.entrySet()) {
                    if (((String) entry.getKey()).equals(MediaFormat.KEY_AUDIO_SESSION_ID)) {
                        int sessionId = 0;
                        try {
                            int sessionId2 = ((Integer) entry.getValue()).intValue();
                            keys2[i] = "audio-hw-sync";
                            values2[i] = Integer.valueOf(AudioSystem.getAudioHwSyncForSession(sessionId2));
                        } catch (Exception e) {

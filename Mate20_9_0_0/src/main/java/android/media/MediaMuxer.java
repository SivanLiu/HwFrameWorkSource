...
public final class MediaMuxer {
    ...
    public MediaMuxer(String path, int format) throws IOException {
        ...
        if (path != null) {
            ...
            try {
                ...
                try {
                    file = new ExternalStorageRandomAccessFileImpl(path, "rws");
                    Log.i(TAG, "MediaMuxer ExternalStorage SD .");
                } catch (FileNotFoundException NotNormalE) {
                    String str = TAG;
                    StringBuilder stringBuilder = new StringBuilder();
                    stringBuilder.append("MediaMuxer ExternalStorage NotNormalE = ");
    ...
    public void release() {
        ...
        if (this.mIsHeifFormat) {
            HwMediaMonitorManager.writeBigData(HwMediaMonitorUtils.BD_MEDIA_HEIF, 0);
        }
    }
}

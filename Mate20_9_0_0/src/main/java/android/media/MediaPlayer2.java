...
public abstract class MediaPlayer2 extends MediaPlayerBase implements Listener, AudioRouting {
    ...
    public abstract KeyRequest getDrmKeyRequest(byte[] bArr, byte[] bArr2, String str, int i, Map<String, String> map) throws NoDrmSchemeException;
    ...
    private static final String[] decodeMediaPlayer2Uri(String location) {
        ...
        if ("mediaplayer2".equals(uri.getScheme())) {
            ...
            if (keys.size() != values.size()) {
                return new String[]{(String) uris.get(0)};
            }
            List<String> ls = new ArrayList();
            ls.add((String) uris.get(0));
            while (i < keys.size()) {
                ls.add((String) keys.get(i));
                ls.add((String) values.get(i));
                i++;
            }

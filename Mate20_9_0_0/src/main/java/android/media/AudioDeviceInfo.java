...
public final class AudioDeviceInfo {
    ...
    public int[] getChannelCounts() {
        ...
        int i = 0;
        for (int mask2 : getChannelMasks()) {
            int channelCountFromOutChannelMask;
            if (isSink()) {
                channelCountFromOutChannelMask = AudioFormat.channelCountFromOutChannelMask(mask2);
            } else {
                channelCountFromOutChannelMask = AudioFormat.channelCountFromInChannelMask(mask2);
            }
            countSet.add(Integer.valueOf(channelCountFromOutChannelMask));
        }
        int[] channelIndexMasks = getChannelIndexMasks();
        int length = channelIndexMasks.length;
        while (i < length) {
            countSet.add(Integer.valueOf(Integer.bitCount(channelIndexMasks[i])));
            i++;
        }
        channelIndexMasks = new int[countSet.size()];
        length = 0;
        Iterator it = countSet.iterator();
        while (it.hasNext()) {
            mask2 = length + 1;
            channelIndexMasks[length] = ((Integer) it.next()).intValue();
            length = mask2;
        }
    ...
    static {
        ...
        INT_TO_EXT_DEVICE_MAPPING.put(-2013265920, 11);

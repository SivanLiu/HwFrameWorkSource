...
public class ThumbnailUtils {
    ...
    public static Bitmap createImageThumbnail(String filePath, int kind) {
        ...
        if (bitmap == null) {
            ...
            try {
                ...
                stringBuilder.append("Unable to decode file ");
                stringBuilder.append(str);
                stringBuilder.append(". OutOfMemoryError.");
                Log.e(str2, stringBuilder.toString(), oom);
                if (stream != null) {
                    stream.close();
                }
            } catch (Throwable th) {
                ...
                if (stream != null) {
                    try {
                        ...
                        IOException iOException2 = ex222;
                        Log.e(TAG, "", ex222);
                    }
                }
            }
    ...
    private static Bitmap makeBitmap(int minSideLength, int maxNumOfPixels, Uri uri, ContentResolver cr, ParcelFileDescriptor pfd, Options options) {
        ...
        if (pfd == null) {
            try {
                ...
                Log.e(TAG, "Got oom exception ", ex2);
                return null;
            } finally {
                closeSilently(pfd);
            }
        ...
        ex2 = pfd.getFileDescriptor();
        options.inSampleSize = 1;
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFileDescriptor(ex2, null, options);
        if (!(options.mCancel || options.outWidth == -1)) {
            if (options.outHeight != -1) {
                ...
                options.inDither = false;
                options.inPreferredConfig = Config.ARGB_8888;
                Bitmap b2 = BitmapFactory.decodeFileDescriptor(ex2, null, options);
                closeSilently(pfd);
                return b2;
            }
        }
    ...
    private static Bitmap transform(Matrix scaler, Bitmap source, int targetWidth, int targetHeight, int options) {
        ...
        if (scaleUp || (deltaX >= 0 && deltaY >= 0)) {
            ...
            if (scaler2 != null) {
                b1 = Bitmap.createBitmap(bitmap, 0, 0, source.getWidth(), source.getHeight(), scaler2, true);
            } else {
                b1 = bitmap;
            }
    ...
    private static Bitmap createThumbnailFromMetadataRetriever(String filePath, int targetSize, int maxPixels) {
        ...
        try {
            ...
            params.setPreferredConfig(Config.ARGB_8888);

...
public class HWArpPacket {
    ...
    public boolean doArp(int timeoutMillis, Inet4Address requestedAddress, boolean fillSenderIp) {
        ...
        buf.putShort((short) 1);
        buf.put(this.mMyMac);
        byte[] address = (!fillSenderIp || this.mMyAddr == null) ? new byte[4] : this.mMyAddr.getAddress();
        buf.put(address);
        buf.put(new byte[6]);
        buf.put(desiredIp);
        buf.flip();
        ...
        while (SystemClock.elapsedRealtime() < timeout) {
            if (this.mSocket.read(recvBuf, 0, recvBuf.length, -1, (int) (timeout - SystemClock.elapsedRealtime())) >= 28 && recvBuf[0] == (byte) 0 && recvBuf[i] == i) {
                if (recvBuf[2] == (byte) 8 && recvBuf[3] == (byte) 0 && recvBuf[b] == (byte) 6 && recvBuf[5] == b && recvBuf[6] == (byte) 0 && recvBuf[7] == (byte) 2 && recvBuf[14] == desiredIp[0] && recvBuf[15] == desiredIp[1] && recvBuf[16] == desiredIp[2] && recvBuf[17] == desiredIp[3] && !isMyMac(this.mMyMac, recvBuf)) {

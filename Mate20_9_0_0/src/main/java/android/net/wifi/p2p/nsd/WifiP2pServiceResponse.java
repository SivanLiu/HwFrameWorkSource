...
public class WifiP2pServiceResponse implements Parcelable {
        ...
        while (true) {
            ...
            try {
                ...
                if (length == 0) {
                    ...
                    dis2.readFully(data);
                    if (type == 1) {
                        wifiP2pServiceResponse = WifiP2pDnsSdServiceResponse.newInstance(status, transId, dev, data);
                    } else if (type == 2) {
                        wifiP2pServiceResponse = WifiP2pUpnpServiceResponse.newInstance(status, transId, dev, data);
                    } else {
                        wifiP2pServiceResponse = new WifiP2pServiceResponse(type, status, transId, dev, data);
    ...
    public int hashCode() {
        int i = 0;
        int result = 31 * ((31 * ((31 * ((31 * ((31 * 17) + this.mServiceType)) + this.mStatus)) + this.mTransId)) + (this.mDevice.deviceAddress == null ? 0 : this.mDevice.deviceAddress.hashCode()));
        if (this.mData != null) {

...
import android.net.wifi.rtt.IRttCallback.Stub;
...
public class WifiRttManager {
    ...
    public void startRanging(WorkSource workSource, RangingRequest request, final Executor executor, final RangingResultCallback callback) {
        if (executor == null) {
            ...
            try {
                this.mService.startRanging(new Binder(), this.mContext.getOpPackageName(), workSource, request, new Stub() {
                    public void onRangingFailure(int status) throws RemoteException {
                        Binder.clearCallingIdentity();
                        executor.execute(new -$$Lambda$WifiRttManager$1$j3tVizFtxt_z0tTXfTNSFM4Loi8(callback, status));
                    }

                    public void onRangingResults(List<RangingResult> results) throws RemoteException {
                        Binder.clearCallingIdentity();
                        executor.execute(new -$$Lambda$WifiRttManager$1$3uT7vOEOvW11feiFUB6LWvcBJEk(callback, results));
                    }
                });
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();

...
public class ApfGenerator {
    ...
    private class Instruction {
        ...
        void setLabel(String label) throws IllegalInstructionException {
            if (ApfGenerator.this.mLabels.containsKey(label)) {
                ...
                stringBuilder.append("duplicate label ");
                stringBuilder.append(label);
                throw new IllegalInstructionException(stringBuilder.toString());
        ...
        void generate(byte[] bytecode) throws IllegalInstructionException {
            if (this.mOpcode != Opcodes.LABEL.value) {
                ...
                if (writingOffset2 - this.offset != size()) {
                    ...
                    stringBuilder.append(writingOffset2 - this.offset);
                    stringBuilder.append(" but should have written ");
        ...
        private int calculateTargetLabelOffset() throws IllegalInstructionException {
            ...
            if (targetLabelInstruction != null) {
                ...
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append("backward branches disallowed; label: ");
                stringBuilder.append(this.mTargetLabel);
                throw new IllegalInstructionException(stringBuilder.toString());
            }
            StringBuilder stringBuilder2 = new StringBuilder();
            stringBuilder2.append("label not found: ");
            stringBuilder2.append(this.mTargetLabel);
            throw new IllegalInstructionException(stringBuilder2.toString());
        }
    ...
    private void requireApfVersion(int minimumVersion) throws IllegalInstructionException {
        if (this.mVersion < minimumVersion) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Requires APF >= ");
            stringBuilder.append(minimumVersion);
            throw new IllegalInstructionException(stringBuilder.toString());
        }
    }
    ...
    public ApfGenerator addLoadFromMemory(Register register, int slot) throws IllegalInstructionException {
        if (slot < 0 || slot > 15) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("illegal memory slot number: ");
    ...
    private int updateInstructionOffsets() {
        int offset = 0;
        Iterator it = this.mInstructions.iterator();
        while (it.hasNext()) {
    ...
    public byte[] generate() throws IllegalInstructionException {
        ...
        while (true) {
            ...
            it = this.mInstructions.iterator();
            while (it.hasNext()) {
                if (((Instruction) it.next()).shrink()) {
                    shrunk = true;
                }
            }
            if (!shrunk) {
        ...
        it = this.mInstructions.iterator();
        while (it.hasNext()) {
            ((Instruction) it.next()).generate(bytecode);
        }
        return bytecode;
    }
}

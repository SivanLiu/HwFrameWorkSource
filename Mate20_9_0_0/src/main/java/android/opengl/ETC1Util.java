...
public class ETC1Util {
    ...
    public static ETC1Texture createTexture(InputStream input) throws IOException {
        ...
        if (input.read(ioBuffer, 0, 16) == 16) {
            ...
            if (ETC1.isValid(headerBuffer)) {
                ...
                headerBuffer = ByteBuffer.allocateDirect(encodedSize).order(ByteOrder.nativeOrder());
                int i = 0;
                while (i < encodedSize) {
                    int chunkSize = Math.min(ioBuffer.length, encodedSize - i);
                    if (input.read(ioBuffer, 0, chunkSize) == chunkSize) {
                        headerBuffer.put(ioBuffer, 0, chunkSize);
                        i += chunkSize;
                    } else {
                ...
                return new ETC1Texture(width, height, headerBuffer);
            }
    ...
    public static ETC1Texture compressTexture(Buffer input, int width, int height, int pixelSize, int stride) {
        Buffer compressedImage = ByteBuffer.allocateDirect(ETC1.getEncodedDataSize(width, height)).order(ByteOrder.nativeOrder());

...
import android.rms.iaware.AwareConstant.Database.HwPkgRecord;
...
class GLLogWrapper extends GLWrapperBase {
    ...
    private String getTextureEnvTarget(int target) {
        if (target != GLES10.GL_TEXTURE_ENV) {
            return getHex(target);
        }
    ...
    private String getTexturePName(int pname) {
        if (pname == GLES11.GL_GENERATE_MIPMAP) {
            return "GL_GENERATE_MIPMAP";
        }
        if (pname == GLES11Ext.GL_TEXTURE_CROP_RECT_OES) {
    ...
    private void doElement(StringBuilder builder, int ordinal, int vertexIndex) {
        ...
        stringBuilder.append("] =");
        builder.append(stringBuilder.toString());
        doArrayElement(builder, this.mVertexArrayEnabled, BaseMmsColumns.MMS_VERSION, this.mVertexPointer, vertexIndex);

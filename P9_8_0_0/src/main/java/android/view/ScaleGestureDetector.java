...
public class ScaleGestureDetector {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        boolean z2 = this.mAnchoredScaleMode == 2 ? isStylusButtonDown ^ 1 : false;
        if (action == 1 || action == 3) {
            int i2 = 1;
        } else {
            boolean streamComplete = z2;
        }
        if (action == 0 || i2 != 0) {
            if (this.mInProgress) {
                ...
                this.mAnchoredScaleMode = 0;
            } else if (inAnchoredScaleMode() && i2 != 0) {
                this.mInProgress = false;
                this.mInitialSpan = 0.0f;
                this.mAnchoredScaleMode = 0;
            }
        ...
        if (!(this.mInProgress || !this.mStylusScaleEnabled || (inAnchoredScaleMode() ^ 1) == 0 || (i2 ^ 1) == 0 || !isStylusButtonDown)) {
        ...
        this.mFocusX = focusX;
        this.mFocusY = focusY;
        if (!inAnchoredScaleMode() && this.mInProgress && (span < ((float) this.mMinSpan) || z)) {
            this.mListener.onScaleEnd(this);
            this.mInProgress = false;

    ...
    private static PropertyValuesHolder getPVH(TypedArray styledAttributes, int valueType, int valueFromId, int valueToId, String propertyName) {
        ...
        if (getFloats) {
            ...
            if (hasFrom) {
                ...
                if (hasTo) {
                    if (toType == 5) {
                        valueTo = styledAttributes.getDimension(valueToId, TonemapCurve.LEVEL_BLACK);
                    } else {
                        valueTo = styledAttributes.getFloat(valueToId, TonemapCurve.LEVEL_BLACK);
                    }
    ...
    private static void setupValues(ValueAnimator anim, TypedArray arrayAnimator, boolean getFloats, boolean hasFrom, int fromType, boolean hasTo, int toType) {
        if (getFloats) {
            ...
            if (hasFrom) {
                ...
                if (hasTo) {
                    if (toType == 5) {
                        valueTo = arrayAnimator.getDimension(6, TonemapCurve.LEVEL_BLACK);
                    } else {
                        valueTo = arrayAnimator.getFloat(6, TonemapCurve.LEVEL_BLACK);
                    }

        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            ...
            switch (code) {
                ...
                case 3:
                    ...
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {
                        volumeInfo = (VolumeInfo) VolumeInfo.CREATOR.createFromParcel(data);
                    } else {
                        volumeInfo = null;
                    }
                    onVolumeStateChanged(volumeInfo, data.readInt(), data.readInt());
                    return true;
                case 4:
                    VolumeRecord volumeRecord;
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {
                        volumeRecord = (VolumeRecord) VolumeRecord.CREATOR.createFromParcel(data);
                    } else {
                        volumeRecord = null;
                    }
                ...
                case 6:
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {
                        diskInfo = (DiskInfo) DiskInfo.CREATOR.createFromParcel(data);
                    } else {
                        diskInfo = null;
                    }
                    onDiskScanned(diskInfo, data.readInt());
                    return true;
                case 7:
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {

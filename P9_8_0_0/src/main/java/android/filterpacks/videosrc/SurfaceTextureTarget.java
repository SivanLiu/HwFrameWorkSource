...
public class SurfaceTextureTarget extends Filter {
    ...
    private Quad mSourceQuad = new Quad(new Point(TonemapCurve.LEVEL_BLACK, 1.0f), new Point(1.0f, 1.0f), new Point(TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK), new Point(1.0f, TonemapCurve.LEVEL_BLACK));
    private int mSurfaceId;
    @GenerateFinalPort(name = "surfaceTexture")
    private SurfaceTexture mSurfaceTexture;
    @GenerateFieldPort(hasDefault = true, name = "targetQuad")
    private Quad mTargetQuad = new Quad(new Point(TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK), new Point(1.0f, TonemapCurve.LEVEL_BLACK), new Point(TonemapCurve.LEVEL_BLACK, 1.0f), new Point(1.0f, 1.0f));
        ...
        if (this.mScreenWidth > 0 && this.mScreenHeight > 0 && this.mProgram != null) {
            ...
            if (relativeAspectRatio != 1.0f || this.mRenderMode == 3) {
                switch (this.mRenderMode) {
                    case 0:
                        this.mTargetQuad.p0.set(TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK);
                        this.mTargetQuad.p1.set(1.0f, TonemapCurve.LEVEL_BLACK);
                        this.mTargetQuad.p2.set(TonemapCurve.LEVEL_BLACK, 1.0f);
                        this.mTargetQuad.p3.set(1.0f, 1.0f);
                        this.mProgram.setClearsOutput(false);
                        break;
                    case 1:
                        if (relativeAspectRatio > 1.0f) {
                            this.mTargetQuad.p0.set(0.5f - (0.5f / relativeAspectRatio), TonemapCurve.LEVEL_BLACK);
                            this.mTargetQuad.p1.set((0.5f / relativeAspectRatio) + 0.5f, TonemapCurve.LEVEL_BLACK);
                            this.mTargetQuad.p2.set(0.5f - (0.5f / relativeAspectRatio), 1.0f);
                            this.mTargetQuad.p3.set((0.5f / relativeAspectRatio) + 0.5f, 1.0f);
                        } else {
                            this.mTargetQuad.p0.set(TonemapCurve.LEVEL_BLACK, 0.5f - (0.5f * relativeAspectRatio));
                            this.mTargetQuad.p1.set(1.0f, 0.5f - (0.5f * relativeAspectRatio));
                            this.mTargetQuad.p2.set(TonemapCurve.LEVEL_BLACK, (0.5f * relativeAspectRatio) + 0.5f);

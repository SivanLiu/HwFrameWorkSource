...
public class BackDropperFilter extends Filter {
    ...
    private static final float[] DEFAULT_BG_FIT_TRANSFORM = new float[]{1.0f, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, 1.0f, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, 1.0f};
    ...
    private float mAdaptRateBg = TonemapCurve.LEVEL_BLACK;
    @GenerateFieldPort(hasDefault = true, name = "adaptRateFg")
    private float mAdaptRateFg = TonemapCurve.LEVEL_BLACK;
    ...
    private float mWhiteBalanceBlueChange = TonemapCurve.LEVEL_BLACK;
    @GenerateFieldPort(hasDefault = true, name = "whitebalanceredChange")
    private float mWhiteBalanceRedChange = TonemapCurve.LEVEL_BLACK;
    ...
    public void setupPorts() {
        ...
        for (String outputName : mOutputNames) {
            addOutputBasedOnInput(outputName, MetricsConstants.MODE_VIDEO);
        }
    ...
    private void allocateFrames(FrameFormat inputFormat, FilterContext context) {
        if (createMemoryFormat(inputFormat)) {
            ...
            for (i = 0; i < numBytes; i++) {
                initialBgMean[i] = BluetoothInputHost.SUBCLASS1_MOUSE;
    ...
    public void process(FilterContext context) {
        ...
        if (this.mFrameCount <= this.mLearningDuration) {
            ...
            if (this.mFrameCount == this.mLearningDuration) {
                ...
                if (this.mLogVerbose) {
                    Log.v(TAG, String.format("Mask_average is %d, threshold is %d", new Object[]{Integer.valueOf(bi), Integer.valueOf(20)}));
                }
                if (bi >= 20) {
                    this.mStartLearning = true;
                } else {
        ...
        if (this.mFrameCount >= this.mLearningDuration - this.mLearningVerifyDuration && ((double) this.mAdaptRateBg) <= 0.0d) {
            ...
            if (this.mLogVerbose && this.mFrameCount % 30 == 0) {
                ...
                context.getGLEnvironment().activate();
                GLES20.glFinish();
                Log.v(TAG, "Avg. frame duration: " + String.format("%.2f", new Object[]{Double.valueOf(((double) (endTime - this.startTime)) / 30.0d)}) + " ms. Avg. fps: " + String.format("%.2f", new Object[]{Double.valueOf(1000.0d / (((double) (SystemClock.elapsedRealtime() - this.startTime)) / 30.0d))}));
    ...
    private void updateBgScaling(Frame video, Frame background, boolean fitModeChanged) {
        ...
        if (currentRelativeAspect != this.mRelativeAspect || fitModeChanged) {
            ...
            float xMin = TonemapCurve.LEVEL_BLACK;
            float xWidth = 1.0f;
            float yMin = TonemapCurve.LEVEL_BLACK;
            float yWidth = 1.0f;
            ...
            this.mBgSubtractProgram.setHostValue("bg_fit_transform", new float[]{xWidth, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, TonemapCurve.LEVEL_BLACK, yWidth, TonemapCurve.LEVEL_BLACK, xMin, yMin, 1.0f});
        }
    }

...
class Cea708CCWidget extends ClosedCaptionWidget implements DisplayListener {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            while (i < count) {
                ...
                if (params instanceof ScaledLayoutParams) {
                    ...
                    if (scaleStartRow < TonemapCurve.LEVEL_BLACK || scaleStartRow > 1.0f) {
                        throw new RuntimeException("A child of ScaledLayout should have a range of scaleStartRow between 0 and 1");
                    } else if (scaleEndRow < scaleStartRow || scaleStartRow > 1.0f) {
                        throw new RuntimeException("A child of ScaledLayout should have a range of scaleEndRow between scaleStartRow and 1");
                    } else if (scaleEndCol < TonemapCurve.LEVEL_BLACK || scaleEndCol > 1.0f) {
    ...
    static class CCWindowLayout extends RelativeLayout implements OnLayoutChangeListener {
        ...
        public void initWindow(CCLayout ccLayout, CaptionWindow captionWindow) {
            ...
            if (scaleRow < TonemapCurve.LEVEL_BLACK || scaleRow > 1.0f) {
                Log.i(TAG, "The vertical position of the anchor point should be at the range of 0 and 1 but " + scaleRow);
                scaleRow = Math.max(TonemapCurve.LEVEL_BLACK, Math.min(scaleRow, 1.0f));
            }
            if (scaleCol < TonemapCurve.LEVEL_BLACK || scaleCol > 1.0f) {
                Log.i(TAG, "The horizontal position of the anchor point should be at the range of 0 and 1 but " + scaleCol);
                scaleCol = Math.max(TonemapCurve.LEVEL_BLACK, Math.min(scaleCol, 1.0f));
            }
            int gravity = 17;
            int horizontalMode = captionWindow.anchorId % 3;
            int verticalMode = captionWindow.anchorId / 3;
            float scaleStartRow = TonemapCurve.LEVEL_BLACK;
            float scaleEndRow = 1.0f;
            float scaleStartCol = TonemapCurve.LEVEL_BLACK;
            ...
            switch (horizontalMode) {
                ...
                case 1:
                    ...
                    float halfMaxWidthScale = this.mCCLayout.getWidth() > 0 ? (paint.measureText(widestTextBuilder.toString()) / 2.0f) / (((float) this.mCCLayout.getWidth()) * 0.8f) : TonemapCurve.LEVEL_BLACK;
                    if (halfMaxWidthScale > TonemapCurve.LEVEL_BLACK && halfMaxWidthScale < scaleCol) {
                        gravity = 3;
                        this.mCCView.setAlignment(Alignment.ALIGN_NORMAL);
        ...
        private void updateWidestChar() {
            ...
            float widestCharWidth = TonemapCurve.LEVEL_BLACK;
            for (int i = 0; i < 256; i++) {
        ...
        private void updateTextSize() {
            if (this.mCCLayout != null) {
                ...
                Paint paint = new Paint();
                paint.setTypeface(this.mCaptionStyle.getTypeface());
                float startFontSize = TonemapCurve.LEVEL_BLACK;

...
public class SQLiteAuditLog {
    ...
    private void getRecordsFromProvider(String dataBaseName) {
        ...
        if (AppName_array.length == dataBaseName_array.length && AppName_array.length == timeAudit_array.length && AppName_array.length == timeAudit_timeThresholdSQL_array.length && AppName_array.length == timeAudit_recordSQL_array.length && AppName_array.length == timeAudit_recordExecPlan_array.length && AppName_array.length == timeAudit_turnOnDetailedStatistics_array.length && AppName_array.length == statInfoAudit_array.length && AppName_array.length == groupOrderAudit_array.length && AppName_array.length == groupOrderAudit_timeThresholdSQL_array.length && AppName_array.length == groupOrderAudit_recordSQL_array.length && AppName_array.length == transLockAudit_array.length && AppName_array.length == transLockAudit_timeThresholdTransactionLock_array.length && AppName_array.length == transLockAudit_recordSQL_array.length && AppName_array.length == convLinkAudit_array.length && AppName_array.length == memManageAudit_array.length) {
            ...
            for (int k = 0; k < AppName_array.length; k++) {
                ...
                record.transLockAudit = transLockAudit_array[k];
                ...
                record.convLinkAudit = convLinkAudit_array[k];
                record.memManageAudit = memManageAudit_array[k];
                if ("ALL".equals(dataBaseName_array[k])) {
                    this.hashMap.put(AppName_array[k] + dataBaseName_array[k], record);
                } else {
                    this.hashMap.put(AppName_array[k] + dataBaseName, record);
                }
            }
        }
    }
    ...
    private Record getRecord(String AppName) {
        Record record;
        synchronized (this.hashMap) {
            record = (Record) this.hashMap.get(AppName + this.mMainDbFile);
            if (record == null) {
                record = (Record) this.hashMap.get("ALL" + this.mMainDbFile);
            }
            if (record == null) {
                record = (Record) this.hashMap.get(AppName + "ALL");
            }
            if (record == null) {
    ...
    private static String getProcessName(int pid) {
        ...
        if (r0 != null) {
            for (RunningAppProcessInfo running : r0) {
                if (running.pid == pid) {

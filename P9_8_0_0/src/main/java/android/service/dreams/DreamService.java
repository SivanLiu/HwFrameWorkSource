...
public class DreamService extends Service implements Callback {
    ...
    private HwCustDreamService mCust = ((HwCustDreamService) HwCustUtils.createObj(HwCustDreamService.class, this));
...
*/
            ...
            if (!this.mWindowless || (this.mCanDoze ^ 1) == 0) {
                if (!this.mWindowless) {
                    ...
                    if (this.mCust == null || !this.mCust.isChargingAlbumEnabled()) {
                        ...
                        int i3 = lp.flags;
                        int i4 = 4784385 | (this.mFullscreen ? 1024 : 0);
                        if (this.mScreenBright) {
                            i2 = 128;
                        } else {
                            i2 = 0;
                    ...
                    if (!(this.mCust == null || (this.mCust.isChargingAlbumEnabled() ^ 1) == 0)) {
                        if (this.mLowProfile) {
                            i = 1;
                        }
                        applySystemUiVisibilityFlags(i, 1);
    ...
    private void applySystemUiVisibilityFlags(int flags, int mask) {
        ...
        if (decorView != null) {
            decorView.setSystemUiVisibility(applyFlags(decorView.getSystemUiVisibility(), flags, mask));
        }
    }

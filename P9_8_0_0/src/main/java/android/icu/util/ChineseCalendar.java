...
public class ChineseCalendar extends Calendar {
    ...
    private void computeChineseFields(int days, int gyear, int gmonth, boolean setAllFields) {
        ...
        if (this.isLeapYear && hasNoMajorSolarTerm(thisMoon)) {
            isLeapMonthBetween = isLeapMonthBetween(firstMoon, newMoonNear(thisMoon - 25, false)) ^ 1;
        } else {
            isLeapMonthBetween = 0;
        }
        internalSet(2, month - 1);
        internalSet(22, isLeapMonthBetween != 0 ? 1 : 0);
        if (setAllFields) {

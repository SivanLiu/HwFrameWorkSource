...
public final class ULocale implements Serializable, Comparable<ULocale> {
        ...
        private static ULocale toULocale7(Locale loc) {
            ...
            try {
                        ...
                        try {
                            if (extKey.charValue() == 'u') {
                                ...
                                if (extVal != null) {
                                    ...
                                    try {
                                        map2.put(String.valueOf(extKey), extVal);
                                        attributes2 = attributes;
                ...
                if (r8 != null) {
                    ...
                    map.put(ULocale.LOCALE_ATTRIBUTE_KEY, attrBuf.toString());
                }
                if (map != null) {
                    buf.append('@');
                    boolean addSep = false;
                    for (Entry<String, String> kwEntry : map.entrySet()) {
    ...
    private static String getFallbackString(String fallback) {
        ...
        if (last == -1) {
            ...
            while (last > 0) {
                if (fallback.charAt(last - 1) != UNDERSCORE) {
                    break;
                }
                last--;
            }
        ...
        for (ULocale aLocale : acceptLanguageList) {
            ...
            do {
                ...
                while (j < availableLocales.length) {
                    if (availableLocales[j].equals(uLocale)) {
                        if (zArr != null) {
                            zArr[0] = false;
                        }
                        return availableLocales[j];
                    } else if (uLocale.getScript().length() == 0 && availableLocales[j].getScript().length() > 0 && availableLocales[j].getLanguage().equals(uLocale.getLanguage()) && availableLocales[j].getCountry().equals(uLocale.getCountry()) && availableLocales[j].getVariant().equals(uLocale.getVariant()) && minimizeSubtags(availableLocales[j]).getScript().length() == 0) {
                        if (zArr != null) {
                            zArr[0] = false;
                        }
                        return uLocale;
                ...
                Locale parent = LocaleUtility.fallback(uLocale.toLocale());
                if (parent != null) {
                    uLocale = new ULocale(parent);
                } else {
                    uLocale = null;
                }
                zArr = null;

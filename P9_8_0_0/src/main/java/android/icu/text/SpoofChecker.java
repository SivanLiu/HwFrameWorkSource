...
public class SpoofChecker {
    ...
    public static class Builder {
        ...
        private static class ConfusabledataBuilder {
            static final /* synthetic */ boolean -assertionsDisabled = (ConfusabledataBuilder.class.desiredAssertionStatus() ^ 1);
            ...
            void build(Reader confusables, SpoofData dest) throws ParseException, IOException {
                ...
                while (matcher.find()) {
                    ...
                    if (matcher.start(7) < 0) {
                        ...
                        if (-assertionsDisabled || mapString.length() >= 1) {
                            ...
                            this.fTable.put(Integer.valueOf(keyChar), smapString);
                ...
                for (String keyCharStr : this.fKeySet) {
                    ...
                    SPUString targetMapping = (SPUString) this.fTable.get(Integer.valueOf(keyChar));
                    if (!-assertionsDisabled && targetMapping == null) {
                        throw new AssertionError();
                    } else if (targetMapping.fStr.length() > 256) {
                ...
                i = 0;
                while (i < numKeys) {
                    key = ((Integer) this.fKeyVec.get(i)).intValue();
                    int codePoint = ConfusableDataUtils.keyToCodePoint(key);
                    if (-assertionsDisabled || codePoint > previousCodePoint) {
                        dest.fCFUKeys[i] = key;
    ...
    private static final class ConfusableDataUtils {
        static final /* synthetic */ boolean -assertionsDisabled = (ConfusableDataUtils.class.desiredAssertionStatus() ^ 1);
        ...
        public static final int codePointAndLengthToKey(int codePoint, int length) {
            if (!-assertionsDisabled && (16777215 & codePoint) != codePoint) {
                throw new AssertionError();
            } else if (-assertionsDisabled || length <= 256) {
    ...
    private static class SpoofData {
        ...
        private static final class IsAcceptable implements Authenticate {
            ...
            public boolean isDataVersionAcceptable(byte[] version) {
                if (version[0] != (byte) 2 && version[1] == (byte) 0 && version[2] == (byte) 0 && version[3] == (byte) 0) {
                    return false;
                }
                return true;
            }
        ...
        public boolean equals(Object other) {
            ...
            if (Utility.sameObjects(this.fCFUStrings, otherData.fCFUStrings) || this.fCFUStrings == null || (this.fCFUStrings.equals(otherData.fCFUStrings) ^ 1) == 0) {
                return true;
            }
            return false;
        }
    ...
    public int areConfusable(String s1, String s2) {
        if ((this.fChecks & 7) == 0) {
            ...
            if (s1RSS.intersects(s2RSS)) {
                ...
                result = 2;
                if (!(s1RSS.isEmpty() || (s2RSS.isEmpty() ^ 1) == 0)) {
                    result = 2 | 4;
                }
            }
    ...
    public boolean equals(Object other) {
        ...
        if ((this.fSpoofData != otherSC.fSpoofData && this.fSpoofData != null && (this.fSpoofData.equals(otherSC.fSpoofData) ^ 1) != 0) || this.fChecks != otherSC.fChecks) {
            return false;
        }
        if (this.fAllowedLocales != otherSC.fAllowedLocales && this.fAllowedLocales != null && (this.fAllowedLocales.equals(otherSC.fAllowedLocales) ^ 1) != 0) {
            return false;
        }
        if ((this.fAllowedCharsSet == otherSC.fAllowedCharsSet || this.fAllowedCharsSet == null || (this.fAllowedCharsSet.equals(otherSC.fAllowedCharsSet) ^ 1) == 0) && this.fRestrictionLevel == otherSC.fRestrictionLevel) {
            return true;
        }
        return false;
    }
    ...
    private void getResolvedScriptSet(CharSequence input, ScriptSet result) {
        getResolvedScriptSetWithout(input, 175, result);
    }
    ...
    private void getResolvedScriptSetWithout(CharSequence input, int script, ScriptSet result) {
        ...
        while (utf16Offset < input.length()) {
            ...
            if (script == 175 || (temp.get(script) ^ 1) != 0) {
                result.and(temp);
            }
        }
    }
    ...
    private RestrictionLevel getRestrictionLevel(String input) {
        ...
        if (resolvedNoLatn.isEmpty() || (resolvedNoLatn.get(8) ^ 1) == 0 || (resolvedNoLatn.get(14) ^ 1) == 0 || (resolvedNoLatn.get(6) ^ 1) == 0) {

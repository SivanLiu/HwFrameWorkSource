...
public class DecimalFormat extends NumberFormat {
    ...
    private void subformatExponential(StringBuffer result, FieldPosition fieldPosition, boolean parseAttr) {
        ...
        while (i2 < totalDigits) {
            ...
            int digitValue = i2 < this.digitList.count ? this.digitList.getDigitValue(i2) : 0;
            result.append(digits[digitValue]);
            if (recordFractionDigits) {
                fractionalDigitsCount++;
                fractionalDigits = (fractionalDigits * 10) + ((long) digitValue);
            }
            i2++;
        }
        ...
        if (text.regionMatches(position, this.symbols.getInfinity(), 0, this.symbols.getInfinity().length())) {
            ...
            while (position < text.length()) {
                ...
                if (matchLen <= 0) {
                    ...
                    if (backup != -1) {
                        if (strictParse && ((lastGroup != -1 && groupedDigitCount != gs2) || (lastGroup == -1 && groupedDigitCount > gs2))) {
                            strictFail = true;
                            break;
                        }
                        lastGroup = backup;
    ...
    private String toPattern(boolean localized) {
        ...
        while (part < 2) {
            ...
            if (!(stringBuffer == null || (this.useExponentialNotation ^ 1) == 0)) {
                int length;
    ...
    private void applyPatternWithoutExpandAffix(String pattern, boolean localized) {
        ...
        while (part < 2 && pos < pattern.length()) {
                                ...
                                if (ch != c3) {
                                    if (ch != c4) {
                                        if (pattern.regionMatches(pos, exponent, 0, exponent.length())) {
                                            ...
                                            if ((digitLeftCount + zeroDigitCount >= 1 || sigDigitCount + digitRightCount >= 1) && (sigDigitCount <= 0 || digitLeftCount <= 0)) {
                                                if (expDigits < (byte) 1) {
                                                }
                                            }
                    ...
                    case 2:
                        if (ch != c7 && ch != c3 && ch != c4 && ((ch < c || ch > nineDigit) && ch != c2)) {
                            if (ch != '¤') {
                                ...
                                boolean doubled = pos + 1 < pattern.length() ? pattern.charAt(pos + 1) == '¤' : false;
                                if (doubled) {
                                    pos++;
                                    affix.append(ch);
                                    if (pos + 1 >= pattern.length() || pattern.charAt(pos + 1) != '¤') {
            ...
            if ((decimalPos >= 0 || digitRightCount <= 0 || sigDigitCount != 0) && ((decimalPos < 0 || (sigDigitCount <= 0 && decimalPos >= digitLeftCount && decimalPos <= digitLeftCount + zeroDigitCount)) && groupingCount != (byte) 0 && groupingCount2 != (byte) 0 && (sigDigitCount <= 0 || zeroDigitCount <= 0))) {
                if (subpart > 2) {
                }
                if (padPos >= 0) {
                    if (padPos == start) {

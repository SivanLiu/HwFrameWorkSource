...
public class LocaleValidityChecker {
    ...
    private boolean isValidU(ULocale locale, Datatype datatype, String extensionString, Where where) {
        ...
        StringBuilder stringBuilder = datatype == Datatype.t ? new StringBuilder() : null;
        for (String subtag : SEPARATOR.split(extensionString)) {
            String subtag2;
            if (subtag2.length() == 2 && (stringBuilder == null || subtag2.charAt(1) <= '9')) {
                if (stringBuilder != null) {
                    if (stringBuilder.length() != 0 && (isValidLocale(stringBuilder.toString(), where) ^ 1) != 0) {
                        return false;
                    }
                    stringBuilder = null;
                }
                key = KeyTypeData.toBcpKey(subtag2);
                if (key == null) {
                    return where.set(datatype, subtag2);
                }
                ...
                stringBuilder.append(subtag2);
            } else {
                typeCount++;
                switch (-getandroid-icu-impl-locale-KeyTypeData$ValueTypeSwitchesValues()[valueType.ordinal()]) {
                    case 1:
                        if (typeCount != 1) {
                            prefix.append('-').append(subtag2);
                            subtag2 = prefix.toString();
                            break;
                        }
                        prefix.setLength(0);
                        prefix.append(subtag2);
                        break;
                    case 2:
                        if (typeCount == 1) {
                            seen.clear();
                    ...
                    case 3:
                        if (typeCount > 1) {
                            return where.set(datatype, key + LanguageTag.SEP + subtag2);
                        }
                        break;
                }
                switch (-getandroid-icu-impl-locale-LocaleValidityChecker$SpecialCaseSwitchesValues()[specialCase.ordinal()]) {
                    ...
                    case 2:
                        try {
                            if (Integer.parseInt(subtag2, 16) <= 1114111) {
                                break;
                            }
                            return where.set(datatype, key + LanguageTag.SEP + subtag2);
                    ...
                    case 3:
                        String str;
                        if (subtag2.equals(DateFormat.SPECIFIC_TZ)) {
                            str = "others";
                        } else {
                            str = subtag2;
                        }
                        if (seen.add(str) && (isScriptReorder(subtag2) ^ 1) == 0) {
                            break;
                        }
                        return where.set(datatype, key + LanguageTag.SEP + subtag2);
                        break;
                    case 4:
                        if (subtag2.length() >= 6 && (subtag2.endsWith(DateFormat.SPECIFIC_TZ) ^ 1) == 0) {
                            if (isValid(Datatype.region, subtag2.substring(0, subtag2.length() - 4), where)) {
                                break;
                            }
                            return false;
                        }
                        ...
                        break;
                    case 5:
                        if (isSubdivision(locale, subtag2)) {
                            break;
                        }
                        return where.set(datatype, key + LanguageTag.SEP + subtag2);
                    default:
                        if (KeyTypeData.toBcpType(key, subtag2, new Output(), new Output()) == null) {
                            return where.set(datatype, key + LanguageTag.SEP + subtag2);
                        }
                        if (!this.allowsDeprecated && KeyTypeData.isDeprecated(key, subtag2)) {
                            return where.set(datatype, key + LanguageTag.SEP + subtag2);
                        }
                }
            }
        }
        if (stringBuilder == null || stringBuilder.length() == 0 || (isValidLocale(stringBuilder.toString(), where) ^ 1) == 0) {
            return true;
        }
        return false;
    }

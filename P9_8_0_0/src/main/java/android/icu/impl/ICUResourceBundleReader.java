...
*/
    ...
    private boolean isNoInheritanceMarker(int res) {
        ...
        if (offset != 0) {
            if (res == offset) {
                offset = getResourceByteOffset(offset);
                if (getInt(offset) == 3 && this.bytes.getChar(offset + 4) == '∅' && this.bytes.getChar(offset + 6) == '∅' && this.bytes.getChar(offset + 8) == '∅') {
                    z = true;
                }
                return z;
    ...
    private boolean isStringV2NoInheritanceMarker(int offset) {
        ...
        int first = this.b16BitUnits.charAt(offset);
        if (first == 8709) {
            if (this.b16BitUnits.charAt(offset + 1) != '∅' || this.b16BitUnits.charAt(offset + 2) != '∅') {
                z = false;
            } else if (this.b16BitUnits.charAt(offset + 3) != '\u0000') {
                z = false;
            }
            return z;
        } else if (first != 56323) {

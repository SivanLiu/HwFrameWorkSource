...
public class AndroidTestCase extends TestCase {
    ...
    public void assertActivityRequiresPermission(String packageName, String className, String permission) {
        ...
        try {
            ...
            Assert.fail("expected security exception for " + permission);
        } catch (SecurityException expected) {
            Assert.assertNotNull("security exception's error message.", expected.getMessage());
            Assert.assertTrue("error message should contain " + permission + ".", expected.getMessage().contains(permission));
        }
    }
    ...
    public void assertReadingContentUriRequiresPermission(Uri uri, String permission) {
        try {
            ...
            Assert.fail("expected SecurityException requiring " + permission);
        } catch (SecurityException expected) {
            Assert.assertNotNull("security exception's error message.", expected.getMessage());
            Assert.assertTrue("error message should contain " + permission + ".", expected.getMessage().contains(permission));
        }
    ...
    public void assertWritingContentUriRequiresPermission(Uri uri, String permission) {
        try {
            ...
            Assert.assertNotNull("security exception's error message.", expected.getMessage());
            Assert.assertTrue("error message should contain \"" + permission + "\". Got: \"" + expected.getMessage() + "\".", expected.getMessage().contains(permission));
        }
    }

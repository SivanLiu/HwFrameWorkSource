...
public class ApplicationPackageManager extends PackageManager {
    public static final int[] CORP_BADGE_LABEL_RES_ID = new int[]{17040332, 17040333, 17040334};
    ...
    public boolean isPermissionReviewModeEnabled() {
        return this.mContext.getResources().getBoolean(17956987);
    }
    ...
    public List<ResolveInfo> queryIntentActivitiesAsUser(Intent intent, int flags, int userId) {
        try {
            ...
            if (!intent.hasCategory(Intent.CATEGORY_LAUNCHER) || (HwSystemManager.allowOp(33554432) ^ 1) == 0) {
                return list;
            }
    ...
    public Drawable getUserBadgedIcon(Drawable icon, UserHandle user) {
        ...
        if (isManagedProfile(user.getIdentifier())) {
            return getBadgedDrawable(icon, new LauncherIcons(this.mContext).getBadgeDrawable(17302335, getUserBadgeColor(user)), null, false);
        }
        return icon;
    }
    ...
    public Drawable getUserBadgeForDensity(UserHandle user, int density) {
        Drawable badgeColor = getManagedProfileIconForDensity(user, 17302331, density);
        if (badgeColor == null) {
            return null;
        }
        badgeColor.setTint(getUserBadgeColor(user));
        Drawable badgeForeground = getDrawableForDensity(17302330, density);
    ...
    public Drawable getUserBadgeForDensityNoBackground(UserHandle user, int density) {
        ...
        Drawable badge = getManagedProfileIconForDensity(user, 17302332, density);
        if (badge != null) {
    ...
    private boolean isPackageCandidateVolume(ContextImpl context, ApplicationInfo app, VolumeInfo vol, IPackageManager pm) {
        ...
        if (VolumeInfo.ID_PRIVATE_INTERNAL.equals(vol.getId())) {
            ...
            return z;
        } else if (app.isSystemApp() || (mHwCustApplicationPackageManager != null && (mHwCustApplicationPackageManager.isSdMoveabled(app) ^ 1) != 0)) {
            return false;
        } else {
    ...
    private Drawable getBadgedDrawable(Drawable drawable, Drawable badgeDrawable, Rect badgeLocation, boolean tryBadgeInPlace) {
        ...
        if (tryBadgeInPlace && (drawable instanceof BitmapDrawable)) {
            isMutable = ((BitmapDrawable) drawable).getBitmap().isMutable();
        } else {
            isMutable = false;
        }
        if (isMutable) {

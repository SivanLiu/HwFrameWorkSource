...
class FileBackupHelperBase {
    ...
    static void performBackup_checked(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState, String[] files, String[] keys) {
        if (files.length != 0) {
            ...
            FileDescriptor fileDescriptor = oldState != null ? oldState.getFileDescriptor() : null;
            FileDescriptor newStateFd = newState.getFileDescriptor();
            if (newStateFd == null) {
                throw new NullPointerException();
            }
            int err = performBackup_native(fileDescriptor, data.mBackupWriter, newStateFd, files, keys);
            if (err != 0) {

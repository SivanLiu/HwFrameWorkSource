...
public class SearchView extends LinearLayout implements CollapsibleActionView {
    ...
    public static class SearchAutoComplete extends AutoCompleteTextView {
        ...
        /* synthetic */ void lambda$-android_widget_SearchView$SearchAutoComplete_74835() {
            showSoftInputIfNecessary();
        }

        public SearchAutoComplete(Context context) {
            super(context);
            this.mRunShowSoftInputIfNecessary = new -$Lambda$yUT_tnnGmSVjI_-JmJWApmrQubw(this);
            this.mThreshold = getThreshold();
        }

        public SearchAutoComplete(Context context, AttributeSet attrs) {
            super(context, attrs);
            this.mRunShowSoftInputIfNecessary = new android.widget.-$Lambda$yUT_tnnGmSVjI_-JmJWApmrQubw.AnonymousClass1(this);
            this.mThreshold = getThreshold();
        }

        public SearchAutoComplete(Context context, AttributeSet attrs, int defStyleAttrs) {
            super(context, attrs, defStyleAttrs);
            this.mRunShowSoftInputIfNecessary = new android.widget.-$Lambda$yUT_tnnGmSVjI_-JmJWApmrQubw.AnonymousClass2(this);
            this.mThreshold = getThreshold();
        }

        public SearchAutoComplete(Context context, AttributeSet attrs, int defStyleAttrs, int defStyleRes) {
            super(context, attrs, defStyleAttrs, defStyleRes);
            this.mRunShowSoftInputIfNecessary = new android.widget.-$Lambda$yUT_tnnGmSVjI_-JmJWApmrQubw.AnonymousClass3(this);
            this.mThreshold = getThreshold();
        }
        ...
        public boolean enoughToFilter() {
            if (this.mSearchView.isInActionMode() && this.mSearchView.isSubmitButtonEnabled()) {
                return false;
            }
            ...
            if (keyCode == 4) {
                ...
                if (event.getAction() == 0 && event.getRepeatCount() == 0) {
                    ...
                    if (event.isTracking() && (event.isCanceled() ^ 1) != 0) {
                        if (this.mSearchView.isInActionMode()) {
                            return false;
                        }
        ...
        private int getSearchViewTextMinWidthDp() {
            ...
            if (width >= 960 && height >= MetricsEvent.ACTION_PERMISSION_DENIED_RECEIVE_WAP_PUSH && orientation == 2) {
                return 256;
            }
    ...
    private static class UpdatableTouchDelegate extends TouchDelegate {
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            if (!hit || (this.mActualBounds.contains(x, y) ^ 1) == 0) {
    ...
    public SearchView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        inflateSearchView(context, attrs, a);
    ...
    public void setQuery(CharSequence query, boolean submit) {
        ...
        if ((!isEmuiStyle() || (isInActionMode() ^ 1) != 0) && submit && (TextUtils.isEmpty(query) ^ 1) != 0) {
            onSubmitQuery();
        }
    }
    ...
    private int getPreferredHeight() {
        if (HwWidgetFactory.isHwTheme(getContext())) {
            return getContext().getResources().getDimensionPixelSize(34471937);
        }
    ...
    private boolean isSubmitAreaEnabled() {
        return (this.mSubmitButtonEnabled || this.mVoiceButtonEnabled) ? isIconified() ^ 1 : false;
    }

    protected void updateSubmitButton(boolean hasText) {
        int visibility = 8;
        if (this.mSubmitButtonEnabled && isSubmitAreaEnabled() && hasFocus() && (hasText || (this.mVoiceButtonEnabled ^ 1) != 0)) {
            visibility = 0;
        }
    ...
    private void updateCloseButton() {
        boolean hasText = TextUtils.isEmpty(this.mSearchSrcTextView.getText()) ^ 1;
        int i = !hasText ? this.mIconifiedByDefault ? this.mExpandedInActionView ^ 1 : 0 : 1;
        this.mCloseButton.setVisibility(i != 0 ? 0 : 8);
    ...
    private void updateVoiceButton(boolean empty) {
        int visibility = 8;
        if (this.mVoiceButtonEnabled && (isIconified() ^ 1) != 0 && empty) {
            visibility = 0;
            if (!isEmuiStyle()) {
    ...
    private void onTextChanged(CharSequence newText) {
        ...
        updateCloseButton();
        updateSubmitArea();
        if (!(this.mOnQueryChangeListener == null || (TextUtils.equals(newText, this.mOldQueryText) ^ 1) == 0)) {
    ...
    private void onSubmitQuery() {
        ...
        if (query != null && TextUtils.getTrimmedLength(query) > 0) {
            if (this.mOnQueryChangeListener == null || (this.mOnQueryChangeListener.onQueryTextSubmit(query.toString()) ^ 1) != 0) {
                if (this.mSearchable != null) {
    ...
    private void onCloseClicked() {
        if (!TextUtils.isEmpty(this.mSearchSrcTextView.getText())) {
            this.mSearchSrcTextView.setText(LogException.NO_VALUE);
            this.mSearchSrcTextView.requestFocus();
            this.mSearchSrcTextView.setImeVisibility(true);
        } else if (!this.mIconifiedByDefault) {
        } else {
            if (this.mOnCloseListener == null || (this.mOnCloseListener.onClose() ^ 1) != 0) {
                clearFocus();
    ...
    public void onActionViewCollapsed() {
        setQuery(LogException.NO_VALUE, false);
        clearFocus();
        updateViewsVisibility(true);
    ...
    private boolean onItemClicked(int position, int actionKey, String actionMsg) {
        if (this.mOnSuggestionListener != null && (this.mOnSuggestionListener.onSuggestionClick(position) ^ 1) == 0) {
            return false;
        }
    ...
    private boolean onItemSelected(int position) {
        if (this.mOnSuggestionListener != null && (this.mOnSuggestionListener.onSuggestionSelect(position) ^ 1) == 0) {
            return false;
        }
    ...
    public void inflateSearchView(Context context, AttributeSet attrs, TypedArray a) {
        ((LayoutInflater) context.getSystemService("layout_inflater")).inflate(a.getResourceId(0, R.layout.search_view), (ViewGroup) this, true);
    }
    ...
    protected ImageView getGoButton() {
        return this.mGoButton;
    }

    protected View getSubmitArea() {
        return this.mSubmitArea;
    }

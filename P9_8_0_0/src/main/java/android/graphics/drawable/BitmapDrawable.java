...
public class BitmapDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (bitmap != null) {
            ...
            if (shader == null) {
                if (needMirroring) {
                    canvas.save();
                    canvas.translate((float) (this.mDstRect.right - this.mDstRect.left), TonemapCurve.LEVEL_BLACK);
    ...
    private void updateShaderMatrix(Bitmap bitmap, Paint paint, Shader shader, boolean needMirroring) {
        ...
        if (needScaling || needMirroring) {
            ...
            matrix.reset();
            if (needMirroring) {
                matrix.setTranslate((float) (this.mDstRect.right - this.mDstRect.left), TonemapCurve.LEVEL_BLACK);
    ...
    public void getOutline(Outline outline) {
        ...
        if (this.mBitmapState.mBitmap != null) {
            hasAlpha = this.mBitmapState.mBitmap.hasAlpha() ^ 1;
        } else {
            hasAlpha = 0;
        }
        outline.setAlpha(hasAlpha != 0 ? ((float) getAlpha()) / 255.0f : TonemapCurve.LEVEL_BLACK);
    }

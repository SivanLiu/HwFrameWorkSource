                    ...
                    try {
                        ...
                        if (result >= 0) {
                            ...
                            if (TextUtils.isEmpty(voiceName)) {
                                ...
                                String voiceLanguage = LogException.NO_VALUE;
                                try {
                                ...
                                String voiceCountry = LogException.NO_VALUE;
                                try {
    ...
    public Locale getLanguage() {
        return (Locale) runAction(new Action<Locale>() {
            public Locale run(ITextToSpeechService service) {
                return new Locale(TextToSpeech.this.mParams.getString("language", LogException.NO_VALUE), TextToSpeech.this.mParams.getString(Engine.KEY_PARAM_COUNTRY, LogException.NO_VALUE), TextToSpeech.this.mParams.getString(Engine.KEY_PARAM_VARIANT, LogException.NO_VALUE));
            }
        }, null, "getLanguage");
    }
    ...
    public Voice getVoice() {
        return (Voice) runAction(new Action<Voice>() {
            public Voice run(ITextToSpeechService service) throws RemoteException {
                String voiceName = TextToSpeech.this.mParams.getString(Engine.KEY_PARAM_VOICE_NAME, LogException.NO_VALUE);
                if (TextUtils.isEmpty(voiceName)) {
                    return null;
                }
    ...
    public Voice getDefaultVoice() {
        return (Voice) runAction(new Action<Voice>() {
            public Voice run(ITextToSpeechService service) throws RemoteException {
                ...
                String country = defaultLanguage.length > 1 ? defaultLanguage[1] : LogException.NO_VALUE;
                String variant = defaultLanguage.length > 2 ? defaultLanguage[2] : LogException.NO_VALUE;

...
public class ApkSignatureSchemeV2Verifier {
    ...
    private static SignatureInfo findSignature(RandomAccessFile apk) throws IOException, SignatureNotFoundException {
        ...
        return new SignatureInfo(findApkSignatureSchemeV2Block(apkSigningBlock), ((Long) apkSigningBlockAndOffsetInFile.second).longValue(), centralDirOffset, eocdOffset, eocd);
    }
    ...
    private static byte[][] computeContentDigests(int[] digestAlgorithms, DataSource[] contents) throws DigestException {
        ...
        for (i = 0; i < digestAlgorithms.length; i++) {
            byte[] concatenationOfChunkCountAndChunkDigests = new byte[((totalChunkCount * getContentDigestAlgorithmOutputSizeBytes(digestAlgorithms[i])) + 5)];

...
public final class URI implements Comparable<URI>, Serializable {
    static final /* synthetic */ boolean -assertionsDisabled = (URI.class.desiredAssertionStatus() ^ 1);
    ...
    private class Parser {
        ...
        private int scanEscape(int start, int n, char first) throws URISyntaxException {
            ...
            } else if (!(first <= '' || (Character.isSpaceChar(first) ^ 1) == 0 || (Character.isISOControl(first) ^ 1) == 0)) {
                return start + 1;
            }
            return start;
        }
        ...
        private int parseServer(int start, int n) throws URISyntaxException {
            ...
            if (at(p, n, ':')) {
                ...
                q = scan(p, n, "", "/");
                if (q > p) {
                    checkChars(p, q, URI.L_DIGIT, 0, "port number");
                    try {
        ...
        private int parseHostname(int start, int n) throws URISyntaxException {
            ...
            if (l > start && (URI.match(charAt(l), 0, URI.H_ALPHA) ^ 1) != 0) {
        ...
        if (this.string == null) {
            ...
            if (isOpaque()) {
                ...
                if (this.host != null) {
                    ...
                    if (this.host.indexOf(58) < 0 || (this.host.startsWith("[") ^ 1) == 0) {
                        needBrackets = 0;
                    } else {
                        needBrackets = this.host.endsWith("]") ^ 1;
                    }
                    if (needBrackets != 0) {
                        sb.append('[');
                    }
    ...
    private static URI relativize(URI base, URI child) {
        if (child.isOpaque() || base.isOpaque() || !equalIgnoringCase(base.scheme, child.scheme) || (equal(base.authority, child.authority) ^ 1) != 0) {
            return child;
        }

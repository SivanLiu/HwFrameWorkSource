...
public class BcKeyStoreSpi extends KeyStoreSpi implements BCKeyStore {
    ...
    protected void loadStore(InputStream in) throws IOException {
        ...
        for (int type = dataInputStream.read(); type > 0; type = dataInputStream.read()) {
            ...
            if (chainLength != 0) {
                certificateArr = new Certificate[chainLength];
            ...
            switch (type) {
                case 1:
                    ...
                    break;
                case 2:
                    this.table.put(alias, new StoreEntry(alias, date, 2, decodeKey(dataInputStream), certificateArr));
                    break;
                case 3:
                case 4:
                    byte[] b = new byte[dataInputStream.readInt()];
                    dataInputStream.readFully(b);
                    this.table.put(alias, new StoreEntry(alias, date, type, b, certificateArr));
                    break;
                default:

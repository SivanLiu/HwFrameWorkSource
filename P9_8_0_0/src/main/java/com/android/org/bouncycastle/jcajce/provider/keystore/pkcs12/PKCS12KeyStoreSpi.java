...
public class PKCS12KeyStoreSpi extends KeyStoreSpi implements PKCSObjectIdentifiers, X509ObjectIdentifiers, BCKeyStore {
    ...
    private static class IgnoresCaseHashtable {
        ...
        public void put(String key, Object value) {
            Object toLowerCase = key == null ? null : Strings.toLowerCase(key);
            String k = (String) this.keys.get(toLowerCase);
            if (k != null) {
                this.orig.remove(k);
            }
            this.keys.put(toLowerCase, key);
                            ...
                            if (b.getBagId().equals(pkcs8ShroudedKeyBag)) {
                                ...
                                if (b.getBagAttributes() != null) {
                                    ...
                                    while (e.hasMoreElements()) {
                                        ...
                                        aSN1Primitive = null;
                                        if (attrSet.size() > 0) {
                                            aSN1Primitive = (ASN1Primitive) attrSet.getObjectAt(0);
                                            existing = bagAttr.getBagAttribute(aOid);
                                            if (existing == null) {
                                                bagAttr.setBagAttribute(aOid, aSN1Primitive);
                                            } else if (!existing.toASN1Primitive().equals(aSN1Primitive)) {
            ...
            while (i != chain.size()) {
                ...
                if (cb.getCertId().equals(x509Certificate)) {
                    try {
                            ...
                            while (e.hasMoreElements()) {
                                ...
                                if (attrSet.size() > 0) {
                                    ...
                                    if (cert instanceof PKCS12BagAttributeCarrier) {
                                        ...
                                        if (existing == null) {
                                            bagAttr.setBagAttribute(oid, aSN1Primitive);
                                        } else if (!existing.toASN1Primitive().equals(aSN1Primitive)) {
                                            throw new IOException("attempt to add existing attribute with different value");
                                        }
                                    }
                        ...
                        if (!unmarkedKey) {
                            if (aSN1OctetString != null) {
                                this.keyCerts.put(new String(Hex.encode(aSN1OctetString.getOctets())), cert);
                            }
                            if (str != null) {
                                this.certs.put(str, cert);
                            }
                        } else if (this.keyCerts.isEmpty()) {
                            str2 = new String(Hex.encode(createSubjectKeyId(cert.getPublicKey()).getKeyIdentifier()));
                            this.keyCerts.put(str2, cert);
                            this.keys.put(str2, this.keys.remove("unmarked"));
                        }
                        i++;
                    } catch (Exception e32) {
    ...
    public void engineStore(LoadStoreParameter param) throws IOException, NoSuchAlgorithmException, CertificateException {
        ...
        if (!(param instanceof PKCS12StoreParameter) ? param instanceof JDKPKCS12StoreParameter : true) {
            ...
            if (protParam == null) {
                ...
                cArr = ((PasswordProtection) protParam).getPassword();
            } else {
                throw new IllegalArgumentException("No support for protection parameter of type " + protParam.getClass().getName());
            }
            doStore(bcParam.getOutputStream(), cArr, bcParam.isForDEREncoding());
            return;
        }

...
import com.android.framework.protobuf.nano.MapFactories.MapFactory;
    ...
    public static final <K, V> Map<K, V> mergeMapEntry(CodedInputByteBufferNano input, Map<K, V> map, MapFactory mapFactory, int keyType, int valueType, V value, int keyTag, int valueTag) throws IOException {
        ...
        while (true) {
            ...
            if (tag == 0) {
                ...
                k = input.readPrimitiveField(keyType);
        ...
        if (k == null) {
            primitiveDefaultValue = primitiveDefaultValue(keyType);
        }
        if (value == null) {
            value = primitiveDefaultValue(valueType);
        }
        map.put(primitiveDefaultValue, value);
        return map;
    }

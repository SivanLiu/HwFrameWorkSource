            ...
            while (startIndex < len) {
                ...
                if (startId < range.mStartId || endId > range.mEndId) {
                    if (startId - 1 == range.mEndId) {
                        ...
                        if (startIndex + 1 < len) {
                            intRange = (IntRange) this.mRanges.get(startIndex + 1);
                            if (intRange.mStartId - 1 <= endId) {
                                if (endId <= intRange.mEndId) {
                                    newRangeEndId = intRange.mStartId - 1;
                                }
                            } else {
                                intRange = null;
                            }
                        ...
                        if (intRange != null) {
                            if (range.mEndId < intRange.mEndId) {
                                range.mEndId = intRange.mEndId;
                            }
                            range.mClients.addAll(intRange.mClients);

...
public class HwInnerSmsManagerImpl implements HwInnerSmsManager {
    ...
    public boolean isMatchSMSPattern(String address, String policyTag, int phoneId) {
        ...
        if (countryIso != null) {
            String formatNum = PhoneNumberUtils.formatNumberToE164(address, countryIso.toUpperCase(Locale.getDefault()));
            ...
            if (len <= 0 || formatNum == null) {
                ...
                obj = formatNum.substring(len);
            }
        }
        if (!TextUtils.isEmpty(obj)) {
            try {
                isMatchPattern = obj.matches(pattern);

...
public class HwCustGsmServiceStateManagerImpl extends HwCustGsmServiceStateManager {
    ...
    public OnsDisplayParams setOnsDisplayCustomization(OnsDisplayParams odp, ServiceState currentState) {
        ...
        if (currentState != null && currentState.getState() == 0) {
            ...
            if ("65507".equals(str) && (TextUtils.isEmpty(regplmn) ^ 1) != 0 && (TextUtils.isEmpty(spnRes) ^ 1) != 0 && "FNB".equalsIgnoreCase(spnRes.trim())) {
                odp.mSpn = "FNB";
                odp.mShowSpn = HWDBG;
                odp.mShowPlmn = false;
            }
            if ("20404".equals(str) && (TextUtils.isEmpty(spnRes) ^ 1) != 0 && "ziggo.dataxs.mob".equalsIgnoreCase(spnRes.trim())) {
                odp.mSpn = "Ziggo";
            }
            if (isShowNetnameByMcc(str, regplmn) && (TextUtils.isEmpty(currentState.getOperatorAlphaLong()) ^ 1) != 0) {
    ...
    public boolean checkIsInternationalRoaming(boolean roaming, ServiceState currentState) {
        ...
        if (UseUSA_gsmroaming_cmp) {
            ...
            Rlog.d("GsmServiceStateTracker", "After checkIsInternationalRoaming, roaming is " + z);
        }
        return z;
    }

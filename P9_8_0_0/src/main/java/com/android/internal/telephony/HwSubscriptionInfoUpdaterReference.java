...
public class HwSubscriptionInfoUpdaterReference implements SubscriptionInfoUpdaterReference {
        ...
        if (mUiccController != null) {
            ...
            if (PROJECT_SIM_NUM <= 1 || (subHelper.proceedToHandleIccEvent(slotId) ^ 1) == 0) {
                ...
                if (!IccCardStatusUtils.isCardPresent(newState)) {
                    ...
                    boolean isCardPresent = (!HwVSimUtils.needBlockUnReservedForVsim(slotId) || (IccCardStatusUtils.isCardPresent(newState) ^ 1) == 0) ? false : IccCardStatusUtils.isCardPresent(oldState);
                    if (isCardPresent) {
                    ...
                    if (subscriptionInfoUpdaterUtils.getIccId(this.mSubscriptionInfoUpdater)[slotId] != null && subscriptionInfoUpdaterUtils.getIccId(this.mSubscriptionInfoUpdater)[slotId].equals(ICCID_STRING_FOR_NO_SIM)) {
    ...
    private String printIccid(String iccid) {
        ...
        return iccid.substring(0, 6) + new String(new char[(iccid.length() - 6)]).replace('\u0000', '*');
    }

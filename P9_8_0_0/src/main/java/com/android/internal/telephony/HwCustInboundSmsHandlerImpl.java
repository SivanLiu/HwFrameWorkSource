...
public class HwCustInboundSmsHandlerImpl extends HwCustInboundSmsHandler {
    ...
    public boolean isIQISms(SmsMessage sms) {
        ...
        try {
            isIQIsms = ((Boolean) jarClass.getMethod("checkSMS", new Class[]{String.class}).invoke(jarObj, new Object[]{sms.getMessageBody()})).booleanValue();
        } catch (Exception e) {
    ...
    public boolean isIQIWapPush(ByteArrayOutputStream output) {
        ...
        try {
            isIQIwap = ((Boolean) jarClass.getMethod("checkWAPPush", new Class[]{byte[].class}).invoke(jarObj, new Object[]{output.toByteArray()})).booleanValue();
        } catch (Exception e) {

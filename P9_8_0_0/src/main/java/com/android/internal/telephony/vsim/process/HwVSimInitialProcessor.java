        ...
        if (!this.mSlotSwitchInitDone) {
            ...
            if (HwVSimUtilsInner.isPlatformRealTripple() && HwVSimUtilsInner.IS_DSDSPOWER_SUPPORT) {
                if (this.mDSDSAutoSetModemModeDone) {
                    boolean needBlockUnReservedForVsim = (-1 != this.mVSimController.getSubState(0) || (this.mVSimController.needBlockUnReservedForVsim(0) ^ 1) == 0) ? -1 == this.mVSimController.getSubState(1) ? this.mVSimController.needBlockUnReservedForVsim(1) ^ 1 : false : true;
                    if (needBlockUnReservedForVsim) {

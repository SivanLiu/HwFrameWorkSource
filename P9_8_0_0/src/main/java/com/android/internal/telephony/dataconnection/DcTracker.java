...
public class DcTracker extends AbstractDcTrackerBase {
    ...
    public boolean isDataPossible(String apnType) {
        ...
        boolean isBipApnType = (apnContext.getApnType().equals("emergency") || isDataAllowedByApnType(null, apnType)) ? true : isBipApnType(apnType);
        boolean z = isBipApnType ? apnTypePossible : false;
        ...
        log(String.format("isDataPossible(%s): possible=%b isDataAllowed=%b apnTypePossible=%b apnContextisEnabled=%b apnContextState()=%s", new Object[]{apnType, Boolean.valueOf(z), Boolean.valueOf(isBipApnType), Boolean.valueOf(apnTypePossible), Boolean.valueOf(apnContextIsEnabled), apnContextState}));
        return z;
    }
    ...
    public boolean isDataAllowed(DataAllowFailReason failureReason, boolean isMms, boolean isUserEnable) {
        ...
        int i = 0;
        if (r != null) {
            i = !r.getImsiReady() ? r.getRecordsLoaded() : 1;
            if ((i ^ 1) != 0) {
        ...
        if (!(i != 0 || (isUserEnable ^ 1) == 0 || (isDataAllowedVoWiFi ^ 1) == 0)) {
            if (failureReason == null) {
                return false;
            }
        ...
        if (SystemProperties.getBoolean("ro.config.enable_wl_coexist", false)) {
            ...
            if (apnType.equals("default") || "hipri".equals(apnType)) {
                ...
                if (enabled != 1) {
                    ...
                    if (!apnContext.isDisconnected() && (this.mIsWifiConnected || r2)) {
    ...
    boolean isNeedFilterVowifiMms(ApnSetting apn, String requestedApnType) {
        ...
        boolean contains = apn != null ? ArrayUtils.contains(apn.types, APN_TYPE_VOWIFIMMS) : false;
        if (isMmsRequested && contains) {

...
public class BatteryStatsHelper {
    ...
    private void addIdleUsage() {
        double totalPowerMah = ((((double) (this.mTypeBatteryRealtimeUs / 1000)) * this.mPowerProfile.getAveragePower(PowerProfile.POWER_CPU_IDLE)) + (((double) (this.mStats.getScreenOnTime(SystemClock.elapsedRealtime() * 1000, this.mStatsType) / 1000)) * this.mPowerProfile.getAveragePower(PowerProfile.POWER_CPU_AWAKE))) / 3600000.0d;
    ...
    public double removeHiddenBatterySippers(List<BatterySipper> sippers) {
        ...
        for (int i = sippers.size() - 1; i >= 0; i--) {
            ...
            if (!(!sipper.shouldHide || sipper.drainType == DrainType.OVERCOUNTED || sipper.drainType == DrainType.SCREEN || sipper.drainType == DrainType.UNACCOUNTED)) {
    ...
    public void smearScreenBatterySipper(List<BatterySipper> sippers, BatterySipper screenSipper) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (uid != null) {
                long timeMs = getForegroundActivityTotalTimeMs(uid, rawRealtimeMs);
                activityTimeArray.put(uid.getUid(), timeMs);
                totalActivityTimeMs += timeMs;
            }
        }
        if (totalActivityTimeMs >= 600000) {
            double screenPowerMah = screenSipper != null ? screenSipper.totalPowerMah : 0.0d;
            size = sippers.size();
            for (i = 0; i < size; i++) {
                BatterySipper sipper = (BatterySipper) sippers.get(i);
    ...
    public long getForegroundActivityTotalTimeMs(Uid uid, long rawRealtimeMs) {
    ...
    private static BatteryStatsImpl getStats(IBatteryStats service) {
        ...
        try {
            ...
            if (parcelFileDescriptor != null && MemoryFile.valid(parcelFileDescriptor.getFileDescriptor())) {

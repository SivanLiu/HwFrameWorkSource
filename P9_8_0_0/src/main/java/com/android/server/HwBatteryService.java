...
public final class HwBatteryService extends BatteryService {
    ...
    private static final String ACTION_NOT_TIPS_BTN_CLICKED = "huawei.intent.action.NOT_TIPS_BTN_CLICKED";
    ...
    private static final long CHARGING_NOTICE_PERIOD_DAY = TimeUnit.DAYS.toMillis(1);
    private static final long CHARGING_NOTICE_PERIOD_WEEK = TimeUnit.DAYS.toMillis(7);
    private static final String FACTORY_VERSION = "factory";
    private static final String FAKE_BATTERY_LAST_NOTICE_TIME = "fake_battery_last_notice_time";
    private static final String FAKE_BATTERY_NOT_TIPS_STATUS = "fake_battery_not_tips_status";
    ...
    private static final int MSG_BATTERY_FAKE_ERROR = 4;
    ...
    private final UEventObserver mBatteryFakeObserver = new UEventObserver() {
        public void onUEvent(UEvent event) {
            Slog.d(HwBatteryService.TAG, "onUEVENT fake_battery error");
            try {
                String promptStatus = FileUtils.readTextFile(new File(HwBatteryService.this.getBatteryFakePromptStatePath()), 0, null).trim();
                Slog.d(HwBatteryService.TAG, "promptStatus = " + promptStatus);
                if (!"1".equals(promptStatus) || !HwBatteryService.this.mIsBootFinish) {
                    return;
                }
                if (HwBatteryService.this.mIsBootHasReport) {
                    HwBatteryService.this.handleChargingScene();
                    return;
                }
                Slog.i(HwBatteryService.TAG, "Prompt the user each time boot completed.");
                HwBatteryService.this.mIsBootHasReport = true;
                HwBatteryService.this.handleBatteryFakeStatus();
            } catch (Exception e) {
                Slog.e(HwBatteryService.TAG, "Error get promptStatus.", e);
            }
        }
    };
    ...
    private long mChargingNoticeTime = 0;
    ...
    private int mFakeBatteryNotTipsStatus = 0;
    private boolean mFakeBatteryWarning = SystemProperties.getBoolean("ro.config.hw_fakebattery_warning", false);
    ...
    private boolean mIsBootHasReport = false;
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (HwBatteryService.ACTION_CALL_BNT_CLICKED.equals(intent.getAction())) {
                ...
                notificationID = intent.getIntExtra(HwBatteryService.BATTERY_ERR_NOTIFICATION_ID, 0);
                ...
                if (HwBatteryService.this.mFakeBatteryWarning) {
                    HwBatteryService.this.mBatteryFakeObserver.startObserving("BATTERY_EVENT=FAKE_BATTERY_DETECTED");
                    try {
                        Slog.i(HwBatteryService.TAG, "fbdpStatus: " + FileUtils.readTextFile(new File(HwBatteryService.this.getBatteryFakeEnabledStatePath()), 0, null).trim());
                    } catch (Exception e) {
                        Slog.e(HwBatteryService.TAG, "Error get fbdpstatus.", e);
                    }
                    HwBatteryService.this.mChargingNoticeTime = Global.getLong(HwBatteryService.this.mContext.getContentResolver(), HwBatteryService.FAKE_BATTERY_LAST_NOTICE_TIME, 0);
                    HwBatteryService.this.mFakeBatteryNotTipsStatus = Global.getInt(HwBatteryService.this.mContext.getContentResolver(), HwBatteryService.FAKE_BATTERY_NOT_TIPS_STATUS, 0);
                    Log.d(HwBatteryService.TAG, "mChargingNoticeTime = " + HwBatteryService.this.mChargingNoticeTime + ", mFakeBatteryNotTipsStatus = " + HwBatteryService.this.mFakeBatteryNotTipsStatus);
                }
            } else if (HwBatteryService.ACTION_NOT_TIPS_BTN_CLICKED.equals(intent.getAction())) {
                Slog.i(HwBatteryService.TAG, "mFakeBatteryNotTipsStatus = " + HwBatteryService.this.mFakeBatteryNotTipsStatus);
                HwBatteryService.this.mFakeBatteryNotTipsStatus = 1;
                Global.putInt(HwBatteryService.this.mContext.getContentResolver(), HwBatteryService.FAKE_BATTERY_NOT_TIPS_STATUS, 1);
                notificationID = intent.getIntExtra(HwBatteryService.BATTERY_ERR_NOTIFICATION_ID, 0);
                if (HwBatteryService.this.mNotificationManager != null) {
                    HwBatteryService.this.mNotificationManager.cancelAsUser(null, notificationID, UserHandle.CURRENT);
                }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    return;
                case 4:
                    HwBatteryService.this.sendBatteryFakeNotification();
                    return;
                case 6:
                    HwBatteryService.this.sendBatteryErrorNotification(33685530, 33685529, 33751135);
    ...
    private String getBatteryFakeEnabledStatePath() {
        return "/sys/class/hw_power/fake_battery/fbdp";
    }

    private String getBatteryFakePromptStatePath() {
        return "/sys/class/hw_power/fake_battery/fake_prompt";
    }
    ...
    private void initAndRegisterReceiver() {
        ...
        if (this.mFakeBatteryWarning) {
            filter.addAction(ACTION_NOT_TIPS_BTN_CLICKED);
        }
    ...
    private void sendBatteryFakeNotification() {
        Slog.i(TAG, "sendBatteryFakeNotification");
        if (this.mNotificationManager == null) {
            this.mNotificationManager = (NotificationManager) this.mContext.getSystemService("notification");
        }
        CharSequence title = this.mContext.getResources().getString(33685528);
        CharSequence message = this.mContext.getResources().getString(33685527);
        makeNotificationChannel(title.toString());
        Intent skipIntent = new Intent(ACTION_NOT_TIPS_BTN_CLICKED);
        skipIntent.putExtra(BATTERY_ERR_NOTIFICATION_ID, 33685528);
        skipIntent.setPackage(this.mContext.getPackageName());
        PendingIntent actionClickPI = PendingIntent.getBroadcast(this.mContext, 0, skipIntent, 0);
        Notification.Builder build = new Notification.Builder(this.mContext, OVP_ERR_CHANNEL_ID).setSmallIcon(33751135).setContentTitle(title).setContentText(message).setStyle(new BigTextStyle().bigText(message)).setTicker(title).setVibrate(new long[0]).setPriority(2).setWhen(System.currentTimeMillis()).setShowWhen(true).setVisibility(1).setAutoCancel(true);
        build.addAction(new Action.Builder(null, this.mContext.getString(33686009), actionClickPI).build());
        Notification notification = build.build();
        if (this.mNotificationManager != null) {
            this.mNotificationManager.notifyAsUser(null, 33685528, notification, UserHandle.ALL);
        }
    }

    private void handleBatteryFakeStatus() {
        Slog.i(TAG, "handleBatteryFakeStatus");
        if (this.mHwBatteryHandler.hasMessages(4)) {
            this.mHwBatteryHandler.removeMessages(4);
        }
        this.mHwBatteryHandler.sendMessage(Message.obtain(this.mHwBatteryHandler, 4));
    }

    private void handleChargingScene() {
        Slog.i(TAG, "handleChargingScene");
        long currentTime = System.currentTimeMillis();
        long elapsedTime = currentTime - this.mChargingNoticeTime;
        if (this.mFakeBatteryNotTipsStatus == 1) {
            if (elapsedTime > CHARGING_NOTICE_PERIOD_WEEK) {
                Slog.i(TAG, "The last prompt was one week ago. Now prompt the user again, noticeTime is " + currentTime);
                this.mChargingNoticeTime = currentTime;
                Global.putLong(this.mContext.getContentResolver(), FAKE_BATTERY_LAST_NOTICE_TIME, currentTime);
                handleBatteryFakeStatus();
            }
        } else if (elapsedTime > CHARGING_NOTICE_PERIOD_DAY) {
            Slog.i(TAG, "The last prompt was one day ago, or the first. Now prompt the user, noticeTime is " + currentTime);
            this.mChargingNoticeTime = currentTime;
            Global.putLong(this.mContext.getContentResolver(), FAKE_BATTERY_LAST_NOTICE_TIME, currentTime);
            handleBatteryFakeStatus();
        }

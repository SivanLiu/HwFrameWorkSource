    ...
    private void setupDemoUser(UserInfo userInfo) {
        ...
        if (this.mIsCarrierDemoMode) {
            ...
            for (String packageName2 : packageNames) {
                try {
                    iPm.setApplicationEnabledSetting(packageName2, 1, 0, userInfo.id, null);
                } catch (RemoteException re) {
                    Slog.e(TAG, "Error enabling application: " + packageName2, re);
                }
            }
            return;
        }
        ...
        if (!TextUtils.isEmpty(demoLauncher)) {
            ...
            packageName2 = componentToEnable.getPackageName();
            try {
                iPm = AppGlobals.getPackageManager();

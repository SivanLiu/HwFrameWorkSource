...
public abstract class ManagedServices {
    ...
    private void rebuildRestoredPackages() {
        ...
        String restoredSettingName = this.mConfig.secondarySettingName == null ? null : restoredSettingName(this.mConfig.secondarySettingName);
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (restoredSettingName != null) {
                names.addAll(loadComponentNamesFromSetting(restoredSettingName, userIds[i]));
            }
    ...
    private void storeComponentsToSetting(Set<ComponentName> components, String settingName, int userId) {
        ...
        Object[] objArr = null;
        if (components != null) {
            objArr = new String[components.size()];
            int index = 0;
            for (ComponentName c : components) {
                int index2 = index + 1;
                objArr[index] = c.flattenToString();
                index = index2;
            }
        }
        if (objArr == null) {
            ...
            value = TextUtils.join(ENABLED_SERVICES_SEPARATOR, objArr);
        }

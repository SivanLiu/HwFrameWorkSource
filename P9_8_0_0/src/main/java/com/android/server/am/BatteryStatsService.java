    ...
    void updateExternalStatsSync(String reason, int updateFlags) {
        ...
        synchronized (this.mExternalStatsLock) {
            if (this.mContext != null) {
                ...
                try {
                    wifiActivityEnergyInfo = (WifiActivityEnergyInfo) awaitControllerInfo(synchronousResultReceiver);
                    if (isShutdown) {
                ...
                try {
                    bluetoothActivityEnergyInfo = (BluetoothActivityEnergyInfo) awaitControllerInfo(synchronousResultReceiver2);
                    if (isShutdown) {
                ...
                try {
                    modemActivityInfo = (ModemActivityInfo) awaitControllerInfo(synchronousResultReceiver3);
                    if (isShutdown) {
                ...
                if (wifiActivityEnergyInfo != null) {
                    if (wifiActivityEnergyInfo.isValid()) {
                        this.mStats.updateWifiState(extractDelta(wifiActivityEnergyInfo));
                        if (isShutdown) {

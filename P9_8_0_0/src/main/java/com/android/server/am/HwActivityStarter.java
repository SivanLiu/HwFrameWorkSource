        ...
        if (intent.getComponent() != null && HwDeviceManager.disallowOp(4, intent.getComponent().getPackageName())) {
            ...
            UiThread.getHandler().post(new Runnable() {
                public void run() {
                    ...
                    if (context != null) {
                        Toast.makeText(context, context.getString(33685978), 0).show();
                    }
                }
            });
            return -96;
    ...
    private void showErrorDialogToRemoveUser(final Context context, final int userId) {
        UiThread.getHandler().post(new Runnable() {
            public void run() {
                ...
                final int i = userId;
                AlertDialog ErrorDialog = builder.setPositiveButton(33685948, new OnClickListener() {
                ...
                }).setTitle(context.getString(33685950)).setMessage(context.getString(33685949)).create();

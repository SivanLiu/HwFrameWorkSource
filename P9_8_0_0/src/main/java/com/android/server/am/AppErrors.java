...
class AppErrors {
    ...
    boolean handleAppCrashLocked(ProcessRecord app, String reason, String shortMsg, String longMsg, String stackTrace, Data data) {
        ...
        if (crashTime == null || now >= crashTime.longValue() + LingerMonitor.DEFAULT_NOTIFICATION_RATE_LIMIT_MILLIS) {
            ...
            if (!(data == null || r15 == null || now >= r15.longValue() + LingerMonitor.DEFAULT_NOTIFICATION_RATE_LIMIT_MILLIS)) {
                data.repeating = true;
            }
        } else {
                ...
                if (proc == null || proc.crashDialog == null) {
                    ...
                    if (isBackground == 0 || (showBackground ^ 1) == 0) {
                        ...
                        if (this.mHwCustAppErrors == null || !this.mHwCustAppErrors.isCustom()) {
                            if (isDebuggable) {
                                if (!((!this.mService.canShowErrorDialogs() && !showBackground) || (r3 ^ 1) == 0 || (HwFrameworkFactory.getVRSystemServiceManager().isVRMode() ^ 1) == 0)) {
                                    isShowCrash = hasForegroundUI(proc);
                                }
                            }
                            isShowCrash = false;
                        } else if ((!this.mService.canShowErrorDialogs() && !showBackground) || (r3 ^ 1) == 0 || (HwFrameworkFactory.getVRSystemServiceManager().isVRMode() ^ 1) == 0) {
                            isShowCrash = false;
                        } else {
                            isShowCrash = hasForegroundUI(proc);
                        }

...
public class ValidPeriodProcessor extends BaseProcessor {
    ...
    public boolean readCert(String line, CertificationData rawCert) {
        ...
        if (key == null || key.isEmpty()) {
            HwAuthLogger.e("HwCertificationManager", "readCert:ValidPeriod is empty");
            return false;
        }
    ...
    public boolean parserCert(HwCertification rawCert) {
        ...
        if (period == null) {
            HwAuthLogger.e("HwCertificationManager", "parserCert:ValidPeriod error,period time is null");
            return false;
        }
        ...
        if (m.matches()) {
            try {
                ...
                HwAuthLogger.e("HwCertificationManager", "parserCert:ValidPeriod error,catch ParseException");
        ...
        HwAuthLogger.e("HwCertificationManager", "parserCert:ValidPeriod error");
        return false;
    }

    public boolean verifyCert(Package pkg, HwCertification cert) {
        if (HwAuthLogger.getHWFLOW()) {
            HwAuthLogger.i("HwCertificationManager", "--Verify date--");
        }
        ...
        if (!toDate.after(fromDate)) {
            HwAuthLogger.e("HwCertificationManager", "verifyCert:date error, fromTime is : " + fromDate + ", toTime is:" + toDate);
            return false;
        } else if (!isCurrentDataExpired(toDate)) {
            return true;
        } else {
            HwAuthLogger.e("HwCertificationManager", "verifyCert:date expired," + cert.mCertificationData.mPeriodString);
            return false;
        }
    }

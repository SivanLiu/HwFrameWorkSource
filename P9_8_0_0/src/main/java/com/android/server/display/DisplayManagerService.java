...
public final class DisplayManagerService extends SystemService {
    ...
    private void configureDisplayInTransactionLocked(DisplayDevice device) {
        ...
        if (!ownContent) {
            if (!(findLogicalDisplayForDeviceLocked == null || (findLogicalDisplayForDeviceLocked.hasContentLocked() ^ 1) == 0)) {
                boolean isPcCastModeInServerEarly;
                if (HwPCUtils.isValidExtDisplayId(findLogicalDisplayForDeviceLocked.getDisplayIdLocked())) {
        ...
        setExternalTouchViewport(device, info, findLogicalDisplayForDeviceLocked);
        ...
        if (info.touch == 3 && (TextUtils.isEmpty(info.uniqueId) ^ 1) != 0) {
            setViewportLocked(getVirtualTouchViewportLocked(info.uniqueId), findLogicalDisplayForDeviceLocked, device);
        }
    }
    ...
    private void setExternalTouchViewport(DisplayDevice device, DisplayDeviceInfo info, LogicalDisplay display) {
        ...
        if (HwPCUtils.isPcCastModeInServer() && (r1 ^ 1) != 0 && (info.flags & 1) == 0 && display != null && HwPCUtils.isValidExtDisplayId(display.getDisplayIdLocked())) {
            try {

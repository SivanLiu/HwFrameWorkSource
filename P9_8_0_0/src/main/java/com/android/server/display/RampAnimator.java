...
public class RampAnimator<T> {
    ...
    public boolean animateTo(int target, int rate) {
        if (!this.mFirstTime && rate > 0) {
            boolean changed;
            if (this.mAnimating && rate <= this.mRate && (target > this.mCurrentValue || this.mCurrentValue > this.mTargetValue)) {
                if (this.mTargetValue <= this.mCurrentValue && this.mCurrentValue <= target) {
                }
                changed = this.mTargetValue == target;

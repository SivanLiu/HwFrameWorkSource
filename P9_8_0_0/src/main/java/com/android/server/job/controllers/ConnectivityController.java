...
public final class ConnectivityController extends StateController implements OnNetworkActiveListener {
    ...
    private boolean updateConstraintsSatisfied(JobStatus jobStatus, NetworkCapabilities capabilities) {
        ...
        if (capabilities != null) {
            hasCapability = capabilities.hasCapability(16);
        } else {
            hasCapability = false;
        }
        boolean isConnected = info != null ? info.isConnected() : false;
        boolean changed = ((jobStatus.setConnectivityConstraintSatisfied(isConnected ? hasCapability : false) | jobStatus.setMeteredConstraintSatisfied(isConnected ? info.isMetered() : false)) | jobStatus.setUnmeteredConstraintSatisfied(isConnected ? info.isMetered() ^ 1 : false)) | jobStatus.setNotRoamingConstraintSatisfied(isConnected ? info.isRoaming() ^ 1 : false);
        if (jobUid == 1000) {

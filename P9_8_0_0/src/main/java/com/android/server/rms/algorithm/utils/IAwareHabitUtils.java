...
public class IAwareHabitUtils {
    ...
    public static void loadUsageData(ContentResolver resolver, Map<String, Integer> usageCount, int userId) {
        if (resolver != null && usageCount != null) {
            ...
            Cursor cursor = null;
            try {
                cursor = resolver.query(Database.PKGNAME_URI, new String[]{"appPkgName", "totalUseTimes"}, whereClause, new String[]{String.valueOf(userId)}, null);
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        String pkgName = cursor.getString(0);
                        int useTotalTimes = cursor.getInt(1);
    ...
    public static void loadAppAssociateInfo(ContentResolver resolver, Map<String, Integer> map, ArrayList<ArrayList<Integer>> data, int userId) {
        ...
        if (!checkParam) {
            ...
            Cursor cursor = null;
            try {
                cursor = resolver.query(Database.ASSOCIATE_URI, new String[]{"srcPkgName", "dstPkgName", "transitionTimes"}, "srcPkgName NOT IN  (select appPkgName from PkgName WHERE deleted=1 AND userID = ?) AND dstPkgName NOT IN  (select appPkgName from PkgName WHERE deleted=1 AND userID = ?) AND userID=?", new String[]{String.valueOf(userId), String.valueOf(userId), String.valueOf(userId)}, null);
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        ...
                        int j;
                        boolean isNeedSet;
                        String srcPkgName = cursor.getString(0);
                        String dstPkgName = cursor.getString(1);
                        int transitionTimes = cursor.getInt(2);
                        if (map.containsKey(srcPkgName)) {
                            if (map.containsKey(dstPkgName)) {
                                isContainsKey = true;
                                if (isContainsKey) {
                                    i = ((Integer) map.get(srcPkgName)).intValue();
                                    j = ((Integer) map.get(dstPkgName)).intValue();
    ...
    public static void loadPkgInfo(ContentResolver resolver, Map<String, Integer> map, Map<Integer, String> revertMap, Map<String, Integer> usageCount, Map<String, UsageDistribution> appUsageDistributionMap, int userId) {
        if (resolver != null && map != null && revertMap != null && usageCount != null && appUsageDistributionMap != null) {
            ...
            Cursor cursor = null;
            try {
                cursor = resolver.query(Database.PKGNAME_URI, new String[]{"appPkgName", "totalUseTimes", "totalInDay", "totalInNight"}, whereClause, new String[]{String.valueOf(userId)}, null);
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        String name = cursor.getString(0);
                        int useTotalTimes = cursor.getInt(1);
                        appUsageDistributionMap.put(name, new UsageDistribution(cursor.getInt(2), cursor.getInt(3)));
    ...
    public static void updatePkgNameTable(Context ctx) {
        if (ctx != null) {
            ...
            if (resolver != null) {
                Cursor cursor = null;
                try {
                    cursor = resolver.query(Database.PKGRECORD_URI, new String[]{"appPkgName", "flag", "userID"}, null, null, null);
                    if (cursor != null) {
    ...
    public static void updateReInstallPkgNameInfo(ContentResolver resolver, List<Integer> userIdList) {
        if (resolver != null && userIdList != null) {
            ...
            for (int i = 0; i < userIdList.size(); i++) {
                ...
                Object -l_4_R = set.iterator();
                while (-l_4_R.hasNext()) {
                    String name = (String) -l_4_R.next();
    ...
    private static void loadReInstallPkgFromUserData(ContentResolver resolver, Set<String> set, int userId) {
        ...
        try {
            ...
            cursor = contentResolver.query(Database.USERDATA_URI, new String[]{"UserData.appPkgName"}, whereClause, new String[]{String.valueOf(userId), String.valueOf(userId), String.valueOf(userId)}, "UserData._id");
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    String name = cursor.getString(0);

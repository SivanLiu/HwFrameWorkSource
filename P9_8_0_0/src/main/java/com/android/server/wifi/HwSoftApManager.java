...
public class HwSoftApManager extends SoftApManager {
    ...
    private Handler mApLinkedStaChangedHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            if (HwSoftApManager.this.mLinkedStaCount >= 0 && HwSoftApManager.this.mLinkedStaCount <= 8) {
                if (HwSoftApManager.this.mLinkedStaCount != HwSoftApManager.this.mMacList.size()) {
                }
    ...
    public int getApChannel(WifiConfiguration config) {
        ...
        if (config.apBand == 0 && SystemProperties.getBoolean(ISM_COEX_ON, false)) {
            ...
            if (apChannel != 0 && (config.apBand != 0 || apChannel <= 14)) {
                if (config.apBand == 1 && apChannel < 34) {
                }
            }

...
public class PortalAutoFillManager {
    ...
    private void parseXmlAndInsertDatabase(String fileName, boolean fromAssets) {
        ...
        try {
            ...
            while (eventType != 1) {
                int currentParsedRow2;
                if (eventType == 0) {
                    LOGD("eventType = START_DOCUMENT");
                    currentParsedRow2 = currentParsedRow;
                } else {
                    if (eventType == 2) {
                        if (xpp.getName() != null && xpp.getName().equals(PAGE_ITEM)) {
                            if (lastParsedRowNum >= 0 && ((long) currentParsedRow) > 1000 + lastParsedRowNum) {
                                LOGW("parseXmlAndInsertDatabase, currentParsedRow = " + currentParsedRow + ", enter next interval...");
                                this.mHandler.sendEmptyMessageDelayed(108, 30000);
                                break;
                            } else if (((long) currentParsedRow) > lastParsedRowNum) {
                                if (pageInfo == null) {
                        ...
                        if (xpp.getName() == null) {
                            ...
                            currentParsedRow2 = currentParsedRow + 1;
                            if (((long) currentParsedRow) > lastParsedRowNum && pageInfo != null) {

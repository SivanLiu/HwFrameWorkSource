...
public class HwOEMInfoUtil {
    ...
    private static int writeByteArrayToOeminfo(int type, int sizeOf, byte[] mByte) {
        int rst = -1;
        try {
            rst = ((Integer) Class.forName("com.huawei.android.os.HwOemInfoCustEx").getMethod("writeByteArrayToOeminfo", new Class[]{Integer.TYPE, Integer.TYPE, byte[].class}).invoke(null, new Object[]{Integer.valueOf(type), Integer.valueOf(sizeOf), mByte})).intValue();
        } catch (ClassNotFoundException e) {
            Slog.w(TAG, "Unable to find class");
        } catch (NoSuchMethodException e2) {

...
public class SecurityProfileService implements IHwSecurityPlugin {
    ...
    public boolean shouldPreventMediaProjection(int uid) {
        ...
        for (RunningTaskInfo taskInfo : hwAm.getTasks(10, 0)) {
            String packageName = taskInfo.topActivity == null ? null : taskInfo.topActivity.getPackageName();
            if (!(packageName == null || projectionSet.contains(packageName) || !hwAm.isTaskVisible(taskInfo.id))) {
                ScreenshotProtectorCallback callback = this.screenshotProtector.getRegister(packageName);
                if (callback != null) {
        ...
        if (isBlackApp(targetPackage)) {
            if (type == 0 && isLauncherApp(callerPackage)) {
                ...
                if (curTime < this.blackToastTime || curTime - this.blackToastTime > 3500) {
                    ...
                    String text = this.mContext.getResources().getString(33686075);
                    if (text != null) {
                        showToast(text);
                    }
                }

    ...
    public void sendBadBatteryWarningNotification(Context context, BatteryProperties oldBatteryProps, BatteryProperties newBatteryProps) {
        ...
        if (oldBatteryProps.batteryLevel - newBatteryProps.batteryLevel >= ERROR_BATTERY_LEVEL && newBatteryProps.batteryTemperature < ERROR_BATTERY_TEMP) {
            ...
            if (BAD_BATTERY_VALUE.equals(badBattery)) {
                CharSequence title = context.getResources().getString(33685532);
                if (this.mNotificationManager == null) {
                    this.mNotificationManager = (NotificationManager) context.getSystemService("notification");
                }
                makeNotificationChannel(title.toString());
                Notification notification = new Builder(context, TEMP_ERR_CHANNEL_ID).setSmallIcon(33751168).setContentTitle(title).setContentText(context.getResources().getString(33685531)).setTicker(title).setVibrate(new long[0]).setPriority(2).setVisibility(1).setAutoCancel(true).build();
                if (this.mNotificationManager != null) {
                    this.mNotificationManager.notifyAsUser(null, 33685532, notification, UserHandle.ALL);
                }
            }
        }
    }

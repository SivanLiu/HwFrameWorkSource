...
public class HwAlarmManagerService extends AlarmManagerService {
    ...
    public void removePackageAlarm(final String pkg, final List<String> tags) {
        if (this.mHandler != null) {
            this.mHandler.post(new Runnable() {
                public void run() {
                    synchronized (HwAlarmManagerService.this.mLock) {
                        HwAlarmManagerService.this.removeByTagLocked(pkg, tags);
                    }
                }
            });
        }
    ...
    private boolean removeSingleTagLocked(String packageName, String tag) {
        int i;
        ...
        for (i = this.mAlarmBatches.size() - 1; i >= 0; i--) {
            ...
            if (tag == null) {
                ...
                didRemove |= removeFromBatch(b, packageName, tag);
            }
            if (b.size() == 0) {
                this.mAlarmBatches.remove(i);
            }
    ...
    private void removeByTagLocked(String packageName, List<String> tags) {
        boolean didRemove = false;
        if (packageName != null) {
            if (tags == null) {
                didRemove = removeSingleTagLocked(packageName, null);
            } else {
                for (String tag : tags) {
                    didRemove |= removeSingleTagLocked(packageName, tag);
                }
            }
            if (didRemove) {
    ...
    boolean removeFromBatch(Batch batch, String packageName, String tag) {
        if (packageName == null) {
            return false;
        }
        boolean didRemove = false;
            ...
            if (alarm.matches(packageName)) {
                ...
                if (alarmTag != null) {
                    ...
                    if (splits.length > 1 && splits[1].equals(tag)) {
                        ...
                        didRemove = true;
                        if (tag != null) {
                            Slog.d(TAG, "remove package alarm " + packageName + "' (" + tag + ")");
                        }

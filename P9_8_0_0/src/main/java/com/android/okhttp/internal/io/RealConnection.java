...
public final class RealConnection implements Connection {
    ...
    private void connectTls(int readTimeout, int writeTimeout, ConnectionSpecSelector connectionSpecSelector) throws IOException {
        ...
        Socket socket = null;
        try {
            socket = (SSLSocket) address.getSslSocketFactory().createSocket(this.rawSocket, address.getUriHost(), address.getUriPort(), true);
            ConnectionSpec connectionSpec = connectionSpecSelector.configureSecureSocket(socket);
            if (connectionSpec.supportsTlsExtensions()) {
                Platform.get().configureTlsExtensions(socket, address.getUriHost(), address.getProtocols());
            }
            socket.startHandshake();
            Handshake unverifiedHandshake = Handshake.get(socket.getSession());
            if (address.getHostnameVerifier().verify(address.getUriHost(), socket.getSession())) {

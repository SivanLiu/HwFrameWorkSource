            ...
            while (pos < value.length()) {
                ...
                if (pos == value.length() || value.charAt(pos) == ',' || value.charAt(pos) == ';') {
                    ...
                    if (pos >= value.length() || value.charAt(pos) != '\"') {
                        parameterStart = pos;
                        pos = HeaderParser.skipUntil(value, pos, ",;");
                        str = value.substring(parameterStart, pos).trim();
                    } else {
                        pos++;
                        parameterStart = pos;
                        pos = HeaderParser.skipUntil(value, pos, "\"");
                        str = value.substring(parameterStart, pos);
                        pos++;
                    }
                }
                if ("no-cache".equalsIgnoreCase(directive)) {
                    ...
                    maxAgeSeconds = HeaderParser.parseSeconds(str, -1);
                } else if ("s-maxage".equalsIgnoreCase(directive)) {
                    sMaxAgeSeconds = HeaderParser.parseSeconds(str, -1);
                    ...
                    maxStaleSeconds = HeaderParser.parseSeconds(str, Integer.MAX_VALUE);
                } else if ("min-fresh".equalsIgnoreCase(directive)) {
                    minFreshSeconds = HeaderParser.parseSeconds(str, -1);

    ...
    public void run() throws Exception {
        ...
        try {
            if (this.proto) {
                ...
                statusReporter = new ProtoStatusReporter();
            } else if (this.wait) {
                statusReporter = new TextStatusReporter(this.rawMode);
            }
            Object obj = null;
            IUiAutomationConnection iUiAutomationConnection = null;
            if (statusReporter != null) {
                obj = new InstrumentationWatcher(statusReporter);
            ...
            if (!this.mAm.startInstrumentation(cn, this.profileFile, 0, this.args, obj, iUiAutomationConnection, this.userId, this.abi)) {
                throw new AndroidException("INSTRUMENTATION_FAILED: " + cn.flattenToString());
            } else if (obj == null || obj.waitForFinish()) {
                if (oldAnims != null) {
                    this.mWm.setAnimationScales(oldAnims);
                }
            } else {
                statusReporter.onError("INSTRUMENTATION_ABORTED: System has crashed.", false);
                if (oldAnims != null) {
                    this.mWm.setAnimationScales(oldAnims);
                }
            }
            ...
            if (statusReporter != null) {
                statusReporter.onError(ex.getMessage(), true);
            }
            throw ex;
        } catch (Throwable th) {
            if (null != null) {

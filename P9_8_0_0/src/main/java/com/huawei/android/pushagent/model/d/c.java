...
public class c {
    ...
    private boolean kh(TrsRsp trsRsp) {
        ...
        if (this.cp.getWifiMinHeartbeat() == trsRsp.getWifiMinHeartbeat() && this.cp.getWifiMaxHeartbeat() == trsRsp.getWifiMaxHeartbeat() && this.cp.get3GMinHeartbeat() == trsRsp.get3GMinHeartbeat()) {
            if (this.cp.get3GMaxHeartbeat() != trsRsp.get3GMaxHeartbeat()) {
            }
            ki(trsRsp.getAnalyticUrl());
            trsRsp.encryptRsaPubKey();
            trsRsp.encryptConnectionId();
            trsRsp.encryptDeviceId();
            deviceId = trsRsp.getDeviceId();

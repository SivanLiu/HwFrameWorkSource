...
public class SdrController {
    ...
    private static void setPropertyForKeyCode(int keyCode) {
        ...
        boolean z = false;
        if (!isGame) {
            z = SystemProperties.get("sys.aps.browserProcessName", "").isEmpty() ^ 1;
            if (!z) {
                return;
            }
        }
        ...
        if (needSetPropertyForKeyCode) {
            ...
            ApsCommon.logD(TAG, "SDR: Controller, setPropertyForKeyCode 2.5/5s, isGame:" + (isGame ? 1 : 0) + ", isBrowser:" + (z ? 1 : 0) + ", keycode: " + keyCode + ", msg: " + msg);
        }
    }
}

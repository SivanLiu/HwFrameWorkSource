...
public class SerEngine {
    public static int[] search(String sentence) {
        ...
        while (mAddr.find()) {
            int start = mAddr.start() + noBlack(mAddr.group(1), mAddr.group(2), mAddr.group(3));
            int end = mAddr.end();
            Matcher mTemp = matchers(sentence.substring(end), "pReg_city");
            if (mTemp.lookingAt()) {
                end += mTemp.end();
        ...
        while (mAddr.find()) {
            if (keyNotOnly(mAddr.group(1), mAddr.group(3), mAddr.group(4))) {
                int offset = noBlack(mAddr.group(1), mAddr.group(2), mAddr.group(3));
                start = mAddr.start();
                end = mAddr.end();
                String mat = sentence.substring(start, end);
    ...
    private static int merge(List<int[]> indx, int len) {
        ...
        while (i < len) {
            if (((int[]) indx.get(i))[2] >= ((int[]) indx.get(i + 1))[1] && ((int[]) indx.get(i))[2] < ((int[]) indx.get(i + 1))[2]) {
                ...
                if ((((int[]) indx.get(i + 1))[0] & ((int[]) indx.get(i))[0]) == 0) {
                    if (((int[]) indx.get(i))[0] != 1 || ((int[]) indx.get(i + 1))[0] != 2) {

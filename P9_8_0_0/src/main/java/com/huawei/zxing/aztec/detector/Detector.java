...
public final class Detector {
    ...
    private void extractParameters(ResultPoint[] bullsEyeCorners) throws NotFoundException {
        if (isValid(bullsEyeCorners[0]) && (isValid(bullsEyeCorners[1]) ^ 1) == 0 && (isValid(bullsEyeCorners[2]) ^ 1) == 0 && (isValid(bullsEyeCorners[3]) ^ 1) == 0) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (this.compact) {
                    parameterData = (parameterData << 7) + ((long) ((side >> 1) & AlarmInfoEx.EVERYDAY_CODE));
                } else {
    ...
    private ResultPoint[] getBullsEyeCorners(Point pCenter) throws NotFoundException {
        ...
        while (this.nbCenterLayers < 9) {
            ...
            if (this.nbCenterLayers > 2) {
                ...
                if (((double) q) < 0.75d || ((double) q) > 1.25d || (isWhiteOrBlackRectangle(pouta, poutb, poutc, poutd) ^ 1) != 0) {
                    break;
                }
            }
            pina = pouta;

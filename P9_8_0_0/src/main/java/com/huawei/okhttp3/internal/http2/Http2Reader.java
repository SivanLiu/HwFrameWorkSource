...
final class Http2Reader implements Closeable {
    ...
    private void readHeaders(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        handler.headers(endStream, streamId, -1, readHeaderBlock(lengthWithoutPadding(length, flags, readByte), readByte, flags, streamId));
    }
    ...
    private void readData(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        short readByte = (flags & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
        handler.data(inFinished, streamId, this.source, lengthWithoutPadding(length, flags, readByte));
    ...
    private void readPriority(Handler handler, int streamId) throws IOException {
        ...
        handler.priority(streamId, w1 & Integer.MAX_VALUE, (this.source.readByte() & 255) + 1, (AppOpsManagerEx.TYPE_NET & w1) != 0);
    }
    ...
    private void readPushPromise(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        short readByte = (flags & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
        handler.pushPromise(streamId, this.source.readInt() & Integer.MAX_VALUE, readHeaderBlock(lengthWithoutPadding(length - 4, flags, readByte), readByte, flags, streamId));
    }

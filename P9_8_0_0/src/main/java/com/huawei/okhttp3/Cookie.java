...
public final class Cookie {
    ...
    public boolean matches(HttpUrl url) {
        ...
        if (!this.secure || (url.isHttps() ^ 1) == 0) {
            return true;
        }
        return false;
    }
    ...
    private static boolean domainMatch(HttpUrl url, String domain) {
        ...
        if (urlHost.endsWith(domain) && urlHost.charAt((urlHost.length() - domain.length()) - 1) == '.' && (Util.verifyAsIpAddress(urlHost) ^ 1) != 0) {
            return true;
        }
        return false;
    }
    ...
    private static int dateCharacterOffset(String input, int pos, int limit, boolean invert) {
        for (int i = pos; i < limit; i++) {
            ...
            boolean dateCharacter = ((c >= 32 || c == 9) && c < AlarmInfoEx.EVERYDAY_CODE && ((c < 48 || c > 57) && ((c < 97 || c > JlogConstantsEx.JLID_DEF_CONTACT_ITEM_CLICK) && (c < 65 || c > 90)))) ? c == 58 : true;

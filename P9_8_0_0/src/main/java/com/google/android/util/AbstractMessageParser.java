...
public abstract class AbstractMessageParser {
    ...
    public static class FlickrPhoto extends Token {
        ...
        public List<String> getInfo() {
            List<String> info = super.getInfo();
            info.add(getUrl());
            info.add(getUser() != null ? getUser() : LogException.NO_VALUE);
            info.add(getPhoto() != null ? getPhoto() : LogException.NO_VALUE);
            info.add(getGrouping() != null ? getGrouping() : LogException.NO_VALUE);
            info.add(getGroupingId() != null ? getGroupingId() : LogException.NO_VALUE);
            return info;
        }
    ...
    public static class Video extends Token {
        ...
        public static String getURL(String docid, String extraParams) {
            if (extraParams == null) {
                extraParams = LogException.NO_VALUE;
    ...
    public static class YouTubeVideo extends Token {
        ...
        public static String getPrefixedURL(boolean http, String prefix, String docid, String extraParams) {
            String protocol = LogException.NO_VALUE;
            if (http) {
                protocol = "http://";
            }
            if (prefix == null) {
                prefix = LogException.NO_VALUE;
            }
            if (extraParams == null) {
                extraParams = LogException.NO_VALUE;
            } else if (extraParams.length() > 0) {
    ...
    private boolean parseURL() {
        ...
        String url = LogException.NO_VALUE;
        boolean done = false;

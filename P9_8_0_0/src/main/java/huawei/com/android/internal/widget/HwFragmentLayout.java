...
import huawei.com.android.internal.widget.HwFragmentFrameLayout.HwFragmentFrameLayoutCallback;
import java.math.BigDecimal;

public class HwFragmentLayout extends RelativeLayout implements OnTouchListener, HwFragmentFrameLayoutCallback {
    ...
    private LayoutParams mBlurLayerParams;
    ...
    private void init(Context context, float displayRate, boolean canMove) {
        ...
        if (this.mRightIn == null) {
            ...
            this.mRightIn.setAnimationListener(new AnimationListener() {
                public void onAnimationStart(Animation animation) {
        ...
        if (this.mRightOut == null) {
            ...
            this.mRightOut.setAnimationListener(new AnimationListener() {
                public void onAnimationStart(Animation animation) {
                    HwFragmentLayout.this.getRightLayout().setBackgroundColor(-197380);
                }
    ...
    private void init(Context context) {
        ...
        this.mBlurLayerParams = new LayoutParams(-1, -1);
    ...
    protected void setDisplayRate(float displayRate) {
        ...
        if (this.mLeftParams.width != leftWidth) {
            ...
            this.mSplitLineParams.width = this.mWidthSplitLineDefault;
            this.mSplitLineParams.addRule(17, LEFT_CONTENT_ID);
            this.mSplitLine.setLayoutParams(this.mSplitLineParams);
            if (this.mCanMove) {
    ...
    protected void setSelectedContainer(int selectedContainer) {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction() & 255) {
            case 0:
                ...
                this.mSplitLineParams.setMarginStart((-(this.mSplitLineParams.width - this.mWidthSplitLineDefault)) / 2);
                this.mSplitLine.setLayoutParams(this.mSplitLineParams);
    ...
    protected int calculateColumnsNumber(int orientation, int appWidth) {
        double sizeInch = calculateDeviceSize();
        double landSeparateSize = this.mLandSeparateSize > 0.0d ? this.mLandSeparateSize : 5.5d;
        double portSeparateSize = this.mPortSeparateSize > 0.0d ? this.mPortSeparateSize : 8.0d;
        ...
        if (this.mSplitMode == 2) {
            return (orientation != 2 || appWidth < dip2px(this.mContext, 592.0f)) ? 1 : 2;
        } else {
            if (sizeInch >= landSeparateSize || Math.abs(sizeInch - landSeparateSize) <= 0.0d) {
                return sizeInch - portSeparateSize > 0.0d ? orientation == 2 ? appWidth >= dip2px(this.mContext, 592.0f) ? 2 : 1 : appWidth >= dip2px(this.mContext, 533.0f) ? 2 : 1 : (orientation != 2 || appWidth < dip2px(this.mContext, 592.0f)) ? 1 : 2;
            } else {
                return 1;
            }
        }
    ...
    private IWindowManager getIWindowManager() {
        if (this.mIWindowManager == null) {
            this.mIWindowManager = Stub.asInterface(ServiceManager.checkService(FreezeScreenScene.WINDOW_PARAM));
        }

...
public class SimpleAsynchronousFileChannelImpl extends AsynchronousFileChannelImpl {
    ...
    <A> Future<FileLock> implLock(long position, long size, boolean shared, A attachment, CompletionHandler<FileLock, ? super A> handler) {
        if (shared && (this.reading ^ 1) != 0) {
            ...
            final Future pendingFuture = handler == null ? new PendingFuture(this) : null;
                        ...
                        try {
                            ...
                            if (n == 0 && (SimpleAsynchronousFileChannelImpl.this.isOpen() ^ 1) == 0) {
                                ...
                                if (completionHandler == null) {
                                    pendingFuture.setResult(fli, th);
                                    return;
                                } else {
                                    Invoker.invokeUnchecked(completionHandler, a, fli, th);
                                    return;
                                }
                            }
    ...
    <A> Future<Integer> implRead(ByteBuffer dst, long position, A attachment, CompletionHandler<Integer, ? super A> handler) {
        if (position < 0) {
            ...
            if (handler == null) {
                return CompletedFuture.withResult(Integer.valueOf(0), closedChannelException);
            }
            Invoker.invokeIndirectly((CompletionHandler) handler, (Object) attachment, Integer.valueOf(0), closedChannelException, this.executor);
            return null;
        } else {
            final Future pendingFuture = handler == null ? new PendingFuture(this) : null;
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (n >= 0 || (SimpleAsynchronousFileChannelImpl.this.isOpen() ^ 1) == 0) {
                            ...
                            SimpleAsynchronousFileChannelImpl.this.threads.remove(ti);
                            if (completionHandler == null) {
                                pendingFuture.setResult(Integer.valueOf(n), th);
                                return;
                            } else {
                                Invoker.invokeUnchecked(completionHandler, a, Integer.valueOf(n), th);
                                return;
                            }
                        }
                        throw new AsynchronousCloseException();

                    ...
                    for (Proxy proxy2 : sel.select(uri)) {
                        if (proxy2 != null && proxy2 != Proxy.NO_PROXY) {
                            if (proxy2.type() == Type.SOCKS) {
                                break;
                            } else if (proxy2.type() == Type.HTTP && ((proxy2.address() instanceof InetSocketAddress) ^ 1) == 0) {
                                sel.connectFailed(uri, proxy2.address(), new IOException("FTP connections over HTTP proxy not supported"));
                            } else {

...
public class ElemNumber extends ElemTemplateElement {
    ...
    private DecimalFormat getNumberFormatter(TransformerImpl transformer, int contextNode) throws TransformerException {
        ...
        if (this.m_groupingSeparator_avt != null) {
            evaluate = this.m_groupingSeparator_avt.evaluate(transformer.getXPathContext(), contextNode, this);
        } else {
            evaluate = null;
        }
        if (!(evaluate == null || (this.m_groupingSeparator_avt.isSimple() ^ 1) == 0 || evaluate.length() == 1)) {
        ...
        if (this.m_groupingSize_avt != null) {
            evaluate2 = this.m_groupingSize_avt.evaluate(transformer.getXPathContext(), contextNode, this);
        } else {
            evaluate2 = null;
        }
        if (evaluate == null || evaluate2 == null || evaluate.length() <= 0) {
            return null;
        }
        try {
            formatter = (DecimalFormat) NumberFormat.getNumberInstance(locale);
            formatter.setGroupingSize(Integer.valueOf(evaluate2).intValue());
            DecimalFormatSymbols symbols = formatter.getDecimalFormatSymbols();
            symbols.setGroupingSeparator(evaluate.charAt(0));
    ...
    String formatNumberList(TransformerImpl transformer, long[] list, int contextNode) throws TransformerException {
        ...
        try {
            ...
            if (this.m_format_avt != null) {
                evaluate = this.m_format_avt.evaluate(transformer.getXPathContext(), contextNode, this);
            } else {
                evaluate = null;
            }
            if (evaluate == null) {
            ...
            NumberFormatStringTokenizer formatTokenizer = new NumberFormatStringTokenizer(evaluate);
    ...
    private void getFormattedNumber(TransformerImpl transformer, int contextNode, char numberType, int numberWidth, long listElement, FastStringBuffer formattedNumber) throws TransformerException {
        String evaluate;
        if (this.m_lettervalue_avt != null) {
            evaluate = this.m_lettervalue_avt.evaluate(transformer.getXPathContext(), contextNode, this);
        } else {
            evaluate = null;
        }

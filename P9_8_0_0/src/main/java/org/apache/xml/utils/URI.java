...
public class URI implements Serializable {
    ...
    private void initialize(URI p_base, String p_uriSpec) throws MalformedURIException {
        if (p_base == null && (p_uriSpec == null || p_uriSpec.trim().length() == 0)) {
            ...
            if (colonIndex >= 0) {
                ...
                if (!(this.m_scheme == null || p_base == null || (!uriSpec.startsWith(PsuedoNames.PSEUDONAME_ROOT) && (this.m_scheme.equals(p_base.m_scheme) ^ 1) == 0 && (p_base.getSchemeSpecificPart().startsWith(PsuedoNames.PSEUDONAME_ROOT) ^ 1) == 0))) {
                    p_base = null;
                }
    ...
    private void initializeAuthority(String p_uriSpec) throws MalformedURIException {
        ...
        if (p_uriSpec.indexOf(64, 0) != -1) {
            while (index < end) {
                c = p_uriSpec.charAt(index);
                if (c == '@') {
                    break;
                }
                index++;
            }
            str = p_uriSpec.substring(0, index);
            index++;
        }
        int start = index;
        while (index < end) {
    ...
    private void initializePath(String p_uriSpec) throws MalformedURIException {
        ...
        while (index < end) {
            ...
            } else if (!(isReservedCharacter(c) || (isUnreservedCharacter(c) ^ 1) == 0 || '\\' == c)) {
        ...
        if (c == '?') {
            ...
            while (index < end) {
                ...
                } else if (!(isReservedCharacter(c) || (isUnreservedCharacter(c) ^ 1) == 0)) {
    ...
    public String getSchemeSpecificPart() {
        ...
        if (this.m_userinfo == null && this.m_host == null) {
            if (this.m_port != -1) {
            }
    ...
    public boolean equals(Object p_test) {
        if (p_test instanceof URI) {
            ...
            if ((this.m_scheme == null && testURI.m_scheme == null) || !(this.m_scheme == null || testURI.m_scheme == null || !this.m_scheme.equals(testURI.m_scheme))) {
                if (!(this.m_userinfo == null && testURI.m_userinfo == null)) {
                    if (!(this.m_userinfo == null || testURI.m_userinfo == null || !this.m_userinfo.equals(testURI.m_userinfo))) {
                    }
                }
                if (!(this.m_host == null && testURI.m_host == null)) {
                    if (!(this.m_host == null || testURI.m_host == null || !this.m_host.equals(testURI.m_host))) {
                    }
                }
                if (this.m_port == testURI.m_port) {
                    if (!(this.m_path == null && testURI.m_path == null)) {
                        if (!(this.m_path == null || testURI.m_path == null || !this.m_path.equals(testURI.m_path))) {
                        }
                    }
                    if (!(this.m_queryString == null && testURI.m_queryString == null)) {
                        if (!(this.m_queryString == null || testURI.m_queryString == null || !this.m_queryString.equals(testURI.m_queryString))) {
                        }
                    }

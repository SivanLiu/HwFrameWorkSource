...
public abstract class ToStream extends SerializerBase {
    ...
    protected void cdata(char[] ch, int start, int length) throws SAXException {
        ...
        try {
            ...
            boolean escapingNotNeeded = length >= 1 ? escapingNotNeeded(ch[start]) : false;
            if (escapingNotNeeded && (this.m_cdataTagOpen ^ 1) != 0) {
            ...
            if (escapingNotNeeded && ch[(start + length) - 1] == ']') {
                closeCDATA();
            }
            if (this.m_tracer != null) {
    ...
    private void addCdataSectionElement(String URI_and_localName, Vector v) {
        ...
        Object nextToken = tokenizer.hasMoreTokens() ? tokenizer.nextToken() : null;
        if (nextToken == null) {
            v.addElement(null);
            v.addElement(s1);
            return;
    ...
    public boolean addAttributeAlways(String uri, String localName, String rawName, String type, String value, boolean xslAttribute) {
        ...
        if (index >= 0) {
            String str = null;
            if (this.m_tracer != null) {
                str = this.m_attributes.getValue(index);
                if (value.equals(str)) {
                    str = null;
                }
            }

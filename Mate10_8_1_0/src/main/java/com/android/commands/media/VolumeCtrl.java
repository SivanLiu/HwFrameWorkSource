    ...
    public static void run(BaseCommand cmd) throws Exception {
        ...
        if (mode == VOLUME_CONTROL_MODE_ADJUST) {
            if (str == null) {
                ...
                return;
            } else if (str.equals(ADJUST_RAISE)) {
                adjDir = VOLUME_CONTROL_MODE_SET;
            } else if (str.equals(ADJUST_SAME)) {
                adjDir = 0;
            } else if (str.equals(ADJUST_LOWER)) {
                adjDir = -1;
            } else {
                cmd.showError("Error: no valid volume adjustment, was " + str + ", expected " + ADJUST_LOWER + "|" + ADJUST_SAME + "|" + ADJUST_RAISE);
                return;
            }
        }

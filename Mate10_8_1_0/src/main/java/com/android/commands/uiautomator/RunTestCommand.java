...
public class RunTestCommand extends Command {
    ...
    private void addTestClasses(String classes) {
        for (String clazz : classes.split(CLASS_SEPARATOR)) {
    ...
    private void addTestClassesFromJars() {
        ...
        if (jars != null) {
            for (String fileName : jars.split(JARS_SEPARATOR)) {
                String fileName2 = fileName2.trim();
                if (!fileName2.isEmpty()) {
                    try {
                        DexFile dexFile = new DexFile(fileName2);
                        ...
                        Log.w(LOGTAG, String.format("Could not read %s: %s", new Object[]{fileName2, e2.getMessage()}));
                    }
                }
            }
        }

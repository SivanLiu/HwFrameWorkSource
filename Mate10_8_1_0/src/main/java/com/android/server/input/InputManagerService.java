...
public class InputManagerService extends AbsInputManagerService implements Monitor {
    ...
    private final class LocalService extends InputManagerInternal {
        ...
        public void setPulseGestureEnabled(boolean enabled) {
            ...
            if (InputManagerService.this.mDoubleTouchGestureEnableFile != null) {
                ...
                try {
                    String str;
                    FileWriter writer2 = new FileWriter(InputManagerService.this.mDoubleTouchGestureEnableFile);
                    if (enabled) {
                        try {
                            str = "1";
    ...
    public InputManagerService(Context context) {
        ...
        String doubleTouchGestureEnablePath = context.getResources().getString(17039777);
    ...
    public void setInputFilter(IInputFilter filter) {
        synchronized (this.mInputFilterLock) {
            ...
            if (DEBUG_HWFLOW) {
                Slog.d(TAG, "newFilter=" + filter + ",oldFilter=" + oldFilter);
            }
            if (oldFilter != null) {
    ...
    protected boolean injectInputEventInternal(InputEvent event, int displayId, int mode, int appendPolicyFlag) {
        if (event == null) {
            ...
            try {
                int result = nativeInjectInputEvent(this.mPtr, event, displayId, pid, uid, mode, INJECTION_TIMEOUT_MILLIS, 134217728 | appendPolicyFlag);
                if (DEBUG_HWFLOW && (event instanceof KeyEvent)) {
                    Slog.d(TAG, "injectInputEvent code=" + ((KeyEvent) event).getKeyCode() + ",pid=" + pid + ",uid=" + uid + ",result=" + result);
                }
                switch (result) {
                    case 0:
                        return true;
    ...
    private void showMissingKeyboardLayoutNotification(InputDevice device) {
        if (!this.mKeyboardLayoutNotificationShown) {
            ...
            this.mNotificationManager.notifyAsUser(null, 19, new Builder(this.mContext, SystemNotificationChannels.PHYSICAL_KEYBOARD).setContentTitle(r.getString(17040957)).setContentText(r.getString(17040956)).setContentIntent(keyboardLayoutIntent).setSmallIcon(17302736).setColor(this.mContext.getColor(17170772)).build(), UserHandle.ALL);
    ...
    public void setInputWindows(InputWindowHandle[] windowHandles, InputWindowHandle focusedWindowHandle) {
        IWindow iWindow = focusedWindowHandle != null ? focusedWindowHandle.clientWindow : null;
    ...
    public void updateAccessibilityLargePointerFromSettings() {
        ...
        if (HwPCUtils.isPcCastModeInServer()) {
            ...
            String str = TAG;
            StringBuilder append = new StringBuilder().append("update Pointer, isDefaultDisplay:");
            if (getExternalContext() != null) {
                z = false;
            }
            Slog.i(str, append.append(z).append(", accessibilityConfig:").append(accessibilityConfig).toString());
            return;
        }

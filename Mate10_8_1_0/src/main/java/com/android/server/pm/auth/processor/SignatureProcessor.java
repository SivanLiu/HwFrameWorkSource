...
public class SignatureProcessor extends BaseProcessor {
    ...
    public boolean verifyCert(Package pkg, HwCertification cert) {
        if (HwAuthLogger.getHWFLOW()) {
            HwAuthLogger.i("HwCertificationManager", "SN_VC start " + (cert.isReleased() ? "released cert--" : "debugged cert--"));
        }
        ...
        try {
            ...
            if (HwAuthLogger.getHWFLOW()) {
                HwAuthLogger.i("HwCertificationManager", "SN_VC result:" + (result ? "OK" : "not the same"));
            }
            return result;

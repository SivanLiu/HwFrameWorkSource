...
public abstract class InstantAppResolver {
                ...
                void onPhaseTwoResolved(List<InstantAppResolveInfo> instantAppResolveInfoList, long startTime) {
                    ...
                    if (instantAppResolveInfoList == null || instantAppResolveInfoList.size() <= 0) {
                        ...
                        if (instantAppIntentInfo == null || instantAppIntentInfo.resolveInfo == null) {
                            ...
                            str = instantAppIntentInfo.splitName;
                            versionCode = instantAppIntentInfo.resolveInfo.getVersionCode();
                            intent = instantAppIntentInfo.failureIntent;
                        }
                    }
                    Intent installerIntent = InstantAppResolver.buildEphemeralInstallerIntent("android.intent.action.RESOLVE_INSTANT_APP_PACKAGE", instantAppRequest.origIntent, intent, instantAppRequest.callingPackage, instantAppRequest.verificationBundle, instantAppRequest.resolvedType, instantAppRequest.userId, packageName, str, instantAppRequest.responseObj.installFailureActivity, versionCode, token, false);
    ...
    public static Intent buildEphemeralInstallerIntent(String action, Intent origIntent, Intent failureIntent, String callingPackage, Bundle verificationBundle, String resolvedType, int userId, String instantAppPackageName, String instantAppSplitName, ComponentName installFailureActivity, int versionCode, String token, boolean needsPhaseTwo) {
        ...
        intent.setFlags((((268435456 | flags) | 32768) | 1073741824) | DumpState.DUMP_VOLUMES);
        if (token != null) {
        ...
        if (!needsPhaseTwo) {
            if (!(failureIntent == null && installFailureActivity == null)) {
                ...
                intent = intent;
                intent.putExtra("android.intent.extra.EPHEMERAL_FAILURE", new IntentSender(ActivityManager.getService().getIntentSender(2, callingPackage, null, null, 1, new Intent[]{onFailureIntent}, new String[]{resolvedType}, 1409286144, null, userId)));
            }

...
public class KeyValueAdbRestoreEngine implements Runnable {
    ...
    private void invokeAgentForAdbRestore(IBackupAgent agent, FileMetadata info, File restoreData, int versionCode) throws IOException {
        ...
        try {
            agent.doRestore(ParcelFileDescriptor.open(restoreData, 268435456), versionCode, ParcelFileDescriptor.open(newStateName, 1006632960), this.mToken, this.mBackupManagerService.getBackupManagerBinder());
        } catch (IOException e) {
    ...
    private void sortKeyValueData(File restoreData, File sortedData) throws IOException {
        ...
        try {
            ...
            try {
                copyKeysInLexicalOrder(new BackupDataInput(inputStream2.getFD()), new BackupDataOutput(outputStream2.getFD()));

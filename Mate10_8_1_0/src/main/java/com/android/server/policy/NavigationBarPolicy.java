...
public class NavigationBarPolicy implements OnGestureListener {
    ...
    private boolean touchDownIsValid(float pointX, float pointY) {
        ...
        boolean ret = false;
        HIT_REGION_TO_MAX = (int) (((double) this.mContext.getResources().getDimensionPixelSize(17105147)) / 3.5d);
        if (this.mMinNavigationBar) {
            updateRealSize();
            ret = this.mPolicy.mNavigationBarPosition == 4 ? pointY > ((float) (this.realSize.y - HIT_REGION_TO_MAX)) : isNaviBarNearByHole() ? pointX > ((float) ((this.realSize.x - this.mHoleHeight) - HIT_REGION_TO_MAX)) : pointX > ((float) (this.realSize.x - HIT_REGION_TO_MAX));
        }
        return ret;
    }
        ...
        if (!isFlingOnFrontNaviMode() || (isInLockTaskMode() ^ 1) == 0) {
            if (this.IS_SUPPORT_PRESSURE && IS_CHINA_AREA) {
                ...
                if (this.mMinNavigationBar) {
                    ...
                    ret = this.mPolicy.mNavigationBarPosition == 4 ? e2.getY() < ((float) (this.realSize.y - HIT_REGION_TO_TOP_BOTTOM)) : isNaviBarNearByHole() ? pointX < ((float) ((this.realSize.x - this.mHoleHeight) - HIT_REGION_TO_TOP_BOTTOM)) : pointX < ((float) (this.realSize.x - HIT_REGION_TO_TOP_BOTTOM));
                }
                if (!ret) {

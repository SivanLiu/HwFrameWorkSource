...
public class TrustSpaceManagerService extends Stub implements IHwSecurityPlugin {
    ...
    private class ScreenshotProtector extends ScreenshotProtectorCallback {
        ...
        public void notifyInfo(String projectionPack) {
            String text = TrustSpaceManagerService.this.mContext.getResources().getString(33686101);
            if (text != null) {
    ...
    private boolean checkIntent(int type, String calleePackage, int callerUid, int callerPid, String callingPackage, int userId) {
        ...
        boolean isMaliciousApp = !isMaliciousApp(callingPackage) ? isMaliciousApp(calleePackage) : true;
        if (isMaliciousApp && type != 1) {
        ...
        if (needPrevent) {
            Slog.i(TAG, "prevent Intent, type: " + typeString + " calleePackage: " + calleePackage + " calleelevel=" + calleeLevel + " callerPid:" + callerPid + " callerUid:" + callerUid + " callerPackage:" + callingPackage + " callerLevel=" + callerLevel + " isMalicious=" + isMaliciousApp + " userId:" + userId);
        }
        return needPrevent;
    }

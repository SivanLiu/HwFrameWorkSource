...
public final class JobPackageTracker {
    ...
    public void noteActive(JobStatus job) {
        ...
        addEvent(job.getJob().isPeriodic() ? 3 : 1, job.getSourceUid(), job.getBatteryName(), job.getJobId(), 0);
    }
    ...
    public void noteInactive(JobStatus job, int stopReason) {
        ...
        rebatchIfNeeded(now);
        addEvent(job.getJob().isPeriodic() ? 2 : 4, job.getSourceUid(), job.getBatteryName(), job.getJobId(), stopReason);
    }
    ...
    public float getLoadFactor(JobStatus job) {
        ...
        PackageEntry entry = this.mLastDataSets[0] != null ? this.mLastDataSets[0].getEntry(uid, pkg) : null;
        if (cur == null) {
            return 0.0f;
        }
        long now = SystemClock.uptimeMillis();

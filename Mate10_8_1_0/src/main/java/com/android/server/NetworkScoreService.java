...
public class NetworkScoreService extends Stub {
    ...
    public NetworkScoreService(Context context) {
        this(context, new NetworkScorerAppManager(context), -$Lambda$Ash-36Gr90yYPZEIENlguvJE7uk.$INST$0, Looper.myLooper());
    }
    ...
    public boolean setActiveScorer(String packageName) {
        if (isCallerSystemProcess(getCallingUid()) || (canCallerScoreNetworks() ^ 1) == 0) {
    ...
    public List<NetworkScorerAppData> getAllValidScorers() {
        if (isCallerSystemProcess(getCallingUid()) || (canCallerRequestScores() ^ 1) == 0) {
    ...
    public void disableScoring() {
        if (!isCallerActiveScorer(getCallingUid()) && (canCallerRequestScores() ^ 1) != 0) {
            throw new SecurityException("Caller is neither the active scorer nor the scorer manager.");
        }
    }

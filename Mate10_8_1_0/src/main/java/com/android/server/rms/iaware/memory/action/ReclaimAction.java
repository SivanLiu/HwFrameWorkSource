                    ...
                    for (AwareProcessInfo proc : processList) {
                        if (!(proc == null || proc.mProcInfo == null || Process.myPid() == proc.mPid)) {
                            ...
                            if (!(currentProcInfo.mProcessName == null || currentProcInfo.mProcessName.contains("launcher") || currentProcInfo.mPackageName == null || 1 != currentProcInfo.mPackageName.size())) {
                                HiberAppInfo hiberAppInfo = historyMap == null ? null : (HiberAppInfo) historyMap.get(Integer.valueOf(currentProcInfo.mPid));
                                if (hiberAppInfo == null || currentProcInfo.mUid != hiberAppInfo.mUid || !currentProcInfo.mProcessName.equals(hiberAppInfo.mProcessName) || SystemClock.uptimeMillis() - hiberAppInfo.mReclaimTime >= AwareAppMngSort.PREVIOUS_APP_DIRCACTIVITY_DECAYTIME) {

...
public class CertInstallHelper {
    ...
    private static boolean isCa(X509Certificate cert) {
        ...
        try {
            ...
            ASN1InputStream asn1InputString = new ASN1InputStream(asn1EncodedBytes);
            try {
                DEROctetString derOctetString = (DEROctetString) asn1InputString.readObject();
                if (derOctetString == null) {
                    return false;
                }
                aSN1InputStream = new ASN1InputStream(derOctetString.getOctets());
                return BasicConstraints.getInstance((ASN1Sequence) aSN1InputStream.readObject()).isCA();
            } catch (IOException e) {
                aSN1InputStream = asn1InputString;

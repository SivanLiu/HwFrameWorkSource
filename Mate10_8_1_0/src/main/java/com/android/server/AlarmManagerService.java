        ...
        public void set(String callingPackage, int type, long triggerAtTime, long windowLength, long interval, int flags, PendingIntent operation, IAlarmListener directReceiver, String listenerTag, WorkSource workSource, AlarmClockInfo alarmClock) {
            ...
            if (interval == 0 || directReceiver == null) {
                ...
                if (alarmClock != null) {
                    ...
                    if (workSource != null || (callingUid >= 10000 && callingUid != AlarmManagerService.this.mSystemUiUid && Arrays.binarySearch(AlarmManagerService.this.mDeviceIdleUserWhitelist, UserHandle.getAppId(callingUid)) < 0)) {
            ...
            while (true) {
                ...
                if ((65536 & result) != 0) {
                    ...
                    if (lastTimeChangeClockTime != 0 && nowRTC >= expectedClockTime - 1000) {
                        if (nowRTC > 1000 + expectedClockTime) {
                        }
                    }
        ...
        boolean remove(PendingIntent operation, IAlarmListener listener) {
            ...
            while (i < this.alarms.size()) {
                ...
                if (alarm.matches(operation, listener) || HwActivityManagerServiceUtil.isPendingIntentCanceled(alarm.operation)) {
                    ...
                    didRemove = true;
                    if (operation != null && (alarm.packageName == null || (alarm.packageName.equals("com.google.android.gms") ^ 1) != 0)) {
                        Slog.i(AlarmManagerService.TAG, "remove alarm.operation :" + Integer.toHexString(System.identityHashCode(alarm.operation)) + " according to operation:" + Integer.toHexString(System.identityHashCode(operation)));
                    }
        ...
        boolean remove(String packageName, String action) {
            ...
            for (int i = this.alarms.size() - 1; i >= 0; i--) {
                ...
                if (alarm.matches(packageName)) {
                    ...
                    this.alarms.remove(i);
                    didRemove = true;
                    Slog.i(AlarmManagerService.TAG, "remove " + alarm + " according to " + packageName + " (" + action + ")");
                    AlarmManagerService.this.hwRemoveRtcAlarm(alarm, true);
                    if (alarm.alarmClock != null) {
                        AlarmManagerService.this.mNextAlarmClockMayChange = true;
                    }
            ...
            while (i >= 0) {
                ...
                try {
                    if (alarm.uid == uid && ActivityManager.getService().isAppStartModeDisabled(uid, alarm.packageName)) {
                        this.alarms.remove(i);
                        didRemove = true;
                        Slog.i(AlarmManagerService.TAG, "remove " + alarm + " according to uid:" + uid);
                        if (alarm.alarmClock != null) {
                            AlarmManagerService.this.mNextAlarmClockMayChange = true;
                        }
                        i--;
        ...
        boolean remove(int userHandle) {
            ...
            while (i < this.alarms.size()) {
                ...
                if (UserHandle.getUserId(alarm.creatorUid) == userHandle) {
                    this.alarms.remove(i);
                    didRemove = true;
                    Slog.i(AlarmManagerService.TAG, "remove " + alarm + " according to uid:" + userHandle);
                    AlarmManagerService.this.hwRemoveRtcAlarm(alarm, true);
                    if (alarm.alarmClock != null) {
                        AlarmManagerService.this.mNextAlarmClockMayChange = true;
                    }
    ...
    void rescheduleKernelAlarmsLocked() {
        ...
        if (this.mAlarmBatches.size() > 0) {
            ...
            if (this.mIsAlarmDataOnlyMode || !isAwareAlarmManagerEnabled()) {
                ...
                if (!(firstWakeupEnd == -1 || this.mNextWakeup == firstWakeupEnd)) {
            ...
            for (i = 0; i < N; i++) {
                ...
                if (alarm.repeatInterval > 0) {
                    ...
                    if (nextElapsed > 0) {
                        ...
                        Flog.i(500, "nextElapsed <= 0, don't need to set again");
                    }
                }
                if (alarm.wakeup) {
                    hasWakeup = true;
        ...
        for (i = 0; i < triggerList.size(); i++) {
            ...
            if (talarm.type == 0 || 2 == talarm.type) {
                ...
                if (talarm.operation != null) {
                    ...
                    alarmInfo.append(" tag: ").append(talarm.statsTag).append(" window:").append(talarm.windowLength).append(" originWhen:").append(convertToElapsed(talarm.when, talarm.type)).append(" when:").append(talarm.whenElapsed).append(" maxWhen:").append(talarm.maxWhenElapsed);
                    if (intent != null) {
                        Log.w(TAG, "mIsScreenOn is: " + this.mIsScreenOn + ", WAKEUP alarm trigger action = " + resetActionCallingIdentity(talarm.operation) + " package name is: " + talarm.packageName + alarmInfo.toString());
                    } else {
                        Log.w(TAG, "mIsScreenOn is: " + this.mIsScreenOn + ", WAKEUP alarm intent == null and  package name is: " + talarm.packageName + " listenerTag is: " + talarm.listenerTag + " createor uid is: " + talarm.creatorUid + alarmInfo.toString());
                    }
                } else {
                    Log.w(TAG, "mIsScreenOn is: " + this.mIsScreenOn + ", WAKEUP alarm talarm.operation == null,package name is: " + talarm.packageName + " listenerTag is: " + talarm.listenerTag + " creator uid is: " + talarm.creatorUid + alarmInfo.toString());
                }
            }
        }

...
public class HwBrightnessPgSceneDetection {
    ...
    private static final int LOW_LIGHT_REFERENCE_RATIO = 44;
    ...
    private static final float PG_POWER_SAVE_MODE_LIGHT_RATIO = 0.8f;
    ...
    private Sink mStateRecognitionListener = new Sink() {
        public void onStateChanged(int stateType, int eventType, int pid, String pkg, int uid) {
            ...
            if (eventType == 1) {
                ...
                boolean isApkShouldAdjBackLight = pkg == null ? false : PGSdk.isApkShouldAdjBackLight(pkg);
                boolean pdtNotCfgAdjApp = HwBrightnessPgSceneDetection.isNoUsePowerCureAppEnable(pkg);
                if (HwBrightnessPgSceneDetection.DEBUG) {
                    Slog.d(HwBrightnessPgSceneDetection.TAG, "in onStateChanged, pkg = " + pkg + ", appType = " + appType + ", pdtCfgAdjApp = " + isApkShouldAdjBackLight + ",pdtNotCfgAdjApp=" + pdtNotCfgAdjApp);
                }
                if (pkg != null && HwBrightnessPgSceneDetection.this.mBacklightAppList != null) {
                    boolean z = HwBrightnessPgSceneDetection.this.mBacklightAppList.contains(Integer.valueOf(appType)) ? isApkShouldAdjBackLight : true;
                    if (pdtNotCfgAdjApp && z) {
                        z = false;
                    }
                    HwBrightnessPgSceneDetection.this.mShouldUpdateBLight = z ? exempStateType ^ 1 : false;
    ...
    public HwBrightnessPgSceneDetection(HwBrightnessPgSceneDetectionCallbacks callbacks, Context context) {

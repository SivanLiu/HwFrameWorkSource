...
public final class DisplayManagerService extends SystemService {
    ...
    DisplayManagerService(Context context, Injector injector) {
        ...
        this.mDefaultDisplayDefaultColorMode = this.mContext.getResources().getInteger(17694765);
    ...
    private void loadStableDisplayValuesLocked() {
        ...
        if (size.x <= 0 || size.y <= 0) {
            ...
            int width = res.getInteger(17694858);
            int height = res.getInteger(17694857);
            if (width > 0 && height > 0) {
    ...
    private void configureDisplayInTransactionLocked(DisplayDevice device) {
        ...
        if (!ownContent) {
            if (!(display == null || (display.hasContentLocked() ^ 1) == 0)) {
                int displayID = display.getDisplayIdLocked();
                boolean isPcCastModeInServerEarly = HwPCUtils.isValidExtDisplayId(displayID) ? HwPCUtils.isPcCastModeInServerEarly() : false;
    ...
    private void setExternalTouchViewport(DisplayDevice device, DisplayDeviceInfo info, LogicalDisplay display) {
        ...
        if (HwPCUtils.isPcCastModeInServer() && (this.mExternalTouchViewport.valid ^ 1) != 0 && (info.flags & 1) == 0 && display != null && HwPCUtils.isValidExtDisplayId(display.getDisplayIdLocked())) {
            try {

...
public class HwAmbientLuxFilterAlgo {
    ...
    private void updateAmbientLux(long time) {
        ...
        if (this.mData.offsetValidAmbientLuxEnable) {
            ...
            long nextDarkenTransitionOffset = nextAmbientLightDarkeningTransitionForOffset(time);
            ...
            needToBrightenOffset = needToBrightenOffset && nextBrightenTransitionOffset <= time;
            needToDarkenOffset = needToDarkenOffset && nextDarkenTransitionOffset <= time;
    ...
    private void updateSceneBufferAmbientLuxMaxMinAvg(HwRingBuffer buffer, int start, int end) {
        ...
        float luxSum = GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
    ...
    public boolean getProximityPositiveEnable() {
        return this.mData.allowLabcUseProximity ? this.mProximityPositiveStatus : false;
    }
    ...
    private long nextAmbientLightBrighteningTransitionForOffset(long time) {
        ...
        return earliestValidTime + ((long) this.mData.offsetBrightenDebounceTime);
    }

    private long nextAmbientLightDarkeningTransitionForOffset(long time) {
        long earliestValidTime = time;
        for (int i = this.mAmbientLightRingBufferFilter.size() - 1; i >= 0; i--) {
            boolean DarkenChange;
            if (this.mOffsetValidAmbientLux - this.mAmbientLightRingBufferFilter.getLux(i) >= this.mOffsetValidAmbientDarkenDeltaLux) {
                DarkenChange = true;
            } else {
                DarkenChange = false;
            }
        ...
        return earliestValidTime + ((long) this.mData.offsetDarkenDebounceTime);
    }
    ...
    private boolean decideToDarkenForOffset(float ambientLux) {
        ...
        return needToDarken ? this.mAutoBrightnessIntervened ^ 1 : false;
    }

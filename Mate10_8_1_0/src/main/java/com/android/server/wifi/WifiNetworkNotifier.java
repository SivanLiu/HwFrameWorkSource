...
public class WifiNetworkNotifier {
    ...
    private class NotificationEnabledSettingObserver extends ContentObserver {
        ...
        private boolean getValue() {
            return WifiNetworkNotifier.this.mFrameworkFacade.getIntegerSetting(WifiNetworkNotifier.this.mContext, "wifi_networks_available_notification_on", 1) == 1;
        }
    }
    ...
    private void showAvailableNetworkNotification() {
        ...
        CharSequence title = resource.getText(33686105);
        CharSequence message = resource.getText(33686104);
        Builder builder = new Builder(this.mContext, SystemNotificationChannels.NETWORK_AVAILABLE);
        builder.setWhen(0).setSmallIcon(17303456).setContentTitle(title).setContentText(message).setStyle(new BigTextStyle().bigText(message)).setContentIntent(pi).setAutoCancel(true);

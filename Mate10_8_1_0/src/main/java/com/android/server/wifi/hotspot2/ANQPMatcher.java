...
public class ANQPMatcher {
    ...
    private static int matchNAIRealmData(NAIRealmData realmData, String realm, int eapMethodID, AuthParam authParam) {
        ...
        if (realmMatch == -1 || realmData.getEAPMethods().isEmpty()) {
            return realmMatch;
        }
    ...
    private static int matchEAPMethod(EAPMethod method, int eapMethodID, AuthParam authParam) {
        ...
        Set<AuthParam> paramSet = (Set) method.getAuthParams().get(Integer.valueOf(authParam.getAuthTypeID()));
        if (paramSet == null || (paramSet.contains(authParam) ^ 1) != 0) {
            return -1;
        }
        return 3;
    }

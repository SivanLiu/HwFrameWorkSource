...
public class HwWifiService extends WifiServiceImpl {
        ...
        switch (code) {
            case 1001:
                ...
                if (this.mWifiStateMachine instanceof HwWifiStateMachine) {
                    result2 = ((HwWifiStateMachine) this.mWifiStateMachine).getWpaSuppConfig();
                }
            ...
            case 1005:
                ...
                if (this.mWifiStateMachine instanceof HwWifiStateMachine) {
                    ...
                    if (list == null) {
                        ...
                        Slog.d(TAG, "HwWifiService getApLinkedStaList result = " + list.toString().replaceAll(regularIP, ".*.*.").replaceAll(regularMAC, ":**:**:"));
                    }
                }
                reply.writeNoException();
            ...
            case 1011:
                ...
                byte[] result3 = null;
                if (this.mWifiStateMachine instanceof HwWifiStateMachine) {
                    result3 = ((HwWifiStateMachine) this.mWifiStateMachine).fetchWifiSignalInfoForVoWiFi();
                }
                reply.writeNoException();
            ...
            case CODE_GET_VOWIFI_DETECT_MODE /*1013*/:
                ...
                if (this.mWifiStateMachine instanceof HwWifiStateMachine) {
                    result4 = ((HwWifiStateMachine) this.mWifiStateMachine).getVoWifiDetectMode();
                }
                reply.writeNoException();
                if (result4 != null) {
                    reply.writeInt(1);
            ...
            case 2008:
                ...
                if (this.mWifiStateMachine instanceof HwWifiStateMachine) {
                    mHwWifiStateMachine = (HwWifiStateMachine) this.mWifiStateMachine;
                    if (wifiServiceUtils.getWifiStateMachineChannel(this) != null) {
                        z = mHwWifiStateMachine.isRSDBSupported();
                    } else {
                        Slog.e(TAG, "Exception mWifiStateMachineChannel is not initialized");
                    }
                }
                reply.writeNoException();
                reply.writeBooleanArray(new boolean[]{z});
                return true;
            case CODE_WIFI_QOE_START_MONITOR /*3001*/:
                ...
                if (mHwQoEService != null) {
                    z = mHwQoEService.registerHwQoEMonitor(monitorType, period, callback);
                }
                reply.writeNoException();
            ...
            case CODE_WIFI_QOE_STOP_MONITOR /*3002*/:
                ...
                mHwQoEService = HwQoEService.getInstance();
                if (mHwQoEService != null) {
                    z = mHwQoEService.unRegisterHwQoEMonitor(monitorType);
                }
                reply.writeNoException();
                reply.writeInt(z ? 1 : 0);
                return true;
            case CODE_WIFI_QOE_EVALUATE /*3003*/:
                ...
                mHwQoEService = HwQoEService.getInstance();
                if (mHwQoEService != null) {
                    z = mHwQoEService.evaluateNetworkQuality(callback);
                }
                reply.writeNoException();
                reply.writeInt(z ? 1 : 0);
                return true;
            case CODE_WIFI_QOE_UPDATE_STATUS /*3004*/:
                ...
                mHwQoEService = HwQoEService.getInstance();
                if (mHwQoEService != null) {
                    z = mHwQoEService.updateVOWIFIState(state);
                }
                reply.writeNoException();
                reply.writeInt(z ? 1 : 0);
                return true;
            ...
            case CODE_IS_BG_LIMIT_ALLOWED /*3008*/:
                ...
                if (qoeService_check_bg_limit != null) {
                    z = qoeService_check_bg_limit.isBgLimitAllowed(uid);
                }
                reply.writeNoException();
                reply.writeBooleanArray(new boolean[]{z});
                return true;
            ...
            case CODE_DISABLE_RX_FILTER /*3021*/:
                ...
                if (this.mContext.checkCallingPermission(ACCESS_WIFI_FILTER_PERMISSION) != 0) {
                    ...
                    z = disableWifiFilter(data.readStrongBinder());
                    reply.writeNoException();
                    reply.writeBooleanArray(new boolean[]{z});
                    return true;
                }
            case CODE_ENABLE_RX_FILTER /*3022*/:
                ...
                if (this.mContext.checkCallingPermission(ACCESS_WIFI_FILTER_PERMISSION) != 0) {
                    ...
                    z = enableWifiFilter(data.readStrongBinder());
                    reply.writeNoException();
                    reply.writeBooleanArray(new boolean[]{z});
                    return true;
                }
    ...
    private boolean updateWifiFilterState() {
        synchronized (this.mFilterSynchronizeLock) {
            boolean startRxFilter;
            if (this.mFilterLockList.size() == 0) {
                if (this.isRxFilterDisabled) {
                    Slog.d(TAG, "enableWifiFilter");
                    this.isRxFilterDisabled = false;
                    startRxFilter = WifiInjector.getInstance().getWifiNative().startRxFilter();
                    return startRxFilter;
                }
            } else if (!this.isRxFilterDisabled) {
                Slog.d(TAG, "disableWifiFilter");
                this.isRxFilterDisabled = true;
                startRxFilter = WifiInjector.getInstance().getWifiNative().stopRxFilter();
                return startRxFilter;
            }
        }
    }
    ...
    protected boolean limitForegroundWifiScanRequest(String packageName, int uid) {
        ...
        try {
            netLocationStrategy = HwServiceFactory.getNetLocationStrategy(packageName, uid, 1);
            if (netLocationStrategy == null) {
                Slog.e(TAG, "Get wifiScanStrategy from iAware is null.");
                return false;
            }
            Slog.d(TAG, "Get wifiScanStrategy from iAware, WifiScanStrategy = " + netLocationStrategy);
            if (netLocationStrategy.getCycle() == -1) {
                return true;
            }
            if (netLocationStrategy.getCycle() == 0) {
                return false;
            }
            if (netLocationStrategy.getCycle() <= 0) {
                Slog.e(TAG, "Invalid wifiScanStrategy.");
                return false;
            } else if (this.lastScanResultsAvailableTime > netLocationStrategy.getTimeStamp()) {
                long msSinceLastScan = this.mClock.getElapsedSinceBootMillis() - this.lastScanResultsAvailableTime;
                if (msSinceLastScan <= netLocationStrategy.getCycle()) {
                    return true;
                }

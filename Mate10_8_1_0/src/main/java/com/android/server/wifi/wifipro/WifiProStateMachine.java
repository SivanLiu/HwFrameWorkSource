...
public class WifiProStateMachine extends StateMachine implements INetworkQosCallBack, INetworksHandoverCallBack, IWifiProUICallBack, IDualBandManagerCallback {
    ...
    private static final int[] QUICK_SCAN_INTERVAL = new int[]{HwQoEWeChatBooster.WECHART_COLLECT_HANDOVER_INFO_TIME, HwQoEWeChatBooster.WECHART_COLLECT_HANDOVER_INFO_TIME, 15000};
    ...
    class WiFiLinkMonitorState extends State {
        ...
        private void handleCheckResultInLinkMonitorState(Message msg) {
            if (!WifiProStateMachine.this.isFullscreen() && !WifiProStateMachine.this.mIsWiFiNoInternet && (((Boolean) msg.obj).booleanValue() ^ 1) == 0 && !this.isWiFiHandoverPriority && !this.isWifi2WifiProcess) {
                ...
                if (curRssiLevel < 3) {
                    int targetRssiLevel = WifiProStateMachine.this.mWiFiProEvaluateController.calculateSignalLevelHW(true, Integer.valueOf(msg.arg1).intValue());
        ...
        private void tryWifiHandoverPreferentially(int curRssiLevel) {
            if (curRssiLevel <= 2) {
                if (curRssiLevel < 1 && (WifiProStateMachine.this.mIsScanedRssiLow ^ 1) != 0) {
                    WifiProStateMachine.this.mIsScanedRssiLow = true;
                } else if (curRssiLevel >= 1 && (WifiProStateMachine.this.mIsScanedRssiMiddle ^ 1) != 0) {
                    WifiProStateMachine.this.mIsScanedRssiMiddle = true;
                } else {
                    return;
                }
                this.isWiFiHandoverPriority = true;
                WifiProStateMachine.this.logW("try wifi --> wifi only, current rssi = " + WifiProStateMachine.this.mCurrentRssi);
        ...
        private void tryWifi2Mobile(int mobile_level) {
            ...
            if (!this.isWifi2WifiProcess && (WifiProStateMachine.this.isAllowWifi2Mobile() ^ 1) == 0 && (this.isAllowWiFiHandoverMobile ^ 1) == 0 && (WifiProStateMachine.this.mPowerManager.isScreenOn() ^ 1) == 0 && !WifiProCommonUtils.isCalling(WifiProStateMachine.this.mContext)) {
        ...
        private void handleWiFiRoveOut() {
            ...
            if (HwQoEService.getInstance().isWeChating() && (WifiProStateMachine.this.mIsWiFiNoInternet ^ 1) != 0) {
                WifiProStateMachine.this.logW("isWeChating so rove out handler by Streaming !");
                if (HwQoEService.getInstance().isHandoverToMobile()) {
                    wifi2WifiFailed();
                    return;
                }
                ...
                WifiProStateMachine.this.logW("isWeChating do not handover currRssiLevel = " + currRssiLevel);
                if (currRssiLevel <= 2) {
                    this.isSwitching = false;
                    return;
                }
    ...
    class WiFiProEnableState extends State {
        ...
        private void handleDualBandNetworkType(Message msg) {
            ...
            if (r1 == null || r1.size() == 0) {
                ...
                WifiProStateMachine.this.logD("onDualBandNetWorkType type = " + type + " apList.size() = " + r1.size());
                WifiProStateMachine.this.mDualBandMonitorApList.clear();
                WifiProStateMachine.this.mDualBandMonitorInfoSize = r1.size();
                for (HwDualBandMonitorInfo monitorInfo : r1) {
        ...
        private void handleWifiQosChangedInVerifyLinkState(Message msg) {
            if (!((Boolean) msg.obj).booleanValue()) {
                ...
                if (this.wifiQosLevel > 2 && (WifiProStateMachine.this.mIsWiFiNoInternet ^ 1) != 0 && this.isWifiGoodIntervalTimerOut) {
                    WifiProStateMachine.this.logD("wifi Qos is [" + this.wifiQosLevel + " ]Ok, start check wifi internet, wifiNoInternetCounter = " + this.wifiNoInternetCounter);
                    this.isRecoveryWifi = true;
                    this.isWifiGoodIntervalTimerOut = false;
                    this.wifiNoInternetCounter = this.wifiNoInternetCounter > 12 ? 12 : this.wifiNoInternetCounter;
    ...
    class WifiConnectedState extends State {
        ...
        private boolean isHiLinkUnconfigRouter() {
            int result = 0;
            if (!(WifiProStateMachine.this.mContext == null || (TextUtils.isEmpty(WifiProStateMachine.this.mCurrentSsid) ^ 1) == 0)) {
            ...
            WifiProStateMachine.this.logD("isHiLinkUnconfigRouter, getHiLinkSsidType = " + result);
            if (result == 1) {
                return true;
            }
            return false;
    ...
    class WifiSemiAutoScoreState extends State {
            ...
            switch (msg.what) {
                ...
                case WifiProStateMachine.EVENT_EVALUATE_START_CHECK_INTERNET /*136307*/:
                    ...
                    if (this.checkCounter == 0) {
                        ...
                        WifiProStateMachine.this.mWiFiProCHRMgr.updateWifiException(122, "BG_CONN_AP_TIME_LEN");
                        WifiProStateMachine.this.removeMessages(WifiProStateMachine.EVENT_WIFI_EVALUTE_CONNECT_TIMEOUT);
                    }
                    this.checkTime = System.currentTimeMillis();
                    this.checkCounter++;
        ...
        private int handleWifiCheckResult(int result) {
            if (7 != result) {
                ...
                WifiProStateMachine.this.mWiFiProCHRMgr.updateWifiException(122, "BG_AC_TIME_LEN");
            }
            if (5 == result) {
                WifiProStateMachine wifiProStateMachine = WifiProStateMachine.this;
    ...
    private boolean isKeepCurrWiFiConnected() {
        ...
        if (this.mIsVpnWorking || this.mIsUserHandoverWiFi || this.mHiLinkUnconfig || ((this.mIsUserManualConnectSuccess && (this.mIsWiFiProEnabled ^ 1) != 0) || isAppinWhitelists())) {
            return true;
        }
        return isWifiRepeaterOn();
    }

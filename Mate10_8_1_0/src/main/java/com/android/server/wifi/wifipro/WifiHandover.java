...
public class WifiHandover {
    ...
    private int notifyGoodApFound() {
        ...
        for (int i = 0; i < scanResults.size(); i++) {
            ...
            if (!equals && !r16 && !this.mNetworkBlackListManager.isInAbnormalWifiBlacklist(nextResult.BSSID) && HwFrameworkFactory.getHwInnerWifiManager().calculateSignalLevelHW(nextResult.frequency, nextResult.level) > 2 && nextResult.level > nextRssi) {
    ...
    private WifiSwitchCandidate selectQualifiedCandidate() {
        ...
        int currentLevel = WifiProCommonUtils.getCurrenSignalLevel(this.mWifiManager.getConnectionInfo());
        if (scanResults == null || scanResults.size() == 0) {
            ...
            for (int i = 0; i < scanResults.size(); i++) {
                ...
                int isSameEncryptType2 = this.mNetwoksHandoverState == 0 ? currentSsid.equals(scanSsid) ? WifiProCommonUtils.isSameEncryptType(scanResultEncrypt, currentEncrypt) ^ 1 : 1 : 0;
                int signalLevel = HwFrameworkFactory.getHwInnerWifiManager().calculateSignalLevelHW(nextResult.frequency, nextResult.level);
                if (!equals && !r23 && r14 == 0 && signalLevel > 2) {

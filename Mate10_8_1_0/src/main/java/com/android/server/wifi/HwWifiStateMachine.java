...
public class HwWifiStateMachine extends WifiStateMachine {
                    ...
                    for (ScanDetail scanned : this.lstScanRet) {
                        if (scanned != null) {
                            ...
                            if (!(strBssid == null || strBssid.isEmpty() || !strCurBssid.equals(strBssid))) {
                                ...
                                hwmCHR.updateWifiException(213, "{BSSID:\"" + strCurBssid + "\"," + HwWifiStateMachine.AP_CAP_KEY + ":" + (((scanned.getNetworkDetail().getPrimaryFreq() / 1000) * 10) + Math.abs(((stream1 + stream2) + stream3) + stream4)) + "," + HwWifiStateMachine.TX_MCS_SET + ":" + scanned.getNetworkDetail().getTxMcsSet() + "}");
    ...
    public HwWifiStateMachine(Context context, FrameworkFacade facade, Looper looper, UserManager userManager, WifiInjector wifiInjector, BackupManagerProxy backupManagerProxy, WifiCountryCode countryCode, WifiNative wifiNative, WrongPasswordNotifier wrongPasswordNotifier) {
        super(context, facade, looper, userManager, wifiInjector, backupManagerProxy, countryCode, wifiNative, wrongPasswordNotifier);
        HwWifiCHRStateManager hwWifiCHR = HwWifiServiceFactory.getHwWifiCHRStateManager();
        if (hwWifiCHR != null && (hwWifiCHR instanceof HwWifiCHRStateManagerImpl)) {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (!HwWifiStateMachine.this.mWifiSwitchOnGoing && changeConfig != null) {
                    if (switchType == 1) {
                        ...
                        HwWifiStateMachine.this.startConnectToUserSelectNetwork(changeConfig.networkId, Binder.getCallingUid(), null);
                    } else {
    ...
    public void handleConnectFailedInWifiPro(int netId, int disableReason) {
        if (this.mWifiSwitchOnGoing && disableReason >= 2 && disableReason <= 4) {
            ...
            sendWifiHandoverCompletedBroadcast(status, str, failedSsid, null);
        }
    }
    ...
    public void handleDualbandHandoverFailed(int disableReason) {
        if (this.mWifiSwitchOnGoing && disableReason == 3 && WifiProStateMachine.getWifiProStateMachineImpl().getNetwoksHandoverType() == 4) {
            ...
            sendWifiHandoverCompletedBroadcast(-7, str, failedSsid, null);
        }
    }
    ...
    protected void handleSimAbsent(WifiConfiguration config) {
        ...
        if (PreconfiguredNetworkManager.IS_R1 && config.enterpriseConfig != null && TelephonyUtil.isSimEapMethod(config.enterpriseConfig.getEapMethod()) && PreconfiguredNetworkManager.getInstance().isPreconfiguredNetwork(config.SSID)) {
            ...
            this.wifiEapUIManager.showDialog(Resources.getSystem().getString(33686093), Resources.getSystem().getString(33686091));
        }
    }
    ...
    protected void handleEapErrorcodeReport(int networkId, String ssid, int errorCode) {
        if (PreconfiguredNetworkManager.IS_R1 && PreconfiguredNetworkManager.getInstance().isPreconfiguredNetwork(ssid)) {
            wifiStateMachineUtils.getWifiConfigManager(this).updateNetworkSelectionStatus(networkId, 14);
    ...
    private void setLowPwrMode(boolean isConnected, String ssid, boolean isMobileAP, boolean isScreenOn) {
        ...
        if (!isConnected || (!r3 && (!(isMobileAP && isAndroidMobileAP() && (r2 ^ 1) != 0) && (isScreenOn ^ 1) == 0))) {

    ...
    class InternetReadyState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case 7:
                    if (HwDualBandBlackListManager.getHwDualBandBlackListMgrInstance().getWifiBlacklist().isEmpty() && HwDualBandStateMachine.this.mWifiManager != null) {
                        List<ScanResult> mLists = HwDualBandStateMachine.this.mWifiManager.getScanResults();
                        if (mLists != null && mLists.size() > 0 && is5gApAvailble(mLists)) {
                ...
                case 16:
                    ...
                    if (!(mWifiInfo == null || mWifiInfo.getBSSID() == null)) {
                        ...
                        if (apinfo != null) {
                            List<WifiProRelateApRcd> mLists2 = apinfo.getRelateApRcds();
                            if (mLists2.size() > 0) {
                                Log.e(HwDualBandMessageUtil.TAG, "MSG_DUAL_BAND_WIFI_TYPE_SINGLE mLists.size() = " + mLists2.size());
                                WifiProRelateApRcd info = (WifiProRelateApRcd) mLists2.get(0);
                    ...
                    if (signalLevel >= 3) {
                        for (int k = 0; k < configNetworksSize; k++) {
                            ...
                            if (nextConfig == null || nextConfig.SSID == null || !nextConfig.SSID.equals(scanSsid)) {
                                ...
                                networkMatched = WifiProCommonUtils.isSameEncryptType(nextResult.capabilities, nextConfig.configKey());
                            }
                            if (networkMatched && (nextConfig.noInternetAccess ^ 1) != 0 && (WifiProCommonUtils.isOpenAndPortal(nextConfig) ^ 1) != 0) {
                                return true;
                            }
                        }
                        continue;
            ...
            for (HwDualBandMonitorInfo info : HwDualBandStateMachine.this.mTargetAPList) {
                if (info.mDualBandApInfoRcd == null) {
                    ...
                    if (!(RelationInfo == null || wifiProDualBandApInfoRcd == null)) {
                        ...
                        Log.e(HwDualBandMessageUtil.TAG, "MonitorState mix AP 2.4G = " + mWifiInfo.getSSID() + " 5G = " + info.mSsid + " info.mScanRssi = " + info.mScanRssi + " info.mTargetRssi = " + info.mTargetRssi + " info.mAuthType = " + info.mAuthType + " info.mIsNearAP = " + info.mIsNearAP + " APInfo.mChannelFrequency = " + wifiProDualBandApInfoRcd.mChannelFrequency + " APInfo.mApAuthType = " + wifiProDualBandApInfoRcd.mApAuthType);
                        HwDualBandStateMachine.this.mMonitorAPList.add(info);
                    }
                }
            }

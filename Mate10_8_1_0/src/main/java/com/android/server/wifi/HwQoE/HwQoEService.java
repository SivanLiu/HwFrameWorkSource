...
public class HwQoEService implements IHwQoEContentAwareCallback {
    ...
    Runnable mCheckFGApp = new Runnable() {
        public void run() {
            if (!HwQoEService.this.isKOGInRateAjustMode || HwQoEService.isKOGInFg(HwQoEService.this.mContext)) {
                if (HwQoEService.this.isGameKOGInWar && HwQoEService.isKOGInFg(HwQoEService.this.mContext)) {
    ...
    public void updateWifiConnectionMode(boolean isUserManualConnect, boolean isUserHandoverWiFi) {
        this.mHwQoEWeChatBooster.updateWifiConnectionMode(isUserManualConnect, isUserHandoverWiFi);
    }
    ...
    private void setGameKOGMode(int mode) {
        if ((6 != mode && 7 != mode) || (isPermitUpdateWifiPowerMode(mode) ^ 1) == 0) {
            ...
            this.mHwQoEWiFiOptimization.hwQoESetMode(mode);
        }
    }
    ...
    public boolean updateAppExperienceStatus(int uid, int experience, long rtt, int reserved) {
        ...
        if (isWifi()) {
            if (rtt == 460) {
                ...
                if (this.gameRttNotifyCountInOneGame < 3 && gameKOGCurrentTime - this.gameKOGChrLastReportTime > GAME_RTT_NOTIFY_INTERVAL && this.isGameKOGInWar) {
                    ...
                    this.gameKOGTcpRttDuration = gameKOGTcpMonitor.getRttDuration(uid, 1);
                    this.gameKOGTcpRttSegs = gameKOGTcpMonitor.getRttSegs(uid, 1);
        ...
        if (rtt > 400) {
            gameKOGTcpMonitor = HwUidTcpMonitor.getInstance(this.mContext);
            this.gameKOGTcpRttDuration = gameKOGTcpMonitor.getRttDuration(uid, 0);
            this.gameKOGTcpRttSegs = gameKOGTcpMonitor.getRttSegs(uid, 0);
            if (0 != this.gameKOGTcpRttSegs) {
                this.gameKOGTcpRtt = this.gameKOGTcpRttDuration / this.gameKOGTcpRttSegs;
            } else {
                this.gameKOGTcpRtt = 0;
            }
            sendIntentDsKOGRttBigLatency(this.mContext, rtt, this.gameKOGTcpRtt);
        }
    ...
    private void enableLowPowerMode() {
        ...
        if (this.mWifiManager != null && this.mWifiManager.getConnectionInfo() != null) {
            ...
            if ((ssid != null && ssid.equals(hwSsid)) || (isMobileAP && r1)) {
                setGameKOGMode(7);
            }
        }
    }
                ...
                switch (msg.what) {
                    ...
                    case HwQoEUtils.QOE_MSG_UPDATE_UID_TCP_RTT /*123*/:
                        if (HwQoEService.this.mCurNetworkType == 0) {
                            HwQoEService.this.mGameKOGTcpMonitor.updateUidTcpStatistics();
                        }
                        ...
                        HwQoEUtils.logD("QOE_MSG_UPDATE_UID_TCP_RTT gameKOGTcpRttSegs:" + HwQoEService.this.gameKOGTcpRttSegs + ", lastGameKOGTcpRttSegs:" + HwQoEService.this.lastGameKOGTcpRttSegs + ", mForegroundAppUid:" + HwQoEService.this.mForegroundAppUid);
                    ...
                    case HwQoEUtils.QOE_MSG_BT_SCAN_STARTED /*125*/:
                        ...
                        break;
                    case 126:
                        if (HwQoEService.this.isGameKOGInWar && !HwQoEService.this.isKOGInBackGround) {
                            HwQoEService.this.setGameKOGMode(4);
    ...
    private void sendIntentDsKOGRttBigLatency(Context context, long rtt, long tcprtt) {
        try {
            Date now = new Date();
    ...
    public boolean isConnectWhenWeChating(ScanResult scanResult) {
        return this.mHwQoEWeChatBooster.isConnectWhenWeChating(scanResult);
    }

    public boolean isWeChating() {
        return this.mHwQoEWeChatBooster.isWeChating();
    }
    ...
    public boolean isHandoverToMobile() {
        return this.mHwQoEWeChatBooster.isHandoverToMobile();
    }
    ...
    public void onPeriodSpeed(long outSpeed, long inSpeed) {
        this.mHwQoEWeChatBooster.onPeriodSpeed(outSpeed, inSpeed);
    }

    public void onSensitiveAppStateChange(int uid, int state, boolean isBackground) {
        this.mHwQoEWeChatBooster.onSensitiveAppStateChange(uid, state, isBackground);
    }
    ...
    public void onForegroundAppTypeChange(int type, String appName) {
        ...
        if (this.mPreviousUid != 0) {
            int i = this.gameKOGUid;
            int i2 = this.mPreviousUid;
        }
        if (this.mHwQoEContentAware.isGameType(type, appName)) {
            HwQoEUtils.logD("APP_TYPE_GAME, isKOGInBackGround = " + this.isKOGInBackGround);
            this.gameKOGUid = this.mHwQoEContentAware.getAppUid(appName);
            this.mPreviousUid = this.gameKOGUid;
        ...
        this.isGameNeedDis = false;
        this.mQoEHandler.removeMessages(HwQoEUtils.QOE_MSG_WIFI_DELAY_DISCONNECT);
        this.mPreviousUid = this.mHwQoEContentAware.getAppUid(appName);
        restartABSHandover();
    }
    ...
    private void restartABSHandover() {
        if (HwABSUtils.getABSEnable()) {
            HwABSDetectorService mHwABSDetectorService = HwABSDetectorService.getInstance();
            if (mHwABSDetectorService != null) {

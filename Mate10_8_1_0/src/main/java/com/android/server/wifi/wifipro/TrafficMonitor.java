...
public class TrafficMonitor {
    ...
    private static final int[] TIMER = new int[]{2000, ClientHandler.DEFAULT_WIFI_ROUTE_CHECK_TIME_FIRST, ApInfoManager.SCAN_INTERVAL_NORMAL_3, ClientHandler.DEFAULT_WIFI_ROUTE_CHECK_TIME_FIRST, 1000, 1000};
    ...
    private int getTimerVal() {
        int i = this.mTimerLen > 0 ? this.mTimerLen : TIMER[0];
        int count = this.txPktLst.size();
        int idle_conut = 0;
        if (count > 10) {
            int i2 = 1;
            while (i2 <= 10 && ((Integer) this.txPktLst.get(count - i2)).intValue() == 0) {
                idle_conut++;
                i2++;
            }
        }

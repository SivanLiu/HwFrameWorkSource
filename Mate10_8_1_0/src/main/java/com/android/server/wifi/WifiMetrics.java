...
public class WifiMetrics {
    ...
    private static class StaEventWithTime {
        ...
        public String toString() {
            ...
            sb.append(" ").append(WifiMetrics.staEventToString(this.staEvent));
            return sb.toString();
        }
    }
            ...
            if (!(this.mWifiConfigManager == null || this.mWifiNetworkSelector == null)) {
                if (this.mPasspointManager != null) {
                    ...
                    if (isFullBand) {
                        ...
                        for (ScanDetail scanDetail : scanDetails) {
                            ...
                            if (!this.mWifiNetworkSelector.isSignalTooWeak(scanResult)) {
                                ...
                                if (networkDetail.isInterworking()) {
                                    ...
                                    obj = providerMatch != null ? providerMatch.first : null;
                                }
                                ssids.add(matchInfo);
                                bssids++;
                                ...
                                if (config == null || (config.isEphemeral() ^ 1) == 0) {
                                    isSaved = 0;
                                } else {
                                    isSaved = config.isPasspoint() ^ 1;
                                }
                                boolean isSavedPasspoint = obj != null;
                                if (isOpen) {
                                ...
                                if (isSavedPasspoint) {
                                    savedPasspointProviderProfiles.add(obj);
                        ...
                        incrementSsid(this.mAvailableSavedPasspointProviderProfilesInScanHistogram, savedPasspointProviderProfiles.size());
                        incrementBssid(this.mAvailableSavedPasspointProviderBssidsInScanHistogram, savedPasspointProviderBssids);
                        return;
                    }
                    wifiLog = this.mWifiLogProto;
                    wifiLog.partialAllSingleScanListenerResults++;
                }
            }
        }
    }
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        synchronized (this.mLock) {
            ...
            for (StaEventWithTime event3 : this.mStaEventList) {
                pw.println(event3);
            }
    ...
    private void consolidateProto(boolean incremental) {
        ...
        synchronized (this.mLock) {
            ...
            for (i = 0; i < this.mConnectToNetworkNotificationCount.size(); i++) {
                ConnectToNetworkNotificationAndActionCount keyVal4 = new ConnectToNetworkNotificationAndActionCount();
                keyVal4.notification = this.mConnectToNetworkNotificationCount.keyAt(i);
                keyVal4.recommender = 1;
                keyVal4.count = this.mConnectToNetworkNotificationCount.valueAt(i);
                notificationCountArray[i] = keyVal4;
            }
            ...
            for (i = 0; i < this.mConnectToNetworkNotificationActionCount.size(); i++) {
                keyVal4 = new ConnectToNetworkNotificationAndActionCount();
                int key = this.mConnectToNetworkNotificationActionCount.keyAt(i);
                keyVal4.notification = key / CONNECT_TO_NETWORK_NOTIFICATION_ACTION_KEY_MULTIPLIER;
                keyVal4.action = key % CONNECT_TO_NETWORK_NOTIFICATION_ACTION_KEY_MULTIPLIER;
                keyVal4.recommender = 1;
                keyVal4.count = this.mConnectToNetworkNotificationActionCount.valueAt(i);
                notificationActionCountArray[i] = keyVal4;
            }

...
public class WificondControl {
    ...
    public IClientInterface setupDriverForClientMode() {
        ...
        try {
            ...
            try {
                ...
                this.mScanEventHandler = new ScanEventHandler();
                this.mWificondScanner.subscribeScanEvents(this.mScanEventHandler);
                this.mPnoScanEventHandler = new PnoScanEventHandler();
    ...
    public IApInterface setupDriverForSoftApMode() {
        ...
        try {
            ...
            try {
                ...
                this.mApLinkedEventHandler = new ApLinkedEventHandler();
    ...
    public ArrayList<ScanDetail> getScanResults(int scanType) {
        ...
        for (NativeScanResult result : nativeResults) {
            ...
            try {
                str = NativeUtil.macAddressFromByteArray(result.bssid);
                ScanResultRecords.getDefault().recordOriSsid(str, wifiSsid.toString(), result.ssid);
                ...
                if (str == null) {
                    ...
                    networkDetail = new NetworkDetail(str, ies, null, result.frequency);
                    try {
                        if (!wifiSsid.toString().equals(networkDetail.getTrimmedSSID())) {
                            Log.d(TAG, String.format("Inconsistent SSID on BSSID '%s': '%s' vs '%s' ", new Object[]{str, wifiSsid.toString(), networkDetail.getTrimmedSSID()}));
                        }
                        ScanDetail scanDetail = new ScanDetail(networkDetail, wifiSsid, str, flags, result.signalMbm / 100, result.frequency, result.tsf, ies, null);
                        ...
                        if (hiLinkNetwork.isHiLinkNetwork || (ScanResultRecords.getDefault().isHiLink(str) ^ 1) == 0) {
                            scanDetail.getScanResult().isHiLinkNetwork = true;
                            ScanResultRecords.getDefault().recordHiLink(str);
                        } else {
                ...
                Log.e(TAG, "Illegal argument for scan result with bssid: " + str, e2);
                networkDetail = null;
            }
        }
        String result2 = "";

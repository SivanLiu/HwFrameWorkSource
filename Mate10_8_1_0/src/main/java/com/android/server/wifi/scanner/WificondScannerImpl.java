...
public class WificondScannerImpl extends WifiScannerImpl implements Callback {
        ...
        private boolean setPnoState(boolean enable) {
            ...
            if (!this.mWaitForTimer) {
                ...
                if (timeDifference >= 5000) {
                    ...
                    this.mAlarmManager.set(3, this.mClock.getElapsedSinceBootMillis() + alarmTimeout, PNO_DEBOUNCER_ALARM_TAG, this.mAlarmListener, this.mEventHandler);
    ...
    public WificondScannerImpl(Context context, WifiNative wifiNative, WifiMonitor wifiMonitor, ChannelHelper channelHelper, Looper looper, Clock clock) {
        ...
        this.mHwPnoScanSupported = this.mContext.getResources().getBoolean(17957062);
            ...
            if (this.mLastScanSettings == null || (this.mLastScanSettings.hwPnoScanActive ^ 1) == 0) {
                ...
                if (!this.mBackgroundScanPaused) {
                    ...
                    if (this.mBackgroundScanPeriodPending && this.mBackgroundScanSettings != null) {
                        str = this.mBackgroundScanSettings.handlerId;
                        int reportEvents = 4;
                ...
                if (this.mPendingSingleScanSettings != null) {
                    int i2;
                    str = this.mPendingSingleScanSettings.handlerId;
                ...
                if (newScanSettings.backgroundScanActive || newScanSettings.singleScanActive) {
                    ...
                    if (allFreqs.isEmpty()) {
                        ...
                        if (!success) {
                            Log.e(TAG, str + "Failed to start scan, freqs=" + freqs);
                        }
                    }
                    if (success) {
                        WifiScanLog.getDefault().addEvent(str, "15", "WifiNative start wifi scan success!", null);
    ...
    private void pollLatestScanDataForPno() {
        synchronized (this.mSettingsLock) {
            ...
            if (numFilteredScanResults != 0 || this.mNativeScanResults.size() == 0) {
                Log.d("WifiScanLog", "Pno Filtering out " + numFilteredScanResults + " pno scan results.total size " + this.mNativeScanResults.size());
            }
    ...
    private void pollLatestScanData() {
        synchronized (this.mSettingsLock) {
            ...
            for (i = 0; i < list_size; i++) {
                ...
                if (isTimeFiltered || isFrequencyFiltered) {
                    if (numFilteredScanResults == 0) {
                        filteredResultsString.append(this.mLastScanSettings.backgroundScanActive).append("|").append(this.mLastScanSettings.singleScanActive).append("|").append(this.mLastScanSettings.startTime).append("/");
                    }
                    if (isFrequencyFiltered) {
                        filteredResultsString.append(result.SSID).append("|").append(result.frequency).append("|").append(ScanResultUtil.getConfusedBssid(result.BSSID)).append("|");
                    }
                    if (isTimeFiltered) {
                        String filterSsid = new StringBuffer("\"").append(result.SSID).append("\"").toString();
                        if (filterResults.containsKey(filterSsid)) {
                            StringBuffer filterSsidValue = (StringBuffer) filterResults.get(filterSsid);
                            if (filterSsidValue == null) {
                                filterSsidValue = new StringBuffer();
                            }
                            try {
                                filterResults.put(filterSsid, filterSsidValue.append(result.BSSID.substring(result.BSSID.length() - 5)).append("|").append(result.timestamp / 1000).append("|"));
            ...
            for (Entry<String, StringBuffer> entry : filterResults.entrySet()) {
                ...
                if (arrayMap != null) {
                    filteredResultsString.append((String) entry.getKey()).append("|").append(arrayMap);
                }
            }

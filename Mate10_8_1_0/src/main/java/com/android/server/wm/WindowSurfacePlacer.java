...
class WindowSurfacePlacer {
    ...
    public WindowSurfacePlacer(WindowManagerService service) {
        ...
        this.mPerformSurfacePlacement = new -$Lambda$aEpJ2RCAIjecjyIIYTv6ricEwh4((byte) 12, this);
    }
    ...
    int handleAppTransitionReadyLocked() {
        ...
        if (WindowManagerService.HW_SUPPORT_LAUNCHER_EXIT_ANIM && transit == 13 && topOpeningApp != null) {
            String appWindowToken = topOpeningApp.toString();
            this.mService.getClass();
            if (!appWindowToken.contains("com.huawei.android.launcher/.unihome.UniHomeLauncher")) {
                appWindowToken = topOpeningApp.toString();
                this.mService.getClass();
            }
        ...
        int layoutRedo = this.mService.mAppTransition.goodToGo(transit, appWindowAnimator, appWindowAnimator2, this.mService.mOpeningApps, this.mService.mClosingApps);
    ...
    private int findMaxWindowSizeTokenIndex(int appsCount, int transit) {
        int maxHeight = -1;
        int maxHeightTokenIndex = -1;
        String permissionCmpName = "com.android.packageinstaller/.permission.ui.GrantPermissionsActivity";
        if (WindowManagerService.HW_SUPPORT_LAUNCHER_EXIT_ANIM && transit == 13) {
            for (int i = 0; i < appsCount; i++) {
                ...
                if (wtoken != null) {
                    Task wTask = wtoken.getTask();
                    if (wTask == null) {
                        Slog.w(TAG, "wtoken's task is null");
                    } else {
                        DisplayContent displayContent = wTask.getDisplayContent();
                        if (displayContent == null) {
                            Slog.w(TAG, "displayContent is null");
                        } else {
                            DisplayInfo displayInfo = displayContent.getDisplayInfo();
                            if (displayInfo == null) {
                                Slog.w(TAG, "displayInfo is null");
                            } else {
                                Rect frame = new Rect(0, 0, displayInfo.appWidth, displayInfo.appHeight);
                                ...
                                if (win != null) {
                                    ...
                                    if (frame.height() >= maxHeight && (wtoken.toString().contains("com.android.packageinstaller/.permission.ui.GrantPermissionsActivity") ^ 1) != 0) {
                                        maxHeight = frame.height();
                                        maxHeightTokenIndex = i;
                                    }
                                }
                            }
                        }
    ...
    private void handleClosingApps(int transit, LayoutParams animLp, boolean voiceInteraction, LayerAndToken layerAndToken) {
        ...
        int maxWinHeightTokenIndex = findMaxWindowSizeTokenIndex(appsCount, transit);
    ...
    private boolean transitionGoodToGo(int appsCount, SparseIntArray outReasons) {
        ...
        for (int i = 0; i < appsCount; i++) {
            ...
            int isRelaunching = wtoken.allDrawn ? wtoken.isRelaunching() ^ 1 : 0;
            if (isRelaunching == 0 && (wtoken.startingDisplayed ^ 1) != 0 && (wtoken.startingMoved ^ 1) != 0) {
                return false;
            }
        ...
        if (this.mService.mAppTransition.isFetchingAppTransitionsSpecs()) {
            ...
            if (this.mWallpaperControllerLocked.isWallpaperVisible()) {
                wallpaperTransitionReady = this.mWallpaperControllerLocked.wallpaperTransitionReady();
            } else {
            ...
            Flog.i(307, "Not ready for transition, wallpaperReady=" + wallpaperTransitionReady);
            return false;
        }
    }
    ...
    private int maybeUpdateTransitToWallpaper(int transit, boolean openingAppHasWallpaper, boolean closingAppHasWallpaper) {
        ...
        if (openingApps != null) {
            String arraySet = openingApps.toString();
            this.mService.getClass();
            if (!arraySet.contains("com.huawei.android.launcher/.unihome.UniHomeLauncher")) {
                arraySet = openingApps.toString();
                this.mService.getClass();
            }
            if (windowState != null) {
                arraySet = windowState.toString();
                this.mService.getClass();
                if (arraySet.contains("GestureNavAnimBottom")) {
                    return 13;
                }
            }
        }
        if (!openingCanBeWallpaperTarget || transit != 20) {
            if (!AppTransition.isKeyguardGoingAwayTransit(transit)) {
                ...
                } else if (windowState != null && (this.mService.mOpeningApps.isEmpty() ^ 1) != 0 && (openingApps.contains(windowState.mAppToken) ^ 1) != 0 && closingApps.contains(windowState.mAppToken)) {
                    transit = 12;
    ...
    private void createThumbnailAppAnimator(int transit, AppWindowToken appToken, int openingLayer, int closingLayer) {
        AppWindowAnimator appWindowAnimator = appToken == null ? null : appToken.mAppAnimator;
        if (appWindowAnimator != null && appWindowAnimator.animation != null) {
            ...
            if (thumbnailHeader != null) {
                ...
                try {
                    ...
                    if (this.mService.mAppTransition.isNextThumbnailTransitionAspectScaled()) {
                        ...
                        Rect rect = win != null ? win.mContentInsets : null;
                        Configuration displayConfig = displayContent.getConfiguration();
                        anim = this.mService.mAppTransition.createThumbnailAspectScaleAnimationLocked(appRect, rect, thumbnailHeader, taskId, displayConfig.uiMode, displayConfig.orientation);
                        appWindowAnimator.thumbnailForceAboveLayer = Math.max(openingLayer, closingLayer);
                        appWindowAnimator.deferThumbnailDestruction = this.mService.mAppTransition.isNextThumbnailTransitionScaleUp() ^ 1;
                    } else {
                    ...
                    appWindowAnimator.thumbnailLayer = openingLayer;
                    appWindowAnimator.thumbnailAnimation = anim;

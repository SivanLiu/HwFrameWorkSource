    ...
    public void release() throws RemoteException {
        if (this.mActivityToken != null || this.mPermissionOwnerToken != null) {
            ...
            if (this.mActivityToken != null) {
                try {
                    iBinder = ActivityManager.getService().getUriPermissionOwnerForActivity(this.mActivityToken);
                } catch (Exception e) {
                    return;
                } finally {
            ...
            for (int i = 0; i < this.mUris.size(); i++) {
                ActivityManager.getService().revokeUriPermissionFromOwner(iBinder, (Uri) this.mUris.get(i), this.mMode, this.mSourceUserId);
            }
        }
    }

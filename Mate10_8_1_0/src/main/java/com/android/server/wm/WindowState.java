...
public class WindowState extends WindowContainer<WindowState> implements android.view.WindowManagerPolicy.WindowState {
    ...
    private int mDecorTopCompensation = 0;
    boolean mDestroying;
    ...
    public void computeFrameLw(Rect parentFrame, Rect displayFrame, Rect overscanFrame, Rect contentFrame, Rect visibleFrame, Rect decorFrame, Rect stableFrame, Rect outsetFrame) {
        if (!this.mWillReplaceWindow || (!this.mAnimatingExit && (this.mReplacingRemoveRequested ^ 1) == 0)) {
            ...
            int isFloating = task != null ? task.isFloating() : 0;
            ...
            if (inFullscreenContainer || layoutInParentFrame()) {
                ...
                if (isFloating != 0 && this.mContainingFrame.isEmpty()) {
            ...
            if (isFloating != 0 && (this.mFrame.isEmpty() ^ 1) != 0) {
                ...
                offsetSystemDialog(contentFrame);
            ...
            if (inFullscreenContainer && (isFloating ^ 1) != 0) {
            ...
            if (this.mAttrs.type == 2034) {
                ...
                boolean overrideRightInset = (isFloating != 0 || (inFullscreenContainer ^ 1) == 0) ? false : this.mFrame.right > this.mTmpRect.right;
                boolean overrideBottomInset = (isFloating != 0 || (inFullscreenContainer ^ 1) == 0) ? false : this.mFrame.bottom > this.mTmpRect.bottom;
    ...
    void updateResizingWindowIfNeeded() {
        ...
        if (this.mHasSurface && this.mService.mLayoutSeq == this.mLayoutSeq && !isGoneForLayoutLw()) {
            ...
            if (task == null || !task.mStack.isAnimatingBounds()) {
                ...
                int isDragResizingChangeReported;
                if (isDragResizeChanged()) {
                    isDragResizingChangeReported = isDragResizingChangeReported() ^ 1;
                } else {
                    isDragResizingChangeReported = 0;
                }
                this.mLastFrame.set(this.mFrame);
                if (this.mContentInsetsChanged || this.mVisibleInsetsChanged || winAnimator.mSurfaceResized || this.mOutsetsChanged || this.mFrameSizeChanged || configChanged || r1 != 0 || (isResizedWhileNotDragResizingReported() ^ 1) != 0 || this.mReportOrientationChanged) {
                    if (this.mAppToken == null || !this.mAppDied) {
                        updateLastInsetValues();
                        this.mService.makeWindowFreezingScreenIfNeededLocked(this);
                        if (getOrientationChanging() || r1 != 0 || isResizedWhileNotDragResizing()) {
    ...
    void prelayout() {
        if (this.mEnforceSizeCompat) {
            IApsManager apsManager;
            float resolutionRatio;
            String pkgName = null;
            if (this.mAppToken != null) {
                pkgName = this.mAppToken.appPackageName;
            }
            if (!(this.mAppToken == null || pkgName == null)) {
                if (pkgName.isEmpty()) {
                }
                apsManager = (IApsManager) LocalServices.getService(IApsManager.class);
    ...
    boolean mightAffectAllDrawn(boolean visibleOnly) {
        ...
        if ((this.mAppToken == null || (this.mAppToken.isClientHidden() ^ 1) != 0) && this.mViewVisibility == 0) {
            i = this.mWindowRemovalAllowed ^ 1;
        } else {
            i = 0;
        }
        if ((!isOnScreen() || (visibleOnly && r0 == 0)) && this.mWinAnimator.mAttrType != 1) {
            if (this.mWinAnimator.mAttrType != 4) {
                return false;
            }
        }
        if ((this.mAnimatingExit ^ 1) != 0) {
            return this.mDestroying ^ 1;
        }
        return false;
    }
    ...
    public boolean canAffectSystemUiFlags() {
        int isAnimationStarting;
        int exitingSelf;
        boolean shown = this.mWinAnimator.getShown();
        if (this.mAppToken != null) {
            isAnimationStarting = this.mAppToken.mAppAnimator.isAnimationStarting();
        } else {
            isAnimationStarting = 0;
        }
        if (!this.mAnimatingExit || this.mWinAnimator.isAnimationStarting()) {
            exitingSelf = 0;
        } else {
            exitingSelf = isAnimationStarting ^ 1;
        }
        int i = (this.mAppToken == null || !this.mAppToken.hidden) ? 0 : isAnimationStarting ^ 1;
        int i2 = (exitingSelf != 0 || this.mDestroying) ? 1 : i;
        boolean translucent = this.mAttrs.alpha == 0.0f;
        boolean enteringWithDummyAnimation = this.mWinAnimator.isDummyAnimation() && this.mWinAnimator.mShownAlpha == 0.0f;
        if (!shown || (i2 ^ 1) == 0 || (translucent ^ 1) == 0) {
            return false;
        }
    ...
    void onResize() {
        ...
        if (this.mHasSurface && (resizingWindows.contains(this) ^ 1) != 0) {
            ...
            Configuration temConfig = getConfiguration();
            boolean isNoFullScreen = temConfig != null && temConfig.nonFullScreen == 1;
            if (!(computeDragResizing() || this.mAttrs.type != 1 || (this.mWinAnimator.isForceScaled() ^ 1) == 0 || (isGoneForLayoutLw() ^ 1) == 0 || (isNoFullScreen ^ 1) == 0 || (getTask().inPinnedWorkspace() ^ 1) == 0)) {
    ...
    void handleWindowMovedIfNeeded() {
        if (hasMoved()) {
            ...
            if (this.mToken.okToAnimate() && (this.mAttrs.privateFlags & 64) == 0 && (isDragResizing() ^ 1) != 0 && (r0 ^ 1) != 0 && ((task == null || getTask().mStack.hasMovementAnimations()) && (this.mWinAnimator.mLastHidden ^ 1) != 0)) {
    ...
    private void removeIfPossible(boolean keepVisibleDeadWindow) {
        ...
        if (this.mHasSurface && this.mToken.okToAnimate()) {
            if (this.mWillReplaceWindow) {
                ...
                int isDummyAnimation = this.mWinAnimator.isAnimationSet() ? this.mWinAnimator.isDummyAnimation() ^ 1 : 0;
                ...
                if (this.mWinAnimator.getShown() && this.mAnimatingExit && !(r2 && isDummyAnimation == 0)) {
        ...
        removeImmediately();
        if (z && this.mService.updateOrientationFromAppTokensLocked(false, displayId)) {
    ...
    Rect getBackdropFrame(Rect frame) {
        int isDragResizeChanged = !isDragResizing() ? isDragResizeChanged() : 1;
        if (StackId.useWindowFrameForBackdrop(getStackId()) || (isDragResizeChanged ^ 1) != 0) {
            return frame;
        }
        DisplayInfo displayInfo = getDisplayInfo();
    ...
    void applyGravityAndUpdateFrame(Rect containingFrame, Rect displayFrame) {
        ...
        if (this.mEnforceSizeCompat) {
            this.mCompatFrame.scale(this.mInvGlobalScale);
            String charSequence = getWindowTag() != null ? getWindowTag().toString() : null;
            if (charSequence == null || !charSequence.equals("Toast") || this.mRequestedWidth - this.mCompatFrame.width() != 1) {
                return;
            }
            Rect rect;
    ...
    int getLowResolutionMode() {
        if (isStatusBarWindow()) {
            boolean statusBarExpanded = (this.mAttrs.hwFlags & 4) != 0;
    ...
    boolean isAppVisibleSysAlertWin() {
        return isVisibleOrAdding() ? LayoutParams.isSystemAlertWindowType(this.mAttrs.type) : false;
    }

    boolean isLayerLowerThan(WindowState win) {
        if (win == null || this.mWinAnimator.mAnimLayer >= win.mWinAnimator.mAnimLayer) {
            return false;
        }
        return true;
    }
    ...
    public boolean isWindowUsingNotch() {
        switch (this.mService.mHwWMSEx.getAppUseNotchMode(getOwningPackage())) {
            ...
            default:
                ...
                if ((getAttrs().hwFlags & 65536) == 0) {
                    hwNotchSupport = getHwNotchSupport();
                } else {
                    hwNotchSupport = true;
                }
                return hwNotchSupport;
        }
    }

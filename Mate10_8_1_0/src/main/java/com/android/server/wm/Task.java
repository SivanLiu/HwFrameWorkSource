...
class Task extends WindowContainer<AppWindowToken> implements DimLayerUser {
    ...
    public void getDimBounds(Rect out) {
        if (this.mStack != null) {
            ...
            DisplayContent displayContent = this.mStack.getDisplayContent();
            if (displayContent != null) {
                isResizing = displayContent.mDividerControllerLocked.isResizing();
            } else {
                isResizing = false;
            }
            if (!useCurrentBounds()) {
            ...
            } else if ((HwPCUtils.isExtDynamicStack(this.mStack.mStackId) && this.mFillsParent) || !inFreeformWorkspace() || !getMaxVisibleBounds(out)) {
                if (this.mFillsParent) {
                    out.set(this.mBounds);
                } else {
                    if (isResizing) {
    ...
    boolean isSamePackageInTask() {
        ...
        for (int i = this.mChildren.size() - 1; i >= 0; i--) {
            ...
            if (uid == -1) {
                uid = tmpToken.isEmpty() ? -1 : ((WindowState) tmpToken.mChildren.peekLast()).mOwnerUid;
            } else {
                ret = uid == (tmpToken.isEmpty() ? -1 : ((WindowState) tmpToken.mChildren.peekLast()).mOwnerUid);
            }
            if (!ret) {
                return ret;
            }
    ...
    public int getLayerForDim(WindowStateAnimator animator, int layerOffset, int defaultLayer) {
        AppWindowToken appToken = animator.mWin.mAppToken;
        if (animator.mAttrType == 3 && hasChild(appToken)) {

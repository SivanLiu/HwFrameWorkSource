...
final class InputMonitor implements WindowManagerCallbacks {
        ...
        public void accept(WindowState w) {
            ...
            if (inputChannel != null && inputWindowHandle != null && !w.mRemoved && !w.canReceiveTouchInput()) {
                ...
                if (this.wallpaperController.isWallpaperTarget(w) && (privateFlags & 1024) == 0) {
                    -get3 = InputMonitor.this.mDisableWallpaperTouchEvents ^ 1;
                } else {
                    -get3 = false;
                }
                ...
                InputMonitor.this.addInputWindowHandle(inputWindowHandle, w, flags, type, isVisible, hasFocus, -get3);
            }
        }
    }
    ...
    public long notifyANR(InputApplicationHandle inputApplicationHandle, InputWindowHandle inputWindowHandle, String reason) {
        ...
        if (appWindowToken != null && appWindowToken.appToken != null) {
            ...
            if (controller != null) {
                keyDispatchingTimedOut = controller.keyDispatchingTimedOut(reason, windowState != null ? windowState.mSession.mPid : -1);
            } else {
                keyDispatchingTimedOut = false;
            }
            if (!keyDispatchingTimedOut) {
    ...
    void addInputWindowHandle(InputWindowHandle inputWindowHandle, WindowState child, int flags, int type, boolean isVisible, boolean hasFocus, boolean hasWallpaper) {
        ...
        if (inputWindowHandle.windowState != null && (inputWindowHandle.windowState instanceof WindowState)) {
            ...
            if ((windowState.getAttrs().hwFlags & 65536) == 0) {
                hwNotchSupport = windowState.getHwNotchSupport();
            } else {
                hwNotchSupport = true;
            }
            switch (this.mService.mHwWMSEx.getAppUseNotchMode(windowState.getOwningPackage())) {
                case 1:
                    inputWindowHandle.layoutParamsPrivateFlags = 65536;
                    break;
                case 2:
                    inputWindowHandle.layoutParamsPrivateFlags = 0;
                    break;
                default:
                    inputWindowHandle.layoutParamsPrivateFlags = hwNotchSupport ? 65536 : 0;
                    break;
            }
            inputWindowHandle.layoutParamsPrivateFlags = ((windowState.getAttrs().hwFlags & DumpState.DUMP_INTENT_FILTER_VERIFIERS) != 0 ? DumpState.DUMP_INTENT_FILTER_VERIFIERS : 0) | inputWindowHandle.layoutParamsPrivateFlags;
        }

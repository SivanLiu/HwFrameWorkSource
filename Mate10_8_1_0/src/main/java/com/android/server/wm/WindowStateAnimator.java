...
public class WindowStateAnimator {
    ...
    public void setAnimation(Animation anim, long startTime, int stackClip) {
        ...
        if (this.mWin.toString().contains("com.autonavi.minimap/com.autonavi.map.activity.NewMapActivity")) {
    ...
    void setViewAlpha(float alpha) {
        ...
        if (this.mSurfaceController != null) {
            this.mSurfaceController.setViewAlpha(alpha);
        }
    }

    void setWindowClipFlag(int flag) {
        if (this.mSurfaceController == null) {
            Log.e(TAG, "mWindowSurfaceController is null!!");
        } else {
    ...
    void computeShownFrameLocked() {
        ...
        Transformation transformation = (this.mParentWinAnimator == null || !this.mParentWinAnimator.mHasLocalTransformation) ? null : this.mParentWinAnimator.mTransformation;
        ...
        if (this.mIsWallpaper && wallpaperTarget != null && this.mService.mAnimateWallpaperWithTarget) {
            ...
            if (!(appWindowAnimator == null || !appWindowAnimator.hasTransformation || appWindowAnimator.animation == null || (appWindowAnimator.animation.getDetachWallpaper() ^ 1) == 0)) {
        ...
        if ((selfTransformation || transformation != null || transformation2 != null || isAnimating) && this.mHasTransform) {
    ...
    private boolean calculateCrop(Rect clipRect) {
        ...
        Task task = w.getTask();
        int isFullscreen = !w.fillsDisplay() ? task != null ? task.isFullscreen() : 0 : 1;
        boolean isFreeformResizing = w.isDragResizing() && w.getResizeMode() == 0;
        Rect rect = (!this.mHasClipRect || (isFullscreen ^ 1) == 0) ? this.mSystemDecorRect : this.mClipRect;
        clipRect.set(rect);
        if (isFreeformResizing && (w.isChildWindow() ^ 1) != 0) {
            clipRect.offset(w.mShownPosition.x, w.mShownPosition.y);
        }
        w.expandForSurfaceInsets(clipRect);
        if (this.mHasClipRect && isFullscreen != 0) {
        ...
        if (!hasSurface()) {
            ...
            if (this.mIsWallpaper && (this.mWin.mWallpaperVisible ^ 1) != 0) {
                ...
                if (prepared && this.mDrawState == 4) {
                    ...
                    if (this.mWin.mTurnOnScreen && (this.mWin.mAppToken == null || this.mWin.mAppToken.canTurnScreenOn() || (this.mWin.toString().contains("com.android.incallui/com.android.incallui.InCallActivity") && this.mWin.isVisibleLw()))) {
                        this.mWin.mTurnOnScreen = false;
                        if (this.mWin.mAppToken != null) {
                            this.mWin.mAppToken.setCanTurnScreenOn(false);
                        }
                        if (!this.mService.mPowerManager.isInteractive()) {
    ...
    private void applyFadeoutDuringKeyguardExitAnimation() {
        ...
        if (fadeDuration > 0) {
            ...
            Animation fadeOut = AnimationUtils.loadAnimation(this.mContext, 17432595);
    ...
    void setMoveAnimation(int left, int top) {
        setAnimation(AnimationUtils.loadAnimation(this.mContext, 17432756));

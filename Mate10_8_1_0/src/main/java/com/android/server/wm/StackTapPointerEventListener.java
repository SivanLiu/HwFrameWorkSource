...
public class StackTapPointerEventListener implements PointerEventListener {
    ...
    public void onPointerEvent(MotionEvent motionEvent) {
        ...
        switch (action & 255) {
            ...
            case 1:
            case 6:
                index = (JobPackageTracker.EVENT_STOP_REASON_MASK & action) >> 8;
            ...
            case 2:
                if (this.mPointerId >= 0) {
                    ...
                    if (motionEvent.getEventTime() - motionEvent.getDownTime() <= 300 && index >= 0 && Math.abs(motionEvent.getX(index) - this.mDownX) <= ((float) this.mMotionSlop)) {
                        if (Math.abs(motionEvent.getY(index) - this.mDownY) <= ((float) this.mMotionSlop)) {
                            return;
                        }
                    }
                    this.mPointerId = -1;

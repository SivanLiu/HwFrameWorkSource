...
public class AppTransition implements Dump {
    ...
    private TimeInterpolator[] mSizeBigInterpolators = new TimeInterpolator[]{new PathInterpolator(0.44f, 0.43f, 0.7f, 0.75f), new PathInterpolator(0.13f, 0.79f, 0.3f, 1.0f)};
    private TimeInterpolator[] mSizeSmallInterpolators = new TimeInterpolator[]{new PathInterpolator(0.41f, 0.38f, 0.7f, 0.71f), new PathInterpolator(0.16f, 0.64f, 0.33f, 1.0f)};
    ...
    Animation createAppExitToIconAnimation(AppWindowToken atoken, int containingHeight, int iconWidth, int iconHeight, float originPivotX, float originPivotY, Bitmap icon) {
        if (atoken == null) {
            Log.w(TAG, "create app exit animation find no app window token!");
            return null;
        }
        WindowState window = atoken.findMainWindow();
        if (window == null) {
            Log.w(TAG, "create app exit animation find no app main window!");
            return null;
        }
        Rect winDecorFrame = window.mDecorFrame;
        if (winDecorFrame == null) {
            Log.w(TAG, "create app exit animation find no app window frame!");
            return null;
        }
        ...
        float middleX = 1.0f - ((((float) (winWidth - iconWidth)) * (isHorizontal ? 0.54f : 0.44f)) / ((float) winWidth));
        float middleY = 1.0f - ((((float) (winHeight - iconHeight)) * (isHorizontal ? 0.44f : 0.54f)) / ((float) winHeight));
        float toX = ((float) iconWidth) / ((float) winWidth);
        float toY = ((float) iconHeight) / ((float) winHeight);
        Log.d(TAG, "now set the app exit scale animation for: " + window + ", " + "[winWidth, winHeight] = [" + winWidth + ", " + winHeight + "]" + "[originPivotX, originPivotY] = [" + originPivotX + ", " + originPivotY + "]" + "[fromX, fromY] = [" + 1.0f + ", " + 1.0f + "]" + "[toX, toY] = [" + toX + ", " + toY + "]");
        float pivotX = computeFloatPivot(originPivotX - (((float) iconWidth) / 2.0f), toX) + 0.5f;
        float pivotY = computeFloatPivot(originPivotY - (((float) iconHeight) / 2.0f), toY) + 0.5f;
        ...
        pivotY -= ((float) (winDecorFrame.top > winFrame.top ? winDecorFrame.top : winFrame.top)) * toY;
        Log.d(TAG, "Retrieved [pivotX, pivotY] = [" + pivotX + ", " + pivotY + "]");
        ...
        alphaAnimation.setInterpolator(new PhaseInterpolator(new float[]{0.0f, 0.16f, 0.32f, 1.0f}, new float[]{1.0f, 1.0f, 0.0f, 0.0f}, this.mAlphaInterpolators));
        r41 = new float[3];
        r41 = new float[]{0.0f, 0.16f, 1.0f};
        float[] scaleOutValuesX = new float[]{1.0f, middleX, toX};
        float[] scaleOutValuesY = new float[]{1.0f, middleY, toY};
        if (isHorizontal) {
        ...
        PhaseInterpolator phaseInterpolator = new PhaseInterpolator(r41, scaleOutValuesX, sizeXInterpolators);
        phaseInterpolator = new PhaseInterpolator(r41, scaleOutValuesY, sizeYInterpolators);
        ScaleAnimation animX = new ScaleAnimation(0.0f, 1.0f, 1.0f, 1.0f, pivotX, pivotY);
        animX.setFillEnabled(true);
        animX.setFillBefore(true);
        animX.setFillAfter(true);
        animX.setDuration(350);
        animX.setInterpolator(phaseInterpolator);
        ScaleAnimation animY = new ScaleAnimation(1.0f, 1.0f, 0.0f, 1.0f, pivotX, pivotY);
        animY.setFillEnabled(true);
        animY.setFillBefore(true);
        animY.setFillAfter(true);
        animY.setDuration(350);
        animY.setInterpolator(phaseInterpolator);
        animationSet.addAnimation(alphaAnimation);
        animationSet.addAnimation(animX);
        animationSet.addAnimation(animY);
        animationSet.setZAdjustment(1);
        float roundx = ((float) Math.min(winWidth, winHeight)) * 0.2f;
        float roundy = roundx;
        window.mWinAnimator.setWindowClipFlag(1);
        window.mWinAnimator.setWindowClipRound(roundx, roundx);
        if (!atoken.mShouldDrawIcon) {
            window.mWinAnimator.setWindowClipFlag(2);
        }
        if (icon != null) {
            window.mWinAnimator.setWindowClipIcon(iconWidth, iconHeight, icon);
        }
        return animationSet;
    }
    ...
    Animation createLauncherEnterAnimation(AppWindowToken atoken, int containingHeight, int iconWidth, int iconHeight, float originPivotX, float originPivotY) {
        if (atoken == null) {
            Log.w(TAG, "create launcher enter animation find no app window token!");
            return null;
        }
        WindowState window = atoken.findMainWindow();
        if (window == null) {
            Log.w(TAG, "create launcher enter animation find no app main window!");
            return null;
        }
        Rect winDecorFrame = window.mDecorFrame;
        Rect winVisibleFrame = window.mVisibleFrame;
        if (winDecorFrame == null || winVisibleFrame == null) {
            Log.w(TAG, "create launcher enter animation find no app window frame!");
            return null;
        }
        int winWidth = winDecorFrame.right - winDecorFrame.left;
        int winHeight = containingHeight - winDecorFrame.top;
        ...
        float toY = ((float) iconHeight) / ((float) winHeight);
        float iconTop = originPivotY - ((float) (iconHeight / 2));
        float pivotX = computePivot((int) (originPivotX - ((float) (iconWidth / 2))), ((float) iconWidth) / ((float) winWidth));
        ...
        AnimationSet launcherEnterAnimation;
        ...
        long duration = 350;
        if (this.mService.mExitIconWidth >= 0 && this.mService.mExitIconHeight >= 0) {
            if (this.mService.mExitIconBitmap == null) {
            }
            launcherEnterAnimation = new AnimationSet(false);
            scaleAnimation = new ScaleAnimation(1.2f, 1.0f, 1.2f, 1.0f, pivotX, pivotY);
            scaleAnimation.setFillEnabled(true);
            scaleAnimation.setFillBefore(true);
            scaleAnimation.setFillAfter(true);
            scaleAnimation.setDuration(duration);
            scaleAnimation.setInterpolator(new PathInterpolator(0.18f, 0.19f, 0.59f, 1.0f));
    ...
    Animation loadAnimation(LayoutParams lp, int transit, boolean enter, int uiMode, int orientation, Rect frame, Rect displayFrame, Rect insets, Rect surfaceInsets, Rect stableInsets, boolean isVoiceInteraction, boolean freeform, int taskId) {
        ...
        if (transit == 23 && (enter ^ 1) != 0) {
            return loadAnimationRes(lp, 17432755);
        }
        int i;
        if (isVoiceInteraction && (transit == 6 || transit == 8 || transit == 10)) {
            ...
            if (this.mNextAppTransitionType == 1) {
                ...
                if (this.mNextAppTransitionType == 3 || this.mNextAppTransitionType == 4) {
                    ...
                    return createAspectScaledThumbnailEnterExitAnimationLocked(getThumbnailTransitionState(enter), uiMode, orientation, transit, frame, insets, surfaceInsets, stableInsets, freeform, taskId);
                } else {
                    int animAttr = 0;
                    switch (transit) {
                        case 6:
    ...
    /* synthetic */ void lambda$-com_android_server_wm_AppTransition_101764(IAppTransitionAnimationSpecsFuture future) {
    ...
    boolean prepareAppTransitionLocked(int transit, boolean alwaysKeepCurrent, int flags, boolean forceOverride) {
        if (isKeyguardGoingAwayTransit(transit) || transit == 23) {

...
class AlertWindowNotification {
    ...
    /* synthetic */ void -com_android_server_wm_AlertWindowNotification-mthref-0() {
        onPostNotification();
    }
    ...
    /* synthetic */ void -com_android_server_wm_AlertWindowNotification-mthref-1() {
    ...
    private void onPostNotification() {
        if (!this.mPosted) {
            ...
            String message = context.getString(17039568, new Object[]{appName});
            Builder builder = new Builder(context, this.mNotificationTag).setOngoing(true).setContentTitle(context.getString(17039569, new Object[]{appName})).setContentText(message).setSmallIcon(17301708).setColor(context.getColor(17170772)).setStyle(new BigTextStyle().bigText(message)).setLocalOnly(true).setContentIntent(getContentIntent(context, this.mPackageName));
            if (aInfo != null) {
    ...
    private void createNotificationChannel(Context context, String appName) {
        if (sChannelGroup == null) {
            sChannelGroup = new NotificationChannelGroup(CHANNEL_PREFIX, this.mService.mContext.getString(17039566));
            this.mNotificationManager.createNotificationChannelGroup(sChannelGroup);
        }
        String nameChannel = context.getString(17039567, new Object[]{appName});

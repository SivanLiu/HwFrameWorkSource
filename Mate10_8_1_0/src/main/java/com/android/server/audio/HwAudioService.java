...
import vendor.huawei.hardware.dolby.dms.V1_0.IDmsCallbacks.Stub;
...
public class HwAudioService extends AudioService {
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            ...
            switch (state) {
                ...
                case 1:
                    ...
                    return;
                case 2:
                    SystemProperties.set("persist.sys.audio_mode", PPPOEStateMachine.PHASE_SERIALCONN);
                    return;
                default:
                    return;
            }
    ...
    private IDms mDms;
    private IHwBinder.DeathRecipient mDmsDeathRecipient = new IHwBinder.DeathRecipient() {
        public void serviceDied(long cookie) {
            if (HwAudioService.this.mAudioHandler != null) {
                Slog.e(HwAudioService.TAG, "Dolby service has died, try to reconnect 1s later.");
                HwAudioService.sendMsg(HwAudioService.this.mAudioHandler, HwAudioService.MSG_CONNECT_DOLBY_SERVICE, 0, 0, 0, null, 1000);
            }
        }
    };
    private IDmsCallbacks mDolbyClient = new Stub() {
        public void onDapParamUpdate(ArrayList<Byte> params) {
            byte[] buf = new byte[params.size()];
            for (int i = 0; i < buf.length; i++) {
                buf[i] = ((Byte) params.get(i)).byteValue();
            }
            int dlbParam = HwAudioService.byteArrayToInt32(buf, 0);
            int status = HwAudioService.byteArrayToInt32(buf, 4);
            Slog.d(HwAudioService.TAG, "Got dap param update, param = " + dlbParam + " status = " + status);
            if (dlbParam == 16777216 || dlbParam == HwAudioService.EFFECT_PARAM_EFF_PROF) {
                if (HwAudioService.this.mSystemReady) {
                    HwAudioService.sendMsg(HwAudioService.this.mAudioHandler, HwAudioService.MSG_SEND_DOLBYUPDATE_BROADCAST, 0, dlbParam, status, null, 1000);
                } else {
                    Slog.w(HwAudioService.TAG, "onDapParamUpdate() called before boot complete");
                }
            }
            if (dlbParam == 16777216) {
                HwAudioService.this.mDolbyEnable = status;
                if (HwAudioService.this.mHeadSetPlugState) {
                    SystemProperties.set(HwAudioService.DOLBY_MODE_HEADSET_STATE, HwAudioService.this.mDolbyEnable > 0 ? PreciseIgnore.COMP_SCREEN_ON_VALUE_ : "off");
                }
            }
        }
    };
    ...
    private Handler mMyHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    if (!(AudioSystem.getDeviceConnectionState(8, "") == 1 || AudioSystem.getDeviceConnectionState(4, "") == 1 || AudioSystem.getDeviceConnectionState(67108864, "") == 1)) {
                        if (AudioSystem.getDeviceConnectionState(536870912, "") != 1) {
                            return;
                        }
                    }
                    ...
                    if (HwAudioService.DOLBY_SOUND_EFFECTS_SUPPORT && HwAudioService.this.mDolbyEnable > 0) {
                        SystemProperties.set(HwAudioService.DOLBY_MODE_PRESTATE, PreciseIgnore.COMP_SCREEN_ON_VALUE_);
                        HwAudioService.this.setDolbyEnable(false);
                        return;
                    }
                    return;
                case 2:
                    ...
                    if (HwAudioService.DOLBY_SOUND_EFFECTS_SUPPORT) {
                        String preDolbyState = SystemProperties.get(HwAudioService.DOLBY_MODE_PRESTATE, "unknown");
                        if (preDolbyState != null && preDolbyState.equals(PreciseIgnore.COMP_SCREEN_ON_VALUE_)) {
                            HwAudioService.this.setDolbyEnable(true);
                            SystemProperties.set(HwAudioService.DOLBY_MODE_PRESTATE, "unknown");
                            if (HwAudioService.DEBUG) {
                                Slog.i(HwAudioService.TAG, "set DOLBY on");
                                return;
                            }
                            return;
                        }
    ...
    private boolean mPreDolbyState = true;
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action != null && (action.equals("android.intent.action.REBOOT") || action.equals("android.intent.action.ACTION_SHUTDOWN"))) {
                Slog.i(HwAudioService.TAG, "Release Dolby service HIDL client");
                if (HwAudioService.this.mDms != null && HwAudioService.this.mDolbyClient != null) {
                    try {
                        HwAudioService.this.mDms.unregisterClient(HwAudioService.this.mDolbyClient, 3, HwAudioService.this.mDolbyClient.hashCode());
                        HwAudioService.this.mDms.unlinkToDeath(HwAudioService.this.mDmsDeathRecipient);
                    } catch (RuntimeException e) {
                        Slog.e(HwAudioService.TAG, "Release Dolby RuntimeException");
                    } catch (Exception e2) {
                        Slog.e(HwAudioService.TAG, "Release Dolby error");
                    }
                }
    ...
    private class ForeAppTypeListener extends IForegroundAppTypeCallback {
        public void reportForegroundAppType(int type) {
            Slog.i(HwAudioService.TAG, "now type = " + HwAudioService.this.type + ",reportForegroundAppType = " + type + ",systemload = " + SystemProperties.get("sys.iaware.type", "-1"));
            ...
            if (HwAudioService.mVoipOptimizeInGameMode) {
                ...
                switch (type) {
                    case 9:
                        Slog.d(HwAudioService.TAG, "game mode turn on.");
                        AudioSystem.setParameters("game_mode=on");
                        return;
                    default:
                        Slog.d(HwAudioService.TAG, "game mode turn off.");
            ...
            switch (device) {
                ...
                case 2:
                    ...
                    if (HwAudioService.mIsUsbPowercosumeTips) {
                        ...
                        this.mTitle = this.mContext.getResources().getString(33686066);
                        this.mContent = this.mContext.getResources().getString(33686067);
                    } else {
    ...
    protected void handleMessageEx(Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_SHOW_DISABLE_HEADPHONE_TOAST /*92*/:
                ...
                break;
            case MSG_CONNECT_DOLBY_SERVICE /*93*/:
                setDolbyServiceClient();
                break;
            case MSG_SEND_DOLBYUPDATE_BROADCAST /*94*/:
                sendDolbyUpdateBroadcast(msg.arg1, msg.arg2);
                break;
            case 10000:
                int i = msg.arg1;
                if (msg.obj != null) {
    ...
    private void showRecordWarnDialog(int requestRecordPid, int activeRecordPid) {
        ...
        if (activePkgName == null) {
            activePkgName = this.mContext.getString(17039881);
        }
        String str2 = TAG;
    ...
    private void sendDolbyUpdateBroadcast(int dlbParam, int status) {
        ...
        intent.putExtra(EVENTNAME, dlbParam == 16777216 ? DOLBY_UPDATE_EVENT_DS_STATE : DOLBY_UPDATE_EVENT_PROFILE);
    ...
    private void setDolbyStatus() {
        ...
        if (StorageUtils.SDCARD_ROMOUNTED_STATE.equalsIgnoreCase(SystemProperties.get("ro.config.dolby_dap", StorageUtils.SDCARD_RWMOUNTED_STATE))) {
    ...
    private void setDolbyServiceClient() {
        try {
            ...
            sendMsg(this.mAudioHandler, MSG_CONNECT_DOLBY_SERVICE, 0, 0, 0, null, 1000);
        } catch (Exception e) {
    ...
    protected void onSetSoundEffectState(int device, int state) {
        if (!checkAudioSettingsPermission("onSetSoundEffectState()") || !SOUND_EFFECTS_SUPPORT) {
            return;
        }
        if (device == 4 || device == 8 || device == 67108864 || device == 536870912) {
            if (DOLBY_SOUND_EFFECTS_SUPPORT) {
    ...
    void restoreSoundEffectAndHSState(String processName) {
        ...
        for (String equals : FORBIDDEN_SOUND_EFFECT_WHITE_LIST) {
            if (equals.equals(processName)) {
                ...
                if (DTS_SOUND_EFFECTS_SUPPORT && r2 != null && r2.equals(PreciseIgnore.COMP_SCREEN_ON_VALUE_)) {
                    AudioSystem.setParameters(DTS_MODE_ON_PARA);
                    SystemProperties.set(DTS_MODE_PRESTATE, "unknown");
                    if (DEBUG) {
                ...
                if (SWS_SOUND_EFFECTS_SUPPORT && mIsHuaweiSWS31Config && r3 != null && r3.equals(PreciseIgnore.COMP_SCREEN_ON_VALUE_)) {
                    AudioSystem.setParameters(SWS_MODE_ON_PARA);
                    SystemProperties.set(SWS_MODE_PRESTATE, "unknown");
                    if (DEBUG) {
                ...
                if (DOLBY_SOUND_EFFECTS_SUPPORT && r1 != null && r1.equals(PreciseIgnore.COMP_SCREEN_ON_VALUE_)) {
                    setDolbyEnable(true);
                    SystemProperties.set(DOLBY_MODE_PRESTATE, "unknown");
                    if (DEBUG) {

...
public class SmartcareProc {
    ...
    public static class RegInfo {
        ...
        public void update(ServiceState ss) {
            ...
            if (ss.getDataRegState() == 0) {
                ...
                if (!(8 == this.mDataNetworkType || 9 == this.mDataNetworkType || 10 == this.mDataNetworkType)) {
                    if (15 == this.mDataNetworkType) {
                    }
                }
                if (TextUtils.equals(this.mMcc, WifiProCommonUtils.COUNTRY_CODE_CN) && (TextUtils.equals(this.mMnc, "00") || TextUtils.equals(this.mMnc, "02") || TextUtils.equals(this.mMnc, "07") || TextUtils.equals(this.mMnc, "08"))) {
    ...
    static final class VideoUploadTask extends UploadTask {
        ...
        protected boolean hasNormalData(SmartcareInfos is) {
            ...
            SmartcareProc proc = SmartcareProc.getInstance();
            if (is.videoInfo.fullDelay <= proc.getConfigedVideoIntBufferDelayThres() && is.videoInfo.srDelay <= proc.getConfigedVideoSRDelayThres() && is.videoInfo.totalLen <= proc.getConfigedStallingDurationThres() && is.videoInfo.times <= proc.getConfigedStallingTimesThres()) {
                if (is.videoInfo.videoTerminateFlag == (byte) 0) {
                }
                return bSuccess;
            }
            bSuccess = false;
        ...
        private void addNetworkInfo(EventStream eStream, short imonitorKeyBase, FwkNetworkInfo fwkNetworkInfo) {
            ...
            param.setParam((short) imonitorKeyBase, fwkNetworkInfo.rsrq).setParam((short) (imonitorKeyBase + 1), fwkNetworkInfo.sinr);
        }
    }
    ...
    static final class WechatUploadTask extends UploadTask {
        ...
        protected void addToTask(SmartcareInfos is) {
            ...
            if (this.mSmartcareInfosList.isEmpty()) {
                ...
                if (SmartcareConstants.DEBUG_MODE) {
                    Log.d(SmartcareProc.LOG_TAG, "WechatUploadTask adding new info: " + is.wechatInfo + ",foundMatchedType: " + foundMatchedType);
                }
            }
            if (this.mSmartcareInfosList.size() == 3) {
                notifySampleWinStat(false);
    ...
    private void handleTriggerAllUpload() {
        ...
        for (i = 0; i < configTypesSize && uploadedTaskList.size() < maxReportSize; i++) {
            UploadTask task = (UploadTask) this.mTaskMap.get(configTypes[i]);

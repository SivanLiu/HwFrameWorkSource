...
public class HighSpeedDevicesManager {
    ...
    private static final String CONNECTED_DEVICE_TYPE_ANA = "ANA";
    ...
    private boolean mIsANAConnected = false;
    ...
    class HighSpeedDevicesObserver extends UEventObserver {
        ...
        public void onUEvent(UEvent event) {
            ...
            String ANA_STATE = event.get("ANA_AUDIO_STATE");
            Log.d(HighSpeedDevicesManager.TAG, "DP_STATE: " + DP_STATE + "; USB3_STATE: " + USB3_STATE + "; ANA_STATE: " + ANA_STATE + ".");
            ...
            if (AwareJobSchedulerConstants.BAR_STATUS_ON.equals(ANA_STATE)) {
                HighSpeedDevicesManager.this.onANAConectedChanged(true);
            } else if (AwareJobSchedulerConstants.BAR_STATUS_OFF.equals(ANA_STATE)) {
                HighSpeedDevicesManager.this.onANAConectedChanged(false);
            }
        }
    }
    ...
    private void onANAConectedChanged(boolean isANAConnected) {
        Log.d(TAG, "onANAConectedChanged, mIsUSB3Connected: " + this.mIsANAConnected + " -> " + isANAConnected);
        if (this.mIsANAConnected != isANAConnected) {
            this.mIsANAConnected = isANAConnected;
            reportANAConected();
        }
    }
    ...
    public boolean mIsANAConnected() {
        return this.mIsANAConnected;
    }
    ...
    private void reportANAConected() {
        try {
            if (mIsANAConnected()) {
                HwTelephonyManager.getDefault().notifyDeviceState(CONNECTED_DEVICE_TYPE_ANA, CONNECTED_DEVICE_STATE_IN, "");
                Log.d(TAG, "USB3Manager notifyDeviceState CONNECTED_DEVICE_STATE_IN.");
                return;
            }
            HwTelephonyManager.getDefault().notifyDeviceState(CONNECTED_DEVICE_TYPE_ANA, CONNECTED_DEVICE_STATE_OUT, "");
            Log.d(TAG, "USB3Manager notifyDeviceState CONNECTED_DEVICE_STATE_OUT.");
        } catch (NoExtAPIException e) {
            Log.e(TAG, "notifyDeviceState - NoExtAPIException! " + e.toString());
        }

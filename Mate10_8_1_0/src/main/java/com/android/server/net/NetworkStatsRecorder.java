...
public class NetworkStatsRecorder {
    ...
    public NetworkStatsCollection getOrLoadPartialLocked(long start, long end) {
        Preconditions.checkNotNull(this.mRotator, "missing FileRotator");
        NetworkStatsCollection networkStatsCollection = this.mComplete != null ? (NetworkStatsCollection) this.mComplete.get() : null;
    ...
    public static void operatorAppwatch(int uid, boolean foreground) {
        ...
        if (!(foreground || (mMarkapp.contains(Integer.valueOf(uid)) ^ 1) == 0)) {
            mSilentapp.add(Integer.valueOf(uid));
        }
        if (!mWatchapp.contains(Integer.valueOf(uid)) && (foreground ^ 1) != 0 && mMarkapp.contains(Integer.valueOf(uid))) {
    ...
    private boolean nomarkeduid(int uid) {
        return (mWatchapp.contains(Integer.valueOf(uid)) || (mSilentapp.contains(Integer.valueOf(uid)) ^ 1) == 0) ? false : mMarkapp.contains(Integer.valueOf(uid)) ^ 1;
    }

    public void recordSilentEntry(Entry entry, long start, long end, NetworkIdentitySet ident, NetworkStatsCollection complete) {
        if (entry.set == 0 && mWatchapp.contains(Integer.valueOf(entry.uid)) && entry.rxBytes + entry.txBytes == 0 && end - start >= LocationFudger.FASTEST_INTERVAL_MS) {
        ...
        if (snapshot != null) {
            ...
            for (int i = 0; i < delta.size(); i++) {
                ...
                if (ident == null) {
                    ...
                    recordSilentEntry(entry, start, currentTimeMillis, ident, networkStatsCollection);
                    if (!entry.isEmpty()) {
                        if ((entry.tag == 0) != this.mOnlyTags) {
                            if (this.mPending != null) {
                            ...
                            if (networkStatsCollection != null) {
                                networkStatsCollection.recordData(ident, entry.uid, entry.set, entry.tag, start, currentTimeMillis, entry);
                            }
                        }
                    }
                }

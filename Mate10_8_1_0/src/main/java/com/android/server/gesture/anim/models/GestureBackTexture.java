...
public class GestureBackTexture implements GLModel {
    ...
    private float[] mRectData = new float[]{-1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f};
    private float mTextureAlpha;
    private FloatBuffer mTextureBuffer;
    private final float[] mTextureData = new float[]{GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, 1.0f, GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO, 1.0f, 1.0f, 1.0f};
    ...
    public void prepare() {
        ...
        createTexture(this.mContext, 33751792);
        this.mAttrVertexLoc = GLES20.glGetAttribLocation(this.mProgram, "aPosition");
        this.mAttrTextureLoc = GLES20.glGetAttribLocation(this.mProgram, "aTexCoordinate");
        this.mAlphaLoc = GLES20.glGetUniformLocation(this.mProgram, "uAlpha");
        this.mTextureLoc = GLES20.glGetUniformLocation(this.mProgram, "uTexture");
    }
    ...
    public void drawSelf() {
        if (this.mDraw) {
            ...
            if (this.mWidth < this.mHeight) {
                viewRatio = ((float) this.mWidth) / 1080.0f;
                GLES20.glViewport(0, centerY - ((int) ((62.0f * viewRatio) / 2.0f)), this.mWidth, (int) (62.0f * viewRatio));
                refreshVertexLoc((float) this.mWidth, viewRatio);
            } else if (this.mIsLeft) {
                viewRatio = ((float) this.mHeight) / 1080.0f;
                GLES20.glViewport(0, centerY - ((int) ((62.0f * viewRatio) / 2.0f)), this.mHeight, (int) (62.0f * viewRatio));
                refreshVertexLoc((float) this.mHeight, viewRatio);
            } else {
                viewRatio = ((float) this.mHeight) / 1080.0f;
                GLES20.glViewport(this.mWidth - this.mHeight, centerY - ((int) ((62.0f * viewRatio) / 2.0f)), this.mHeight, (int) (62.0f * viewRatio));
                refreshVertexLoc((float) this.mHeight, viewRatio);
            }
            GLES20.glUseProgram(this.mProgram);
            GLES20.glUniform1f(this.mAlphaLoc, this.mTextureAlpha);
            ...
            GLES20.glEnableVertexAttribArray(this.mAttrTextureLoc);
            GLES20.glVertexAttribPointer(this.mAttrTextureLoc, 2, 5126, false, 8, this.mTextureBuffer);
    ...
    public void setProcess(float process) {
        ...
        if (this.mAnimProcess >= 0.55f) {
            ...
            setAlpha((this.mAnimProcess - 0.275f) / 0.275f);
        }
    }

...
public class ConfigReader {
    ...
    private static final int MIN_SUPPORTED_VERSION = 41;
    ...
    private HashMap<AppMngFeature, HashMap<EnumWithDesc, HashMap<String, ListItem>>> mAllProcessList = new HashMap();
    private boolean mIsAppAlarmCorrect = true;
    private boolean mIsAppCleanCorrect = true;
    private boolean mIsAppFreezeCorrect = true;
    private boolean mIsAppIoLimitCorrect = true;
    private boolean mIsAppStartCorrect = true;
    ...
    private static /* synthetic */ int[] -getandroid-app-mtm-iaware-appmng-AppMngConstant$AppMngFeatureSwitchesValues() {
        ...
        try {
            iArr[AppMngFeature.APP_ALARM.ordinal()] = 1;
        } catch (NoSuchFieldError e) {
        }
        try {
            iArr[AppMngFeature.APP_CLEAN.ordinal()] = 2;
        } catch (NoSuchFieldError e2) {
        }
        try {
            iArr[AppMngFeature.APP_FREEZE.ordinal()] = 3;
        } catch (NoSuchFieldError e3) {
        }
        try {
            iArr[AppMngFeature.APP_IOLIMIT.ordinal()] = 4;
        } catch (NoSuchFieldError e4) {
        }
        try {
            iArr[AppMngFeature.APP_START.ordinal()] = 5;
    ...
    public void parseFile(AppMngFeature targetFeature, Context context) {
        ...
        if (!isConfigCorrect(targetFeature) || this.mAllConfig.isEmpty()) {
            file = new File(CONFIG_FILEPATH);
            if (file.exists()) {
                revertCorrectTagWhenFailed();
                parseFileInternal(file, targetFeature, context);
            } else {
                AwareLog.e(TAG, "default config file is not exist!");
            }
        }
    }
    ...
    private void parseFileInternal(File file, AppMngFeature targetFeature, Context context) {
        ...
        try {
            InputStream rawIs = new FileInputStream(file);
            try {
                inputStream2 = IAwareDecrypt.decryptInputStream(context, rawIs);
                parser = Xml.newPullParser();
                if (parser == null || inputStream2 == null) {
                    closeStream(rawIs, inputStream2, parser);
                    return;
                }
            ...
            try {
                ...
                closeStream(inputStream, inputStream2, parser);
            } catch (Throwable th3) {
                th = th3;
                closeStream(inputStream, inputStream2, parser);
                throw th;
    ...
    private void parseFeature(XmlPullParser parser, AppMngFeature feature) throws XmlPullParserException, IOException, NumberFormatException {
        ...
        HashMap<EnumWithDesc, HashMap<String, ListItem>> featureProcessList = new HashMap();
                ...
                if (eventType == 2) {
                    ...
                    if (tagName != null) {
                        ...
                        if (configName != null) {
                            ...
                            } else if (tagName.equals(XML_TAG_PROCESSLIST)) {
                            ...
                            if (values[i] == null || values[i].isEmpty()) {
                                ...
                                if (AppMngFeature.APP_START.equals(feature)) {
                                    if (!isRule) {
                                        ...
                                        String hwstop = parser.getAttributeValue(null, XML_TAG_HWSTOP);
    ...
    private boolean isConfigCorrect(AppMngFeature feature) {
        boolean z = false;
        if (feature == null) {
            if (this.mIsAppIoLimitCorrect && this.mIsAppFreezeCorrect && this.mIsAppCleanCorrect && this.mIsAppStartCorrect) {
                z = this.mIsAppAlarmCorrect;
            }
            return z;
        }
        switch (-getandroid-app-mtm-iaware-appmng-AppMngConstant$AppMngFeatureSwitchesValues()[feature.ordinal()]) {
            case 1:
                return this.mIsAppAlarmCorrect;
            case 2:
                return this.mIsAppCleanCorrect;
            case 3:
                return this.mIsAppFreezeCorrect;
            case 4:
                return this.mIsAppIoLimitCorrect;
            case 5:
                return this.mIsAppStartCorrect;
            default:
                AwareLog.e(TAG, "there is no correct info about feature named : " + feature);
                return false;
        }
    }
    ...
    private void setConfigFailed(AppMngFeature feature) {
        if (feature == null) {
            this.mIsAppStartCorrect = false;
            this.mIsAppCleanCorrect = false;
            this.mIsAppFreezeCorrect = false;
            this.mIsAppIoLimitCorrect = false;
            this.mIsAppAlarmCorrect = false;
            return;
        }
        switch (-getandroid-app-mtm-iaware-appmng-AppMngConstant$AppMngFeatureSwitchesValues()[feature.ordinal()]) {
            case 1:
    ...
    private void revertCorrectTagWhenFailed() {
        this.mIsAppStartCorrect ^= 1;
        this.mIsAppCleanCorrect ^= 1;
        this.mIsAppFreezeCorrect ^= 1;
        this.mIsAppIoLimitCorrect ^= 1;
        this.mIsAppAlarmCorrect ^= 1;
    }
    ...
    public HashMap<AppMngFeature, HashMap<EnumWithDesc, HashMap<String, ListItem>>> getProcessList() {

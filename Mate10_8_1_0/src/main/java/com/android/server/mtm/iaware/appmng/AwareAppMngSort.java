...
public class AwareAppMngSort {
    ...
    private ArrayMap<Integer, AwareProcessInfo> getNeedSortedProcesses(Map<Integer, AwareProcessInfo> foreGrdProc, Set<Integer> keyPercepServicePid, CachedWhiteList cachedWhitelist, ShortageProcessInfo shortageProc) {
        ...
        Map allProcessBaseInfo = this.mHwAMS != null ? this.mHwAMS.getAllProcessBaseInfo() : null;
        if (allProcessBaseInfo == null || allProcessBaseInfo.isEmpty()) {
            return null;
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (procInfo != null) {
                AwareProcessBaseInfo updateInfo = (AwareProcessBaseInfo) allProcessBaseInfo.get(Integer.valueOf(procInfo.mPid));
                if (updateInfo != null) {
                    ...
                    if (curUserUid != 0 || (isCurUserProc(procInfo.mUid, curUserUid) ^ 1) == 0) {
            ...
            if (chkRestartFlag) {
                ...
                if (AppMngConfig.getAbroadFlag() || AppMngConfig.getRestartFlag()) {
                    ...
                    if (getRestartFlagByAppType(value.mAppType)) {
                        isRestartByAppType = true;
                    } else if (!(cachedWhitelist.isLowEnd() || (getAllowCleanResByAppType(value.mAppType) ^ 1) == 0)) {
    ...
    private void adjustClassByStrongAssoc(AwareProcessBlockInfo blockInfo, Map<Integer, AwareProcessInfo> allProcNeedSort, Map<AppBlockKeyBase, AwareProcessBlockInfo> uids, ArrayMap<Integer, AppBlockKeyBase> pidsAppBlock, ArrayMap<Integer, AppBlockKeyBase> uidAppBlock, Map<AppBlockKeyBase, AwareProcessBlockInfo> assocNormalUids) {
        ...
        for (Entry<Integer, AwareProcessInfo> sm : strong.entrySet()) {
            ...
            if (!(procInfo == null || procInfo.mProcInfo == null)) {
                ...
                AwareProcessBlockInfo awareProcessBlockInfo = blockKeyValue == null ? null : (AwareProcessBlockInfo) uids.get(blockKeyValue);
                if (!(awareProcessBlockInfo == null || awareProcessBlockInfo.mProcessList == null || awareProcessBlockInfo.mProcessList.size() <= 0)) {
                    if (awareProcessBlockInfo.mClassRate > blockInfo.mClassRate) {
                        awareProcessBlockInfo.mClassRate = blockInfo.mClassRate;
                        awareProcessBlockInfo.mSubClassRate = blockInfo.mSubClassRate;
                        awareProcessBlockInfo.mSubTypeStr = SUBTYPE_ASSOCIATION;
                        if (assocNormalUids != null) {
                            assocNormalUids.put(blockKeyValue, awareProcessBlockInfo);
                        }
                    } else if (awareProcessBlockInfo.mClassRate == blockInfo.mClassRate && awareProcessBlockInfo.mSubClassRate > blockInfo.mSubClassRate) {
                        awareProcessBlockInfo.mSubClassRate = blockInfo.mSubClassRate;
                        awareProcessBlockInfo.mSubTypeStr = SUBTYPE_ASSOCIATION;
                    }
                }
    ...
    public boolean isGroupBeHigher(int pid, int uid, String processName, ArrayList<String> arrayList, int groupId) {
        ...
        AwareProcessBaseInfo processBaseInfo = this.mHwAMS != null ? this.mHwAMS.getProcessBaseInfo(pid) : null;
        if (processBaseInfo == null) {
            return false;
        }
        if (processBaseInfo.mCurAdj < 200) {
            return true;
        }
        if (groupId == 2) {
            if (processBaseInfo.mCurAdj == 300 || processBaseInfo.mCurAdj == 400) {
                return true;
            }
            return processBaseInfo.mCurAdj == 200 && (isFgServicesImportantByAdjtype(processBaseInfo.mAdjType) ^ 1) != 0;
        }
    }
    ...
    private ArrayMap<Integer, List<AwareProcessBlockInfo>> getAppMngSortGroupForMemCleanChina(int groupId, int memLevel) {
        ...
        int level;
        boolean needKillMore = AppMngConfig.getKillMoreFlag() && memLevel == 1;
        if (needKillMore) {
            level = 1;
        } else {
            level = 0;
        ...
        AwareUserHabit habit = AwareUserHabit.getInstance();
        if (habit != null) {
            map = habit.getAllTopList();
            pkgTopN = habit.getTopN(AppMngConfig.getTopN());
        }
        List<AwareProcessBlockInfo> rawInfo = DecisionMaker.getInstance().decideAll(allAwareProcNeedProcess, level, AppMngFeature.APP_CLEAN, AppCleanSource.MEMORY);
        ...
        if (!(needClean == null || (needClean.isEmpty() ^ 1) == 0)) {
            for (AwareProcessBlockInfo block : needClean) {
                setPropImportance(block, map, pkgTopN);
            }
            if (AppMngConfig.getAbroadFlag()) {
                Collections.sort(needClean, BLOCK_BY_ADJ);
            } else {
    ...
    private void setPropImportance(AwareProcessBlockInfo block, Map<String, Integer> allTopList, List<String> pkgTopN) {
        ...
        if (block != null) {
            ...
            Integer num = null;
            if (allTopList != null) {
                num = (Integer) allTopList.get(block.mPackageName);
            }
            if (pkgTopN != null) {
        ...
        if (list != null && (pw != null || !toPrint)) {
            for (AwareProcessBlockInfo pinfo : list) {
                if (pinfo != null) {
                    ...
                    print(pw, "AppProc:uid:" + pinfo.mUid + ",import:" + pinfo.mImportance + ",classRates:" + pinfo.mClassRate + ",classStr:" + getClassRateStr(pinfo.mClassRate) + ",subStr:" + getClassStr(pinfo.mClassRate, pinfo.mSubClassRate) + ",subTypeStr:" + pinfo.mSubTypeStr + ",appType:" + pinfo.mAppType + ",reason:" + pinfo.mReason);
    ...
    public void dumpRemoveAlarm(PrintWriter pw, String[] args) {
        ...
        if (this.mHwAMS != null) {
            ...
            tags.add(tag);
            this.mHwAMS.removePackageAlarm(pkg, tags, uid);
    ...
    public void dumpAlarm(PrintWriter pw, String[] args) {
        ...
        packageList.add(pkg);
        pw.println("hasPerceptAlarm: " + AwareIntelligentRecg.getInstance().hasPerceptAlarm(uid, packageList));
    }
}

    ...
    public Policy apply(String packageName, AppStartSource source, AwareAppStartStatusCache status, int tristate) {
        ...
        while (node != null && node.hasChild()) {
            ...
            if (childType != null) {
                ...
                if (node != null) {
                    xmlValue = node.getValue();
                }
            } else {
                node = null;
            }
        }
        if (!(node == null || xmlValue == null)) {
            AppStartTag type = (AppStartTag) node.getCurrentType();
            if (type != null && type.equals(AppStartTag.POLICY)) {
                if (xmlValue.getHwStop() == -1 || (AwareIntelligentRecg.getInstance().isPkgHasHwStopFlag(UserHandle.getUserId(status.mUid), packageName) ^ 1) != 0) {
                    return new AppStartPolicy(packageName, xmlValue.getIntValue(), xmlValue.getIndex());
                }
                return new AppStartPolicy(packageName, xmlValue.getHwStop(), xmlValue.getIndex());
            }
        }

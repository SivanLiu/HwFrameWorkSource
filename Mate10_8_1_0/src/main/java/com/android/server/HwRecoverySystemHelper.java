...
public class HwRecoverySystemHelper {
    ...
    public static boolean clearWipeDataFactoryLowlevel(final Context mContext, Intent mIntent) {
        ...
        final boolean wipeEuicc = mIntent.getBooleanExtra("wipeEuicc", false);
        new Thread("Reboot") {
            public void run() {
                try {
                    HwRecoverySystemHelper.rebootWipeUserDataFactoryLowlevel(mContext, wipeEuicc);
    ...
    private static void rebootWipeUserDataFactoryLowlevel(Context context, boolean wipeEuicc) throws IOException {
        ...
        condition.block();
        RecoverySystem.hwWipeEuiccData(context, wipeEuicc);

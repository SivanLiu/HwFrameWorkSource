...
public class ActiveServices {
    ...
    ComponentName startServiceLocked(IApplicationThread caller, Intent service, String resolvedType, int callingPid, int callingUid, boolean fgRequired, String callingPackage, int userId) throws TransactionTooLargeException {
        ...
        if (this.mAm.mUserController.exists(r.userId)) {
            ...
            if (!(pgm == null || callingPackage == null || this.mAm.getUidStateLocked(callingUid) == 2)) {
    ...
    int bindServiceLocked(IApplicationThread caller, IBinder token, Intent service, String resolvedType, IServiceConnection connection, int flags, String callingPackage, int userId) throws TransactionTooLargeException {
        ...
        if ((16777216 & flags) == 0 || (isCallerSystem ^ 1) == 0) {
            ...
            PGManagerInternal pgm = (PGManagerInternal) LocalServices.getService(PGManagerInternal.class);
            if (!(pgm == null || callingPackage == null || this.mAm.getUidStateLocked(callerApp.info.uid) == 2)) {
            ...
            int connectionSize = s.connections.size();
        ...
        try {
            ...
            if (r != null) {
                ...
                if (!(b == null || (b.received ^ 1) == 0)) {
                    ...
                    b.requested = true;
                    b.received = true;
                    int connectionSize = r.connections.size();
                    if (ActivityManagerDebugConfig.HWFLOW && ((long) connectionSize) > SERVICE_CONNECTIONS_THRESHOLD) {
                        Flog.i(102, "publishServiceLocked " + r + ",connection size= " + connectionSize);
                    }
                    long start = SystemClock.uptimeMillis();
                    for (int conni = connectionSize - 1; conni >= 0; conni--) {
                        ArrayList<ConnectionRecord> clist = (ArrayList) r.connections.valueAt(conni);
                        for (int i = 0; i < clist.size(); i++) {
    ...
    private final void bumpServiceExecutingLocked(ServiceRecord r, boolean fg, String why) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
            ...
            StringBuilder append = new StringBuilder().append(">>> EXECUTING ").append(why).append(" of ").append(r).append(" in app ").append(r.app).append(", r.executeNesting: ").append(r.executeNesting).append(", fg: ").append(fg).append(", r.app.execServicesFg: ").append(r.app == null ? false : r.app.execServicesFg).append(", r.app.executingServices.size: ");
            if (r.app != null) {
                i = r.app.executingServices.size();
            }
            Slog.v(str, append.append(i).toString());
        ...
        if (r.app != null && r.app.thread != null) {
            ...
            if (this.mAm.mUserController.hasStartedUserState(r.userId)) {
                ...
                if (isolated) {
                    ...
                    if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
                        Slog.v(TAG_MU, "bringUpServiceLocked: appInfo.uid=" + r.appInfo.uid + " app=" + app + " app.thread=" + (app != null ? app.thread : null));
                    }
                ...
                if (app == null && (permissionsReviewRequired ^ 1) != 0) {
                    ...
                    app = this.mAm.startProcessLocked(procName, r.appInfo, true, intentFlags, hostingType, r.name, false, isolated, 0, false, null, null, initPCEntryArgs(r), null);
                    if (app == null) {
    ...
    private final void bringDownServiceLocked(ServiceRecord r) {
        ...
        long end = SystemClock.uptimeMillis();
        if (ActivityManagerDebugConfig.HWFLOW && end - start > 1000) {
            Flog.i(102, "bringDownServiceLocked " + r + ",took " + (end - start) + "ms");
        }
        if (!(r.app == null || r.app.thread == null)) {
                ...
                if (sameComponent && (service.app == null || evenPersistent || (service.app.persistent ^ 1) != 0)) {
                    ...
                    if (service.app != null) {
                        ...
                        if (service.app.executingServices.size() == 1 && service.app.executingServices.contains(service)) {
                            Slog.w(TAG, "Remove timeout message for service: " + service);
    ...
    final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        for (i = app.services.size() - 1; i >= 0; i--) {
            ...
            if (curRec == sr) {
                ...
                if (allowRestart && ((long) sr.crashCount) >= this.mAm.mConstants.BOUND_SERVICE_MAX_CRASH_RETRY && (sr.serviceInfo.applicationInfo.flags & 8) == 0) {
    ...
    private String[] initPCEntryArgs(ServiceRecord r) {
        ...
        if (TextUtils.equals(r.intent.getIntent().getAction(), "android.view.InputMethod")) {
            ...
            strArr = (wmi == null || !HwPCUtils.enabledInPad()) ? (wmi == null || !wmi.isHardKeyboardAvailable()) ? new String[]{String.valueOf(0)} : new String[]{String.valueOf(this.mAm.mWindowManager.getFocusedDisplayId())} : new String[]{String.valueOf(HwPCUtils.getPCDisplayID())};
        }
        if (TextUtils.equals(r.appInfo.packageName, "com.huawei.desktop.systemui") || TextUtils.equals(r.appInfo.packageName, "com.huawei.desktop.explorer")) {
            strArr = new String[]{String.valueOf(-HwPCUtils.getPCDisplayID())};
        }
        return strArr;
    }
}

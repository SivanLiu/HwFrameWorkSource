...
final class UserController {
    ...
    /* synthetic */ void lambda$-com_android_server_am_UserController_19724(UserState uss) {
        finishUserUnlockedCompleted(uss);
    }
    ...
    int restartUser(int userId, final boolean foreground) {
        return stopUser(userId, true, new IStopUserCallback.Stub() {
            /* synthetic */ void lambda$-com_android_server_am_UserController$3_23658(int userId, boolean foreground) {
            ...
            if (!(userInfo != null ? userInfo.isHwHiddenSpace() : false)) {
                try {
                    if (this.mIsSupportISec) {
                        getStorageManager().lockUserKeyISec(userId);
                    } else {
            ...
            synchronized (this.mLock) {
                ...
                if (oldUserId != userId) {
                    ...
                    if (foreground && this.mUserSwitchUiEnabled && (this.mInjector.shouldSkipKeyguard(lastUserInfo, userInfo) ^ 1) != 0) {
                        this.mInjector.getWindowManager().startFreezingScreen(17432709, 17432708);
                    }
                    ...
                    if (!(getUserInfo(userIdInt.intValue()) != null ? getUserInfo(userIdInt.intValue()).isClonedProfile() : false)) {
        ...
        synchronized (this.mLock) {
            ...
            if (!StorageManager.isUserKeyUnlocked(userId)) {
                try {
                    ...
                    Slog.w(TAG, "Failed to unlock: " + e.getMessage() + " ,SupportISec: " + this.mIsSupportISec);
                }
            } else if (this.mIsSupportISec) {
                if (token == null && secret == null) {
                    Slog.w(TAG, "is SupportISec,Failed to unlockUserScreenISec: token is null  And secret is null");
                } else {
                    boolean isSuccess = false;
                    try {
                        isSuccess = storageManager.setScreenStateFlag(userId, userInfo.serialNumber, 2);
                    } catch (Exception e2) {
                        Slog.w(TAG, "is SupportISec,Failed to setScreenStateFlag: " + e2.getMessage());
                    }
                    if (isSuccess) {
                        final int i = userId;
                        final byte[] bArr = token;
                        final byte[] bArr2 = secret;
                        this.mHandler.post(new Runnable() {
                            public void run() {
                                try {
                                    storageManager.unlockUserScreenISec(i, userInfo.serialNumber, bArr, bArr2, 1);
                                } catch (Exception e) {
                                    Slog.w(UserController.TAG, "is SupportISec,Failed to unlockUserScreenISec: " + e.getMessage());
                                }
                            }
                        });
                    }

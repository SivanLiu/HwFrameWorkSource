...
public class HwMtmBroadcastResourceManager implements AbsHwMtmBroadcastResourceManager {
    ...
    private boolean processBroadcastScheduler(boolean isParallel, BroadcastRecord r, Object target) {
        ...
        if (filter == null || filter.countActionFilters() <= 0) {
            ...
            AwareLog.w(str, append.append(obj).append(", count: ").append(filter == null ? "null" : Integer.valueOf(filter.countActionFilters())).toString());
    ...
    private boolean enqueueIawareProxyBroacast(boolean isParallel, BroadcastRecord r, Object target) {
        ...
        if (r.callingPid != this.mSysServicePid) {
            return false;
        }
        ...
        if (isAbnormalValue(pkg, pid, uid)) {
            return false;
        }
        ...
        if (this.mIawareBrPolicy.isNotProxySysPkg(pkg, action) || isInstrumentationApp(target)) {
            return false;
        }
        ...
        if (-1 == curProcState || -10000 == curAdj) {
            return false;
        }
        ...
        hwBroadcastRecord.setReceiverPid(pid);
        hwBroadcastRecord.setReceiverCurAdj(curAdj);
        hwBroadcastRecord.setReceiverPkg(pkg);
        hwBroadcastRecord.setSysApp(isSystemApp);
        hwBroadcastRecord.setReceiverCurProcState(curProcState);
    ...
    private void trackBrFlow(boolean enqueue, boolean isParallel, BroadcastRecord r, Object target) {
        if (getBrDumpRadar() != null && r != null && isParallel && target != null && ((target instanceof BroadcastFilter) ^ 1) == 0) {
            ...
            boolean startProxy = policy == null ? false : policy.getStartProxy();
            if (policy != null) {
                this.mBrDumpRadar.trackBrFlowDetail(enqueue, isProxyed, startProxy, packageName, action);
                this.mBrDumpRadar.trackBrFlowSpeed(enqueue, isProxyed, policy.isSpeedNoCtrol() ^ 1, policy.isScreenOff() ^ 1);
            }
        }
    }

...
public final class HwActivityStarter extends ActivityStarter {
    ...
    final int startActivityLocked(IApplicationThread caller, Intent intent, Intent ephemeralIntent, String resolvedType, ActivityInfo aInfo, ResolveInfo rInfo, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int callingPid, int callingUid, String callingPackage, int realCallingPid, int realCallingUid, int startFlags, ActivityOptions options, boolean ignoreTargetSecurity, boolean componentSpecified, ActivityRecord[] outActivity, TaskRecord inTask, String reason) {
        if (intent.getComponent() != null && HwDeviceManager.disallowOp(4, intent.getComponent().getPackageName())) {
            ...
            UiThread.getHandler().post(new Runnable() {
                public void run() {
                    ...
                    if (context != null) {
                        Toast toast = Toast.makeText(context, context.getString(33686058), 0);
    ...
    int startActivityMayWait(IApplicationThread caller, int callingUid, String callingPackage, Intent intent, String resolvedType, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int startFlags, ProfilerInfo profilerInfo, WaitResult outResult, Configuration globalConfig, Bundle bOptions, boolean ignoreTargetSecurity, int userId, TaskRecord inTask, String reason) {
                    ...
                    if (rInfo.activityInfo != null) {
                        if (shouldDisplayClonedAppToChoose(callerApplication == null ? null : callerApplication.info.packageName, intent, resolvedType, rInfo, userId, mapForwardUserId, shouldCheckDual)) {
                            ...
                            try {
                                ...
                                intent2 = Intent.createChooser(intent, this.mService.mContext.getResources().getText(17041236));
        ...
        return super.startActivityMayWait(caller, callingUid, callingPackage, intent, resolvedType, voiceSession, voiceInteractor, resultTo, resultWho, requestCode, startFlags, profilerInfo, outResult, globalConfig, bOptions, ignoreTargetSecurity, userId, inTask, reason);
    }
    ...
    private boolean shouldDisplayClonedAppToChoose(String callerPackageName, Intent intent, String resolvedType, ResolveInfo rInfo, int userId, Map<String, Integer> mapForwardUserId, boolean shouldCheckDual) {
        ...
        if (GestureNavConst.DEFAULT_LAUNCHER_PACKAGE.equals(callerPackageName) || "android".equals(callerPackageName) || FingerViewController.PKGNAME_OF_KEYGUARD.equals(callerPackageName) || "com.android.settings".equals(callerPackageName)) {
            return false;
        }
        ...
        if ((shouldCheckDual || HwPackageManagerService.isSupportCloneAppInCust(callerPackageName)) && userId != 0) {
            userInfo = this.mService.mUserController.mInjector.getUserManagerInternal().findClonedProfile();
    ...
    protected void setInitialState(ActivityRecord r, ActivityOptions options, TaskRecord inTask, boolean doResume, int startFlags, ActivityRecord sourceRecord, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor) {
        if (HwPCUtils.isPcCastModeInServer() && (HwPCUtils.enabledInPad() ^ 1) != 0 && options != null && HwPCUtils.isValidExtDisplayId(options.getLaunchDisplayId()) && (TextUtils.equals(r.packageName, "com.android.incallui") || TextUtils.equals(r.packageName, FingerViewController.PKGNAME_OF_KEYGUARD))) {
    ...
    protected boolean hasStartedOnOtherDisplay(ActivityRecord startActivity, int sourceDisplayId) {
        if (!HwPCUtils.isPcCastModeInServer() || ("com.huawei.android.hwpay".equals(startActivity.packageName) ^ 1) == 0 || (HwBluetoothBigDataService.BIGDATA_RECEIVER_PACKAGENAME.equals(startActivity.packageName) ^ 1) == 0 || packageShouldNotHandle(startActivity.packageName)) {
            return false;
        }
    ...
    protected boolean killProcessOnOtherDisplay(ActivityRecord startActivity, int sourceDisplayId) {
        if (HwPCUtils.isPcCastModeInServer()) {
            ...
            if (HwBluetoothBigDataService.BIGDATA_RECEIVER_PACKAGENAME.equals(startActivity.packageName) || "com.huawei.android.hwpay".equals(startActivity.packageName)) {
                killPackageProcess = false;
            }
            if (killPackageProcess) {
                this.mService.forceStopPackageLocked(startActivity.packageName, UserHandle.getAppId(startActivity.appInfo.uid), false, false, true, false, false, UserHandle.getUserId(startActivity.appInfo.uid), "relaunch due to in diff display");
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                if (uri != null && "file".equals(uri.getScheme()) && uri.getPath().startsWith(sdcard)) {
                    if (charSequence == null) {
                        StorageManager storageManager = (StorageManager) this.mService.mContext.getSystemService("storage");

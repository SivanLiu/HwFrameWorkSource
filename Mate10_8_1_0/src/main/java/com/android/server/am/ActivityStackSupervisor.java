...
public class ActivityStackSupervisor extends AbsActivityStackSupervisor implements DisplayListener {
    ...
    public class ActivityDisplay extends ConfigurationContainer {
    ...
    public ActivityStackSupervisor(ActivityManagerService service, Looper looper) {
        ...
        this.mIsPerfBoostEnabled = this.mService.mContext.getResources().getBoolean(17956949);
        this.mIsperfDisablepackingEnable = this.mService.mContext.getResources().getBoolean(17956927);
        if (this.mIsPerfBoostEnabled) {
            this.lBoostTimeOut = this.mService.mContext.getResources().getInteger(17694940);
            this.lBoostCpuParamVal = this.mService.mContext.getResources().getIntArray(17236052);
        }
        if (this.mIsperfDisablepackingEnable) {
            this.lDisPackTimeOut = this.mService.mContext.getResources().getInteger(17694932);
            this.lBoostPackParamVal = this.mService.mContext.getResources().getIntArray(17236051);
        }
    }
    ...
    private boolean taskTopActivityIsUser(TaskRecord task, int userId) {
        ...
        if (activityRecord2 == null || activityRecord2.userId != userId) {
            return false;
        }
        return true;
    }
        ...
        for (int displayNdx = this.mActivityDisplays.size() - 1; displayNdx >= 0; displayNdx--) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; stackNdx--) {
                ...
                if (isFocusedStack(stack)) {
                    ...
                    if (!(top == null || (this.mTmpActivityList.contains(top) ^ 1) == 0)) {
                        this.mTmpActivityList.add(top);
                        Slog.d(TAG, "attachApplicationLocked add top running activity: " + top);
                    }
    ...
    ActivityRecord topRunningActivityLocked() {
        ...
        for (int i = this.mTmpOrderedDisplayIds.size() - 1; i >= 0; i--) {
            ...
            List<ActivityStack> stacks = ((ActivityDisplay) this.mActivityDisplays.get(displayId)).mStacks;
            if (stacks != null) {
                if (!HwVRUtils.isVRMode() || (HwPCUtils.isValidExtDisplayId(displayId) ^ 1) == 0 || (HwVRUtils.isValidVRDisplayId(displayId) ^ 1) == 0) {
                    for (int j = stacks.size() - 1; j >= 0; j--) {
                        ActivityStack stack = (ActivityStack) stacks.get(j);
                        if (stack != focusedStack && isFrontStackOnDisplay(stack) && stack.isFocusable() && (HwVRUtils.isVRDynamicStack(stack.getStackId()) || !isStackInVisible(stack))) {
                    ...
                    Slog.e(TAG, "topRunningActivityLocked is not ValidExtDisplayId ,displayId = " + displayId);
                }
            }
        }
        return null;
    ...
    ActivityInfo resolveActivity(Intent intent, ResolveInfo rInfo, int startFlags, ProfilerInfo profilerInfo) {
        ...
        if (activityInfo != null) {
            ...
            if (activityInfo.launchToken == null && intentLaunchToken != null) {
                activityInfo.launchToken = intentLaunchToken;
            }
        }
        return activityInfo;
    }
            ...
            try {
                ...
                if (this.mService.mProfileApp != null && this.mService.mProfileApp.equals(app.processName) && (this.mService.mProfileProc == null || this.mService.mProfileProc == app)) {
                    ...
                    if (!(profilerInfoSvc == null || profilerInfoSvc.profileFile == null)) {
                        ...
                        ProfilerInfo profilerInfo2 = new ProfilerInfo(profilerInfoSvc);
                    }
                }
                app.hasShownUi = true;
                ...
                app.thread.scheduleLaunchActivity(new Intent(r.intent), r.appToken, System.identityHashCode(r), r.info, mergedConfiguration.getGlobalConfiguration(), mergedConfiguration.getOverrideConfiguration(), r.compat, r.launchedFromPackage, task.voiceInteractor, app.repProcState, r.icicle, r.persistentState, list, newIntents, andResume ^ 1, this.mService.isNextTransitionForward(), profilerInfo);
                if (forceHardAccel) {
                    activityInfo = r.info;
                ...
                if (r.launchFailed) {
                    Slog.e(TAG, "Second failure launching " + r.intent.getComponent().flattenToShortString() + ", giving up", e2);
    ...
    void startSpecificActivityLocked(ActivityRecord r, boolean andResume, boolean checkConfig) {
        ...
        if (r.intent.getComponent() != null) {
            equals = "com.huawei.android.launcher".equals(r.intent.getComponent().getPackageName());
        } else {
            equals = false;
        }
        if (equals) {
    ...
    ActivityStack getNextFocusableStackLocked(ActivityStack currentFocus) {
        ...
        for (int i = this.mTmpOrderedDisplayIds.size() - 1; i >= 0; i--) {
            ...
            if (HwVRUtils.isVRMode() && displayId == 0) {
                Slog.e(TAG, "getNextFocusableStackLocked, is VR mode, dispalyId  " + displayId + " continue");
            } else {
                List<ActivityStack> stacks = getActivityDisplayOrCreateLocked(displayId).mStacks;
                for (int j = stacks.size() - 1; j >= 0; j--) {
                    ActivityStack stack = (ActivityStack) stacks.get(j);
                    if (HwVRUtils.isVRDynamicStack(stack.getStackId())) {
                        if (stack != currentFocus && stack.isFocusable()) {
                            return stack;
                        }
                    } else if (!(stack == currentFocus || !stack.isFocusable() || stack.shouldBeVisible(null) == 0)) {
                        return stack;
                    }
                }
                continue;
    ...
    /* synthetic */ void lambda$-com_android_server_am_ActivityStackSupervisor_131760(int fromStackId, boolean onTop) {
    ...
    ActivityStack createStack(int stackId, ActivityDisplay display, boolean onTop) {
        switch (stackId) {
            ...
            default:
                return HwServiceFactory.createActivityStack(display, stackId, this, this.mRecentTasks, onTop);
        }
    }
    ...
    /* synthetic */ void lambda$-com_android_server_am_ActivityStackSupervisor_140703(int stackId) {
                ...
                if (HwDeviceManager.disallowOp(3, packageName)) {
                    ...
                    UiThread.getHandler().post(new Runnable() {
                        public void run() {
                            ...
                            if (context != null) {
                                Toast toast = Toast.makeText(context, context.getString(33686057), 0);
                    ...
                    return false;
                } else if (HwPCUtils.enabledInPad() && HwPCUtils.isPcCastModeInServer() && "com.android.incallui".equals(packageName)) {
        ...
        if (killProcess) {
            ...
            if (!shouldNotKillProcWhenRemoveTask(pkg)) {
                ...
                for (i = 0; i < procsToKill.size(); i++) {
                    ...
                    if (pr != null) {
                        if (pr.curAdj >= 900 && (pr.info.flags & 1) != 0 && (pr.info.hwFlags & 33554432) == 0 && notKillProcessWhenRemoveTask(pr)) {
    ...
    final ArrayList<ActivityRecord> processStoppingActivitiesLocked(ActivityRecord idleActivity, boolean remove, boolean processPausingActivities) {
        ...
        for (int activityNdx = this.mStoppingActivities.size() - 1; activityNdx >= 0; activityNdx--) {
            ...
            boolean contains = this.mActivitiesWaitingForVisibleActivity.contains(s);
            if (ActivityManagerDebugConfig.DEBUG_STATES) {
                Slog.v(TAG, "Stopping " + s + ": nowVisible=" + nowVisible + " waitingVisible=" + contains + " finishing=" + s.finishing);
            }
            if (contains && nowVisible) {
            ...
            if (remove) {
                ...
                if (!contains || shouldSleepOrShutDown) {
    ...
    static boolean dumpHistoryList(FileDescriptor fd, PrintWriter pw, List<ActivityRecord> list, String prefix, String label, boolean complete, boolean brief, boolean client, String dumpPackage, boolean needNL, String header1, String header2) {
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            ...
            printed = true;
            int isInHistory = !brief ? !complete ? r.isInHistory() ^ 1 : 1 : 0;
            if (needNL) {
                pw.println("");
                needNL = false;
            }
    ...
    private void handleNonResizableTaskIfNeeded(TaskRecord task, int preferredStackId, int preferredDisplayId, int actualStackId, boolean forceNonResizable) {
        ...
        if (task != null) {
            if (HwVRUtils.isVRMode() || HwVRUtils.isVRDynamicStack(preferredStackId)) {
                Slog.e(TAG, "handleNonResizableTaskIfNeeded preferredStackId = " + preferredStackId);
            } else if ((isStackDockedInEffect(actualStackId) || preferredStackId == 3 || (isSecondaryDisplayPreferred ^ 1) == 0) && (!task.isHomeTask() || getConfiguration().extraConfig.getConfigItem(2) == 2)) {
                ...
                if (launchOnSecondaryDisplayFailed || (task.supportsSplitScreen() ^ 1) != 0 || forceNonResizable || (topActivity != null && "com.huawei.systemmanager".equals(topActivity.appInfo.packageName))) {
                    boolean z;
                    ...
                    } else if (!HwPCUtils.isPcDynamicStack(topActivity.getStackId())) {
                    ...
                    moveTasksToFullscreenStackLocked(3, z);
                } else if (topActivity != null && topActivity.isNonResizableOrForcedResizable() && (topActivity.noDisplay ^ 1) != 0 && !HwPCUtils.isExtDynamicStack(topActivity.getStackId())) {
                    int reason;
    ...
    final int startActivityFromRecentsInner(int taskId, Bundle bOptions) {
        ...
        if (this.mService.mUserController.shouldConfirmCredentials(task.userId) || task.getRootActivity() == null) {
            ...
            int result = this.mService.startActivityInPackage(callingUid, callingPackage, intent, null, null, null, 0, 0, bOptions, task.userId, task, "startActivityFromRecents");
            if (launchStackId == 3) {
        ...
        this.mService.mActivityStarter.sendPowerHintForLaunchStartIfNeeded(true, targetActivity);
        ...
        this.mService.mActivityStarter.postStartActivityProcessing(task.getTopActivity(), 2, topActivity != null ? topActivity.getTask().getStackId() : -1, topActivity, task.getStack());
        if (HwPCUtils.isPcCastModeInServer()) {

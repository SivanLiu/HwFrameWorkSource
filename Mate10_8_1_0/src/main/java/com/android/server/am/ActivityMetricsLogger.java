...
class ActivityMetricsLogger {
    ...
    void notifyActivityLaunched(int resultCode, ActivityRecord launchedActivity) {
        ...
        if (processRecord != null) {
            hasStartedActivity = hasStartedActivity(processRecord, launchedActivity) ^ 1;
        } else {
            hasStartedActivity = true;
        }
        notifyActivityLaunched(resultCode, launchedActivity, processRunning, hasStartedActivity);
    }
    ...
    void logAppTransitionReportedDrawn(ActivityRecord r, boolean restoredFromBundle) {
        StackTransitionInfo info = (StackTransitionInfo) this.mLastStackTransitionInfo.get(r.getStackId());
        if (info != null) {
            int i;
            LogMaker builder = new LogMaker(1090);
            builder.setPackageName(r.packageName);
            builder.addTaggedData(871, r.info.name);
            builder.addTaggedData(1091, Long.valueOf(SystemClock.uptimeMillis() - this.mLastTransitionStartTime));

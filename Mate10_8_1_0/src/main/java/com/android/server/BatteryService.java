...
public class BatteryService extends AbsBatteryService {
    ...
    public BatteryService(Context context) {
        ...
        this.mCriticalBatteryLevel = this.mContext.getResources().getInteger(17694759);
        ...
        if (new File("/sys/devices/virtual/switch/invalid_charger/state").exists()) {
            new UEventObserver() {
                public void onUEvent(UEvent event) {
                    int invalidCharger = BatteryService.CHARGE_WIRED_STATUS.equals(event.get("SWITCH_STATE")) ? 1 : 0;
                    synchronized (BatteryService.this.mLock) {
    ...
    private void sendIntentLocked() {
        ...
        Slog.i(TAG, "normal charge status : " + status);
        intent.putExtra(DeviceStorageMonitorService.EXTRA_SEQUENCE, this.mSequence);
    ...
    private int getIconLocked(int level) {
        ...
        if (this.mBatteryProps.batteryStatus == 4 || this.mBatteryProps.batteryStatus == 5) {
            return (!isPoweredLocked(7) || this.mBatteryProps.batteryLevel < 100) ? 17303459 : 17303473;
        } else {
            return 17303487;
        }
    }

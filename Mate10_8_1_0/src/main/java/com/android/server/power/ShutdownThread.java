...
public final class ShutdownThread extends Thread {
    ...
    private static ProgressDialog showShutdownDialog(Context context) {
        ...
        if (mReason != null && mReason.startsWith("recovery-update")) {
            ...
            pd.setTitle(context.getText(17040873));
            if (mRebootHasProgressBar) {
                ...
                pd.setMessage(context.getText(17040871));
            } else if (showSysuiReboot()) {
                return null;
            } else {
                pd.setIndeterminate(true);
                pd.setMessage(context.getText(17040872));
            }
        } else if (mReason == null || !mReason.equals("recovery")) {
            if (showSysuiReboot()) {
                return null;
            }
            pd.setTitle(context.getText(17040844));
            pd.setMessage(context.getText(17040995));
            pd.setIndeterminate(true);
            ...
            pd.setTitle(context.getText(17040869));
            pd.setMessage(context.getText(17040868));
            pd.setIndeterminate(true);
        }
        ...
        final ProgressListener progressListener = new ProgressListener() {
            public void onProgress(int status) {
                if (status >= 0 && status < 100) {
                    ShutdownThread.sInstance.setRebootProgress(((int) ((((double) status) * 80.0d) / 100.0d)) + 20, ShutdownThread.this.mContext.getText(17040870));
                } else if (status == 100) {
                    ShutdownThread.sInstance.setRebootProgress(status, ShutdownThread.this.mContext.getText(17040872));
                }
            }
        };

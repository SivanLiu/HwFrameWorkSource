...
public class ServiceStateTracker extends Handler {
    ...
    protected static final int EVENT_RADIO_POWER_OFF = 53;
        ...
        public void onSubscriptionsChanged() {
            ...
            if (this.mPreviousSubId.getAndSet(subId) != subId) {
                if (SubscriptionManager.isValidSubscriptionId(subId)) {
                    ...
                    ServiceStateTracker.this.mPhone.sendSubscriptionSettings(context.getResources().getBoolean(17957096) ^ 1);
    ...
    public void updateTimeZoneNoneNitz() {
        if (Global.getInt(this.mPhone.getContext().getContentResolver(), "auto_time_zone", 0) != 0) {
            ...
            if (!(iso == null || (TextUtils.isEmpty(iso) ^ 1) == 0)) {
                ...
                if (uniqueZoneIds.size() == 1 || defaultZones != null) {
                    zoneId = (String) uniqueZoneIds.get(0);
                    if (defaultZones != null) {
                        zoneId = defaultZones.getID();
    ...
    public boolean updateTimeByNitz() {
    ...
    public ServiceStateTracker(GsmCdmaPhone phone, CommandsInterface ci) {
        ...
        this.mVoiceCapable = this.mPhone.getContext().getResources().getBoolean(17957060);
        ...
        updatePhoneType();
        this.mCi.registerForOffOrNotAvailable(this, 53, null);
    ...
    public void dispose() {
        ...
        if (this.mHwCustGsmServiceStateTracker.isDataOffForbidLTE() || this.mHwCustGsmServiceStateTracker.isDataOffbyRoamAndData()) {
    ...
    protected void updatePhoneObject() {
        if (this.mPhone.getContext().getResources().getBoolean(17957041)) {
        ...
        if (this.mHwCustGsmServiceStateTracker == null || !this.mHwCustGsmServiceStateTracker.handleMessage(msg)) {
            ...
            switch (msg.what) {
                ...
                case 17:
                    ...
                    boolean skipRestoringSelection = this.mPhone.getContext().getResources().getBoolean(17957096);
                ...
                case 43:
                    ...
                    if (!(ar.userObj instanceof CellInfoResult)) {
                        log("EVENT_GET_CELL_INFO_LIST userObj:" + ar.userObj);
                        break;
                    }
                ...
                case 48:
                    ...
                    if (!(this.mPhone == null || this.mPhone.getImsPhone() == null)) {
                        ...
                        if (tempPreVowifiState && (this.mPhone.getImsPhone().isWifiCallingEnabled() ^ 1) != 0) {
                            pollState();
                            break;
                        }
                    }
        ...
        switch (what) {
            case 4:
                ...
                if (this.mPhone.isPhoneTypeGsm()) {
                    ...
                    boolean isVoiceCapable = this.mPhone.getContext().getResources().getBoolean(17957060);
                    ...
                    if (regCodeIsRoaming(registrationState)) {
                        isRoamIndForHomeSystem = isRoamIndForHomeSystem(Integer.toString(roamingIndicator)) ^ 1;
                    } else {
                        isRoamIndForHomeSystem = false;
                    }
                    this.mNewSS.setVoiceRoaming(isRoamIndForHomeSystem);
            ...
            case 5:
                ...
                if (this.mPhone.isPhoneTypeGsm()) {
                    ...
                    if ((oldDataRAT != 0 || newDataRat == 0) && !(ServiceState.isCdma(oldDataRAT) && ServiceState.isLte(newDataRat))) {
                        if (ServiceState.isLte(oldDataRAT) && ServiceState.isCdma(newDataRat)) {
                        }
                        isDataRoaming = regCodeIsRoaming(regState);
                        this.mNewSS.setDataRoaming(isDataRoaming);
                        this.mNewSS.setDataRoamingFromRegistration(isDataRoaming);
                ...
                if (this.mPhone.isPhoneTypeGsm()) {
                    ...
                    if (opNames != null && opNames.length >= 3) {
                        ...
                        if (operatorBrandOverride != null) {
                            ...
                            this.mNewSS.setOperatorName(operatorBrandOverride, operatorBrandOverride, opNames[2]);
                        } else {
                            this.mNewSS.setOperatorName(opNames[0], opNames[1], opNames[2]);
                        }
                        upatePlmn(operatorBrandOverride, opNames[0], opNames[1], opNames[2]);
                        return;
                    }
                    return;
                }
    ...
    public void updateSpnDisplay() {
        ...
        if (this.mPhone.getImsPhone() != null && this.mPhone.getImsPhone().isWifiCallingEnabled() && combinedRegState == 0) {
            String[] wfcSpnFormats = this.mPhone.getContext().getResources().getStringArray(17236076);
            int voiceIdx = 0;
            int dataIdx = 0;
            ...
            charSequence = wfcSpnFormats[voiceIdx];
        ...
        if (this.mPhone.isPhoneTypeGsm()) {
            ...
            if (combinedRegState == 1 || combinedRegState == 2) {
                ...
                if (this.mEmergencyOnly) {
                    plmn = Resources.getSystem().getText(17039946).toString();
                } else {
                    plmn = Resources.getSystem().getText(17040285).toString();
                }
                if (this.mHwCustGsmServiceStateTracker != null) {
            ...
            if (!TextUtils.isEmpty(spn) && (TextUtils.isEmpty(charSequence) ^ 1) != 0 && (TextUtils.isEmpty(wfcDataSpnFormat) ^ 1) != 0) {
                dataSpn = String.format(wfcDataSpnFormat, new Object[]{spn.trim()});
                showSpn = true;
                showPlmn = false;
            } else if (!TextUtils.isEmpty(plmn) && (TextUtils.isEmpty(charSequence) ^ 1) != 0) {
        ...
        if (this.mSubId == subId && (TextUtils.equals(plmn, this.mCurPlmn) ^ 1) == 0 && this.mCurShowWifi == showWifi) {
            if ((TextUtils.equals(wifi, this.mCurWifi) ^ 1) != 0) {
            }
            updateOperatorProp();
            this.mSubId = subId;
    ...
    private void pollStateDone() {
        ...
        boolean voiceRoaming2 = this.mSS.getVoiceRoaming() ? this.mNewSS.getVoiceRoaming() ^ 1 : false;
        boolean dataRoaming = !this.mSS.getDataRoaming() ? this.mNewSS.getDataRoaming() : false;
        boolean dataRoaming2 = this.mSS.getDataRoaming() ? this.mNewSS.getDataRoaming() ^ 1 : false;
        ...
        if (this.mPhone.isPhoneTypeCdmaLte()) {
            if (this.mNewSS.getDataRegState() != 0) {
                ...
                z = ServiceState.isLte(this.mNewSS.getRilDataRadioTechnology());
            } else {
                z = false;
            }
        ...
        log("pollStateDone: hasRegistered=" + hasRegistered + " hasDeregistered=" + hasDeregistered + " hasDataAttached=" + hasDataAttached + " hasDataDetached=" + hasDataDetached + " hasDataRegStateChanged=" + hasDataRegStateChanged + " hasRilVoiceRadioTechnologyChanged= " + hasRilVoiceRadioTechnologyChanged + " hasRilDataRadioTechnologyChanged=" + hasRilDataRadioTechnologyChanged + " hasChanged=" + hasChanged + " hasVoiceRoamingOn=" + voiceRoaming + " hasVoiceRoamingOff=" + voiceRoaming2 + " hasDataRoamingOn=" + dataRoaming + " hasDataRoamingOff=" + dataRoaming2 + " hasLocationChanged=" + hasLocationChanged + " has4gHandoff = " + z + " hasMultiApnSupport=" + hasMultiApnSupport + " hasLostMultiApnSupport=" + hasLostMultiApnSupport + " hasOperatorNumericChanged" + hasOperatorNumericChanged);
        ...
        if (hasChanged) {
            ...
            if (isInvalidOperatorNumeric(operatorNumeric)) {
                ...
                if (!(this.mNitzUpdatedTime || (mcc.equals(INVALID_MCC) ^ 1) == 0 || (TextUtils.isEmpty(iso) ^ 1) == 0 || !getAutoTimeZone())) {
                    ...
                    List<String> uniqueZoneIds = TimeUtils.getTimeZoneIdsWithUniqueOffsets(iso);
                    TimeZone defaultZones = getTimeZoneFromMcc(mcc);
                    if (uniqueZoneIds.size() == 1 || testOneUniqueOffsetPath || defaultZones != null) {
                        String zoneId = (String) uniqueZoneIds.get(0);
                        if (defaultZones != null) {
                            TimeZone zone = defaultZones;
                            zoneId = defaultZones.getID();
                            log("some countrys has more than two timezone, choose a default");
                        }
                        ...
                        setAndBroadcastNetworkSetTimeZone(zoneId);
                        HwTelephonyFactory.getHwReportManager().reportSetTimeZoneByIso(this.mPhone, zoneId, this.mNitzUpdatedTime, "Iso");
                    } else {
            ...
            boolean voiceRoaming3 = this.mPhone.isPhoneTypeGsm() ? this.mSS.getVoiceRoaming() : !this.mSS.getVoiceRoaming() ? this.mSS.getDataRoaming() : true;
            tm.setNetworkRoamingForPhone(phoneId, voiceRoaming3);
            if (this.mHwCustGsmServiceStateTracker != null) {
        ...
        if (hasDataAttached || z || hasDataDetached || hasRegistered || hasDeregistered) {
            logAttachChange();
        }
        if (hasDataAttached || z) {
        ...
        if (voiceRoaming || voiceRoaming2 || dataRoaming || dataRoaming2) {
            logRoamingChange();
        }
        if (voiceRoaming) {
        ...
        if (this.mPhone.isPhoneTypeCdma()) {
            if (this.mCi.getRadioState().isOn() && (this.mIsSubscriptionFromRuim ^ 1) != 0) {
                if (this.mSS.getVoiceRegState() == 0) {
                    ...
                    eriText = this.mPhone.getContext().getText(17040929).toString();
                }
    ...
    protected void fixTimeZone(String isoCountryCode, TimeZone mccZone) {
        ...
        if (zone != null) {
            ...
            HwTelephonyFactory.getHwReportManager().reportSetTimeZoneByNitz(this.mPhone, zone.getID(), -1, "NitzFix");
        } else {
    ...
    private void setTimeFromNITZString(String nitz, long nitzReceiveTime) {
        ...
        try {
            ...
            if (zone != null && this.mZoneOffset == tzOffset) {
                ...
                tmpLog = "NITZ: nitz=" + nitz + " nitzReceiveTime=" + nitzReceiveTime + " tzOffset=" + tzOffset + " dst=" + dst + " zone=" + (zone == null ? zone.getID() : "NULL") + " iso=" + iso + " mGotCountryCode=" + this.mGotCountryCode + " mNeedFixZoneAfterNitz=" + this.mNeedFixZoneAfterNitz + " getAutoTimeZone()=" + getAutoTimeZone();
                log(tmpLog);
                ...
                if (zone != null) {
                    if (getAutoTimeZone()) {
                        setAndBroadcastNetworkSetTimeZone(zone.getID());
                        HwTelephonyFactory.getHwNetworkManager().sendNitzTimeZoneUpdateMessage(this.mCellLoc);
                        HwTelephonyFactory.getHwReportManager().reportSetTimeZoneByNitz(this.mPhone, zone.getID(), tzOffset, "Nitz");
                    }
                    saveNitzTimeZone(zone.getID());
                }
                ...
                if (!this.mPhone.isPhoneTypeGsm() || getAutoTime()) {
                    ...
                    if (millisSinceNitzReceived < 0) {
                        ...
                        } else if (Math.abs(c.getTimeInMillis() - System.currentTimeMillis()) < 3000 || this.mSavedAtTime == 0) {
                            setAndBroadcastNetworkSetTime(c.getTimeInMillis());
    ...
    public void setNotification(int notifyType) {
        if (SystemProperties.getBoolean("ro.hwpp.cell_access_report", false)) {
            ...
            if (this.mPhone.getContext().getResources().getBoolean(17957059)) {
                Context context = this.mPhone.getContext();
                ...
                switch (notifyType) {
                    case 1001:
                        if (((long) SubscriptionManager.getDefaultDataSubscriptionId()) == ((long) this.mPhone.getSubId())) {
                            ...
                            details = context.getText(17039476);
                            break;
                        }
                        return;
                    case 1002:
                ...
                this.mNotification = new Builder(context).setWhen(System.currentTimeMillis()).setAutoCancel(true).setSmallIcon(icon).setTicker(title).setColor(context.getResources().getColor(17170772)).setContentTitle(title).setStyle(new BigTextStyle().bigText(details)).setContentText(details).setChannel(NotificationChannelController.CHANNEL_ID_ALERT).build();
        ...
        synchronized (this) {
            ...
            if (!this.mPendingRadioPowerOffAfterDataOff) {
                ...
                if (this.mPhone.isPhoneTypeGsm() || this.mPhone.isPhoneTypeCdmaLte()) {
                    ...
                    String[] networkNotClearData = this.mPhone.getContext().getResources().getStringArray(17236057);
    ...
    private boolean isOperatorChanged(boolean showPlmn, boolean showSpn, String spn, String dataSpn, String plmn, boolean showWifi, String wifi, String regplmn) {
        ...
        if (showPlmn == this.mCurShowPlmn && showSpn == this.mCurShowSpn && (TextUtils.equals(spn, this.mCurSpn) ^ 1) == 0 && (TextUtils.equals(dataSpn, this.mCurDataSpn) ^ 1) == 0 && (TextUtils.equals(plmn, this.mCurPlmn) ^ 1) == 0 && showWifi == this.mCurShowWifi && (TextUtils.equals(wifi, this.mCurWifi) ^ 1) == 0) {
            if ((TextUtils.equals(regplmn, this.mCurRegplmn) ^ 1) == 0) {
                return false;
            }
        }
        return isRealChange;

...
public class HwSignalStrength {
    ...
    private static String getSigCustString(Context context) {
        String custStr = "";
        try {
            custStr = System.getString(context.getContentResolver(), "cust_signal_thresholds");
        } catch (Exception e) {
        }
        return custStr;
    }
    ...
    private boolean isCustConfigValid(String custInfo) {
        ...
        if (config.length < 6) {
            ...
            if (!"true".equals(config[1])) {
                if (!"4".equals(config[0]) || config.length == 6) {
                    if ("5".equals(config[0]) && config.length != 7) {
                    }
                }
                loge("Length of config Error!!:" + config.length);
                return false;
            } else if (("4".equals(config[0]) && config.length != 10) || ("5".equals(config[0]) && config.length != 12)) {
                loge("Length of config Error!!:" + config.length);
                return false;
            }
    ...
    public static boolean isRssiValid(SignalType type, int rssi) {
        switch (-getcom-android-internal-telephony-HwSignalStrength$SignalTypeSwitchesValues()[type.ordinal()]) {
            ...
            case 6:
                if (!(rssi == 0 || rssi == WCDMA_STRENGTH_INVALID || rssi == 99 || rssi > 0)) {
                    if (-1 == rssi) {
                    }
                }
                return false;

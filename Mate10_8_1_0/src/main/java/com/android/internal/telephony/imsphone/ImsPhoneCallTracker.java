...
import vendor.huawei.hardware.radio.V1_2.RilConstS32;
...
public class ImsPhoneCallTracker extends AbstractImsPhoneCallTracker implements ImsPullCall {
    ...
    private Listener mImsCallListener = new Listener() {
        ...
        public void onCallResumeReceived(ImsCall imsCall) {
            ...
            if (ImsPhoneCallTracker.this.mPhone.getContext().getResources().getBoolean(17957054) && ImsPhoneCallTracker.this.mSupportPauseVideo && VideoProfile.isVideo(conn.getVideoState())) {
        ...
        public void onCallMerged(ImsCall call, ImsCall peerCall, boolean swapCalls) {
            ...
            foregroundImsPhoneCall.merge(imsPhoneCall, State.ACTIVE);
            try {
    ...
    static {
        ...
        PRECISE_CAUSE_MAP.append(RilConstS32.RIL_REQUEST_HW_SET_DATA_SWITCH, 1331);
        PRECISE_CAUSE_MAP.append(RilConstS32.RIL_REQUEST_HW_SET_DATA_ROAM_SWITCH, 1332);
        PRECISE_CAUSE_MAP.append(RilConstS32.RIL_REQUEST_HW_SET_IMS_SMS_CONFIG, 1333);
        PRECISE_CAUSE_MAP.append(RilConstS32.RIL_REQUEST_HW_GET_CURRENT_CALLS_V1_2, 1340);
    ...
    /* synthetic */ void lambda$-com_android_internal_telephony_imsphone_ImsPhoneCallTracker_34201() {
        try {
    ...
    /* synthetic */ int lambda$-com_android_internal_telephony_imsphone_ImsPhoneCallTracker_35273() {
                ...
                if (values.length == 3) {
                    try {
                        ...
                        if (values[0].equals(CharacterSets.MIMENAME_ANY_CHARSET)) {
                            ...
                            num = Integer.valueOf(Integer.parseInt(values[0]));
                        }
                        String message = values[1];
                        int toCode = Integer.parseInt(values[2]);
                        addReasonCodeRemapping(num, message, Integer.valueOf(toCode));
                        log(new StringBuilder().append("Loaded ImsReasonInfo mapping : fromCode = ").append(num).toString() == null ? "any" : num + " ; message = " + message + " ; toCode = " + toCode);
    ...
    public int getDisconnectCauseFromReasonInfo(ImsReasonInfo reasonInfo, State callState) {
        ...
        switch (maybeRemapReasonCode(reasonInfo)) {
            ...
            case 331:
            case 340:
            case RilConstS32.RIL_REQUEST_HW_GET_CURRENT_CALLS_V1_2 /*361*/:
            case 362:
                return 12;
            case 332:
                return 12;
            case 333:
            case RilConstS32.RIL_REQUEST_HW_SET_DATA_SWITCH /*352*/:
            case RilConstS32.RIL_REQUEST_HW_SET_IMS_SMS_CONFIG /*354*/:
                return 9;
            case 337:
            case 341:
                return 8;
    ...
    private void updateVtDataUsage(ImsCall call, long dataUsage) {
        ...
        networkStats.combineValues(new Entry(VT_INTERFACE, -1, 1, 0, 1, isRoaming, delta / 2, 0, delta / 2, 0, 0));
        ...
        networkStats.combineValues(new Entry(VT_INTERFACE, this.mDefaultDialerUid.get(), 1, 0, 1, isRoaming, delta / 2, 0, delta / 2, 0, 0));
    ...
    private void setVideoCallProvider(ImsPhoneConnection conn, ImsCall imsCall) throws RemoteException {
        ...
        if (imsVideoCallProvider != null) {
            boolean useVideoPauseWorkaround = this.mPhone.getContext().getResources().getBoolean(17957054);
    ...
    private boolean shouldDisconnectActiveCallOnAnswer(ImsCall activeCall, ImsCall incomingCall) {
        ...
        boolean wasVideoCall = !activeCall.isVideoCall() ? this.mTreatDowngradedVideoCallsAsVideoCalls ? activeCall.wasVideoCall() : false : true;
        ...
        if (ImsManager.isWfcEnabledByPlatform(this.mPhone.getContext())) {
            ...
            isWfcEnabledByUser = ImsManager.isWfcEnabledByUser(this.mPhone.getContext());
        } else {
            isWfcEnabledByUser = false;
        }
        boolean isIncomingCallAudio = incomingCall.isVideoCall() ^ 1;
        log("shouldDisconnectActiveCallOnAnswer : isActiveCallVideo=" + wasVideoCall + " isActiveCallOnWifi=" + isActiveCallOnWifi + " isIncomingCallAudio=" + isIncomingCallAudio + " isVowifiEnabled=" + isWfcEnabledByUser);
        if (wasVideoCall && isActiveCallOnWifi && isIncomingCallAudio) {
            z = isWfcEnabledByUser ^ 1;
        }
        return z;
    }

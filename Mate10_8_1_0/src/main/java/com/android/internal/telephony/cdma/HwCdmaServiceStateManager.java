...
public class HwCdmaServiceStateManager extends HwServiceStateManager {
    ...
    private class DoubleSignalStrength {
        ...
        public void proccessAlaphFilter(DoubleSignalStrength oldDoubleSS, SignalStrength newSS, SignalStrength modemSS, boolean needProcessDescend) {
            ...
            if (newSS.isGsm()) {
                ...
                i = processEvdoDbmAlaphFilter(oldDoubleSS, newSS, modemSS, needProcessDescend);
                if (HwCdmaServiceStateManager.this.FEATURE_SIGNAL_DUALPARAM) {
                    i = (i | processEvdoEcioAlaphFilter(oldDoubleSS, newSS, modemSS, needProcessDescend)) | processEvdoSnrAlaphFilter(oldDoubleSS, newSS, modemSS, needProcessDescend);
                }
            }
            if ((this.mTechState & 8) != 0) {
                i |= processCdmaDbmAlaphFilter(oldDoubleSS, newSS, modemSS, needProcessDescend);
                if (HwCdmaServiceStateManager.this.FEATURE_SIGNAL_DUALPARAM) {
                    i |= processCdmaEcioAlaphFilter(oldDoubleSS, newSS, modemSS, needProcessDescend);
                }
            }
    ...
    public String getPlmn() {
        ...
        if (getCombinedRegState(getSS()) != 1 ? isHisiDsdsCardDeactived(getSS()) : true) {
            plmnValue = Resources.getSystem().getText(17040285).toString();
    ...
    private void setCTRoaming(ServiceState newSS) {
        ...
        if (plmn != null && plmn.length() >= 5 && -1 == INVAILD_PLMN.indexOf(plmn.trim() + "-")) {
            boolean equals = !TextUtils.isEmpty(sim_mcc) ? sim_mcc.equals(nw_mcc) ^ 1 : false;
            log("setCTRoaming: setRoaming" + equals);
            newSS.setRoaming(equals);
        } else if (sid == 0 || -1 == sid) {
            newSS.setRoaming(false);
    ...
    public boolean proccessCdmaLteDelayUpdateRegisterStateDone(ServiceState oldSS, ServiceState newSS) {
        ...
        int i = IS_CMCC_4G_DSDX_ENABLE ? lostCSNetwork : 0;
        ...
        if (newSS.getDataRegState() == 0 || (i ^ 1) != 0 || (this.mSST.getDesiredPowerState() ^ 1) != 0 || this.mPhone.mCi.getRadioState() == RadioState.RADIO_OFF || this.mSST.getDoRecoveryTriggerState() || isCardInvalid(isSubDeactivated, this.mPhone.getSubId()) || this.mMainSlot != userPref4GSlot || newSS.getDataRegState() == 3 || mExternalState == State.PUK_REQUIRED) {

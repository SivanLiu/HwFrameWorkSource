...
public class HwDcTrackerBaseReference implements DcTrackerBaseReference {
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction() != null && intent.getAction().equals("android.intent.action.SIM_STATE_CHANGED")) {
                ...
                editor.commit();
                HwDcTrackerBaseReference.this.log("Put mDSUseDuration into SharedPreferences, put: " + HwDcTrackerBaseReference.this.mDSUseDuration + ", get: " + sp.getInt(HwDcTrackerBaseReference.DS_USE_DURATION_KEY, 0));
                ...
                hwDcTrackerBaseReference.mDSUseDuration = hwDcTrackerBaseReference.mDSUseDuration + lastDSUseDuration;
                HwDcTrackerBaseReference.this.log("Read last mDSUseDuration back from SharedPreferences, lastDSUseDuration: " + lastDSUseDuration + ", mDSUseDuration: " + HwDcTrackerBaseReference.this.mDSUseDuration);
    ...
    public boolean isDataAllowedForRoaming(boolean isMms) {
        ...
        if (-1 != allowMmsPropertyByPlmn) {
            ...
            if (!this.mDcTrackerBase.mPhone.getServiceState().getRoaming() || this.mDcTrackerBase.getDataRoamingEnabled()) {
                isMms = true;
            } else if (!(this.ALLOW_MMS || mmsOnRoaming)) {
                isMms = false;
            }
            return isMms;
        }
        if (!this.mDcTrackerBase.mPhone.getServiceState().getRoaming() || this.mDcTrackerBase.getDataRoamingEnabled()) {
            z = true;
        } else if (this.ALLOW_MMS || MMS_ON_ROAMING) {
            z = isMms;
        }
        return z;
    }
    ...
    public boolean getAnyDataEnabledByApnContext(ApnContext apnContext, boolean enable) {
        ...
        if (this.mDcTrackerBase.mPhone.getServiceState().getRoaming()) {
            ...
            if (-1 != allowMmsPropertyByPlmn) {
                ...
                if ((this.ALLOW_MMS || ignoreDSSwitchOnRoaming) && "mms".equals(apnContext.getApnType())) {
                    enable = true;
                }
                return enable;
            }
            ...
            if (!((this.ALLOW_MMS || ignoreDSSwitchNotRoaming) && "mms".equals(apnContext.getApnType()))) {
                z = enable;
            }
            return z;
        } else {
    ...
    public boolean isPingOk() {
        ...
        try {
            ...
            if (operatorNumeric != null && operatorNumeric.length() > 3) {
                obj = operatorNumeric.substring(0, 3);
                debugLog("isPingOk mcc = " + obj);
            }
    ...
    private int getallowMmsPropertyByPlmn() {
        ...
        PersistableBundle bundle = configLoader.getConfigForSubId(this.mDcTrackerBase.mPhone.getSubId());
        if (bundle == null) {
            return -1;
        }
        int allowMmsPropertyInt = bundle.getInt(ALLOW_MMS_PROPERTY_INT, -1);
        if (allowMmsPropertyInt < 0 || allowMmsPropertyInt > 7) {
            allowMmsPropertyInt = -1;
        }
        log("getallowMmsPropertyByPlmn:allowMmsPropertyInt " + allowMmsPropertyInt);

...
public class HwSIMRecords extends SIMRecords {
        ...
        private void checkDataServiceRemindMsim() {
            ...
            for (String plmn : plmnsConfig.split(",")) {
                if (plmn != null && plmn.equals(HwSIMRecords.this.getOperatorNumeric())) {
                    if (!"true".equals(System.getString(HwSIMRecords.this.mContext.getContentResolver(), HwSIMRecords.ANY_SIM_DETECTED)) && (lDataVal == lSimSlotVal || (r0 ^ 1) != 0)) {
    ...
    protected boolean checkVMEditToSim() {
        boolean custvmToSim = true;
        CarrierConfigManager carrierConfigManager = this.mContext != null ? (CarrierConfigManager) this.mContext.getSystemService("carrier_config") : null;
        PersistableBundle configForSubId = carrierConfigManager != null ? carrierConfigManager.getConfigForSubId(getSlotId()) : null;
        if (configForSubId != null) {
            custvmToSim = configForSubId.getBoolean("vm_edit_to_sim_bool", true);
        }
        Rlog.d("SIMRecords", "custvmToSim=" + custvmToSim + " slotId=" + getSlotId());
        return custvmToSim;
    }
}

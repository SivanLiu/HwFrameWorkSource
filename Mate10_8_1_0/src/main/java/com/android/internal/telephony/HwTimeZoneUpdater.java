...
public class HwTimeZoneUpdater {
    ...
    private void onPollTimeZone(int event) {
        ...
        if (isAutomaticTimeZoneRequested()) {
            ...
            if (!TextUtils.isEmpty(iso)) {
                ...
                List<TimeZone> isoZones = TimeUtils.getTimeZones(iso);
                ...
                fixUniqueZones(uniqueZoneIds, isChinaMCC);
                if (uniqueZoneIds.size() != 1 && !isChinaMCC) {
                    Log.d(TAG, "uniqueZones more than one.");
                } else if (isChinaMCC) {
                    str2 = TimeZone.getTimeZone("Asia/Shanghai").getID();
                } else {
                    str2 = (String) uniqueZoneIds.get(0);
                }
                if (str2 != null) {
                    Log.d(TAG, "zoneId:" + str2);
                    setTimeZone(str2);
                }
                unregisterForScreenOnIntents();
            }
    ...
    private void fixUniqueZones(List<String> uniqueZoneIds, boolean isChinaMCC) {
        String zoneId = null;
        if (uniqueZoneIds.size() != 1 && !isChinaMCC) {
            Log.d(TAG, "uniqueZones more than one.");
        } else if (isChinaMCC) {
            zoneId = TimeZone.getTimeZone("Asia/Shanghai").getID();
        } else {

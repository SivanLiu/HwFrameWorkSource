...
public class RIL extends AbstractRIL implements CommandsInterface {
    ...
    private IRadio getRadioProxy(Message result) {
        if (!this.mIsMobileNetworkSupported) {
            ...
            try {
                huaweiRadio = vendor.huawei.hardware.radio.V1_0.IRadio.getService(HIDL_SERVICE_NAME[this.mPhoneId == null ? 0 : this.mPhoneId.intValue()]);
    ...
    RadioCapability makeStaticRadioCapability() {
        int intValue;
        int raf = 1;
        String rafString = this.mContext.getResources().getString(17039815);

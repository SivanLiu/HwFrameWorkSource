...
public class IccCardProxy extends AbstractIccCardProxy implements IccCard {
    ...
    private void updateQuietMode() {
        synchronized (this.mLock) {
            ...
            if (this.mCurrentAppType == 1) {
                ...
                if (cdmaSource == 1 && this.mCurrentAppType == 2) {
                    newQuietMode = isLteOnCdmaMode ^ 1;
                } else {
                    newQuietMode = false;
                }
    ...
    private void updateIccAvailability() {
        synchronized (this.mLock) {
            ...
            if (newCard != null) {
                ...
                log("state = " + newCard.getCardState() + ", newApp = " + uiccCardApplication);
    ...
    private void updateExternalState() {
        if (this.mUiccCard == null) {
            ...
            if (this.mRadioState == RadioState.RADIO_ON) {
                setExternalState(State.ABSENT);
            } else {
                if (isSimAbsent(this.mContext, this.mUiccCard, this.mRadioState == RadioState.RADIO_ON)) {
                    setExternalState(State.ABSENT);

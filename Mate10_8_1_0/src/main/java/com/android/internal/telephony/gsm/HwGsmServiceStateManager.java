...
public class HwGsmServiceStateManager extends HwServiceStateManager {
    ...
    private static final String EMERGENCY_PLMN = Resources.getSystem().getText(17039946).toString();
    ...
    private static final String NO_SERVICE_PLMN = Resources.getSystem().getText(17040285).toString();
    ...
    private class DoubleSignalStrength {
        ...
        private void setFakeSignalStrengthForSlowDescend(DoubleSignalStrength oldDoubleSS, SignalStrength newSS) {
            ...
            if ((this.mTechState & 4) != 0) {
                ...
                int newLevel = HwGsmServiceStateManager.mHwSigStr.getLevel(SignalType.LTE, newSS.getLteRsrp(), newSS.getLteRssnr());
    ...
    static {
        ...
        if ("389".equals(SystemProperties.get("ro.config.hw_opta"))) {
            z = "840".equals(SystemProperties.get("ro.config.hw_optb"));
        }
        IS_VERIZON = z;
    }
    ...
    public String getPlmn() {
        ...
        Rlog.d(LOG_TAG, "getPlmn languageCode:" + languageCode + "  plmnValue:" + plmnValue);
        if (plmnValue == null) {
            String DEFAULT_PLMN_LANG_EN = "en_us";
            plmnValue = plmnConstants.getPlmnValue(operatorNumeric, "en_us");
            Rlog.d(LOG_TAG, "get default en_us plmn name:" + plmnValue);
        }
        ...
        boolean notUseVirtualName = this.mHwCustGsmServiceStateManager != null ? this.mHwCustGsmServiceStateManager.notUseVirtualName(imsi) : false;
        boolean roaming = getSS().getRoaming();
        boolean iscustRoamingRuleAffect = this.mHwCustGsmServiceStateManager != null ? this.mHwCustGsmServiceStateManager.iscustRoamingRuleAffect(roaming) : false;
        if (notUseVirtualName || iscustRoamingRuleAffect) {
            Rlog.d("HwGsmServiceStateTracker", "passed the Virtualnet cust");
        } else {
            plmnValue = HwTelephonyFactory.getHwPhoneManager().getVirtualNetOperatorName(plmnValue, roaming, hasNitzOperatorName(slotId), slotId, str);
        ...
        boolean hasNitzOperatorName = (apnReminder.isPopupApnSettingsEmpty() || (getSS().getRoaming() ^ 1) == 0 || (hasNitzOperatorName(slotId) ^ 1) == 0 || str == null) ? false : true;
    ...
    private boolean isChinaMobileMccMnc() {
        ...
        if (TextUtils.isEmpty(regplmn) || (TextUtils.isEmpty(charSequence) ^ 1) == 0) {
            return false;
        }
        ...
        if (this.mGsmPhone.mIccRecords.get() != null) {
            ...
            if (cpm.acquireFromCust(hplmn, getSS(), custSpn)) {
                ...
                if (4 == cpm.rule) {
                    ...
                    String temPnn = getEonsWithoutCphs();
                ...
                if (5 == cpm.rule) {
                    OnsDisplayParams odprule5 = getGsmOnsDisplayParamsSpnPrior(showSpn, showPlmn, rule, plmn, spn);
                    showSpn = odprule5.mShowSpn;
                    showPlmn = odprule5.mShowPlmn;
                    rule = odprule5.mRule;
                    plmn = odprule5.mPlmn;
                    spn = odprule5.mSpn;
                }
    ...
    private OnsDisplayParams getGsmOnsDisplayParamsBySpecialCust(boolean showSpn, boolean showPlmn, int rule, String plmn, String spn) {
        ...
        if (r != null) {
            ...
            if ("21405".equals(hplmn) && "21407".equals(regPlmn) && "tuenti".equalsIgnoreCase(spnSim)) {
                ...
                z = (getCombinedRegState(getSS()) != 0 || (TextUtils.isEmpty(spnRslt) ^ 1) == 0) ? false : (rule & 1) == 1;
                Rlog.d(LOG_TAG, "getGsmOnsDisplayParamsBySpecialCust: spnRslt = " + spnRslt + ", showSpnRslt = " + z);
            } else {
                matched = false;
            }
        } else {
    ...
    private OnsDisplayParams getGsmOnsDisplayParamsForChinaOperator(boolean showSpn, boolean showPlmn, int rule, String plmn, String spn) {
        ...
        if (!TextUtils.isEmpty(regplmn)) {
            if (!isChinaMobileMccMnc()) {
                ...
                if (HuaweiTelephonyConfigs.isChinaTelecom() || (getSS().getRoaming() && (TextUtils.isEmpty(charSequence) ^ 1) != 0 && "20404".equals(charSequence) && "20404".equals(regplmn) && 3 == (HwAllInOneController.getInstance().getSpecCardType(this.mGsmPhone.getPhoneId()) & 15))) {
        ...
        switch (msg.what) {
            ...
            case EVENT_4R_MIMO_ENABLE /*106*/:
                if (MIMO_4R_REPORT) {
                    ...
                    ar = (AsyncResult) msg.obj;
                    if (ar.exception != null) {
                        Rlog.e(LOG_TAG, "EVENT_MIMO_ENABLE exception " + ar.exception);
                        return;
                    } else {
                        on4RMimoChange(ar);
                        return;
    ...
    private OnsDisplayParams getOnsDisplayParamsBySpnOnly(boolean showSpn, boolean showPlmn, int rule, String plmn, String spn) {
        ...
        if (!TextUtils.isEmpty(spnSim) && isMccForSpn(r.getOperatorNumeric()) && (TextUtils.isEmpty(hPlmn) ^ 1) != 0 && hPlmn.length() > 3) {
    ...
    public int updateHSPAStatus(int type, GsmCdmaPhone phone) {
        if (KEEP_3GPLUS_HPLUS) {
            ...
            if (this.lastLac == lac && this.lastCid == cid && this.lastType == 15) {
                if (!(3 == type || 9 == type || 10 == type)) {
                    if (11 == type) {
                    }
                }
                type = this.lastType;
    ...
    private OnsDisplayParams getGsmOnsDisplayParamsSpnPrior(boolean showSpn, boolean showPlmn, int rule, String plmn, String spn) {
        ...
        String temPnn = null;
        int Rule = 1;
        String serviceProviderName = this.mGsmPhone.mIccRecords.get() != null ? ((IccRecords) this.mGsmPhone.mIccRecords.get()).getServiceProviderName() : null;
        if (TextUtils.isEmpty(serviceProviderName)) {
            ...
            Rule = 1;
            spnRes = serviceProviderName;
        }
        showSpnRes = (Rule & 1) == 1;
        showPlmnRes = (Rule & 2) == 2;
        Rlog.d(LOG_TAG, "getGsmOnsDisplayParamsSpnPrior: cardspn= " + serviceProviderName + " temPnn= " + temPnn + " Rule= " + Rule);
        return new OnsDisplayParams(showSpnRes, showPlmnRes, Rule, plmnRes, spnRes);
    }
}

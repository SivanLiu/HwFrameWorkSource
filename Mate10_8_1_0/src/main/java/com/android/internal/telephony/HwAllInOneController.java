...
public class HwAllInOneController extends Handler {
    ...
    private static final int TL_SLAVE_NETWORK_MODE_CDMA_HISI = SystemProperties.getInt("ro.config.tl_slave_network_mode_cdma_hisi", 4);
    private static final int TL_SLAVE_NETWORK_MODE_GSM_HISI = SystemProperties.getInt("ro.config.tl_slave_network_mode_gsm_hisi", 3);
            ...
            while (i < SIM_NUM) {
                ...
                if (phone == null) {
                    ...
                    if (getUserSwitchDualCardSlots() == i) {
                        ...
                        if (isCDMASimCard(i)) {
                            networkMode = TL_SLAVE_NETWORK_MODE_CDMA_HISI;
                        } else {
                            networkMode = TL_SLAVE_NETWORK_MODE_GSM_HISI;
                        }
                        ...
                        networkMode = isCDMASimCard(i) ? TL_SLAVE_NETWORK_MODE_CDMA_HISI : TL_SLAVE_NETWORK_MODE_GSM_HISI;
                    }
    ...
    public void disposeCardStatusWhenAllTrayOut() {
        boolean isCardTrayOut = IS_SINGLE_CARD_TRAY ? HwCardTrayUtil.isCardTrayOut(0) : false;
        boolean isCardTrayOut2 = (IS_SINGLE_CARD_TRAY || !HwCardTrayUtil.isCardTrayOut(0)) ? false : HwCardTrayUtil.isCardTrayOut(1);
    ...
    private void restartRildForNvcfg() {
        logd("restartRildForNvcfg  mNvRestartRildDone: " + this.mNvRestartRildDone + " mAutoSwitchDualCardsSlotDone: " + this.mAutoSwitchDualCardsSlotDone);
        if (!this.mNvRestartRildDone && this.mAutoSwitchDualCardsSlotDone && this.mSetSdcsCompleteMsg == null && "1".equals(SystemProperties.get("gsm.nvcfg.resetrild", "0"))) {
            ...
            logd("restartRildForNvcfg needSetDataAllowCount: " + this.needSetDataAllowCount);
    ...
    private void switchDualCardsSlotIfNeeded() {
        boolean isBalongSimSynced = SystemProperties.getBoolean("persist.sys.dualcards", false) ? this.mAutoSwitchDualCardsSlotDone ? isBalongSimSynced() : false : true;
    ...
    private int autoSwitchToSlot() {
        ...
        if (getUserSwitchDualCardSlots() == 0) {
            if (!((this.mSwitchTypes[0] == 1 && this.mSwitchTypes[1] == 3) || ((this.mSwitchTypes[0] == 1 && this.mSwitchTypes[1] == 2) || ((this.mSwitchTypes[0] == 1 && this.mSwitchTypes[1] == 0) || (this.mSwitchTypes[0] == 0 && this.mSwitchTypes[1] == 3))))) {
                if (this.mSwitchTypes[0] == 0 && this.mSwitchTypes[1] == 2) {
                }
            }
            slot = 1;
        } else {
            if (!((this.mSwitchTypes[0] == 0 && this.mSwitchTypes[1] == 1) || ((this.mSwitchTypes[0] == 3 && this.mSwitchTypes[1] == 1) || ((this.mSwitchTypes[0] == 3 && this.mSwitchTypes[1] == 0) || (this.mSwitchTypes[0] == 2 && this.mSwitchTypes[1] == 1))))) {
                if (this.mSwitchTypes[0] == 2 && this.mSwitchTypes[1] == 0) {
                }
            }
            slot = 0;
        }
    ...
    private void processSubInfoRecordUpdated(int detectedType) {
        if (4 != detectedType) {
            ...
            boolean z = (IS_FULL_NETWORK_SUPPORTED || IS_CMCC_CU_DSDX_ENABLE) ? true : IS_CMCC_4G_DSDX_ENABLE;
            if (z) {
                if (this.isSet4GSlotInProgress) {
    ...
    public boolean isCTHybird() {
        ...
        return hasCTCard ? hasOther : false;
    }
    ...
    public boolean judgeDefault4GSlotForCT(boolean forceSwitch) {
        ...
        logd("judgeDefault4GSlotForCT isAnySimCardChanged = " + z);
    ...
    public boolean judgeDefaultSlotId4Hisi() {
        ...
        int i = isCard1Present ? isCard2Present ^ 1 : 0;
        boolean z = !isCard1Present ? isCard2Present : false;
        if (IS_FAST_SWITCH_SIMSLOT) {
            if (i != 0) {
                temSub = 0;
            } else if (z) {
    ...
    public boolean judgeDefaultSlotId4HisiCmcc(boolean forceSwitch) {
        ...
        boolean isCard1Present = mUiccCards[0].getCardState() == CardState.CARDSTATE_PRESENT;
        boolean isCard2Present = mUiccCards[1].getCardState() == CardState.CARDSTATE_PRESENT;
        boolean z = (anySimCardChanged() || this.isPreBootCompleted || forceSwitch) ? true : this.isHotPlugCompleted;
        if (this.isPreBootCompleted) {
    ...
    private void setMaxRadioCapability(int ddsSubId) {
        ...
        if (SubscriptionManager.isValidSubscriptionId(ddsSubId) && phoneArr != null) {
            ...
            for (int phoneId = 0; phoneId < phoneArr.length; phoneId++) {
                int raf;
    ...
    private void recordPrimaryAndSecondaryStackNetworkType(int ability, int lteOnMappingMode) {
        ...
        int isNetworkRoaming = (IS_CMCC_4GSWITCH_DISABLE && isCmccHybirdBySubCarrierType()) ? TelephonyManager.getDefault().isNetworkRoaming(getCMCCCardSlotId()) ^ 1 : 0;
    ...
    private void revertDefaultDataSubId() {
        int mainSlot = getUserSwitchDualCardSlots();
    ...
    private boolean isSwitchSlotEnabledForCMCC() {
        boolean isCmccHybirdBySubCarrierType = !isCMCCHybird() ? isCmccHybirdBySubCarrierType() : true;
        boolean isCmccSlotIdRoaming = TelephonyManager.getDefault().isNetworkRoaming(getCMCCCardSlotId());
        if (IS_CMCC_4GSWITCH_DISABLE && isCmccHybirdBySubCarrierType && (isCmccSlotIdRoaming ^ 1) != 0) {
            return false;
        }
        return true;
    }

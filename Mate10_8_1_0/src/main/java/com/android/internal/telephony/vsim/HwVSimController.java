...
public class HwVSimController extends StateMachine {
    ...
    private VSimRcReadyState mRcReadyState = new VSimRcReadyState();
    private VSimRcWorkState mRcWorkState = new VSimRcWorkState();
    private VSimReconnectState mReconnectState = new VSimReconnectState();
    ...
    private boolean needSwitchModeHotplug(int[] cardTypes) {
        ...
        boolean vSimULOnlyMode = (!hasIccCardOnM2(cardTypes) || (getULOnlyProp() ^ 1) == 0) ? false : getVSimULOnlyMode();
        if (vSimULOnlyMode) {
    ...
    private void processNoNeedSwitchModeForCmcc(int[] cardTypes) {
        if (HwVSimUtilsInner.IS_CMCC_4GSWITCH_DISABLE) {
            ...
            if (insertCardCount > 0 && (isDisableProcess() ^ 1) != 0) {
                int slotInModem1 = HwVSimUtilsInner.getAnotherSlotId(getMainSlot(this.mSimSlotsTable));
                logd("processNoNeedSwitchModeForCmcc: slotInModem1 = " + slotInModem1);
                this.mModemAdapter.setPreferredNetworkType(null, null, slotInModem1, HwVSimPhoneFactory.getVSimSavedNetworkMode(0));
            }
        }
    }

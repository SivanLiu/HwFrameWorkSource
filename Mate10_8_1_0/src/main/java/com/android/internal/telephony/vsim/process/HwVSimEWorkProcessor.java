...
public class HwVSimEWorkProcessor extends HwVSimWorkProcessor {
    ...
    protected void onSetPreferredNetworkTypeDone(Message msg) {
        ...
        if (isAsyncResultValid(ar)) {
            ...
            int slotIdInModem1 = slotIdInModem2 == 0 ? 1 : 0;
            if (!HwVSimModemAdapter.IS_FAST_SWITCH_SIMSLOT) {
                radioPowerOnForVSim();
            } else if (flag == 0) {
                this.mModemAdapter.setPreferredNetworkType(this, request, slotIdInModem1, getNetworkTypeOnModem1(0));
                ...
                if (HwVSimUtilsInner.isDualImsSupported()) {
                    this.mModemAdapter.saveNetworkTypeToDB(slotIdInModem1, getNetworkTypeOnModem1(0));
                }
            } else {
                radioPowerOnForVSim();
                if (HwVSimUtilsInner.isDualImsSupported()) {

...
public class GsmCdmaConnection extends AbstractGsmCdmaConnection {
    ...
    public boolean update(DriverCall dc) {
        ...
        } else if (isIncoming() && !equalsHandlesNulls(this.mAddress, dc.number) && (!(this.mNumberConverted && (equalsHandlesNulls(this.mConvertedNumber, dc.number) ^ 1) == 0) && (showConnectedNum || this.mIsIncoming))) {
    ...
    private static int findNextPCharOrNonPOrNonWCharIndex(String phoneNumber, int currIndex) {
        ...
        if (index >= length || index <= currIndex + 1 || r3 || !isPause(phoneNumber.charAt(currIndex))) {
            return index;
        }
        return currIndex + 1;
    }

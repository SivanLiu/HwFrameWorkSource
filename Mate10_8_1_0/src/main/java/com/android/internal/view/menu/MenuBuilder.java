...
public class MenuBuilder implements Menu {
    ...
    void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (((Menu.SUPPORTED_MODIFIERS_MASK & modifierState) == (Menu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != '\u0000') {
                    if (!(shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2])) {
                        if (qwerty && shortcutChar == '\b' && keyCode == 67) {
                        }
                    }
                    if (item.isEnabled()) {
                        items.add(item);
    ...
    public boolean performItemAction(MenuItem item, MenuPresenter preferredPresenter, int flags) {
        ...
        boolean hasSubMenu = provider != null ? provider.hasSubMenu() : false;
        if (itemImpl.hasCollapsibleActionView()) {
            invoked |= itemImpl.expandActionView();
            if (invoked) {
                close(true);
            }
        } else if (itemImpl.hasSubMenu() || hasSubMenu) {

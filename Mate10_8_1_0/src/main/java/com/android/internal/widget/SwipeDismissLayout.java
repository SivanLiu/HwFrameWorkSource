...
public class SwipeDismissLayout extends FrameLayout {
        ...
        try {
            this.mScreenOffReceiver = new BroadcastReceiver() {
                public void onReceive(Context context, Intent intent) {
                    SwipeDismissLayout.this.post(new -$Lambda$hZenqyGYSNt5KiruOSsv736MIDs((byte) 2, this));
                }
    ...
    private void updateDismiss(MotionEvent ev) {
        ...
        if (!this.mDismissed) {
            if (deltaX <= ((float) getWidth()) * Math.max(Math.min(((-0.23000002f * xVelocity) / ((float) this.mMinFlingVelocity)) + MAX_DIST_THRESHOLD, MAX_DIST_THRESHOLD), MIN_DIST_THRESHOLD) || ev.getRawX() < this.mLastX) {
                if (xVelocity >= ((float) this.mMinFlingVelocity)) {
                }
            }

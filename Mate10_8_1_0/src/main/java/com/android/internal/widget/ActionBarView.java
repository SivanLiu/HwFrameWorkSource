...
public class ActionBarView extends AbsActionBarView implements DecorToolbar {
    ...
    public void setDisplayOptions(int options) {
        ...
        if ((flagsChanged & 63) != 0) {
            ...
            int homeVis = ((showHome || z) && this.mExpandedActionView == null) ? 0 : 8;
        ...
        if (MeasureSpec.getMode(widthMeasureSpec) != 1073741824) {
            ...
            if (homeLayout.getVisibility() == 8 || homeLayout.getParent() != this.mUpGoerFive) {
                ...
                if (customView != null) {
                    ...
                    layoutParams = lp instanceof ActionBar.LayoutParams ? (ActionBar.LayoutParams) lp : null;
                    horizontalMargin = 0;
                    verticalMargin = 0;
                    ...
                    customNavHeightMode = this.mContentHeight > 0 ? Integer.MIN_VALUE : lp.height == -2 ? 1073741824 : Integer.MIN_VALUE;
                    if (lp.height >= 0) {
                        height = Math.min(lp.height, height);
                    }
                    customNavHeight = Math.max(0, height - verticalMargin);
                    customNavWidthMode = lp.width == -2 ? 1073741824 : Integer.MIN_VALUE;
                    if (lp.width < 0) {
                        min = Math.min(lp.width, availableWidth);
                    } else {
                        min = availableWidth;
                    }
                    customNavWidth = Math.max(0, min - horizontalMargin);
                    if (((layoutParams == null ? layoutParams.gravity : DEFAULT_CUSTOM_GRAVITY) & 7) == 1 && lp.width == -1) {
            ...
            customView = null;
            customView = this.mExpandedActionView != null ? this.mCustomNavView : this.mExpandedActionView;
            if (customView != null) {
                lp = generateLayoutParams(customView.getLayoutParams());
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (contentHeight > 0) {
            ...
            if (customView != null) {
                ...
                ActionBar.LayoutParams layoutParams = lp instanceof ActionBar.LayoutParams ? (ActionBar.LayoutParams) lp : null;
                int gravity = layoutParams != null ? layoutParams.gravity : DEFAULT_CUSTOM_GRAVITY;
                int navWidth = customView.getMeasuredWidth();
                int topMargin = 0;
                int bottomMargin = 0;
                if (layoutParams != null) {
                    x2 = AbsActionBarView.next(x2, layoutParams.getMarginStart(), isLayoutRtl);
                    menuStart += layoutParams.getMarginEnd() * direction;

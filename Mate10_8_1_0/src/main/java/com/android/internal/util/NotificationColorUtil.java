...
public class NotificationColorUtil {
    ...
    private static int ensureTextContrast(int color, int bg, boolean isBgDarker) {
        if (isBgDarker) {
            return findContrastColorAgainstDark(color, bg, true, 4.5d);
        }
    ...
    public static int resolveContrastColor(Context context, int notificationColor, int backgroundColor, boolean isDark) {
        ...
        int i = resolvedColor;
        return ensureTextContrast(ensureLargeTextContrast(resolvedColor, context.getColor(R.color.notification_action_list), isDark), backgroundColor, isDark);
    }

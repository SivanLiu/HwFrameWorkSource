...
public final class RealConnection extends Listener implements Connection {
    ...
    private void connectTls(int readTimeout, int writeTimeout, ConnectionSpecSelector connectionSpecSelector) throws IOException {
        ...
        Socket socket = null;
        try {
            socket = (SSLSocket) address.sslSocketFactory().createSocket(this.rawSocket, address.url().host(), address.url().port(), true);
            ...
            if (connectionSpec.supportsTlsExtensions()) {
                Platform.get().configureTlsExtensions(socket, headerHost, address.protocols());
            }
            socket.startHandshake();
            Handshake unverifiedHandshake = Handshake.get(socket.getSession());
            if (address.hostnameVerifier().verify(headerHost, socket.getSession())) {

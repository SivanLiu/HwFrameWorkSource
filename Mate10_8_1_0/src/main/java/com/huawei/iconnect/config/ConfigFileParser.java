...
public class ConfigFileParser {
    ...
    private static final String CFG_NODE_FASTINQUIRYPARTY = "fastinquiryparty";
            ...
            try {
                ...
                while (reader.hasNext()) {
                    ...
                    if (name.equals(CFG_NODE_HEADER)) {
                        fileStruct.setHeader(readHeader(reader));
                    } else if (name.equals(CFG_NODE_GUIDE_BODY)) {
                        fileStruct.setGuideBodyItems(readConfigArray(reader));
                    } else if (name.equals(CFG_NODE_BT_BODY)) {
                        readBtBody(fileStruct, reader);
                    } else {
                        reader.skipValue();
                    }
                }
    ...
    private static void readBtBody(ConfigFileStruct fileStruct, JsonReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            ...
            if (name.equals(CFG_NODE_HUAWEIPARTY)) {
                ...
                fileStruct.setBtBodyItems(configs);
            } else if (name.equals(CFG_NODE_THIRDPARTY)) {
                readBtBodyConfigArray(reader, configs, "Bt");
                fileStruct.setBtBodyItems(configs);
            } else if (name.equals(CFG_NODE_FASTINQUIRYPARTY)) {
                readBtBodyConfigArray(reader, configs, "Bt_hw");
                fileStruct.setFastInquiryItems(configs);
            } else {
                reader.skipValue();
            }
        }
    ...
    public static ConfigFileStruct parseConfigFile(Context context, String path, boolean isAsset) {
        ...
        if (isAsset) {
            ...
            try {
                inputStream = iConnectContext.getResources().getAssets().open(path);
        ...
        ConfigFileStruct cfgStruct = readJsonStream(inputStream);
        if (inputStream != null) {
            try {
                inputStream.close();

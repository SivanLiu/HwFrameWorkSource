    ...
    public static Info getDetectInfoImpl(Context context, BluetoothDeviceData deviceData, boolean forceLoadFile) {
        ...
        if (context == null || deviceData == null) {
            ...
            if (configFile == null) {
                ...
                return null;
            } else if (configFile.getFastInquiryItems() == null) {
                HwLog.w(TAG, "configFile.getFastInquiryItems() == null");
                return null;
            } else {
                Info info;
                for (BtBodyConfigItem c : configFile.getFastInquiryItems()) {
                    c.outputString();
                    info = c.getInfo(deviceData);
                    if (info != null) {
                        HwLog.d(TAG, "getFastInquiryItems getDetectInfo:" + info);
                        return info;
                    }
                }
                if (configFile.getBtBodyItems() == null) {
                    HwLog.w(TAG, "configFile.getBtBodyItems() == null");
                    return null;
                }
                for (BtBodyConfigItem c2 : configFile.getBtBodyItems()) {
                    c2.outputString();
                    info = c2.getInfo(deviceData);
                    if (info != null) {

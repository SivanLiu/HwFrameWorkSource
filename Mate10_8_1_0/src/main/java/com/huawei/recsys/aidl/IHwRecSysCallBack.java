...
public interface IHwRecSysCallBack extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IHwRecSysCallBack {
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                case 1:
                    HwObjectContainer hwObjectContainer;
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() != 0) {
                        hwObjectContainer = (HwObjectContainer) HwObjectContainer.CREATOR.createFromParcel(data);
                    } else {
                        hwObjectContainer = null;
                    }
                    onRecResult(hwObjectContainer);

...
public class IntelligentAction extends Action {
    ...
    private String mActionType = EventIdConstant.PURPOSE_STR_BLANK;
    private Handler mHandler = null;
    private String mLastChangeUri = null;
    private long mLastchangeTimeStamp = System.currentTimeMillis();
    private ContentObserver mObserver = null;

    private static class IntelligentHandler extends Handler {
        private final WeakReference<IntelligentAction> service;

        IntelligentHandler(IntelligentAction service) {
                ...
                if (action != null) {
                    ...
                    if (uriString != null) {
                        if (System.currentTimeMillis() - action.mLastchangeTimeStamp >= IntelligentAction.TIME_THRESHOLD || !uriString.equals(action.mLastChangeUri)) {
                            action.mLastchangeTimeStamp = System.currentTimeMillis();
                            action.mLastChangeUri = uriString;
                            action.perform();
                        }
                    }
                }
    ...
    private static boolean isLegalUri(String str) {
    ...
    private boolean collectIntelligentData(Uri uri) {
        ...
        Cursor cursor3 = null;
        try {
            cursor3 = this.mContext.getContentResolver().query(uri, new String[]{TYPE}, null, null, null);
            if (isNullOrEmptyCursor(cursor3)) {
                if (cursor3 != null) {
                    cursor3.close();
                }
                return false;
            ...
            if (cursor3.moveToFirst()) {
                String type = cursor3.getString(cursor3.getColumnIndex(TYPE));
                ...
                if (TYPE_HOTEL.equalsIgnoreCase(this.mActionType) && TYPE_HOTEL.equalsIgnoreCase(type)) {
                    ...
                    cursor2 = this.mContext.getContentResolver().query(uri, new String[]{HOTEL_NAME, HOTEL_ADDRESS, HOTEL_CHECK_IN_TIME}, null, null, null);
                    if (isNullOrEmptyCursor(cursor2)) {
                        if (cursor3 != null) {
                            cursor3.close();
                        }
                        if (cursor2 != null) {
                        ...
                        String hotel_name = cursor2.getString(cursor2.getColumnIndex(HOTEL_NAME));
                        String hotel_address = cursor2.getString(cursor2.getColumnIndex(HOTEL_ADDRESS));
                        Date check_in_time = new Date(cursor2.getLong(cursor2.getColumnIndex(HOTEL_CHECK_IN_TIME)));
                    ...
                    cursor = this.mContext.getContentResolver().query(uri, new String[]{TRIP_NO, TRIP_SEAT, TRIP_START_TIME, TRIP_ARRIVAL_TIME, TRIP_START_PLACE, TRIP_ARRIVAL_PLACE}, null, null, null);
                    if (isNullOrEmptyCursor(cursor)) {
                        ...
                        String trip_event_number = cursor.getString(cursor.getColumnIndex(TRIP_NO));
                        String trip_seat = cursor.getString(cursor.getColumnIndex(TRIP_SEAT));
                        long trip_begin_time = cursor.getLong(cursor.getColumnIndex(TRIP_START_TIME));
                        long trip_end_time = cursor.getLong(cursor.getColumnIndex(TRIP_ARRIVAL_TIME));
                        String trip_begin_place = cursor.getString(cursor.getColumnIndex(TRIP_START_PLACE));
                        String trip_end_place = cursor.getString(cursor.getColumnIndex(TRIP_ARRIVAL_PLACE));

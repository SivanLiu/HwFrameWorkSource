...
public class Action {
    ...
    private void parseFromjson(String jsonData) throws JSONException {
        if (jsonData != null && !jsonData.trim().equals(EventIdConstant.PURPOSE_STR_BLANK)) {
            List<String> list;
    ...
    public boolean isEnable() {
        return !this.mTimeDisable ? this.mEnable : false;
    }
    ...
    public boolean checkEventTriggers(String eventname) {
        ...
        if (this.eventtriger_list == null || (isEnable() ^ 1) != 0) {
            return false;
        }
    ...
    public boolean checkActionTriggers(String actionname) {
        if (this.actiontriger_list == null || (isEnable() ^ 1) != 0) {
            return false;
        }

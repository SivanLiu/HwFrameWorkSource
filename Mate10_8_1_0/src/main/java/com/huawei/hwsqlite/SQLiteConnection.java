...
public final class SQLiteConnection implements OnCancelListener {
    ...
    private Set<String> attachedAlias = new HashSet();
    ...
    private boolean mStepQueryStatementBindArgs = false;
    ...
    private void attachAlias() {
        SQLiteAttached sQLiteAttached = null;
        try {
            int length = this.mConfiguration.attachedAlias.size();
            for (int i = 0; i < length; i++) {
                SQLiteAttached attached = (SQLiteAttached) this.mConfiguration.attachedAlias.get(i);
                if (!this.attachedAlias.contains(attached.alias)) {
                    sQLiteAttached = attached;
            ...
            if (sQLiteAttached != null) {
                Log.w(TAG, "Failed to attach '" + sQLiteAttached.path + "' as '" + sQLiteAttached.alias + "'");
            }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        applyBlockGuardPolicy(statement);
                        attachCancellationSignal(cancellationSignal);
                        long result = nativeExecuteForCursorWindow(this.mConnectionPtr, statement.mStatementPtr, window, startPos, requiredPos, countAllRows);
    ...
    int executeForStepQuery(String sql, Object[] bindArgs, CancellationSignal cancellationSignal) {
        if (sql == null) {
            throw new IllegalArgumentException("sql must not be null.");
        } else if (-assertionsDisabled || this.mStepQueryStatement != null) {
    ...
    public void changeEncryptKey(SQLiteEncryptKeyLoader newLoader) {
        if (!-assertionsDisabled && this.mConnectionPtr == 0) {
            throw new AssertionError();
    ...
    private void bindArguments(PreparedStatement statement, Object[] bindArgs) {
        ...
        if (count != statement.mNumParameters) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                switch (SQLiteDatabaseUtils.getTypeOfObject(arg)) {
                    case 0:

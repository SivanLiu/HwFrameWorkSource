...
public interface IIntelliListener extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IIntelliListener {
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                case 1:
                    IntelliAlgoResult intelliAlgoResult;
                    data.enforceInterface(DESCRIPTOR);
                    if (data.readInt() == 0) {
                        intelliAlgoResult = null;
                    } else {
                        intelliAlgoResult = (IntelliAlgoResult) IntelliAlgoResult.CREATOR.createFromParcel(data);
                    }
                    onEvent(intelliAlgoResult);

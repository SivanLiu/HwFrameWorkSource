...
public class DevicePhoneManager {
    ...
    private boolean isAddNumbersOutOfLimit(int size, boolean isOutgoing) {
        ...
        return (numbers == null || numbers.isEmpty() || numbers.size() + size <= 200) ? false : true;
    }
    ...
    private boolean updateListTypePolicy(ComponentName admin, boolean isOutgoing) {
        ...
        if (listBundle != null) {
            ArrayList<String> numbers = listBundle.getStringArrayList("value");
            if (!(numbers == null || (numbers.isEmpty() ^ 1) == 0)) {
                return true;
            }
        }
        if (this.mDpm.removePolicy(admin, isOutgoing ? PHONE_NUMBER_OUTGOING_LIST_TYPE : PHONE_NUMBER_INCOMING_LIST_TYPE, null)) {
            z = this.mDpm.removePolicy(admin, listPolicy, null);
        }
        return z;
    }
    ...
    private boolean setListTypePolicy(ComponentName admin, boolean isBlackList, boolean isOutgoing) {
        ...
        if (!setListBefore(isOutgoing) || (isListTypeValid(isBlackList, policyName) ^ 1) == 0) {
            Bundle bundle = new Bundle();
    ...
    private boolean setListBefore(boolean isOutgoing) {
        ...
        if (listBundle != null) {
            ArrayList<String> numbers = listBundle.getStringArrayList("value");
            return (numbers == null || (numbers.isEmpty() ^ 1) == 0) ? false : true;
        }
    }
    ...
    private void updataNumbersForPrefixMode(ArrayList<String> numbers) {
        if (numbers != null && !numbers.isEmpty()) {
            ...
            for (int i = 0; i < numSize; i++) {
                numbers.set(i, ((String) numbers.get(i)) + PhoneConstantsEx.APN_TYPE_ALL);
            }
        }
    }
    ...
    private boolean isValidPhoneNumbers(ArrayList<String> numbers) {
        ...
        while (it.hasNext()) {
            ...
            if (num != null) {
                if (num.endsWith(PhoneConstantsEx.APN_TYPE_ALL)) {
                }
            }
            return false;
        }
    ...
    private boolean removeAllNumbers(ComponentName admin, boolean isOutgoing) {
        String listTypePolicy = isOutgoing ? PHONE_NUMBER_OUTGOING_LIST_TYPE : PHONE_NUMBER_INCOMING_LIST_TYPE;
        String applyToAllPolicy = isOutgoing ? APPLY_TO_ALL_OUTGOING_CALLS : APPLY_TO_ALL_INCOMING_CALLS;
        String listPolicy = isOutgoing ? PHONE_NUMBER_OUTGOING_LIST : PHONE_NUMBER_INCOMING_LIST;
        if (this.mDpm.removePolicy(admin, listTypePolicy, null) && this.mDpm.removePolicy(admin, applyToAllPolicy, null)) {
            return this.mDpm.removePolicy(admin, listPolicy, null);
        }
        return false;
    }
    ...
    public boolean removePhoneCallLimitation(ComponentName admin, boolean isOutgoing, int dateType) {
        ...
        switch (dateType) {
            case 0:
                if (!isOutgoing) {
                    policyName = PHONE_CALLS_LIMITATION_INCOMING_DAY;
                    break;
                }
                policyName = PHONE_CALLS_LIMITATION_OUTGOING_DAY;
                break;
            case 1:
                if (!isOutgoing) {
                    policyName = PHONE_CALLS_LIMITATION_INCOMING_WEEK;
                    break;
                }
                policyName = PHONE_CALLS_LIMITATION_OUTGOING_WEEK;
                break;
            case 2:
                if (!isOutgoing) {
                    policyName = PHONE_CALLS_LIMITATION_INCOMING_MONTH;
                    break;
                }
                policyName = PHONE_CALLS_LIMITATION_OUTGOING_MONTH;
                break;
            default:

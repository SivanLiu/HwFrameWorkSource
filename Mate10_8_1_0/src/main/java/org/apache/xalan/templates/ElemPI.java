...
public class ElemPI extends ElemTemplateElement {
    ...
    public void execute(TransformerImpl transformer) throws TransformerException {
        ...
        String evaluate = this.m_name_atv == null ? null : this.m_name_atv.evaluate(xctxt, xctxt.getCurrentNode(), this);
        if (evaluate != null) {
            if (evaluate.equalsIgnoreCase("xml")) {
                transformer.getMsgMgr().warn(this, XSLTErrorResources.WG_PROCESSINGINSTRUCTION_NAME_CANT_BE_XML, new Object[]{"name", evaluate});
            } else if (this.m_name_atv.isSimple() || (XML11Char.isXML11ValidNCName(evaluate) ^ 1) == 0) {
                try {
                    transformer.getResultTreeHandler().processingInstruction(evaluate, transformer.transformToString(this));
                } catch (SAXException se) {
                    throw new TransformerException(se);
                }
            } else {
                transformer.getMsgMgr().warn(this, XSLTErrorResources.WG_PROCESSINGINSTRUCTION_NOTVALID_NCNAME, new Object[]{"name", evaluate});
            }
        }
    }

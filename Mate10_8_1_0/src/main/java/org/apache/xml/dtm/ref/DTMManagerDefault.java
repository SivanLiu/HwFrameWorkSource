...
public class DTMManagerDefault extends DTMManager {
    ...
    public synchronized DTM getDTM(Source source, boolean unique, DTMWSFilter whiteSpaceFilter, boolean incremental, boolean doIndexing) {
        ...
        if (source == null || !(source instanceof DOMSource)) {
            boolean z = source != null ? source instanceof SAXSource : true;
            boolean z2 = source != null ? source instanceof StreamSource : false;
            if (z || z2) {
                ...
                if (source == null) {
                    ...
                    String urlOfSource = inputSource.getSystemId();
                    if (urlOfSource != null) {
                        try {
                        ...
                        try {
                            inputSource.setSystemId(urlOfSource);
                ...
                addDTM(dtm, dtmPos, 0);
                if (xMLReader != null) {
                    equals = xMLReader.getClass().getName().equals("org.apache.xerces.parsers.SAXParser");
                } else {
                    equals = false;
                }
                if (equals) {
                ...
                if (this.m_incremental && incremental) {
                    ...
                    if (incrementalSAXSource == null) {
                        if (xMLReader == null) {
                            ...
                            Object coParser = filter;
                        }
                    }
                    dtm.setIncrementalSAXSource(incrementalSAXSource);
                    if (inputSource == null) {
                        if (xMLReader != null) {
                            if (((this.m_incremental ? incremental : 0) ^ 1) != 0) {
                                xMLReader.setContentHandler(this.m_defaultHandler);
                    ...
                    if (xMLReader.getErrorHandler() == null) {
                        xMLReader.setErrorHandler(dtm);
                    }
                    xMLReader.setDTDHandler(dtm);
                    ...
                    xMLReader.setContentHandler(dtm);
                    xMLReader.setDTDHandler(dtm);
                    if (xMLReader.getErrorHandler() == null) {
                        xMLReader.setErrorHandler(dtm);
                    }
                    try {
                        xMLReader.setProperty("http://xml.org/sax/properties/lexical-handler", dtm);
    ...
    public synchronized XMLReader getXMLReader(Source inputSource) {
        XMLReader xMLReader;
        try {
            xMLReader = inputSource instanceof SAXSource ? ((SAXSource) inputSource).getXMLReader() : null;
            if (xMLReader == null) {

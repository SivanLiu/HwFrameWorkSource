...
public final class ClientSession extends ObexSession {
    ...
    public HeaderSet connect(HeaderSet header) throws IOException {
        ...
        if (header != null) {
            ...
            bArr = ObexHelper.createHeader(header, false);
            totalLength = bArr.length + 4;
        }
        ...
        if (bArr != null) {
            System.arraycopy(bArr, 0, requestPacket, 4, bArr.length);
        }
    ...
    public HeaderSet disconnect(HeaderSet header) throws IOException {
        if (this.mObexConnected) {
            ...
            HeaderSet returnHeaderSet = new HeaderSet();
            sendRequest(ObexHelper.OBEX_OPCODE_DISCONNECT, bArr, returnHeaderSet, null, false);
            synchronized (this) {

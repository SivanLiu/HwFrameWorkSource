...
public interface IHwQoECallback extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IHwQoECallback {
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                case 1:
                    HwQoEQualityInfo hwQoEQualityInfo;
                    data.enforceInterface(DESCRIPTOR);
                    boolean _arg0 = data.readInt() != 0;
                    if (data.readInt() != 0) {
                        hwQoEQualityInfo = (HwQoEQualityInfo) HwQoEQualityInfo.CREATOR.createFromParcel(data);
                    } else {
                        hwQoEQualityInfo = null;
                    }
                    onNetworkEvaluate(_arg0, hwQoEQualityInfo);

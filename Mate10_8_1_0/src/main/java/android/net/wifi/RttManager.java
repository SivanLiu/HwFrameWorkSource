...
public class RttManager {
    ...
    public static class RttClient implements Parcelable {
        public static final Creator<RttClient> CREATOR = new Creator<RttClient>() {
    ...
    public RttManager(Context context, IRttManager service, Looper looper) {
        ...
        try {
            ...
            Messenger messenger = this.mService.getMessenger(new Binder(), key);
            if (messenger == null) {
            ...
            this.mAsyncChannel.sendMessage(CMD_OP_REG_BINDER, key[0]);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }

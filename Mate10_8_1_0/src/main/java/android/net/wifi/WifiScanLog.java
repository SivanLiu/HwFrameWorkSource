...
public class WifiScanLog {
    ...
    private static final Singleton<WifiScanLog> gDefault = new Singleton<WifiScanLog>() {
        protected WifiScanLog create() {
            return new WifiScanLog();
        }
    };
    private HashMap<String, Integer> eventLogDuration;
    private HashMap<String, Long> eventLogStartTime;
    private HashMap<String, StringBuilder> eventLogs;
    ...
    private WifiScanLog() {
        ...
        this.eventLogStartTime = new HashMap();
        this.eventLogDuration = new HashMap();
    }

    public static WifiScanLog getDefault() {
        return (WifiScanLog) gDefault.get();
    }
    ...
    private synchronized void addEvent(String scanKey, String eventKey, Object... params) {
        if (HWFLOW) {
            long startTime;
            StringBuilder eventLog = (StringBuilder) this.eventLogs.get(scanKey);
            int lastTempDiffTime = -1;
            if (eventLog == null) {
                eventLog = new StringBuilder();
                startTime = System.currentTimeMillis();
                Calendar.getInstance().setTimeInMillis(startTime);
                eventLog.append(String.format("%tY-%tm-%td %tH:%tM:%tS.%tL", new Object[]{c, c, c, c, c, c, c}));
                this.eventLogStartTime.put(scanKey, Long.valueOf(startTime));
                this.eventLogDuration.put(scanKey, Integer.valueOf(0));
            } else {
                startTime = ((Long) this.eventLogStartTime.get(scanKey)).longValue();
                lastTempDiffTime = ((Integer) this.eventLogDuration.get(scanKey)).intValue();
            }
            int tempDiffTime = (int) (System.currentTimeMillis() - startTime);
            if (lastTempDiffTime != tempDiffTime) {
                eventLog.append("#").append(tempDiffTime);
                this.eventLogDuration.put(scanKey, Integer.valueOf(tempDiffTime));
            }
            eventLog.append("$").append(eventKey);
            if (params != null) {
                for (Object param : params) {
                    if (param == null) {
                        eventLog.append(',').append(param);
                    } else {
                        eventLog.append(',').append(param.toString().replaceAll("[$#,\n]", "*"));
                    }
                }
            }
    ...
    public synchronized void flush() {
        if (HWFLOW) {
            for (Entry<String, StringBuilder> entry : this.eventLogs.entrySet()) {
                Log.i(TAG, ((String) entry.getKey()) + ((StringBuilder) entry.getValue()).toString());
            }

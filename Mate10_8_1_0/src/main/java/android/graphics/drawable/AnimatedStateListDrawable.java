...
public class AnimatedStateListDrawable extends StateListDrawable {
    ...
    protected boolean onStateChange(int[] stateSet) {
        ...
        boolean selectDrawable = targetIndex != getCurrentIndex() ? !selectTransition(targetIndex) ? selectDrawable(targetIndex) : true : false;
        Drawable current = getCurrent();
        if (current != null) {
            return selectDrawable | current.setState(stateSet);
        }

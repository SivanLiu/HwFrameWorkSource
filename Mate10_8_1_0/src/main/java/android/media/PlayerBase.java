...
public abstract class PlayerBase {
    ...
    private static final int EROOR_GET_MODE = -1;
    ...
    private int getZenMode() {
        INotificationManager sService = INotificationManager.Stub.asInterface(ServiceManager.getService(Context.NOTIFICATION_SERVICE));
        if (sService != null) {
            try {
                return sService.getZenMode();
            } catch (RemoteException e) {
                Log.i(TAG, "getZenMode error");
            }
        }
        return -1;
    }
    ...
    boolean isRestricted_sync() {
        ...
        if (this.mHasAppOpsPlayAudio && pkgName != null && ((pkgName.equals("com.tencent.mobileqq") || pkgName.equals("com.tencent.mm")) && getZenMode() == 2)) {
            IAudioService audioService = getService();
            int usage = this.mAttributes.getUsage();
            if (usage == 2 && audioService != null) {
                try {
                    if (audioService.getMode() == 3) {
                        Log.i(TAG, "unmute USAGE_VOICE_COMMUNICATION of " + pkgName + " in ZEN_MODE_NO_INTERRUPTIONS and voip");
                        return false;
                    }
                } catch (RemoteException e) {
                    Log.e(TAG, "Cannot access getMode in isRestricted_sync()");
                }
            }
            Log.i(TAG, "mute " + usage + " of " + pkgName + " in ZEN_MODE_NO_INTERRUPTIONS");
            return true;
        } else if (this.mHasAppOpsPlayAudio || (this.mAttributes.getAllFlags() & 64) != 0) {
            return false;
        } else {

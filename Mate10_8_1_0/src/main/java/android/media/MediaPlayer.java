...
public class MediaPlayer extends PlayerBase implements Listener, VolumeAutomation {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 100:
                    ...
                    MediaPlayer.this.mOnCompletionInternalListener.onCompletion(this.mMediaPlayer);
                    onCompletionListener = MediaPlayer.this.mOnCompletionListener;
                    if (!(onCompletionListener == null || (r13 ^ 1) == 0)) {
                        onCompletionListener.onCompletion(this.mMediaPlayer);
                    }
                    MediaPlayer.this.stayAwake(false);
                    return;
                ...
                case 210:
                    ...
                    return;
                case 250:
                    if (msg.arg1 == 802 && MediaPlayer.MEDIA_REPORT_PROP && MediaPlayer.this.mMediaReportSwitch && MediaPlayer.this.mIsHLS) {
                        parcel = (Parcel) msg.obj;
                        if (parcel != null) {
                            ...
                            data3.putInt("videoProtocol", 0);
                            data3.putLong("videoRemainingPlayTime", videoRemainingPlayTime);
                            data3.putLong("segDuration", segDuration);
                            data3.putInt("segIndex", segIndex);
                            data3.putLong("aveCodeRate", aveCodeRate);
                            int res = HwFrameworkFactory.getHwCommBoosterServiceManager().reportBoosterPara(MediaPlayer.MEDIA_REPORT_PKG_NAME, 202, data3);
                            if (res != 0) {
    ...
    private class ProvisioningThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    ...
                    if (bArr != null) {
                        try {
                            ...
                            this.status = 2;
                            Log.w(MediaPlayer.TAG, "HandleProvisioninig: Thread run: provideProvisionResponse " + e2);
                        }
                    }
                    succeeded = false;
                ...
                this.status = 1;
                Log.w(MediaPlayer.TAG, "HandleProvisioninig: Thread run: openConnection " + e22);
            }
            if (bArr != null) {
                MediaPlayer.this.mDrmObj.provideProvisionResponse(bArr);
                Log.v(MediaPlayer.TAG, "HandleProvisioninig: Thread run: provideProvisionResponse SUCCEEDED!");
    ...
    private void setDataSource(String path, String[] keys, String[] values, List<HttpCookie> cookies) throws IOException, IllegalArgumentException, SecurityException, IllegalStateException {
        ...
        if (file.exists()) {
            if ((path.toLowerCase(Locale.getDefault()).endsWith(".sdp") ^ 1) != 0) {
    ...
    public void prepareDrm(UUID uuid) throws UnsupportedSchemeException, ResourceBusyException, ProvisioningNetworkErrorException, ProvisioningServerErrorException {
        ...
        synchronized (this.mDrmLock) {
            if (this.mDrmInfo == null) {
                ...
                OnDrmPreparedHandlerDelegate onDrmPreparedHandlerDelegate = this.mOnDrmPreparedHandlerDelegate;
                try {

...
public final class DisplayInfo implements Parcelable {
    ...
    private void getMetricsWithSize(DisplayMetrics outMetrics, CompatibilityInfo compatInfo, Configuration configuration, int width, int height) {
        ...
        if (IS_NOTCH_PROP) {
            ...
            int i2 = 0;
            boolean isAppNotchSupport = false;
            if (!(this.application == null || this.application.mLoadedApk == null)) {
                ...
                if (this.application.getApplicationInfo() != null) {
                    ...
                    if (this.application.getApplicationInfo().packageName.contains("com.tencent.game.rhythmmaster")) {
                        z = true;
                    } else {
                        z = this.application.getApplicationInfo().packageName.contains("com.vectorunit.redcmgeplaycn");
                    }
                    this.isQQgame = z;
                    this.mIsWhitelistApp = mWhitelistActivities.contains(this.application.getApplicationInfo().packageName);
                    i2 = isAppAlwaysUseNotch(this.application.getApplicationInfo().packageName);
                    isAppNotchSupport = isAppNotchSupport(this.application.getApplicationInfo().packageName);
                }
            }
            if (isAppNotchSupport || (i2 ^ 1) == 0 || (this.isLauncher ^ 1) == 0 || !(this.rotation == 1 || this.rotation == 3)) {
                if (i2 == 0 && this.mIsWhitelistApp && this.rotation == 0) {
    ...
    public boolean isAppAlwaysUseNotch(String packageName) {
        boolean z = true;
        try {
            ...
            if (wm.getAppUseNotchMode(packageName) != 1) {
                z = false;
            }
            return z;
        } catch (Exception e) {
    ...
    private boolean isAppNotchSupport(String packageName) {
        try {
            IHwWindowManager wm = HwWindowManager.getService();
            if (wm != null) {
                return wm.isAppNotchSupport(packageName);
            }
            return false;
        } catch (Exception e) {
            return false;

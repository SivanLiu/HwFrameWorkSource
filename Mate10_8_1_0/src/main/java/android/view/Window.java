...
*/
    ...
    public boolean isWideColorGamut() {
        if (getColorMode() == 1) {
            return getContext().getResources().getConfiguration().isScreenWideColorGamut();
        }
        return false;
    }
    ...
    public boolean shouldCloseOnTouch(Context context, MotionEvent event) {
        boolean isOutside = (event.getAction() == 0 && isOutOfBounds(context, event)) ? true : event.getAction() == 4;
        if (this.mCloseOnTouchOutside && peekDecorView() != null && isOutside) {
            return true;
        }
        return false;
    }

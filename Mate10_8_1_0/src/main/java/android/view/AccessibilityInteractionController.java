...
final class AccessibilityInteractionController {
    ...
    private class AccessibilityNodePrefetcher {
        ...
        public void prefetchAccessibilityNodeInfos(View view, int virtualViewId, int fetchFlags, List<AccessibilityNodeInfo> outInfos, Bundle arguments) {
            ...
            if (arguments == null) {
                ...
                str = arguments.getString(AccessibilityNodeInfo.EXTRA_DATA_REQUESTED_KEY);
            }
            ...
            if (provider == null) {
                ...
                if (root != null) {
                    if (str != null) {
                        view.addExtraDataToAccessibilityNodeInfo(root, str, arguments);
                    }
                    outInfos.add(root);
                    if ((fetchFlags & 1) != 0) {
            ...
            if (root != null) {
                if (str != null) {
                    provider.addExtraDataToAccessibilityNodeInfo(virtualViewId, root, str, arguments);
                }
                outInfos.add(root);
                if ((fetchFlags & 1) != 0) {
    ...
    private void scheduleMessage(Message message, int interrogatingPid, long interrogatingTid, boolean ignoreRequestPreparers) {
        if (!ignoreRequestPreparers && (holdOffMessageIfNeeded(message, interrogatingPid, interrogatingTid) ^ 1) == 0) {
            return;
        }
    ...
    private boolean holdOffMessageIfNeeded(Message originalMessage, int callingPid, long callingTid) {
        synchronized (this.mLock) {
            if (this.mNumActiveRequestPreparers != 0) {
                ...
                SomeArgs originalMessageArgs = originalMessage.obj;
                Bundle requestArguments = originalMessageArgs.arg4;
    ...
    private void findAccessibilityNodeInfosByTextUiThread(Message message) {
        ...
        try {
            ...
            if (root != null && isShown(root)) {
                ...
                if (provider != null) {
                    list = provider.findAccessibilityNodeInfosByText(text, virtualDescendantId);
                            ...
                            if (isShown(foundView)) {
                                ...
                                if (provider != null) {
                                    ...
                                    if (infosFromProvider != null) {
                                        list.addAll(infosFromProvider);
                                    }
                                } else {
                                    list.add(foundView.createAccessibilityNodeInfo());
                                }
                            }
                        }
                    }

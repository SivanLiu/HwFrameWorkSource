...
public class HwApsInterface {
    ...
    public static native void nativeSetNewHisiStatus(int i, int i2);
    ...
    public static int setGameProcessName(final String processName, final int pid, final int gameType) {
        if (processName == null) {
            return 0;
        }
        new Thread(new Runnable() {
            public void run() {
                HwFrameworkFactory.getHwNsdImpl().setGameProcessName(processName, pid, gameType);
            }
        }).start();
        return 1;
    }

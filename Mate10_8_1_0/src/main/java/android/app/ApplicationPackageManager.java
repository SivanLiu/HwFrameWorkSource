...
public class ApplicationPackageManager extends PackageManager {
    public static final int[] CORP_BADGE_LABEL_RES_ID = new int[]{17040339, 17040340, 17040341};
    ...
    public boolean isPermissionReviewModeEnabled() {
        return this.mContext.getResources().getBoolean(17956996);
    }
    ...
    public Drawable getUserBadgedIcon(Drawable icon, UserHandle user) {
        ...
        if (isManagedProfile(user.getIdentifier())) {
            return getBadgedDrawable(icon, new LauncherIcons(this.mContext).getBadgeDrawable(17302336, getUserBadgeColor(user)), null, false);
        }
        return icon;
    }
    ...
    public Drawable getUserBadgeForDensity(UserHandle user, int density) {
        Drawable badgeColor = getManagedProfileIconForDensity(user, 17302332, density);
        if (badgeColor == null) {
            return null;
        }
        badgeColor.setTint(getUserBadgeColor(user));
        Drawable badgeForeground = getDrawableForDensity(17302331, density);
    ...
    public Drawable getUserBadgeForDensityNoBackground(UserHandle user, int density) {
        ...
        Drawable badge = getManagedProfileIconForDensity(user, 17302333, density);
        if (badge != null) {

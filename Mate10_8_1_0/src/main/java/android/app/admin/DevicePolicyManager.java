...
public class DevicePolicyManager {
    ...
    public boolean installKeyPair(ComponentName admin, PrivateKey privKey, Certificate[] certs, String alias, boolean requestAccess) {
        ...
        try {
            byte[] pemCert = Credentials.convertToPem(new Certificate[]{certs[0]});
            byte[] pemChain = null;
            if (certs.length > 1) {
            ...
            return this.mService.installKeyPair(admin, this.mContext.getPackageName(), pkcs8Key, pemCert, pemChain, alias, requestAccess);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();

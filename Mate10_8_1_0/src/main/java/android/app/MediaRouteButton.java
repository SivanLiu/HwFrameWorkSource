...
public class MediaRouteButton extends View {
    ...
    private void refreshRoute() {
        ...
        boolean matchesTypes = !route.isDefault() ? route.matchesTypes(this.mRouteTypes) : false;
        boolean isConnecting = matchesTypes ? route.isConnecting() : false;
        boolean needsRefresh = false;
        if (this.mRemoteActive != matchesTypes) {
            this.mRemoteActive = matchesTypes;
            needsRefresh = true;
        }
        ...
        if (this.mRemoteIndicator != null && (this.mRemoteIndicator.getCurrent() instanceof AnimationDrawable)) {
            ...
            if (this.mAttachedToWindow) {
                if ((needsRefresh || isConnecting) && (curDrawable.isRunning() ^ 1) != 0) {
                    curDrawable.start();
                }
            } else if (matchesTypes && (isConnecting ^ 1) != 0) {

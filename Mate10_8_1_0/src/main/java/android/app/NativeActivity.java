...
public class NativeActivity extends Activity implements Callback2, Callback, OnGlobalLayoutListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (savedInstanceState != null) {
                byteArray = savedInstanceState.getByteArray(KEY_NATIVE_SAVED_STATE);
            } else {
                byteArray = null;
            }
            this.mNativeHandle = loadNativeCode(path, funcname, Looper.myQueue(), getAbsolutePath(getFilesDir()), getAbsolutePath(getObbDir()), getAbsolutePath(getExternalFilesDir(null)), VERSION.SDK_INT, getAssets(), byteArray, classLoader, classLoader.getLdLibraryPath());
    ...
    public void onGlobalLayout() {
        ...
        if (this.mLocation[0] == this.mLastContentX && this.mLocation[1] == this.mLastContentY && w == this.mLastContentWidth) {
            if (h == this.mLastContentHeight) {
                return;
            }
        }

    ...
    private Bundle callUnchecked(String method, String arg, Bundle extras) throws FileNotFoundException {
        ...
        if (this.mAuthority.equals(authority)) {
            ...
            if (DocumentsContract.METHOD_IS_CHILD_DOCUMENT.equals(method)) {
                ...
                enforceWritePermissionInner(documentUri, getCallingPackage(), null);
                out.putParcelable(DocumentsContract.EXTRA_RESULT, createWebLinkIntent(documentId, extras.getBundle(DocumentsContract.EXTRA_OPTIONS)));
            } else if (DocumentsContract.METHOD_RENAME_DOCUMENT.equals(method)) {
                enforceWritePermissionInner(documentUri, getCallingPackage(), null);
                newDocumentId = renameDocument(documentId, extras.getString("_display_name"));
                ...
                if (isTreeUri) {
                    treeDocumentId = DocumentsContract.getTreeDocumentId(documentUri);
                } else {
                    treeDocumentId = null;
                }
                Path path = findDocumentPath(treeDocumentId, documentId);
                if (isTreeUri) {
                    if (!Objects.equals(path.getPath().get(0), treeDocumentId)) {
                        Log.wtf(TAG, "Provider doesn't return path from the tree root. Expected: " + treeDocumentId + " found: " + ((String) path.getPath().get(0)));
                        LinkedList<String> docs = new LinkedList(path.getPath());
                        while (docs.size() > 1 && (Objects.equals(docs.getFirst(), treeDocumentId) ^ 1) != 0) {
                            docs.removeFirst();
                        }
                        Path path2 = new Path(null, docs);
                    }
                ...
                return getDocumentMetadata(documentId, extras.getStringArray(DocumentsContract.EXTRA_METADATA_TAGS));
            } else {

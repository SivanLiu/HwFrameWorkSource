...
import android.telecom.Logging.-$Lambda$Bho-6fQ_lBTm8N3FcbHLVOfu_sY.AnonymousClass1;
...
public class EventManager {
    ...
    public class EventRecord {
        ...
        public void dump(IndentingPrintWriter pw) {
            ...
            pw.increaseIndent();
            for (Event event : this.mEvents) {
                pw.print(EventManager.this.sDateFormat.format(new Date(event.time)));
                pw.print(" - ");
    ...
    public void dumpEventsTimeline(IndentingPrintWriter pw) {
        ...
        for (EventRecord er : this.mEventRecords) {
            for (Event ev : er.getEvents()) {
                events.add(new Pair(er.getRecordEntry(), ev));
            }
        }
        events.sort(-$Lambda$Bho-6fQ_lBTm8N3FcbHLVOfu_sY.$INST$0);
        pw.increaseIndent();
        for (Pair<Loggable, Event> event : events) {
            pw.print(this.sDateFormat.format(new Date(((Event) event.second).time)));
            pw.print(",");
    ...
    public void changeEventCacheSize(int newSize) {
        ...
        oldEventLog.forEach(new AnonymousClass1(this));
    }

    /* synthetic */ void lambda$-android_telecom_Logging_EventManager_12735(EventRecord newRecord) {

...
public final class ParcelableConference implements Parcelable {
    public static final Creator<ParcelableConference> CREATOR = new Creator<ParcelableConference>() {
        public ParcelableConference createFromParcel(Parcel source) {
            ...
            return new ParcelableConference(phoneAccount, state, capabilities, source.readInt(), connectionIds, Stub.asInterface(source.readStrongBinder()), source.readInt(), connectTimeMillis, source.readLong(), (StatusHints) source.readParcelable(classLoader), source.readBundle(classLoader));
        }

...
public class HwInnerTelephonyManagerImpl implements HwInnerTelephonyManager {
    ...
    public int getLteLevel(SignalStrength strength) {
        if (strength.isCdma()) {
            if (IS_USE_RSRQ) {
                return mHwSigStr.getLevel(SignalType.CDMALTE, strength.getLteRsrp(), strength.getLteRsrq());
            }
            return mHwSigStr.getLevel(SignalType.CDMALTE, strength.getLteRsrp(), strength.getLteRssnr());
        } else if (isVZW) {
            return mHwSigStr.getLevel(SignalType.LTE, -((-strength.getLteRsrp()) + strength.getTdScdmaDbm()), strength.getLteRssnr());
        } else if (IS_USE_RSRQ) {
            return mHwSigStr.getLevel(SignalType.LTE, strength.getLteRsrp(), strength.getLteRsrq());
        } else {

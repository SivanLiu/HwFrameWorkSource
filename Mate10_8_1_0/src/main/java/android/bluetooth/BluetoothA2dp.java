...
public final class BluetoothA2dp implements BluetoothProfile {
    ...
    public static final int A2DP_CODEC_TYPE_ERROR = -1;
    ...
    public int getDeviceCodecInfo(String deviceMac) {
        Log.d(TAG, "getDeviceCodecInfo");
        try {
            this.mServiceLock.readLock().lock();
            if (this.mService == null || !isEnabled()) {
                if (this.mService == null) {
            ...
            int deviceCodecInfo = this.mService.getDeviceCodecInfo(deviceMac);
            return deviceCodecInfo;
        } catch (RemoteException e) {
            Log.e(TAG, "Error talking to BT service in getDeviceCodecInfo()", e);
            return -1;
        } finally {
            this.mServiceLock.readLock().unlock();
        }

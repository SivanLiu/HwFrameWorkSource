...
public abstract class BatteryStats implements Parcelable {
    ...
    public static final String[] HISTORY_EVENT_NAMES = new String[]{"null", "proc", FOREGROUND_ACTIVITY_DATA, "top", "sync", "wake_lock_in", "job", Context.USER_SERVICE, "userfg", "conn", "active", "pkginst", "pkgunin", "alarm", "stats", "pkginactive", "pkgactive", "tmpwhitelist", "screenwake", "wakeupap", "longwake", "est_capacity"};
    ...
    private static final IntToString sIntToString = -$Lambda$-dncxFEc2F2bgG2fsIoC6FC6WNE.$INST$1;
    private static final IntToString sUidToString = -$Lambda$-dncxFEc2F2bgG2fsIoC6FC6WNE.$INST$0;
    ...
    static {
        ...
        r7[15] = new BitDescription(448, 6, "phone_state", "Pst", new String[]{"in", "out", "emergency", "off"}, new String[]{"in", "out", "em", "off"});
        int i = 3;
        r7[16] = new BitDescription(56, i, "phone_signal_strength", "Pss", SignalStrength.SIGNAL_STRENGTH_NAMES, new String[]{WifiEnterpriseConfig.ENGINE_DISABLE, "1", WifiScanLog.EVENT_KEY2, WifiScanLog.EVENT_KEY3, WifiScanLog.EVENT_KEY4});
        r7[17] = new BitDescription(7, 0, "brightness", "Sb", SCREEN_BRIGHTNESS_NAMES, SCREEN_BRIGHTNESS_SHORT_NAMES);
    ...
    private final void printControllerActivity(PrintWriter pw, StringBuilder sb, String prefix, String controllerName, ControllerActivityCounter counter, int which) {
        ...
        sb.append(controllerName);
        sb.append(" Tx time:     ");
        String[] powerLevel = controllerName.equals("Cellular") ? new String[]{"   less than 0dBm: ", "   0dBm to 8dBm: ", "   8dBm to 15dBm: ", "   15dBm to 20dBm: ", "   above 20dBm: "} : new String[]{"[0]", "[1]", "[2]", "[3]", "[4]"};
        int numTxLvls = Math.min(counter.getTxTimeCounters().length, powerLevel.length);
        long txLvlTimeMs;
        if (numTxLvls > 1) {
            pw.println(sb.toString());
            for (int lvl = 0; lvl < numTxLvls; lvl++) {
                txLvlTimeMs = counter.getTxTimeCounters()[lvl].getCountLocked(which);
                sb.setLength(0);
                sb.append(prefix);
                sb.append("    ");
            ...
            txLvlTimeMs = counter.getTxTimeCounters()[0].getCountLocked(which);
            formatTimeMs(sb, txLvlTimeMs);
            sb.append("(");
            sb.append(formatRatioLocked(txLvlTimeMs, totalControllerActivityTimeMs));
            sb.append(")");
        ...
        if (powerDrainMaMs > 0) {
            ...
            sb.append("     ");
            sb.append(controllerName);
            sb.append(" Battery drain: ").append(BatteryStatsHelper.makemAh(((double) powerDrainMaMs) / 3600000.0d));
            sb.append("mAh");
            pw.println(sb.toString());
        }
    }
    ...
    public final void dumpCheckinLocked(Context context, PrintWriter pw, int which, int reqUid, boolean wifiOnly) {
        ...
        long screenDozeTime = getScreenDozeTime(rawRealtime, which);
        ...
        long dischargeCount = getMahDischarge(which);
        long dischargeScreenOffCount = getMahDischargeScreenOff(which);
        long dischargeScreenDozeCount = getMahDischargeScreenDoze(which);
        ...
        if (which == 2) {
            dumpLine(pw, 0, category, BATTERY_DISCHARGE_DATA, Integer.valueOf(getDischargeStartLevel() - getDischargeCurrentLevel()), Integer.valueOf(getDischargeStartLevel() - getDischargeCurrentLevel()), Integer.valueOf(getDischargeAmountScreenOn()), Integer.valueOf(getDischargeAmountScreenOff()), Long.valueOf(dischargeCount / 1000), Long.valueOf(dischargeScreenOffCount / 1000), Integer.valueOf(getDischargeAmountScreenDoze()), Long.valueOf(dischargeScreenDozeCount / 1000));
        } else {
            dumpLine(pw, 0, category, BATTERY_DISCHARGE_DATA, Integer.valueOf(getLowDischargeAmountSinceCharge()), Integer.valueOf(getHighDischargeAmountSinceCharge()), Integer.valueOf(getDischargeAmountScreenOnSinceCharge()), Integer.valueOf(getDischargeAmountScreenOffSinceCharge()), Long.valueOf(dischargeCount / 1000), Long.valueOf(dischargeScreenOffCount / 1000), Integer.valueOf(getDischargeAmountScreenDozeSinceCharge()), Long.valueOf(dischargeScreenDozeCount / 1000));
        }
        if (reqUid < 0) {
            ...
            if (kernelWakelocks.size() > 0) {
                for (Entry<String, ? extends Timer> ent : kernelWakelocks.entrySet()) {
                    ...
                    printWakeLockCheckin(stringBuilder, (Timer) ent.getValue(), rawRealtime, null, which, "");
        ...
        if (rpmStats.size() > 0) {
            for (Entry<String, ? extends Timer> ent22 : rpmStats.entrySet()) {
                stringBuilder.setLength(0);
                Timer totalTimer = (Timer) ent22.getValue();
                long timeMs = (totalTimer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000;
                count = totalTimer.getCountLocked(which);
                Timer screenOffTimer = (Timer) screenOffRpmStats.get(ent22.getKey());
                if (screenOffTimer != null) {
                    long screenOffTimeMs = (screenOffTimer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000;
                }
                if (screenOffTimer != null) {
                    int screenOffCount = screenOffTimer.getCountLocked(which);
                }
                dumpLine(pw, 0, category, RESOURCE_POWER_MANAGER_DATA, "\"" + ((String) ent22.getKey()) + "\"", Long.valueOf(timeMs), Integer.valueOf(count));
            }
        }
        ...
        if (cpuFreqs != null) {
            ...
            while (i < cpuFreqs.length) {
                stringBuilder.append(i == 0 ? "" : ",").append(cpuFreqs[i]);
                i++;
            }
        ...
        for (iu = 0; iu < NU; iu++) {
            ...
            if (reqUid < 0 || uid == reqUid) {
                ...
                if (mobileBytesRx <= 0 && mobileBytesTx <= 0 && wifiBytesRx <= 0 && wifiBytesTx <= 0 && mobilePacketsRx <= 0 && mobilePacketsTx <= 0 && wifiPacketsRx <= 0 && wifiPacketsTx <= 0 && mobileActiveTime <= 0 && mobileActiveCount <= 0 && btBytesRx <= 0 && btBytesTx <= 0 && mobileWakeup <= 0 && wifiWakeup <= 0 && mobileBytesBgRx <= 0 && mobileBytesBgTx <= 0 && wifiBytesBgRx <= 0 && wifiBytesBgTx <= 0 && mobilePacketsBgRx <= 0 && mobilePacketsBgTx <= 0 && wifiPacketsBgRx <= 0) {
                    ...
                    wifiScanCountBg = u.getWifiScanBackgroundCount(which);
                    wifiScanActualTimeMs = (u.getWifiScanActualTime(rawRealtime) + 500) / 1000;
                    wifiScanActualTimeMsBg = (u.getWifiScanBackgroundTime(rawRealtime) + 500) / 1000;
                    uidWifiRunningTime = u.getWifiRunningTime(rawRealtime, which);
                    if (fullWifiLockOnTime == 0 && wifiScanTime == 0 && wifiScanCount == 0 && wifiScanCountBg == 0 && wifiScanActualTimeMs == 0 && wifiScanActualTimeMsBg == 0) {
                        ...
                        if (bleTimer != null) {
                            if ((bleTimer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000 != 0) {
                                ...
                                bleTimerBg = u.getBluetoothScanBackgroundTimer();
                                countBg = bleTimerBg != null ? bleTimerBg.getCountLocked(which) : 0;
                                actualTime = bleTimer.getTotalDurationMsLocked(rawRealtimeMs);
                                actualTimeBg = bleTimerBg != null ? bleTimerBg.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                                resultCount = u.getBluetoothScanResultCounter() != null ? u.getBluetoothScanResultCounter().getCountLocked(which) : 0;
                                resultCountBg = u.getBluetoothScanResultBgCounter() != null ? u.getBluetoothScanResultBgCounter().getCountLocked(which) : 0;
                                unoptimizedScanTimer = u.getBluetoothUnoptimizedScanTimer();
                                unoptimizedScanTotalTime = unoptimizedScanTimer != null ? unoptimizedScanTimer.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                                unoptimizedScanMaxTime = unoptimizedScanTimer != null ? unoptimizedScanTimer.getMaxDurationMsLocked(rawRealtimeMs) : 0;
                                unoptimizedScanTimerBg = u.getBluetoothUnoptimizedScanBackgroundTimer();
                                unoptimizedScanTotalTimeBg = unoptimizedScanTimerBg != null ? unoptimizedScanTimerBg.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                                unoptimizedScanMaxTimeBg = unoptimizedScanTimerBg != null ? unoptimizedScanTimerBg.getMaxDurationMsLocked(rawRealtimeMs) : 0;
                        ...
                        if (u.getAggregatedPartialWakelockTimer() != null) {
                            ...
                            totTimeMs = timer.getTotalDurationMsLocked(rawRealtimeMs);
                            bgTimer = timer.getSubTimer();
                            bgTimeMs = bgTimer != null ? bgTimer.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                        ...
                        for (iw = wakelocks.size() - 1; iw >= 0; iw--) {
                            ...
                            stringBuilder.setLength(0);
                            linePrefix = printWakeLockCheckin(stringBuilder, wl.getWakeTime(1), rawRealtime, FullBackup.FILES_TREE_TOKEN, which, "");
                            pTimer = wl.getWakeTime(0);
                        ...
                        completions = u.getJobCompletionStats();
                        for (ic = completions.size() - 1; ic >= 0; ic--) {
                            if (((SparseIntArray) completions.valueAt(ic)) == null) {
                                dumpLine(pw, uid, category, JOB_COMPLETION_DATA, "\"" + ((String) completions.keyAt(ic)) + "\"", Integer.valueOf(((SparseIntArray) completions.valueAt(ic)).get(0, 0)), Integer.valueOf(((SparseIntArray) completions.valueAt(ic)).get(1, 0)), Integer.valueOf(((SparseIntArray) completions.valueAt(ic)).get(2, 0)), Integer.valueOf(((SparseIntArray) completions.valueAt(ic)).get(3, 0)), Integer.valueOf(((SparseIntArray) completions.valueAt(ic)).get(4, 0)));
                            }
                        }
                        ...
                        for (ise = 0; ise < NSE; ise++) {
                            ...
                            if (timer == null) {
                                if ((timer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000 == 0) {
                                    ...
                                    bgActualTime = bgTimer != null ? bgTimer.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                        ...
                        if (cpuFreqs != null) {
                            cpuFreqTimeMs = u.getCpuFreqTimes(which);
                            if (cpuFreqTimeMs != null && cpuFreqTimeMs.length == cpuFreqs.length) {
                                stringBuilder.setLength(0);
                                i = 0;
                                while (i < cpuFreqTimeMs.length) {
                                    stringBuilder.append(i == 0 ? "" : ",").append(cpuFreqTimeMs[i]);
                                    i++;
                                }
                                screenOffCpuFreqTimeMs = u.getScreenOffCpuFreqTimes(which);
                    ...
                    if (bleTimer != null) {
                        if ((bleTimer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000 != 0) {
                            ...
                            if (u.getBluetoothScanResultCounter() != null) {
                            }
                    ...
                    if (cpuFreqs != null) {
                        ...
                        if (screenOffCpuFreqTimeMs != null) {
                            ...
                            while (i < screenOffCpuFreqTimeMs.length) {
                                stringBuilder.append(",").append(append);
                            }
                        }
                        dumpLine(pw, uid, category, CPU_TIMES_AT_FREQ_DATA, "A", Integer.valueOf(cpuFreqTimeMs.length), stringBuilder.toString());
    ...
    public final void dumpLocked(Context context, PrintWriter pw, String prefix, int which, int reqUid, boolean wifiOnly) {
        ...
        sb.append("(");
        sb.append(formatRatioLocked(whichBatteryUptime, whichBatteryRealtime));
        sb.append(") uptime");
        pw.println(sb.toString());
        sb.setLength(0);
        sb.append(prefix);
        ...
        if (dischargeScreenDozeCount >= 0) {
            ...
            sb.append(BatteryStatsHelper.makemAh(((double) dischargeScreenDozeCount) / 1000.0d));
            sb.append(" mAh");
            pw.println(sb.toString());
        }
        long dischargeScreenOnCount = (dischargeCount - dischargeScreenOffCount) - dischargeScreenDozeCount;
        ...
        pw.println(formatBytesLocked(mobileRxTotalBytes));
        pw.print("     Cellular data sent: ");
        pw.println(formatBytesLocked(mobileTxTotalBytes));
        ...
        pw.println(sb.toString());
        printControllerActivity(pw, sb, prefix, "Cellular", getModemControllerActivity(), which);
        pw.print(prefix);
        sb.setLength(0);
        sb.append(prefix);
        ...
        pw.println(formatBytesLocked(wifiTxTotalBytes));
        ...
        String[] wifiRxSignalStrengthDescription = new String[5];
        wifiRxSignalStrengthDescription[0] = "very poor (less than -88.75dBm): ";
        wifiRxSignalStrengthDescription[1] = "poor (-88.75 to -77.5dBm): ";
        wifiRxSignalStrengthDescription[2] = "moderate (-77.5dBm to -66.25dBm): ";
        wifiRxSignalStrengthDescription[3] = "good (-66.25dBm to -55dBm): ";
        wifiRxSignalStrengthDescription[4] = "great (greater than -55dBm): ";
        didOne = false;
        int numWifiRxBins = Math.min(5, wifiRxSignalStrengthDescription.length);
        for (i = 0; i < numWifiRxBins; i++) {
        ...
        if (rpmStats.size() > 0) {
            ...
            if (rpmStats.size() > 0) {
                for (Entry<String, ? extends Timer> ent22 : rpmStats.entrySet()) {
                    printTimer(pw, sb, (Timer) ent22.getValue(), rawRealtime, which, prefix, (String) ent22.getKey());
                }
            }
            pw.println();
        }
            ...
            if (reqUid < 0 || uid == reqUid || uid == 1000) {
                ...
                if (mobileRxBytes <= 0 && mobileTxBytes <= 0 && mobileRxPackets <= 0) {
                    ...
                    if (wifiRxBytes <= 0 && wifiTxBytes <= 0 && wifiRxPackets <= 0) {
                        ...
                        if (fullWifiLockOnTime == 0 && wifiScanTime == 0 && wifiScanCount == 0 && wifiScanCountBg == 0 && wifiScanActualTime == 0 && wifiScanActualTimeBg == 0) {
                            if (uidWifiRunningTime != 0) {
                            }
                            if (wifiWakeup > 0) {
                                sb.setLength(0);
                            ...
                            bleTimer = u.getBluetoothScanTimer();
                            if (bleTimer != null) {
                                totalTimeMs = (bleTimer.getTotalTimeLocked(rawRealtime, which) + 500) / 1000;
                                if (totalTimeMs != 0) {
                                    count = bleTimer.getCountLocked(which);
                                    bleTimerBg = u.getBluetoothScanBackgroundTimer();
                                    countBg = bleTimerBg == null ? bleTimerBg.getCountLocked(which) : 0;
                                    actualTimeMs = bleTimer.getTotalDurationMsLocked(rawRealtimeMs);
                                    actualTimeMsBg = bleTimerBg == null ? bleTimerBg.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                                    resultCount = u.getBluetoothScanResultCounter() == null ? u.getBluetoothScanResultCounter().getCountLocked(which) : 0;
                                    resultCountBg = u.getBluetoothScanResultBgCounter() == null ? u.getBluetoothScanResultBgCounter().getCountLocked(which) : 0;
                                    unoptimizedScanTimer = u.getBluetoothUnoptimizedScanTimer();
                                    unoptimizedScanTotalTime = unoptimizedScanTimer == null ? unoptimizedScanTimer.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                            ...
                            if (countWakelock > 1) {
                                actualTotalPartialWakelock = 0;
                                actualBgPartialWakelock = 0;
                                if (u.getAggregatedPartialWakelockTimer() != null) {
                                    aggTimer = u.getAggregatedPartialWakelockTimer();
                                    actualTotalPartialWakelock = aggTimer.getTotalDurationMsLocked(rawRealtimeMs);
                                    bgAggTimer = aggTimer.getSubTimer();
                                    actualBgPartialWakelock = bgAggTimer == null ? bgAggTimer.getTotalDurationMsLocked(rawRealtimeMs) : 0;
                                }
                                if (actualTotalPartialWakelock == 0 && actualBgPartialWakelock == 0 && totalFullWakelock == 0 && totalPartialWakelock == 0) {
                            ...
                            completions = u.getJobCompletionStats();
                            for (ic = completions.size() - 1; ic >= 0; ic--) {
                                types = (SparseIntArray) completions.valueAt(ic);
                                if (types == null) {
                                    pw.print(prefix);
                                    pw.print("    Job Completions ");
                                    pw.print((String) completions.keyAt(ic));
                                    pw.print(":");
                                    for (it = 0; it < types.size(); it++) {
                                        pw.print(WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER);
                            ...
                            uidActivity = ((uidActivity | printTimer(pw, sb, u.getVibratorOnTimer(), rawRealtime, which, prefix, "Vibrator")) | printTimer(pw, sb, u.getForegroundActivityTimer(), rawRealtime, which, prefix, "Foreground activities")) | printTimer(pw, sb, u.getForegroundServiceTimer(), rawRealtime, which, prefix, "Foreground services");
                            totalStateTime = 0;
                            for (ips = 0; ips < 6; ips++) {
                                time = u.getProcessStateTime(ips, rawRealtime, which);
                                if (time <= 0) {
                            ...
                            cpuFreqTimes = u.getCpuFreqTimes(which);
                            ...
                            screenOffCpuFreqTimes = u.getScreenOffCpuFreqTimes(which);
                            ...
                            if (processStats != null) {
                                for (ipr = processStats.size() - 1; ipr >= 0; ipr--) {
                                    ...
                                    numCrashes = ps.getNumCrashes(which);
                                    numAnrs = ps.getNumAnrs(which);
                                    numExcessive = which != 0 ? ps.countExcessivePowers() : 0;
                        ...
                        if (screenOffCpuFreqTimes != null) {
                            sb.setLength(0);
                            sb.append("    Total screen-off cpu time per freq:");
                            while (i < screenOffCpuFreqTimes.length) {
                                sb.append(WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER).append(append22);
                            }
                            pw.println(sb.toString());
                        }
    ...
    public void dumpLocked(Context context, PrintWriter pw, int flags, int reqUid, long histStart) {
        ...
        if (!filtering || (flags & 6) != 0) {
            ...
            if (!(filtering && (flags & 2) == 0)) {
                ...
                dumpLocked(context, pw, "", 0, reqUid, (flags & 64) != 0);
                pw.println();
            }
        }
    }
    ...
    public void dumpCheckinLocked(Context context, PrintWriter pw, List<ApplicationInfo> apps, int flags, long histStart) {
        int i;
        prepareForDumpLocked();
        dumpLine(pw, 0, "i", VERSION_DATA, "27", Integer.valueOf(getParcelVersion()), getStartPlatformVersion(), getEndPlatformVersion());

            ...
            if (typedArray.length() > 0) {
                ...
                if (typedArray.getResourceId(0, 0) != 0) {
                    for (int i = 0; i < typedArray.length(); i++) {
                        list.add(Arrays.asList(resources.getStringArray(typedArray.getResourceId(i, 0))));
                    }
                } else {
                    list.add(Arrays.asList(resources.getStringArray(certsId)));
                }
            }
        }
        return new ProviderResourceEntry(authority, providerPackage, query, list);
    }

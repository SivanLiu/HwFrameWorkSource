        ...
        while (x < gridWidth) {
            ...
            while (y < gridHeight) {
                ...
                for (int i = 0; i < this.mKeys.size(); i++) {
                    ...
                    if (key.squaredDistanceFrom(x, y) >= this.mProximityThreshold && key.squaredDistanceFrom((this.mCellWidth + x) - 1, y) >= this.mProximityThreshold && key.squaredDistanceFrom((this.mCellWidth + x) - 1, (this.mCellHeight + y) - 1) >= this.mProximityThreshold) {
                        if (key.squaredDistanceFrom(x, (this.mCellHeight + y) - 1) >= this.mProximityThreshold) {
                        }
                    }
                    int count2 = count + 1;
                    indices[count] = i;

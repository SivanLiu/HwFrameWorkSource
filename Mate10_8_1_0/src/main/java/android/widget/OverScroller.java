...
public class OverScroller {
    ...
    private static boolean SMART_SLIDE_PROPERTIES = SystemProperties.getBoolean("ro.config.uifirst_listview_optimization_enable", true);
    ...
    public OverScroller(Context context, Interpolator interpolator, boolean flywheel) {
        ...
        if (SMART_SLIDE_PROPERTIES) {
            Log.i(LOG_TAG, "Smart slider init OverscrollerConfig SMART_SLIDE_PROPERTIES is " + SMART_SLIDE_PROPERTIES);
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        if (SMART_SLIDE_PROPERTIES && mIsApplicationEnable) {
            mIsEnable = true;
            fling(startX, startY, velocityX, velocityY, minX, maxX, minY, maxY, DEFAULT_MULTIPLE_FLING_LENGTH_THRESHOLD);
            return;
        }
        mIsEnable = false;
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY, float slidingDistance) {
        float oldVelocityX = this.mScrollerX.mCurrVelocity;
        float oldVelocityY = this.mScrollerY.mCurrVelocity;
        mIsEnable = true;
        int multipleVelocity = mSmartSlideOverScroller.fling(startX, startY, velocityX, velocityY, oldVelocityX, oldVelocityY, minX, maxX, minY, maxY, 0, 0, slidingDistance);
        this.mMode = 1;
        sVelocityToSplineOverScroller = velocityY;
        this.mScrollerX.fling(startX, velocityX, minX, maxX, 0);
        this.mScrollerY.fling(startY, multipleVelocity, minY, maxY, 0);
    }

...
public class TextInputTimePickerView extends RelativeLayout {
    ...
    boolean validateInput() {
        boolean parseAndSetMinuteInternal;
        if (parseAndSetHourInternal(this.mHourEditText.getText().toString())) {
            parseAndSetMinuteInternal = parseAndSetMinuteInternal(this.mMinuteEditText.getText().toString());
        } else {
            parseAndSetMinuteInternal = false;
        }
        setError(parseAndSetMinuteInternal ^ 1);
        return parseAndSetMinuteInternal;
    }

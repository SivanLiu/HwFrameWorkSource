...
*/
    ...
    public TextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            ...
            for (i = 0; i < n; i++) {
                ...
                switch (attr) {
                    ...
                    case 10:
                        ...
                        break;
                    case 11:
                        z = appearance.getBoolean(attr, false);
                        break;
                    case 12:
                        ...
                        break;
                    case 13:
                        z2 = appearance.getBoolean(attr, false);
                        break;
                    case 14:
        ...
        setTypefaceFromAttrs(typeface, str, typefaceIndex, styleIndex);
        if (shadowcolor != 0) {
        ...
        boolean isClickable = !canInputOrMove ? isClickable() : true;
        boolean isLongClickable = !canInputOrMove ? isLongClickable() : true;
        ...
        this.isHwTheme = HwWidgetFactory.checkIsHwTheme(getContext(), attrs);
        this.isNeo = SystemProperties.get("ro.product.model", "").startsWith("NEO-");
        boolean isUpdatedSystemApp = this.isNeo ? !context.getApplicationInfo().isSystemApp() ? context.getApplicationInfo().isUpdatedSystemApp() : true : false;
        this.isSystemApp = isUpdatedSystemApp;
    }
    ...
    public void onEditorAction(int actionCode) {
        InputContentType inputContentType = this.mEditor == null ? null : this.mEditor.mInputContentType;
        if (inputContentType != null) {
            if (inputContentType.onEditorActionListener != null && inputContentType.onEditorActionListener.onEditorAction(this, actionCode, null)) {
                return;
            }
            View v;
            InputMethodManager imm;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int colorOri = color;
        if (this.isNeo && color == -16777216 && this.isHwTheme && this.isSystemApp) {
            color = COLOR_FIX_TO;
        }
        ...
        if (this.isNeo && this.isHwTheme && this.isSystemApp && colorOri != color) {
            this.mTextPaint.setColor(colorOri);
        }
        canvas.restore();
    }
    ...
    public boolean onPrivateIMECommand(String action, Bundle data) {
        ...
        for (int i = 0; i < this.mFilters.length; i++) {
            if (this.mFilters[i] instanceof LengthFilter) {
                LengthFilter lf = this.mFilters[i];
    ...
    protected Layout makeSingleLayout(int wantWidth, Metrics boring, int ellipsisWidth, Alignment alignment, boolean shouldEllipsize, TruncateAt effectiveEllipsize, boolean useSaved) {
        ...
        if (this.mText instanceof Spannable) {
            layout = new DynamicLayout(this.mText, this.mTransformed, this.mTextPaint, wantWidth, alignment, this.mTextDir, this.mSpacingMult, this.mSpacingAdd, this.mIncludePad, this.mBreakStrategy, this.mHyphenationFrequency, this.mJustificationMode, getKeyListener() == null ? effectiveEllipsize : null, ellipsisWidth);
        } else {
    ...
    void handleTextChanged(CharSequence buffer, int start, int before, int after) {
        ...
        if (inputMethodState != null) {
            ...
            if (inputMethodState.mChangedStart < 0) {
                inputMethodState.mChangedStart = start;
                inputMethodState.mChangedEnd = start + before;
            } else {
                inputMethodState.mChangedStart = Math.min(inputMethodState.mChangedStart, start);
                inputMethodState.mChangedEnd = Math.max(inputMethodState.mChangedEnd, (start + before) - inputMethodState.mChangedDelta);
            }
            inputMethodState.mChangedDelta += after - before;
        }
    ...
    boolean selectAllText() {
        if (this.mEditor != null) {
            this.mEditor.hideFloatingToolbar(500);
        }

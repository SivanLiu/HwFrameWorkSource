...
public class SRBitmapManager extends HwSuperResolutionListener {
    ...
    private static int SR_MIN_EDGE_LIMIT = 300;
    ...
    private static SRBitmapManager sInstance = null;
    ...
    private class ManagerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    SRBitmapManager.this.doSuperResolution();
                    return;
                case 2:
                    SRBitmapManager.this.SRClientStop();
                    return;
                default:
                    return;
            }
    ...
    public static synchronized SRBitmapManager getInstance() {
        synchronized (SRBitmapManager.class) {
            if (sInstance == null) {
                sInstance = new SRBitmapManager();
                SRBitmapManager sRBitmapManager = sInstance;
                return sRBitmapManager;
            }
            sRBitmapManager = sInstance;
            return sRBitmapManager;
        }
    }
    ...
    public void srBitmap(Bitmap bitmap) {
        ...
        if (bitmap == null || (bitmap.isMutable() ^ 1) != 0) {
    ...
    private void SRClientStop() {
        ...
        switch (getSuperResolution().stop()) {
            case 0:
                waitQueueThread();
                break;
            default:
    ...
    private synchronized void pushTask(SRBitmapTask task) {
        ...
        if (this.mTaskQueue.size() == 1 && (this.mDoing ^ 1) != 0) {
            sendDoSRMessage();
        }
    }

...
public final class ZoneMeta {
    ...
    private static String getShortIDFromCanonical(String canonicalID) {
        String shortID = null;
        try {
            shortID = UResourceBundle.getBundleInstance(ICUData.ICU_BASE_NAME, "keyTypeData", ICUResourceBundle.ICU_DATA_CLASS_LOADER).get("typeMap").get("timezone").getString(canonicalID.replace('/', ':'));
        } catch (MissingResourceException e) {
        }
        return shortID;
    }

...
public final class UCharacter implements ECharacterCategory, ECharacterDirection {
        ...
        public static final UnicodeBlock forName(String blockName) {
            ...
            if (map == null) {
                ...
                for (UnicodeBlock b : BLOCKS_) {
                    map.put(trimBlockName(UCharacter.getPropertyValueName(4097, b.getID(), 1)), b);
                }
                mref = new SoftReference(map);
            }
            UnicodeBlock b2 = (UnicodeBlock) map.get(trimBlockName(blockName));
            if (b2 != null) {
                return b2;
            }
    ...
    public static int codePointCount(CharSequence text, int start, int limit) {
        ...
        while (limit > start) {
            ...
            while (ch >= MIN_LOW_SURROGATE && ch <= '?' && limit > start) {
                limit--;
                ch = text.charAt(limit);
                if (ch >= '?' && ch <= MAX_HIGH_SURROGATE) {
                    len--;
                    break;
                }
            }
    ...
    public static int offsetByCodePoints(CharSequence text, int index, int codePointOffset) {
        ...
        if (codePointOffset < 0) {
            while (true) {
                ...
                while (ch >= MIN_LOW_SURROGATE && ch <= '?' && index > 0) {
                    index--;
                    ch = text.charAt(index);
                    if (ch < '?' || ch > MAX_HIGH_SURROGATE) {
                        codePointOffset++;
            ...
            while (true) {
                ...
                index2 = index;
                while (ch >= '?' && ch <= MAX_HIGH_SURROGATE && index2 < limit) {
                    index = index2 + 1;
                    ch = text.charAt(index2);
                    if (ch < MIN_LOW_SURROGATE || ch > '?') {
                        codePointOffset--;
                        if (codePointOffset < 0) {
                            return index - 1;
                        }
    ...
    public static int offsetByCodePoints(char[] text, int start, int count, int index, int codePointOffset) {
        ...
        if (codePointOffset < 0) {
            while (true) {
                ...
                while (ch >= MIN_LOW_SURROGATE && ch <= '?' && index > start) {
                    index--;
                    ch = text[index];
                    if (ch < '?' || ch > MAX_HIGH_SURROGATE) {
                        codePointOffset++;
            ...
            while (true) {
                ...
                while (true) {
                    index2 = index;
                    if (ch >= '?' && ch <= MAX_HIGH_SURROGATE && index2 < limit) {
                        index = index2 + 1;
                        ch = text[index2];
                        if (ch < MIN_LOW_SURROGATE || ch > '?') {
                            codePointOffset--;

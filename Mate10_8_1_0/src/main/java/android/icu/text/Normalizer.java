...
*/
    ...
    static int cmpEquivFold(CharSequence cs1, CharSequence cs2, int options) {
        ...
        if ((524288 & options) != 0) {
            normalizer2Impl = Norm2AllModes.getNFCInstance().impl;
        } else {
            normalizer2Impl = null;
        }
        if ((65536 & options) != 0) {
                ...
                if (c1 >= 0) {
                    if (c2 >= 0) {
                        ...
                        if (level2 == 0 && (65536 & options) != 0) {
                            length = csp.toFullFolding(cp2, stringBuilder, options);
                            if (length >= 0) {
                                if (UTF16.isSurrogate((char) c2)) {
                                    if (UTF16Plus.isSurrogateLead(c2)) {
                                        s2++;
                                ...
                                level2++;
                                if (length <= 31) {
                                    stringBuilder.delete(0, stringBuilder.length() - length);
                                } else {
                                    stringBuilder.setLength(0);
                                    stringBuilder.appendCodePoint(length);
                                }
                                cs2 = stringBuilder;
                                s2 = 0;
                                limit2 = stringBuilder.length();
                                c2 = -1;
                            }
                        }
                        if (level1 < 2 && (524288 & options) != 0) {
                            String decomp1 = normalizer2Impl.getDecomposition(cp1);
                            if (decomp1 != null) {
                                if (UTF16.isSurrogate((char) c1)) {
                                    if (UTF16Plus.isSurrogateLead(c1)) {
                                        s1++;
                        ...
                        String decomp2 = normalizer2Impl.getDecomposition(cp2);
                        if (decomp2 == null) {
                            break;
                        }
                        if (UTF16.isSurrogate((char) c2)) {

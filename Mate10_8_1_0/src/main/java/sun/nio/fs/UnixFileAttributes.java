...
class UnixFileAttributes implements PosixFileAttributes {
    ...
    boolean isDevice() {
        ...
        if (type == UnixConstants.S_IFCHR || type == UnixConstants.S_IFBLK || type == UnixConstants.S_IFIFO) {
            return true;
        }
        return false;
    }
    ...
    public boolean isOther() {
        int type = this.st_mode & UnixConstants.S_IFMT;
        if (type == UnixConstants.S_IFREG || type == UnixConstants.S_IFDIR || type == UnixConstants.S_IFLNK) {
            return false;
        }
        return true;
    }

...
class UnixAsynchronousSocketChannelImpl extends AsynchronousSocketChannelImpl implements PollableChannel {
    ...
    <A> Future<Void> implConnect(SocketAddress remote, A attachment, CompletionHandler<Void, ? super A> handler) {
        ...
        if (isOpen()) {
            ...
            try {
                ...
                if (th2 != null) {
                    try {
                        ...
                        th2.addSuppressed(suppressed);
                    }
                }
                if (handler == null) {
                    return CompletedFuture.withResult(null, th2);
                }
                Invoker.invoke(this, handler, attachment, null, th2);
                return null;
            } catch (Throwable th5) {
                end();
            }
            ...
            if (handler == null) {
                return CompletedFuture.withFailure(th2);
            }
            Invoker.invoke(this, handler, attachment, null, th2);
            return null;
        }
    ...
    private void finishRead(boolean mayInvokeDirect) {
        ...
        try {
            ...
            Object valueOf = exc != null ? null : scattering ? Long.valueOf((long) n) : Integer.valueOf(n);
            if (handler == null) {
                future.setResult(valueOf, exc);
            } else if (mayInvokeDirect) {
                Invoker.invokeUnchecked(handler, att, valueOf, exc);
            } else {
                Invoker.invokeIndirectly((AsynchronousChannel) this, handler, att, valueOf, exc);
            }
        } catch (Throwable th) {
            Throwable x = th;
            try {
    ...
    <V extends Number, A> Future<V> implRead(boolean isScatteringRead, ByteBuffer dst, ByteBuffer[] dsts, long timeout, TimeUnit unit, A attachment, CompletionHandler<V, ? super A> handler) {
        ...
        try {
            ...
            if (attemptRead) {
                if (isScatteringRead) {
                    i = (int) IOUtil.read(this.fd, dsts, nd);
                } else {
                    i = IOUtil.read(this.fd, dst, -1, nd);
                }
            }
            if (i == -2) {
                ...
                synchronized (this.updateLock) {
                    try {
                        ...
                        if (handler == null) {
                            ...
                            PendingFuture<V, A> result = new PendingFuture(this, OpType.READ);
                            try {
            ...
            Object valueOf = th2 != null ? null : isScatteringRead ? Long.valueOf((long) i) : Integer.valueOf(i);
            if (handler == null) {
                return CompletedFuture.withResult(valueOf, th2);
            }
            if (invokeDirect) {
                Invoker.invokeDirect(myGroupAndInvokeCount, handler, attachment, valueOf, th2);
            } else {
                Invoker.invokeIndirectly((AsynchronousChannel) this, (CompletionHandler) handler, (Object) attachment, valueOf, th2);
            }
            return null;
        } catch (Throwable th5) {
            Throwable x = th5;
    ...
    private void finishWrite(boolean mayInvokeDirect) {
        ...
        try {
            ...
            Object valueOf = exc != null ? null : gathering ? Long.valueOf((long) n) : Integer.valueOf(n);
            if (handler == null) {
                future.setResult(valueOf, exc);
            } else if (mayInvokeDirect) {
                Invoker.invokeUnchecked(handler, att, valueOf, exc);
    ...
    <V extends Number, A> Future<V> implWrite(boolean isGatheringWrite, ByteBuffer src, ByteBuffer[] srcs, long timeout, TimeUnit unit, A attachment, CompletionHandler<V, ? super A> handler) {
        ...
        try {
            ...
            if (attemptWrite != 0) {
                if (isGatheringWrite) {
                    i = (int) IOUtil.write(this.fd, srcs, nd);
                } else {
                    i = IOUtil.write(this.fd, src, -1, nd);
                }
            }
            if (i == -2) {
                Future<V> future = null;
                synchronized (this.updateLock) {
                    try {
                        ...
                        if (handler == null) {
                            ...
                            PendingFuture<V, A> result = new PendingFuture(this, OpType.WRITE);
                            try {
            ...
            Object valueOf = th2 != null ? null : isGatheringWrite ? Long.valueOf((long) i) : Integer.valueOf(i);
            if (handler == null) {
                return CompletedFuture.withResult(valueOf, th2);
            }
            if (invokeDirect) {

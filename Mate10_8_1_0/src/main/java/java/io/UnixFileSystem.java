    ...
    public String canonicalize(String path) throws IOException {
        ...
        if (res == null) {
            ...
            if (useCanonPrefixCache) {
                str = parentOrNull(path);
                if (str != null) {
                    resDir = this.javaHomePrefixCache.get(str);
                    if (resDir != null) {
                        String filename = path.substring(str.length() + 1);
                        res = resDir + this.slash + filename;
                        this.cache.put(str + this.slash + filename, res);
                    }
                }
            }
            if (res == null) {
                ...
                if (useCanonPrefixCache && str != null && str.startsWith(this.javaHome)) {
                    resDir = parentOrNull(res);
                    if (resDir != null && resDir.equals(str)) {
                        File f = new File(res);
                        if (f.exists() && (f.isDirectory() ^ 1) != 0) {
                            this.javaHomePrefixCache.put(str, resDir);
                        }
                    }
                }
            }

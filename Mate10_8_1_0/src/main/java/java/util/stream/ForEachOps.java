    ...
    static final class ForEachOrderedTask<S, T> extends CountedCompleter<Void> {
        ...
        private static <S, T> void doCompute(ForEachOrderedTask<S, T> task) {
            ...
            if (task2.getPendingCount() > 0) {
                task2.node = ((Builder) task2.helper.wrapAndCopyInto(task2.helper.makeNodeBuilder(task2.helper.exactOutputSizeIfKnown(rightSplit), -$Lambda$Lw2Alu2LEcypd4PXrfG0QIAAKKo.$INST$2), rightSplit)).build();

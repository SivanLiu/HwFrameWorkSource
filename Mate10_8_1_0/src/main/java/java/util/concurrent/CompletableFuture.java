...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
        ...
        if (a != null) {
            ...
            if (!(s == null || f == null)) {
                if (this.result == null) {
                    ...
                    CompletableFuture<T> g = ((CompletionStage) f.apply(s)).toCompletableFuture();
    ...
    public boolean cancel(boolean mayInterruptIfRunning) {
        ...
        if (this.result == null) {
            internalComplete = internalComplete(new AltResult(new CancellationException()));
        } else {
            internalComplete = false;
        }
        postComplete();
        return !internalComplete ? isCancelled() : true;
    }

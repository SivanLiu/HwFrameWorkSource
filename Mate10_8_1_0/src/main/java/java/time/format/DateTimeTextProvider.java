    ...
    private Object createStore(TemporalField field, Locale locale) {
        ...
        if (field == ChronoField.ERA) {
            ...
            for (TextStyle textStyle2 : TextStyle.values()) {
                displayNames = CalendarDataUtility.retrieveJavaTimeFieldValueNames("gregory", 2, textStyle2.toCalendarStyle(), locale);
                map = new HashMap();
                if (displayNames != null) {
                    ...
                    for (int month = 0; month <= 11; month++) {
                        name = CalendarDataUtility.retrieveJavaTimeFieldValueName("gregory", 2, month, textStyle2.toCalendarStyle(), locale);
                        if (name == null) {
                            break;
                        }
            ...
            for (TextStyle textStyle22 : TextStyle.values()) {
                displayNames = CalendarDataUtility.retrieveJavaTimeFieldValueNames("gregory", 7, textStyle22.toCalendarStyle(), locale);
                map = new HashMap();
                if (displayNames != null) {
                    ...
                    for (int wday = 1; wday <= 7; wday++) {
                        name = CalendarDataUtility.retrieveJavaTimeFieldValueName("gregory", 7, wday, textStyle22.toCalendarStyle(), locale);
                        if (name == null) {
                            break;
                        }
                ...
                if (!map.isEmpty()) {
                    styleMap.put(textStyle22, map);
                }
            }
            return new LocaleStore(styleMap);
        } else if (field == ChronoField.AMPM_OF_DAY) {
            for (TextStyle textStyle222 : TextStyle.values()) {
                if (!textStyle222.isStandalone()) {
                    displayNames = CalendarDataUtility.retrieveJavaTimeFieldValueNames("gregory", 9, textStyle222.toCalendarStyle(), locale);
                    if (displayNames != null) {
                        map = new HashMap();

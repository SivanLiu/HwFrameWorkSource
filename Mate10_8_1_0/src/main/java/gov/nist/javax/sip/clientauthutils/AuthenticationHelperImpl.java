...
public class AuthenticationHelperImpl implements AuthenticationHelper {
    ...
    private AuthorizationHeader getAuthorization(String method, String uri, String requestBody, WWWAuthenticateHeader authHeader, UserCredentials userCredentials) {
        String str = authHeader.getQop() != null ? "auth" : null;
        String nc_value = "00000001";
        String cnonce = "xyz";
        String response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(), userCredentials.getUserName(), authHeader.getRealm(), userCredentials.getPassword(), authHeader.getNonce(), nc_value, cnonce, method, uri, requestBody, str, this.sipStack.getStackLogger());
        try {
            AuthorizationHeader authorization;
    ...
    private AuthorizationHeader getAuthorization(String method, String uri, String requestBody, WWWAuthenticateHeader authHeader, UserCredentialHash userCredentials) {
        ...
        String nc_value = "00000001";
        String cnonce = "xyz";
        String response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(), userCredentials.getHashUserDomainPassword(), authHeader.getNonce(), nc_value, cnonce, method, uri, requestBody, str, this.sipStack.getStackLogger());
        try {
            AuthorizationHeader authorization;
            if (authHeader instanceof ProxyAuthenticateHeader) {
                authorization = this.headerFactory.createProxyAuthorizationHeader(authHeader.getScheme());

...
public class SystemWideActionsListener implements PointerEventListener, OnGesturePerformedListener, OnKnockGestureListener {
    ...
    public SystemWideActionsListener(Context context, HwPhoneWindowManager phoneWindowManager) {
        ...
        this.mNavBarPortraitHeight = context.getResources().getDimensionPixelSize(17105147);
        this.mNavBarLandscapeHeight = context.getResources().getDimensionPixelSize(17105149);
        ...
        this.handler = new Handler() {
            public void handleMessage(Message msg) {
                ...
                Log.d(SystemWideActionsListener.TAG, "handleMessage msg " + msg.what);
                switch (msg.what) {
                    case 0:
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 2, 1);
                        SystemWideActionsListener.this.knuckGestureSetting.reportTpAndAccVendor(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 23, (short) 24);
                        return;
                    case 1:
                        String lowerGestureName = msg.obj;
                        if (StylusGestureSettings.STYLUS_GESTURE_S_SUFFIX.equals(lowerGestureName)) {
                            Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 10, 1);
                        } else if (StylusGestureSettings.STYLUS_GESTURE_C_SUFFIX.equals(lowerGestureName)) {
                            Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 7, 1);
                        } else if ("e".equals(lowerGestureName)) {
                            Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 8, 1);
                        } else if (StylusGestureSettings.STYLUS_GESTURE_M_SUFFIX.equals(lowerGestureName)) {
                            Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 9, 1);
                        } else if (StylusGestureSettings.STYLUS_GESTURE_W_SUFFIX.equals(lowerGestureName)) {
                            Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 11, 1);
                        }
                        SystemWideActionsListener.this.knuckGestureSetting.reportTpAndAccVendor(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 23, (short) 24);
                        return;
                    case 2:
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 12, 1);
                        SystemWideActionsListener.this.knuckGestureSetting.reportTpAndAccVendor(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 23, (short) 24);
                        return;
                    case 3:
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 1, 1);
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 5, SystemWideActionsListener.this.knockGestureDetector.getDoubleKnockTimeInterval());
                        SystemWideActionsListener.this.knuckGestureSetting.reportTpAndAccVendor(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 23, (short) 24);
                        return;
                    case 4:
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 0, 1);
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 4, SystemWideActionsListener.this.knockGestureDetector.getDoubleKnockTimeInterval());
                        Reporter.reportUserData((int) KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 6, SystemWideActionsListener.this.knockGestureDetector.getDoubleKnockDisInterval());
                        SystemWideActionsListener.this.knuckGestureSetting.reportTpAndAccVendor(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD, (short) 23, (short) 24);
                        return;
                    default:
                        return;
    ...
    public void onLetterGesture(String gestureName, Gesture gesture, double predictionScore) {
        ...
        if (StylusGestureSettings.STYLUS_GESTURE_S_SUFFIX.equals(gestureName.toLowerCase()) && FingerSenseSettings.isFingerSenseSmartshotEnabled(this.context.getContentResolver())) {
            ...
            if (startIntent != null) {
                try {
                    List<RecentTaskInfo> recentTask = ActivityManagerNative.getDefault().getRecentTasks(1, 5, ActivityManagerNative.getDefault().getCurrentUser().id).getList();
    ...
    public void onLineGesture(String gestureName, Gesture gesture, double predictionScore) {
        ...
        if (FingerSenseSettings.isFingerSenseLineGestureEnabled(this.context.getContentResolver())) {
            ...
            if (orientedDistance <= ((float) minWindowSize) || orientedDistance >= ((float) (availableScreenSize - minWindowSize))) {
                ...
                KeyguardManager keyguardManager = getKeyguardService();
                if (!(keyguardManager == null || (keyguardManager.isKeyguardLocked() ^ 1) == 0)) {
                    try {

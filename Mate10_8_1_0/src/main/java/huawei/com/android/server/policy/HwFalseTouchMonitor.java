...
public class HwFalseTouchMonitor {
    ...
    public class TrembleChecker extends FalseTouchChecker {
        ...
        public boolean reportData() {
            ...
            if (this.leftCount < 10 || this.rightCount < 10) {
                if (this.belowCount >= 10 && this.aboveCount >= 10) {
                }
                return true;
            }
    ...
    private boolean edgePoint(MonitorPoint point) {
        ...
        float x = point.x;
        boolean edge = point.orientation == 0 ? (x >= GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO && x <= 100.0f) || (x >= ((float) (width - 100)) && x <= ((float) width)) : (x >= GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO && x <= 100.0f) || (x >= ((float) (height - 100)) && x <= ((float) height));

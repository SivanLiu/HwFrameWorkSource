...
public class RoamingBroker {
    ...
    private String decryptInfo(Context context, String encryptTag) {
        ...
        if (!"".equals(sensitiveinfo)) {
            try {
                sensitiveinfo = HwAESCryptoUtil.decrypt(HwAllInOneController.MASTER_PASSWORD, sensitiveinfo);
            } catch (Exception ex) {
    ...
    private boolean checkSpecRBState() {
        ...
        try {
            ...
            for (String iccidRbCfg : specIccRBSeqList.trim().split(";")) {
                String[] items = iccidRbCfg.split(":");
                if (items.length >= 2) {

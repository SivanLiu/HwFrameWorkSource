...
public class HwFragmentLayout extends RelativeLayout implements OnTouchListener, HwFragmentFrameLayoutCallback {
    ...
    protected int calculateColumnsNumber(int orientation, int appWidth) {
        ...
        if (this.mSplitMode == 2) {
            ...
            if (appWidth >= dip2px(this.mContext, 592.0f)) {
                return 2;
            }
            return 1;
        } else if (sizeInch < landSeparateSize && Math.abs(sizeInch - landSeparateSize) > 0.1d) {
            return 1;
        } else {
            if (sizeInch - portSeparateSize > 0.1d) {
                if (orientation == 2) {
                    if (appWidth >= dip2px(this.mContext, 592.0f)) {
                        return 2;
                    }
                    return 1;
                } else if (appWidth >= dip2px(this.mContext, 533.0f)) {
                    return 2;
                } else {
                    return 1;
                }

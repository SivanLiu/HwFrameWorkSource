...
public class HwScrollingTabContainerView extends ScrollingTabContainerView {
    ...
    private static final int TAB_DEFAULT_TEXTSIZE = 18;
    private static final int TAB_MIN_TEXTSIZE = 15;
    ...
    private int mMinTabTextSize = 18;
    ...
    private void measureTabView(TabView child, int w, int heightMeasureSpec) {
        ...
        if (child.getCustomView() == null && tv != null) {
            ...
            int newRequiredWidth = requiredWidth;
            if (requiredWidth > w) {
                if (this.mAvailableSpace > 0) {
                ...
                int currentTextSize = this.mMinTabTextSize;
                while (newRequiredWidth > w && currentTextSize > 15) {
                    currentTextSize--;
                    tv.setTextSize(2, (float) currentTextSize);
                    tv.measure(0, heightMeasureSpec);
                    newRequiredWidth = (tv.getMeasuredWidth() + iconSize) + tabPadding;
                }
                if (currentTextSize < this.mMinTabTextSize) {
                    this.mMinTabTextSize = currentTextSize;
                }
                LinearLayout tabLayout = getTabLayout();
                int tabCount = tabLayout.getChildCount();
                for (int i = 0; i < tabCount; i++) {
                    TabView tabView = (TabView) tabLayout.getChildAt(i);
                    if (!(tabView == null || tabView == child)) {
                        TextView tabTv = tabView.getTextView();
                        if (tabTv != null) {
                            tabTv.setTextSize(2, (float) currentTextSize);
                        }
                    }
                }
                if (newRequiredWidth > lp.width) {
            ...
            if (!HwWidgetUtils.isActionbarBackgroundThemed(getContext()) && this.mTabIndicator != null) {
                ...
                if (this.mScrollDirection == 0) {
                    if (x < SCALESIZE - 1.0f) {
                        ...
                        scale = (x - (SCALESIZE - 1.0f)) / (DOUBLE - SCALESIZE);
                        if (this.mRTLFlag) {
                    ...
                    scale = ((DOUBLE - SCALESIZE) - x) / (DOUBLE - SCALESIZE);
                    if (this.mRTLFlag) {

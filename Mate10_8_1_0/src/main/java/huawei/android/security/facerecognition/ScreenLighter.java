...
public class ScreenLighter {
    ...
    private static class BrightnessHandler extends Handler {
        ...
        private static final int BRIGHTNESS_THRESHOLD = SystemProperties.getInt("ro.config.face_rise_light", 30);
        ...
        private SensorEventListener mLightListener = new SensorEventListener() {
            public void onSensorChanged(SensorEvent sensorEvent) {
                ...
                LogUtil.d(BrightnessHandler.TAG, "current lux is " + lux + ", threshold is " + 1.0f);
                synchronized (BrightnessHandler.this.mSensorLock) {
                    if (BrightnessHandler.this.mNeedStartLight && lux < 1.0f) {
        ...
        private int getScreenMode() {
            int mode = 1;
            try {
                mode = System.getInt(this.mContentResolver, "screen_brightness_mode");
    ...
    public ScreenLighter(Context context) {
        this.mContext = context;
    }

...
public class Instrument {
    ...
    private class ProtoStatusReporter implements StatusReporter {
            ...
            if (Instrument.this.protoFile) {
                if (Instrument.this.logPath == null) {
                    ...
                    if (logDir.exists() || logDir.mkdirs()) {
                        ...
                        objArr[Instrument.STATUS_TEST_PASSED] = new SimpleDateFormat("yyyyMMdd-hhmmss-SSS", Locale.US).format(new Date());
                        this.mLog = new File(logDir, String.format("log-%s.instrumentation_data_proto", objArr));
                    } else {
                        PrintStream printStream = System.err;
                        Object[] objArr2 = new Object[1];
                        objArr2[Instrument.STATUS_TEST_PASSED] = logDir.getAbsolutePath();
                        printStream.format("Unable to create log directory: %s\n", objArr2);
                    ...
                    if (!logDir2.exists() && !logDir2.mkdirs()) {
                        ...
                        objArr3[Instrument.STATUS_TEST_PASSED] = logDir2.getAbsolutePath();
                        printStream2.format("Unable to create log directory: %s\n", objArr3);
                        Instrument.this.protoFile = false;
                        return;
                    }
                }
        ...
        public void onInstrumentationStatusLocked(ComponentName name, int resultCode, Bundle results) {
            ...
            if (resultCode == 1) {
                ...
                if (j > 0) {
                    proto.write((long) InstrumentationData.TestStatus.LOGCAT, Instrument.readLogcat(j));
                }
                this.mTestStartMs = 0;
            }
        ...
        public void onInstrumentationFinishedLocked(ComponentName name, int resultCode, Bundle results) {
            ProtoOutputStream proto = new ProtoOutputStream();
            long sessionStatusToken = proto.start((long) InstrumentationData.Session.SESSION_STATUS);
            proto.write((long) InstrumentationData.SessionStatus.STATUS_CODE, (int) Instrument.STATUS_TEST_PASSED);
            proto.write(1172526071811L, resultCode);
            writeBundle(proto, 1146756268036L, results);
        ...
        public void onError(String errorText, boolean commandError) {
            ProtoOutputStream proto = new ProtoOutputStream();
            long sessionStatusToken = proto.start((long) InstrumentationData.Session.SESSION_STATUS);
            proto.write((long) InstrumentationData.SessionStatus.STATUS_CODE, 1);
        ...
        private void writeBundle(ProtoOutputStream proto, long fieldId, Bundle bundle) {
            ...
            for (String key : Instrument.sorted(bundle.keySet())) {
                ...
                proto.write((long) InstrumentationData.ResultsBundleEntry.KEY, key);
                ...
                if (val instanceof String) {
                    ...
                    proto.write((long) InstrumentationData.ResultsBundleEntry.VALUE_DOUBLE, ((Double) val).doubleValue());
                } else if (val instanceof Float) {
                    proto.write((long) InstrumentationData.ResultsBundleEntry.VALUE_FLOAT, ((Float) val).floatValue());
                } else if (val instanceof Integer) {
                    proto.write(1172526071811L, ((Integer) val).intValue());
                } else if (val instanceof Long) {
                    proto.write((long) InstrumentationData.ResultsBundleEntry.VALUE_LONG, ((Long) val).longValue());
                    ...
                    proto.write((long) InstrumentationData.ResultsBundleEntry.VALUE_BYTES, (byte[]) val);
                }
        ...
        private void outputProto(ProtoOutputStream proto) {
            ...
            if (Instrument.this.protoFile) {
                try {
                    ...
                    PrintStream printStream = System.err;
                    Object[] objArr = new Object[1];
                    objArr[Instrument.STATUS_TEST_PASSED] = this.mLog.getAbsolutePath();
                    printStream.format("Cannot write to %s:\n", objArr);
                    ex2.printStackTrace();
                }
            }
        }
    ...
    public static String readLogcat(long startTimeMs) {
        try {
            String timestamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new Date(startTimeMs));
            ProcessBuilder processBuilder = new ProcessBuilder(new String[STATUS_TEST_PASSED]);
            String[] strArr = new String[5];
            strArr[STATUS_TEST_PASSED] = "logcat";
            strArr[1] = "-d";
            strArr[2] = "-v threadtime,uid";
            strArr[3] = "-T";
            strArr[4] = timestamp;
            Process process = processBuilder.command(strArr).start();

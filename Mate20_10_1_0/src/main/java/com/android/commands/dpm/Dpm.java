...
public final class Dpm extends BaseCommand {
    ...
    private static final String COMMAND_GRANT_PO_DEVICE_ID_ACCESS = "grant-profile-owner-device-ids-access";
    ...
    public void onRun() throws Exception {
        ...
        switch (command.hashCode()) {
            case -2077120112:
                if (command.equals(COMMAND_FORCE_NETWORK_LOGS)) {
                    c = 5;
                    break;
                }
                break;
            ...
            case 1325530298:
                ...
                break;
            case 1341700225:
                if (command.equals(COMMAND_GRANT_PO_DEVICE_ID_ACCESS)) {
                    c = 7;
                    break;
                }
                break;
    ...
    private void runForceNetworkLogs() throws RemoteException, InterruptedException {
        while (true) {
            long toWait = this.mDevicePolicyManager.forceNetworkLogs();
            if (toWait == 0) {
    ...
    private void runGrantProfileOwnerDeviceIdsAccess() throws RemoteException {
        parseArgs(false);
        this.mDevicePolicyManager.grantDeviceIdsAccessToProfileOwner(this.mComponent, this.mUserId);
        System.out.println("Success");
    }

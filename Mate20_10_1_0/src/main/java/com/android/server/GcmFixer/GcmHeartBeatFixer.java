    ...
    private static void init(Context content) {
        if (!sHasReceiverRegistered) {
            ...
            BroadcastReceiver backgroundDataReceiver = new BroadcastReceiver() {
                /* class com.android.server.GcmFixer.GcmHeartBeatFixer.AnonymousClass1 */

                public void onReceive(Context receiverContext, Intent intent) {
                    NetworkInfo activeNetworkInfo;
                    if (GcmHeartBeatFixer.ACTION_BACKGROUND_DATA_CHANGED.equals(intent.getAction()) && (activeNetworkInfo = ((ConnectivityManager) receiverContext.getSystemService("connectivity")).getActiveNetworkInfo()) != null && activeNetworkInfo.isConnected()) {

...
public class HwPCMultiWindowManager extends DefaultHwPCMultiWindowManager {
    ...
    private boolean isPadFullscreen(TaskRecordEx recordEx) {
        if (recordEx == null || recordEx.getRootActivityInfo() == null || !HwPCUtils.enabledInPad() || (!this.mPadFullscreenList.contains(recordEx.getRootActivityInfo().packageName) && !this.mPadFullscreenList.contains(recordEx.getRootActivityInfo().name.toLowerCase()))) {
            return false;
        }
        return true;
    }
    ...
    private boolean isFullscreenNoNavigationBar(TaskRecordEx recordEx) {
        if (recordEx != null && recordEx.getRootActivityInfo() != null && this.mFullscreenNoNavigationBar.contains(recordEx.getRootActivityInfo().packageName.toLowerCase())) {
            return true;
        }
        if (recordEx == null || recordEx.getRootActivityInfo() == null) {
            return false;
    ...
    private Rect adjustBounds(Rect rect, int taskId) {
        ...
        int offsetMin = HwPCMultiWindowPolicy.dpToPx(hwPCMultiWindowManager.getRightContext(hwPCMultiWindowManager.mService.getContext()), DP_WINDOW_OVERLAP_OFFSET_MIN);
        int i3 = 30;
    ...
    public void updateTaskByRequestedOrientationInternal(TaskRecordEx recordEx, int customRequestedOrientation) {
        String str;

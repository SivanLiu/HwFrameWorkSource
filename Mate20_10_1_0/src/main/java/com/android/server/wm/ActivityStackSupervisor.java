...
public class ActivityStackSupervisor extends AbsActivityStackSupervisor implements RecentTasks.Callbacks {
    ...
    public void startSpecificActivityLocked(ActivityRecord r, boolean andResume, boolean checkConfig) {
        ...
        if (r.intent.getComponent() == null || !"com.huawei.android.launcher".equals(r.intent.getComponent().getPackageName())) {
    ...
    public boolean isCallerAllowedToLaunchOnDisplay(int callingPid, int callingUid, int launchDisplayId, ActivityInfo aInfo) {
        ...
        if (!activityDisplay.isPrivate()) {
            ...
            return true;
        } else if (!HwPCUtils.isHiCarCastMode() || !DisplayManagerGlobal.getInstance().isUidPresentOnDisplay(callingUid, launchDisplayId)) {
                ...
                if (HwDeviceManager.disallowOp(3, packageName)) {
                    ...
                    UiThread.getHandler().post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (context != null) {
                                Toast toast = Toast.makeText(context, context.getString(33686003), 0);
    ...
    protected final class ActivityStackSupervisorHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == ActivityStackSupervisor.LAUNCH_TIMEOUT_RESUME_MSG) {
                ...
                switch (i) {
                    case ActivityStackSupervisor.IDLE_TIMEOUT_MSG /*{ENCODED_INT: 200}*/:
                        if (ActivityTaskManagerDebugConfig.DEBUG_IDLE) {
                    ...
                    default:
                        switch (i) {
                            case ActivityStackSupervisor.LAUNCH_TASK_BEHIND_COMPLETE /*{ENCODED_INT: 212}*/:
                                synchronized (ActivityStackSupervisor.this.mService.mGlobalLock) {
                                    try {
                                        WindowManagerService.boostPriorityForLockedSection();
    ...
    /* JADX INFO: Multiple debug info for r7v5 java.lang.String: [D('callingPackage' java.lang.String), D('task' com.android.server.wm.TaskRecord)] */
    /* access modifiers changed from: package-private */
    /* JADX WARNING: Removed duplicated region for block: B:118:0x02be A[Catch:{ all -> 0x02c7 }] */
    public int startActivityFromRecents(int callingPid, int callingUid, int taskId, SafeActivityOptions options) {
        ...
        this.mWindowManager.deferSurfaceLayout();
        String str3 = "startActivityFromRecents: homeVisibleInSplitScreen";
        if (windowingMode == 3) {
            try {
        ...
        try {
            ...
            if (task4 != null) {
                if (windowingMode != 3) {
                    try {
                        if (!task4.inHwSplitScreenWindowingMode()) {
                        ...
                        try {
                            ...
                            display = task3.getStack().getDisplay();
                ...
                try {
                    if (!this.mService.mAmInternal.shouldConfirmCredentials(task4.userId)) {
                        try {
                            if (task4.getRootActivity() != null) {
                                ...
                                Flog.i(101, "task.userId =" + task4.userId + ", task.taskId = " + task4.taskId + ", task.getRootActivity() = " + task4.getRootActivity() + ", task.getTopActivity() = " + task4.getTopActivity());
                                this.mActivityMetricsLogger.notifyActivityLaunching(task4.intent);
                                this.mHwActivityStackSupervisorEx.handleFreeFormWindow(task4);
                                try {
                                    try {
                                        i6 = 4;
                                            ...
                                            if (windowingMode == 3) {
                                                try {
                                                    ...
                                                    if (display2.getTopStackInWindowingMode(4).isActivityTypeHome()) {
                                                        display2.moveHomeStackToFront(str2);
                    ...
                    z = false;
                    i = 4;
                    if (windowingMode2 == i2 && task3 != null) {
                        setResizingDuringAnimation(task3);
                        display = task3.getStack().getDisplay();
                        if (display.getTopStackInWindowingMode(i).isActivityTypeHome()) {
                ...
                try {
                    ...
                    try {
                        int startActivityInPackage = this.mService.getActivityStartController().startActivityInPackage(task.mCallingUid, callingPid, callingUid, callingPackage, intent, null, null, null, 0, 0, options, task.userId, task, "startActivityFromRecents", false, null, false);
                        if (windowingMode == 3) {
                            try {

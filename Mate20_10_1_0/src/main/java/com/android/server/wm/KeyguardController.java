...
class KeyguardController {
    ...
    public boolean canShowActivityWhileKeyguardShowing(ActivityRecord r, boolean dismissKeyguard) {
        if (!isDisplayOccluded(r.getDisplayId()) && this.mService.mHwATMSEx.isActivityVisiableInFingerBoost(r)) {
            return true;
        }
        ...
        if (dismissKeyguard && canDismissKeyguard() && !this.mAodShowing) {
            if (this.mDismissalRequested) {
                return true;
            }
            if (!r.canShowWhenLocked() || getDisplay(r.getDisplayId()).mDismissingKeyguardActivity == r) {
                return false;
            }
            return true;
        }

...
public class TaskStack extends WindowContainer<Task> implements BoundsAnimationTarget, ConfigurationContainerListener {
    ...
    private boolean mInHwFreeFormMoveBackState = false;
    ...
    @Override // com.android.server.wm.ConfigurationContainerListener, com.android.server.wm.WindowContainer, com.android.server.wm.ConfigurationContainer
    ...
    private void positionChildAt(int position, Task child, boolean includingParents, boolean showForAllUsers) {
        ...
        super.positionChildAt(targetPosition, (WindowContainer) child, includingParents);
    ...
    public void removeChild(Task task) {
        super.removeChild((WindowContainer) task);
        task.mStack = null;
    ...
    public void updateSurfaceSize(SurfaceControl.Transaction transaction) {
        if (this.mSurfaceControl != null) {
            ...
            if (this.mWmService.getLazyMode() != 0 && inMultiWindowMode() && !HwPCUtils.isPcDynamicStack(this.mStackId)) {
    ...
    public void resetAdjustedForIme(boolean adjustBoundsNow) {
        if (adjustBoundsNow) {
            ...
            if (this.mAdjustedForIme) {
                ...
                updateAdjustedBounds();
                this.mWmService.setResizeDimLayer(false, getWindowingMode(), RING_ANIMATION_DIM_AMOUNT);
                if (inHwSplitScreenWindowingMode()) {
                    this.mActivityStack.mService.applyUpdateSplitBarPos(1);
                    return;
                }
                return;
            }
        ...
        if (imeWin != null) {
            if (dockedTopOrBottom) {
                ...
                if (dockedSide == 2) {
                    ...
                    rect.bottom = (int) ((((float) bottom) * f) + ((RING_ANIMATION_DIM_AMOUNT - f) * ((float) getRawBounds().bottom)));
    ...
    private boolean adjustForMinimizedDockedStack(float minimizeAmount) {
        ...
        if (dockSide == 2) {
            ...
            this.mTmpAdjustedBounds.set(getRawBounds());
            this.mTmpAdjustedBounds.bottom = (int) ((((float) topInset) * minimizeAmount) + ((RING_ANIMATION_DIM_AMOUNT - minimizeAmount) * ((float) getRawBounds().bottom)));
        } else if (dockSide == 1) {
            this.mTmpAdjustedBounds.set(getRawBounds());
            int width = getRawBounds().width();
            this.mTmpAdjustedBounds.right = (int) ((((float) this.mDockedStackMinimizeThickness) * minimizeAmount) + ((RING_ANIMATION_DIM_AMOUNT - minimizeAmount) * ((float) getRawBounds().right)));
            Rect rect = this.mTmpAdjustedBounds;
            rect.left = rect.right - width;
        } else if (dockSide == 3) {
            this.mTmpAdjustedBounds.set(getRawBounds());
            this.mTmpAdjustedBounds.left = (int) ((((float) (getRawBounds().right - this.mDockedStackMinimizeThickness)) * minimizeAmount) + ((RING_ANIMATION_DIM_AMOUNT - minimizeAmount) * ((float) getRawBounds().left)));
        }
        return true;
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:13:0x002a, code lost:
        if (inPinnedWindowingMode() == false) goto L_0x0041;
     */
    ...
    public void onAnimationEnd(boolean schedulePipModeChangedCallback, Rect finalStackSize, boolean moveToFullscreen) {
        if (inPinnedWindowingMode()) {
            ...
            if (this.mAnimationType == 1) {
                setPinnedStackAlpha(RING_ANIMATION_DIM_AMOUNT);
    ...
    public void prepareSurfaces() {
        ...
        if (this.mDimmer.updateDims(getPendingTransaction(), this.mTmpDimBoundsRect)) {
            if (this.mActivityStack.inHwFreeFormWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), (float) this.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472524));
            }
            if (this.mActivityStack.inHwSplitScreenWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), HwActivityManager.IS_PHONE ? 0.0f : (float) this.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472527));
            }
            scheduleAnimation();
        }
    }
    ...
    public boolean setPinnedStackAlpha(float alpha) {
        synchronized (this.mWmService.mGlobalLock) {
            try {
                ...
                if (sc != null) {
                    if (sc.isValid()) {
                        getPendingTransaction().setAlpha(sc, this.mCancelCurrentBoundsAnimation ? RING_ANIMATION_DIM_AMOUNT : alpha);
                        scheduleAnimation();
                        if (!this.mCancelCurrentBoundsAnimation) {
                            z = true;
                        }
    ...
    private void applyDimIfNeed() {
        Task topTask;
        ActivityRecord ar;
        if (HwDisplaySizeUtil.hasSideInScreen() && !isActivityTypeHome() && getWindowingMode() == 1 && (topTask = (Task) getTopChild()) != null) {
            ActivityStack stack = this.mDisplayContent.mAcitvityDisplay.getRealLastFocusedStack();
            boolean isUniHomeLauncher = false;
            if (!(stack == null || !stack.isActivityTypeHome() || (ar = stack.getTopActivity()) == null || ar.mActivityComponent == null || ar.mActivityComponent.getClassName() == null)) {
                isUniHomeLauncher = ar.mActivityComponent.getClassName().equals(WindowManagerConstants.HW_LAUNCHER_COMPONENT_NAME);
            }
            AppWindowToken topApp = topTask.getTopVisibleAppToken();
            if (topApp == null || !isUniHomeLauncher || stack.isTopActivityVisible()) {
                this.mDimmer.dontAnimateExit();
            } else if (shouldShowDimLayer(topApp)) {
                this.mDimmer.dimBelow(getPendingTransaction(), this, RING_ANIMATION_DIM_AMOUNT);
            } else {
                this.mDimmer.dontAnimateExit();
            }
        }
    ...
    private boolean shouldShowDimLayer(AppWindowToken topApp) {
        WindowState win;
        String pkgName = topApp.appPackageName;
        if (pkgName == null || (win = topApp.findMainWindow(false)) == null) {
            return false;
        }
        return this.mWmService.isNeedExceptDisplaySide(win, pkgName);
    }
    ...
    /* access modifiers changed from: package-private */
    public void setInHwFreeFormMoveBackState(boolean state) {
        this.mInHwFreeFormMoveBackState = state;
    }

    /* access modifiers changed from: package-private */
    public boolean inHwFreeFormMoveBackState() {
        return this.mInHwFreeFormMoveBackState;
    }
}

...
public class HwActivityStackSupervisorEx implements IHwActivityStackSupervisorEx {
        ...
        while (!stacks.isEmpty()) {
            ...
            if (tr != null) {
                ...
                if (tr instanceof HwTaskRecord) {
                    ArrayList<ActivityRecord> activities = ((HwTaskRecord) tr).getActivities();
                    if (HwPCUtils.isHiCarCastMode()) {
                        int activityNdx = activities.size() - 1;
                        while (true) {
                            if (activityNdx < 0) {
                                break;
                            } else if (activities.get(activityNdx).packageName.equals("com.android.incallui")) {
                                needRestartIntentList.add(tr.intent);
                                isNeedRestart = true;
                                break;
                            } else {
                                activityNdx--;
                    ...
                    removeProcessesActivityNotFinished(activities, procs);
                }
                if (tr.intent != null && !stack.inPinnedWindowingMode() && !isNeedRestart) {
                    if (!HwPCUtils.isHiCarCastMode()) {
    ...
    private void removeProcessesActivityNotFinished(ArrayList<ActivityRecord> activities, ArrayList<WindowProcessController> procs) {
        for (int activityNdx = activities.size() - 1; activityNdx >= 0; activityNdx--) {
            ...
            while (true) {
                if (i >= NP) {
                    break;
                } else if (!"com.huawei.hidisk".equals(procs.get(i).mName) && procs.get(i).mName.equals(r.processName)) {
                    procs.remove(i);
                    break;
                } else {
                    i++;
    ...
    public void handleFreeFormWindow(TaskRecord task) {
        if (HwFreeFormUtils.isFreeFormEnable()) {
            ...
            boolean isNull = true;
            ActivityStack freeFormStack = activityDisplay == null ? null : activityDisplay.getStack(5, 1);

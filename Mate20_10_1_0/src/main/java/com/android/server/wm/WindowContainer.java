...
class WindowContainer<E extends WindowContainer> extends ConfigurationContainer<E> implements Comparable<WindowContainer>, SurfaceAnimator.Animatable {
    ...
    public final Pools.SynchronizedPool<ForAllWindowsConsumerWrapper> mConsumerWrapperPool = new Pools.SynchronizedPool<>(3);
    ...
    WindowContainer(WindowManagerService wms) {
        ...
        this.mWindowManager = (WindowManager) this.mWmService.mContext.getSystemService("window");
    }
    ...
    public void updateSurfacePosition() {
        if (this.mSurfaceControl != null) {
            ...
            if ((this instanceof TaskStack) && inMultiWindowMode() && !HwPCUtils.isPcDynamicStack(((TaskStack) this).mStackId)) {
                ...
                float pendingX = 0.0f;
                float pendingY = 0.0f;
                if (this.mWmService.getLazyMode() == 1) {
                    ratio = this.mLazyScale;
                    pendingY = ((float) this.mHeight) * (1.0f - this.mLazyScale);
                    if (HwDisplaySizeUtil.hasSideInScreen()) {
                        pendingX = 0.0f + ((float) HwDisplaySizeUtil.getInstance(this.mWmService).getSafeSideWidth());
                    }
                } else if (this.mWmService.getLazyMode() == 2) {
                    ratio = this.mLazyScale;
                    float f = this.mLazyScale;
                    pendingX = ((float) this.mWidth) * (1.0f - f);
                    pendingY = ((float) this.mHeight) * (1.0f - f);
                    if (HwDisplaySizeUtil.hasSideInScreen()) {
                        pendingX -= (float) HwDisplaySizeUtil.getInstance(this.mWmService).getSafeSideWidth();
                    }
                }
                Point point = this.mTmpPos;
                point.x = (int) ((((float) point.x) * ratio) + pendingX);
                Point point2 = this.mTmpPos;
                point2.y = (int) ((((float) point2.y) * ratio) + pendingY);
            }
    ...
    private void updatedisplayinfo() {
        ...
        DisplayInfo displayInfo = this.mDefaultDisplayInfo;
        this.mWidth = isPortrait ? displayInfo.logicalWidth : displayInfo.logicalHeight;
        DisplayInfo displayInfo2 = this.mDefaultDisplayInfo;
        this.mHeight = isPortrait ? displayInfo2.logicalHeight : displayInfo2.logicalWidth;
    }
    ...
    public void getRelativeDisplayedPosition(Point outPos) {
        ...
        if (parent != null) {
            ...
            if (HwFoldScreenState.isFoldScreenDevice() && this.mWmService.isInSubFoldScaleMode() && !inHwPCFreeformWindowingMode() && ((this instanceof AppWindowToken) || (this instanceof Task) || (this instanceof TaskStack))) {

...
public class HwTaskLaunchParamsModifierEx implements IHwTaskLaunchParamsModifierEx {
    ...
    public HashMap<String, Integer> computeDefaultParaForFreeForm(ActivityDisplay activityDisplay, Context context) {
        ...
        if (activityDisplay == null || context == null) {
            HwFreeFormUtils.log(TAG, "computeDefaultParaForFreeForm input null.");
            return DEFAULT_FREEFORM_MAPS;
        }
        sDisplayMetrics = new DisplayMetrics();
        activityDisplay.mDisplay.getRealMetrics(sDisplayMetrics);
        sDisplayInfo = new DisplayInfo();
        activityDisplay.mDisplay.getDisplayInfo(sDisplayInfo);
        int navbarHeight = context.getResources().getDimensionPixelSize(17105305);
        int navbarWidth = context.getResources().getDimensionPixelSize(17105310);
    ...
    private static void getDefaultBounds(int width, int height, int navbarHeight, int navbarWidth) {
        if (!IS_TABLET) {
            if (width < height) {
                ...
                sDefaultFreeformStartY = (((height - ((int) (((float) width) * 0.85f))) - dip2px(16)) - navbarHeight) - sNotchSize;
                sDefaultFreeformWidth = (int) (((float) width) * DEFAULT_WINDTH_RATION_MOBILE);
                sDefaultFreeformHeight = (int) (((float) width) * 0.85f);
                return;
            }
            sDefaultFreeformStartX = (((width - ((int) (((float) height) * 0.85f))) - dip2px(8)) - navbarWidth) - sNotchSize;
            sDefaultFreeformStartY = ((int) (((float) height) * 0.19999999f)) - dip2px(16);
            sDefaultFreeformWidth = (int) (((float) height) * 0.85f);

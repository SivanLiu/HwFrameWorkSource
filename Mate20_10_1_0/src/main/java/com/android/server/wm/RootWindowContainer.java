...
class RootWindowContainer extends WindowContainer<DisplayContent> implements ConfigurationContainerListener {
    ...
    /* JADX WARN: Type inference failed for: r0v19, types: [android.util.SparseIntArray] */
        ...
        try {
            ...
            if (!leakedSurface) {
                ...
                while (displayNdx2 < numDisplays) {
                    try {
                        ((DisplayContent) this.mChildren.get(displayNdx2)).forAllWindows(new Consumer(sparseIntArray) {
                        ...
                        if (sparseIntArray.size() > 0) {
                            int[] pids = new int[sparseIntArray.size()];
                            for (int i = z; i < pids.length; i++) {
                                pids[i] = sparseIntArray.keyAt(i);
                            }
                            try {
                                try {

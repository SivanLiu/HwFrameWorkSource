...
class ScreenRotationAnimation {
        ...
        try {
            try {
                ...
                if (!HwFoldScreenState.isFoldScreenDevice()) {
                    gb = ((WindowManagerService) this.mService).mDisplayManagerInternal.screenshot(displayId);
                } else {
                    gb = null;
                    Rect sourceCrop = createSourceCropForFoldScreen();
                    IBinder displayToken = this.mService.mDisplayManagerInternal.getDisplayToken(displayId);
                    if (displayToken != null) {
                        gb = SurfaceControl.screenshotToBufferWithSecureLayersUnsafe(displayToken, sourceCrop, this.mWidth, this.mHeight, false, 0);
                    } else {
                        Slog.w(TAG, "displayToken is null,displayId:" + displayId);
                    }
                    Slog.d(TAG, "Create sourceCrop:" + sourceCrop + ", mWidth:" + this.mWidth + ", mHeight:" + this.mHeight);
                }
                if (gb != null) {
                    try {
    ...
    private boolean startAnimation(SurfaceControl.Transaction t, long maxAnimationDuration, float animationScale, int finalWidth, int finalHeight, boolean dismissing, int exitAnim, int enterAnim) {
        ...
        if (exitAnim == 0 || enterAnim == 0) {
            customAnim = false;
            try {
                hwextContext = this.mContext.createPackageContext(HWEXT_STR_CONST, 0);
            ...
            if (hwextContext != null) {
                if (this.mUsingFoldAnim) {
                    ...
                    int rotateExitAnimationId = hwextContext.getResources().getIdentifier(SCREEN_ROTATE_STR_CONST + delta + EXIT_STR_CONST, ANIM_STR_CONST, HWEXT_STR_CONST);
                    int rotateEnterAnimationId = hwextContext.getResources().getIdentifier(SCREEN_ROTATE_STR_CONST + delta + ENTER_STR_CONST, ANIM_STR_CONST, HWEXT_STR_CONST);
    ...
    private Rect createSourceCropForFoldScreen() {
        ...
        sourceCrop.left = (this.mService.isScreenFolding() ? this.mService.getFromFoldDisplayMode() : this.mService.getFoldDisplayMode()) == 2 ? sFullFoldDisplayModeRect.width() - sMainFoldDisplayModeRect.width() : 0;
        sourceCrop.top = 0;

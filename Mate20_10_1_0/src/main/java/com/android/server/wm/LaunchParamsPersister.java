...
class LaunchParamsPersister {
    ...
    private void loadLaunchParams(int userId) {
        ...
        File[] paramsFiles;
        Set<String> packages2;
        File launchParamsFolder2;
        List<File> filesToDelete = new ArrayList<>();
        File launchParamsFolder3 = getLaunchParamFolder(userId);
        if (!launchParamsFolder3.isDirectory()) {
            Slog.i(TAG, "Didn't find launch param folder for user " + userId);
            return;
        }
        Set<String> packages3 = new ArraySet<>(this.mPackageList.getPackageNames());
        File[] paramsFiles2 = launchParamsFolder3.listFiles();
        ...
        while (i2 < length) {
            ...
            if (!paramsFile.isFile()) {
                ...
                if (name == null) {
                    ...
                    packages2 = packages;
                    paramsFiles = paramsFiles2;
                } else if (!packages.contains(name.getPackageName())) {
                    filesToDelete.add(paramsFile);
                    launchParamsFolder2 = launchParamsFolder;
                    packages2 = packages;
                    paramsFiles = paramsFiles2;
                                            ...
                                            if (!"launch_params".equals(tagName)) {
                                                ...
                                                try {
                                                    ...
                                                    try {
                                                        ...
                                                        i2++;
                                                        launchParamsFolder3 = launchParamsFolder2;
                                                        packages3 = packages2;
                                                        paramsFiles2 = paramsFiles;
                                                        i = 0;
                                                    } catch (Throwable th) {
        ...
        synchronized (this.mPersisterQueue) {
            this.mPersisterQueue.removeItems(new Predicate(packageName) {
                ...
                public final boolean test(Object obj) {
                    return ((LaunchParamsWriteQueueItem) obj).mComponentName.getPackageName().equals(this.f$0);
                }

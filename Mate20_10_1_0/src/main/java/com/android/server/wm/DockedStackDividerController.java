...
public class DockedStackDividerController {
    ...
    private void loadDimens() {
        ...
        this.mTaskHeightInMinimizedMode = context.getResources().getDimensionPixelSize(17105453);
    ...
    private void notifyDockedStackMinimizedChanged(boolean minimizedDock, boolean animate, boolean isHomeStackResizable) {
        ...
        if (animate) {
            ...
            if (isAnimationMaximizing()) {
                ...
                transitionDuration = IME_ADJUST_DRAWN_TIMEOUT;
            }
    ...
    private void startImeAdjustAnimation(boolean adjustedForIme, boolean adjustedForDivider, WindowState imeWin) {
        boolean z = this.mAnimatingForIme;
        float f = CLIP_REVEAL_MEET_LAST;
        if (!z) {
        ...
        notifyAdjustedForImeChanged(z2, IME_ADJUST_ANIM_DURATION);
    }
    ...
    private boolean animateForIme(long now) {
        ...
        float t = (this.mAnimationTarget == CLIP_REVEAL_MEET_LAST ? IME_ADJUST_ENTRY_INTERPOLATOR : AppTransition.TOUCH_RESPONSE_INTERPOLATOR).getInterpolation(Math.min((float) CLIP_REVEAL_MEET_LAST, ((float) (now - this.mAnimationStartTime)) / ((float) this.mAnimationDuration)));
    ...
    private boolean animateForMinimizedDockedStack(long now) {
        ...
        float t = (isAnimationMaximizing() ? AppTransition.TOUCH_RESPONSE_INTERPOLATOR : this.mMinimizedDockInterpolator).getInterpolation(Math.min((float) CLIP_REVEAL_MEET_LAST, ((float) (now - this.mAnimationStartTime)) / ((float) this.mAnimationDuration)));
    ...
    private float adjustMaximizeAmount(TaskStack stack, float t, float naturalAmount) {
        ...
        float amountPrime = (this.mAnimationTarget * t) + ((CLIP_REVEAL_MEET_LAST - t) * (((float) this.mDisplayContent.mAppTransition.getLastClipRevealMaxTranslation()) / ((float) stack.getMinimizeDistance())));
        float t2 = Math.min(t / this.mMaximizeMeetFraction, (float) CLIP_REVEAL_MEET_LAST);
    ...
    private float getClipRevealMeetFraction(TaskStack stack) {
        ...
        return ((CLIP_REVEAL_MEET_LAST - Math.max(0.0f, Math.min((float) CLIP_REVEAL_MEET_LAST, ((((float) Math.abs(this.mDisplayContent.mAppTransition.getLastClipRevealMaxTranslation())) / ((float) stack.getMinimizeDistance())) - CLIP_REVEAL_MEET_FRACTION_MIN) / CLIP_REVEAL_MEET_FRACTION_MIN))) * 0.39999998f) + CLIP_REVEAL_MEET_EARLIEST;
    }

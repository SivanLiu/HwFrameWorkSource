...
class RootActivityContainer extends ConfigurationContainer implements DisplayManager.DisplayListener, IHwRootActivityContainerInner {
    ...
    public boolean resumeFocusedStacksTopActivities(ActivityStack targetStack, ActivityRecord target, ActivityOptions targetOptions) {
        ...
        if (this.mStackSupervisor.readyToResume()) {
            ...
            if (targetStack != null && ((targetStack.isTopStackOnDisplay() || getTopDisplayFocusedStack() == targetStack) && !this.mHwRacEx.resumeAppLockActivityIfNeeded(targetStack, targetOptions))) {
            ...
            for (int displayNdx = this.mActivityDisplays.size() - 1; displayNdx >= 0; displayNdx--) {
                ...
                for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; stackNdx--) {
                    ...
                    if (stack.isFocusableAndVisible() && topRunningActivity != null) {
                        if (stack == targetStack) {
                            ...
                            if (!topRunningActivity.shouldResumeActivity(target) || !this.mHwRacEx.resumeAppLockActivityIfNeeded(stack, targetOptions)) {
                ...
                if (!resumedOnDisplay && (focusedStack = display.getFocusedStack()) != null && !this.mHwRacEx.resumeAppLockActivityIfNeeded(focusedStack, targetOptions)) {
    ...
    /* JADX WARN: Type inference failed for: r0v16, types: [com.android.server.wm.ActivityStack] */
    /* access modifiers changed from: package-private */
    ...
    public <T extends ActivityStack> T getLaunchStack(ActivityRecord r, ActivityOptions options, TaskRecord candidateTask, boolean onTop, LaunchParamsController.LaunchParams launchParams) {
        ...
        if (displayId != -1 && canLaunchOnDisplay(r, displayId)) {
            ...
            if (r != null) {
                ...
                if (stack3 != null) {
                    if (HwFreeFormUtils.isFreeFormEnable() && stack3.inFreeformWindowingMode()) {
                        if (getTopResumedActivity() != null) {
                            pkgUnderFreeForm = getTopResumedActivity().packageName;
                        }
                        stack3.setCurrentPkgUnderFreeForm(pkgUnderFreeForm);
                        stack3.setFreeFormStackVisible(true);
            ...
            if (!(display2 == null || (stack2 = (T) display2.getOrCreateStack(r, options, candidateTask, activityType, onTop)) == null)) {
                if (HwFreeFormUtils.isFreeFormEnable() && stack2.inFreeformWindowingMode()) {
                    if (getTopResumedActivity() != null) {
                        pkgUnderFreeForm = getTopResumedActivity().packageName;
                    }
                    stack2.setCurrentPkgUnderFreeForm(pkgUnderFreeForm);
                    stack2.setFreeFormStackVisible(z);
                    HwFreeFormUtils.log(str, "Start freeform above " + pkgUnderFreeForm);
                }
                return stack2;
            }
        ...
        T stack4 = (activityStack != null || r == null) ? (T) activityStack : r.getActivityStack();
        if (!(stack == null || (display3 = stack.getDisplay()) == null || !canLaunchOnDisplay(r, display3.mDisplayId))) {
            ...
            if (windowingMode == 4 && display3.getSplitScreenPrimaryStack() == stack && candidateTask == stack.topTask()) {
                return stack;
            }
        }
    ...
    private void calculateDefaultMinimalSizeOfResizeableTasks() {
        ...
        this.mDefaultMinSizeOfResizeableTaskDp = (int) (res.getDimension(17105123) / res.getDisplayMetrics().density);
    }

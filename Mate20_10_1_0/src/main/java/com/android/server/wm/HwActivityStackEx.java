...
public class HwActivityStackEx implements IHwActivityStackEx {
    ...
    private boolean mHiddenFromHome = false;
            ...
            synchronized (this.mService.mGlobalLock) {
                ...
                if (!this.mService.mVrMananger.isVRDynamicStack(this.mStack.getStackId())) {
                    if (visible) {
                        ...
                        this.mService.mHwATMSEx.updateUsageStatsForPCMode(this.mStack.getTopActivity(), visible, this.mService.mUsageStatsInternal);
                    }
                }
            }
        }
    ...
    public void resetOtherStacksVisible(boolean visible) {
        if (this.mStack != null && HwPCUtils.enabledInPad() && HwPCUtils.isPcDynamicStack(this.mStack.getStackId()) && HwPCUtils.isPcCastModeInServer() && hasFullscreenTaskInPad()) {
            makeStackVisibleInPad(visible, this.mStack.mDisplayId, this.mStack.getStackId());
        }
    }
    ...
    private void makeStackVisibleInPad(boolean visible, int displayId, int currentStackId) {
        ...
        for (int stackNdx = activityDisplay.getChildCount() - 1; stackNdx >= 0; stackNdx--) {
            ...
            if (stack.getStackId() != currentStackId) {
                if (!visible || (stack.mHwActivityStackEx.getStackVisibleBeforeHidden() && !stack.mHwActivityStackEx.isInCallActivityStack())) {
                    if (!visible && stack.mHwActivityStackEx.getStackVisible()) {
                        HwPCUtils.log(TAG, "makeStackVisibleInPad stack=" + stack + " make invisible because the top activity is fullscreen ,mStackVisibleBeforeHidden=" + stack.mHwActivityStackEx.getStackVisibleBeforeHidden());
                        stack.mHwActivityStackEx.setStackVisibleBeforeHidden(true);
                    }
                    stack.mHwActivityStackEx.makeStackVisible(visible);
                    if (visible) {
                        stack.mHwActivityStackEx.setStackVisibleBeforeHidden(false);
                    }
                } else {
                    HwPCUtils.log(TAG, "makeStackVisibleInPad stack=" + stack + " Skipping: is invisible before launch fullscreen activity or this stack contains InCallUI activity ,mStackVisibleBeforeHidden=" + stack.mHwActivityStackEx.getStackVisibleBeforeHidden());
                }
            }
        }
    }
    ...
    public void setStackVisibleBeforeHidden(boolean visible) {
    ...
    public boolean getStackVisible() {
        return this.mStackVisible;
    }

    public boolean getHiddenFromHome() {
        return this.mHiddenFromHome;
    }

    public void setHiddenFromHome(boolean hidden) {
        this.mHiddenFromHome = hidden;
    }
    ...
    public boolean shouldSkipPausing(ActivityRecord resumed, ActivityRecord resuming, int staskId) {
        ...
        } else if (!resumed.inCoordinationPrimaryWindowingMode() || !resuming.inCoordinationSecondaryWindowingMode()) {
            return false;
        } else {
            return true;
        }

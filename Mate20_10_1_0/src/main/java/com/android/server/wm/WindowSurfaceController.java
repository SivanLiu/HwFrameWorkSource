...
public class WindowSurfaceController {
        ...
        if (this.mSurfaceControl != null) {
            ...
            try {
                ...
                synchronized (this.mAnimator.mInsetSurfaceLock) {
                    if (!this.mAnimator.mIsWallpaper && this.mAnimator.mSplitScreenSideSurfaceBox != null) {
                        this.mAnimator.mSplitScreenSideSurfaceBox.hide(transaction);
                    }
                    if (this.mAnimator.mIsWallpaper && !this.mAnimator.mWin.mIsNeedToShowSurfaceBox && this.mAnimator.mSplitScreenSideSurfaceBox != null) {
                        this.mAnimator.mSplitScreenSideSurfaceBox.hide(transaction);
                        WindowState focusedWindow = this.mService.getFocusedWindow();
                        if (focusedWindow != null && !isExpandInputMethodWindowShow()) {
                            Slog.v(TAG, "wallPaper hideSurface. global gesture case call notchControlFilletForSideScreen by pass focusedWindow = " + focusedWindow);
                            WindowManagerPolicy policy = this.mService.getPolicy();
                            if (policy != null && (policy instanceof PhoneWindowManager)) {
                                ((PhoneWindowManager) policy).notchControlFilletForSideScreen(focusedWindow, true);
                            }
                        }
                    }
                }
    ...
    public void destroyNotInTransaction() {
        ...
        try {
            ...
            synchronized (this.mAnimator.mInsetSurfaceLock) {
                if (!this.mAnimator.mIsWallpaper && this.mAnimator.mSplitScreenSideSurfaceBox != null) {
                    this.mAnimator.mSplitScreenSideSurfaceBox.destroy();
                    this.mAnimator.mSplitScreenSideSurfaceBox = null;
                }
            }
        } catch (RuntimeException e) {
            try {
    ...
    private boolean showSurface() {
        TaskStack primaryStack;
        Task topTask;
        try {
            synchronized (this.mAnimator.mInsetSurfaceLock) {
                if (this.mAnimator.mIsWallpaper && this.mAnimator.mWin.mIsNeedToShowSurfaceBox && this.mAnimator.mSplitScreenSideSurfaceBox != null) {
                    this.mAnimator.mSplitScreenSideSurfaceBox.show();
                }
                if (!(this.mAnimator.mIsWallpaper || this.mAnimator.mSplitScreenSideSurfaceBox == null || (primaryStack = this.mService.getDefaultDisplayContentLocked().getSplitScreenPrimaryStack()) == null || (topTask = (Task) primaryStack.getTopChild()) == null)) {
                    ...
                    if (!(displayPolicy == null || primaryWindow == null)) {
                        int rotation = displayPolicy.getDisplayRotation();
                        if (displayPolicy.getHwDisplayPolicyEx() != null && displayPolicy.getHwDisplayPolicyEx().isNeedExceptDisplaySide(primaryWindow.getAttrs(), primaryWindow, rotation)) {
                            Slog.i(TAG, "dock divider showSurface. show split screen side surface");
                            this.mAnimator.mSplitScreenSideSurfaceBox.show();
                        }
                    }
                }
    ...
    public void setWindowIconInfo(int iconType, int iconViewWidth, int iconViewHeight, Bitmap icon) {
        if (this.mSurfaceControl == null) {
            ...
            this.mSurfaceControl.setWindowIconInfo(0, 0, 0, new byte[1], 1, 0, 0);
        } else {
            ...
            this.mSurfaceControl.setWindowIconInfo(iconType, iconViewWidth, iconViewHeight, byteBuffer.array(), byteCount, width, height);
        }
    }
}

    ...
    private void addPackagesToSet(TreeMap<String, PackageInfo> set, List<String> pkgNames) {
        for (String pkgName : pkgNames) {
            if (!set.containsKey(pkgName)) {
                try {
                    set.put(pkgName, this.mUserBackupManagerService.getPackageManager().getPackageInfo(pkgName, 134217728));
                                ...
                                try {
                                    ...
                                    if (out != null) {
                                        try {
                                            ...
                                            Slog.e(BackupManagerService.TAG, "IO error closing adb backup file: " + e21.getMessage());
                                        }
                                    }
                                    this.mOutputFile.close();
                                    synchronized (this.mLatch) {

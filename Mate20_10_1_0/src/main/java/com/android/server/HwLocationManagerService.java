...
public class HwLocationManagerService extends LocationManagerService {
    ...
    private static Map<Integer, Integer> mSettingsByUserIds = new HashMap(16);
    ...
    public boolean isLbsServiceStart = false;
    ...
    private HashMap<String, ArrayList<UpdateRecord>> mPreservedRecordsByPkgs = new HashMap<>(16);
    ...
    public HwLocationManagerService(Context context) {
        ...
        Object mHwActivityManagerObject = this.mHwContext.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG);
    ...
    private void sendMessageQuickLocation(Message msg) {
        ...
        ArrayList<LocationProvider> locationProviderManagers = this.mLocationManagerServiceUtil.getRealProviders();
        if (locationProviderManagers == null) {
            LBSLog.e(TAG, false, "locationProviderManagers is null", new Object[0]);
            return;
        }
        Iterator<LocationProvider> it = locationProviderManagers.iterator();
        while (true) {
            if (!it.hasNext()) {
                break;
            }
            LocationProvider providerManager = it.next();
    ...
    public Location screenLocationLocked(Location location, String provider) {
        ...
        LocationProvider providerProxy = null;
        Object locationProviderObject = this.mLocationManagerServiceUtil.getProvidersByName().get("network");
        if (locationProviderObject != null && (locationProviderObject instanceof LocationProvider)) {
            providerProxy = (LocationProvider) locationProviderObject;
        }
        ...
        if (!extras.containsKey(this.mComboNlpReadyMarker)) {
            ArrayList<UpdateRecord> records = this.mLocationManagerServiceUtil.getRecordsByProvider().get("passive");
            if (records == null) {
                return location;
            }
    ...
    private boolean callLocationChanged(Location location, Bundle extras, ArrayList<UpdateRecord> records) {
    ...
    private void updateProviderInControl(String packageName, int uid, String expectedModeEx) {
        HashMap<UpdateRecord, String> switchRecords = new HashMap<>(16);
    ...
    private void updateSwitchRecords(HashMap<Object, Receiver> receivers, HashMap<UpdateRecord, String> switchRecords, int uid, String expectedModeEx, String packageName) {
        if (receivers != null) {
    ...
    private void updateProvider(HashMap<UpdateRecord, String> switchRecords, String packageName, int uid) {
        ...
        ArrayList<UpdateRecord> preservedRecords = this.mPreservedRecordsByPkgs.get(packageName);
    ...
    private HashMap<UpdateRecord, String> getRemoveSwitchRecords(String packageName) {
        ...
        HashMap<UpdateRecord, String> mSwitchRecords = new HashMap<>(16);
        if (mReceivers != null) {
    ...
    private ArrayList<String> updateLocationMode(HashMap<UpdateRecord, String> switchRecords) {
        if (switchRecords.size() == 0) {
            LBSLog.i(TAG, false, " package has no request send yet", new Object[0]);
            return null;
        }
        HashMap<String, ArrayList<UpdateRecord>> recordsByProviders = this.mLocationManagerServiceUtil.getRecordsByProvider();
        int i = 16;
        ArrayList<String> updateproviders = new ArrayList<>(16);
        for (Map.Entry<UpdateRecord, String> entry : switchRecords.entrySet()) {
            ...
            Receiver receiver = entry.getKey().mReceiver;
            UpdateRecord oldRecord = (UpdateRecord) entry.getKey().mReceiver.mUpdateRecords.put(target, entry.getKey());
            if (oldRecord != null) {
                ArrayList<UpdateRecord> oldRecords = this.mPreservedRecordsByPkgs.get(oldRecord.mReceiver.mCallerIdentity.mPackageName);
                if (oldRecords == null) {
    ...
    private void hwRemoveSwitchUpdates(Receiver receiver) {
        synchronized (this.mLocationManagerServiceUtil.getmLock()) {
            ArrayList<UpdateRecord> preservedRecords = this.mPreservedRecordsByPkgs.get(receiver.mCallerIdentity.mPackageName);
            if (preservedRecords != null) {
                Iterator<UpdateRecord> it = preservedRecords.iterator();
                while (it.hasNext()) {
                    UpdateRecord record = it.next();
    ...
    private int checkLocationSettings(int userId, String name, String value) {
        if (!hasAccessHwLmsPermission(Binder.getCallingPid(), Binder.getCallingUid())) {
            LBSLog.i(TAG, false, "checkLocationSettings has no hw access_location_service permission!", new Object[0]);
            return -1;
        } else if (!isChineseVersion()) {
            return -1;
        } else {
            if ("location_mode".equals(name)) {
                LBSLog.i(TAG, false, "LOCATION_MODE, name: %{public}s, value: %{public}s", name, value);
                if (LOCATION_MODE_SENSORS_ONLY.equals(value) || LOCATION_MODE_BATTERY_SAVING.equals(value)) {
                    mSettingsByUserIds.put(Integer.valueOf(userId), 2);
                    return 3;
                ...
                LBSLog.i(TAG, false, "LOCATION_PROVIDERS_ALLOWED, name: %{public}s, value: %{public}s", name, value);
                if (mSettingsByUserIds.containsKey(Integer.valueOf(userId))) {
                    int cnt = mSettingsByUserIds.get(Integer.valueOf(userId)).intValue();
                    if (cnt <= 0) {
                        return -1;
                    }
                    int cnt2 = cnt - 1;
                    if (cnt2 == 0) {
                        LBSLog.i(TAG, false, "clear mSettingsByUserIds, cnt: %{public}d", Integer.valueOf(cnt2));
                        mSettingsByUserIds.clear();
                        return -1;
                    }
                    mSettingsByUserIds.put(Integer.valueOf(userId), Integer.valueOf(cnt2));
                    return -1;
    ...
    private int getPowerType(String packageName) {
        HashMap<String, UpdateRecord> updateRecords;
        int powerTypeRet = -1;
        for (Receiver receiver : this.mLocationManagerServiceUtil.getReceivers().values()) {
    ...
    public class LbsServiceDeathHandler implements ServiceConnection, IBinder.DeathRecipient {
        ...
        public void onServiceConnected(ComponentName arg0, IBinder service) {
            ...
            boolean unused2 = HwLocationManagerService.this.isLbsServiceStart = true;
        ...
        public void onServiceDisconnected(ComponentName name) {
            ...
            boolean unused2 = HwLocationManagerService.this.isLbsServiceStart = false;
        }
    ...
    public void sendLbsServiceRestartMessage() {
        ...
        if (!this.mLbsServiceHandler.hasMessages(26)) {
            ...
            delayTime = new Double(Math.pow(2.0d, (double) this.mServiceRestartCount)).longValue() * 60000;
            if (delayTime > 0) {
    ...
    public void hwRemoveUpdatesLocked(Receiver receiver) {
        ...
        while (it.hasNext()) {
            providers = providers + ((UpdateRecord) it.next()).mRealRequest.getProvider() + ",";
        }
        ...
        LogPower.push(203, receiver.mCallerIdentity.mPackageName, Integer.toString(receiver.mCallerIdentity.mPid), Integer.toString(-1), new String[]{Integer.toHexString(System.identityHashCode(receiver))});
        if (this.mHwLbsConfigManager.isEnableForParam(LbsConfigContent.CONFIG_NLP_MEMORY_ENABLE)) {
    ...
    public void initializeLockedEx() {
        ...
        if (HwLocalLocationProvider.isLocalDBEnabled()) {
            LocationProvider localLocationProviderManager = new LocationProvider(this, HwLocalLocationManager.LOCAL_PROVIDER);
    ...
    private void refreshProvider(String providerName, String packageName) {
        ...
        ArrayList<UpdateRecord> records = this.mLocationManagerServiceUtil.getRecordsByProvider().get(providerName);
        if (records != null) {
            Iterator<UpdateRecord> it = records.iterator();
            while (true) {
                if (it.hasNext()) {
    ...
    private <TListener extends IInterface> void refreshMeasurementsProvider(ArrayMap<IBinder, ? extends LinkedListener> gnssDataListeners, RemoteListenerHelper<TListener> gnssDataProvider, Function<IBinder, TListener> mapBinderToListener, String packageName) {
        if (gnssDataListeners != null && gnssDataProvider != null) {
            for (Map.Entry<IBinder, ? extends LinkedListener> entry : gnssDataListeners.entrySet()) {
                CallerIdentity callerIdentity = ((LinkedListener) entry.getValue()).mCallerIdentity;
    ...
    private class MemoryTimerTask extends TimerTask {
    ...
    private void startMemoryMonitor(Receiver receiver) {
        int networkSize;
        int gpsSize;
        synchronized (MEMORY_LOCK) {
            if (!this.isTimerRunning) {
                HashMap<String, ArrayList<UpdateRecord>> recordsMap = this.mLocationManagerServiceUtil.getRecordsByProvider();
                if (recordsMap != null) {
                    ArrayList<UpdateRecord> networkRecords = recordsMap.get("network");
                    ArrayList<UpdateRecord> gpsRecords = recordsMap.get("gps");
    ...
    private boolean isSameReceiver(Receiver receiverOne, Receiver receiverTwo) {
    ...
    private boolean isExistRecords(int networkSize, int gpsSize, List<UpdateRecord> networkRecords, List<UpdateRecord> gpsRecords, Receiver receiver) {
    ...
    private boolean isMockProviderExist() {
        synchronized (this.mLocationManagerServiceUtil.getmLock()) {
            Iterator<LocationProvider> it = this.mLocationManagerServiceUtil.getProviders().iterator();
            while (it.hasNext()) {
                if (it.next().isMock()) {
                    return true;
                }
    ...
    /* access modifiers changed from: private */
    public void releaseAppMemory(String packageName) {
    ...
    private void removeMockByName(String name) {
        LocationProvider provider = this.mLocationManagerServiceUtil.getLocationProviderLocked(name);
        if (provider != null && provider.isMock()) {
            ...
            Iterator<LocationProvider> it = this.mLocationManagerServiceUtil.getRealProviders().iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
                LocationProvider isMockProvider = it.next();
    ...
    private void removeMockTest(String type, String opPackageName) {
        synchronized (this.mLocationManagerServiceUtil.getmLock()) {
            ...
            try {
                ArrayList<LocationProvider> providers = this.mLocationManagerServiceUtil.getProviders();
                ArrayList<String> providerNames = new ArrayList<>();
                Iterator<LocationProvider> it = providers.iterator();
                while (it.hasNext()) {

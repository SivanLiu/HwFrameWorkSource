...
public class NetworkNotificationManager {
    ...
    public void showNotification(int id, NotificationType notifyType, NetworkAgentInfo nai, NetworkAgentInfo switchToNai, PendingIntent intent, boolean highPriority) {
        ...
        if (notifyType == NotificationType.NO_INTERNET && transportType == 1) {
            CharSequence title3 = r.getString(17041540, WifiInfo.removeDoubleQuotes(nai.networkCapabilities.getSSID()));
            details = r.getString(17041541);
            title = title3;
            z = false;
        } else if (notifyType == NotificationType.PARTIAL_CONNECTIVITY && transportType == 1) {
            CharSequence title4 = r.getString(17040635, WifiInfo.removeDoubleQuotes(nai.networkCapabilities.getSSID()));
            details = r.getString(17040636);
            title = title4;
            z = false;
        } else if (notifyType == NotificationType.LOST_INTERNET && transportType == 1) {
            CharSequence title5 = r.getString(17041540, WifiInfo.removeDoubleQuotes(nai.networkCapabilities.getSSID()));
            details = r.getString(17041541);
            title = title5;
            z = false;
        } else if (notifyType == NotificationType.SIGN_IN) {
            if (transportType == 0) {
                CharSequence title6 = r.getString(17040631, 0);
                ...
                z = false;
                CharSequence title7 = r.getString(17040631, 0);
                details = r.getString(17040632, name);
                title = title7;
            } else {
                CharSequence title8 = r.getString(17041530, 0);
                details = r.getString(17040632, WifiInfo.removeDoubleQuotes(nai.networkCapabilities.getSSID()));
            ...
            z = false;
            title = r.getString(17040637, toTransport);
            details = r.getString(17040638, toTransport, fromTransport);
    ...
    public void showToast(NetworkAgentInfo fromNai, NetworkAgentInfo toNai) {
        ...
        Toast.makeText(this.mContext, this.mContext.getResources().getString(17040639, fromTransport, toTransport), 1).show();
    }

...
public class SettingsMDMPlugin extends DevicePolicyPlugin {
    ...
    public boolean onSetPolicy(ComponentName who, String policyName, Bundle policyData, boolean isGlobalPolicyChanged) {
        ...
        if (POLICY_FORCE_ENCRYPT_SDCARD.equals(policyName) && retVal) {
            boolean isEnabled = policyData.getBoolean("value");
            String str3 = TAG;
            HwLog.i(str3, "force encrypt sdcard is set to " + isEnabled);
        }
    ...
    private boolean sendFixComponentBroadcast(ComponentName who, String action, String apkName, String policyName, Bundle policyData) {
        ...
        if (who != null) {
            ...
            String admClass = "";
            data.putString(ADM_APK_NAME, TextUtils.isEmpty(admName) ? admClass : admName);
        ...
        if (policyData != null) {
            ...
            if (policyType == 0) {
                boolean isActive = policyData.getBoolean("value", false);
                data.putInt(MDM_POLICY_TYPE, 0);
                data.putBoolean("value", isActive);
    ...
    private int getPolicyType(String policyName) {
        ...
        int i = 0;
        while (true) {
            String[] strArr = LIST_POLICY_ARRAY;
            if (i >= strArr.length) {
                return -1;
            }
    ...
    private boolean checkPolicyValue(String policyName, Bundle policy) {
        ...
        if (policyType == 2 && CONFIG_NORMAL_VALUE.equals(policy.getString("value", CONFIG_NORMAL_VALUE))) {

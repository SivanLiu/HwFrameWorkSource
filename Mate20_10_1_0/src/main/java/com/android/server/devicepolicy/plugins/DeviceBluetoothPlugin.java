...
public class DeviceBluetoothPlugin extends DevicePolicyPlugin {
    ...
    public boolean onSetPolicy(ComponentName who, String policyName, Bundle policyData, boolean isChanged) {
        ...
        switch (policyName.hashCode()) {
            case -1837809145:
                if (policyName.equals(BLUETOOTH_BLACK_LIST)) {
                    c = 0;
                    break;
                }
                break;
        ...
        switch (c) {
            ...
            case 1:
                if (policyData != null) {
                    notifyBluetoohListPolicyChange(0, policyData.getStringArrayList("value"), policyName);
                    break;
                }
                break;
            case 2:
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:28:0x0079, code lost:
        if (r6.equals(com.android.server.devicepolicy.plugins.DeviceBluetoothPlugin.BLUETOOTH_BLACK_LIST) != false) goto L_0x007d;
     */
    public boolean onRemovePolicy(ComponentName who, String policyName, Bundle policyData, boolean isChanged) {
        ...
        switch (c) {
            ...
            case 1:
                if (policyData != null) {
                    notifyBluetoohListPolicyChange(1, policyData.getStringArrayList("value"), policyName);
                    break;
                }
                break;
            case 2:
        ...
        if (removedPolicies != null && removedPolicies.size() > 0) {
            ...
            for (int j = 0; j < k; j++) {
                ...
                if (policyName.equals(BLUETOOTH_BLACK_LIST) || policyName.equals(BLUETOOTH_WHITE_LIST)) {
                    ...
                    for (int i = 0; i < n; i++) {
                        Bundle oldAttr = ((PolicyStruct.PolicyItem) item.getChildItem().get(i)).getAttributes();
                        if (oldAttr == null) {
                            HwLog.i(TAG, "oldAttr is null");
                            return false;
                        }
                        notifyBluetoohListPolicyChange(1, oldAttr.getStringArrayList("value"), policyName);
                    }
                    continue;

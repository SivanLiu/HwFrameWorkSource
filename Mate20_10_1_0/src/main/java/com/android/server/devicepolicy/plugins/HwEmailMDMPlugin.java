...
public class HwEmailMDMPlugin extends DevicePolicyPlugin {
    ...
    private static final String EMAIL_ANDROID = "com.android.email";
    private static final String EMAIL_DISABLE_FORWARDING = "email-disable-forwarding";
    private static final String EMAIL_HUAWEI = "com.huawei.email";
    ...
    public PolicyStruct getPolicyStruct() {
        ...
        struct.addStruct(RESTRICT_DISABLE_DELETE_ACCOUNT, PolicyStruct.PolicyType.STATE, new String[]{"value"});
        struct.addStruct(RESTRICT_DISABLE_ADD_ACCOUNT, PolicyStruct.PolicyType.STATE, new String[]{"value"});
        struct.addStruct(RESTRICT_DISABLE_IMAP_POP3, PolicyStruct.PolicyType.STATE, new String[]{"value"});
        struct.addStruct(RESTRICT_DISABLE_CLEAR_DATA, PolicyStruct.PolicyType.STATE, new String[]{"value"});
        struct.addStruct(EMAIL_DISABLE_FORWARDING, PolicyStruct.PolicyType.CONFIGURATION, new String[]{"email_address", RESTRICT_EMAIL_FORWARDING});
        struct.addStruct(DISABLING_ADDITION_WHITE_LIST_ITEM, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct(ALLOWING_ADDITION_BLACK_LIST_ITEM, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct(DISABLING_DELETION_WHITE_LIST_ITEM, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct(ALLOWING_DELETION_BLACK_LIST_ITEM, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct(EMAIL_ACCOUNT_CONFIG, PolicyStruct.PolicyType.CONFIGURATION, new String[]{EMAIL_ACCOUNT_INCOMING_PROTOCOL, EMAIL_ACCOUNT_INCOMING_SERVER_ADDRESS, EMAIL_ACCOUNT_INCOMING_SERVER_PASSWORD, EMAIL_ACCOUNT_INCOMING_SERVER_LOGIN, EMAIL_ACCOUNT_INCOMING_SERVER_PORT, EMAIL_ACCOUNT_INCOMING_SERVER_ACCEPT_ALL_CERTIFICATES, EMAIL_ACCOUNT_INCOMING_SERVER_USE_SSL, EMAIL_ACCOUNT_INCOMING_SERVER_USE_TLS, EMAIL_ACCOUNT_INCOMING_PATH_PREFIX, EMAIL_ACCOUNT_OUTGOING_PROTOCOL, EMAIL_ACCOUNT_OUTGOING_SERVER_ADDRESS, EMAIL_ACCOUNT_OUTGOING_SERVER_PASSWORD, EMAIL_ACCOUNT_OUTGOING_SERVER_LOGIN, EMAIL_ACCOUNT_OUTGOING_SERVER_PORT, EMAIL_ACCOUNT_OUTGOING_SERVER_ACCEPT_ALL_CERTIFICATES, EMAIL_ACCOUNT_OUTGOING_SERVER_USE_SSL, EMAIL_ACCOUNT_OUTGOING_SERVER_USE_TLS, EMAIL_ACCOUNT_OUTGOING_PATH_PREFIX, EMAIL_ACCOUNT_ADDRESS, EMAIL_ACCOUNT_SENDER_NAME, EMAIL_ACCOUNT_SIGNATURE, EMAIL_ACCOUNT_IS_DEFAULT});
        struct.addStruct(EXCHANGE_ACCOUNT_CONFIG, PolicyStruct.PolicyType.CONFIGURATION, new String[]{EXCHANGE_ACCOUNT_ACCEPT_ALL_CERTIFICATES, EXCHANGE_ACCOUNT_DISPLAY_NAME, EXCHANGE_ACCOUNT_DOMAIN, EXCHANGE_ACCOUNT_USER, EXCHANGE_ACCOUNT_EMAIL_ADDRESS, EXCHANGE_ACCOUNT_IS_DEFAULT, EXCHANGE_ACCOUNT_PERIOD_CALENDAR, EXCHANGE_ACCOUNT_PROTOCOL_VERSION, EXCHANGE_ACCOUNT_SENDER_NAME, EXCHANGE_ACCOUNT_SERVER_ADDRESS, EXCHANGE_ACCOUNT_SERVER_PASSWORD, EXCHANGE_ACCOUNT_SERVER_PATH_PREFIX, EXCHANGE_ACCOUNT_SIGNATURE, EXCHANGE_ACCOUNT_SYNC_INTERVAL, EXCHANGE_ACCOUNT_SYNC_LOOKBACK, EXCHANGE_ACCOUNT_USE_SSL, EXCHANGE_ACCOUNT_USE_TSL, EXCHANGE_ACCOUNT_SCREENSHOT_DISABLED, EXCHANGE_ACCOUNT_DISPLAY_HTML_DISABLED, EXCHANGE_ACCOUNT_FORWARD_EMAIL_DISABLED});
        struct.addStruct(EXCHANGE_FORCE_SMIME_SIGN, PolicyStruct.PolicyType.CONFIGURATION, new String[]{"email_address", SMIME_ALIAS, SMIME_FORCE_REQUIRED});
        struct.addStruct(EXCHANGE_FORCE_SMIME_ENCRYPTION, PolicyStruct.PolicyType.CONFIGURATION, new String[]{"email_address", SMIME_FORCE_REQUIRED, SMIME_FORCE_REQUIRED});
        return struct;
    }
    ...
    public boolean onSetPolicy(ComponentName admin, String policyName, Bundle policyData, boolean isGlobalPolicyChanged) {
        String emailPackageName = getApkPackageName();
        HwLog.i(TAG, "onSetPolicy PolicyName " + policyName + " globalPolicyChanged:" + isGlobalPolicyChanged);
        Intent intent = new Intent(DEVICE_POLICY_ACTION_POLICY_CHANGED);
        intent.setClassName(emailPackageName, "com.huawei.emailmdm.EmailMDMBroadcastReceiver");
        intent.putExtras(policyData);
        intent.putExtra("MDMName", admin.flattenToString());
        if ((!isGlobalPolicyChanged && isStatePolicy(policyName)) || policyName.equals(RESTRICT_DISABLE_CLEAR_DATA)) {
            return true;
        }
    ...
    private boolean isStatePolicy(String policyName) {
        return policyName.equals(RESTRICT_DISABLE_DELETE_ACCOUNT) || policyName.equals(RESTRICT_DISABLE_ADD_ACCOUNT) || policyName.equals(RESTRICT_DISABLE_IMAP_POP3);
    }
    ...
    public boolean onActiveAdminRemoved(ComponentName admin, ArrayList<PolicyStruct.PolicyItem> removedPolicies) {
        return onActiveAdminRemoved(admin, removedPolicies, -1);
    }

    public boolean onActiveAdminRemoved(ComponentName admin, ArrayList<PolicyStruct.PolicyItem> arrayList, int userId) {
        String emailPackageName = getApkPackageName();
        HwLog.i(TAG, "onActiveAdminRemoved ComponentName");
        Intent intent = new Intent(DEVICE_POLICY_ACTION_POLICY_CHANGED);
        intent.setClassName(emailPackageName, "com.huawei.emailmdm.EmailMDMBroadcastReceiver");
    ...
    private String getApkPackageName() {
        ...
        if (pm != null) {
            try {
                if (pm.getApplicationInfo(EMAIL_HUAWEI, 8192) != null) {
                    return EMAIL_HUAWEI;
                }
                return EMAIL_ANDROID;
            } catch (PackageManager.NameNotFoundException ex) {
                HwLog.d(TAG, "getApkPackageName NameNotFoundException ex :" + ex.toString());
            }
        }
        return EMAIL_ANDROID;
    }

...
public class HwDevicePolicyManagerServiceUtil extends EasyInvokeUtils {
    ...
    public static boolean matchWithWildcards(String testPart, String providerPart) {
        int providerLength;
        if (testPart == null || providerPart == null || testPart.length() != (providerLength = providerPart.length())) {
            return false;
        }
    ...
    public static boolean isValidatePackageName(String name) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (isCharLowerCaseValidate(c) || isCharUpperCaseValidate(c)) {
                isFront = false;
            } else if (isFront || (!isCharNumberCaseValidate(c) && c != '_')) {
                if (c != '.') {
                    return false;
                }
                isHasSep = true;
    ...
    private static boolean isCharNumberCaseValidate(char c) {
        return c >= '0' && c <= '9';
    }

    private static boolean isCharUpperCaseValidate(char c) {
        return c >= 'A' && c <= 'Z';
    }

    private static boolean isCharLowerCaseValidate(char c) {
        return c >= 'a' && c <= 'z';
    }
    ...
    public static void collectMdmDoSuccessDftData(String pkgName) {
        PackageManager pm;
        try {
            if (mContext != null && (pm = mContext.getPackageManager()) != null) {

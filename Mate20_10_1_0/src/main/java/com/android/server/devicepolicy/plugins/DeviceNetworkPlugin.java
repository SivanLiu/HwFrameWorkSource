...
public class DeviceNetworkPlugin extends DevicePolicyPlugin {
    ...
    public PolicyStruct getPolicyStruct() {
        ...
        struct.addStruct("network-white-ip-list", PolicyStruct.PolicyType.LIST, new String[0]);
        struct.addStruct(ITEM_NETWORK_WHITE_IP_LIST, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct("network-white-domain-list", PolicyStruct.PolicyType.LIST, new String[0]);
        struct.addStruct(ITEM_NETWORK_WHITE_DOMAIN_LIST, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct("network-black-ip-list", PolicyStruct.PolicyType.LIST, new String[0]);
        struct.addStruct(ITEM_NETWORK_BLACK_IP_LIST, PolicyStruct.PolicyType.LIST, new String[]{"value"});
        struct.addStruct("network-black-domain-list", PolicyStruct.PolicyType.LIST, new String[0]);
    ...
    public boolean checkCallingPermission(ComponentName who, String policyName) {
        ...
        switch (policyName.hashCode()) {
            case -30728870:
                if (policyName.equals("network-black-domain-list")) {
                    c = 4;
                    break;
                }
                c = 65535;
                break;
            case 81510017:
                if (policyName.equals("network-white-ip-list")) {
                    c = 1;
                    break;
                }
                c = 65535;
                break;
            case 746015831:
                if (policyName.equals("network-black-ip-list")) {
                    c = 3;
                    break;
                }
                c = 65535;
                break;
            case 1297508996:
                if (policyName.equals("network-white-domain-list")) {
                    c = 2;
                    break;
                }
                c = 65535;
    ...
    public boolean onSetPolicy(ComponentName who, String policyName, Bundle policyData, boolean isChanged) {
        ...
        if (c == 0) {
            ...
            if (!isChanged || checkPolicySize(policyName, data, 1000)) {
                return true;
            }
    ...
    public ArrayList<String> queryBrowsingHistory() {
        ArrayList<String> historyList = new ArrayList<>(10);
        Uri[] uriArr = HISTORY_URI;
        int length = uriArr.length;
        int i = 0;
        while (i < length) {
            try {
                Cursor cursor = this.mContext.getContentResolver().query(uriArr[i], new String[]{URL_COLUMN_NAME}, null, null, null);
                if (cursor == null) {
    ...
    private boolean setWhiteIpPolicyOfNetworkList(Bundle policyData, String policyName, boolean isChanged) {
        ...
        try {
            policyList = policyData.getStringArrayList("value");
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    private boolean removeIpPolicyOfNetworkList(String policyName, Bundle policyData, int whiteOrBlack, boolean isChanged) {
        if (this.mPolicyStruct == null || policyData == null) {
            ...
            try {
                data = policyData.getStringArrayList("value");
            } catch (ArrayIndexOutOfBoundsException e) {
    ...
    private void setIptablesAfterRemoveWhitePolicy() {
        List<String> networkWhiteIpPolicies = getPolicyArrayList("network-white-ip-list");
        List<String> networkWhiteDomainPolicies = getPolicyArrayList("network-white-domain-list");
    ...
    private void setIptablesAfterRemoveBlackPolicy() {
        List<String> networkBlackIpPolicies = getPolicyArrayList("network-black-ip-list");
        List<String> networkBlackDomainPolicies = getPolicyArrayList("network-black-domain-list");
        HwServiceFactory.getHwConnectivityManager().clearIpCacheOfDnsEvent();
    ...
    private boolean isNetworkListLimitPolicy(String policyName) {
        return "network-white-ip-list".equals(policyName) || "network-white-domain-list".equals(policyName) || "network-black-ip-list".equals(policyName) || "network-black-domain-list".equals(policyName);
    }
}

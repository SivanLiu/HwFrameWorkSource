...
public class Wpa3SelfCureImpl {
    ...
    public static boolean mIsWpa3SelfCureTrigger = false;
    ...
    private LinkedHashMap<String, Integer> mConnectFailEventStats = new LinkedHashMap<String, Integer>() {
        ...
        public boolean removeEldestEntry(Entry<String, Integer> entry) {
            return size() > 200;
        }
    };
    ...
    private WifiConfiguration getConfig(WifiConfigManager wifiConfigManager) {
        if (!this.mAuthType.contains(WifiProCommonUtils.KEY_MGMT_SAE)) {
            return null;
        }
    ...
    private boolean isStatusCodeAllowed(int statusCode) {
        if (statusCode == 10 || statusCode == 16 || statusCode == 38 || statusCode == 72 || statusCode == 123 || statusCode == 12 || statusCode == 13) {
            return true;
        }
        switch (statusCode) {
            case 40:
            ...
            default:
                switch (statusCode) {
                    case WLAN_STATUS_AUTH_RSP2_TIMEOUT /*{ENCODED_INT: 5201}*/:
                    case WLAN_STATUS_AUTH_RSP4_TIMEOUT /*{ENCODED_INT: 5202}*/:
                    case WLAN_STATUS_ASSOC_RSP_TIMEOUT /*{ENCODED_INT: 5203}*/:
                        return true;
                    default:
                        return false;
                }
    ...
    private boolean isNeedSelfCure(WifiConfigManager wifiConfigManager, String eventInfo, WifiConfiguration config) {
        if (!KEY_MGMT_SAE_TRANSITION.equals(this.mAuthType)) {
            ...
            return false;
        } else if (!ASSOC_TIME_OUT.equals(this.mFailReason) || getRssi(wifiConfigManager, config) > -70) {
        ...
        while (it.hasNext()) {
            ...
            if (this.mSsid.equals(info.ssid) && this.mAuthType.equals(info.origAuthType)) {
                ...
                if (this.mBssid.equals(info.bssid)) {
                    long unused = info.lastTimeStamp = SystemClock.elapsedRealtime();
                    return;
    ...
    private void connectBySuggestAuthType(WifiConfiguration config, String authType) {
    ...
    public void handleConnectFailReport(WifiConfigManager wifiConfigManager, String eventInfo) {
        if (wifiConfigManager == null || eventInfo == null) {
            ...
            if (isNeedSelfCure(wifiConfigManager, eventInfo, config)) {
                addToBlackList();
                connectBySuggestAuthType(config, getSuggestAuthType(this.mAuthType));
            }
            this.mIsSelfCureOngoing = false;
        }
    }
    ...
    private class Wpa3BlackListInfo {
        /* access modifiers changed from: private */
        public String bssid;
        /* access modifiers changed from: private */
        public long lastTimeStamp;
        /* access modifiers changed from: private */
        public String origAuthType;
        /* access modifiers changed from: private */
        public String ssid;
        /* access modifiers changed from: private */
        public String suggestAuthType;

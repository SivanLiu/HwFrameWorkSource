    ...
    private void logLinkMetrics(WifiInfo wifiInfo, long now, int netId, int s1, int s2, int score) {
        if (now >= FIRST_REASONABLE_WALL_CLOCK) {
            ...
            try {
                ...
                String s = String.format(Locale.US, "%s,%d,%d,%.1f,%.1f,%.1f,%d,%d,%.2f,%.2f,%.2f,%.2f,%d,%d,%d,%d,%d", timestamp, Integer.valueOf(this.mSessionNumber), Integer.valueOf(netId), Double.valueOf(rssi), Double.valueOf(filteredRssi), Double.valueOf(rssiThreshold), Integer.valueOf(freq), Integer.valueOf(linkSpeed), Double.valueOf(txSuccessRate), Double.valueOf(txRetriesRate), Double.valueOf(txBadRate), Double.valueOf(rxSuccessRate), Integer.valueOf(this.mNudYes), Integer.valueOf(this.mNudCount), Integer.valueOf(s1), Integer.valueOf(s2), Integer.valueOf(score));

...
public class HwInformationElementUtilEx implements IHwInformationElementUtilEx {
    ...
    private static final int NARROWBAND_OFFSET = 3;
    ...
    private static boolean isHuaweiRouter(ScanResult.InformationElement ie) {
        ...
        for (int index = 0; index < VENDOR_HUAWEI_OUI.length; index++) {
            if ((ie.bytes[index] & Constants.BYTE_MASK) != VENDOR_HUAWEI_OUI[index]) {
                return false;
            }
        }
        return true;
    ...
    private boolean is160mConfigurated(ScanResult.InformationElement[] ies) {
        ...
        for (ScanResult.InformationElement ie : ies) {
            if (ie != null && ie.id == 61) {
                ...
                channelIndex = ie.bytes[0] & Constants.BYTE_MASK;
                isHtInfoFound = true;
            }
            if (ie != null && ie.id == 192) {
                ...
                int channelWidth = ie.bytes[0] & Constants.BYTE_MASK;
                int channelCenterSegment0 = ie.bytes[1] & Constants.BYTE_MASK;
                channelCenterSegment1 = ie.bytes[2] & Constants.BYTE_MASK;
                if (channelWidth == 2) {
                    return true;
                }
    ...
    private static boolean isNarrowBandSupported(ScanResult.InformationElement ie) {
        ...
        for (int index = 0; index < BCC_IE_HEADER.length; index++) {
            if ((ie.bytes[index] & Constants.BYTE_MASK) != BCC_IE_HEADER[index]) {
                return false;
            }
        }
        int index2 = 9;
        while (index2 + 1 < ie.bytes.length && index2 > 0) {
            if ((ie.bytes[index2] & 255) == TYPE_NARROWBAND_SUBIE) {
                int index3 = index2 + (((ie.bytes[index2 + 1] & Constants.BYTE_MASK) + 2) - 3);
                if (index3 < ie.bytes.length && (ie.bytes[index3] & 32) == 32) {
                    return true;
                }
                return false;
            }
            index2 += (ie.bytes[index2 + 1] & Constants.BYTE_MASK) + 2;
        }
        return false;
    }

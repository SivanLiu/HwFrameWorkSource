...
public class SupplicantStateTracker extends StateMachine {
    ...
    public void handleNetworkConnectionFailure(int netId, int disableReason) {
        if (DBG) {
            Log.i(TAG, "handleNetworkConnectionFailure netId=" + netId + " reason " + disableReason + " mNetworksDisabledDuringConnect=" + this.mNetworksDisabledDuringConnect);
        }
        if (this.mNetworksDisabledDuringConnect) {
            this.mWifiConfigManager.mHwWifiConfigManagerEx.enableAllNetworks();
        ...
        WifiInjector.getInstance().getClientModeImpl().handleConnectFailedInWifiPro(netId, disableReason);
    }
    ...
    public void transitionOnSupplicantStateChange(StateChangeResult stateChangeResult) {
        ...
        if (DBG) {
            Log.i(TAG, "Supplicant state: " + supState.toString() + "\n");
        }
    ...
    public void sendSupplicantStateChangedBroadcast(SupplicantState state, boolean failedAuth, int reasonCode) {
        ...
        if (!WifiInjector.getInstance().getClientModeImpl().ignoreSupplicantStateChange(state)) {
    ...
    class DefaultState extends State {
        ...
        public void enter() {
            if (SupplicantStateTracker.DBG) {
                Log.i(SupplicantStateTracker.TAG, getName() + "\n");
            }
        }

        public boolean processMessage(Message message) {
            if (SupplicantStateTracker.DBG) {
                Log.i(SupplicantStateTracker.TAG, getName() + message.toString() + "\n");
            }
            switch (message.what) {
                case 131183:
                ...
                case WifiMonitor.SUPPLICANT_STATE_CHANGE_EVENT /*{ENCODED_INT: 147462}*/:
                    ...
                    supplicantStateTracker2.sendSupplicantStateChangedBroadcast(state, supplicantStateTracker2.mAuthFailureInSupplicantBroadcast, SupplicantStateTracker.this.mAuthFailureReason);
    ...
    class HandshakeState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            if (this.mLoopDetectIndex > state.ordinal()) {
                ...
                if (SupplicantStateTracker.DBG) {
                    Log.i(SupplicantStateTracker.TAG, "mLoopDetectIndex:" + this.mLoopDetectIndex + ", state:" + state.ordinal() + ", increase mLoopDetectCount:" + this.mLoopDetectCount);
                }
            }
            if (this.mLoopDetectCount > 4) {
                if (SupplicantStateTracker.DBG) {
                    Log.i(SupplicantStateTracker.TAG, "Supplicant loop detected, disabling network " + stateChangeResult.networkId);
                }
            ...
            SupplicantStateTracker supplicantStateTracker = SupplicantStateTracker.this;
            supplicantStateTracker.sendSupplicantStateChangedBroadcast(state, supplicantStateTracker.mAuthFailureInSupplicantBroadcast, SupplicantStateTracker.this.mAuthFailureReason);
            return true;
        }
    }

...
public class WifiNetworkNotifier {
    ...
    public WifiNetworkNotifier(Context context, Looper looper, FrameworkFacade framework) {
    ...
    private void showAvailableNetworkNotification() {
        ...
        CharSequence title = resource.getText(33686259);
        CharSequence message = resource.getText(33686258);
        Notification.Builder builder = new Notification.Builder(this.mContext, SystemNotificationChannels.NETWORK_AVAILABLE);
        builder.setWhen(0).setSmallIcon(17303543).setContentTitle(title).setContentText(message).setStyle(new Notification.BigTextStyle().bigText(message)).setContentIntent(pi).setAutoCancel(true);

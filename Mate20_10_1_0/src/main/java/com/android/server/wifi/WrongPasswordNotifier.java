...
public class WrongPasswordNotifier {
    ...
    private void showNotification(String ssid) {
        ...
        this.mNotificationManager.notify(42, this.mFrameworkFacade.makeNotificationBuilder(this.mContext, SystemNotificationChannels.NETWORK_ALERTS).setAutoCancel(true).setTimeoutAfter(CANCEL_TIMEOUT_MILLISECONDS).setSmallIcon(17303543).setContentTitle(this.mContext.getString(17041534)).setContentText(ssid).setContentIntent(this.mFrameworkFacade.getActivity(this.mContext, 0, intent, 134217728)).setColor(this.mContext.getResources().getColor(17170460)).build());
    }

...
public class HwPhoneCloneChr {
    ...
    private void uploadPhoneCloneParamStatistics() {
        ...
        IMonitor.EventStream phoneCloneEvent = IMonitor.openEventStream((int) PHONE_CLONE_STATISTICS_EVENT);
        phoneCloneEvent.setParam(CHANNEL, this.mChannel).setParam(REQUEST_BANDWIDTH, this.mRequestBandWidth).setParam(RESPONSE_BANDWIDTH, this.mResponseBandWidth).setParam(DATA_OF_CLONE, this.mCloneTraffic).setParam("time", this.mTimeOfClone).setParam(MODEL_OF_PHONE, this.mPhoneModel);
        IMonitor.sendEvent(phoneCloneEvent);
        IMonitor.closeEventStream(phoneCloneEvent);
        resetParameters();
    }

...
public class HwWifiConnectivityManager extends WifiConnectivityManager {
    ...
    public void startHourPeriodicSingleScan() {
        ...
        localLog("startHourPeriodicSingleScan: screenOn=" + screenOn + " wifiEnabled=" + wifiEnabled + " wifiConnectivityManagerEnabled=" + wifiConnectivityManagerEnabled);
        if (screenOn && wifiEnabled && !wifiConnectivityManagerEnabled) {
    ...
    public boolean forceFullbandScanAgain(boolean mScreenOn, int mWifiState, boolean wasConnectAttempted, boolean isSingleScanStarted) {
        if (!mScreenOn || mWifiState != 2 || wasConnectAttempted || !isWifiScanSpecialChannels() || !isSingleScanStarted) {
            return false;
        }
    ...
    public void notifyScanGenieEvent(int eventId) {
        ...
        if (uploadManager != null) {
            if (eventId == 1000) {
                uploadManager.addChrCntStat(SCAN_GENIE_EVENT, SCAN_GENIE_OCCUR_EVENT);
            } else if (eventId == MSG_SCAN_GENIE_SUCC_EVENT) {
                uploadManager.addChrCntStat(SCAN_GENIE_EVENT, SCAN_GENIE_SUCC_EVENT);
            } else if (eventId == MSG_SCAN_GENIE_FAIL_EVENT) {
                uploadManager.addChrCntStat(SCAN_GENIE_EVENT, SCAN_GENIE_FAIL_EVENT);
            } else {
                HwHiLog.w(TAG, false, "notifyScanGenieEvent not match event", new Object[0]);
            }
        }
    }
}

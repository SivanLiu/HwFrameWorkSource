...
public class HwWifiDevicePolicyImpl implements HwWifiDevicePolicy {
    ...
    public boolean isSSIDOnBlackList(String ssid, boolean isToastNeeded) {
        ...
        synchronized (this) {
            Iterator<Map.Entry<ComponentName, ArrayList<String>>> it = mGlobalSsidBlacklist.entrySet().iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
    ...
    public void procBroadCasts(Intent intent) {
    ...
    private void procBlacklist(int mode, String packageName, String className, ArrayList<String> increment) {
        synchronized (this) {
            for (Map.Entry<ComponentName, ArrayList<String>> entry : mGlobalSsidBlacklist.entrySet()) {

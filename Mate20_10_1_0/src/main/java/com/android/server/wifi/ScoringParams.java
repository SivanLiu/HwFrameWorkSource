...
public class ScoringParams {
    ...
    private class Values {
        ...
        private void validateRssiArray(int[] rssi) throws IllegalArgumentException {
            int low = -126;
            int high = Math.min((int) ChannelHelper.SCAN_PERIOD_PER_CHANNEL_MS, -1);
    ...
    private void loadResources(Context context) {
        this.mVal.rssi2[0] = context.getResources().getInteger(17694945);
        this.mVal.rssi2[1] = context.getResources().getInteger(17694947);
        this.mVal.rssi2[2] = context.getResources().getInteger(17694951);
        this.mVal.rssi2[3] = context.getResources().getInteger(17694949);
        this.mVal.rssi5[0] = context.getResources().getInteger(17694946);
        this.mVal.rssi5[1] = context.getResources().getInteger(17694948);
        this.mVal.rssi5[2] = context.getResources().getInteger(17694952);
        this.mVal.rssi5[3] = context.getResources().getInteger(17694950);
        try {
            this.mVal.validate();
    ...
    private void setupContentObserver(final Context context, final FrameworkFacade facade, Handler handler) {
        final String defaults = toString();
        ContentObserver observer = new ContentObserver(handler) {
            /* class com.android.server.wifi.ScoringParams.AnonymousClass1 */

            public void onChange(boolean selfChange) {
                String params = facade.getStringSetting(context, "wifi_score_params");

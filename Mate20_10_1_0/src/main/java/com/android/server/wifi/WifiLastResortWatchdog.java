...
public class WifiLastResortWatchdog {
        ...
        if (availableNetworks != null) {
            for (Pair<ScanDetail, WifiConfiguration> pair : availableNetworks) {
                ...
                if (scanResult != null) {
                    ...
                    if (this.mVerboseLoggingEnabled) {
                        Log.v(TAG, " " + StringUtilEx.safeDisplayBssid(bssid) + ": " + StringUtilEx.safeDisplaySsid(scanDetail.getSSID()));
                    }
        ...
        while (it.hasNext()) {
            ...
            if (entry.getValue().age < 9) {
                ...
                if (ssidFails != null) {
                    ...
                    Log.d(TAG, "updateAvailableNetworks: SSID to AP count mismatch for " + StringUtilEx.safeDisplaySsid(ssid2));
                }
                it.remove();
            }
        }
    ...
    public boolean noteConnectionFailureAndTriggerIfNeeded(String ssid, String bssid, int reason) {
        if (this.mVerboseLoggingEnabled) {
            Log.v(TAG, "noteConnectionFailureAndTriggerIfNeeded: [" + StringUtilEx.safeDisplaySsid(ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + ", " + reason + "]");
        }
        ...
        if (isRestartNeeded) {
            ...
            localLog(toString());
            if (!this.mClientModeImpl.isInWifiSettings()) {
    ...
    private void takeBugReportWithCurrentProbability(String bugDetail) {
        if (this.mBugReportProbability > Math.random()) {
            new Handler(this.mClientModeImplLooper).post(new Runnable(bugDetail) {
                /* class com.android.server.wifi.$$Lambda$WifiLastResortWatchdog$mIpWfGvGcRn_UMBeTeYZKeAS5FM */
                ...
                public final void run() {
                    WifiLastResortWatchdog.this.lambda$takeBugReportWithCurrentProbability$0$WifiLastResortWatchdog(this.f$1);
                }
            });
        }
    }

    public /* synthetic */ void lambda$takeBugReportWithCurrentProbability$0$WifiLastResortWatchdog(String bugDetail) {
    ...
    private void updateFailureCountForNetwork(String ssid, String bssid, int reason) {
        logv("updateFailureCountForNetwork: [" + StringUtilEx.safeDisplaySsid(ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + ", " + reason + "]");
        if ("any".equals(bssid)) {
    ...
    private void incrementSsidFailureCount(String ssid, int reason) {
        ...
        if (ssidFails == null) {
            Log.d(TAG, "updateFailureCountForNetwork: No networks for ssid = " + StringUtilEx.safeDisplaySsid(ssid));
            return;
        }
    ...
    private void incrementBssidFailureCount(String ssid, String bssid, int reason) {
        AvailableNetworkFailureCount availableNetworkFailureCount = this.mRecentAvailableNetworks.get(bssid);
        if (availableNetworkFailureCount == null) {
            Log.d(TAG, "updateFailureCountForNetwork: Unable to find Network [" + StringUtilEx.safeDisplaySsid(ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + "]");
        } else if (!availableNetworkFailureCount.ssid.equals(ssid)) {
            Log.d(TAG, "updateFailureCountForNetwork: Failed connection attempt has wrong ssid. Failed [" + StringUtilEx.safeDisplaySsid(ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + "], buffered [" + StringUtilEx.safeDisplaySsid(availableNetworkFailureCount.ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + "]");
        } else {
            if (availableNetworkFailureCount.config == null && this.mVerboseLoggingEnabled) {
                Log.v(TAG, "updateFailureCountForNetwork: network has no config [" + StringUtilEx.safeDisplaySsid(ssid) + ", " + StringUtilEx.safeDisplayBssid(bssid) + "]");
            }
    ...
    private boolean isBssidOnlyApOfSsid(String bssid) {
        ...
        Pair<AvailableNetworkFailureCount, Integer> ssidFails = this.mSsidFailureCount.get(ssid);
        if (ssidFails == null) {
            Log.d(TAG, "isOnlyBssidAvailable: Could not find SSID count for " + StringUtilEx.safeDisplaySsid(ssid));
            return false;
    ...
    public int getFailureCount(String bssid, int reason) {
        ...
        Pair<AvailableNetworkFailureCount, Integer> ssidFails = this.mSsidFailureCount.get(ssid);
        if (ssidFails == null) {
            Log.d(TAG, "getFailureCount: Could not find SSID count for " + StringUtilEx.safeDisplaySsid(ssid));
            return 0;
        }

...
public class ConnectToNetworkNotificationBuilder {
    ...
    public Notification createConnectToAvailableNetworkNotification(String notifierTag, ScanResult network) {
        ...
        if (hashCode != 594918769) {
            if (hashCode == 2017428693 && notifierTag.equals(OpenNetworkNotifier.TAG)) {
                c = 0;
                if (c == 0) {
                    title = this.mContext.getText(17041531);
                } else if (c != 1) {
                    Log.wtf("ConnectToNetworkNotificationBuilder", "Unknown network notifier." + notifierTag);
                    return null;
                } else {
                    title = this.mContext.getText(17041528);
                }
                return createNotificationBuilder(title, network.SSID, notifierTag).setContentIntent(getPrivateBroadcast(ACTION_PICK_WIFI_NETWORK, notifierTag)).addAction(new Notification.Action.Builder((Icon) null, this.mResources.getText(17041527), getPrivateBroadcast(ACTION_CONNECT_TO_NETWORK, notifierTag)).build()).addAction(new Notification.Action.Builder((Icon) null, this.mResources.getText(17041526), getPrivateBroadcast(ACTION_PICK_WIFI_NETWORK, notifierTag)).build()).build();
            }
    ...
    public Notification createNetworkConnectingNotification(String notifierTag, ScanResult network) {
        return createNotificationBuilder(this.mContext.getText(17041533), network.SSID, notifierTag).setProgress(0, 0, true).build();
    }

    public Notification createNetworkConnectedNotification(String notifierTag, ScanResult network) {
        return createNotificationBuilder(this.mContext.getText(17041532), network.SSID, notifierTag).build();
    }

    public Notification createNetworkFailedNotification(String notifierTag) {
        return createNotificationBuilder(this.mContext.getText(17041534), this.mContext.getText(17041529), notifierTag).setContentIntent(getPrivateBroadcast(ACTION_PICK_WIFI_NETWORK_AFTER_CONNECT_FAILURE, notifierTag)).setAutoCancel(true).build();
    }

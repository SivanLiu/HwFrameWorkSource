...
public class HwApConfigUtilEx {
    ...
    /* JADX INFO: Multiple debug info for r3v2 int[]: [D('allowed5GFreqListNew' int[]), D('i' int)] */
    public static int getSelected5GChannel(int[] allowed5GFreqList) {
        ...
        for (int i2 = 0; i2 < allowed5GFreqList.length; i2++) {
            if (ApConfigUtil.convertFrequencyToChannel(allowed5GFreqList[i2]) == 165) {
        ...
        for (int i3 = 0; i3 < allowed5GFreqList.length; i3++) {
            if (ApConfigUtil.convertFrequencyToChannel(allowed5GFreqList[i3]) != 165) {

...
public class HSWanMetricsElement extends ANQPElement {
    ...
    public static HSWanMetricsElement parse(ByteBuffer payload) throws ProtocolException {
        if (payload.remaining() == 13) {
            ...
            return new HSWanMetricsElement(status, symmetric, capped, ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 4) & Constants.INT_MASK, ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 4) & Constants.INT_MASK, payload.get() & Constants.BYTE_MASK, payload.get() & 255, ((int) ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 2)) & Constants.SHORT_MASK);
        }

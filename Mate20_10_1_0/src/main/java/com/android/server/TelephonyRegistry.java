...
public class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    public final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == 1) {
                ...
                if (TelephonyRegistry.VDBG) {
                    LocalLog access$800 = TelephonyRegistry.this.mLocalLog;
                    access$800.log("Default subscription updated: mDefaultPhoneId=" + TelephonyRegistry.this.mDefaultPhoneId + ", mDefaultSubId" + TelephonyRegistry.this.mDefaultSubId);
                }
            }
        }
    };
    ...
    public void notifyDataConnection(int state, boolean isDataAllowed, String apn, String apnType, LinkProperties linkProperties, NetworkCapabilities networkCapabilities, int networkType, boolean roaming) {
        notifyDataConnectionForSubscriber(HwBootFail.STAGE_BOOT_SUCCESS, HwBootFail.STAGE_BOOT_SUCCESS, state, isDataAllowed, apn, apnType, linkProperties, networkCapabilities, networkType, roaming);
    }
    ...
    private boolean isModified(int phoneId, int state, String apnType, int networkType) {
        ...
        if (SHOW_DATA_ACT_ALL_APN || IS_NR_SLICES_SUPPORTED) {
            if (state == 2) {
    ...
    public void notifyDataConnectionFailed(String apnType) {
        notifyDataConnectionFailedForSubscriber(HwBootFail.STAGE_BOOT_SUCCESS, HwBootFail.STAGE_BOOT_SUCCESS, apnType);
    }
        ...
        if (checkNotifyPermission("notifyActiveDataSubIdChanged()")) {
            ...
            List<Record> copiedRecords2 = (List) copiedRecords.stream().filter(new Predicate() {
                ...
                public final boolean test(Object obj) {
                    return TelephonyRegistry.this.lambda$notifyActiveDataSubIdChanged$1$TelephonyRegistry((Record) obj);
                }

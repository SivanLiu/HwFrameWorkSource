...
public class HwSuplServiceParse {
    ...
    private boolean isMatchProductAndVersion(JSONObject configObj) {
        ...
        String paraProdcutVersion = configObj.optString(JSON_TAG_PRODUCT_VERSION);
        ...
        char[] productName = PRODUCT_PRODUCT_NAME.toCharArray();
        char[] configProdcutName = paraProdcutName.toCharArray();
        if (configProdcutName.length > productName.length) {
            LBSLog.i(TAG, false, "productName length is not match", new Object[0]);
            return false;
        }
        int i = 0;
        while (i < configProdcutName.length) {
            if (productName[i] == configProdcutName[i] || configProdcutName[i] == '*') {
                isMatchHwSuplServer = true;
                i++;
            } else {
                LBSLog.i(TAG, false, "configProdcutName false", new Object[0]);
                return false;
            }
        }
    ...
    private boolean isMatchVersion(String version) {
        ...
        try {
            String[] productVersionStr = version.split(AwarenessInnerConstants.DASH_KEY);
            String[] splitVersionOne = PRODUCT_PRODUCT_VERSION.split("\\.");
            if (splitVersionOne.length < 4) {
                LBSLog.e(TAG, false, "system subVersion is wrong", new Object[0]);
                return false;
            }
            if (splitVersionOne[3].contains("\\(")) {
                subVersion = splitVersionOne[3].split("\\(")[0];
            } else {
                subVersion = splitVersionOne[3];
            }
            ...
            if (!PARA_DEFAULT_VALUE.equals(productVersionStr[0]) && Integer.parseInt(productVersionStr[0]) > productVersion) {
                isMatchHwSuplServer = false;
            }
            if (PARA_DEFAULT_VALUE.equals(productVersionStr[1]) || Integer.parseInt(productVersionStr[1]) >= productVersion) {

...
public class LbsParaUpdater {
    ...
    private static volatile LbsParaUpdater mInstance;
    private int configVersion = -1;
    /* access modifiers changed from: private */
    public int hwSuplconfigVersion = -1;
    /* access modifiers changed from: private */
    ...
    private ArrayList<ConfigeParam> paramList = new ArrayList<>();
    ...
    public static LbsParaUpdater getInstance(Context context) {
        ...
        synchronized (LbsParaUpdater.class) {
            if (mInstance == null) {
                mInstance = new LbsParaUpdater(context);
            }
            lbsParaUpdater = mInstance;
        }
    ...
    /* access modifiers changed from: private */
    public synchronized void runUpgradeThread(boolean isCotaBroadcast) {
        if (!this.isRunning) {
            this.isRunning = true;
    ...
    private class UpgradeLbsConfigThread extends Thread {
        ...
        public void run() {
            ...
            if (!"".equals(filePath) || !"".equals(hwSuplPath)) {
                ...
                boolean unused = LbsParaUpdater.this.isRunning = false;
                return;
            }
            LBSLog.w(LbsParaUpdater.TAG, false, "no need to update configration", new Object[0]);
            LbsParaUpdater.this.reportParseError(this.mIsCotaBroadcast, 9);
            boolean unused2 = LbsParaUpdater.this.isRunning = false;
        }
    }

    /* access modifiers changed from: private */
    public String getUpdateConfigFilePath(boolean isCotaBroadcast, String configPath) {
    ...
    /* access modifiers changed from: private */
    public String getUpdateHwSuplConfigFilePath(boolean isCotaBroadcast, String configPath) {
        String configFile = getCoteParaFilePath(configPath);
    ...
    private int getConfigVersion(String path) {
        ...
        try {
            ...
            int version = headerObj.optInt(JSON_TAG_VERSION);
            if (version < 0) {
                return -1;
            }
            return version;
    ...
    /* access modifiers changed from: private */
    public void updateConfigToDataBase() {
    ...
    /* access modifiers changed from: private */
    public boolean parseConfigFile(String filePath, boolean isCotaBroadcast) {
    ...
    private boolean parseParams(JSONObject configObj, boolean isCotaBroadcast) {
        String paraName = configObj.optString("name");
    ...
    /* access modifiers changed from: private */
    public void reportParseError(boolean isCotaBroadcast, int errorCode) {
        if (isCotaBroadcast) {
    ...
    private String getCoteParaFilePath(String configPath) {
        String upgradePath = "";
        try {
            String[] cfgFileInfo = HwCfgFilePolicy.getDownloadCfgFile(CONFIG_FILE_DIRECTORY, CONFIG_FILE_DIRECTORY + configPath);
            if (cfgFileInfo == null) {
                LBSLog.e(TAG, false, "Both default and cota config files not exist", new Object[0]);
                return upgradePath;
            }
            String upgradeVersion = cfgFileInfo[1];
            upgradePath = cfgFileInfo[0];

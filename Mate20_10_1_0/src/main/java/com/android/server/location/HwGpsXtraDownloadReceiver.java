...
public class HwGpsXtraDownloadReceiver implements IHwGpsXtraDownloadReceiver {
    ...
    private static volatile HwGpsXtraDownloadReceiver mInstance;
    ...
    public static HwGpsXtraDownloadReceiver getInstance() {
        if (mInstance == null) {
            synchronized (HwGpsXtraDownloadReceiver.class) {
                if (mInstance == null) {
                    mInstance = new HwGpsXtraDownloadReceiver();
                }
            }
        }
        return mInstance;
    ...
    public void setXtraStringWithoutNetCheck(int netType) {
        ...
        this.xtraDownloadInterval = Long.parseLong(this.xtraString) * 3600000;
        LBSLog.i(TAG, false, " pengding:%{public}b, donwload_interval=%{public}d", Boolean.valueOf(this.mDownloadPending), Long.valueOf(this.xtraDownloadInterval));
    }
    ...
    private boolean isXtraDownloadEnable() {
        ...
        LBSLog.i(TAG, false, "xtra download enable =%{public}b", Boolean.valueOf(enable));
        return enable;
    }

...
public class HwNLPCache {
    ...
    private static HwNLPCache mInstance;
    ...
    private boolean mTriggerBySelf = false;
    ...
    public static HwNLPCache getInstance(Context context, AbstractLocationProvider.LocationProviderManager locationProviderManager) {
        ...
        synchronized (HwNLPCache.class) {
            if (mInstance == null) {
                mInstance = new HwNLPCache(context, locationProviderManager);
            }
            hwNLPCache = mInstance;
        }
        return hwNLPCache;
    }
    ...
    public void startCheckCache(boolean reportLocation) {
        if (reportLocation) {
            if (this.mTriggerBySelf) {
                this.mTriggerBySelf = false;
                return;
            }
    ...
    /* access modifiers changed from: private */
    public void handleCheckCache() {
        if (isCacheValid(this.mCachedGPSLocation, 10000)) {
            handleSendMassage(2, 0);
        } else if (isCacheValid(this.mCachedNLPLocation, MAX_REPORT_NLP_CACHE_TIME)) {

...
public class HwGnssLocationProvider implements IHwGnssLocationProvider {
    ...
    public static final boolean GNSS_ADAPT_CARD = SystemProperties.getBoolean("ro.odm.config.gnss_adapt_card", false);
    ...
    private static volatile HwGnssLocationProvider mInstance;
    ...
    /* access modifiers changed from: private */
    public GnssLocationProvider mGnssLocationProvider;
    ...
    /* access modifiers changed from: private */
    public GpsLocationProviderUtils mUtils;
    ...
    public static HwGnssLocationProvider getInstance(Context context, GnssLocationProvider gnssLocationProvider, Looper looper) {
        if (mInstance == null) {
            synchronized (LOCK) {
                if (mInstance == null) {
                    mInstance = new HwGnssLocationProvider(context, gnssLocationProvider, looper);
                }
            }
        }
        return mInstance;
    ...
    private class BroadcastHelper {
        ...
        /* access modifiers changed from: private */
        public void checkAndSetAGpsParameter(String inSuplHost) {
    ...
    public void handleGnssNavigatingStateChange(boolean start) {
        LBSLog.i(TAG, false, "handleGnssNavigateState,start : %{public}b", Boolean.valueOf(start));
        HwLog.dubaie("DUBAI_TAG_NAVIGATING_STATE", "state=" + (start ? 1 : 0));
        if (start) {
    ...
    public boolean sendExtraCommandEx(String command) {
        ...
        boolean result = hwCustGpsLocationProvider.sendPostionModeCommand(false, command);
        this.mUtils.setPositionMode(this.mGnssLocationProvider, this.mCust.setPostionMode(this.mUtils.getPositionMode(this.mGnssLocationProvider)));
        return result;
    }
        ...
        if (message == 0) {
            ...
            switch (message) {
                ...
                case 53:
                    ...
                    return;
                case UPDATE_LOWPOWER_MODE /*{ENCODED_INT: 54}*/:
    ...
    public boolean checkLowPowerMode() {
        ...
        boolean shouldEnterIdle = false;
        if (isDeviceIdle) {
            isGnssStarted = this.mUtils.getGnssStarted(this.mGnssLocationProvider);
            LocationManagerServiceUtil util = LocationManagerServiceUtil.getDefault();
            if (isGnssStarted && util != null) {
                shouldEnterIdle = util.shouldEnterIdle();
            }
            LBSLog.i(TAG, false, "checkLowPowerMode isGnssStarted: %{public}b, shouldEnterIdle: %{public}b", Boolean.valueOf(isGnssStarted), Boolean.valueOf(shouldEnterIdle));
            if (isGnssStarted) {
                try {
                    if (this.mDeviceIdleController != null && !shouldEnterIdle) {
    ...
    public void handleReportLocationEx(boolean hasLatLong, Location location) {
        ...
        if (!this.mGpsFirstFixed && hasLatLong && !"quickgps".equals(location.getProvider())) {

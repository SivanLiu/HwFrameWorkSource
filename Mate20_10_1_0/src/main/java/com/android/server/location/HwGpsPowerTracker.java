...
public class HwGpsPowerTracker {
    ...
    private class RemoveLocationHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            String pkg;
            if (3001 == msg.what && (pkg = msg.getData().getString("pkg")) != null) {
    ...
    private final class GpsAppTracker {
        ...
        private boolean noPidRunning() {
            ...
            if (str == null) {
                ...
                return false;
            } else if (HwGpsPowerTracker.PACKAGE_NAME_FUSED.equals(str) || HwGpsPowerTracker.PACKAGE_NAME_HWID.equals(this.mPkgName)) {
                LBSLog.i(HwGpsPowerTracker.TAG, false, "find app mPkgName= %{public}s", this.mPkgName);
                return false;
            } else {
                List<ActivityManager.RunningAppProcessInfo> appProcessList = ((ActivityManager) HwGpsPowerTracker.this.mContext.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG)).getRunningAppProcesses();
    ...
    public void dump(PrintWriter pw) {
        if (ENABLE_GPS_POWER_TRACK) {
            ...
            synchronized (this.mLock) {
                ...
                while (iter.hasNext()) {
                    pw.println("  " + iter.next().getValue().toString());
                }
            }
        }
    }

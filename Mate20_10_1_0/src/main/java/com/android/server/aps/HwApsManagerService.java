...
public class HwApsManagerService extends IHwApsManager.Stub {
    ...
    private final BroadcastReceiver mCarModeReceiver = new BroadcastReceiver() {
        /* class com.android.server.aps.HwApsManagerService.AnonymousClass1 */
    ...
    public boolean disableFeatures(int bitmask) {
        if (!checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "disableFeatures") || !validBitmask(bitmask)) {
            return false;
        }
        apsMask &= ~bitmask;
        return true;
    ...
    public boolean enableFeatures(int bitmask) {
        if (!checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "enableFeatures") || !validBitmask(bitmask)) {
            return false;
        }
        apsMask |= bitmask;
        return true;
    ...
    public boolean stopPackages(List<String> pkgs) {
        if (!checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "stopPackages")) {
            return false;
        }

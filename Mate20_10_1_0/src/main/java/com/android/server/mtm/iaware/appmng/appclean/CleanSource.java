...
public abstract class CleanSource {
    ...
    private static void setProperFlag(AwareProcessBlockInfo curBlock, boolean forMemory) {
        if (ProcessCleaner.CleanType.FORCESTOP.equals(curBlock.procCleanType) || ProcessCleaner.CleanType.FORCESTOP_REMOVETASK.equals(curBlock.procCleanType)) {
            curBlock.procResCleanAllow = true;
        } else if (ProcessCleaner.CleanType.FORCESTOP_ALARM.equals(curBlock.procCleanType)) {
            curBlock.procResCleanAllow = true;
    ...
    private static List<AwareProcessBlockInfo> mergeBlockInternal(List<AwareProcessBlockInfo> rawBlock, boolean forMemory, List<ProcessCleaner.CleanType> priority, ArrayMap<String, ListItem> processList) {
        ...
        while (iterator.hasNext()) {
            ...
            if (lastBlock == null) {
                ...
                lastBlock = curBlock;
            } else if (lastBlock.procPackageName == null || !lastBlock.procPackageName.equals(curBlock.procPackageName) || UserHandle.getUserId(curBlock.procUid) != UserHandle.getUserId(lastBlock.procUid)) {
            ...
            if (AppSceneMngFeature.isEnable() || !ProcessCleaner.CleanType.NONE.equals(lastBlock.procCleanType)) {
                ...
                if (lastBlock.procProcessList != null) {
                    if (isChanged && lastBlock.procCleanType.ordinal() > ProcessCleaner.CleanType.KILL_ALLOW_START.ordinal()) {
                        ...
                        sb.append(str.replaceFirst(RuleParserUtil.AppMngTag.POLICY.getDesc().toUpperCase() + ":\\d+", RuleParserUtil.AppMngTag.POLICY.getDesc().toUpperCase() + AwarenessInnerConstants.COLON_KEY + ProcessCleaner.CleanType.KILL_ALLOW_START.ordinal()));
                        sb.append(",");
    ...
    private static boolean mergeBlockWithProcessList(AwareProcessBlockInfo lastBlock, Map<String, List<AwareProcessBlockInfo>> groupBy, ArrayMap<String, ListItem> processList, List<ProcessCleaner.CleanType> priority, boolean forMemory) {
        ...
        while (iterator.hasNext()) {
            ...
            if (item == null || (lastBlock.procProcessList != null && item.getPolicy() == lastBlock.procCleanType.ordinal())) {
                ...
                AwareProcessBlockInfo tempBlock = new AwareProcessBlockInfo(AppMngConstant.CleanReason.PROCESSLIST.getCode(), procInfo.procProcInfo.mUid, procInfo, item.getPolicy(), detailedReason);
    ...
    public static boolean setSchedPriority() {
        ...
        try {
            Process.setThreadScheduler(Process.myTid(), 1073741825, 1);
            Process.setThreadGroupAndCpuset(Process.myTid(), 10);
            rtSchedSet = true;
    ...
    public static void resetSchedPriority(boolean rtSchedSet) {
        if (rtSchedSet) {
            try {
                Process.setThreadScheduler(Process.myTid(), 0, 0);
                Process.setThreadGroupAndCpuset(Process.myTid(), 2);
            } catch (IllegalArgumentException e) {

...
public class AwareAppDefaultFreeze extends CleanSource {
    ...
    private final KernelCpuUidTimeReader.KernelCpuUidUserSysTimeReader mKernelCpuUidTimeReader = new KernelCpuUidTimeReader.KernelCpuUidUserSysTimeReader(true);
    ...
    private int addFreezeAppInfos(List<AwareProcessBlockInfo> awareProcessBlockInfos, String pkgName, StringBuffer sb) {
        ...
        for (AwareProcessBlockInfo processInfo : awareProcessBlockInfos) {
            if ((processInfo.procCleanType == ProcessCleaner.CleanType.FREEZE_NOMAL || processInfo.procCleanType == ProcessCleaner.CleanType.FREEZE_UP_DOWNLOAD) && !((pkgName.equals(processInfo.procPackageName) && UserHandle.getUserId(processInfo.procUid) == AwareAppAssociate.getInstance().getCurUserId()) || (infos = processInfo.procProcessList) == null || infos.size() == 0)) {
    ...
    private class UidCpuTimeReaderCallback implements KernelCpuUidTimeReader.Callback<long[]> {

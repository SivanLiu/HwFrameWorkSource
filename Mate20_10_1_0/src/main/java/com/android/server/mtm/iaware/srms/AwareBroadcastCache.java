...
public class AwareBroadcastCache {
    ...
    public AwareBroadcastCache(String name, Object ams) {
        this.mBrName = name;
        this.mAms = ams;
        this.mSameKindsActionList.put(SmartDualCardConsts.SYSTEM_STATE_NAME_SCREEN_ON, SmartDualCardConsts.SYSTEM_STATE_NAME_SCREEN_OFF);
    }
    ...
    private void notifyPgUnFreeze(HwBroadcastRecord r, boolean notify, int pid, String pkgName) {
        if (notify) {
            LogPower.push((int) CPUFeature.MSG_EXIT_GAME_SCENE, r.getAction(), pkgName, String.valueOf(pid), new String[]{r.getCallerPackage()});
            if (AwareBroadcastDebug.getFilterDebug()) {

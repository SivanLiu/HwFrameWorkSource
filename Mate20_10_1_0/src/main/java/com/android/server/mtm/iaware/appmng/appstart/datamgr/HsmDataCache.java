...
class HsmDataCache {
    ...
    private CacheFileOper mCachedBootListFile;
    private CacheFileOper mSettingListFile;
    private final Map<String, HwAppStartupSetting> mStartupSetting = new ArrayMap();
    ...
    public HsmDataCache() {
        ...
        try {
            ...
            AwareLog.e(TAG, "Fail to make dir " + file.getPath() + " for SecurityException");
        }
        this.mCachedBootListFile = new CacheFileOper(new File(file, "appstart_cached_boot.list"));
        File settingListFile = new File(file, "appstart_cached_setting.list");
        this.mSettingListFile = new CacheFileOper(settingListFile);
    ...
    private static class WidgetUpdateInfo {
        public long updateElapse;

        public WidgetUpdateInfo(long updateElapseTime) {
            this.updateElapse = updateElapseTime;
        }
    }
    ...
    public HwAppStartupSetting getAppStartupSetting(String pkgName) {
        synchronized (this.mStartupSetting) {
            ...
            return new HwAppStartupSetting(setting);
        }
    }
    ...
    public void loadBootCache() {
        ...
        for (String bootCached : cachedList) {
            if (bootCached != null) {
                String[] bootPkgList = bootCached.split(":");
    ...
    public void updateWidgetUpdateTime(String pkgName) {
        ...
        if (updateInfo != null) {
            updateInfo.updateElapse = SystemClock.elapsedRealtime();
        }
    }
    ...
    public long getWidgetExistPkgUpdateTime(String pkgName) {
        ...
        return updateInfo.updateElapse;
    }
    ...
    public String toString() {
        ...
        if (AppStartupFeature.isAppStartupEnabled()) {
            ...
            cacheStr.append("):");
            for (Map.Entry<String, WidgetUpdateInfo> pkg : widgetPkgs.entrySet()) {
                cacheStr.append("\n  ");
                cacheStr.append(pkg.getKey());
                cacheStr.append("  sec:");
                cacheStr.append((SystemClock.elapsedRealtime() - pkg.getValue().updateElapse) / 1000);
            }
            cacheStr.append("\nBlind(");

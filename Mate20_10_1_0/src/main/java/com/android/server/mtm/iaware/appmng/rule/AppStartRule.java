...
public class AppStartRule extends Config {
    ...
    public Policy apply(String packageName, AppMngConstant.AppStartSource source, AwareAppStartStatusCache status, int tristate) {
        ...
        if (node == null || value == null || !(node.getCurrentType() instanceof RuleParserUtil.AppStartTag) || (type = (RuleParserUtil.AppStartTag) node.getCurrentType()) == null || !type.equals(RuleParserUtil.AppStartTag.POLICY)) {
            return new AppStartPolicy(packageName, 0, AppMngConstant.AppStartReason.DEFAULT.getDesc());
        }
        if (value.getHwStop() == -1 || !AwareIntelligentRecg.getInstance().isPkgHasHwStopFlag(UserHandle.getUserId(status.cacheUid), packageName)) {

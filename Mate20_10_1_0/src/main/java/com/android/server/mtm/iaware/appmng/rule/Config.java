...
public abstract class Config {
    ...
    private void printRuleNodeValue(PrintWriter pw, RuleNode node, int depth, RuleNode.XmlValue value) {
        if (value.isString()) {
            pw.println("|" + getDepthString(depth) + node.getCurrentType() + AwarenessInnerConstants.COLON_KEY + value.getStringValue());
        } else {
            pw.println("|" + getDepthString(depth) + node.getCurrentType() + AwarenessInnerConstants.COLON_KEY + value.getIntValue());
        }

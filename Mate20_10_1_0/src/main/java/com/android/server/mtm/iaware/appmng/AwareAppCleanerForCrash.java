...
public class AwareAppCleanerForCrash extends AwareAppCleaner {
    ...
    private void addHwStop(AwareProcessBlockInfo procGroup, int count) {
        if (procGroup != null && count != 0 && ProcessCleaner.CleanType.FORCESTOP.equals(procGroup.procCleanType)) {
            AwareIntelligentRecg.getInstance().setHwStopFlag(UserHandle.getUserId(procGroup.procUid), procGroup.procPackageName, true);
        }
    }
}

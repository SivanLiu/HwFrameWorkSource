...
public class AwareBroadcastDumpRadar extends BigDataSupervisor {
    ...
    public void trackBrFlowDetail(boolean enqueue, boolean isProxyed, boolean flowCtrlStarted, String packageName, String action) {
        ...
        if (enqueue) {
            ...
            synchronized (this.mCacheBrNumList) {
                if (this.mCacheBrNumList.size() < 10000) {
                    this.mCacheBrNumList.add((flowCtrlStarted ? 1 : 0) + "," + packageName + "," + action);
                }
            }
        }
    }
    ...
    /* JADX INFO: Multiple debug info for r7v1 java.lang.String: [D('packageName' java.lang.String), D('e' java.lang.NumberFormatException)] */
    ...
    private void getBrDetailData(StringBuilder sb, boolean clear) {
        Map<String, Map<String, TrackBrData>> map = this.mTrackDataMap;
        if (map != null && isBetaUser && sb != null) {
            synchronized (map) {
    ...
    private static class TrackBrData {
        ...
        public String toJsonStr() {
            ...
            try {
                ...
                jsonObj.put(AwarenessConstants.DATA_ACTION_TYPE, this.brAction);
            } catch (JSONException e) {
        ...
        if (statData.size() != 0) {
            for (Map.Entry<String, String> entry : statData.entrySet()) {
                ...
                try {
                    ...
                    jsonObj.put("start", this.mStartTimeBrEx);
                    jsonObj.put("end", System.currentTimeMillis());
                    jsonObj.put(AwarenessConstants.DATA_ACTION_TYPE, entry.getKey());

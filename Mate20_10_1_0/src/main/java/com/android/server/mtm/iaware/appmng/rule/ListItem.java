    ...
    public void dump(PrintWriter pw) {
        ...
        if (arrayMap != null) {
            for (Map.Entry<String, Integer> entry : arrayMap.entrySet()) {
                pw.println("    policy = " + entry.getKey() + AwarenessInnerConstants.COLON_KEY + entry.getValue() + ", weight = " + this.mWeight);
            }
            ...
            if (arrayMap2 != null) {
                for (Map.Entry<String, Config> entry2 : arrayMap2.entrySet()) {
                    ...
                    if (config != null) {
                        pw.println("     rules = " + entry2.getKey() + AwarenessInnerConstants.COLON_KEY);
                        config.dump(pw);
                    }
                }
            }

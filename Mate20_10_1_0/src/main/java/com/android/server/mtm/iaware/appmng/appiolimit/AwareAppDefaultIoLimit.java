...
public class AwareAppDefaultIoLimit extends CleanSource {
    ...
    public void doLimitIO(String pkgName, AppMngConstant.AppIoLimitSource config) {
        if (this.mIsExecutorAlive.compareAndSet(false, true)) {
            new Thread(new Runnable(pkgName, config) {
                /* class com.android.server.mtm.iaware.appmng.appiolimit.$$Lambda$AwareAppDefaultIoLimit$ngJwX9CnfRJxXCVzrWuQd31QxYU */
                ...
                public final void run() {
                    AwareAppDefaultIoLimit.this.lambda$doLimitIO$0$AwareAppDefaultIoLimit(this.f$1, this.f$2);
                }
            }, "iaware.iolimit").start();
        }
    }

    public /* synthetic */ void lambda$doLimitIO$0$AwareAppDefaultIoLimit(String pkgName, AppMngConstant.AppIoLimitSource config) {
    ...
    private boolean checkNeedDiffIoLimit(long curTime) {
        if (!APPIoLimitFeature.isIAware5Enable()) {
            return false;
        }
    ...
    private void doIOBlockGroup(List<AwareProcessBlockInfo> awareProcessBlockInfos, String pkgName, Map<Integer, Integer> iopids, Map<Integer, Integer> heavyIopids, SparseSet iolimitUids) {
        ...
        for (AwareProcessBlockInfo processInfo : awareProcessBlockInfos) {
            if (processInfo.procCleanType == ProcessCleaner.CleanType.IOLIMIT && ((!pkgName.equals(processInfo.procPackageName) || UserHandle.getUserId(processInfo.procUid) != AwareAppAssociate.getInstance().getCurUserId()) && (infos = processInfo.procProcessList) != null && !infos.isEmpty())) {

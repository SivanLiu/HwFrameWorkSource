...
public class ConnectivityController extends AwareStateController {
    ...
    private ConnectivityController(AwareStateChangedListener stateChangedListener, Context context, Object lock) {
        ...
        if (context != null) {
            ...
            this.mContext.registerReceiverAsUser(this.mConnectivityReceiver, UserHandle.SYSTEM, new IntentFilter(SmartDualCardConsts.ACTION_CONNECTIVITY_CHANGE), null, null);
    ...
    private boolean isNetworkSatisfied(String netValue, NetworkInfo info) {
        if ("MOBILEDATACON".equals(netValue)) {
            ...
            return true;
        } else if (!AwareJobSchedulerConstants.NETWORK_STATUS_ALL.equals(netValue)) {
            return false;
        } else {
            if ((info.getType() == 0 || info.getType() == 1) && info.isConnected()) {
                return true;
            }
            return false;
        }

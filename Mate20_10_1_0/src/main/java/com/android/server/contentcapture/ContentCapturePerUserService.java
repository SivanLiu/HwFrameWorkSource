...
public final class ContentCapturePerUserService extends AbstractPerUserSystemService<ContentCapturePerUserService, ContentCaptureManagerService> implements RemoteContentCaptureService.ContentCaptureServiceCallbacks {
    ...
    /* access modifiers changed from: private */
    @GuardedBy({"mLock"})
    public final ArrayMap<String, ArraySet<ContentCaptureCondition>> mConditionsByPkg = new ArrayMap<>();
    @GuardedBy({"mLock"})
    ...
    /* access modifiers changed from: private */
    @GuardedBy({"mLock"})
    public final SparseArray<ContentCaptureServerSession> mSessions = new SparseArray<>();
    @GuardedBy({"mLock"})
    ...
    private final class ContentCaptureServiceRemoteCallback extends IContentCaptureServiceCallback.Stub {
        ...
        public void setContentCaptureWhitelist(List<String> packages, List<ComponentName> activities) {
            ...
            if (((ContentCaptureManagerService) ContentCapturePerUserService.this.mMaster).verbose) {
                String access$200 = ContentCapturePerUserService.TAG;
                StringBuilder sb = new StringBuilder();
                sb.append("setContentCaptureWhitelist(");
                if (packages == null) {
                    str = "null_packages";
                } else {
                    str = packages.size() + " packages";
                }
                sb.append(str);
                sb.append(", ");
                if (activities == null) {
                    str2 = "null_activities";
                } else {
                    str2 = activities.size() + " activities";
                }
                sb.append(str2);
                sb.append(") for user ");
                sb.append(ContentCapturePerUserService.this.mUserId);
                Slog.v(access$200, sb.toString());
            }

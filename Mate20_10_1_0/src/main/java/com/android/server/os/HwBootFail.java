...
public class HwBootFail {
    ...
    public static void setBootStage(int bootStageValue) {
        if (10000 <= Binder.getCallingUid()) {
            ...
            if (83886088 == bootStageValue || 83886090 == bootStageValue) {
                try {
                    nativeDisableTimer();
                } catch (Exception e) {
                    Slog.e(TAG, "failed to write to /dev/hw_bfm");
                }
            } else if (83886089 == bootStageValue || 83886091 == bootStageValue) {
                try {
                    nativeEnableTimer();
                } catch (Exception e2) {
                    Slog.e(TAG, "failed to write to /dev/hw_bfm");
                ...
                if (bootStageValue == Integer.MAX_VALUE) {
                    try {
    ...
    public static void bootFailError(int errNo, int suggestedRcvMethod, String logFilePath) {
        String str = "0x" + Integer.toHexString(errNo) + HwLog.PREFIX + suggestedRcvMethod + HwLog.PREFIX + logFilePath;
        if (10000 <= Binder.getCallingUid()) {
            Slog.e(TAG, "permission not allowed. uid = " + Binder.getCallingUid());
            return;
        }

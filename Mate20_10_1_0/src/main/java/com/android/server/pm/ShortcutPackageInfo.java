...
class ShortcutPackageInfo {
    ...
    public void saveToXml(ShortcutService s, XmlSerializer out, boolean forBackup) throws IOException {
        ...
        for (int i = 0; i < this.mSigHashes.size(); i++) {
            ...
            ShortcutService.writeAttr(out, ATTR_SIGNATURE_HASH, Base64.getEncoder().encodeToString(this.mSigHashes.get(i)));
    ...
    public void loadFromXml(XmlPullParser parser, boolean fromBackup) throws IOException, XmlPullParserException {
        int outerDepth;
        long versionCode = ShortcutService.parseLongAttribute(parser, ATTR_VERSION, -1);
        ...
        long backupSourceVersion = ShortcutService.parseLongAttribute(parser, ATTR_BACKUP_SOURCE_VERSION, -1);

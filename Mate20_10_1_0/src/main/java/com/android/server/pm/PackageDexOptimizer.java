...
public class PackageDexOptimizer {
    ...
    private int performDexOptLI(PackageParser.Package pkg, String[] targetInstructionSets, CompilerStats.PackageStats packageStats, PackageDexUsage.PackageUseInfo packageUseInfo, DexoptOptions options) {
        ...
        char c2 = 65535;
        if (sharedGid4 == -1) {
            Slog.wtf(TAG, "Well this is awkward; package " + r14.applicationInfo.name + " had UID " + r14.applicationInfo.uid, new Throwable());
            sharedGid = 9999;
        } else {
        ...
        boolean mapleProcess = ZygoteInit.sIsMygote;
        if (mapleProcess) {
            MplDexOptAdaptor.getInstance().dexOptParamPrepare();
        }
        int result3 = 0;
        int i6 = 0;
        while (i6 < paths3.size()) {
            if (pathsWithCode3[i6]) {
                if (classLoaderContexts3[i6] != null) {
                    String path2 = paths3.get(i6);
                    if (options.getSplitName() == null || options.getSplitName().equals(new File(path2).getName())) {
                        String str = null;
                        String profileName2 = ArtManager.getProfileName(i6 == 0 ? null : r14.splitNames[i6 - 1]);
                        ...
                        if (!options.isDexoptAsSharedLibrary()) {
                            if (!packageUseInfo.isUsedByOtherApps(path2)) {
                                ...
                                String compilerFilter2 = packageDexOptimizer.getRealCompilerFilter(r14.applicationInfo, options.getCompilerFilter(), isUsedByOtherApps2);
                                ...
                                while (result < length) {
                                    ...
                                    if (mapleProcess) {
                                        MplDexOptAdaptor.getInstance().dexOptParamAdd(path2, dexCodeIsa, compilerFilter2, classLoaderContexts3[i6], profileName2, dexMetadataPath, profileUpdated, options.isDowngrade(), sharedGid, options.getCompilationReason(), dexoptFlags);
                                        i4 = result;
                                        ...
                                        dexCodeInstructionSets2 = dexCodeInstructionSets3;
                                        sharedLibraries2 = sharedLibraries3;
                                        int newResult = dexOptPath(pkg, path, dexCodeIsa, compilerFilter2, profileUpdated, classLoaderContexts3[i6], dexoptFlags, sharedGid, packageStats, options.isDowngrade(), profileName, dexMetadataPath, options.getCompilationReason(), getDexoptNeeded(path2, dexCodeIsa, compilerFilter2, classLoaderContexts3[i6], profileUpdated, options.isDowngrade()));
                                        result2 = result4;
                        ...
                        z = true;
                        boolean isUsedByOtherApps22 = z;
                        String compilerFilter22 = packageDexOptimizer.getRealCompilerFilter(r14.applicationInfo, options.getCompilerFilter(), isUsedByOtherApps22);
                    ...
                    throw new IllegalStateException("Inconsistent information in the package structure. A split is marked to contain code but has no dependency listed. Index=" + i6 + " path=" + paths3.get(i6));
                }
            }
            i = i6;
            classLoaderContexts = classLoaderContexts3;

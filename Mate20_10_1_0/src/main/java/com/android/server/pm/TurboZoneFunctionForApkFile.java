...
public class TurboZoneFunctionForApkFile {
    ...
    public static void setTurboZoneKeyFileFlag(FileDescriptor targetFd, Long lengthBytes, PackageInstaller.SessionParams params) {
        ...
        int flagTurboZoneEnable = nativeGetTzEnable();
        Slog.i(TAG, "TurboZone: apk install processing: tzenable " + flagTurboZoneEnable + "\n");
        if (flagTurboZoneEnable > 0 && (params.installFlags & 16) != 0) {
            ...
            boolean z = true;
            boolean z2 = (lengthBytes.longValue() > 0) & (freeBlocksBytes > 0);
            if (lengthBytes.longValue() >= freeBlocksBytes) {
                z = false;
            }
            if (!z2 || !z) {

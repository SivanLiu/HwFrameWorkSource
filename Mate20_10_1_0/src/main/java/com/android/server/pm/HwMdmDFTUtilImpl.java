...
public class HwMdmDFTUtilImpl {
    ...
    public static void handleMdmDftUploadEvent(int type, HwMdmDFT hwMdmDFT) {
        ...
        if (hwMdmDFT != null) {
            switch (type) {
                case HwMdmDFTConst.EID_MDM_DFT_CERT_PARSE_SUCCESS:
                    ...
                    return;
                case HwMdmDFTConst.EID_MDM_DFT_SET_DEVICEOWNER_SUCCESS:
                    ...
                    return;
                case HwMdmDFTConst.EID_MDM_DFT_SET_WORKPROFILE_SUCCESS:
    ...
    private static String getSha256FingerData(byte[] cert) {
        try {
            return byte2HexFormatted(MessageDigest.getInstance(HwSecDiagnoseConstant.MALAPP_APK_SHA256).digest(((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(cert))).getEncoded()));

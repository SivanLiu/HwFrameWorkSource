...
public class GestureDataTracker {
    ...
    public static IDecision decisionApi = null;
    ...
    private static boolean shouldReportDecision(Context context) {
        if (context == null || Settings.Secure.getIntForUser(context.getContentResolver(), KEY_SUGGESTION_STATE, -1, 0) == 0) {
            return false;
        }
        return true;
    }
    ...
    private void bindService(Context context) {
        if (decisionApi == null) {
            ...
            try {
                isBound = context.bindServiceAsUser(actionService, decisionConnection, 1, UserHandle.CURRENT);

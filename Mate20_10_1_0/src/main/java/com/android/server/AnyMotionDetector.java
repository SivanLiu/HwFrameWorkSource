...
public class AnyMotionDetector {
    ...
    private final SensorEventListener mListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (status != -1) {
                ...
                boolean unused = AnyMotionDetector.this.mWakelockTimeoutIsActive = false;
        ...
        public void run() {
            synchronized (AnyMotionDetector.this.mLock) {
                if (AnyMotionDetector.this.mMeasurementTimeoutIsActive) {
                    boolean unused = AnyMotionDetector.this.mMeasurementTimeoutIsActive = false;
                    int status = AnyMotionDetector.this.stopOrientationMeasurementLocked();
                    if (status != -1) {
                        AnyMotionDetector.this.mHandler.removeCallbacks(AnyMotionDetector.this.mWakelockTimeout);
                        boolean unused2 = AnyMotionDetector.this.mWakelockTimeoutIsActive = false;
                        AnyMotionDetector.this.mCallback.onAnyMotionResult(status);
                    }
                }
            }
        ...
        public void run() {
            synchronized (AnyMotionDetector.this.mLock) {
                if (AnyMotionDetector.this.mSensorRestartIsActive) {
                    boolean unused = AnyMotionDetector.this.mSensorRestartIsActive = false;
    ...
    /* access modifiers changed from: private */
    public final Runnable mWakelockTimeout = new Runnable() {
    ...
    /* access modifiers changed from: private */
    public boolean mWakelockTimeoutIsActive;

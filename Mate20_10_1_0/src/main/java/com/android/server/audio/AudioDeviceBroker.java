...
public final class AudioDeviceBroker {
    ...
    public boolean setSpeakerphoneOn(boolean on, String eventSource) {
        boolean z;
        Log.i(TAG, "setSpeakerphoneOn: " + on + HwLog.PREFIX + eventSource);
        synchronized (this.mDeviceStateLock) {
    ...
    public void setBluetoothScoOn(boolean on, String eventSource) {
        Log.i(TAG, "setBluetoothScoOnInt: " + on + HwLog.PREFIX + eventSource);
        synchronized (this.mDeviceStateLock) {
            if (on) {
    ...
    private class BrokerThread extends Thread {
        ...
        public void run() {
            ...
            synchronized (AudioDeviceBroker.this) {
                BrokerHandler unused = AudioDeviceBroker.this.mBrokerHandler = new BrokerHandler();
    ...
    public class BrokerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case AudioDeviceBroker.MSG_L_BT_SERVICE_CONNECTED_PROFILE_A2DP /*{ENCODED_INT: 23}*/:
                    synchronized (AudioDeviceBroker.this.mDeviceStateLock) {
                        AudioDeviceBroker.this.mBtHelper.onA2dpProfileConnected((BluetoothA2dp) msg.obj);
                    }
                    break;
                case AudioDeviceBroker.MSG_L_BT_SERVICE_CONNECTED_PROFILE_A2DP_SINK /*{ENCODED_INT: 24}*/:
                    synchronized (AudioDeviceBroker.this.mDeviceStateLock) {
                        AudioDeviceBroker.this.mBtHelper.onA2dpSinkProfileConnected((BluetoothProfile) msg.obj);
                    }
                    break;
                case AudioDeviceBroker.MSG_L_BT_SERVICE_CONNECTED_PROFILE_HEARING_AID /*{ENCODED_INT: 25}*/:
                    synchronized (AudioDeviceBroker.this.mDeviceStateLock) {
                ...
                case AudioDeviceBroker.MSG_L_A2DP_DEVICE_CONNECTION_CHANGE_EXT /*{ENCODED_INT: 27}*/:
                    ...
                    break;
                case AudioDeviceBroker.MSG_L_HEARING_AID_DEVICE_CONNECTION_CHANGE_EXT /*{ENCODED_INT: 28}*/:
    ...
    /* access modifiers changed from: private */
    public String getPartAddress(BluetoothDevice device) {
        if (device == null) {
            return "";
        }

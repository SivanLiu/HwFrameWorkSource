...
public class ConfigUpdateInstallReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        new Thread() {
            /* class com.android.server.updates.ConfigUpdateInstallReceiver.AnonymousClass1 */
    ...
    public void writeUpdate(File dir, File file, byte[] content) throws IOException {
        ...
        try {
            ...
            parent.mkdirs();
            if (parent.exists()) {
                File tmp2 = File.createTempFile("journal", "", dir);
                tmp2.setReadable(true, false);
                FileOutputStream out = new FileOutputStream(tmp2);
                out.write(content);
                out.getFD().sync();
                if (tmp2.renameTo(file)) {
                    tmp2.delete();

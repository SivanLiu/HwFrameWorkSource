        ...
        synchronized (this.mLock) {
            ...
            if (result) {
                connection.setOnFirstFrameCapturedLocked(new Runnable() {
                    ...
                    public void run() {
                        boolean unused = hardwareImpl.stopCapture(config);
                    }
                });
            }
            return result;
                    ...
                    if (status != 1) {
                        if (status == 2) {
                            if (!TvInputHardwareManager.this.mHdmiDeviceList.remove(findHdmiDeviceInfo(deviceInfo.getId()))) {
                                String access$900 = TvInputHardwareManager.TAG;
                                Slog.w(access$900, "The list doesn't contain " + deviceInfo + "; ignoring.");
                                return;
                            }
                            messageType = 5;
                            obj = deviceInfo;
                            ...
                            try {
                                if (!TvInputHardwareManager.this.mHdmiDeviceList.remove(findHdmiDeviceInfo(deviceInfo.getId()))) {
                                    ...
                                    Slog.w(access$9002, "The list doesn't contain " + deviceInfo + "; ignoring.");
                                    return;
                                }
                        ...
                        String access$9003 = TvInputHardwareManager.TAG;
                        Slog.w(access$9003, "The list already contains " + deviceInfo + "; ignoring.");
                        return;
                    }

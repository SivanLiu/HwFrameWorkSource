...
public final class TvInputHal implements Handler.Callback {
    ...
    public int addOrUpdateStream(int deviceId, Surface surface, TvStreamConfig streamConfig) {
        synchronized (this.mLock) {
            ...
            if (this.mStreamConfigGenerations.get(deviceId, 0) != streamConfig.getGeneration()) {
                return -2;
            }
            if (nativeAddOrUpdateStream(this.mPtr, deviceId, streamConfig.getStreamId(), surface) == 0) {
                return 0;
            }
            return -3;
        }
    ...
    public int removeStream(int deviceId, TvStreamConfig streamConfig) {
        synchronized (this.mLock) {
            ...
            if (nativeRemoveStream(this.mPtr, deviceId, streamConfig.getStreamId()) == 0) {
                return 0;
            }
            return -3;
        }

...
public class HwAddViewHelper {
    ...
    public void setAddviewPermission(String pkgName, boolean value, int userId, int uid) {
        if (pkgName != null) {
            ...
            if (appOpsManager == null) {
                Log.e(TAG, "setAddviewPermission AppOps is null");
                return;
            }
            appOpsManager.setMode("android:system_alert_window", uid, pkgName, opMode);
            Log.i(TAG, pkgName + " setOp complete");
            if (HwPermDbAdapter.getInstance(this.mContext).checkHwPermCode(pkgName, 17179869184L, userId) != 0) {
                if (value) {
                    operation = 1;
                }
                Log.v(TAG, "setAddviewPermission: " + pkgName + " operation: " + operation);
    ...
    public int checkAddviewPermission(String pkgName, int uid) {
        ...
        if (addViewValue != 1 && blackListValue != 1) {
            Log.v(TAG, pkgName + " doesn't apply addview permission and is not in blacklist");
            return -1;

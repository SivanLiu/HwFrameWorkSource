...
public class ModifyPassWord {
    ...
    public static boolean modifyPrivSpacePw(String newPassword) {
        ...
        PublicKey enPrivSpaceValuesPublicKey = EncryptUtils.getPublicKey(publicKey);
        if (enPrivSpaceValuesPublicKey == null) {
            Log.e(TAG, "getPublicKey failed");
            return false;
        }
        byte[] IvKeyVales = StringUtils.byteMerger(DeviceEncryptUtils.deviceEncode(EncryptUtils.encryptData(newPassword.getBytes(Charset.forName("UTF-8")), enPrivSpaceValuesPublicKey)), publicKey);

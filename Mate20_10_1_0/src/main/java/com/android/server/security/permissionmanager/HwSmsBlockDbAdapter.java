...
public class HwSmsBlockDbAdapter {
    ...
    private HwSmsBlockDbAdapter(Context context) {
        Log.v(LOG_TAG, "create HwSmsBlockDbAdapter");
    ...
    public void initSmsBlockData(int userId, ArrayList<ContentValues> contentValuesList) {
        ...
        if (contentValuesList != null) {
            try {
                if (!contentValuesList.isEmpty()) {
                    ...
                    Log.v(LOG_TAG, "initSmsBlockData complete");
                    return;
                }
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "initSmsBlockData SQLiteException for " + tableName);
                return;
            }
        }
        Log.e(LOG_TAG, "initSmsBlockData contentValuesList is empty!");
    }

    public void hotaUpdateSmsBlockData(int userId, ArrayList<ContentValues> contentValuesList) {
        String tableName = getSmsBlockTableName(userId);
        if (contentValuesList != null) {
            try {
                if (!contentValuesList.isEmpty()) {
                    ...
                    for (int i = 0; i < listSize; i++) {
                        ...
                        if (nameSet != null) {
                            if (!nameSet.isEmpty()) {
                                ...
                                if (preDefinedSmsApps.contains(pkgName)) {
                                    Log.v(LOG_TAG, pkgName + " is preDefined");
                                } else {
                        ...
                        Log.e(LOG_TAG, "hotaUpdateSmsBlockData name set is empty");
                    }
                    Log.v(LOG_TAG, "hotaUpdateSmsBlockData complete");
                    return;
                }
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "hotaUpdateSmsBlockData SQLiteException for " + tableName);
                return;
            }
        }
        Log.e(LOG_TAG, "hotaUpdateSmsBlockData contentValuesList is empty!");
    }

    public void replaceSmsBlockAuthResult(String pkgName, int result, int userId) {
        Cursor cursor = null;
        Log.v(LOG_TAG, "replaceSmsBlockAuthResult:" + pkgName + ", " + result + " for user " + userId);
        if (!validParameterForSmsAuth(pkgName, result)) {
            Log.e(LOG_TAG, "replaceSmsBlockAuthResult parameter invalid");
            return;
        }
        try {
            ...
            Log.v(LOG_TAG, pkgName + " is predefined SMS, can't replace");
            CursorHelper.closeCursor(cursor);
        } catch (SQLiteException e) {
            Log.e(LOG_TAG, "replaceSmsBlockAuthResult SQLiteException");
        } catch (Throwable th) {
    ...
    public int checkSmsBlockAuthResult(String pkgName, int userId) {
            ...
            try {
                if (cursor.getCount() != 0) {
                    if (cursor.moveToFirst()) {
                        ...
                        Log.v(LOG_TAG, "checkSmsBlockAuthResult, pkgName: " + pkgName + ", smsPermission:" + smsPermission);
                        return smsPermission;
                    }
                    Log.e(LOG_TAG, "pkg is null, no sms block value, pkgName:" + pkgName);
                    CursorHelper.closeCursor(cursor);
                    return 2;
                }
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "checkHwPerm SQLiteException");
                return 2;
            } finally {
                CursorHelper.closeCursor(cursor);
            }
        }
        Log.e(LOG_TAG, "cursor is null, no sms block value, pkgName:" + pkgName);
        CursorHelper.closeCursor(cursor);
        return 2;
    }

    public void addSmsBlockAuthResult(String pkgName, int result, int userId) {
        Log.v(LOG_TAG, "addSmsBlockAuthResult:" + pkgName + ", " + result + " for user " + userId);
        if (validParameterForSmsAuth(pkgName, result)) {
            try {
                ContentValues values = new ContentValues();
                values.put("packageName", pkgName);
                ...
                Log.v(LOG_TAG, "addSmsBlockAuthResult complete");
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "addSmsBlockAuthResult SQLiteException");
            }
        }
    }

    public void deleteSmsBlockAuthResult(String pkgName, int userId) {
        Log.v(LOG_TAG, "deleteSmsBlockAuthResult:" + pkgName);
        try {
            Cursor cursor = this.mHwPermDbHelper.query(getSmsBlockTableName(userId), "packageName= ?", new String[]{pkgName});
            if (CursorHelper.checkCursorValid(cursor) && cursor.moveToFirst() && cursor.getInt(cursor.getColumnIndex(PermConst.PRE_DEFINED_SMS_CONFIG)) == 1) {
                Log.v(LOG_TAG, pkgName + " is predefined SMS, can't delete");
            }
            ...
            try {
                if (cursor.getCount() != 0) {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        Log.v(LOG_TAG, "preDefinedSmsApp, pkgName: " + pkgName);
                        preDefinedSmsApps.add(pkgName);
                    }
                    return preDefinedSmsApps;
                }
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "getPreDefinedSmsApps SQLiteException");
                return preDefinedSmsApps;
            } finally {
                CursorHelper.closeCursor(cursor);
            }
        }
        Log.v(LOG_TAG, "cursor is null, can't get pre defined sms apps");
        CursorHelper.closeCursor(cursor);
        return preDefinedSmsApps;
    }
    ...
    public List<String> getUserDefinedSmsApps() {
        ...
        Log.v(LOG_TAG, "cursor is null, can't get user defined sms apps");
        CursorHelper.closeCursor(cursor);
        return userDefinedSmsApps;
    }
    ...
    public HashMap<String, Integer> getAllSmsBlockSetting(int userId) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "getAllAppsPermInfo error");
        } catch (Throwable th) {
            CursorHelper.closeCursor(cursor);
            throw th;
        }
        ...
        if (cursor != null) {
            try {
                if (cursor.getCount() != 0) {
                    ...
                    while (cursor.moveToNext()) {
                        String pkgName = cursor.getString(packageNameIndex);
                        Log.v(LOG_TAG, "deletePreDefinedSmsApps, pkgName: " + pkgName);
                        this.mHwPermDbHelper.delete(getSmsBlockTableName(userId), "packageName= ?", new String[]{pkgName});
                    }
                    return;
                }
            } catch (SQLiteException e) {
                Log.e(LOG_TAG, "deletePreDefinedSmsApps SQLiteException");
                return;
            } finally {
                CursorHelper.closeCursor(cursor);
            }
        }
        Log.v(LOG_TAG, "deletePreDefinedSmsApps cursor is null");
        CursorHelper.closeCursor(cursor);
    }
}

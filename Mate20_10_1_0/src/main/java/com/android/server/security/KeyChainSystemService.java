...
public class KeyChainSystemService extends SystemService {
    ...
    private final BroadcastReceiver mPackageReceiver = new BroadcastReceiver() {
        /* class com.android.server.security.KeyChainSystemService.AnonymousClass1 */
    ...
    public void startServiceInBackgroundAsUser(Intent intent, UserHandle user) {
        if (intent.getComponent() != null) {
            ((DeviceIdleController.LocalService) LocalServices.getService(DeviceIdleController.LocalService.class)).addPowerSaveTempWhitelistApp(Process.myUid(), intent.getComponent().getPackageName(), 30000, user.getIdentifier(), false, "keychain");

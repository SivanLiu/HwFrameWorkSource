...
public abstract class PreBootBroadcaster extends IIntentReceiver.Stub {
    ...
    private Handler mHandler = new Handler(UiThread.get().getLooper(), null, true) {
        /* class com.android.server.am.PreBootBroadcaster.AnonymousClass1 */
        ...
        public void handleMessage(Message msg) {
            ...
            int i = msg.what;
            if (i == 1) {
                CharSequence title = context.getText(17039597);
                ...
                notifManager.notifyAsUser(PreBootBroadcaster.TAG, 13, new Notification.Builder(PreBootBroadcaster.this.mService.mContext, SystemNotificationChannels.UPDATES).setSmallIcon(17303544).setWhen(0).setOngoing(true).setTicker(title).setColor(context.getColor(17170460)).setContentTitle(title).setContentIntent(contentIntent).setVisibility(1).setProgress(max, index, false).build(), UserHandle.of(PreBootBroadcaster.this.mUserId));
            } else if (i == 2) {
    ...
    public void sendNext() {
        ...
        if (this.mIndex >= this.mTargets.size()) {
            ...
            if (this.mProgress != null) {
                ...
                this.mProgress.setProgress(this.mIndex, this.mTargets.size(), this.mService.mContext.getString(17039591, label));
            }

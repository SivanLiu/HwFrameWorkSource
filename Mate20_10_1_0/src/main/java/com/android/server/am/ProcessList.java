...
public final class ProcessList {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r31v0, resolved type: com.android.server.am.ProcessRecord */
    /* JADX DEBUG: Multi-variable search result rejected for r15v0, resolved type: com.android.server.am.ProcessList */
    /* JADX DEBUG: Multi-variable search result rejected for r15v1, resolved type: com.android.server.am.ProcessRecord */
    /* JADX DEBUG: Multi-variable search result rejected for r15v2, resolved type: com.android.server.am.ProcessList */
    /* JADX DEBUG: Multi-variable search result rejected for r15v6, resolved type: com.android.server.am.ProcessList */
    /* JADX DEBUG: Multi-variable search result rejected for r15v8, resolved type: com.android.server.am.ProcessRecord */
    /* JADX DEBUG: Multi-variable search result rejected for r15v9, resolved type: com.android.server.am.ProcessRecord */
    /* JADX DEBUG: Multi-variable search result rejected for r15v10, resolved type: com.android.server.am.ProcessRecord */
    /* JADX DEBUG: Multi-variable search result rejected for r15v11, resolved type: com.android.server.am.ProcessRecord */
    ...
    private Process.ProcessStartResult startProcess(HostingRecord hostingRecord, String entryPoint, ProcessRecord app, int uid, int[] gids, int runtimeFlags, int mountExternal, String seInfo, String requiredAbi, String instructionSet, String invokeWith, long startTime) {
        ...
        try {
            ...
            if (hostingRecord.usesWebviewZygote()) {
                startResult = Process.startWebView(entryPoint, app.processName, uid, uid, gids, runtimeFlags, mountExternal, app.info.targetSdkVersion, seInfo, requiredAbi, instructionSet, app.info.dataDir, null, app.info.packageName, entryPointArgs2);
            } else if (hostingRecord.usesAppZygote()) {
                startResult = createAppZygoteForProcessIfNeeded(app).getProcess().start(entryPoint, app.processName, uid, uid, gids, runtimeFlags, mountExternal, app.info.targetSdkVersion, seInfo, requiredAbi, instructionSet, app.info.dataDir, (String) null, app.info.packageName, false, entryPointArgs2);
            } else {
                ...
                try {
                    try {
                        startResult = Process.start(entryPoint, app.processName, uid, this.mService.mHwAMSEx.changeGidIfRepairMode(uid, app.processName), gids, runtimeFlags3, mountExternal, app.info.targetSdkVersion, seInfo, requiredAbi, instructionSet, app.info.dataDir, invokeWith, app.info.packageName, entryPointArgs2);
                    } catch (Throwable th) {
                        startResult = th;
                        Trace.traceEnd(64);
                        throw startResult;
            ...
            if (startResult != null) {
                HwFrameworkFactory.getHwBehaviorCollectManager().sendEvent(3, app.uid, startResult.pid, app.processName, (String) null);
            }
    ...
    public final ProcessRecord newProcessRecordLocked(ApplicationInfo info, String customProcess, boolean isolated, int isolatedUid, HostingRecord hostingRecord) {
        ...
        if (this.mService.mHwAMSEx.shouldPreventStartProcess(proc, userId)) {
            return null;
        }
        int uid = info.uid;
        if (isolated) {
    ...
    public int updateLruProcessInternalLocked(ProcessRecord app, long now, int index, int lruSeq, String what, Object obj, ProcessRecord srcApp) {
        ...
        int lrui = this.mLruProcesses.lastIndexOf(app);
        if (lrui < 0) {
            Slog.wtf("ActivityManager", "Adding dependent process " + app + " not on LRU list: " + what + HwLog.PREFIX + obj + " from " + srcApp);
            return index;
        } else if (lrui >= index) {
            return index;
        } else {

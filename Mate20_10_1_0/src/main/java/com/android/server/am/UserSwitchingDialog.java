...
class UserSwitchingDialog extends AlertDialog implements ViewTreeObserver.OnWindowShownListener {
    ...
    public void inflateContent() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(17367338, (ViewGroup) null);
        String viewMessage2 = null;
        if (UserManager.isSplitSystemUser() && this.mNewUser.id == 0) {
            viewMessage = res.getString(17041436, this.mOldUser.name);
            ...
            if (viewMessage2 == null) {
                viewMessage = res.getString(17041439, this.mNewUser.name);
            } else {
                viewMessage = viewMessage2;
            }
        } else if (this.mOldUser.isDemo()) {
            viewMessage = res.getString(17039978);
        } else {
            viewMessage = res.getString(17039979);
        }

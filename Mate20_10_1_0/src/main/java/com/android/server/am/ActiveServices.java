...
public class ActiveServices {
        ...
        if (fgRequired) {
            ...
            if (mode != 0) {
                ...
                if (mode == 1) {
                    ...
                    if (str == null || str.equals(r.name.getPackageName()) || ((str.equals(PackageManagerService.PLATFORM_PACKAGE_NAME) && ACTION_HWPUSH_MSG_EVENT.equals(service.getAction())) || this.mAm.getUidState(callingUid) == 2 || (pgm = (PGManagerInternal) LocalServices.getService(PGManagerInternal.class)) == null || !pgm.isServiceProxy(r.name, null))) {
    ...
    public boolean stopServiceTokenLocked(ComponentName className, IBinder token, int startId) {
        if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
            Slog.v(ActivityManagerService.TAG, "stopServiceToken: " + className + HwLog.PREFIX + token + " startId=" + startId);
        }
    ...
    public int bindServiceLocked(IApplicationThread caller, IBinder token, Intent service, String resolvedType, final IServiceConnection connection, int flags, String instanceName, String callingPackage, final int userId) throws TransactionTooLargeException {
        ...
        if (callerApp != null) {
            ...
            if ((flags & DumpState.DUMP_FROZEN) != 0 && !isCallerSystem) {
                ...
                if (callingPackage != null && !callingPackage.equals(s.name.getPackageName())) {
                    if (!callingPackage.equals(PackageManagerService.PLATFORM_PACKAGE_NAME) || !ACTION_HWPUSH_MSG_EVENT.equals(service2.getAction())) {
                        if (!(this.mAm.getUidState(callerApp.info.uid) == 2 || (pgm = (PGManagerInternal) LocalServices.getService(PGManagerInternal.class)) == null || !pgm.isServiceProxy(s.name, null))) {
                            if (!isCallerSystem || !callingPackage.equals("com.huawei.zidaneos.foundation")) {
                                Slog.i(ActivityManagerService.TAG, "bind service is proxy: " + s.name);
                                return 0;
                            }
                            Slog.i(ActivityManagerService.TAG, "foundation should pass");
                        }
                    }
                }
                                    ...
                                    try {
                                        if (ActiveServices.this.mPendingServices.contains(s)) {
                                            if (!ActiveServices.this.mAm.getPackageManagerInternalLocked().isPermissionsReviewRequired(s.packageName, s.userId)) {
                                                try {
                                                    String unused = ActiveServices.this.bringUpServiceLocked(s, service2.getFlags(), callerFg2, false, false);
                ...
                try {
                    ...
                    try {
                        ...
                        if (s.app != null) {
                            ...
                            this.mAm.updateLruProcessLocked(s.app, ((!callerApp.hasActivitiesOrRecentTasks() || !s.app.hasClientActivities()) && (callerApp.getCurProcState() > 2 || (flags & 134217728) == 0)) ? z : true, b.client);
    ...
    public void publishServiceLocked(ServiceRecord r, Intent intent, IBinder service) {
        ...
        try {
            if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
                Slog.v(ActivityManagerService.TAG, "PUBLISHING " + r + HwLog.PREFIX + intent2 + ": " + service);
            }
            if (r != null) {
    ...
    private ServiceLookupResult retrieveServiceLocked(Intent service, String instanceName, String resolvedType, String callingPackage, int callingPid, int callingUid, int userId, boolean createIfNeeded, boolean callingFromFg, boolean isBindExternal, boolean allowInstant) {
        ...
        if (r7 == null) {
            ...
            try {
                ...
                try {
                    ResolveInfo rInfo = this.mAm.getPackageManagerInternalLocked().resolveService(service, resolvedType, flags, userId4, userId5);
                    if (rInfo != null) {
                        try {
                    ...
                    if ((sInfo2.flags & 4) != 0) {
                        if (!isBindExternal) {
                            ...
                            if (aInfo != null) {
                                ...
                                String str6 = aInfo.packageName;
                                if (str3 == null) {
                                    str5 = className.getClassName();
                                } else {
                                    str5 = className.getClassName() + ":" + str3;
                                }
                                className = new ComponentName(str6, str5);
    ...
    private final void bumpServiceExecutingLocked(ServiceRecord r, boolean fg, String why) {
        ...
        if (this.mAm.mBootPhase < 600 && r.app != null && r.app.pid == Process.myPid()) {
            Slog.w(ActivityManagerService.TAG, "Too early to start/bind service in system_server: Phase=" + this.mAm.mBootPhase + HwLog.PREFIX + r.getComponentName());
            timeoutNeeded = false;
        }
        long now = SystemClock.uptimeMillis();
    ...
    public String bringUpServiceLocked(ServiceRecord r, int intentFlags, boolean execInFg, boolean whileRestarting, boolean permissionsReviewRequired) throws TransactionTooLargeException {
        ...
        if (r.app == null || r.app.thread == null) {
            if (whileRestarting || !this.mRestartingServices.contains(r)) {
                if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
                    Slog.v(ActivityManagerService.TAG, "Bringing up " + r + HwLog.PREFIX + r.intent + " fg=" + r.fgRequired);
                }
            ...
            Flog.i(102, "do nothing when waiting for a restart and Bringing up " + r + HwLog.PREFIX + r.intent);
            return null;
    ...
    private final void sendServiceArgsLocked(ServiceRecord r, boolean execInFg, boolean oomAdjusted) throws TransactionTooLargeException {
        ...
        if (N != 0) {
            ...
            while (r.pendingStarts.size() > 0) {
                ...
                if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
                    Slog.v(ActivityManagerService.TAG, "Sending arguments to: " + r + HwLog.PREFIX + r.intent + " args=" + si.intent);
                }
    ...
    private final void bringDownServiceLocked(ServiceRecord r) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
            ...
            here.fillInStackTrace();
            Slog.v(ActivityManagerService.TAG, "Bringing down " + r + HwLog.PREFIX + r.intent, here);
        }
    ...
    /* JADX WARN: Type inference failed for: r17v0, types: [com.android.server.am.ActiveServices] */
    ...
    public final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        while (i2 >= 0) {
            ...
            if (((ActiveServices) this).mDestroyingServices.remove(sr) && ActivityManagerDebugConfig.DEBUG_SERVICE) {
        ...
        ServiceMap smap3 = getServiceMapLocked(app.userId);
        ...
        while (i3 >= 0) {
            ...
            ServiceRecord curRec = smap3.mServicesByInstanceName.get(sr2.instanceName);
            if (curRec != sr2) {
                if (curRec != null) {
            ...
            } else if (!allowRestart2 || ((long) sr2.crashCount) < ((ActiveServices) this).mAm.mConstants.BOUND_SERVICE_MAX_CRASH_RETRY || (sr2.serviceInfo.applicationInfo.flags & 8) != 0) {
                if (!allowRestart2) {
                    smap2 = smap3;
                } else if (!((ActiveServices) this).mAm.mUserController.isUserRunning(sr2.userId, 0)) {
                    smap2 = smap3;
                } else {
                    ...
                    if (!sr2.startRequested) {
                        ...
                        if (sr2.tracker != null) {
                            smap = smap3;
                            sr2.tracker.setStarted(false, ((ActiveServices) this).mAm.mProcessStats.getMemFactorLocked(), SystemClock.uptimeMillis());
                        } else {
                            smap = smap3;
                        }
                    ...
                    if (!bringDown) {
                        if (!getPreventResult) {
                            preventRestart = ((ActiveServices) this).mAm.shouldPreventRestartService(sr2.serviceInfo, false);
        ...
        if (!allowRestart2) {
            ...
            for (int i4 = ((ActiveServices) this).mRestartingServices.size() - 1; i4 >= 0; i4--) {
                ServiceRecord r = ((ActiveServices) this).mRestartingServices.get(i4);
                if (r.processName.equals(app.processName) && r.serviceInfo.applicationInfo.uid == app.info.uid) {
                    ((ActiveServices) this).mRestartingServices.remove(i4);
                    clearRestartingIfNeededLocked(r);
                }
            }
            for (int i5 = ((ActiveServices) this).mPendingServices.size() - 1; i5 >= 0; i5--) {
                ServiceRecord r2 = ((ActiveServices) this).mPendingServices.get(i5);
                if (r2.processName.equals(app.processName) && r2.serviceInfo.applicationInfo.uid == app.info.uid) {
                    ((ActiveServices) this).mPendingServices.remove(i5);
                }
            }
        }
        int i6 = ((ActiveServices) this).mDestroyingServices.size();
        while (i6 > 0) {
            i6--;
            ServiceRecord sr3 = ((ActiveServices) this).mDestroyingServices.get(i6);
            if (sr3.app == app) {
                sr3.forceClearTracker();
                ((ActiveServices) this).mDestroyingServices.remove(i6);
                if (ActivityManagerDebugConfig.DEBUG_SERVICE) {
    ...
    private String[] initPCEntryArgs(ServiceRecord r) {
        ...
        if (!TextUtils.equals(((ApplicationInfo) r.appInfo).packageName, "com.huawei.desktop.systemui") && !TextUtils.equals(r.appInfo.packageName, "com.huawei.desktop.explorer")) {
            return entryPointArgs;
        }
        return new String[]{String.valueOf(-HwPCUtils.getPCDisplayID())};
    }
}

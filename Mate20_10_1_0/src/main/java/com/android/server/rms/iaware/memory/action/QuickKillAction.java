...
public class QuickKillAction extends Action {
    ...
    private final boolean mIsMemEmergKill = SystemProperties.getBoolean("persist.iaware.mem_emerg_kill", false);
    ...
    private List<Integer> execChooseKill(AwareProcessBlockInfo procGroup, boolean isNeedCheckAdj) {
        ...
        if (MemoryConstant.isFastQuickKillSwitch()) {
            return ProcessCleaner.getInstance(this.mContext).killProcessesSameUidFast(procGroup, this.mInterrupt, true, true, "LowMemQuick", isNeedCheckAdj);
        }
            ...
            if (procGroup == null) {
                ...
                if (procs == null || procs.size() < i2) {
                    ...
                    if (appUid == procGroup.procUid) {
                        AwareLog.i(TAG, "execKillProcess: uid " + procGroup.procUid + processName + " is launching,should not be killed, position:" + position);
                        position += -1;
                        i = 0;
                        i2 = 1;
                    } else {
    ...
    private int executeKill(Bundle extras, AwareProcessBlockInfo procGroup, List<AwareProcessInfo> procs, String processName, boolean isNeedCheckAdj) {
        PkgMemHolder pkgMemHolder = new PkgMemHolder(procGroup.procUid, procs);
    ...
    private int execQuickKillGroup(Bundle extras) {
        ...
        if (procGroups != null) {
            ...
            AwareLog.d(TAG, "getprocGroups level" + memCleanLevel + AwarenessInnerConstants.COLON_KEY + procGroups.size() + " killed num:" + killedNum);

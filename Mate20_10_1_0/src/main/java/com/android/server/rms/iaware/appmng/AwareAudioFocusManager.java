...
public class AwareAudioFocusManager {
    ...
    private static AwareAudioFocusManager mInstance;
    ...
    public static AwareAudioFocusManager getInstance() {
        ...
        synchronized (AwareAudioFocusManager.class) {
            if (mInstance == null) {
                mInstance = new AwareAudioFocusManager();
            }
            awareAudioFocusManager = mInstance;
        }
    ...
    private void init() {
        this.mStateChangeHandler = new StateChangeHandler(BackgroundThread.get().getLooper());
        ...
        synchronized (this.mAudioFocusInfoStack) {
            ...
            if (this.mObserver != null) {
                ...
                if (!info.isTransientLoss()) {
                    ...
                    message.arg2 = uid;
                    this.mStateChangeHandler.sendMessageDelayed(message, 5000);
                }
            }
        }
    }
        ...
        synchronized (this.mAudioFocusInfoStack) {
            ...
            if (stateType != 0) {
                ...
                if (info.isExistFocusRequest(stateType, focusName)) {
                    ...
                    message.arg2 = uid;
                    this.mStateChangeHandler.sendMessageDelayed(message, 3000);
                }
            }
        }
    }
    ...
    public void updateForeground(int uid) {
        synchronized (this.mForegroundUids) {
    ...
    /* access modifiers changed from: private */
    public void updateAudioCacheDecay(int uid) {
        if (this.mObserver != null) {
            synchronized (this.mAudioFocusInfoStack) {
                AudioFocusInfo info = this.mAudioFocusInfoStack.get(Integer.valueOf(uid));
    ...
    /* access modifiers changed from: private */
    public void updateAudioFocusStack(int uid, String focusName) {
        synchronized (this.mAudioFocusInfoStack) {
            AudioFocusInfo info = this.mAudioFocusInfoStack.get(Integer.valueOf(uid));
            if (info == null) {

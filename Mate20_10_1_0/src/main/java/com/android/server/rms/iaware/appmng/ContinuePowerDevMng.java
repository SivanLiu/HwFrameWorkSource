...
public class ContinuePowerDevMng {
    ...
    public static final int LAUNCHER_PRELOADOPT = 101;
    ...
    private static class PreloadAppInfo {
    ...
    private void initThreadhandler(HandlerThread thread) {
        if (thread != null && this.mThreadhandler == null) {
            this.mThreadhandler = new Handler(thread.getLooper()) {
                ...
                /* JADX INFO: Multiple debug info for r0v1 java.lang.Object: [D('objMsg' java.lang.Object), D('obj' java.lang.Object)] */
                    ...
                    if (msg != null) {
                        ...
                        if (i == 1) {
                            ...
                            if (obj instanceof HwProcessInfo) {
                                ...
                                boolean ret = ContinuePowerDevMng.this.handlePreloadAppForLauncher(pkg, pi.userId, preloadType);
                                AwareLog.d(ContinuePowerDevMng.TAG, "TouchDownPreloadApp " + pkg + " preloadType:" + preloadType + " result:" + ret);
                            }
                        } else if (i == 2) {
                            Object objMsg = msg.obj;
                            if (objMsg instanceof HwProcessInfo) {
                                ContinuePowerDevMng.this.handleKillpreApplication((HwProcessInfo) objMsg);
                            }
                        } else if (i == 3) {
                            AwareLog.d(ContinuePowerDevMng.TAG, "MSG_KEY_DOWN_BOOST.");
                            UniPerf.getInstance().uniPerfEvent(13200, "", new int[0]);
                        } else if (i == 4) {
                            AwareLog.d(ContinuePowerDevMng.TAG, "MSG_BOOT_PRELOAD.");
                            ContinuePowerDevMng.this.tryPreLoadPermanentApplication();
                        }
                    }
                }
            };
            if (AppQuickStartFeature.isTouchDownPreloadEnable()) {
                this.mThreadhandler.sendMessageDelayed(this.mThreadhandler.obtainMessage(4), 5000);
            }
        }
    }

    private boolean checkPreloadType(int preloadType) {
        return (preloadType == 0 || preloadType == 1) ? AppQuickStartFeature.isTouchDownPreloadEnable() : preloadType == 101 && AppQuickStartFeature.isPreloadOptEnable();
    }

    public boolean startPreLoadApplication(String pkg, int userId, int preloadType) {
        if (pkg == null || pkg.isEmpty() || userId < 0 || !checkPreloadType(preloadType)) {
            return false;
        }
        if (!checkPermission(Binder.getCallingUid(), pkg, userId)) {
            AwareLog.d(TAG, "TouchDownPreloadApp CallingUid not Launcher, packageName: " + pkg + ", userid: " + userId);
            return false;
        } else if (this.mThreadhandler == null) {
            return false;
        } else {
            this.mThreadhandler.sendMessage(this.mThreadhandler.obtainMessage(1, new HwProcessInfo(pkg, "", userId, -1, preloadType)));
            return true;
        }
    }
    ...
    public boolean handlePreloadAppForLauncher(String packageName, int userId, int preloadType) {
        int ret;
        if (packageName == null || packageName.isEmpty()) {
            return false;
        }
        ...
        if ((appInfo.flags & 9) == 9) {
            ...
            return false;
        } else if (checkHasAnyProcessExist(appInfo, userId) || (ret = HwActivityManager.preloadAppForLauncher(packageName, userId, preloadType)) == -1) {
            return false;
        } else {
            if (ret == -2) {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:15:0x0025, code lost:
        if (com.android.server.rms.iaware.feature.AppQuickStartFeature.isTouchDownExcludePkg(r5) == false) goto L_0x0028;
     */
    ...
    private static boolean checkProcRealExist(Context context, String packageName) {
        ...
        Object obj = context.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG);
    ...
    /* access modifiers changed from: private */
    public void handleKillpreApplication(HwProcessInfo pi) {
    ...
    private boolean getPackageStoppedState(String packageName, int userId) {
        ApplicationInfo appInfo = getAppInfo(packageName, userId).orElse(null);
        if (appInfo == null || (appInfo.flags & HighBitsCompModeID.MODE_EYE_PROTECT) == 0) {
            return false;
        }
        return true;
    }
    ...
    private void forceStopApp(String packageName, int userId) {
        ...
        if (packageName != null && userId >= 0 && (context = this.mContext) != null) {
            ...
            if (obj instanceof ActivityManager) {
                ((ActivityManager) obj).forceStopPackageAsUser(packageName, userId);
            }
        }
    }
    ...
    private boolean checkCallingPackage(int callingUid) {
        ...
        if (launcher == null || launcher.isEmpty() || (pm = AppGlobals.getPackageManager()) == null) {
            return false;
        }
        try {
            name = pm.getNameForUid(callingUid);
        } catch (RemoteException e) {

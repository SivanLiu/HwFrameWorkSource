...
public class IAwareHabitUtils {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:54:0x00bc A[SYNTHETIC] */
    ...
    public static void updatePkgNameTable(Context ctx) {
        ...
        if (ctx != null && (resolver = ctx.getContentResolver()) != null) {
            Cursor c = null;
            try {
                c = resolver.query(AwareConstant.Database.PKGRECORD_URI, new String[]{"appPkgName", HwWmConstants.FLAG_STR, "userID"}, null, null, null);
                if (c != null) {
                    while (c.moveToNext()) {
                        String name = c.getString(0);
                        int flag = c.getInt(1);

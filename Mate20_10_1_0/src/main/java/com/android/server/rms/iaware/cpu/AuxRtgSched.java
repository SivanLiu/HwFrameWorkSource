...
public class AuxRtgSched {
    ...
    private static final int ANIM_TID = AnimationThread.get().getThreadId();
    ...
    private Map<Integer, Integer> mTgid2UidMap = new ConcurrentHashMap();
    ...
    public void enable(CPUFeature.CPUFeatureHandler handler, RtgSchedController controller) {
        if (this.mAuxRtgEnable.get()) {
            ...
            if (HwActivityManagerService.self().mHandlerThread != null) {
                this.mAmsTid = HwActivityManagerService.self().mHandlerThread.getThreadId();
            }
            this.mSysAuxThreads.clear();
            this.mSysAuxThreads.put(Integer.valueOf(ANIM_TID), new PidInfo(ANIM_TID, ANIMATOR));
            this.mSysAuxThreads.put(Integer.valueOf(this.mAmsTid), new PidInfo(this.mAmsTid, AMS));
            this.mSysAuxThreads.put(Integer.valueOf(RMS_TID), new PidInfo(RMS_TID, RMS));
    ...
    public void onCommChanged(int pid, int tgid) {
        if (this.mAuxRtgEnable.get()) {
            ...
            if (validThreadComm(comm)) {
                ...
                this.mTgid2UidMap.put(Integer.valueOf(tgid), Integer.valueOf(uid));
                AppInfo appInfo = this.mAppInfos.get(Integer.valueOf(uid));
                if (appInfo == null) {
    ...
    public void onProcessDied(int tgid) {
        Integer uid;
        if (this.mAuxRtgEnable.get() && (uid = this.mTgid2UidMap.get(Integer.valueOf(tgid))) != null) {
            this.mTgid2UidMap.remove(Integer.valueOf(tgid));
    ...
    public String getThreadComm(int tgid, int pid) {
    ...
    static class PidInfo {
        ...
        /* access modifiers changed from: package-private */
        public boolean isAlive(int tgid) {
            boolean ret = false;
            String str = this.comm;
            if (str != null && str.equals(AuxRtgSched.getInstance().getThreadComm(tgid, this.pid))) {
                ret = true;
            }
            if (!ret) {
                setRtgSchedForFg(0);
    ...
    static class TgidInfo {
        ...
        public Optional<PidInfo> add(int pid, String comm) {
            ...
            PidInfo pidInfo2 = new PidInfo(pid, comm);
        ...
        public int refresh() {
            ...
            while (it.hasNext()) {
                ...
                if (pidInfo == null || !pidInfo.isAlive(this.tgid)) {
                    it.remove();
                    count++;
                }
            }

...
public class ProcStateStatisData {
    ...
    public final Map<String, HistoryProcInfo> mHistoryMap = new ArrayMap();
    private long[] mIntervalTime = {120000, 900000};
    /* access modifiers changed from: private */
    public final Object mLock = new Object();
    /* access modifiers changed from: private */
    public final Map<String, ProcMemValueInfo> mProcMemValueMap = new ArrayMap();
    ...
    private long[] mTestIntervalTime = {2000, HwArbitrationDEFS.WIFI_RX_BYTES_THRESHOLD};
    /* access modifiers changed from: private */
    ...
    private void addVssToMap(ProcAttribute procAttribute, int customProcessState, long intervalTime) {
        ...
        if (!(vss <= 0 || this.mDropThreshold <= 0)) {
            ...
            String procKey2 = uid2 + "|" + pid2;
            String procName2 = procAttribute.getProcName();
            long now = SystemClock.uptimeMillis();
            Object obj2 = this.mVssLock;
            synchronized (obj2) {
                try {
                    if (this.mVssListMap.containsKey(procKey2)) {
                        try {
                            AwareLog.d(TAG, "mVssListMap contains procKey: " + procKey2);
                            List<ProcStateData> procStateDataList = this.mVssListMap.get(procKey2);
                        ...
                        try {
                            if (vss < this.mDropThreshold) {
                                ...
                                this.mVssListMap.put(procKey2, procStateDataLists);
                            }
                        } catch (Throwable th9) {
                            th = th9;
                            throw th;
    ...
    private void addToHistoryMap(String pkgName, String procName, int uid, long uss, long swapPss) {
        if (this.mCustomProcessState == 0 && !procName.contains(AwarenessInnerConstants.COLON_KEY)) {
            synchronized (this.mHistoryMap) {

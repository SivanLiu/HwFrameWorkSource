...
public class SRMSDumpRadar {
    ...
    private static class StartupData {
        private boolean mIsZApp;
        private ArrayMap<String, int[]> mReasonList;

        private StartupData(String pkg) {
            this.mIsZApp = false;
            this.mReasonList = new ArrayMap<>();
            this.mIsZApp = AwareMiddleware.getInstance().isZApp(pkg);
        }

        /* JADX INFO: Multiple debug info for r3v2 int[]: [D('val' int[]), D('index' int)] */
        ...
        public boolean isNeedReport(int threshold, boolean onlyDiff) {
            ...
            if (totalAlw == 0) {
                ...
                return index3 >= 0 && (values = this.mReasonList.valueAt(index3)) != null && values.length > 0 && values[values.length - 1] > 0;
            }
        }

        /* access modifiers changed from: private */
        public String encodeString() {
            String[] tagOrderList = {"T", "U", "I", AwarenessInnerConstants.TIME_FENCE_HOUR_MINUTE_SPLIT_KEY, "O", AppMngConstant.AppStartSource.THIRD_ACTIVITY.getDesc(), AppMngConstant.AppStartSource.THIRD_BROADCAST.getDesc(), AppMngConstant.AppStartSource.SYSTEM_BROADCAST.getDesc(), AppMngConstant.AppStartSource.START_SERVICE.getDesc(), AppMngConstant.AppStartSource.BIND_SERVICE.getDesc(), AppMngConstant.AppStartSource.PROVIDER.getDesc(), AppMngConstant.AppStartSource.SCHEDULE_RESTART.getDesc(), AppMngConstant.AppStartSource.JOB_SCHEDULE.getDesc(), AppMngConstant.AppStartSource.ALARM.getDesc(), AppMngConstant.AppStartSource.ACCOUNT_SYNC.getDesc(), AppMngConstant.AppStartReason.DEFAULT.getDesc(), AppMngConstant.AppStartReason.SYSTEM_APP.getDesc(), AppMngConstant.AppStartReason.LIST.getDesc()};
    ...
    private SRMSDumpRadar() {
        ...
        for (int i2 = 0; i2 <= 1; i2++) {
            this.mStatisticsData.add(new StatisticsData(RESOURCE_FEATURE_ID, 2, mSubTypeList[i2], 0, 0, 0, System.currentTimeMillis(), 0));
        }
    }
    ...
    public ArrayList<StatisticsData> getStatisticsData() {
        ...
        synchronized (this.mStatisticsData) {
            for (int i = 0; i <= 1; i++) {
                dataList.add(new StatisticsData(RESOURCE_FEATURE_ID, 2, mSubTypeList[i], this.mStatisticsData.get(i).getOccurCount(), 0, 0, this.mStatisticsData.get(i).getStartTime(), System.currentTimeMillis()));
            }
            resetStatisticsData();
        }

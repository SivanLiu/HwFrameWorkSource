...
public class ComponentPreloadFeature extends RFeature {
    ...
    private void subscribeData(boolean enable) {
        if (this.mIRDataRegister == null) {
            AwareLog.e(TAG, "mIRDataRegister is null,can not subscribeData");
        } else if (enable) {
            this.mIRDataRegister.subscribeData(AwareConstant.ResourceType.RESOURCE_PKG_CLEAR_DATA, this.mFeatureType);
        } else {
            this.mIRDataRegister.unSubscribeData(AwareConstant.ResourceType.RESOURCE_PKG_CLEAR_DATA, this.mFeatureType);
        }
    }
}

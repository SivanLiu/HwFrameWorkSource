    ...
    private static class CleanupData {
        ...
        private String getCondition(Map<String, Integer> data) {
            ...
            for (RuleParserUtil.AppMngTag enums : values) {
                ...
                if (!(value == null || -1 == value.intValue() || (i = AnonymousClass1.$SwitchMap$com$android$server$mtm$iaware$appmng$rule$RuleParserUtil$AppMngTag[enums.ordinal()]) == 1 || i == 2)) {
                    if (i != 3) {
                        if (i != 4) {
                            condition.append("|");
                            condition.append(enums.getUploadBDTag());
                            condition.append(AwarenessInnerConstants.COLON_KEY);

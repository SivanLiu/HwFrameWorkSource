...
public class AppQuickStartFeature extends RFeature {
    ...
    private static final String AQS_KEYBOOST = "KeyBoost";
    private static final String AQS_LAUNCHERCHECKPID = "LauncherCheckPid";
    private static final String AQS_PERMANENT_PRELOADPKG = "PermanentPkg";
    private static final String AQS_PRELOADOPT_SWITCH = "preloadOptSwitch";
    private static final String AQS_PRELOADOPT_SYSPROP = "persist.sys.iaware.preloadoptenable";
    private static final int AQS_TOUCHDOWN_DEFAULT_KILLTIME = 3000;
    private static final int AQS_TOUCHDOWN_DEFAULT_TOPN = 7;
    private static final String AQS_TOUCHDOWN_EXCLUDEPKG = "PreloadExcludePkg";
    private static final String AQS_TOUCHDOWN_KILLTIME = "Killtime";
    private static final String AQS_TOUCHDOWN_LAUNCHER = "LauncherPkg";
    private static final String AQS_TOUCHDOWN_PRELOAD = "PreloadApplication";
    private static final String AQS_TOUCHDOWN_SWITCH = "Switch";
    private static final String AQS_TOUCHDOWN_SYSPROP = "persist.sys.iaware.touchdownpreloadenable";
    private static final String AQS_TOUCHDOWN_TOPN = "TopN";
    ...
    private static final int PACKAGENAME_INDEX = 0;
    ...
    private static int sTopN = 7;
    private static int sTopNActs = 100;
    private static final Set<String> sTouchDownExcludePkg = new ArraySet();
    private static int sTouchDownKillTime = 3000;
    ...
    public AppQuickStartFeature(Context context, AwareConstant.FeatureType featureType, IRDataRegister dataRegister) {
        ...
        if (context != null) {
            ...
            if (obj instanceof StorageManager) {
                totalRomGb = ((StorageManager) obj).getPrimaryStorageSize() / 1073741824;
            }
        }
    ...
    private boolean processUserHabitMsg(Bundle bundle) {
        if (bundle != null) {
            if (1 == bundle.getInt(AwareUserHabit.USERHABIT_INSTALL_APP_UPDATE) || 2 == bundle.getInt(AwareUserHabit.USERHABIT_INSTALL_APP_UPDATE)) {
                String pkgName = bundle.getString(AwareUserHabit.USERHABIT_PACKAGE_NAME);
                if (pkgName != null) {
                    exactPrereadSendCmd(MemoryConstant.MSG_WORKINGSET_TOUCHEDFILES_CLEAN, 0, UserHandle.getAppId(bundle.getInt("uid")), 0, pkgName);
                }
                return true;
            }
        }
    ...
    private boolean checkMemSize(String memSize, long totalMemMb, boolean isRomGB) {
        ...
        try {
            long ramSizeL = Long.parseLong(memSize.trim());
            if (!isRomGB) {
                if (totalMemMb > ramSizeL || totalMemMb <= ramSizeL - 1024) {
                    return false;
                }
                return true;
            } else if (totalMemMb <= ramSizeL && totalMemMb > ramSizeL / 2) {
                return true;
            }
            return false;
    ...
    private void loadDefaultExctprdMemSizeConfig(boolean isMatched, AwareConfig.Item defaultItem, boolean isRomGB, long totalMemSize) {
        ...
        if (!isMatched && defaultItem != null) {
            ...
            if (isRomGB) {
                memSize = configPropertries.get(AQS_EXCTPRD_ROMSIZE);
            } else {
                memSize = configPropertries.get("ramsize");
            }
            if (memSize != null) {
                try {
                    if (totalMemSize > Long.parseLong(memSize.trim())) {
                        if (isRomGB) {
                            applyExctprdTopNConfig(defaultItem);
                        } else {
    ...
    private void loadExctprdConfig(boolean isCustConfig, long totalMemMb, long totalRomGb) {
        ...
        for (AwareConfig.Item item : configList.getConfigList()) {
            if (item == null || item.getProperties() == null) {
                ...
                String typeName = configPropertries.get(HwSecDiagnoseConstant.ANTIMAL_APK_TYPE);
                if (typeName != null) {
    ...
    private void applyExctprdTopNConfig(AwareConfig.Item item) {
        for (AwareConfig.SubItem subItem : item.getSubItemList()) {
            if (subItem != null) {
                String itemName = subItem.getName();
                String itemValue = subItem.getValue();
                if (!(itemName == null || itemValue == null)) {
                    ...
                    try {
                        ...
                        if (c != 0) {
                            AwareLog.w("AppQuickStart", "applyExctprdTopNConfig no such configuration. " + itemName);
                        } else {
                            sTopNActs = Integer.parseInt(itemValue.trim());
                            AwareLog.i("AppQuickStart", "applyExctprdTopNConfig topN = " + itemValue);
                        }
                    } catch (NumberFormatException e) {
                        AwareLog.e("AppQuickStart", "parse applyExctprdTopNConfig error!");
                    }
                }
            }
        }
    ...
    private String checkActivityName(String appName, String activityName, String className) {
        ...
        ComponentName.appendShortString(sb, appName, tmpClassName);
        return sb.toString();
    }
    ...
    private boolean doHotActivityBegin(String appName, String activityName, String origActivityName, int pid, int appId) {
        ...
        if (activityTopManagerRT == null || !activityTopManagerRT.isTopActivity(activityName, sTopNActs)) {
            return false;
        }
    ...
    private boolean handleActivityBegin(ArrayMap<String, String> appInfo) {
        ...
        try {
            ...
            doTouchDownPreloadActivityBegin(appName, UserHandle.getUserId(uid), uid);
    ...
    private boolean handleProcessLaunchBegin(ArrayMap<String, String> appInfo) {
        ...
        if (!BigMemoryConstant.BIGMEMINFO_ITEM_TAG.equals(appInfo.get("launchMode"))) {
            return true;
        }
        try {
    ...
    private void subscribleEvents() {
        if (this.mIRDataRegister != null) {
            ...
            if (sIsExactHotBootPrereadFeature) {
                this.mIRDataRegister.subscribeData(AwareConstant.ResourceType.RESOURCE_TOP_ACTIVITY, this.mFeatureType);
            }
        }
    }
    ...
    private void unSubscribeEvents() {
        if (this.mIRDataRegister != null) {
            ...
            if (sIsExactHotBootPrereadFeature) {
                this.mIRDataRegister.unSubscribeData(AwareConstant.ResourceType.RESOURCE_TOP_ACTIVITY, this.mFeatureType);
            }
        }
    }
    ...
    private final class ExactPrereadWorkHandler extends Handler {
        ...
        public ExactPrereadWorkHandler(Long totalMemMb) {
            if (totalMemMb.longValue() > PermissionType.RECEIVE_SMS) {
                ...
                this.mDelayPauseTimeout = 0;
                this.mPauseTimeout = DeviceStatusConstant.TYPE_TRIPLE_FINGER;
            } else {
                this.mDelayPauseTimeout = 0;
        ...
        private boolean hasValidWorkExisted() {
            if (this.mWork.myAppId == -1 || this.mWork.myPid == -1 || this.mWork.myType == -1 || this.mWork.myOwnerName.equals("")) {
                return false;
            }
            return true;
        }
        ...
        /* access modifiers changed from: private */
        public boolean hasWorkExisted(int appId, int pid, String ownerName) {
    ...
    private class SystemBootCheckRunnable implements Runnable {
    ...
    public static boolean isTouchDownExcludePkg(String pkg) {
        ...
        synchronized (sTouchDownExcludePkg) {
            contains = sTouchDownExcludePkg.contains(pkg);
        }
        return contains;
    }
    ...
    private void parseContinuePowerConfig(AwareConfig configList) {
        if (configList == null) {
            AwareLog.i("AppQuickStart", "loadContinuePowerConfig failure, ");
            return;
        }
        for (AwareConfig.Item item : configList.getConfigList()) {
            if (item == null || item.getProperties() == null) {
                ...
                String typeName = item.getProperties().get(HwSecDiagnoseConstant.ANTIMAL_APK_TYPE);
                if (AQS_TOUCHDOWN_PRELOAD.equals(typeName)) {
                    applyTouchDownPreloadConfig(item);
                } else if (AQS_TOUCHDOWN_EXCLUDEPKG.equals(typeName)) {
                    applyTouchDownPreloadExcludeApp(item);
                } else if (AQS_PERMANENT_PRELOADPKG.equals(typeName)) {
                    applyPermanentPreloadPkgs(item);
                } else {
                    boolean z = false;
                    if (AQS_KEYBOOST.equals(typeName)) {
                        if (applyCommSwitch(item) == 1) {
                            z = true;
                        }
                        sKeyBoostEnable = z;
                    } else if (AQS_LAUNCHERCHECKPID.equals(typeName)) {
                        if (applyCommSwitch(item) == 1) {
                            z = true;
                        }
                        sLauncherCheckPid = z;
                ...
                try {
                    switch (itemName.hashCode()) {
                        case -1805606060:
                            ...
                            break;
                        case -539356341:
                            if (itemName.equals(AQS_TOUCHDOWN_KILLTIME)) {
                                c = 2;
                                break;
                            }
                            break;
                        case 2612665:
                            if (itemName.equals(AQS_TOUCHDOWN_TOPN)) {
                                c = 3;
                                break;
                            }
                            break;
                        case 1443245548:
                            if (itemName.equals(AQS_TOUCHDOWN_LAUNCHER)) {
                                c = 4;
                                break;
                            }
                            break;
                        case 1560390590:
                            if (itemName.equals(AQS_PRELOADOPT_SWITCH)) {
                                c = 1;
                                break;
                            }
                            break;
            ...
            if (!(itemName == null || itemValue == null)) {
                ...
                if (c != 0) {
                    ...
                    if (value != null && !value.isEmpty()) {
                        synchronized (sTouchDownExcludePkg) {
                            sTouchDownExcludePkg.add(value);
                        }
    ...
    private static void setTouchDownPreloadEnable(int value) {
        ...
        SystemProperties.set(AQS_TOUCHDOWN_SYSPROP, String.valueOf(i));
    }
    ...
    private static void setPreloadOptEnable(int value) {
        ...
        SystemProperties.set(AQS_PRELOADOPT_SYSPROP, String.valueOf(i));
    }

    private static void setTouchDownPreloadKillTime(int value) {
        sTouchDownKillTime = value < 0 ? 3000 : value;
    }

    private static void setTouchDownPreloadTopN(int value) {
        sTopN = value < 0 ? 7 : value;
    }

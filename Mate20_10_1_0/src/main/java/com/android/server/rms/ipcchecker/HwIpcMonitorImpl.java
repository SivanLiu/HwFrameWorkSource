...
public class HwIpcMonitorImpl implements IHwIpcMonitor {
    ...
    private boolean recoverBlockingProcessInner() {
        int pid = -1;
        AppEyeFwkBlock appEyeFwkBlock = this.mAppEyeFwkBlock;
        if (appEyeFwkBlock != null) {
            pid = appEyeFwkBlock.getLockOwnerPid(this.mLock);
        }
    ...
    public void uploadIPCMonitorFaultToZerohung(String name, int recoverCount) {
        ...
        AppEyeFwkBlock appEyeFwkBlock = this.mAppEyeFwkBlock;
        if (appEyeFwkBlock != null && (pid = appEyeFwkBlock.getLockOwnerPid(this.mLock)) > 0) {
            cmd.append(",p=");
            cmd.append(pid);
        }

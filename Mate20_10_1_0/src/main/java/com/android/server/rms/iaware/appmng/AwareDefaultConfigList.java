...
public class AwareDefaultConfigList {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:14:?, code lost:
        r3.mAwareProtectList.addAll((android.util.ArraySet<? extends java.lang.String>) r1);
    ...
    public void setHabitWhiteList() {
        ...
        AwareUserHabit habitInstance = AwareUserHabit.getInstance();
        if (habitInstance != null) {
            Collection<? extends String> gcmList = getGcmAppList(habitInstance);
    ...
    private Set<String> updateListFromRMS(int rmsGroupId, int whiteListType) {
        ...
        if (str != null) {
            ...
            for (String content : str.split("#")[0].split(";")) {
                String content2 = content.trim();
                if (!content2.isEmpty()) {
    ...
    private void setAppMngCfg(String str, AppMngCfgXml cfg2G) {
        if (str != null && cfg2G != null) {
            ...
            for (String content : contentArray) {
                if (content != null) {
                    String[] names = content.trim().split(AwarenessInnerConstants.COLON_KEY);
                    if (names.length > 1) {
    ...
    public static boolean isAbroadArea() {
        return !SystemProperties.get("ro.config.hw_optb", "0").equals(OPTB_CHINA);
    }
}

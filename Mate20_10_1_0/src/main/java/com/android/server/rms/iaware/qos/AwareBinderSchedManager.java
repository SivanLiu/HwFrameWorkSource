...
public class AwareBinderSchedManager {
    ...
    private static final int BINDER_BGCTL_FGCHANGED = 1;
    ...
    private static final boolean DEBUG_SWITCH = SystemProperties.getBoolean("persist.sys.iaware.bindersched.debug", false);
    private static final int DEFAULT_BINDERLIMIT_ACTIVITY_DURATION = 10000;
    private static final int DEFAULT_BINDERLIMIT_OTHERS_DURATION = 2000;
    private static final int DEFAULT_INVAILD_APPSTUS = -1;
    ...
    private static AwareBinderSchedManager mInstance = null;
    ...
    public static AwareBinderSchedManager getInstance() {
        ...
        synchronized (INSTANCE_LOCK) {
            if (mInstance == null) {
                mInstance = new AwareBinderSchedManager();
            }
            awareBinderSchedManager = mInstance;
        }
    ...
    public void enable(int switchValue) {
        ...
        if ((switchValue & 32) != 0) {
            ...
            this.mAnimationThreadTid.set(AnimationThread.get().getThreadId());
        } else {
    ...
    public void handleBinderLimitSceneStart(int sceneType) {
    ...
    public void setRtgThreadForAnimation(boolean flag) {
        if (this.mAnimationFeatureEnable.get() && this.mBinderSchedHandler != null) {
            ...
            if (flag) {
                ...
                Process.setThreadScheduler(this.mAnimationThreadTid.get(), 1, 1);
                Message msg = this.mBinderSchedHandler.obtainMessage();
                ...
                this.mBinderSchedHandler.removeMessages(MSG_ANIMATION_REMOVE_VIP);
                logForAnimationThread(flag);
                Process.setThreadScheduler(this.mAnimationThreadTid.get(), 0, 0);
            }
        }
    }
    ...
    private void logForAnimationThread(boolean flag) {
        if (DEBUG_SWITCH) {
            ...
            sb.append("rtgFlag" + AwarenessInnerConstants.COLON_KEY + flag + ";" + "AnimationTid" + AwarenessInnerConstants.COLON_KEY + this.mAnimationThreadTid.get());
    ...
    public void setProcessQos(int uid, int pid, boolean isSet, int type) {
        if (this.mFeatureEnable.get() && this.mBinderSchedHandler != null && UserHandle.getAppId(uid) > 10000) {
            if (pid <= 0) {
            ...
            int value = 1;
            if (type == 1) {
                bundle.putInt("status", isSet ? 100 : -100);
            } else {
                if (!isSet) {
                    value = -1;
                }
                bundle.putInt("status", value);
            }
            msg.setData(bundle);
            this.mBinderSchedHandler.sendMessage(msg);
        }
    ...
    public void reportFgChanged(int pid, int uid, boolean isCurFg) {
        if (this.mFeatureEnable.get() && this.mBinderSchedHandler != null) {
            if (pid <= 0) {
                ...
                bundle.putInt("status", isCurFg ? 1 : 0);
                msg.setData(bundle);
                this.mBinderSchedHandler.sendMessage(msg);
    ...
    private class BinderSchedHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i != AwareBinderSchedManager.MSG_UPDATE_PROTECTED_PORCESS) {
                switch (i) {
                    case AwareBinderSchedManager.MSG_BINDER_SCHED_REPORT_SCENE /*{ENCODED_INT: 1101}*/:
                        AwareBinderSchedManager.this.doReportSceneToBinder(msg);
                        return;
                    case AwareBinderSchedManager.MSG_BINDER_SCHED_SCENE_START /*{ENCODED_INT: 1102}*/:
                        AwareBinderSchedManager.this.doBinderSchedStart(msg);
                        return;
                    case AwareBinderSchedManager.MSG_BINDER_SCHED_SCENE_STOP /*{ENCODED_INT: 1103}*/:
                        AwareBinderSchedManager.this.doBinderSchedStop(msg);
                        return;
                    case AwareBinderSchedManager.MSG_ANIMATION_REMOVE_VIP /*{ENCODED_INT: 1104}*/:
    ...
    /* access modifiers changed from: private */
    public void doBinderSchedStart(Message msg) {
        if (this.mFeatureEnable.get()) {
    ...
    /* access modifiers changed from: private */
    public void doBinderSchedStop(Message msg) {
    ...
    /* access modifiers changed from: private */
    public void doReportSceneToBinder(Message msg) {
        if (this.mFeatureEnable.get()) {
            int msgId = msg.what;

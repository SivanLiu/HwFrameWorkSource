...
public final class MemoryDumpsys {
    ...
    private static final int DEFAULT_PRINT_STRINGBUIDER_SIZE = 800;
    ...
    private static boolean dumpMemRepair(PrintWriter pw, String[] args) {
        if (args.length != 2 || !"getSample".equals(args[1])) {
            return false;
        }
    ...
    private static boolean dumpPkgTracker(String[] args) {
        ...
        if ("PkgTracker".equals(args[1]) && "enable_log".equals(args[2])) {
            PackageTracker.getInstance().enableDebug();
            return true;
        } else if (!"PkgTracker".equals(args[1]) || !"disable_log".equals(args[2])) {
            return false;
        } else {

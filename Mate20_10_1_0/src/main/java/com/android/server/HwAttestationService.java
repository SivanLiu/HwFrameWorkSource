...
public class HwAttestationService extends IHwAttestationService.Stub {
    ...
    private boolean checkCallingPermission(int uid) {
        ...
        try {
            ...
            if (isCheckPermission || pm.checkSignatures(getPackageName(uid), AppStartupDataMgr.HWPUSH_PKGNAME) < 0) {
    ...
    private boolean checkCallingPermission(String pkgName) {
        ...
        try {
            ...
            if (isCheckPermission || pm.checkSignatures(pkgName, AppStartupDataMgr.HWPUSH_PKGNAME) < 0) {
                return isCheckPermission;
            }
            return true;
        } catch (PackageManager.NameNotFoundException e) {

...
public final class PermissionPolicyService extends SystemService {
    ...
    private void grantOrUpgradeDefaultRuntimePermissionsIfNeeded(int userId) {
        ...
        if (packageManagerInternal.wereDefaultPermissionsGrantedSinceBoot(userId)) {
            ...
            new PermissionControllerManager(getUserContext(getContext(), UserHandle.of(userId)), FgThread.getHandler()).grantOrUpgradeDefaultRuntimePermissions(FgThread.getExecutor(), new Consumer(userId, latch) {
                /* class com.android.server.policy.$$Lambda$PermissionPolicyService$DEXxGStoqZ6KMUMDJZoATJO9BXY */
    ...
    static /* synthetic */ void lambda$grantOrUpgradeDefaultRuntimePermissionsIfNeeded$0(int userId, CountDownLatch latch, Boolean success) {
        if (!success.booleanValue()) {
            Slog.wtf(LOG_TAG, "Error granting/upgrading runtime permissions");
            if (!UserHandle.isClonedProfile(userId) && ((ActivityManagerInternal) LocalServices.getService(ActivityManagerInternal.class)).isUserRunning(userId, 0)) {
    ...
    private void synchronizePermissionsAndAppOpsForUser(int userId) {
        ...
        ((PackageManagerInternal) LocalServices.getService(PackageManagerInternal.class)).forEachPackage(new Consumer() {
            ...
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
                PermissionToOpSynchroniser.this.addPackage(((PackageParser.Package) obj).packageName);
            }
        });
    ...
    public static class PermissionToOpSynchroniser {
        ...
        private boolean isBgPermRestricted(String pkg, String perm, int uid) {
            try {
                ...
                String access$600 = PermissionPolicyService.LOG_TAG;
                Slog.w(access$600, "Cannot read permission state of " + perm, e);
                return false;
            }
        }
    ...
    private class Internal extends PermissionPolicyInternal {
        ...
        public boolean checkStartActivity(Intent intent, int callingUid, String callingPackage) {
            ...
            String access$600 = PermissionPolicyService.LOG_TAG;
            Slog.w(access$600, "Action Removed: starting " + intent.toString() + " from " + callingPackage + " (uid=" + callingUid + ")");
            return false;
        }
        ...
        public void setOnInitializedCallback(PermissionPolicyInternal.OnInitializedCallback callback) {
            synchronized (PermissionPolicyService.this.mLock) {
                PermissionPolicyInternal.OnInitializedCallback unused = PermissionPolicyService.this.mOnInitializedCallback = callback;
            }
        }

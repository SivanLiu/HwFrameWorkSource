...
public class NavigationBarPolicy implements GestureDetector.OnGestureListener {
    ...
    private boolean touchDownIsValid(float pointX, float pointY) {
        boolean ret = false;
        if (getDisplay() == null || this.mForceMinNavigationBar || (this.mPolicy.mKeyguardDelegate.isShowing() && !this.mPolicy.mKeyguardDelegate.isOccluded())) {
            return false;
        }
        if (this.IS_SUPPORT_PRESSURE && !IS_CHINA_AREA && !this.mImmersiveMode) {
            return false;
        }
        HIT_REGION_TO_MAX = (int) (((double) this.mContext.getResources().getDimensionPixelSize(17105305)) / 3.5d);
    ...
    private boolean isInLockTaskMode() {
        ActivityManager mActivityManager = (ActivityManager) this.mContext.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG);
    ...
    private void showGameControlNavToast(boolean isEnableSwipe, long timeElapse) {
        if (isEnableSwipe || !isNaviBarEnabled() || isGestureNavigationEnable()) {
            ...
            this.mToast = Toast.makeText(this.mContext, 33686203, 1);
            this.mToast.show();
        }
    }
}

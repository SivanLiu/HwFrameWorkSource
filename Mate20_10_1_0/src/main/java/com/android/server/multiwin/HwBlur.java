...
public final class HwBlur {
    ...
    private static BitmapShader getBitmapScaleShader(Bitmap in, Bitmap out, boolean isNeedSkipNavBar) {
        ...
        float scaleRateY;
        if (isNeedSkipNavBar) {
            scaleRateX = ((float) out.getWidth()) / ((float) (in.getWidth() - HwMultiWindowSplitUI.sNavBarWidth));
            scaleRateY = ((float) out.getHeight()) / ((float) (in.getHeight() - HwMultiWindowSplitUI.sNavBarHeight));
        } else {

...
public class HwMultiWinSwapAcceptView extends HwMultiWinHotAreaView {
    ...
    public Rect mDropTargetBound = new Rect();
    /* access modifiers changed from: private */
    public HwMultiWinHotAreaView mDropTargetView;
    ...
    /* access modifiers changed from: private */
    public HwMultiWinClipImageView mSwapTarget;
    /* access modifiers changed from: private */
    public Rect mSwapTargetBound = new Rect();
    ...
    private float getSplitSwapSize() {
        ...
        return (float) (this.mContext.getResources().getDimensionPixelSize(34472536) + (isLandScape() ? otherDropTargetBound.width() : otherDropTargetBound.height()));
    }
    ...
    private void startToBlurSwapTarget() {
        HwMultiWinClipImageView hwMultiWinClipImageView = this.mSwapTarget;
        if (hwMultiWinClipImageView != null) {
            HwMultiWinUtils.blurForScreenShot(hwMultiWinClipImageView, hwMultiWinClipImageView, (BlurListener) null, ScaleType.FIT_START);
        }
    }
            ...
            this.mDropTargetView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    if (HwMultiWinUtils.isNeedToResizeWithoutNavBar(HwMultiWinSwapAcceptView.this.mDropTargetView.getSplitMode(), HwMultiWinSwapAcceptView.this.mNavBarPos)) {
                        ...
                        Rect unused = hwMultiWinSwapAcceptView.mDropTargetBound = HwMultiWinUtils.getBoundWithoutNavBar(hwMultiWinSwapAcceptView.mNavBarPos, HwMultiWinSwapAcceptView.this.mNavBarBound, HwMultiWinSwapAcceptView.this.mDropTargetBound);
                    }
    ...
    private void resizeSwapTargetWithoutNavBar() {
        ...
        if (!(p instanceof LinearLayout.LayoutParams)) {
            Log.w(TAG, "resizeSwapTargetWithoutNavBar failed, cause p is not LinearLayout.LayoutParams.");
            return;
        }
        ...
        Bitmap swapTargetScreenShotWithoutNavBar = HwMultiWinUtils.getScreenShotBmpWithoutNavBar(swapTargetScreenShot, this.mNavBarPos, this.mNavBarBound, this.mSwapTarget.getWidth() > 0 ? ((float) swapTargetScreenShot.getWidth()) / ((float) this.mSwapTarget.getWidth()) : 1.0f);
        ...
        this.mSwapTarget.setImageBitmap(swapTargetScreenShotWithoutNavBar);

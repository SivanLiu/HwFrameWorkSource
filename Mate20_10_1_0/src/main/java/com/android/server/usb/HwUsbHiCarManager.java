...
public class HwUsbHiCarManager {
    ...
    private boolean isNearbyExtraDescription() {
        ...
        int extraType = (bArr3[offset] & 255) | ((bArr3[offset + 1] & HwLocalLocationProvider.LOCATION_TYPE_ACCURACY_PRIORITY) << 8);
        if (extraType != 1) {
            String str2 = TAG;
        ...
        int extraSubType = ((bArr3[offset2 + 1] & HwLocalLocationProvider.LOCATION_TYPE_ACCURACY_PRIORITY) << 8) | (bArr3[offset2] & 255);
        if (extraSubType != 1) {
            String str3 = TAG;

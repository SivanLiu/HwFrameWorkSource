...
public class PGGoogleServicePolicy {
            ...
            if (action != null) {
                if (action.equals("android.intent.action.BOOT_COMPLETED")) {
                    ...
                    if (PGGoogleServicePolicy.mConManager == null) {
                        ConnectivityManager unused2 = PGGoogleServicePolicy.mConManager = (ConnectivityManager) context.getSystemService("connectivity");
                        if (PGGoogleServicePolicy.mConManager == null) {
                            return;
                        }
                    }
                    Network[] networks = PGGoogleServicePolicy.mConManager.getAllNetworks();
                    int length = networks.length;
                    int i = 0;
                    while (i < length) {
                        NetworkInfo infoItem = PGGoogleServicePolicy.mConManager.getNetworkInfo(networks[i]);
                        if (infoItem == null || !infoItem.isConnected()) {
                            i++;
                        } else {
                            Log.d(PGGoogleServicePolicy.TAG, "network is still connected.");
                            long unused3 = PGGoogleServicePolicy.this.mCheckDuration = 60000;
                            PGGoogleServicePolicy.this.mWakeLockHandler.removeCallbacks(PGGoogleServicePolicy.this.mWakelockMonitor);
                    ...
                    boolean unused4 = PGGoogleServicePolicy.this.mIsGoogleServerConnectedOK = false;
    ...
    /* access modifiers changed from: private */
    public boolean mIsConnectedOnce = false;
    /* access modifiers changed from: private */
    public boolean mIsGoogleServerConnectedOK = (!IS_CHINA_MARKET.booleanValue());
    /* access modifiers changed from: private */
    public final Handler mWakeLockHandler;
    /* access modifiers changed from: private */
    public final Runnable mWakelockMonitor = new Runnable() {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    if (networkOperator != null && !networkOperator.isEmpty() && !networkOperator.startsWith(PGGoogleServicePolicy.CHINA_OPERATOR) && PGGoogleServicePolicy.this.mIsConnectedOnce) {
                        boolean unused2 = PGGoogleServicePolicy.this.mIsGoogleServerConnectedOK = true;
                        ...
                        boolean unused3 = PGGoogleServicePolicy.this.mIsGoogleServerConnectedOK = false;
                        Log.d(PGGoogleServicePolicy.TAG, "connect google failed.");
                    } else {
                        boolean unused4 = PGGoogleServicePolicy.this.mIsGoogleServerConnectedOK = true;
                        boolean unused5 = PGGoogleServicePolicy.this.mIsConnectedOnce = true;
                    ...
                    if (PGGoogleServicePolicy.this.mCheckDuration >= 1800000) {
                        long unused6 = PGGoogleServicePolicy.this.mCheckDuration = 1800000;
                    }
                }
            }.start();
        }

...
public abstract class AbstractMasterSystemService<M extends AbstractMasterSystemService<M, S>, S extends AbstractPerUserSystemService<S, M>> extends SystemService {
    ...
    public final int mPackageUpdatePolicy;
    /* access modifiers changed from: protected */
    public final ServiceNameResolver mServiceNameResolver;
    /* access modifiers changed from: private */
    @GuardedBy({"mLock"})
    public final SparseArray<S> mServicesCache;
    /* access modifiers changed from: protected */
    ...
    public S getServiceForUserLocked(int userId) {
        ...
        S service2 = this.mServicesCache.get(resolvedUserId);
        if (service2 == null) {
            boolean disabled = isDisabledLocked(userId);
            service2 = newServiceLocked(resolvedUserId, disabled);
            if (!disabled) {
                onServiceEnabledLocked(service2, resolvedUserId);
            }
                ...
                if (packageName.equals(activePackageName)) {
                    ...
                    synchronized (AbstractMasterSystemService.this.mLock) {
                        if (AbstractMasterSystemService.this.mUpdatingPackageNames == null) {
                            SparseArray unused = AbstractMasterSystemService.this.mUpdatingPackageNames = new SparseArray(AbstractMasterSystemService.this.mServicesCache.size());
                        }
                    ...
                    if (packageName.equals(activePackageName)) {
                        if (AbstractMasterSystemService.this.mUpdatingPackageNames != null) {
                            ...
                            if (AbstractMasterSystemService.this.mUpdatingPackageNames.size() == 0) {
                                SparseArray unused = AbstractMasterSystemService.this.mUpdatingPackageNames = null;
                            }
                        }

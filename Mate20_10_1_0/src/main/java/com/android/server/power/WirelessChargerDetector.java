...
public class WirelessChargerDetector {
    ...
    private final SensorEventListener mListener = new SensorEventListener() {
        /* class com.android.server.power.WirelessChargerDetector.AnonymousClass1 */
    ...
    private final Runnable mSensorTimeout = new Runnable() {
        /* class com.android.server.power.WirelessChargerDetector.AnonymousClass2 */
        ...
        if (this.mDetectionInProgress) {
            ...
            if (this.mMustUpdateRestPosition) {
                ...
                if (this.mTotalSamples < 3) {
                    Slog.w(TAG, "Wireless charger detector is broken.  Only received " + this.mTotalSamples + " samples from the gravity sensor but we need at least " + 3 + " and we expect to see about " + 16L + " on average.");

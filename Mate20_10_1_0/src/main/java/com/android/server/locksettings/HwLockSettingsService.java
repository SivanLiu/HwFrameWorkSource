...
public class HwLockSettingsService extends LockSettingsService {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:21:0x011b, code lost:
        r0 = move-exception;
     */
     ...
     */
    /* JADX WARNING: Removed duplicated region for block: B:21:0x011b A[ExcHandler: IOException | InvalidKeyException | KeyStoreException | NoSuchAlgorithmException | UnrecoverableKeyException | CertificateException | BadPaddingException | IllegalBlockSizeException | NoSuchPaddingException (r0v11 'e' java.lang.Exception A[CUSTOM_DECLARE]), Splitter:B:4:0x00b9] */
    @VisibleForTesting
    ...
    public void setLockCredential(byte[] credential, int type, byte[] savedCredential, int requestedQuality, int userId, boolean allowUntrustedChange) throws RemoteException {
        ...
        updateRemotePassword(credential, type, userId, null);
        if (credential == null || savedCredential == null) {
    ...
    public VerifyCredentialResponse checkCredential(byte[] credential, int type, final int userId, final ICheckCredentialProgressCallback progressCallback) throws RemoteException {
        ...
        warnLog(TAG, "checkCredential U" + userId + " R" + callback.getResponseCode() + " type:" + type);
        updateCredentialLength(credential, userId, callback.getResponseCode());
        updateLockedTimeAndRetryCount(userId, callback);
        updateRemotePassword(credential, type, userId, callback);
        return callback;
    }
    ...
    public VerifyCredentialResponse verifyCredential(byte[] credential, int type, long challenge, int userId) throws RemoteException {
        ...
        warnLog(TAG, "verifyCredential U" + userId + " R" + response.getResponseCode() + " type:" + type);
    ...
    private void updateCredentialLength(byte[] credential, int userId, int responseCode) {
        if (userId >= 0 && responseCode == 0) {
            ...
            if (storedLen != len) {
                ...
                flog(TAG, "updateCredentialLength U" + userId + " oldL:" + storedLen + " newL:" + len);
            }
        }
    }
    ...
    public void showEncryptionNotificationForUser(UserHandle user) {
        ...
        showEncryptionNotification(user, r.getText(33685896), r.getText(33685897), r.getText(17041433), PendingIntent.getBroadcast(this.mContext, 0, ACTION_NULL, 134217728));
    }
    ...
    private void reportCriticalError(int errorType, String message) {
        HwFrameworkMonitor mMonitor2 = HwFrameworkFactory.getHwFrameworkMonitor();
        Bundle bundle = new Bundle();
        bundle.putInt("errorType", errorType);
        bundle.putSerializable("message", message);
        mMonitor2.monitor(907034002, bundle);
    }
    ...
    /* access modifiers changed from: private */
    public void updateRemainAllowRetryCount(int userId) {
    ...
    /* JADX INFO: Multiple debug info for r14v1 long: [D('startTimeRTC' long), D('time' long)] */
    /* access modifiers changed from: protected */
            ...
            if (storedLockedTime > 0) {
                if (info.id >= 0) {
                    ...
                    if (currTimeRTC <= startTimeRTC) {
                        ...
                        long time = storedLockedTime - (currTimeRTC - startTimeRTC);
                        long remainLockedTime = 0;
                        if (time > 0) {
                            remainLockedTime = time;
                        }
                        setLong(REMAIN_LOCKED_TIME, remainLockedTime, info.id);
                        setLong(REMAIN_LOCKED_TIME_START_RTC, currTimeRTC, info.id);
                        Slog.i(TAG, "updateRemainLockedTimeAfterReboot userId " + info.id + " currTimeRTC " + currTimeRTC + " Elapsed currTimeElapsed " + currTimeElapsed + " time " + time);
                        infos = infos;
                    }
                }
            }

...
public class UserUsageStatsService {
    ...
    private static final int MAX_DAYS_ONECE = 7;
    private static final int MAX_EVENT_ONECE = 280000;
    ...
    private <T> List<T> queryStats(int intervalType, long beginTime, long endTime, UsageStatsDatabase.StatCombiner<T> combiner) {
        ...
        if (intervalType2 >= 0) {
            ...
            if (intervalType2 < intervalStatsArr.length) {
                ...
                List<T> results = this.mDatabase.queryUsageStats(intervalType2, beginTime, Math.min(currentStats.beginTime, endTime), combiner);
    ...
    public UsageEvents queryEvents(final long beginTime, final long endTime, final boolean obfuscateInstantApps) {
        int days;
        int days2 = ((int) ((endTime - beginTime) / 86400000)) + 1;
        if (days2 > 7) {
            days = 7;
        } else {
            days = days2;
        }
        final int maxEventsOneDay = MAX_EVENT_ONECE / days;
        final ArraySet<String> names = new ArraySet<>();
        List<UsageEvents.Event> results = queryStats(0, beginTime, endTime, new UsageStatsDatabase.StatCombiner<UsageEvents.Event>() {
            ...
            public void combine(IntervalStats stats, boolean mutable, List<UsageEvents.Event> accumulatedResult) {
                ...
                if (size > maxEventsOneDay + startIndex) {
                    ...
                    size = startIndex + maxEventsOneDay;
    ...
    private void loadActiveStats(long currentTimeMillis, boolean isForce) {
        ...
        for (int intervalType = 0; intervalType < this.mCurrentStats.length; intervalType++) {
            ...
            if (!isForce) {
                IntervalStats[] intervalStatsArr = this.mCurrentStats;
                if (intervalStatsArr[intervalType] != null && intervalStatsArr[intervalType].beginTime == tempCal.getTimeInMillis()) {
                }
            }

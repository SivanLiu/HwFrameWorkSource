...
public class HwArbitrationCommonUtils {
    ...
    public static String getTopActivityPackageName(Context context) {
        try {
            List<ActivityManager.RunningTaskInfo> tasks = ((ActivityManager) context.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG)).getRunningTasks(1);
            if (tasks != null) {
                if (!tasks.isEmpty()) {
    ...
    public static String getAppNameUid(Context context, int uid) {
        ...
        ActivityManager activityManager = (ActivityManager) context.getSystemService(BigMemoryConstant.BIGMEMINFO_ITEM_TAG);

...
public class HwArbitrationCallbackImpl implements IMpLinkCallback, IHwAPPQoECallback, IWaveMappingCallback, IHwHistreamQoeCallback, IHiRadioBoostCallback {
    ...
    @Override // com.android.server.hidata.mplink.IMpLinkCallback
    public void onBindProcessToNetworkResult(MpLinkBindResultInfo result) {
        if (result != null) {
            HwArbitrationCommonUtils.logD(TAG, false, "onMpLinkBindResult:result = %{public}d, failReason = %{public}d, uid = %{public}d, network = %{public}d, type = %{public}d", Integer.valueOf(result.getResult()), Integer.valueOf(result.getFailReason()), Integer.valueOf(result.getUid()), Integer.valueOf(result.getNetwork()), Integer.valueOf(result.getType()));
            ...
            } else if (result.getType() == 5 || result.getType() == 6) {
                this.mHwArbitrationStateMachine.sendMessage(HwArbitrationDEFS.MSG_MPLINK_UNBIND_SUCCESS, result);
            } else {
    ...
    public void onWiFiAndCellCoexistResult(MpLinkNetworkResultInfo result) {
        HwArbitrationCommonUtils.logD(TAG, false, "onWiFiAndCellCoexistResult", new Object[0]);
        if (result != null) {
            HwArbitrationCommonUtils.logD(TAG, false, "mResult = %{public}d, mFailReason = %{public}d, mApType = %{public}d, mActivteNetwork =%{public}d", Integer.valueOf(result.getResult()), Integer.valueOf(result.getFailReason()), Integer.valueOf(result.getApType()), Integer.valueOf(result.getActiveNetwork()));
            if (100 == result.getResult()) {
                HwArbitrationCommonUtils.logD(TAG, false, "enter wifi cell coex mode,type:%{public}d", Integer.valueOf(result.getApType()));
                if (2 == result.getApType()) {
                ...
                HwArbitrationCommonUtils.logD(TAG, false, "leave wifi cell coex mode", new Object[0]);

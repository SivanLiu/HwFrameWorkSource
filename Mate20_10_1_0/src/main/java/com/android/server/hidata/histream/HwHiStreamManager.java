...
public class HwHiStreamManager implements IHwHiStreamCallback, IWaveMappingCallback, IChannelQoECallback {
    ...
    /* access modifiers changed from: private */
    public HwHiStreamQoeMonitor mHwHiStreamQoeMonitor = null;
    ...
    public void onStallDetectedCallback(Message msg) {
        if (msg != null && msg.obj != null) {
            ...
            if (stateInfo == null) {
                ...
                stateInfo.mNetworkType = this.mHwHiStreamNetworkMonitor.getCurrNetworkType(stateInfo.mAppUID);
                if (HwArbitrationFunction.isInMPLink(this.mContext, stateInfo.mAppUID) || !(stateInfo.mScenceId == 100105 || stateInfo.mScenceId == 100106)) {
                    notifyAppStall(appSceneId, detectResult, stateInfo);
                } else {
                    checkChannelQualityForWeChatVideoAudio(stateInfo, 800);
                }
            } else {
    ...
    private void checkChannelQualityForWeChatVideoAudio(HwAPPStateInfo stateInfo, int network) {
        HwAPPQoEResourceManger manager;
        HwAPPQoEAPKConfig conifg;
        if (!this.mQualityOngoing && (manager = HwAPPQoEResourceManger.getInstance()) != null && (conifg = manager.getAPKScenceConfig(stateInfo.mScenceId)) != null) {
            HwHiStreamUtils.logD(false, "Stall Detected and sence is audio/video, get channel Quality, mAppUID = " + stateInfo.mAppUID + ", network = " + network + ", qci = " + conifg.mQci, new Object[0]);
            HwChannelQoEManager channelQoEManager = HwChannelQoEManager.getInstance();
            if (channelQoEManager != null) {
                channelQoEManager.queryChannelQuality(stateInfo.mAppUID, network, conifg.mQci, false, this);
                this.mManagerHandler.sendEmptyMessageDelayed(18, 2000);
                this.mScene = stateInfo.mScenceId;
                this.mQualityOngoing = true;
            }
        }
    }

    /* access modifiers changed from: private */
    public void onQueryChannelQualityTimeOut() {
    ...
    public void onChannelQuality(int uid, int scence, int network, int label) {
        HwHiStreamUtils.logD(false, "onChannelQuality: mAppUID = " + uid + ", scence = " + scence + ", network = " + network + ", quality label = " + label + ", mQualityOngoing = " + this.mQualityOngoing, new Object[0]);
        if (this.mQualityOngoing) {
            this.mManagerHandler.removeMessages(18);
            this.mQualityOngoing = false;
            if (label != 0) {
                HwAPPStateInfo stateInfo = this.mHwHiStreamContentAware.getCurAPPStateInfo(scence);
                if (stateInfo != null) {
                    notifyAppStall(scence, label, stateInfo);
                } else {
                    HwHiStreamUtils.logD(false, "onChannelQuality: stateInfo is null", new Object[0]);
                }
            }
        }
    }
    ...
    private void notifyAppStall(int scence, int detectResult, HwAPPStateInfo stateInfo) {
        ...
        this.mHwHiStreamCHRManager.onStallDetect(scence, detectResult, this.mQoeCallback);
    }
    ...
    /* access modifiers changed from: private */
    public void onVideoDetectStart() {
        HwAPPStateInfo curAppInfo = getCurStreamAppInfo();
    ...
    /* access modifiers changed from: private */
    public void onCheckAppFirstVideo() {
        HwAPPStateInfo curAppInfo = getCurStreamAppInfo();

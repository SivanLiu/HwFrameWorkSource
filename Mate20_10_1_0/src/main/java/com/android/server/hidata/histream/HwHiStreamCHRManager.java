...
public class HwHiStreamCHRManager implements IChannelQoECallback {
        ...
        if (mHwHiStreamTraffic != null && mHwHiStreamNetworkMonitor != null && stateInfo != null) {
            ...
            if (100 == appState) {
                if (isVoipScene(scenceId)) {
                    ...
                    this.mOtherAppStatisticsInfo = new HwHistreamCHRStatisticsInfo(HwAPPQoEUserAction.DEFAULT_CHIP_TYPE, scenceId, appUID);
    ...
    public void onStallDetect(int appSceneId, int detectResult, IHwHistreamQoeCallback qoeCallback) {
        HwHistreamCHRStatisticsInfo appStatisticsInfo;
        HwChannelQoEManager mHwChannelQoEManager = HwChannelQoEManager.getInstance();
        if (isVoipScene(appSceneId) && this.mWechatStatisticsInfo != null) {
            appStatisticsInfo = this.mWechatStatisticsInfo;
        ...
        if (HwArbitrationDEFS.DelayTimeMillisA <= appStatisticsInfo.mLastBadQoeTime - this.mDsQoeLastReportTime) {
            ...
            if (20 > this.mUploadStallCount && mHwChannelQoEManager != null) {
                ...
                int i = this.mStallEventId;
                chrStallInfo.mEventId = i;
                this.mStallEventId = i + 1;
                if (800 == appStatisticsInfo.mCurrNetwork) {
                    updateTup(appStatisticsInfo, 800);
                    chrStallInfo.mUlTup = appStatisticsInfo.mWifiTxTup;
                    chrStallInfo.mDlTup = appStatisticsInfo.mWifiRxTup1;
                    mHwChannelQoEManager.getCurrentSignalState(appStatisticsInfo.mCurrNetwork, true, this);
                    ...
                    HwChannelQoEManager.CurrentSignalState signalInfo = mHwChannelQoEManager.getCurrentSignalState(appStatisticsInfo.mCurrNetwork, false, this);
                    if (signalInfo != null) {
                        chrStallInfo.mCellSinr = signalInfo.getSigSnr();
                        chrStallInfo.mCellRsrq = signalInfo.getSigQual();
                        chrStallInfo.mCellSig = signalInfo.getSigPwr();
                    }
                    if (qoeCallback != null) {
                        HwHistreamCHRQoeInfo qoeInfo = new HwHistreamCHRQoeInfo(appSceneId, appStatisticsInfo.mLastQoe, chrStallInfo.mUlTup, chrStallInfo.mDlTup, chrStallInfo.mNetDlTup);
                        qoeCallback.onHistreamBadQoedetect(qoeInfo);
                        HwHiStreamUtils.logD(false, "onStallDetect: sceneId= %{public}d,videoQoe= %{public}d,ulTup= %{public}d,dlTup= %{public}d,netDlTup= %{public}d", Integer.valueOf(qoeInfo.mSceneId), Integer.valueOf(qoeInfo.mVideoQoe), Integer.valueOf(qoeInfo.mUlTup), Integer.valueOf(qoeInfo.mDlTup), Integer.valueOf(qoeInfo.mNetDlTup));
                    }
                }
                ...
                Handler handler = this.mCHRManagerHandler;
                handler.sendMessageDelayed(handler.obtainMessage(6, bundle), 6000);
            }
        }
    }
    ...
    private void handleNetworkChange(HwHistreamCHRStatisticsInfo appStatisticsInfo) {
        int networkType;
        HwHiStreamNetworkMonitor mHwHiStreamNetworkMonitor = HwHiStreamNetworkMonitor.getInstance();
        if (appStatisticsInfo != null && mHwHiStreamNetworkMonitor != null && appStatisticsInfo.mCurrNetwork != (networkType = mHwHiStreamNetworkMonitor.getCurrNetworkType(appStatisticsInfo.mUid))) {
            long curTime = System.currentTimeMillis();
            ...
            if (800 == networkType) {
                ...
                if (3000 > curTime - mHwHiStreamNetworkMonitor.mLastCellDisableTime || (true == mHwHiStreamNetworkMonitor.getMoblieDateSettings() && 6000 > curTime - mHwHiStreamNetworkMonitor.mLastWifiEnabledTime)) {
                    ...
                    updateHandoverInfo(appStatisticsInfo, 1);
                }
            } else if (801 == networkType) {
                ...
                if (3000 > curTime - mHwHiStreamNetworkMonitor.mLastWifiDisabledTime) {
                    ...
                    updateHandoverInfo(appStatisticsInfo, 2);
                }
            }
    ...
    private void updateHandoverInfo(HwHistreamCHRStatisticsInfo appStatisticsInfo, int event) {
        QueryHistAppQoeService wmService;
        long curTime = System.currentTimeMillis();
        HwChannelQoEManager mHwChannelQoEManager = HwChannelQoEManager.getInstance();
        ...
        if (20 > this.mUploadHandoverCount && appStatisticsInfo != null && mHwChannelQoEManager != null) {
            ...
            if ((3 == event || 5 == event) && (wmService = QueryHistAppQoeService.getInstance()) != null) {
            ...
            HwHiStreamUtils.logD(false, "updateHandoverInfo : handover event, event=%{public}d,eventId=%{public}d", Integer.valueOf(event), Integer.valueOf(mHwHistreamCHRHandoverInfo.mEventId));
            Bundle bundle = new Bundle();
            bundle.putInt("eventNum", mHwHistreamCHRHandoverInfo.mEventId);
            Handler handler = this.mCHRManagerHandler;
            handler.sendMessageDelayed(handler.obtainMessage(4, bundle), 8000);
        }
    }
    ...
    private void sendCHRStatisticEvent(HwHistreamCHRStatisticsInfo appStatisticsInfo) {
        if (appStatisticsInfo != null) {
            ...
            switch (appStatisticsInfo.mScenario) {
                ...
                case HwAPPQoEUtils.SCENE_TIKTOK:

...
public class SpaceUserDao {
    ...
    private String getAllAppTitleString() {
        ...
        while (it.hasNext()) {
            ...
            sqlTitle.append(COMMA);
            sqlTitle.append(appName + DOT + "POORCOUNT" + SYMBOL_AS + appName + "_" + "POORCOUNT");
            sqlTitle.append(COMMA);
            sqlTitle.append(appName + DOT + "GOODCOUNT" + SYMBOL_AS + appName + "_" + "GOODCOUNT");
        }
    ...
    private void processAllSpace(Cursor cursor, HashMap<String, SpaceExpInfo> spaceInfoHashMap) {
        ...
        while (it.hasNext()) {
            ...
            if (duration > 0) {
                ...
                qoeAppPoor.put(appName, Integer.valueOf(cursor.getInt(cursor.getColumnIndexOrThrow(appName + "_" + "POORCOUNT"))));
                qoeAppGood.put(appName, Integer.valueOf(cursor.getInt(cursor.getColumnIndexOrThrow(appName + "_" + "GOODCOUNT"))));
            }
        }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:19:0x0133, code lost:

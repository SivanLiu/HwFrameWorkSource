...
public class HwAPPQoESystemStateMonitor {
    ...
    private class UserDataEnableObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            HwAPPQoEUserAction mHwAPPQoEUserAction;
            int state = Settings.Global.getInt(HwAPPQoESystemStateMonitor.this.mContext.getContentResolver(), "mobile_data", -1);
            HwAPPQoEUtils.logD(HwAPPQoESystemStateMonitor.TAG, false, "User change Data service state = %{public}d", Integer.valueOf(state));
            if (state == 0 && HwArbitrationFunction.isInMPLink(HwAPPQoESystemStateMonitor.this.mContext, HwAPPQoESystemStateMonitor.this.curAPPStateInfo.mAppUID) && (mHwAPPQoEUserAction = HwAPPQoEUserAction.getInstance()) != null) {

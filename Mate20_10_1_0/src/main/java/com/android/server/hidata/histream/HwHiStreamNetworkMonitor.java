...
class HwHiStreamNetworkMonitor {
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent != null) {
                    ...
                    if (SmartDualCardConsts.SYSTEM_STATE_NAME_WIFI_NETWORK_STATE_CHANGED.equals(action)) {
                        NetworkInfo unused = HwHiStreamNetworkMonitor.this.mNetworkInfo = (NetworkInfo) intent.getParcelableExtra("networkInfo");
    ...
    private class UserDataEnableObserver extends ContentObserver {
        public UserDataEnableObserver(Handler handler) {
            super(handler);
            ContentResolver unused = HwHiStreamNetworkMonitor.this.mResolver = HwHiStreamNetworkMonitor.this.mContext.getContentResolver();
        }

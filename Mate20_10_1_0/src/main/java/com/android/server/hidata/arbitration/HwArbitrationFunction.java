...
public class HwArbitrationFunction {
    ...
    public static boolean isAllowMpLink(Context context, int uid, HwAPPStateInfo appInfo) {
        if (HwArbitrationManager.isExceedMaxTemperature()) {
    ...
    public static int getNetworkID(Context mContext, int network) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (netInfo != null && netInfo.getType() == networkType && (myNetwork = networks[i]) != null) {
                ...
                if (eventType != 0) {
                    if (eventType == 2) {
                        ...
                        if (name != null && name.equalsIgnoreCase("uid-policy")) {
                            ...
                            int xmlPolicy = Integer.parseInt(parser.getAttributeValue(null, MemoryConstant.MEM_SYSTRIM_POLICY));

...
class HwHistreamUserLearning {
    ...
    private boolean isDefaultRadicalUser() {
        ...
        if (chipset == null || (!chipset.contains(HwAPPQoEUserAction.BCM_CHIP_4345) && !chipset.contains(HwAPPQoEUserAction.BCM_CHIP_4359) && !chipset.contains(HwAPPQoEUserAction.HISI_CHIP_1103) && !chipset.contains(HwAPPQoEUserAction.HISI_CHIP_1105))) {
            ret = false;
        } else {
            ret = true;
        }
        HwHiStreamUtils.logD(false, "isDefaultRadicalUser:%{public}s", String.valueOf(ret));
        return ret;
    }

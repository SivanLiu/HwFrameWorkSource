...
public class HwInputMethodManagerService extends InputMethodManagerService {
    ...
    /* access modifiers changed from: protected */
    public boolean ensureEnableSystemIME(String id, InputMethodInfo p, Context context, int userId) {
        if (isFlagExists(userId) || (!isValidSystemDefaultIme(p, context) && !isSystemImeThatHasEnglishSubtype(p) && !isSystemImeThatHasChinaSubtype(p))) {
            return false;
        }
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (uri != null) {
                ...
                if (Settings.Secure.getUriFor(HwInputMethodUtils.SETTINGS_SECURE_KEYBOARD_CONTROL).equals(uri)) {
                    ...
                    boolean unused = hwInputMethodManagerService.isSecureIMEEnabled = HwInputMethodUtils.isSecureIMEEnable(hwInputMethodManagerService.mContext, HwInputMethodManagerService.this.mSettings.getCurrentUserId());
    ...
    public InputBindResult startInputOrWindowGainedFocus(int startInputReason, IInputMethodClient client, IBinder windowToken, int controlFlags, int softInputMode, int windowFlags, EditorInfo attribute, IInputContext inputContext, int missingMethods, int unverifiedTargetSdkVersion) {
        ...
        if (!isUseSecureIME() && !this.isSecMethodUsing) {
            ...
            try {
                ...
                try {
                    InputBindResult inputBindRes = HwInputMethodManagerService.super.startInputOrWindowGainedFocus(startInputReason, client, windowToken, controlFlags3, softInputMode, windowFlags, attribute, inputContext, missingMethods, unverifiedTargetSdkVersion);
                    this.mSecurityInputMethodService.startInputOrWindowGainedFocus(10000, client, windowToken, controlFlags3, softInputMode, windowFlags, attribute, inputContext, missingMethods, unverifiedTargetSdkVersion);
                    return inputBindRes;

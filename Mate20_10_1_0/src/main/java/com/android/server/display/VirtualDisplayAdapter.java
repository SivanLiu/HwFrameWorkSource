...
public class VirtualDisplayAdapter extends DisplayAdapter {
    ...
    public VirtualDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler, Listener listener) {
        this(syncRoot, context, handler, listener, $$Lambda$VirtualDisplayAdapter$PFyqeaYIEBicSVtuy5lL_bT8B0.INSTANCE);
    }

    @VisibleForTesting
    VirtualDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler, Listener listener, SurfaceControlDisplayFactory surfaceControlDisplayFactory) {

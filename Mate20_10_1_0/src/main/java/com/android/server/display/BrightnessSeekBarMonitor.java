...
class BrightnessSeekBarMonitor implements DisplayEffectMonitor.MonitorModule {
    ...
    private void receiveParamForTempManualBrightness(ArrayMap<String, Object> params) {
        if (this.mCommonData.isProductEnable() && !this.mCommonData.isCommercialVersion()) {
            ...
            if (MONITOR_TARGET_PACKAGE.equals(packageName)) {
    ...
    private void receiveParamForTempAutoBrightness(ArrayMap<String, Object> params) {
        if (this.mCommonData.isProductEnable() && !this.mCommonData.isCommercialVersion()) {
            ...
            if (brightness != -1 && MONITOR_TARGET_PACKAGE.equals(packageName)) {
    ...
    private class SeekBarRecorderAuto extends SeekBarRecorder {
    ...
    private class SeekBarRecorderManual extends SeekBarRecorder {
    ...
    private class SeekBarRecorderApp extends SeekBarRecorder {
    ...
    private abstract class SeekBarRecorder {
        ...
        private boolean isMerge2Movement(SeekBarMovement pre, SeekBarMovement now) {
            if (now.startTime - pre.endTime > HwArbitrationDEFS.WIFI_RX_BYTES_THRESHOLD) {
                return false;
            }

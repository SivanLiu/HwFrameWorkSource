...
public class HwWifiDisplayAdapterEx implements IHwWifiDisplayAdapterEx {
    ...
    public IWifiDisplayAdapterInner mWfda;
    ...
    public void setConnectionFailedReason(int reason) {
        ...
        if (this.mWfda.getmActiveDisplayInner() != null && 8 == reason) {
    ...
    public int getConnectionFailReason(boolean reset) {
    ...
    public void updateDensityForPcMode(DisplayDeviceInfo info) {
        if (info != null && HwPCUtils.enabled()) {
            info.densityDpi = ((info.width < info.height ? info.width : info.height) * 240) / BASE_HIGH;

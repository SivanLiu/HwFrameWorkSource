...
public class HwAmbientLuxFilterAlgo {
    ...
    private enum BrightenResponseTimeState {
    ...
    private float calcAmbientLuxInCoverState(float lux) {
        ...
        if (!this.mPowerStatus) {
            ...
            if (!this.mData.coverModelastCloseScreenEnable) {
                ambientLux2 = this.mLastCloseScreenLux;
                this.mAmbientLightRingBuffer.putLux(0, ambientLux2);
                this.mAmbientLightRingBufferFilter.putLux(0, ambientLux2);
                if (HWFLOW) {
    ...
    private float updateAmbientLuxForFirstLux(float lux, long time) {
        ...
        if (this.mFirstAmbientLux) {
            ...
            if (HWFLOW) {
                Slog.i(TAG, "updateAmbientLux: Initializing: mBuffer=" + this.mAmbientLightRingBuffer + ",home=" + this.mHomeModeEnable + ", mLx=" + this.mAmbientLux + ",mCloseLux=" + this.mLastCloseScreenLux + ",mKeyguardLuxDelta=" + this.mKeyguardMinBrightLuxDelta + getLuxThresholdStrings() + ",mAMin=" + this.mAmbientLuxNewMin + ",mAMax=" + this.mAmbientLuxNewMax + ",mFilter=" + this.mAmbientLightRingBufferFilter);
            }
            this.mNeedToUpdateBrightness = true;
        }
        return ambientLux;
    ...
    private boolean needToBrightenByLux(float ambientLux, float lastestLux, float ambientLuxLongTime) {
        ...
        if (!isNeedToBrighten || !needToBrightenNew) {
            isNeedToBrighten2 = false;
        }
        return isNeedToBrighten2;
    }
    ...
    private float resetAmbientLuxForUpdate(float lux, float ambientLuxLongTime, float lastestLux, boolean needToBrighten, boolean needToDarken) {
        ...
        if (this.mData.longTimeFilterEnable && needToBrighten && ambientLuxLongTime < this.mLongTimeFilterLuxTh) {
            ...
            float ambientLux2 = ambientLux > ambientLuxLongTime ? ambientLuxLongTime : ambientLux;
            ambientLux = ambientLux2 > lastestLux ? lastestLux : ambientLux2;
        }
        ...
        float ambientLux3 = updateAmbientLuxFromResetAmbientLuxThMin(ambientLux, needToDarken);
        if (needToDarken && Math.round(ambientLux3) == 0 && this.mDayModeEnable && this.mData.dayModeDarkenMinLux > 0.0f && !this.mKeyguardUnLockedFastResponse) {
    ...
    private void updateNewAmbientLuxFromScene(long time, HwRingBuffer hwBuffer) {
        ...
        if (!getThresholdForGameModeEnable() && !this.mLandscapeModeEnable && !getThresholdForDcModeEnable() && (bufferSize = hwBuffer.size()) != 0 && bufferSize >= this.mData.sceneGapPoints && this.mResponseDurationPoints - this.mData.sceneMinPoints >= this.mData.sceneGapPoints && this.mData.sceneMaxPoints >= this.mData.sceneMinPoints && this.mData.sceneMaxPoints + this.mData.sceneGapPoints <= AMBIENT_SCENE_MAX_NUM) {
    ...
    private long getNextAmbientLightBrighteningTime(long earliedtime) {
        long brightenResponseTime;
        switch (getCurrentBrightenResponseTimeState()) {
            case COVER_MODE_FAST_RESEPONSE:
                brightenResponseTime = this.mData.coverModeBrightenResponseTime;
                break;
            case KEYGUARD_LOCKED:
                brightenResponseTime = (long) this.mData.keyguardResponseBrightenTime;
                break;
            case OUTDOOR_MODE:
                brightenResponseTime = (long) this.mData.outdoorResponseBrightenTime;
                break;
            case PROXIMIT_POSITIVE:
                brightenResponseTime = (long) this.mData.proximityResponseBrightenTime;
                break;
            case POWER_ON_SLOW:
                brightenResponseTime = (long) (this.mData.powerOnBrightenDebounceTime + this.mData.initSlowReponseBrightTime);
                break;
            case POWER_ON_FAST:
                brightenResponseTime = (long) this.mData.powerOnBrightenDebounceTime;
                break;
            case GAME_MODE:
                brightenResponseTime = this.mData.gameModeBrightenDebounceTime;
                break;
            case LANDSCAPE_MODE:
                brightenResponseTime = (long) this.mData.landscapeModeBrightenDebounceTime;
                break;
            case MODE_TO_AUTO_FAST_DAKERN:
                brightenResponseTime = (long) this.mData.resetAmbientLuxBrightenDebounceTime;
                break;
            default:
                brightenResponseTime = this.mNormBrighenDebounceTime;
                break;
        }
        return earliedtime + brightenResponseTime;
    }
    ...
    private boolean getKeyguardLockedBrightenEnable() {
        ...
        if (bufferSize == 0) {
            ...
            return false;
        } else if (!this.mKeyguardIsLocked || this.mAmbientLightRingBuffer.getLux(bufferSize - 1) < this.mData.keyguardLuxThreshold || this.mData.keyguardResponseBrightenTime < 0 || getProximityPositiveBrightenEnable()) {
            return false;
        } else {
            return true;
        }
    ...
    public void clear() {
        synchronized (this.mLock) {
            ...
            if (this.mData.initNumLastBuffer > 0) {
                ...
                int pruneNumTmp = this.mLastCloseScreenRingBuffer.size() - this.mData.initNumLastBuffer;
                int pruneNum = pruneNumTmp > 0 ? pruneNumTmp : 0;
                if (pruneNum > 0) {
    ...
    private void updateDebounceTime(HwRingBuffer buffer, float lux) {
        ...
        if (luxRound < this.mData.brightTimeDelayLuxThreshold && this.mlastFilterLux < this.mData.brightTimeDelayLuxThreshold && this.mData.brightTimeDelayEnable) {
            ...
            this.mNormBrighenDebounceTime = (long) (((float) this.mData.brighenDebounceTime) * (((this.mData.brightenDebounceTimeParaBig * (this.mStability - ((float) this.mData.stabilityConstant))) / STABILITY_MAX_VALUE) + 1.0f));
        }
        if ((luxRound < this.mData.darkTimeDelayLuxThreshold || this.mDarkTimeDelayFromBrightnessEnable) && this.mData.darkTimeDelayEnable) {
            ...
            this.mNormDarkenDebounceTime = (long) (((float) this.mData.darkenDebounceTime) * (((this.mData.darkenDebounceTimeParaBig * (this.mStability - ((float) this.mData.stabilityConstant))) / STABILITY_MAX_VALUE) + 1.0f));
        }
    ...
    private void updateStability(HwRingBuffer buffer) {
        ...
        if (stabilityForSmallThr > STABILITY_MAX_VALUE) {
            this.mStabilityForSmallThr = STABILITY_MAX_VALUE;
        } else {
    ...
    private void setDarkenThresholdNew(float amLux) {
        ...
        float f = this.mDarkenDeltaLuxMax;
        this.mDarkenDeltaLuxForBackSensorCoverMode = this.mRatioForDarkenBackSensorCoverMode * f;
        this.mModeToAutoFastResponseDarkenDeltaLux = f * this.mData.resetAmbientLuxDarkenRatio;
    ...
    private float calculateLuxThresholdDeltaNew(List<PointF> linePoints, float amLux, boolean isBrighten) {
        ...
        for (PointF curPoint : linePoints) {
            ...
            if (amLux < curPoint.x) {
                float luxThreshold2 = 1.0f;
                if (curPoint.x <= prePoint.x) {
                    Slog.w(TAG, "Brighten_prePoint.x <= nexPoint.x,x" + curPoint.x + ", y = " + curPoint.y);
                    return 1.0f;
                }
                float luxThreshold3 = (((curPoint.y - prePoint.y) / (curPoint.x - prePoint.x)) * (amLux - prePoint.x)) + prePoint.y;
                if (isBrighten) {
                    return luxThreshold3;
                }
                if (luxThreshold3 > 1.0f) {
                    luxThreshold2 = luxThreshold3;
                }
        ...
        if (bufferSize == 0) {
            ...
            for (int i2 = bufferSize - 1; i2 >= bufferSize - this.mData.preWeightedMeanFilterNum; i2--) {
                if (this.mAmbientLightRingBuffer.getLux(i2) != 0.0f || maxLux > this.mData.preWeightedMeanFilterLuxTh) {
                    ...
                    weight = this.mData.preWeightedMeanFilterAlpha;
                }
                totalWeight += weight;
    ...
    private boolean decideToBrighten(float ambientLux) {
        ...
        if (ambientLux - getCurrentBrightenAmbientLux() < getCurrentBrightenDelta() || this.mStability >= this.mStabilityBrightenConstant || ambientLux - getCurrentBrightenAmbientLux() < this.mKeyguardMinBrightLuxDelta || ambientLux - getCurrentBrightenAmbientLux() < this.mBrightenDeltaLuxForCurrentBrightness) {
    ...
    private boolean decideToBrightenForSmallThr(float ambientLux) {
        ...
        if (ambientLux - this.mAmbientLux < this.mBrightenDeltaLuxMin || ambientLux - getCurrentBrightenAmbientLux() < this.mKeyguardMinBrightLuxDelta || this.mStabilityForSmallThr >= this.mStabilityBrightenConstantForSmallThr || ambientLux - this.mAmbientLuxNewMax < this.mBrightenDeltaLuxForCurrentBrightness) {
            needToBrighten = false;
        } else {
            needToBrighten = true;
        }
    ...
    public void setHomeModeEnable(boolean enable) {
        if (this.mData.homeModeEnable) {
            ...
            updateDayModeTime();
            updateDayModeEnable();
            Slog.i(TAG, "updatemHomeModeEnable=" + enable + ",mDayModeEnable=" + this.mDayModeEnable);
        }
    }

...
final class OverlayDisplayAdapter extends DisplayAdapter {
    ...
    public OverlayDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler, Listener listener, Handler uiHandler) {
            ...
            while (i < length) {
                ...
                if (!displayMatcher2.matches()) {
                    ...
                    if (!modes.isEmpty()) {
                        int count2 = count + 1;
                        boolean secure = true;
                        String name = getContext().getResources().getString(17039993, Integer.valueOf(count2));

...
public class PluginInstanceManager<T extends Plugin> {
    ...
    public PluginInfo<T> getPlugin() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            info.mPlugin.onCreate(this.mContext, info.mPluginContext);
            return info;
        }
    ...
    public void destroy() {
        Iterator<PluginInfo> it = new ArrayList<>(this.mPluginHandler.mPlugins).iterator();
        while (it.hasNext()) {
    ...
    public boolean disableAll() {
        ArrayList<PluginInfo> plugins = new ArrayList<>(this.mPluginHandler.mPlugins);
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == 1) {
                ...
                if (!(msg.obj instanceof PluginFragment)) {
                    info.mPlugin.onCreate(PluginInstanceManager.this.mContext, info.mPluginContext);
                }
                PluginInstanceManager.this.mListener.onPluginConnected(info.mPlugin, info.mPluginContext);
            } else if (i != 2) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            e = e;
                            Notification.Builder nb = new Notification.Builder(PluginInstanceManager.this.mContext, PluginManager.NOTIFICATION_CHANNEL_ID).setStyle(new Notification.BigTextStyle()).setSmallIcon(PluginInstanceManager.this.mContext.getResources().getIdentifier("tuner", "drawable", PluginInstanceManager.this.mContext.getPackageName())).setWhen(0).setShowWhen(false).setVisibility(1).setColor(PluginInstanceManager.this.mContext.getColor(Resources.getSystem().getIdentifier("system_notification_accent_color", "color", AppStartupDataMgr.HWPUSH_PKGNAME)));
                            String label = cls;
                            try {
                        ...
                        e = e3;
                        plugin = plugin2;
                        Notification.Builder nb2 = new Notification.Builder(PluginInstanceManager.this.mContext, PluginManager.NOTIFICATION_CHANNEL_ID).setStyle(new Notification.BigTextStyle()).setSmallIcon(PluginInstanceManager.this.mContext.getResources().getIdentifier("tuner", "drawable", PluginInstanceManager.this.mContext.getPackageName())).setWhen(0).setShowWhen(false).setVisibility(1).setColor(PluginInstanceManager.this.mContext.getColor(Resources.getSystem().getIdentifier("system_notification_accent_color", "color", AppStartupDataMgr.HWPUSH_PKGNAME)));
                        String label2 = cls;
                    ...
                    Log.w(PluginInstanceManager.TAG, "Couldn't load plugin: " + pkg, e4);
                    return null;
                }
            }
        }

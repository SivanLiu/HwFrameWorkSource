    ...
    public Drawable createNewIconForTask(Task.TaskKey taskKey, ActivityManager.TaskDescription desc, boolean returnDefault) {
        ...
        if (desc.getIconResource() != 0) {
            try {
                ...
                return createBadgedDrawable(pm.getResourcesForApplication(pm.getApplicationInfo(taskKey.getPackageName(), 4194304)).getDrawable(desc.getIconResource(), null), userId, desc);

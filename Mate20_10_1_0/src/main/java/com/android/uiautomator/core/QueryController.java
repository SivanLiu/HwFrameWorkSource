...
class QueryController {
    ...
    public QueryController(UiAutomatorBridge bridge) {
        ...
        bridge.setOnAccessibilityEventListener(new UiAutomation.OnAccessibilityEventListener() {
            /* class com.android.uiautomator.core.QueryController.AnonymousClass1 */
                    ...
                    if (eventType != 32) {
                        if (eventType == 131072) {
                            if (!(event.getText() == null || event.getText().size() <= 0 || event.getText().get(0) == null)) {
                                String unused = QueryController.this.mLastTraversedText = event.getText().get(0).toString();
                            }
                        ...
                        String unused2 = QueryController.this.mLastActivityName = event.getText().get(0).toString();
                    }
    ...
    public AccessibilityNodeInfo findAccessibilityNodeInfo(UiSelector selector, boolean isCounting) {
        ...
        synchronized (this.mLock) {
            ...
            return translateCompoundSelector(new UiSelector(selector), rootNode, isCounting);
        }
    }
    ...
    private AccessibilityNodeInfo findNodeRegularRecursive(UiSelector subSelector, AccessibilityNodeInfo fromNode, int index) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childNode == null) {
                Log.w(LOG_TAG, String.format("AccessibilityNodeInfo returned a null child (%d of %d)", Integer.valueOf(i), Integer.valueOf(childCount)));
                if (!hasNullChild) {

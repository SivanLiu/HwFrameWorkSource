...
public class ContactsAction extends Action {
    ...
    private static ContactsAction instance = null;
    ...
    public static ContactsAction getInstance(Context context) {
        ...
        synchronized (LOCK) {
            if (instance == null) {
                instance = new ContactsAction(context, OPCollectConstant.CONTACTS_ACTION_NAME);
            }
            contactsAction = instance;
        }
    ...
    public boolean destroy() {
        super.destroy();
        destroyContactsActionInstance();
        return true;
    }

    private static void destroyContactsActionInstance() {
        synchronized (LOCK) {
            instance = null;
        }
    }
    ...
    private boolean collectDSContactsInfo() {
        new Thread(new Runnable() {
            /* class com.huawei.opcollect.collector.pullcollection.ContactsAction.AnonymousClass1 */

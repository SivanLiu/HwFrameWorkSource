...
public class BluetoothSocketTransport {
    ...
    public NearbySocket createNearbySocketServer(SocketListenerTransport.ChannelCreateRequestImpl requestImpl, int timeout, InternalNearbySocket internalNearbySocket) {
        ...
        try {
            ...
            try {
                ...
                BluetoothNearbySocket.Builder withBluetoothSocket = new BluetoothNearbySocket.Builder().withBluetoothSocket(portSocket);
                if (portSocket == null) {
                    z = false;
                }
                BluetoothNearbySocket nearbySocket = withBluetoothSocket.withIsAvailability(z).withRemoteNearbyDevice(nearbyDevice).withBusinessId(businessId).withBusinessType(businessType).withChannel(channelId).withTag(tag).withPort(port2).withServiceUuid(serviceUuid).withInternalNearbySocket(internalNearbySocket).withSecurityType(requestImpl.getSecurityType()).build();
                try {
    ...
    private class SocketAcceptMonitor {
        private BluetoothServerSocket mBluetoothServerSocket;
        /* access modifiers changed from: private */
        public volatile boolean mIsAccept = false;
        /* access modifiers changed from: private */

...
public class WifiStatusListenerTransport extends IInternalConnectionListener.Stub {
    ...
    private WifiStatusListenerTransport(NearbyConfig.BusinessTypeEnum businessType, int businessId, NearbyConfiguration configuration, NearbyDevice device, WifiStatusListener listener, Looper looper) {
        ...
        this.mListenerHandler = new Handler(looper) {
            /* class com.huawei.nearbysdk.clone.WifiStatusListenerTransport.AnonymousClass1 */
    ...
    private synchronized void callBackOnStatusChange(int status) {
        ...
        HwLog.i(TAG, String.format("callBackOnStatusChange: mListener=%s state=%d->%d", this.mListener, Integer.valueOf(status), Integer.valueOf(state)));
    ...
    private synchronized void callBackOnConnectionChange(NearbyDevice device, int status) {
        ...
        HwLog.i(TAG, String.format("callBackOnConnectionChange: mListener=%s state=%d->%d device=%s band=%d ipAddr=%s port=%d", this.mListener, Integer.valueOf(status), Integer.valueOf(connectionState), device, Integer.valueOf(band), NearbyConfig.toSecureString(ipAddr), Integer.valueOf(port)));

...
public class KvClient<K extends Key, V extends Value> extends DualProxy<DataServiceProxy, IKvServiceCall> {
    ...
    public V get(K key) {
        ...
        V v2 = pair == null ? null : (V) pair.getValue();
        return v;
    }
    ...
    public List<KvPair> get(List<K> keys) {
        if (keys == null || keys.isEmpty()) {
            ...
            try {
                ObjectContainer outContainer = ((IKvServiceCall) this.remote).get(new ObjectContainer(keys.get(0).getClass(), keys, this.pkgName));
    ...
    public boolean delete(List<K> keys) {
        if (keys == null || keys.isEmpty()) {
            ...
            try {
                return ((IKvServiceCall) this.remote).delete(new ObjectContainer(keys.get(0).getClass(), keys, this.pkgName));
            } catch (RemoteException | RuntimeException e) {

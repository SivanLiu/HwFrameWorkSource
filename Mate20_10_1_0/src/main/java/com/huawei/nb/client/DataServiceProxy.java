...
public class DataServiceProxy extends Proxy<IDataServiceCall> implements IClient {
    ...
    private List executeQueryDirect(IQuery query) {
        ...
        try {
            ObjectContainer objectContainer = ((IDataServiceCall) this.remote).executeQueryDirect(new QueryContainer(query, this.pkgName));
    ...
    public <T extends AManagedObject> T executeInsert(T entity) {
        ...
        List<T> insertedEntities = executeInsert(Arrays.asList(entity));
    ...
    public <T extends AManagedObject> List<T> executeInsert(List<T> entities) {
        if (entities == null || entities.isEmpty()) {
            ...
            try {
                ObjectContainer objectContainer = ((IDataServiceCall) this.remote).executeInsertDirect(new ObjectContainer<>(entities.get(0).getClass(), entities, this.pkgName));
                if (objectContainer != null && objectContainer.get() != null && !objectContainer.get().isEmpty()) {
                    return this.odmfHelper.assignObjectContext(objectContainer.get());
                }
    ...
    public <T extends AManagedObject> boolean executeInsertEfficiently(T entity) {
        ...
        return executeInsertEfficiently(Arrays.asList(entity));
    }
    ...
    public <T extends AManagedObject> boolean executeInsertEfficiently(List<T> entities) {
        if (entities == null || entities.isEmpty()) {
            ...
            try {
                if (((IDataServiceCall) this.remote).executeInsertEfficiently(new ObjectContainer<>(entities.get(0).getClass(), entities, this.pkgName)) >= 0) {
                    return true;
                }
    ...
    public <T extends AManagedObject> boolean executeUpdate(T entity) {
        ...
        return executeUpdate(Arrays.asList(entity));
    }
    ...
    public <T extends AManagedObject> boolean executeUpdate(List<T> entities) {
        ...
        if (entities == null || entities.isEmpty()) {
            ...
            int updatedCount = -1;
            try {
                updatedCount = ((IDataServiceCall) this.remote).executeUpdateDirect(new ObjectContainer<>(entities.get(0).getClass(), entities, this.pkgName));
            } catch (RemoteException | RuntimeException e) {
    ...
    public <T extends AManagedObject> boolean executeDelete(T entity) {
        ...
        return executeDelete(Arrays.asList(entity));
    }
    ...
    public <T extends AManagedObject> boolean executeDelete(List<T> entities) {
        ...
        if (entities == null || entities.isEmpty()) {
            ...
            int deletedCount = -1;
            try {
                deletedCount = ((IDataServiceCall) this.remote).executeDeleteDirect(new ObjectContainer<>(entities.get(0).getClass(), entities, this.pkgName), false);
            } catch (RemoteException | RuntimeException e) {
    ...
    public boolean grantDatabaseAuthority(String databaseName, String grantPkgName, int authority) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(databaseAuthorityGrant.getClass(), Arrays.asList(databaseAuthorityGrant), this.pkgName, databaseName), 2)) {
            return true;
        }
    ...
    public boolean revokeDatabaseAuthority(String databaseName, String revokePkgName) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(databaseAuthorityGrant.getClass(), Arrays.asList(databaseAuthorityGrant), this.pkgName, databaseName), 3)) {
            return true;
        }
    ...
    public boolean grantTableAuthority(Class clazz, String grantPkgName, int authority) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(tableAuthorityGrant.getClass(), Arrays.asList(tableAuthorityGrant), this.pkgName, dbName), 2)) {
            return true;
        }
    ...
    public boolean revokeTableAuthority(Class clazz, String revokePkgName) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(tableAuthorityGrant.getClass(), Arrays.asList(tableAuthorityGrant), this.pkgName, dbName), 3)) {
            return true;
        }
    ...
    public boolean grantFieldAuthority(Class clazz, String fieldName, String grantPkgName, int authority) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(fieldAuthorityGrant.getClass(), Arrays.asList(fieldAuthorityGrant), this.pkgName, dbName), 2)) {
            return true;
        }
    ...
    public boolean revokeFieldAuthority(Class clazz, String fieldName, String revokePkgName) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(fieldAuthorityGrant.getClass(), Arrays.asList(fieldAuthorityGrant), this.pkgName, dbName), 3)) {
            return true;
        }
    ...
    public <T extends ManagedObject> List<T> executeInsertWithGroup(List<T> entities, String groupName) {
        ...
        try {
            ObjectContainer objectContainer = ((IDataServiceCall) this.remote).executeInsertDirect(new ObjectContainer<>(entities.get(0).getClass(), entities, this.pkgName, null, groupName));
            if (objectContainer != null && objectContainer.get() != null && !objectContainer.get().isEmpty()) {
                return this.odmfHelper.assignObjectContext(objectContainer.get());
            }
    ...
    public <T extends ManagedObject> boolean moveObjectToGroup(List<T> entities, String groupName) {
        ...
        String tableName = entities.get(0).getClass().getSimpleName();
        List<ManagedObject> objects = new ArrayList<>();
        int size = entities.size();
        for (int i = 0; i < size; i++) {
            ZTupleAuthorityGrant tupleAuthorityGrant = new ZTupleAuthorityGrant();
            tupleAuthorityGrant.setTableName(tableName);
            tupleAuthorityGrant.setPackageName(this.pkgName);
            tupleAuthorityGrant.setTupleId(entities.get(i).getRowId());
            tupleAuthorityGrant.setReserved(groupName);
            objects.add(tupleAuthorityGrant);
        }
        if (handleAuthorityGrant(new ObjectContainer<>(objects.get(0).getClass(), objects, this.pkgName, entities.get(0).getDatabaseName()), 1)) {
            return true;
        }
    ...
    public boolean createGroup(Class clazz, String groupName) {
        ...
        if (handleAuthorityGrant(new ObjectContainer<>(group.getClass(), Arrays.asList(group), this.pkgName, reflectToGetDatabaseName(clazz)), 2)) {
            return true;
        }
    ...
    public boolean dropGroup(Class clazz, String groupName) {
        ...
        group.setOwner(this.pkgName);
        group.setIsGroupIdentifier(true);
        if (handleAuthorityGrant(new ObjectContainer<>(group.getClass(), Arrays.asList(group), this.pkgName, reflectToGetDatabaseName(clazz)), 3)) {
            return true;
        }
    ...
    public List<DataLifeCycle> queryDataLifeCycleConfig(String dbName, String tableName) {
        ...
        if (dbName == null || dbName.equals("")) {
            ...
            DSLog.d("query dataLifeCycleConfig for db[%s] table[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), this.pkgName);
            DataLifeCycle dataLifeCycle = new DataLifeCycle();
            dataLifeCycle.setMDBName(dbName);
            dataLifeCycle.setMTableName(tableName);
            ...
            if (this.remote != null) {
                try {
                    ObjectContainer result = ((IDataServiceCall) this.remote).handleDataLifeCycleConfig(0, objectContainer);
                    if (result != null) {
                    ...
                    DSLog.e("Failed to query dataLifeCycleConfig, error: query result null with db[%s] table[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), this.pkgName);
                    return null;
                } catch (RemoteException | RuntimeException e) {
    ...
    public int addDataLifeCycleConfig(String dbName, String tableName, String fieldName, int mode, int count) {
        ...
        dataLifeCycle.setMDBName(dbName);
        dataLifeCycle.setMTableName(tableName);
        dataLifeCycle.setMFieldName(fieldName);
        dataLifeCycle.setMMode(Integer.valueOf(mode));
        dataLifeCycle.setMCount(Integer.valueOf(count));
        ObjectContainer objectContainer = new ObjectContainer(DataLifeCycle.class, Collections.singletonList(dataLifeCycle), this.pkgName);
        if (this.remote != null) {
            try {
                ObjectContainer resultObj = ((IDataServiceCall) this.remote).handleDataLifeCycleConfig(1, objectContainer);
                if (resultObj == null || resultObj.get() == null || resultObj.get().isEmpty()) {
                    DSLog.e("Failed to add dataLifeCycleConfig, error: add result null with db[%s] table[%s] fieldName[%s] mode[%s] count[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), DbLogUtil.getSafeNameForLog(fieldName), Integer.valueOf(mode), Integer.valueOf(count), this.pkgName);
                    return 4;
                } else if (resultObj.get().get(0) instanceof Integer) {
                    return ((Integer) resultObj.get().get(0)).intValue();
                } else {
                    DSLog.e("Failed to add dataLifeCycleConfig, error: result is not a integer with db[%s] table[%s] fieldName[%s] mode[%s] count[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), DbLogUtil.getSafeNameForLog(fieldName), Integer.valueOf(mode), Integer.valueOf(count), this.pkgName);
                    return 4;
                }
            } catch (RemoteException | RuntimeException e) {
    ...
    public int removeDataLifeCycleConfig(String dbName, String tableName, String fieldName, int mode, int count) {
        ...
        if (this.remote != null) {
            try {
                ObjectContainer resultObj = ((IDataServiceCall) this.remote).handleDataLifeCycleConfig(2, objectContainer);
                if (resultObj == null || resultObj.get() == null || resultObj.get().isEmpty()) {
                    DSLog.e("Failed to remove dataLifeCycleConfig, error: add result null with db[%s] table[%s] fieldName[%s] mode[%s] count[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), DbLogUtil.getSafeNameForLog(fieldName), Integer.valueOf(mode), Integer.valueOf(count), this.pkgName);
                    return 4;
                } else if (resultObj.get().get(0) instanceof Integer) {
                    return ((Integer) resultObj.get().get(0)).intValue();
                } else {
                    DSLog.e("Failed to remove dataLifeCycleConfig, error: result is not a integer with db[%s] table[%s] fieldName[%s] mode[%s] count[%s] pkgname[%s].", DbLogUtil.getSafeNameForLog(dbName), DbLogUtil.getSafeNameForLog(tableName), DbLogUtil.getSafeNameForLog(fieldName), Integer.valueOf(mode), Integer.valueOf(count), this.pkgName);
                    return 4;
                }
            } catch (RemoteException | RuntimeException e) {

...
public class AiModelClientAgent extends DualProxy<DataServiceProxy, IAIServiceCall> {
    ...
    private boolean batchSetFakePrivateKey(List<AiModelResponse> responseList) {
        for (AiModelResponse response : responseList) {
            response.setPrivateKey(FAKE_PRIVATE_KEY_NAME);
        }
        return true;
    }
    ...
    private boolean batchSetFakePublicKey(List<AiModelRequest> requestList) {
        for (AiModelRequest request : requestList) {
            request.setPublicKey(FAKE_PUBLIC_KEY_NAME);
        }
        return true;
    }

    public AiModelResponse requestAiModel(AiModelRequest request) {
        List<AiModelResponse> responseList = requestAiModel(Arrays.asList(request));
    ...
    public AiModelResponse requestAiModelWithoutEncrypt(AiModelRequest request) {
        List<AiModelResponse> responseList = requestAiModelWithoutEncrypt(Arrays.asList(request));
        if (responseList == null || responseList.isEmpty()) {
            return null;
        }
        return responseList.get(0);
    ...
    public List<AiModelResponse> requestAiModel(List<AiModelRequest> requestList) {
        ...
        if (requestList == null || !batchValidRequest(requestList)) {
            ...
            if (this.callbackTimeout > 0) {
                ...
                responseList = requestAiModel(requestContainer);
            }
    ...
    public List<AiModelResponse> requestAiModelWithoutEncrypt(List<AiModelRequest> requestList) {
        List<AiModelResponse> responseList;
        int size;
        if (requestList == null || !batchValidRequest(requestList)) {
            DSLog.et(TAG, "Failed to get ai models, error: invalid parameter.", new Object[0]);
            ...
            DSLog.et(TAG, "Failed to request Ai model, error: not connected to ai service.", new Object[0]);
            return null;
        } else if (!batchSetFakePublicKey(requestList)) {
            ...
            if (responseList == null || responseList.size() != requestList.size()) {
                ...
                DSLog.et(TAG, "Failed to get ai models, error: just get %s for %s requests.", objArr);
                return null;
            } else if (batchSetFakePrivateKey(responseList)) {
                return responseList;
            } else {
    ...
    public boolean deleteResInfo(ResourceInformation resourceInformation) {
        ...
        deleteResInfo(Arrays.asList(resourceInformation), new DeleteResInfoFromDbCallBack(isDeletes, latch));
        try {
        ...
        DSLog.dt(TAG, "res info is deleted, %s.", Boolean.valueOf(isDeletes[0]));
        return isDeletes[0];
    }
    ...
    private static class DeleteResInfoFromDbCallBack implements DeleteResInfoCallBack {
        ...
        DeleteResInfoFromDbCallBack(boolean[] isDelete, CountDownLatch latch2) {
            this.isDeletes = isDelete;
            this.latch = latch2;
        }
        ...
        public void onFailure(int errorCode, String errorMessage) {
            ...
            DSLog.dt(AiModelClientAgent.TAG, "Fail to delete res info,error: errorCode = " + errorCode + "errorMessage = " + msg, new Object[0]);
            ...
            UpdatePackageCheckCallBackAgent agentCallBack = new UpdatePackageCheckCallBackAgent() {
                ...
                public void onFinish(ObjectContainer oc, int type) {
                    ...
                    if (oc == null || oc.get() == null) {
                        ...
                        cb.onFinish(new ArrayList(0), networkType);
                        return;
                    }
    ...
    public void updatePackage(List<ResourceInformation> resources, final UpdatePackageCallBack cb, long refreshInterval, long refreshBucketSize, boolean isWifiOnly) {
        if (cb == null) {
        ...
        UpdatePackageCallBackAgent agentCallBack = new UpdatePackageCallBackAgent() {
            ...
            public int onRefresh(int status, long totalSize, long downloadedSize, int totalPackages, int downloadedPackages, int errorCode, String errorMessage) {
                ...
                if (cb instanceof UpdatePackageControllableCallBack) {
                    return ((UpdatePackageControllableCallBack) cb).onControllableRefresh(updateStatus, totalSize, downloadedSize, totalPackages, downloadedPackages, errorCode, errorMessage);
                }
                cb.onRefresh(updateStatus, totalSize, downloadedSize, totalPackages, downloadedPackages, errorCode, errorMessage);
                return 0;
            }
        };
        if (cb instanceof UpdatePackageControllableCallBack) {
        ...
        try {
            ((IAIServiceCall) this.remote).updatePackageAgent(oc, agentCallBack, refreshInterval, refreshBucketSize, isWifiOnly);
        } catch (RemoteException | RuntimeException e) {
    ...
    private static class DeleteUpdatedResInfoCallBack extends DeleteResInfoCallBackAgent {
        ...
        @Override // com.huawei.nb.callback.IDeleteResInfoCallBack, com.huawei.nb.client.callback.DeleteResInfoCallBackAgent
        public void onSuccess() {
            this.mCb.onSuccess();
        }
        ...
        public void onFailure(int errorCode, String errorMessage) {
            ...
            this.mCb.onFailure(errorCode, msg);
        }
    }

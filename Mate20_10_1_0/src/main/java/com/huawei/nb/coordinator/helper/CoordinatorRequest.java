...
final class CoordinatorRequest {
    ...
    private boolean isOversea() {
        return DeviceUtil.getDistrict().equals(DeviceUtil.OVERSEA);
    }
    ...
    private boolean isServiceSwitchOn() {
        ...
        } else if (getBusiness() == 3 || getBusiness() == 2) {
            return true;
        } else {
    ...
    private boolean isSuccessCode(HttpResponse response) {
        ...
        if (getBusiness() == 1) {
            ...
            DSLog.d("CoordinatorRequest Response code is " + code, " msg is" + CoordinatorJsonAnalyzer.getJsonValue(responseString, CoordinatorJsonAnalyzer.MSG_TYPE));
            return !TextUtils.isEmpty(expectedStatusCode) && !TextUtils.isEmpty(code) && code.equals(expectedStatusCode);
    ...
    private void failResultReport(HttpResponse httpResponse, boolean isShieldCloudError, CoordinatorAudit coordinatorAudit) {
        ...
        int errorType = getErrorType(httpResponse, isShieldCloudError);
        ...
        switch (errorType) {
            ...
            case ResultStatusCodeEnum.SHIELD_CLOUD_ERROR:
                ...
                break;
            case ResultStatusCodeEnum.EMPTY_SIGNATURE_ERROR_CODE:
            case ResultStatusCodeEnum.INTERRUPT_CODE:
            case ResultStatusCodeEnum.NETWORK_NOT_CONNECTED:
            case -4:
            default:
    ...
    private CoordinatorCloudErrorAudit generateCloudErrorAudit(HttpResponse httpResponse, CoordinatorAudit coordinatorAudit, String businessCode) {
        CoordinatorCloudErrorAudit audit = new CoordinatorCloudErrorAudit();
        audit.addStatusCodeToList(String.valueOf(httpResponse.getStatusCode()));
        audit.addExceptionMessageToList(httpResponse.getHttpExceptionMsg());
        audit.addBusinessCodeToList(businessCode);
        audit.addResponseMessageToList(httpResponse.getResponseMsg());
        audit.addPackageNameToList(coordinatorAudit.getAppPackageName());
        audit.addUrlToList(coordinatorAudit.getUrl());
        audit.addNetworkToList(coordinatorAudit.getNetWorkState());
        audit.addDateToList(coordinatorAudit.getRequestDate());
        return audit;
    }
    ...
    private CoordinatorNetworkErrorAudit generateNetworkErrorAudit(HttpResponse httpResponse, CoordinatorAudit coordinatorAudit) {
        CoordinatorNetworkErrorAudit audit = new CoordinatorNetworkErrorAudit();
        audit.addStatusCodeToList(String.valueOf(httpResponse.getStatusCode()));
        audit.addResponseMessageToList(httpResponse.getResponseMsg());
        audit.addPackageNameToList(coordinatorAudit.getAppPackageName());
        audit.addUrlToList(coordinatorAudit.getUrl());
    ...
    private boolean isTokenOrSessionExpired(HttpResponse result) {
        ...
        if (!TextUtils.isEmpty(code)) {
            return IVerifyVar.TOKEN_HAS_EXPIRED.equals(code) || IVerifyVar.SESSION_HAS_EXPIRED.equals(code);
        }
        return false;
    }

...
public class HttpClient {
    ...
    public HttpClient setDataTrafficSize(Long dataTrafficSize2) {
        this.dataTrafficSize = dataTrafficSize2.longValue();
        return this;
    }
    ...
    private boolean deleteDir(File dir) {
        ...
        if (dir.isDirectory() && (children = dir.list()) != null) {
            ...
            while (i < length) {
                ...
                try {
                    if (!deleteDir(FileUtils.getFile(dir.getCanonicalPath(), child))) {
    ...
    public HttpResponse syncExecute(CoordinatorAudit coordinatorAudit) {
        ...
        try {
            ...
            if (url == null) {
                ...
                HttpURLConnection connection2 = parseUrlConnection(url.openConnection());
                if (connection2 == null) {
                ...
                initConnection(connection2);
                if (this.requestMethod.equals(POST_TYPE)) {
                    connection2.setDoInput(true);
                    connection2.setDoOutput(true);
                    addPostRequestBody(connection2);
                }
                requestAccordingMode(connection2, coordinatorAudit);
                if (connection2 != null) {
                    connection2.disconnect();
                }
    ...
    private void requestForTransferBreakpointFile(HttpURLConnection connection, CoordinatorAudit coordinatorAudit) {
        ...
        try {
            ...
            try {
                String tmpFileCanonicalPath = resumeDownload2.generateTmpFilePath();
                ...
                try {
                    ...
                    if (resumeDownload != null && !TextUtils.isEmpty(null) && 0 != 0) {
                        BreakpointResumeDownload.removePathFromSet(null);
                    }
                } catch (Throwable th) {
                    th = th;
                    BreakpointResumeDownload.removePathFromSet(null);
                ...
                if (isContentLengthValid(connection, coordinatorAudit)) {
                    ...
                    if (!isInputStreamEmpty(0, is)) {
                        ...
                        while (true) {
                            ...
                            if (downloadedLength > this.dataTrafficSize) {
        ...
        try {
            ...
            try {
                ...
                while (readPosition < fileSize) {
                    ...
                    if (downloadedFileLength > this.dataTrafficSize) {
    ...
    private Map<String, List<String>> requestForHeader(CoordinatorAudit coordinatorAudit) {
        ...
        try {
            ...
            HttpURLConnection headConnection2 = parseUrlConnection(headUrl.openConnection());
            if (headConnection2 == null) {
                setErrorHttpResponse(-2, " connection is illegal. Request stops.");
                HashMap hashMap2 = new HashMap();
                if (headConnection2 == null) {
                    return hashMap2;
                }
                headConnection2.disconnect();
                return hashMap2;
            }
            initConnection(headConnection2);
            headConnection2.setRequestMethod(HEAD_TYPE);
            int responseCode = headConnection2.getResponseCode();
            String responseMessage = fullfillEmptyMessage(headConnection2.getResponseMessage());
            DSLog.d("HttpClient response code of HEAD request : " + responseCode, new Object[0]);
            this.response.setResponseMsg(responseMessage);
            if (responseCode != 200 || isContentLengthValid(headConnection2, coordinatorAudit)) {
                if (headConnection2 != null) {
                    headConnection2.disconnect();
                }
                return headConnection2.getHeaderFields();
            }
    ...
    private boolean isContentLengthValid(HttpURLConnection connection, CoordinatorAudit coordinatorAudit) {
        ...
        try {
            ...
            if (contentLength > this.dataTrafficSize) {

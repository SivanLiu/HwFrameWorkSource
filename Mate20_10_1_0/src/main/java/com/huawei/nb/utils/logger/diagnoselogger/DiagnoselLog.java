...
public class DiagnoselLog {
    ...
    public static class Detail {
        ...
        public Detail(String body, Object... args) {
    ...
    public static class Hint {
        ...
        public Hint(String body, Object... args) {
            if (!(args == null || args.length == 0)) {
                body = String.format(body, args);
            }
            this.log = body;
    ...
    private static String logString(String logTag, DiagnoseLogTitle.Type title, Detail detail, Hint hint) {
        StringBuilder builder = new StringBuilder();
        builder.append("\n");
        builder.append(logTag);
        builder.append(mTitles.get(title));
        if (detail != null && !TextUtils.isEmpty(detail.log)) {
            builder.append("\n");
            builder.append(CAUSE_DETAIL_TAG);
            builder.append(detail.log);
        }
        if (hint != null && !TextUtils.isEmpty(hint.log)) {
            builder.append("\n");
            builder.append(ACTION_HINT_TAG);
            builder.append(hint.log);
        }
        return builder.toString();
    }

    public static void e(DiagnoseLogTitle.Type title, Detail detail, Hint hint) {
        if (!title.equals(DiagnoseLogTitle.Type.ODMF_INNER)) {
            DSLog.e(logString(ELOG_TAG, title, detail, hint), new Object[0]);
        } else if (detail != null) {
            DSLog.e(detail.log, new Object[0]);
        }
    }

    public static void w(DiagnoseLogTitle.Type title, Detail detail, Hint hint) {
        if (!title.equals(DiagnoseLogTitle.Type.ODMF_INNER)) {
            DSLog.w(logString(WLOG_TAG, title, detail, hint), new Object[0]);
        } else if (detail != null) {
            DSLog.w(detail.log, new Object[0]);
        }
    }

    public static void i(DiagnoseLogTitle.Type title, Detail detail, Hint hint) {
        if (!title.equals(DiagnoseLogTitle.Type.ODMF_INNER)) {
            DSLog.i(logString(ILOG_TAG, title, detail, hint), new Object[0]);
        } else if (detail != null) {
            DSLog.i(detail.log, new Object[0]);
        }
    }

    public static void d(DiagnoseLogTitle.Type title, Detail detail, Hint hint) {
        if (!title.equals(DiagnoseLogTitle.Type.ODMF_INNER)) {
            DSLog.d(logString(DLOG_TAG, title, detail, hint), new Object[0]);
        } else if (detail != null) {

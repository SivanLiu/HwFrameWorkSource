...
public class HWFeatureCompat {
    ...
    public static synchronized int loadLocalSysSplitApks(Context context, Set<FeatureApkInfo> set) {
                ...
                if (nativeSysInstalling == 0) {
                    ...
                    while (true) {
                        if (!it.hasNext()) {
                            ...
                            i = createDynamicInstaller.dexInstall(classLoader, mStorageManager.getSplitDexDir(next2.getSplitName()), next2.getSplitApk());
                            if (isDexApk && i != 0) {

...
public class HwRouterInternetDetector {
    ...
    public void handleHttpGetGatewayDetect(int sessionId) {
        ...
        } else if (!this.mDisconnected.get() && (str = this.mGatewayAddr) != null && WifiProCommonUtils.isNetworkReachableByICMP(str, 1000)) {
            this.mHandler.sendEmptyMessage(102);
    ...
    private boolean isGatewayReachable(int sessionId) {
        this.mNetwork = (Network) WifiManagerEx.ctrlHwWifiNetwork(HwWifiProService.SERVICE_NAME, 14, new Bundle()).getParcelable(NetworkMonitor.KEY_NETWORK_NAME);

...
public class WifiProHistoryRecordManager implements IGetApRecordCount {
    ...
    public final LruCache<String, WifiProApInfoRecord> mApInfoCache = new LruCache<>(10);
    /* access modifiers changed from: private */
    ...
    private void initFirstConnectApInfo(WifiProApInfoRecord apInfo) {
        if (apInfo != null) {
            ...
            HwHiLog.d(TAG, false, "initFirstConnectApInfo enter for ssid:%{public}s, currDateMs %{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID), String.valueOf(currDateMs)});
        }
    }
    ...
    public void updateCurrentApInfo(String bssid, String ssid, int securityType) {
        if (bssid == null) {
            ...
            if (wifiProApInfoRecord != null) {
                HwHiLog.i(TAG, false, "******** updateCurrentApInfo, AP:%{public}s was disconnected. set CurrentApInfo=null", new Object[]{WifiProCommonUtils.safeDisplaySsid(wifiProApInfoRecord.apSSID)});
        ...
        if (wifiProApInfoRecord2 == null || !bssid.equals(wifiProApInfoRecord2.apBSSID)) {
            HwHiLog.i(TAG, false, "######## updateCurrentApInfo new SSID:%{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(ssid)});
    ...
    private void afterConnectProcess(WifiProApInfoRecord apInfo) {
        if (apInfo != null && this.mApInfoProcessHandler != null) {
            HwHiLog.d(TAG, false, "afterConnectProcess enter for ssid:%{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID)});
    ...
    public void periodSaveRecord(WifiProApInfoRecord apInfo) {
        ...
        if (!apInfo.isEnterpriseAP) {
            HwHiLog.d(TAG, false, "periodSaveRecord enter for normal ssid:%{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID)});
            apInfo.lastConnectTime = getCurrentTimeMs();
    ...
    public void afterDisconnectProcess(WifiProApInfoRecord apInfo) {
        if (apInfo != null && this.mDBMgr != null) {
            if (!apInfo.isEnterpriseAP) {
                HwHiLog.d(TAG, false, "afterDisconnectProcess enter for normal ssid:%{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID)});
                apInfo.lastConnectTime = getCurrentTimeMs();
                updateConnectTime(apInfo, apInfo.connectStartTimeSave, apInfo.lastConnectTime);
                ...
                HwHiLog.d(TAG, false, "afterDisconnectProcess enter ssid:%{public}s, is enterprise AP.", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID)});
            }
            WifiProStatisticsManager wifiProStatisticsManager = this.mWifiProStatisticsManager;
            if (wifiProStatisticsManager != null) {
                wifiProStatisticsManager.updateStatisticToDB();
    ...
    public boolean isEnterpriseAP(String ssid, int secType) {
        ...
        if (wifiProEnterpriseApRecord != null && wifiProEnterpriseApRecord.apSSID.equals(ssid) && this.mWifiProEnterpriseApRecord.apSecurityType == secType) {
            retVal2 = true;
        }
        if (!retVal2) {
    ...
    private void updateConnectTime(WifiProApInfoRecord apInfo, long startDate, long endDate) {
        if (apInfo != null) {
            ...
            int startSecondsOfDay = (apInfo.lastRecordHour * SECOND_OF_ONE_HOUR) + (apInfo.lastRecordMin * 60) + apInfo.lastRecordSec;
            updateRecordTime(apInfo);
            int endSecondsOfDay = (this.mCurrentHour * SECOND_OF_ONE_HOUR) + (this.mCurrentMinute * 60) + this.mCurrentSecond;
    ...
    private boolean checkIsHomeAP(WifiProApInfoRecord apInfo) {
        ...
        HwHiLog.i(TAG, false, "checkIsHomeAP SSID:%{public}s is home AP", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.apSSID)});
        return true;
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 100:
                    ...
                    if (setPara == null) {
                        HwHiLog.d(WifiProHistoryRecordManager.TAG, false, "update ap info null error.", new Object[0]);
                        return;
                    }
    ...
    public boolean saveDualBandApInfo(WifiProDualBandApInfoRcd apInfo) {
        ...
        HwHiLog.d(TAG, false, "saveDualBandApInfo apInfo.mApSSID = %{public}s", new Object[]{WifiProCommonUtils.safeDisplaySsid(apInfo.mApSSID)});

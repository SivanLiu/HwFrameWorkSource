...
public class HwPCDataReporter extends DefaultHwPCDataReporter {
    ...
    private static final String REPORT_FAIL_TO_CONN_EVENT = "reportFailToConnEvent";
    ...
    public void reportFailToConnEvent(int exceptionType, String serviceName, DisplayInfoEx di) {
        if (this.mHandler == null) {
            ...
            eventParams.put((short) 2, Integer.valueOf(di.getType()));
            eventParams.put((short) 3, Integer.valueOf(di.getLogicalDensityDpi()));
            eventParams.put((short) 4, Integer.valueOf(di.getLogicalWidth()));
            eventParams.put((short) 5, Integer.valueOf(di.getLogicalHeight()));
            eventParams.put((short) 6, di.getName());
            EventSender sender = new EventSender();
            sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, FAIL_TO_CONNECT_EVENT_ID);
            this.mHandler.post(sender);
            this.mFailToConnHasReportOnce = true;
        }
    }

    public void reportFailSwitchEvent(int reason, int projMode, DisplayInfoEx di) {
        if (this.mHandler == null) {
            ...
            eventParams.put((short) 5, Integer.valueOf(di.getLogicalHeight()));
            try {
                eventParams.put((short) 7, Integer.valueOf(Integer.parseInt(di.getUniqueId())));
            } catch (NumberFormatException e) {
                HwPCUtils.log(TAG, "fail to convert uniqueId");
            }
            eventParams.put((short) 6, di.getName());
            EventSender sender = new EventSender();
            sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, FAIL_TO_SWITCH_EVENT_ID);
            this.mHandler.post(sender);
            this.mFailToSwitchHasReportOnce = true;
        }
    }

    public void reportFailEnterPadEvent(int reason, DisplayInfoEx di) {
        if (this.mHandler == null) {
            ...
            Map<Short, Object> eventParams = new HashMap<>();
            eventParams.put((short) 0, Integer.valueOf(reason));
            eventParams.put((short) 1, Integer.valueOf(di.getType()));
            eventParams.put((short) 2, Integer.valueOf(di.getLogicalDensityDpi()));
            eventParams.put((short) 3, Integer.valueOf(di.getLogicalWidth()));
            eventParams.put((short) 4, Integer.valueOf(di.getLogicalHeight()));
            try {
                eventParams.put((short) 5, Integer.valueOf(Integer.parseInt(di.getUniqueId())));
            } catch (NumberFormatException e) {
                HwPCUtils.log(TAG, "fail to convert uniqueId");
            }
            EventSender sender = new EventSender();
            sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, FAIL_TO_ENTER_PAD_PC_EVENT_ID);
            this.mHandler.post(sender);
    ...
    public void reportKillProcessEvent(String packageName, String processName, int sourceDisplayId, int targetDisplayId) {
        ...
        EventSender sender = new EventSender();
        sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, KILL_PROCESS_EVENT_ID);
        this.mHandler.post(sender);
    }
    ...
    public void reportFailLightScreen(int exceptionType, int keyCode, String packageName) {
        if (this.mHandler == null) {
            ...
            EventSender sender = new EventSender();
            sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, FAIL_TO_LIGHT_SCREEN_EVENT_ID);
            this.mHandler.post(sender);
            this.mFailToLightHasReportOnce = true;
        }
    }

    public void reportBadViewportEvent(DisplayInfoEx di) {
        if (this.mHandler == null) {
            ...
            Map<Short, Object> eventParams = new HashMap<>();
            eventParams.put((short) 0, Integer.valueOf(di.getType()));
            eventParams.put((short) 1, Integer.valueOf(di.getLogicalDensityDpi()));
            eventParams.put((short) 2, Integer.valueOf(di.getLogicalWidth()));
            eventParams.put((short) 3, Integer.valueOf(di.getLogicalHeight()));
            try {
                eventParams.put((short) 4, Integer.valueOf(Integer.parseInt(di.getUniqueId())));
            } catch (NumberFormatException e) {
                HwPCUtils.log(TAG, "fail to convert uniqueId");
            }
            eventParams.put((short) 5, di.getName());
            EventSender sender = new EventSender();
            sender.setParams(eventParams, REPORT_FAIL_TO_CONN_EVENT, BAD_VIEWPORT_EVENT_ID);
            this.mHandler.post(sender);
    ...
    private static class EventSender implements Runnable {
        ...
        public void run() {
            if (this.mParams != null) {
                IMonitorExt.EventStreamExt eStream = IMonitorExt.openEventStream(this.mEventId);
                try {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (value instanceof Integer) {
                            eStream.setParam(eStream, key.shortValue(), ((Integer) value).intValue());
                        } else if (value instanceof String) {
                            eStream.setParam(eStream, key.shortValue(), (String) value);
                        } else {
                            HwPCUtils.log(HwPCDataReporter.TAG, "error, not known value type, value:" + value);
                        }
                    }
                    IMonitorExt.sendEvent(eStream);
                } finally {
                    if (eStream != null) {

...
public class HwPCMkManager {
    ...
    public static final boolean DEBUG = SystemPropertiesEx.getBoolean("hw_pc_mkmanager_debug", false);
    ...
    private void sendScrollEventIfNecessary(MotionEvent event) {
        ...
        if (this.mRecentEventPointers1stFinger.size() < 2 || this.mRecentEventPointers2ndFinger.size() < 2 || this.mSendZoomGestureDirectly) {
            ...
            onTouchDownEvent(this.mDownEventForTouch, null);
            onTouchEvent(event, 2, 0, false);
            this.mCurrentEventX = event.getX();
            this.mCurrentEventY = event.getY();
    ...
    public boolean sendPointerSync(InputEvent event) {
        if (this.mInputManager == null) {
            this.mInputManager = (HwInputManagerService.HwInputManagerLocalService) LocalServicesExt.getService(HwInputManagerService.HwInputManagerLocalService.class);
        }
        if (this.mInputManager == null) {
            return false;
        }

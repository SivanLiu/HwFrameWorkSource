...
public class HwMultiWindowTips {
    private static final int DIVIDE_EQUALLY = 2;
    private static final String DOCK_BAR_ACTION = "huawei.intent.action.MULTIWINDOW_TIPS";
    private static final String DOCK_TIP_POS = "tipposition";
    ...
    private static final int SHOWN_NONE_TIP = 0;
    ...
    private static final int TIP_TYPE_INVALID = 0;
    ...
    public void updateFloatingWinCount() {
    ...
    /* access modifiers changed from: private */
    public void handleNotifyDockBar(int type) {
        notifyDockBar(type);
    }

    /* access modifiers changed from: private */
    public void updateMultiWinTipStat() {
        ...
        this.mStartFloatingWinCount = Settings.Secure.getIntForUser(this.mResolver, "start_floatwin_count", 0, ActivityManager.getCurrentUser());
        if (ISDEBUG) {
    ...
    private void registerMultiWinStat() {
        ...
        this.mResolver.registerContentObserver(Settings.Secure.getUriFor("start_floatwin_count"), false, this.mMultiWinObserver, -1);
    }
    ...
    private void setFloatWinStartCount() {
        ...
        long identity = Binder.clearCallingIdentity();
        try {
            Settings.Secure.putIntForUser(this.mResolver, "start_floatwin_count", this.mStartFloatingWinCount, ActivityManager.getCurrentUser());
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
    }

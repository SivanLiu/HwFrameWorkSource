...
class CrossPersistentStore extends PersistentStore {
    ...
    CrossPersistentStore(Context appCtx, String modelPath, String uri, Configuration configuration, List<String> databasePaths2, List<byte[]> keyList) {
        ...
        if (modelPath == null || modelPath.equals("")) {
            ...
            try {
                setModel(ObjectModelFactory.parse(appCtx, modelPath));
        ...
        init(configuration.getStorageMode() == 401 ? null : databaseName, configuration.isThrowException(), configuration.isDetectDelete());
    ...
    private void init(String databaseName, boolean throwException, boolean detectDelete) {
        try {

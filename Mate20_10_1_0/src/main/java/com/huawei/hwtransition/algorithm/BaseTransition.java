...
public abstract class BaseTransition {
    ...
    int mBreakTimes = 1;
    float mCameraZ = -8.0f;
    boolean mIsAlphaMode = true;
    boolean mIsEdge = false;
    boolean mIsScrolling = false;
    boolean mIsSetCameraZ = false;
    public boolean mIsUseBg = false;
    int mLayoutType = 0;
    int mOrientation = 0;
    int mPageSpacing = 0;
    ...
    public static class TransformationInfo {
        ...
        public boolean mIsAlphaDirty = false;
        public boolean mIsBackgroundDirty = false;
        public boolean mIsBoundsDirty = false;
        public boolean mIsMatrixDirty = false;
        public final Matrix mMatrix = new Matrix();
        public float[] mMatrix3Ds = new float[16];
        ...
        public void clearDirty() {
            this.mIsAlphaDirty = false;
            this.mAlpha = 1.0f;
            this.mIsMatrixDirty = false;
    ...
    public TransformationInfo getTransformation(int part, boolean isOverScrollFirst, boolean isOverScrollLast, float scrollProgress, boolean isEdge, View parent, View child, int pageSpacing) {
        ...
        if (!this.mIsAlphaMode) {
            this.mTransformationInfo.mIsAlphaDirty = false;
        }
    ...
    private void updateMatrix3D() {
        TransformationInfo info = this.mTransformationInfo;
        if (info.mIsMatrixDirty) {
            android.opengl.Matrix.setIdentityM(info.mMatrix3Ds, 0);
            android.opengl.Matrix.translateM(info.mMatrix3Ds, 0, info.mPivotX + info.mTranslationX, info.mPivotY + info.mTranslationY, info.mPivotZ + info.mTranslationZ);
            if (info.mRotation != 0.0f) {
                android.opengl.Matrix.rotateM(info.mMatrix3Ds, 0, info.mRotation, 0.0f, 0.0f, 1.0f);
            } else if (info.mRotationX != 0.0f) {
                android.opengl.Matrix.rotateM(info.mMatrix3Ds, 0, info.mRotationX, 1.0f, 0.0f, 0.0f);
            } else if (info.mRotationY != 0.0f) {
                android.opengl.Matrix.rotateM(info.mMatrix3Ds, 0, info.mRotationY, 0.0f, 1.0f, 0.0f);
            }
            android.opengl.Matrix.translateM(info.mMatrix3Ds, 0, -info.mPivotX, -info.mPivotY, -info.mPivotZ);
            android.opengl.Matrix.scaleM(info.mMatrix3Ds, 0, info.mScaleX, info.mScaleY, info.mScaleZ);
        }
    }
    ...
    public void setCameraDistance(float z) {
        this.mCameraZ = z;
        this.mIsSetCameraZ = true;
    }

    public void reset() {
        this.mIsSetCameraZ = false;
    }

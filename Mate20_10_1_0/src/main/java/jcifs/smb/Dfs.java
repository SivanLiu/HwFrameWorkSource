...
public class Dfs {
    ...
    static class CacheEntry {
        ...
        CacheEntry(long ttl) {
            this.expiration = System.currentTimeMillis() + (1000 * (ttl == 0 ? Dfs.TTL : ttl));
    ...
    public SmbTransport getDc(String domain, NtlmPasswordAuthentication auth) throws SmbAuthException {
        ...
        if (!DISABLED) {
            try {
                DfsReferral dr = SmbTransport.getSmbTransport(UniAddress.getByName(domain, true), 0).getDfsReferrals(auth, "\\" + domain, 1);
                if (dr != null) {
                    do {
                        try {
        ...
        if (DISABLED || root.equals("IPC$")) {
            ...
            if (domains != null) {
                ...
                if (roots != null) {
                    ...
                    if (links != null) {
                        dr = (DfsReferral) links.map.get("\\");
                        if (dr != null && now > dr.expiration) {
                            links.map.remove("\\");
                            dr = null;
                        }
                        if (dr == null) {
                            if (trans == null && (trans = getDc(domain, auth)) == null) {
                                dfsReferral = null;
                            } else {
                                dr = getReferral(trans, domain, root, path, auth);
                                if (dr != null) {
            ...
            if (dr == null && path != null) {
                ...
                for (String _key : this.referrals.map.keySet()) {
                    ...
                    boolean match = false;
                    if (_klen == key2.length()) {
                        match = _key.equals(key2);
                    } else if (_klen < key2.length()) {
                        match = _key.regionMatches(0, key2, 0, _klen) && key2.charAt(_klen) == '\\';
                    }
                    if (match) {
    ...
    public synchronized void insert(String path, DfsReferral dr) {
        if (!DISABLED) {

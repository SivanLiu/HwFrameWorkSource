...
abstract class ServerMessageBlock extends Response implements Request, SmbConstants {
    ...
    static int readInt2(byte[] src, int srcIndex) {
        return (src[srcIndex] & 255) + ((src[srcIndex + 1] & 255) << SMB_COM_QUERY_INFORMATION);
    }

    static int readInt4(byte[] src, int srcIndex) {
        return (src[srcIndex] & 255) + ((src[srcIndex + 1] & 255) << SMB_COM_QUERY_INFORMATION) + ((src[srcIndex + 2] & 255) << SMB_COM_CHECK_DIRECTORY) + ((src[srcIndex + 3] & 255) << 24);
    }
    ...
    public String toString() {
        ...
        return new String("command=" + c + ",received=" + this.received + ",errorCode=" + str + ",flags=0x" + Hexdump.toHexString((int) (this.flags & 255), 4) + ",flags2=0x" + Hexdump.toHexString(this.flags2, 4) + ",signSeq=" + this.signSeq + ",tid=" + this.tid + ",pid=" + this.pid + ",uid=" + this.uid + ",mid=" + this.mid + ",wordCount=" + this.wordCount + ",byteCount=" + this.byteCount);
    }
}

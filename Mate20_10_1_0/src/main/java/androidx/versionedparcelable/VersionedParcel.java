...
public abstract class VersionedParcel {
    ...
    public <T extends Parcelable> T readParcelable(T def, int fieldId) {
        return !readField(fieldId) ? def : (T) readParcelable();
    }
    ...
    public <T> void writeList(List<T> val, int fieldId) {
        ...
        if (n > 0) {
            ...
            switch (type) {
                case 1:
                    while (i < n) {
                        writeVersionedParcelable(val.get(i));
                        i++;
                    }
                    return;
                case 2:
    ...
    public <T> void writeArray(T[] val) {
        ...
        if (n > 0) {
            ...
            switch (type) {
                case 1:
                    while (i < n) {
                        writeVersionedParcelable(val[i]);
                        i++;
                    }
                    return;
                case 2:
                    while (i < n) {
                        writeParcelable(val[i]);
                        i++;
                    }
                    return;
                case 3:
                    while (i < n) {
                        writeSerializable(val[i]);
                        i++;
                    }
                    return;
                case 4:
    ...
    private Exception createException(int code, String msg) {
        switch (code) {
            ...
            case DTCPValueDef.ERROR_SOCKET /*{ENCODED_INT: -8}*/:
            default:
    ...
    public <T> T[] readArray(T[] def) {
        ...
        return (T[]) arrayList.toArray(def);
    }

    public <T extends VersionedParcelable> T readVersionedParcelable(T def, int fieldId) {
        return !readField(fieldId) ? def : (T) readVersionedParcelable();
    }

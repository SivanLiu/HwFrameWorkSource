...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent2, NestedScrollingChild2, ScrollingView {
    ...
    public float getBottomFadingEdgeStrength() {
        ...
        int length = getVerticalFadingEdgeLength();
        int span = ((child.getBottom() + ((LayoutParams) child.getLayoutParams()).bottomMargin) - getScrollY()) - (getHeight() - getPaddingBottom());
        if (span < length) {
    ...
    public boolean executeKeyEvent(@NonNull KeyEvent event) {
        ...
        if (!canScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, Cea708CCParser.Const.CODE_C1_CW2);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(Cea708CCParser.Const.CODE_C1_CW2)) {
                return false;
            }
            return true;
    ...
    /* JADX INFO: Multiple debug info for r1v1 int: [D('top' int), D('overScrollMode' int)] */
    /* JADX INFO: Multiple debug info for r2v2 int: [D('canScrollHorizontal' boolean), D('bottom' int)] */
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = Cea708CCParser.Const.CODE_C1_CW2;
        } else if (direction == 1) {
            direction = 33;
        }
